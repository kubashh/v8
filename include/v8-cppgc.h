...
class JSHeapConsistency final {
 public:
   ...
   *   into.
   * \param wrappable_base The value that was written.
   * \param params Parameters that may be used for actual write barrier calls.
   *   Only filled if return value indicates that a write barrier is needed. The
   *   contents of the `params` are an implementation detail.
   * \returns whether a write barrier is needed and which barrier to invoke.
  ...
  GetWriteBarrierType(const v8::Local<v8::Object>& wrapper, int wrapper_index,
                      void* wrappable_base, WriteBarrierParams& params) {
#if V8_ENABLE_CHECKS
    CheckWrapper(wrapper, wrapper_index, wrappable_base);
#endif  // V8_ENABLE_CHECKS
    return cppgc::internal::WriteBarrier::
        GetWriteBarrierTypeForExternalReference(wrappable_base, params);
  }
 ...
 private:
  ...
  static void CheckWrapper(v8::Local<v8::Object>&, int, void*);
