...
struct V8_EXPORT TickSample {
  ...
  void* contexts[kMaxFramesCount] = {
      0};                       // Stack of associated native contexts.
...
class V8_EXPORT CpuProfilingOptions {
 public:
  ...
  MaybeLocal<Context> filter_context() const { return filter_context_; }

 private:
  CpuProfilingMode mode_;
  unsigned max_samples_;
  int sampling_interval_us_;
  MaybeLocal<Context> filter_context_;
};
