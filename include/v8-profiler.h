...
class V8_EXPORT CpuProfilingOptions {
 public:
  ...
  // Proxy struct containing internally exposed members.
  const internal::CpuProfilingOptions* impl() const { return internal_.get(); }

 private:
  CpuProfilingMode mode_;
  unsigned max_samples_;
  int sampling_interval_us_;
  std::shared_ptr<internal::CpuProfilingOptions> internal_;
};
