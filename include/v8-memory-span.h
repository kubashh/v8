...
class V8_EXPORT MemorySpan {
 private:
  ...
  using is_compatible_iterator = std::conjunction<
      ...
      is_array_convertible<std::remove_reference_t<iter_reference_t<It>>, T>>;
 ...
 public:
  ...
  /** Constructors from pointer and count */
  constexpr MemorySpan(nullptr_t, size_t) {}

  template <typename U, typename = std::is_convertible<U*, T*>>
  constexpr MemorySpan(U* data, size_t size) : data_(data), size_(size) {}

  /** Constructor from iterator and count. */
  template <typename Iterator, typename = is_compatible_iterator<Iterator>>
  constexpr MemorySpan(Iterator first,
                       size_t count)  // NOLINT(runtime/explicit)
      : data_(first.operator->()), size_(count) {}

  /** Constructor from two iterators. */
  template <typename Iterator,
            typename = std::conjunction<
                is_compatible_iterator<Iterator>,
                std::negation<std::is_convertible<Iterator, size_t>>>>
  constexpr MemorySpan(Iterator first,
                       Iterator last)  // NOLINT(runtime/explicit)
      : data_(first.operator->()), size_(last - first) {}
  ...
  template <typename U, size_t N, typename = is_array_convertible<U, T>>
  ...
  template <typename U, size_t N, typename = is_array_convertible<const U, T>>
  constexpr MemorySpan(
