...
class LocalBase : public api_internal::DirectHandleBase {
 protected:
  ...
  V8_INLINE static LocalBase<T> FromInternal(
      internal::ValueHelper::InternalType internal) {
    return LocalBase<T>(internal);
  }
};
...
#endif
{
 ...
 private:
  ...
  V8_INLINE static Local<T> FromInternal(
      internal::ValueHelper::InternalType internal) {
    return Local<T>(LocalBase<T>::FromInternal(internal));
  }
