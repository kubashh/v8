...
#define EXTERNAL_POINTER_TAGS(V)                                              \
  ...
  /* All property handler callbacks currently use this tag. In the future, */ \
  /* the V8 heap object will probably reference the struct containing all  */ \
  /* callback pointers through a single external pointer with this tag.    */ \
  V(kInterceptorInfoTag,                     unsandboxed, 0b1000000001111111) \
