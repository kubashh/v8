...
class ValueHelper final {
 public:
  ...
  using RepresentationType = internal::Address;
  static constexpr RepresentationType kEmpty = kTaggedNullAddress;
#else
  using RepresentationType = internal::Address*;
  static constexpr RepresentationType kEmpty = nullptr;
  ...
  template <typename T>
  V8_INLINE static RepresentationType ValueAsRepr(const T* value) {
    return reinterpret_cast<RepresentationType>(value);
  }

  template <typename T>
  V8_INLINE static T* ReprAsValue(RepresentationType repr) {
  ...
  template <typename T>
  V8_INLINE static RepresentationType ValueAsRepr(const T* value) {
    return const_cast<RepresentationType>(
