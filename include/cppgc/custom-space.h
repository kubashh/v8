...
class CustomSpaceBase {
 public:
  ...
  virtual bool NeedsAsanContiguousContainerAnnotations() const = 0;
};
...
class CustomSpace : public CustomSpaceBase {
 public:
   ...
   * the garbage collector accesses potentially unsafe areas in the object.
   *
   * It is up to the embedder to tighten limits after a garbage collection has
   * run.
   *
   * Note that this flag may only specified when building with ASAN support.
   */
  static constexpr bool kNeedsAsanContiguousContainerAnnotations = false;
  ...
  bool NeedsAsanContiguousContainerAnnotations() const final {
    return ConcreteCustomSpace::kNeedsAsanContiguousContainerAnnotations;
  }
};
