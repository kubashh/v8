...
class V8_EXPORT WriteBarrier final {
 public:
  ...
  static V8_INLINE void DijkstraMarkingBarrier(Result& result,
                                               const void* object);
  template <typename LazyHeapCallback>
  static V8_INLINE void DijkstraMarkingBarrierRange(
      Result& result, LazyHeapCallback heap_callback, const void* first_element,
      size_t element_size, size_t number_of_elements,
      TraceCallback trace_callback);
  static V8_INLINE void SteeleMarkingBarrier(Result& result,
                                             const void* object);
#if defined(CPPGC_YOUNG_GENERATION)
  static V8_INLINE void GenerationalBarrier(Result& result, const void* slot);
#endif  // CPPGC_YOUNG_GENERATION
...
#if V8_ENABLE_CHECKS
  static void CheckResult(Type expected_type, Result& result);
...
WriteBarrier::Type WriteBarrier::GetWriteBarrierType(
  ...
  result.value_offset =
      value ? reinterpret_cast<uintptr_t>(value) - result.start : 0;
#if V8_ENABLE_CHECKS
...
// static
void WriteBarrier::DijkstraMarkingBarrier(Result& result, const void* object) {
...
template <typename LazyHeapCallback>
// static
void WriteBarrier::DijkstraMarkingBarrierRange(Result& result,
                                               LazyHeapCallback heap_callback,
                                               const void* first_element,
                                               size_t element_size,
                                               size_t number_of_elements,
                                               TraceCallback trace_callback) {
  CheckResult(Type::kMarking, result);
  DijkstraMarkingBarrierRangeSlow(heap_callback(), first_element, element_size,
                                  number_of_elements, trace_callback);
}

// static
void WriteBarrier::SteeleMarkingBarrier(Result& result, const void* object) {
  CheckResult(Type::kMarking, result);
#if defined(CPPGC_CAGED_HEAP)
  // Caged heap already filters out sentinels.
...
#if defined(CPPGC_YOUNG_GENERATION)
// static
void WriteBarrier::GenerationalBarrier2(Result& result, const void* slot) {
