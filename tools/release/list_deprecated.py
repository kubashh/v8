...
def get_blame(file_path):
  ...
  while True:
    ...
    if RE_GITHASH.match(line):
      ...
      hash = line.split(" ")[0]
      current_blame = {'datetime': 0, 'filename': None, 'content': None , 'hash': hash}
      continue
    match = RE_AUTHOR_TIME.match(line)
    if match:
      current_blame['datetime'] = datetime.fromtimestamp(int(match.groups()[0]))
      continue
...
def filter_and_print(blame_list, macro, options):
  ...
  for linenumber, commit_datetime, commit_hash, content in deprecated:
    ...
    file_position = (f"{options.v8_header}:{linenumber}").rjust(len(options.v8_header) + 5)
    print(f"   {file_position}\t{commit_date}\t{commit_hash[:8]}\t{extract_version(commit_hash)}\t{content}")
...
def parse_options(args):
  ...
  if options.v8_header is None:
    ...
    options.v8_header = str((base_path / 'include' / 'v8.h').relative_to(base_path))
  return options
