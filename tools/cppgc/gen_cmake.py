...
if({cond})
  {' '.join(then_stmts)}
    ...
    def _GenTargetString(target_type, source_sets):
        CMAKE_TARGETS = {
            'lib': ('cppgc', 'add_library', 'Main library', 'Threads::Threads'),
            'sample': ('cppgc_sample', 'add_executable', 'Example', 'cppgc'),
            'tests': ('cppgc_unittests', 'add_executable', 'Unittests',
                      'cppgc gtest gmock')
        }
        target = CMAKE_TARGETS[target_type]
        return f"""
# {target[2]} target.
{target[1]}({target[0]} {' '.join(source_sets)})

{'target_link_libraries(' + target[0] + ' ' + target[3] + ')' if target[3] else ''}

target_include_directories({target[0]} PRIVATE "${{CMAKE_SOURCE_DIR}}"
                                       PRIVATE "${{CMAKE_SOURCE_DIR}}/include")

if(CPPGC_ENABLE_OBJECT_NAMES)
  target_compile_definitions({target[0]} PRIVATE "-DCPPGC_SUPPORTS_OBJECT_NAMES")
endif()
if(CPPGC_ENABLE_CAGED_HEAP)
  target_compile_definitions({target[0]} PRIVATE "-DCPPGC_CAGED_HEAP")
endif()
if(CPPGC_ENABLE_YOUNG_GENERATION)
target_compile_definitions({target[0]} PRIVATE "-DCPPGC_YOUNG_GENERATION")
endif()"""
