...
export abstract class MovableView<GraphType extends Graph | TurboshaftGraph> extends PhaseView {
  ...
  protected viewSelection(visibleNodes: d3.Selection<any, Node<any>, any, any>, extentHeight: boolean): void {
    let minX;
    let maxX;
    let minY;
    let maxY;
    let hasSelection = false;
    visibleNodes.selectAll<SVGGElement, Node<any>>("g").each((node: Node<any>) => {
      if (this.state.selection.isSelected(node)) {
        ...
        maxY = maxY
          ? Math.max(maxY, node.y + node.getHeight(extentHeight))
          : node.y + node.getHeight(extentHeight);
      }
    });
    if (hasSelection) {
  ...
  protected searchNodes(filterFunction: (node: TurboshaftGraphNode | GraphNode) => boolean | RegExpExecArray,
                        e: KeyboardEvent, onlyVisible: boolean): Array<TurboshaftGraphNode | GraphNode> {
...
export class MovableViewState {
  ...
  public get turboshaftLayoutType(): TurboshaftLayoutType {
