...
export abstract class MovableView<GraphType extends Graph | TurboshaftGraph> extends PhaseView {
  ...
  protected viewSelection(visibleNodes: d3.Selection<any, Node<any>, any, any>,
                          extendHeight: boolean): void {
    let minX;
    let maxX;
    let minY;
    let maxY;
    ...
    visibleNodes.selectAll<SVGGElement, Node<any>>("g").each((node: Node<any>) => {
      if (this.state.selection.isSelected(node)) {
        ...
        maxY = maxY
          ? Math.max(maxY, node.y + node.getHeight(extendHeight))
          : node.y + node.getHeight(extendHeight);
      }
    });
    if (hasSelection) {
