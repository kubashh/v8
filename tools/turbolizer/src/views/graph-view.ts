...
export class GraphView extends PhaseView {
  ...
  constructor(idOrContainer: string | HTMLElement, broker: SelectionBroker,
    ...
    view.state.selection = new MySelection(n => n.identifier(), n => n?.origin?.identifier());
  ...
  svgKeyDown() {
    ...
    const showSelectionFrontierNodes = (inEdges: boolean, filter: (e: GraphEdge, i: number) => boolean, doSelect: boolean) => {
  ...
  updateGraphVisibility() {
    ...
    const selEdges = view.visibleEdges.selectAll<SVGPathElement, GraphEdge>("path").data(filteredEdges, e => e.toString());
    ...
    newGs.classed("turbonode", function (n) { return true; })
      ...
      .on('mouseenter', function (node) {
        ...
        visibleNodes.data<GraphNode>(adjInputNodes, n => n.toString()).attr('relToHover', "input");
        const adjOutputNodes = adjOutputEdges.data().map(e => e.target);
        visibleNodes.data<GraphNode>(adjOutputNodes, n => n.toString()).attr('relToHover', "output");
        view.updateGraphVisibility();
      })
