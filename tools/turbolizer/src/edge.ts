...
export abstract class Edge<NodeType extends GraphNode | TurboshaftGraphNode
  ...
  public generatePath(graph: Graph | TurboshaftGraph, extendHeight: boolean, layoutType?: LayoutType): string {
    const target = this.target;
    const source = this.source;
    ...
    const outputY = source.y + source.getHeight(extendHeight, layoutType) + C.DEFAULT_NODE_BUBBLE_RADIUS;
