  ...
  constructor(parentId, broker: SelectionBroker) {
    ...
    const ADDRESS_STYLE = {
      associateData: (text, fragment: HTMLElement) => {
        ...
        const blockId = view.sourceResolver.getBlockIdsForOffset(offset);
        ...
        if (blockId && blockId.length > 0) {
          view.addHtmlElementForBlockId(blockId[0], fragment);
          blockIdElement.innerText = `B${blockId.join(",")}:`;
          blockIdElement.dataset.blockId = `${blockId[0]}`;
        }
    ...
    const OPCODE_ARGS = {
      associateData: function (text, fragment) {
        ...
        const replacer = (match, hexOffset) => {
          ...
          return `<span class="tag linkable-text" data-pc-offset="${keyOffset}" data-block-id="${blockIds[0]}">${block}${match}</span>`;
        };
