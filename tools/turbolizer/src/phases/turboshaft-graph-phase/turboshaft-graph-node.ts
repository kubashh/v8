...
export class TurboshaftGraphNode extends Node<TurboshaftGraphEdge<TurboshaftGraphNode>> {
  ...
  public getHeight(showProperties: boolean, layoutType: LayoutType): number {
    if (this.properties && showProperties && layoutType != LayoutType.Boxes) {
  ...
  public getWidth(): number {
    return Math.max(this.inputs.length * C.NODE_INPUT_WIDTH, this.labelBox.width) + 2 * C.TURBOSHAFT_NODE_CONTENT_INDENT;
  }
