        ...
        if (block.visible) {
          ...
          if (this.graph.graphPhase.layoutType == LayoutType.Boxes) {
            for (let i = 0; i < block.nodes.length; i++) {
              block.nodes[i].x = block.x + C.TURBOSHAFT_NODE_X_INDENT;
              block.nodes[i].y = block.y + block.labelBox.height +
                (i * block.nodes[i].labelBox.height) + (i * C.TURBOSHAFT_NODE_SEPARATION);
            }
          }
