...
class MjsunitNamesProvider {
 public:
  ...
  MjsunitNamesProvider(const WasmModule* module, ModuleWireBytes wire_bytes)
    ...
    // - and the name doesn't clash with auto-generated names.
  ...
  void PrintTypeIndex(StringBuilder& out, uint32_t index) {
    if (module_->has_struct(index)) {
      ...
      out << index;  // Support building invalid modules for testing.
    }
  }
...
#define ABSTRACT_TYPE_LIST(V)         \
  ...
  V(None, NullRef)                    \
  V(NoExn, NullExnRef)                \
  V(NoExtern, NullExternRef)          \
  V(NoFunc, NullFuncRef)              \
  V(String, StringRef)                \
  V(StringViewWtf16, StringViewWtf16) \
  V(StringViewWtf8, StringViewWtf8)   \
  V(StringViewIter, StringViewIter)   \
  V(Struct, StructRef)
...
#define CASE(Cpp, _) case HeapType::k##Cpp:
          ABSTRACT_TYPE_LIST(CASE)
#undef CASE
...
void MjsunitFunctionDis::WriteMjsunit(MultiLineStringBuilder& out) {
  ...
  while (pc_ < end_ && ok()) {
    ...
    if (WasmOpcodes::IsPrefixOpcode(opcode)) {
      ...
      if (increase_indentation) indentation_.increase();
    }
...
void PrintF32Const(StringBuilder& out, ImmF32Immediate& imm) {
  ...
  char buffer[100];
  const char* str = DoubleToCString(imm.value, base::VectorOf(buffer, 100u));
...
class MjsunitModuleDis {
 public:
  ...
  void PrintModule() {
    ...
    bool compiles = true;  // TODO: compute properly.
    if (compiles) {
