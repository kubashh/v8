...
class Script {
  ...
  getAccumulatedTimeMetrics(metrics, start, end, delta) {
    ...
    // [ [0, 300, ...], [1, 300, ...], [2, 400, ...], [3, 400, ...] ... ]

    let result = new Array(kSteps);
    // Pre create the metrics array.
    for (let t = 0; t < result.length; t++) {
      result[t] = new Array(metrics.length + 1);
      result[t].fill(0);
      // Fill in the time value.
      result[t][0] = start + t * delta;
    }
    // Create the actual metric's property name on the Funktion object.
...
class Funktion {
  ...
  getFirstEvent() {
    if (this.firstEventTimestamp == -1) {
      ...
      if (!(this.firstEventTimestamp > 0)) throw "firstEventTimestamp < 0";
    }
  ...
  getFirstParseEvent() {
    if (this.firstParseEventTimestamp == -1) {
      ...
      if (!(this.firstParseEventTimestamp > 0)) throw "firstParseEventTimestamp < 0";
    }
  ...
  getLastParseEvent() {
    if (this.lastParseTimestamp == -1) {
      ...
      if (!(this.lastParseTimestamp > 0)) throw "lastParseTimestamp < 0";
    }
  ...
  addNestedFunktion(child) {
    ...
    if (child == undefined) throw "Nesting non child";
...
function startOf(timestamp, time) {
  let result = timestamp / kTimestampFactor - time;
