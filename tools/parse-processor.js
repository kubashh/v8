  ...
  getAccumulatedTimeMetrics(metrics, start, end, delta, incremental = false) {
    ...
    this.funktions.forEach(funktion => {
      ...
      for (let i = 1; i < metricProperties.length; i += 2) {
        ...
        row[i] += funktion.getBytes();
        let timeMetricName = metricProperties[i + 1];
        row[i + 1] += funktion[timeMetricName];
      }
    });
...
function ParseProcessor() {
  LogReader.call(this, {
    ...
    'compile': functionEventProcessor(this.processCompile)
  });
...
ParseProcessor.prototype.processCompile = function(file, scriptId,
  startPosition, endPosition, time, timestamp, functionName) {
