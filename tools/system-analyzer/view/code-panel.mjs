...
// Look for patterns like:  movl rbx,[rcx-0x30]
const kRegisterRegexp =
    new RegExp(`${kAvoidBytecodeOps}(${kRegisters.join('|')}|r[0-9]+)`)
...
class AssemblyFormatter {
  ...
  _addLine(line) {
    ...
    let formattedCode = content.innerHTML.split(kRegisterRegexp)
  ...
  _formatRegisterPart(part) {
    if (!kRegisterRegexp.test(part)) return part;
