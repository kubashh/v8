 ...
 class StatsPanel extends HTMLElement {
  ...
  updateGeneralStats() {
    ...
    pairs.forEach(([name, color, filter]) => {
      ...
      row.onclick = (e) => {
        ...
        //TODO(zc) emit show maps event
        let dataModel = Object.create(null);
        dataModel.content = node.maps;
        console.log("updateGeneralStats");
  ...
  updateNamedTransitionsStats() {
    ...
    nameMapPairs.sort((a, b) => b[1].length - a[1].length).forEach(([
      ...
      tableNode.appendChild(row);
      let dataModel = Object.create(null);
      dataModel.content = row.maps;
      console.log("updateNamedTransitionsStats");
      this.dispatchEvent(new CustomEvent(
        'change', {bubbles: true, composed: true, detail: dataModel}));
    });
    this.statsSelect.appendChild(tableNode);
