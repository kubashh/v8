...
defineCustomElement('ic-panel', (templateText) =>
 class ICPanel extends HTMLElement {
  ...
  get entries(){
   ...
   filterEntriesByTime() {
    this.entries =  this.filteredEntries.filter(e => e.time >= this._startTime && e.time <= this._endTime);
  }
 ...
 toggleDetails(node) {
  ...
  handleICTimeFilter(e) {
    ...
    console.assert(this._endTime <= this._filteredEntries[this._filteredEntries.length - 1].time,
