...
export class TimelineView {
  constructor(entries, timeline, timelineCanvas, timelineLegend) {
    this.element = timeline;
    this.canvas = timelineCanvas;
    this.legend = timelineLegend;
    ...
    // ZC this.currentCode = $("timeline-currentCode");
  ...
  onMouseMove(e) {
    if (this.selecting) {
      this.selectionEnd =
        e.clientX - this.canvas.getBoundingClientRect().left;
      //TODO(zc): provide a drawing method
      //this.drawSelection();
    }
  }
  ...
  onMouseUp(e) {
    if (this.selectionStart !== null) {
      ...
      //TODO(zc): provide some data here
  ...
  //TODO(zc) NEW METHOD
  // draws IC event entry on to the timeline
  drawGridLines() {
    const numberIntervals = 100;
    const dx = timeElapsed/numberIntervals;
    const startTime = this.entries[0].time ;
    const endTime = this.entries[this.entries.length - 1].time;
    const timeElapsed = endTime - startTime + 1;
    let time = startTime;
    while (time < endTime) {
  ...
  drawEntries() {
    ...
    const endTime = this.entries[this.entries.length - 1].time;
    const timeElapsed = endTime - startTime + 1;
    //TODO(zc) expend so you can increase bar height as more events fall same place
    let offsetHeight = 70;
    let eventPosX;
    let eventPosY;
    ctx.lineWidth = 1;
    ...
    for (const entry of this.entries) {
      eventPosX = ((entry.time - startTime)/ timeElapsed) * canvasWidth;
      //console.log(entry.time);
      ctx.moveTo(eventPosX, canvasHeight);
      ctx.lineTo(eventPosX, canvasHeight - offsetHeight);
      ctx.fillText("" + entry.time, eventPosX ,12);

    }
    ctx.stroke();
  }

  //TODO(zc) Draw something
  drawSelection() {
    /*
    let ctx = this.canvas.getContext("2d");
