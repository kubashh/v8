...
class TraceFileReader extends HTMLElement {
  ...
  createModelFromChromeTraceFile(contents) {
    ...
    // * {traceEvents:[]}
    // * []
    let mode = (contents[0][0] === '{') ? 'object' : 'array';
    console.log(`Processing log as chrome trace file (${mode}).`);
    ...
    // Terminate JSON.
    const terminatingToken = (mode === 'object') ? ']}' : ']';
    const sanitized_contents = [...contents, terminatingToken].join('');

    const data = Object.create(null);  // Final data container.
    try {
      const raw_data = JSON.parse(sanitized_contents);
      const raw_array_data =
          (mode === 'object') ? raw_data.traceEvents : raw_data;
