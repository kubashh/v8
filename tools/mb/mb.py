...
class MetaBuildWrapper(object):
  ...
  def ParseArgs(self, argv):
    def AddCommonOptions(subp):
      ...
      subp.add_argument(
          '-m',  '--builder-group',
          # TODO(crbug.com/1165806): Remove the 'master' args.
          '--master',
  ...
  def ConfigFromArgs(self):
    if self.args.config:
      if self.args.builder_group or self.args.builder:
        raise MBErr('Can not specific both -c/--config and -m/--builder-group or '
                    '-b/--builder')
    ...
    if not self.args.builder in self.builder_groups[self.args.builder_group]:
      raise MBErr('Builder name "%s"  not found under builder_groups[%s] in "%s"' %
      ...
      if 'args_file' in mixin_vals:
        if vals['args_file']:
            raise MBErr('args_file specified multiple times in mixins '
                        'for %s on %s' % (self.args.builder, self.args.builder_group))
