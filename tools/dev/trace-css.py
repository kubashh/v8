...
def add_payload_to(J, P):
  if isinstance(P, (int, float)):
    assert isinstance(J, dict), f'P={P}, J={J}'
    J['count'] += 1
    J['max'] = max(J['max'], P)
    J['min'] = min(J['max'], P)
    J['sum'] += P
    ...
    assert isinstance(J, list), f'P={P}, J={J}'
    assert len(P) == len(J)
    for j, p in zip(J, P):
      add_payload_to(j, p)
...
def process(workbook, filename, f, args):
  ...
  for lineno, line in enumerate(f, 1):
    if json_error:
      print(f'Line {lineno-1} is probably incomplete.', file=sys.stderr)
      json_error = False
    ...
    try:
      ...
      if reason == 'GC':
        gc_category = 'with stack' \
          if payload['primary']['total'] > 0 else 'without stack'
        J[reason][gc_category] += 1
...
def sanity(S):
  ...
  assert S['secondary']['total'] == S['normal page']['total'] + \
    ...
    S['out of cage']['total'] + S['out of allocated space']['total'], \
    json.dumps(S, indent=2)
  assert S['young from']['total'] == 0, \
    json.dumps(S, indent=2)
  not_goto_visitor = S['out of cage']['total'] + \
    S['out of allocated space']['total'] + \
    S['page not found']['total'] + S['free space']['total'] + \
    S['not in young']['total'] + S['young from']['total'] + \
    S['already marked']['total']
...
def print_to_worksheet(worksheet, J):
  ...
  for category in ['with stack', 'without stack']:
    worksheet.write(row, 0, 'number of GC')
    worksheet.write(row, 1, category)
    worksheet.write(row, 2, J[category])
    row += 1
