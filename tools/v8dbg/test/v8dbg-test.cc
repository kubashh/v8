...
#include "tools/v8dbg/test/debug-callbacks.h"
...
void RunTests() {
  ...
  // test (d8.exe, v8dbg.dll, and v8dbg-test-script.js).
  ...
  std::wstring command_line = std::wstring(L"\"") + this_module_path +
                              L"\\d8.exe\" \"" + this_module_path +
                              L"\\obj\\tools\\v8dbg\\v8dbg-test-script.js\"";
  ...
  LoadExtensionScope extension_loaded(
      p_debug_control, this_module_path + std::wstring(L"\\v8dbg.dll"));
