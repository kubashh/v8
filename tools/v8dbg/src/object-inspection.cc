...
#include "tools/v8dbg/src/object-inspection.h"
...
V8CachedObject::V8CachedObject(IModelObject* p_v8_object_instance) {
  HRESULT hr = p_v8_object_instance->GetLocation(&location_);
  if (FAILED(hr)) return;  // TODO error handling

  hr = p_v8_object_instance->GetContext(&context_);
  if (FAILED(hr)) return;
  ...
  uncompressed_type_name_ =
      is_compressed_ ? kObject : static_cast<const char*>(type_name);
}
...
IFACEMETHODIMP V8CachedObject::GetCachedV8HeapObject(
  ...
  if (!heap_object_initialized_) {
    ...
    HRESULT hr = sp_debug_host_memory->ReadBytes(
        ...
        is_compressed_ ? 4 : sizeof(void*), &bytes_read);
...
IFACEMETHODIMP V8ObjectDataModel::ToDisplayString(
  ...
  WRL::ComPtr<IV8CachedObject> sp_v8_cached_object =
      GetCachedObject(context_object);
  V8HeapObject* p_v8_heap_object;
  sp_v8_cached_object->GetCachedV8HeapObject(&p_v8_heap_object);
...
HRESULT GetModelForBitField(uint64_t address, const uint8_t num_bits,
  ...
  if (bytes_to_read > 8) {
    std::stringstream message;
    message << "Fatal v8dbg error: found bitfield struct of " << bytes_to_read
            << "bytes, which exceeds the supported size of 8 bytes";
...
#define INVALID_ARG                                                           \
  ...
  if (FAILED(UnboxULong64(pp_arguments[0], &tagged_value, /*convert=*/true))) {
    INVALID_ARG;
  }
  _bstr_t type_name;
  if (arg_count >= 2) {
    if (FAILED(UnboxString(pp_arguments[1], type_name.GetAddress()))) {
      INVALID_ARG;
    }
  }
