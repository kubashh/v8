...
#ifndef V8_TOOLS_V8DBG_SRC_OBJECT_INSPECTION_H_
#define V8_TOOLS_V8DBG_SRC_OBJECT_INSPECTION_H_
...
class V8ObjectDataModel
 ...
 public:
  WRL::ComPtr<IV8CachedObject> GetCachedObject(IModelObject* context_object) {
    ...
    HRESULT hr = context_object->GetParentModel(0, &sp_parent_model,
                                                &sp_context_adjuster);

    // See if the cached object is already present
    WRL::ComPtr<IUnknown> sp_context;
    hr = context_object->GetContextForDataModel(sp_parent_model.Get(),
                                                &sp_context);

    WRL::ComPtr<IV8CachedObject> sp_v8_cached_object;

    if (SUCCEEDED(hr)) {
      sp_context.As(&sp_v8_cached_object);
    } else {
      sp_v8_cached_object = WRL::Make<V8CachedObject>(context_object);
      sp_v8_cached_object.As(&sp_context);
      context_object->SetContextForDataModel(sp_parent_model.Get(),
                                             sp_context.Get());
    }
...
#endif  // V8_TOOLS_V8DBG_SRC_OBJECT_INSPECTION_H_
