...
so that we can test v8dbg. (The version of dbgeng.dll in system32, which would
be loaded otherwise, is insufficient.)
Arguments:
...
def CopyDebuggerFile(debug_file):
  ...
  vs_toolchain._CopyRuntimeImpl(target_path, full_path)
