...
branch by comparing the counts of the two destination basic blocks. This
comparison makes a few false yet useful assumptions. That the only time the
basic block is executed is directly following that branch and that there is only
one branch with this particular pair of destinations. These assumptions are
required due to the simplicity of the profiling method only counting basic block
executions, while ignoring control flow.
