...
def ClangTidyRunFull(build_folder, skip_output_filter, checks):
  """
...
def ClangTidyRunAggregate(build_folder, print_files):
  ...
  while True:
    ...
    if res is not None:
      warnings.setdefault(res.group(2), CTWarning(res.group(2))).add_occurrence(res.group(1))
...
def ClangTidyRunDiff(diff_branch):
  ...
  if diff_branch is None:
    diff_branch = subprocess.check_output(['git', 'merge-base', 'HEAD', 'origin/master']).strip()

  git_ps = subprocess.Popen(
    ['git', 'diff', '-U0', diff_branch], stdout=subprocess.PIPE)
  with open(os.devnull, 'w') as DEVNULL:
    ct_ps = subprocess.Popen(
      ['clang-tidy-diff.py', '-p', '.', '-p1'],
      stdin=git_ps.stdout,
      stdout=subprocess.PIPE,
      stderr=DEVNULL)
  git_ps.wait()
...
def ClangTidyRunSingleFile(build_folder, filename_to_check, line_ranges=[]):
  ...
  with open(os.devnull, 'w') as DEVNULL:
    for file_with_relative_path in files_with_relative_path:
      ...
      subprocess.call(['clang-tidy', '-p', '.'] + extra_args + [file_with_relative_path],
          cwd=build_folder,
          stderr=DEVNULL)
...
def GetOptions():
  ...
  full_run_g.add_option(
    ...
    default=False,
    action='store_true')
  full_run_g.add_option('--checks', help='Clang-tidy checks to use.', default=None)
...
def main():
  ...
  if not CheckClangTidy():
    ...
    if options.full:
      ...
      ClangTidyRunFull(options.build_folder, options.no_output_filter, options.checks)
