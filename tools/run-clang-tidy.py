...
def ClangTidyRunSingleFile(build_folder, filename_to_check, line_ranges=[]):
    ...
    compdb = json.load(
        open(os.path.join(build_folder, "compile_commands.json")))
    ...
    with open(os.devnull, "w") as DEVNULL:
        for file_with_relative_path in files_with_relative_path:
            ...
            if line_filter:
                ct_ps = subprocess.call(
                    [
                        "clang-tidy", "-p", ".", "-line-filter=" + line_filter,
                ...
                ct_ps = subprocess.call(
                    ["clang-tidy", "-p", ".", file_with_relative_path],
                    cwd=build_folder,
                    stderr=DEVNULL)
...
def CheckBuildFolderExists(build_folder):
    """
    Checks if the provided build_folder exists.
    """
    return os.path.isdir(build_folder)
...
def GetOptions():
    ...
    result.add_option(
        '-j', help='Set V8 build folder', dest='threads', default=None)
    result.add_option(
        '--gen-compdb',
...
def main():
    ...
    elif not CheckBuildFolderExists(options.build_folder):
