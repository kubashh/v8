...
def GenerateCompileCommands(build_folder):
    ...
    with open(os.path.join(build_folder, "compile_commands.json"), "w") as cc_file:
...
def ClangTidyRunFull(build_folder, skip_output_filter):
    with open(os.devnull, "w") as DEVNULL:
        ct_process = subprocess.Popen(["run-clang-tidy", "-j" + str(THREADS), "-p", "."],
                                      cwd=build_folder,
...
def ClangTidyRunDiff(diff_branch):
    if diff_branch is None:
        git_mb = subprocess.Popen(["git", "merge-base", "HEAD", "origin/master"], stdout=subprocess.PIPE)
        git_mb.wait()
...
def ClangTidyRunSingleFile(build_folder, filename_to_check, line_ranges=[]):
    ...
    compdb = json.load(open(os.path.join(build_folder, "compile_commands.json")))
    ...
    with open(os.devnull, "w") as DEVNULL:
        for file_with_relative_path in files_with_relative_path:
            ...
            if len(line_ranges) != 0:
                ...
                line_filter += "], \"name\":\"" + rm_prefix(file_with_relative_path, "../") + "\"}"
                line_filter += "]"

            if line_filter:
                ct_ps = subprocess.call(["clang-tidy", "-p", ".", "-line-filter=" + line_filter, file_with_relative_path], cwd=build_folder, stderr=DEVNULL)
            else:
                ct_ps = subprocess.call(["clang-tidy", "-p", ".", file_with_relative_path], cwd=build_folder, stderr=DEVNULL)
...
def GetOptions():
    ...
    result.add_option('-j', help='Set V8 build folder', dest='threads', default=4)
    result.add_option('--gen-compdb', help='Generate a compilation database for clang-tidy',
                      default=False, action='store_true')
    result.add_option('--no-output-filter', help='Done use any output filterning',
                      default=False, action='store_true')
    ...
    diff_run_g.add_option('--branch', help='Run clang-tidy on the diff between HEAD and the \
                          merge-base between HEAD and DIFF_BRANCH (origin/master by default).',
    ...
    single_run_g.add_option('--single', help="", default=False, action='store_true')
    single_run_g.add_option('--file', help="File name to check", default=None, dest='file_name')
    single_run_g.add_option('--lines', help="Limit checks to a line range.", default=[], dest='line_ranges')
...
def main():
  ...
  if not CheckClangTidy():
    ...
    print "Could not find compilatin database, please generate with --gen-compdb"
  else:
    ...
    if options.full:
        ...
        if options.file_name is not None:
            ...
            ClangTidyRunSingleFile(options.build_folder, options.file_name, line_ranges)
    else:
