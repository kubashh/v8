...
def KernelLogger(target):
  ...
  try:
    ...
    logging.info('Terminating kernel log reader.')
    kernel_logger.kill()
def RunTestWithKernelLog(
    target, ffx_session, package_paths, package_name,
    package_component_version, package_args, args,
    print_stream=DEFAULT_PRINT_STREAM):
  with KernelLogger(target) as kernel_logger:
    return RunTest(target, ffx_session, package_paths, package_name,
                   package_component_version, package_args, args, kernel_logger, print_stream)
def RunTest(target, ffx_session, package_paths, package_name,
            package_component_version, package_args, args, kernel_logger,
            print_stream=DEFAULT_PRINT_STREAM):
...
def RunTestPackage(target, ffx_session, package_paths, package_name,
  ...
  with InstallTestPackage(target, package_paths) as kernel_logger:
    return RunTest(
        target, ffx_session, package_paths, package_name,
        package_component_version, package_args, args, kernel_logger)