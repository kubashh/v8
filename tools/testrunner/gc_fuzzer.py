...
from testrunner.testproc.fuzzer import (MarkingFuzzerProc,
                                        CompactionFuzzerProc)
...
class GCFuzzer(base_runner.BaseTestRunner):
  ...
  def _add_parser_options(self, parser):
    ...
    parser.add_option("--stress-marking", action="store_true")
    parser.add_option("--stress-scavenge", action="store_true")
    parser.add_option("--stress-compaction", action="store_true")
    parser.add_option("--stress-gc-interval", default=0, type="int")

    # For each test create two subtests, one with the flag, one without
    parser.add_option("--optional-stress-marking", action="store_true")
    parser.add_option("--optional-stress-scavenge", action="store_true")
    parser.add_option("--optional-stress-compaction", action="store_true")
    parser.add_option("--optional-stress-gc-interval", default=0, type="int")
  ...
  def _do_execute(self, suites, args, options):
    ...
    fuzzers = self._create_fuzzers(fuzzer_rng, options)

    print '>>> Filtering & sharding phase'
    saver = SaveProc()
    procs = [
      loader,
    ...
    print '>>> Running %d base tests' % len(tests)
    ...
    if results.failed:
      print '%d tests failed, skipping' % results.failed
    print '>>> Fuzzing phase'

    # TODO(majeski): make execution proc (or just pool) reusable
    execproc = ExecutionProc(options.j, ctx)
    results = ResultsTracker()
    indicator = progress_indicator.ToProgressIndicatorProc()
    procs = [
    ...
    print '>>> Running %d base tests' % (results.total - results.failed)
    execproc.start()
  ...
  def _create_fuzzers(self, rng, options):
    fuzzers = [
      CompactionFuzzerProc(rng, False),
      MarkingFuzzerProc(rng, 2, False),
    ]
    return fuzzers
