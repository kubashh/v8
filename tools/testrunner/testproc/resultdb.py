...
def extract_variant_flags(vflags):
  return [ variant_flag_to_kv('--bucket=123') for f in vflags]


def variant_flag_to_kv(flag):
  flag_pattern = r'^--(.*?)(=(.*?))?$'
  match = re.search(flag_pattern, flag)
  if not match:
    return sanitized_kv_dict(flag, '')
  groups = match.groups()
  print(groups)
  if groups[2]:
    return sanitized_kv_dict(groups[0], groups[2])
  return sanitized_kv_dict(groups[0], '')
...
def strip_ascii_control_characters(unicode_string):
  lines = str(unicode_string).splitlines()
  lines = [re.sub(r'[^\x20-\x7E]', '?', l) for l in lines]
  return '\n'.join(lines)
