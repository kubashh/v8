...
class JsonTestProgressIndicator(ProgressIndicator):
  ...
  def process_results(self, test, results):
    for run, result in enumerate(results):
      ...
      self._buffer_slow_tests(test, result, output)
      ...
      self.results.append(
        self._test_record(test, result, output).update({
  ...
  def _buffer_slow_tests(self, test, result, output):
    self.tests.append(self._test_record(test, result, output).update({
          "marked_slow": test.is_slow,
        }))
    self.tests = heapq.nlargest(
        self.options.slow_tests_cutoff,
        self.tests,
        lambda _res: _res['duration'])

  def _test_record(self, test, result, output):
    return {
        "name": str(test),
