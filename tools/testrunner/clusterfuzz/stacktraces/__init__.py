...
class StackParser:
  ...
  def address_to_integer(self, address):
  ...
  def filter_crash_parameters(self, state):
    ...
    if state.crash_address and state.crash_type.startswith('UNKNOWN'):
      int_crash_address = self.address_to_integer(state.crash_address)
      if self.is_null_dereference(int_crash_address):
    ...
    for line in split_crash_stacktrace:
      ...
      if segv_match:
        ...
        if 'ASSERT' in state.crash_type:
          ...
          int_crash_address = self.address_to_integer(temp_crash_address)
          if (self.is_assert_crash_address(int_crash_address) or
      ...
      self.update_state_on_check_failure(state, line,
                                         SECURITY_CHECK_FAILURE_REGEX,
                                         'Security CHECK failure')
      self.update_state_on_check_failure(state, line,
                                         SECURITY_DCHECK_FAILURE_REGEX,
      ...
      if not state.crash_type:
        ...
        self.update_state_on_check_failure(state, line, GOOGLE_LOG_FATAL_REGEX,
                                           'Fatal error')
        self.update_state_on_check_failure(state, line,
                                           CHROME_CHECK_FAILURE_REGEX,
                                           'CHECK failure')
        self.update_state_on_check_failure(state, line,
                                           GOOGLE_CHECK_FAILURE_REGEX,
                                           'CHECK failure')
...
def fix_win_cdb_crash_type(crash_type):
  ...
  crash_type = strip_from_right(crash_type, '_AVRF')
...
def fix_check_failure_string(failure_string):
  ...
  failure_string = strip_from_left(failure_string, CHECK_FAILURE_PATTERN)
