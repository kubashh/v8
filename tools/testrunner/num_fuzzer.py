...
class NumFuzzer(base_runner.BaseTestRunner):
  ...
  def _do_execute(self, tests, args):
    ...
    fuzzer_rng = random.Random(self.options.fuzzer_random_seed)

    combiner = self._create_combiner(fuzzer_rng)
    results = self._create_result_tracker()
    ...
    procs = [
      ...
      ExpectationProc(),
      combiner,
      self._create_fuzzer(fuzzer_rng),
      sigproc,
    ] + indicators + [
      results,
      self._create_timeout_proc(),
      self._create_rerun_proc(),
      execproc,
    ]
  ...
  def _create_combiner(self, rng):
    if not self.options.combine_tests:
      return None
    return CombinerProc(rng, self.options.combine_min, self.options.combine_max,
                        self.options.tests_count)

  def _create_fuzzer(self, rng):
