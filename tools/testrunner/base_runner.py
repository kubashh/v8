...
class BaseTestRunner(object):
  ...
  def _load_testsuites(self, args, options):
    ...
    for name in names:
      ...
      suite = self._process_and_load_testsuite(suite, options)
      if suite:
  ...
  def _process_and_load_testsuite(self, suite, options):
    """Serves as a placeholder for customization by different TestRunners.

    The TestSuite can be modified in this function to add/remove behaviors like
    NumFuzzer. The result of this method can also be None to convey that this
    particular TestSuite should be discarded."""
    return suite
