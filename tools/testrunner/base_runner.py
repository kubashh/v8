...
class BaseTestRunner(object):
  ...
  def _load_testsuites(self, args, options):
    ...
    for name in names:
      ...
      if self._should_load_testsuite(suite, options):
        suite.load_tests_from_disk(variables)
        suites.append(suite)

    return suites

  def _should_load_testsuite(self, suite, options):
    """A predicate that can be overridden to filter out unwanted TestSuite instances
    (see NumFuzzer)."""
    return True
