...
class BaseTestRunner(object):
  ...
  def execute(self, sys_args=None):
    ...
    try:
      ...
      with command.command_context(self.target_os, self.options.device):
        tests = self._load_testsuite_generators(args)
        self._setup_env()
  ...
  def _parse_args(self, parser, sys_args):
    ...
    return augmented_options(options), args

  def _load_build_config(self):
    for outdir in self._possible_outdirs():
      try:
        self.build_config = _do_load_build_config(outdir, self.options.verbose)
  ...
  def _load_testsuite_generators(self, args):
    names = self._args_to_suite_names(args, self.options.test_root)
  ...
  def _create_shard_proc(self):
    myid, count = self._get_shard_info()
    if count == 1:
      return None
  ...
  def _get_shard_info(self):
    ...
    shard_count = int(
      os.environ.get('GTEST_TOTAL_SHARDS', self.options.shard_count))
    ...
    if shard_run is not None:
      ...
      shard_run = self.options.shard_run

    if self.options.shard_count > 1:
      # Log if a value was passed on the cmd line and it differs from the
      # environment variables.
      if self.options.shard_count != shard_count:  # pragma: no cover
        print("shard_count from cmd line differs from environment variable "
              "GTEST_TOTAL_SHARDS")
      if (self.options.shard_run > 1 and
          self.options.shard_run != shard_run):  # pragma: no cover
  ...
  def _create_result_tracker(self):
    return progress.ResultsTracker(self.options.exit_after_n_failures)

  def _create_timeout_proc(self):
    if not self.options.total_timeout_sec:
      return None
    return TimeoutProc(self.options.total_timeout_sec)

  def _create_signal_proc(self):
    return SignalProc()

  def _create_rerun_proc(self):
    if not self.options.rerun_failures_count:
      return None
    return RerunProc(self.options.rerun_failures_count,
                     self.options.rerun_failures_max)
