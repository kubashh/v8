...
def preprocess(input):
  ...
  def createFunctionReplacement(m):
    ...
    if len(torque_def) < function_and_comment_len:
  ...
  def createClassReplacement(m):
    ...
    if len(torque_def) < class_and_comment_len:
...
def postprocess(output):
  output = re.sub(r'^(\s*)//@eXpOrT\b', r'\1@export', output, flags=re.MULTILINE)
