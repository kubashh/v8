...
function main() {
  ...
  if (program.mutate_corpus) {
    ...
    const testRunner = new runner.AllTestsRunner(
        inputDir, settings.engine, program.no_of_files);
    inputGen = testRunner.randomInputGen();
  }

  for (const [i, inputs] of runner.enumerate(inputGen)) {
