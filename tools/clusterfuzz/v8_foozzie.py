...
def print_difference(
    options, source, source_key, first_config_flags, second_config_flags,
...
def main():
  ...
  def run_d8(d8, config_flags, testcase=options.testcase):
    preamble = PREAMBLE[:]
  ...
  first_config_output = run_d8(
      options.first_d8, first_config_flags, SANITY_CHECKS)
  second_config_output = run_d8(
      options.second_d8, second_config_flags, SANITY_CHECKS)
  difference, _ = suppress.diff(
      first_config_output.stdout, second_config_output.stdout)
  if difference:
    # Special source key for sanity checks so that clusterfuzz dedupes all cases
    # on this in case it's hit.
    sanity_label = 'sanity check failed'
    print_difference(
        options, sanity_label, sanity_label, first_config_flags,
        second_config_flags, first_config_output, second_config_output,
        difference)
    return RETURN_FAIL
