...
The suite json format is expected to be:
{
  ...
  "cmd_prefix": [<command prefix, e.g. wrapper measuring system resources>],
...
from testrunner.object.output import Output
...
class GraphConfig(Node):
  ...
  def __init__(self, suite, parent, arch):
    ...
    self.cmd_prefix = suite.get('cmd_prefix', parent.cmd_prefix)
...
class RunnableConfig(GraphConfig):
  ...
  def PostProcess(self, outputs_iter):
    if self.results_processor:
      def it():
        for i, output in enumerate(output_iter()):
  ...
  def GetCommand(self, cmd_prefix, shell_dir, extra_flags=None):
    ...
    if self.binary.endswith('.py'):
      ...
      cmd_prefix = cmd_prefix + self.cmd_prefix + [sys.executable]
