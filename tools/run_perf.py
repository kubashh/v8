...
class AndroidPlatform(Platform):  # pragma: no cover
  ...
  def _Run(self, runnable, count, secondary=False):
    ...
    if runnable.process_size:
      stdout += 'MaxMemory: Unsupported'
    return Output(stdout=stdout, duration=(time.time() - start))
  ...
  with CustomMachineConfiguration(governor = options.cpu_governor,
    ...
    for path in args:
      ...
      for runnable in FlattenRunnables(root, NodeCB):
        ...
        def IsSuccessOutput(output):
          if output is None:
            return True
          return output.exit_code == 0 and not output.timed_out
        ...
        def Runner():
          ...
          for i in range(0, max(1, total_runs)):
            ...
            while attempts_left:
              ...
              if IsSuccessOutput(output) and IsSuccessOutput(output_secondary):
