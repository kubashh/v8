...
std::unique_ptr<ObjectPropertiesResult> GetHeapObjectProperties(
  ...
  d::Value map_ptr = heap_object.GetMap(memory_accessor);
  ...
  d::Value instance_type = TqMap(static_cast<uintptr_t>(map_ptr.value))
                               .GetInstanceType(memory_accessor);
  if (instance_type.validity != d::MemoryAccessResult::kOk) {
    ...
    if (type_check_result == d::TypeCheckResult::kUsedTypeHint) {
      ...
      instance_type.value = static_cast<uint64_t>(type_from_hint);
    } else {
      // TODO use known maps here. If known map is just a guess (because root
      // pointers weren't provided), then return a synthetic
      // property like we do for weak maps, with the more specific type. Then
      // the caller could presumably ask us again with the type hint we
      // provided. Otherwise, just go ahead and use it to generate properties.
      return std::make_unique<ObjectPropertiesResult>(
          map_ptr.validity == d::MemoryAccessResult::kAddressNotValid
  ...
  return GetHeapObjectProperties(
      address, memory_accessor,
      static_cast<i::InstanceType>(instance_type.value), type_check_result);
}
...
std::unique_ptr<ObjectPropertiesResult> GetObjectPropertiesImpl(
  ...
  if (i::Internals::HasHeapObjectTag(address)) {
    if ((address & i::kHeapObjectTagMask) != i::kWeakHeapObjectTag) {
      return GetHeapObjectProperties(address, memory_accessor, roots,
                                     type_hint);
    }
    // For weak references, construct a response with a single property
    // containing the corresponding strong reference.
    type_check_result = d::TypeCheckResult::kWeakRef;
    type = "v8::internal::HeapObject";
    if (static_cast<uint32_t>(address) == i::kClearedWeakHeapObjectLower32) {
      brief = "cleared weak ref";
    } else {
      i::Address strong = address & ~i::kWeakHeapObjectMask;
      std::stringstream stream;
      stream << "weak ref to " << std::hex << strong;
      props.push_back(std::make_unique<ObjectProperty>(
          "(strong ref)", "v8::internal::HeapObject",
          static_cast<uint64_t>(strong)));
    }
  } else {
    // For smi values, construct a response with a single property representing
    // the untagged value.
    int32_t value = i::PlatformSmiTagging::SmiToInt(address);
    type_check_result = d::TypeCheckResult::kSmi;
    props.push_back(std::make_unique<ObjectProperty>(
        "(untagged value)", "int", static_cast<uint64_t>(value)));
    brief = std::to_string(value);
    type = "v8::internal::Smi";
  }
  return std::make_unique<ObjectPropertiesResult>(type_check_result, brief,
                                                  type, std::move(props));
}
