...
enum class PropertyType {
  // The type_name string is telling the truth.
  kNormal,

  // A 4-byte representation of an 8-byte tagged pointer. When making a
  // subsequent request for data about this object, be sure to include its
  // address as Roots::any_heap_pointer so we can decompress the data.
  kCompressedTaggedObject,
};

struct ObjectProperty {
  const char* name;

  // Statically-determined type, such as from .tq definition. Be sure to also
  // check the PropertyType, which can disambiguate between compressed and
