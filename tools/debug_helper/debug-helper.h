...
struct Value {
  MemoryAccessResult validity;
  uint64_t value;  // Can fit tagged types, raw pointers, and scalar types
};

struct ObjectProperty {
  const char* name;
  const char* type;  // Statically-determined type, such as from .tq definition.
  size_t num_values;
  Value* values;
  PropertyKind kind;
};
