...
class SystemTest(unittest.TestCase):
  ...
  def testFailWithRerunAndJSON(self):
    ...
    with temp_base() as basedir:
      ...
      self.assertEqual(3, len(json_output['slowest_tests']), pretty_json)
      example_duration = json_output['slowest_tests'][0]
      self.assertEqual(
          'sweet/strawberries', example_duration['name'], pretty_json)
      self.assertTrue(example_duration['marked_slow'], pretty_json)
      self.assertEqual(3, len(json_output['results']), pretty_json)
      example_result = json_output['results'][0]
      self.assertEqual(1, example_result['run'], pretty_json)
      # TODO(majeski): Previously we only reported the variant flags here.
      # After recent changes we report all flags, including the file names.
      # This is redundant to the command. Needs investigation.
      self.assertIn('strawberries', example_result['flags'], pretty_json)
      self.assertEqual(['PASS'], example_result['expected'], pretty_json)
      self.assertEqual('FAIL', example_result['result'], pretty_json)
      self.assertEqual('default', example_result['variant'], pretty_json)
...
>>> Running tests for ia32.release"""
  ...
  def testRandomSeedStressWithDefault(self):
    """Test using random-seed-stress feature."""
    with temp_base() as basedir:
      result = run_tests(
          basedir,
          '--mode=Release',
