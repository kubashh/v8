...
>>> Running tests for ia32.release"""
  ...
  def testReport(self):
    with temp_base() as basedir:
      ...
      # TODO(machenbach): This looks like a bug. There should be three. Both
      # apples and cherries are specified to FAIL as well.
      self.assertIn(
          '1 tests are expected to fail that we should fix',
          result.stdout, result)
      self.assertEqual(1, result.returncode, result)
