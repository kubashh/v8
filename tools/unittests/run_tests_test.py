...
class SystemTest(unittest.TestCase):
  ...
  def testFailWithRerunAndJSON(self):
    ...
    with temp_base() as basedir:
      ...
      # Replace duration in actual output as it's non-deterministic.
      for example_duration in json_output['slowest_tests']:
        example_duration['duration'] = 1
      for result in json_output['results']:
        result['duration'] = 1
        # TODO(machenbach): Remove after rebase.
        result['target_name'] = result['target_name'].split('/')[-1]
