...
def make_clang_command_line(plugin, plugin_args, options):
  ...
  return ([
      ...
      f"-I{options.v8_root_dir / 'third_party' / 'icu' / 'source' / 'common'}",
      f"-I{options.v8_root_dir / 'third_party' / 'icu' / 'source' / 'i18n'}",
...
def parse_gn_file(options, for_test):
  if for_test:
    return {
        "all": [options.v8_root_dir / "tools" / "gcmole" / "gcmole-test.cc"]
    }
  result = {}
  gn_files = [
...
def test_run(options):
  ...
  with open(new_file, "w") as f:
    f.write(output)
  log(f"Wrote test-results: {new_file}")
...
def relative_parents(path, level=0):
  return Path(os.path.relpath(path.absolute().parents[level]))
...
def main(args):
  ...
  parser.add_option(
      ...
      f"Default: env['CLANG_DIR'] or '{default_clang_bin_dir}'")
  parser.add_option(
      "--clang-plugins-dir",
      metavar="DIR",
      help="Containing dir for libgcmole.so."
      f"Default: env['CLANG_PLUGINS'] or '{gcmole_dir}'")
...
def verify_and_convert_dirs(parser, options, gcmole_dir, default_clang_bin_dir):
  ...
  if not options.clang_bin_dir:
    ...
    print(f"Using --clang-bin-dir={options.clang_bin_dir}")
  else:
  ...
  if not options.clang_plugins_dir:
    ...
    print(f"Using --clang-plugins-dir={options.clang_plugins_dir}")
  else:
  ...
  if not options.v8_build_dir:
    ...
    options.v8_build_dir = options.v8_root_dir / 'out' / f"{config.name}.release"
    print(f"Using --v8-build-dir={options.v8_build_dir}")
  else:
...
def verify_build_config(parser, options):
  with open(options.v8_build_dir / 'v8_build_config.json') as f:
    config = json.load(f)
  found_cpu = None
  for key in ('v8_target_cpu', 'target_cpu', 'current_cpu'):
    ...
    if found_cpu == options.v8_target_cpu:
      return
  parser.error(
      f"Build dir '{options.v8_build_dir}' config doesn't match request cpu."
      f"{options.v8_target_cpu}: {found_cpu}")
