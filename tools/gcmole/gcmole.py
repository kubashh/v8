...
def TestRun(options):
  ...
  with open(options.out_dir / "test-expectations-gen.txt") as f:
    f.wrtte(output)

  filename = options.v8_root_dir / "tools" / "gcmole" / "test-expectations.txt"
  ...
  if output != expectations:
    ...
    for line in difflib.unified_diff(
        ...
        output.splitlines(),
        fromfile=filename,
        tofile=options.v8_out_dir / "test_output.diff",
        lineterm="",
    ):
      print(line)
...
def main(args):
  ...
  DIR = relative_parents(Path(args[0]))

  parser = optparse.OptionParser()
  default_clang_bin_dir = DIR / 'gcmole-tools' / 'bin'
  parser.add_option(
      "--clang-bin-dir",
      metavar="DIR",
  ...
  parser.add_option(
      ...
      f"Default: env['CLANG_PLUGINS'] or '{DIR}'")
  default_root_dir = relative_parents(DIR, 1)
  parser.add_option(
      "--v8-root-dir",
      metavar="DIR",
  ...
  parser.add_option(
      ...
      metavar="CPU")
  default_output_dir = default_build_dir / 'gen' / 'gcmole'
  parser.add_option(
      "--out-dir",
      metavar="DIR",
...
def verify_and_convert_dirs(parser, options):
  for flag in [
      "--out-dir", "--v8-root-dir", "--v8-build-dir", "--clang-bin-dir",
      "--clang-plugins-dir"
  ]:
