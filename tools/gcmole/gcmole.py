...
def log(format, *args, **kwargs):
  ...
  print(mark * 2, str(format).format(map(str, args)))
...
def make_clang_command_line(plugin, plugin_args, options):
  ...
  return ([
      ...
      "-I{}".format(
          options.v8_root_dir / 'third_party' / 'icu' / 'source' / 'common'),
      "-I{}".format(
          options.v8_root_dir / 'third_party' / 'icu' / 'source' / 'i18n'),
...
def verify_and_convert_dirs(parser, options, gcmole_dir, default_clang_bin_dir):
  ...
  if not options.v8_build_dir:
    ...
    options.v8_build_dir = options.v8_root_dir / 'out' / (
        config.name + ".release")
...
def prepare_gcmole_files(options):
  cmd = [
      "ninja", "-C",
      str(options.v8_build_dir), "v8_gcmole_files", "v8_dump_build_config"
  ]
  log("Preparing files: {}", cmd)
