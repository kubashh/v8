...
let arguments_array;
function fast_sloppy (a) {
  arguments_array = arguments;
}
fast_sloppy.apply(null, smi_array);
start = performance.now();
for (let x = 0; x < kIterations; ++x) {
  Array.prototype.slice.call(arguments_array, x % kArraySize);
}
stop = performance.now();
print("arguments_array: " + (Math.floor((stop - start)*10)/10) + " ms");

delete fast_sloppy[5];
start = performance.now();
for (let x = 0; x < kIterations; ++x) {
  Array.prototype.slice.call(arguments_array, x % kArraySize);
}
stop = performance.now();
print("arguments_array slow: " + (Math.floor((stop - start)*10)/10) + " ms");
