...
def _equivalent_builder_line(description, bucket, name):
    # Compipe only builders should set this equivalent property, but in case of pairs
    ...
    if bucket.startswith("ci"):
        ...
        if cq_base:
            equivalent = cq_base + ("_ng" if name.endswith(" - builder") else "_ng_triggered")
    elif bucket.startswith("try"):
        html += "CI"
        ci_base = description.get("ci_base", None)
        if ci_base:
            equivalent = ci_base + ("" if name.endswith("_triggered") else " - builder")
...
def _configuration_line(description, name):
    ...
    # convention.
    default_kind = "builder" if (name.endswith(" - builder") or name.endswith("_ng")) else "tester"
