...
inline bool IsComputedEntryShortEncodable(unsigned name_index,
                                          unsigned enum_order) {
  typedef ClassBoilerplate::ComputedEntryFlags Flags;
  return Flags::ShortNameIndexBits::is_valid(name_index) &&
         Flags::ShortEnumerationOrderValueBits::is_valid(enum_order);
}

inline int ComputedEntryEncodeShort(ClassBoilerplate::ValueKind value_kind,
                                    unsigned name_index, unsigned enum_order) {
  typedef ClassBoilerplate::ComputedEntryFlags Flags;
  DCHECK(Flags::ShortNameIndexBits::is_valid(name_index));
  DCHECK(Flags::ShortEnumerationOrderValueBits::is_valid(enum_order));
  int flags = Flags::IsShortEncodingBit::encode(true) |
              Flags::ValueKindBits::encode(value_kind) |
              Flags::ShortEnumerationOrderValueBits::encode(enum_order) |
              Flags::ShortNameIndexBits::encode(name_index);
  return flags;
}

inline int ComputedEntryEncodeFull(ClassBoilerplate::ValueKind value_kind,
                                   unsigned name_index) {
  typedef ClassBoilerplate::ComputedEntryFlags Flags;
  DCHECK(Flags::FullNameIndexBits::is_valid(name_index));
  int flags = Flags::IsShortEncodingBit::encode(false) |
              Flags::ValueKindBits::encode(value_kind) |
              Flags::FullNameIndexBits::encode(name_index);
  return flags;
}
...
class ObjectDescriptor {
 public:
  ...
  void AddComputed(ClassBoilerplate::ValueKind value_kind, int name_index) {
    // Reserve enumeration index in the properties dictionary for this computed
    // property.
    DCHECK_EQ(next_property_enumeration_index_,
              properties_dictionary_template_->NextEnumerationIndex());
    ++next_property_enumeration_index_;
    ...
    if (IsComputedEntryShortEncodable(name_index, enum_order)) {
      int flags = ComputedEntryEncodeShort(value_kind, name_index, enum_order);
      computed_properties_->set(current_computed_index_++, Smi::FromInt(flags));

    } else {
      int flags = ComputedEntryEncodeFull(value_kind, name_index);
      computed_properties_->set(current_computed_index_++, Smi::FromInt(flags));
      computed_properties_->set(current_computed_index_++,
