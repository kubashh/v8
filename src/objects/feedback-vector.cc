...
FeedbackNexusImpl<T>::FeedbackNexusImpl(Handle<FeedbackVector> vector,
    ...
    : vector_handle_(vector), slot_(slot), g_(isolate_or_local_heap) {
  if (!vector.is_null()) {
    // If we are using handles, then we read and cache the slot values in
    // handles of their own.
    kind_ = vector->GetKind(slot);
    int slot_count = FeedbackMetadata::GetSlotSize(kind_);
    feedback_value_ = MaybeObjectHandle(Smi::FromInt(0), g_);
    if (slot_count == 2) {
      feedback_extra_value_ = MaybeObjectHandle(Smi::FromInt(0), g_);
    }
    Reload();
  } else {
...
bool FeedbackNexusImpl<T>::ConfigureMegamorphic(IcCheckType property_type) {
  ...
  changed = changed || GetFeedbackExtra() != MaybeObject::FromSmi(extra);
  if (changed) {
...
int FeedbackNexusImpl<T>::GetCallCount() {
  ...
  Object call_count = GetFeedbackExtra()->template cast<Object>();
...
template <class T>
void FeedbackNexusImpl<T>::Reload() {
  // Nothing to do if we don't have handles.
  if (vector_handle_.is_null()) return;

  const int index = vector().GetIndex(slot());
  MaybeObject tmp = vector().get(index);
  if (tmp.IsCleared()) {
    feedback_value_ = MaybeObjectHandle();
  } else {
    feedback_value_ = MaybeObjectHandle(tmp, g_);
  }
  int slot_count = FeedbackMetadata::GetSlotSize(kind_);
  if (slot_count == 1) return;

  const int extra_index = vector().GetIndex(slot()) + 1;
  tmp = vector().get(extra_index);
  if (tmp.IsCleared()) {
    feedback_extra_value_ = MaybeObjectHandle();
  } else {
    feedback_extra_value_ = MaybeObjectHandle(tmp, g_);
  }
}
...
int FeedbackNexusImpl<T>::ExtractMapsAndHandlers(
    ...
    const {
  return ExtractMapsAndHandlers(
      maps_and_handlers, [](Handle<Map> map) { return MaybeHandle<Map>(); });
}
...
int FeedbackNexusImpl<T>::ExtractMapsAndHandlers(
  ...
  if ((feedback->GetHeapObjectIfStrong(&heap_object) &&
    ...
    for (int i = 0; i < array.length(); i += increment) {
      ...
      if (array.Get(i)->GetHeapObjectIfWeak(&heap_object)) {
        ...
        if (!handler->IsCleared()) {
          DCHECK(IC::IsHandler(handler));
          Handle<Map> map(Map::cast(heap_object), g_);
          if (!map_handler(map).ToHandle(&map)) {
            continue;
          }
          maps_and_handlers->push_back(MapAndHandler(map, handle(handler, g_)));
          found++;
        }
      }
    }
    ...
    if (!handler->IsCleared()) {
      DCHECK(IC::IsHandler(handler));
      Handle<Map> map = handle(Map::cast(heap_object), g_);
      if (!map_handler(map).ToHandle(&map)) {
        return 0;
      }
      maps_and_handlers->push_back(MapAndHandler(map, handle(handler, g_)));
...
IcCheckType FeedbackNexusImpl<T>::GetKeyType() const {
  ...
  if (feedback == MaybeObject::FromObject(
    ...
    return static_cast<IcCheckType>(
        Smi::ToInt(GetFeedbackExtra()->template cast<Object>()));
  }
...
MaybeHandle<JSObject> FeedbackNexusImpl<T>::GetConstructorFeedback() const {
  ...
  HeapObject heap_object;
  if (feedback->GetHeapObjectIfWeak(&heap_object)) {
    return handle(JSObject::cast(heap_object), g_);
  }
...
template class FeedbackNexusImpl<Isolate>;
template class FeedbackNexusImpl<LocalHeap>;
}  // namespace internal
}  // namespace v8
