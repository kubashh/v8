...
void FeedbackVector::Init(
    Isolate* isolate, FeedbackVector vector, DisallowGarbageCollection& no_gc,
    int length, Handle<SharedFunctionInfo> shared,
  ...
  MemsetTagged(ObjectSlot(vector.slots_start()),
               ReadOnlyRoots(isolate).undefined_value(), length);

  DCHECK(is_compiled_scope->is_compiled());
  FeedbackMetadata feedback_metadata = shared->feedback_metadata();
  ...
  Symbol uninitialized_sentinel = ReadOnlyRoots(isolate).uninitialized_symbol();
  for (int i = 0; i < slot_count;) {
    FeedbackSlot slot(i);
