...
Handle<FeedbackMetadata> FeedbackMetadata::New(LocalIsolate* isolate,
  ...
  const int feedback_cell_count =
      spec == nullptr ? 0 : spec->feedback_cell_count();
  if (slot_count == 0 && feedback_cell_count == 0) {
  ...
  Handle<FeedbackMetadata> metadata =
      factory->NewFeedbackMetadata(slot_count, feedback_cell_count);
