...
class JSRegExp : public JSObject {
 public:
  ...
  static constexpr int FlagShiftBits(Flag flag) {
    STATIC_ASSERT(FlagCount() == 6);
    // clang-format off
    return flag == kGlobal ? FlagShiftBit::kGlobal
         : flag == kIgnoreCase ? FlagShiftBit::kIgnoreCase
         : flag == kMultiline ? FlagShiftBit::kMultiline
         : flag == kSticky ? FlagShiftBit::kSticky
         : flag == kUnicode ? FlagShiftBit::kUnicode
         : flag == kDotAll ? FlagShiftBit::kDotAll
         : FlagShiftBit::kInvalid;
    // clang-format on
  }
  ...
  STATIC_ASSERT(FlagCount() == 6);
  STATIC_ASSERT(kNone == v8::RegExp::kNone);
  STATIC_ASSERT(kGlobal == v8::RegExp::kGlobal);
  STATIC_ASSERT(kIgnoreCase == v8::RegExp::kIgnoreCase);
  STATIC_ASSERT(kMultiline == v8::RegExp::kMultiline);
  STATIC_ASSERT(kSticky == v8::RegExp::kSticky);
  STATIC_ASSERT(kUnicode == v8::RegExp::kUnicode);
  STATIC_ASSERT(kDotAll == v8::RegExp::kDotAll);
  STATIC_ASSERT(FlagCount() == v8::RegExp::FlagCount());
