...
// static
ContextSlotRepr ContextSlotReprCell::RepresentationOf(Tagged<Object> object) {
  if (IsUndefined(object)) return ContextSlotRepr::kUndefined;
  if (Is<Smi>(object)) return ContextSlotRepr::kSmi;
  if (Is<HeapNumber>(object)) return ContextSlotRepr::kMutableHeapNumber;
  return ContextSlotRepr::kOther;
}

ContextSlotRepr ContextSlotReprCell::repr() const {
  return static_cast<ContextSlotRepr>(raw_repr().value());
}
void ContextSlotReprCell::set_repr(ContextSlotRepr repr) {
