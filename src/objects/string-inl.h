...
class SharedStringAccessGuardIfNeeded {
 public:
  ...
  SharedStringAccessGuardIfNeeded(String str, LocalIsolate* local_isolate) {
    // TOOD(solanes): This DCHECK should go away before submitting.
    DCHECK(IsNeeded(str, local_isolate) == IsNeeded(str));
    if (IsNeeded(str, local_isolate)) {
      Isolate* isolate;
      if (GetIsolateFromHeapObject(str, &isolate)) {
        // TODO(solanes): this DCHECK fails
        DCHECK_EQ(isolate->string_access(), local_isolate->string_access());
        mutex_guard.emplace(isolate->string_access());
      }
    }
    // This should be just mutex_guard.emplace(local_isolate->string_access());
  }
  ...
  static bool IsNeeded(String str, LocalIsolate* local_isolate) {
    // Don't acquire the lock for the main thread, or for read only strings.
    // TODO(solanes): Which one is better for performance: querying the
    // ReadOnlyHeap, or acquiring an uncontested read lock?
    return !local_isolate->heap()->is_main_thread() &&
           !ReadOnlyHeap::Contains(str);
  }
...
uint16_t String::Get(int index, LocalIsolate* isolate) {
  SharedStringAccessGuardIfNeeded(*this, isolate);
