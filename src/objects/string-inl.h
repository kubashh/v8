...
DEF_GETTER(ExternalString, resource_as_address, Address) {
  return ReadExternalPointerField(kResourceOffset);
}

void ExternalString::allocate_external_pointer_entries(Isolate* isolate) {
  WriteField<size_t>(kResourceOffset,
                     isolate->external_pointer_table().allocate());
  if (is_uncached()) return;
  WriteField<size_t>(kResourceDataOffset,
                     isolate->external_pointer_table().allocate());
}

void ExternalString::set_address_as_resource(Isolate* isolate,
                                             Address address) {
  WriteExternalPointerField(kResourceOffset,
                            reinterpret_cast<ExternalPointer_t>(address));
...
uint32_t ExternalString::resource_as_uint32() {
  Address address = ReadField<Address>(kResourceOffset);
...
void ExternalString::set_uint32_as_resource(Isolate* isolate, uint32_t value) {
  ...
  WriteField<Address>(kResourceDataOffset,
                      reinterpret_cast<Address>(kNullAddress));
}

void ExternalString::DisposeResource(Isolate* isolate) {
  const ExternalPointer_t address = ReadExternalPointerField(kResourceOffset);
  v8::String::ExternalStringResourceBase* resource =
      reinterpret_cast<v8::String::ExternalStringResourceBase*>(address);

  // Dispose of the C++ object if it has not already been disposed.
  if (resource != nullptr) {
    resource->Dispose();
    WriteExternalPointerField(
        kResourceOffset, reinterpret_cast<ExternalPointer_t>(kNullAddress));
  }
}

DEF_GETTER(ExternalOneByteString, resource,
           const ExternalOneByteString::Resource*) {
  return reinterpret_cast<Resource*>(ReadExternalPointerField(kResourceOffset));
}
...
void ExternalOneByteString::update_data_cache(Isolate* isolate) {
  ...
  WriteExternalPointerField(
      kResourceDataOffset,
      reinterpret_cast<ExternalPointer_t>(resource()->data()));
}
...
void ExternalOneByteString::set_resource(
    Isolate* isolate, const ExternalOneByteString::Resource* resource) {
  WriteExternalPointerField(kResourceOffset,
                            reinterpret_cast<ExternalPointer_t>(resource));
