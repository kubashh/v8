...
void WaiterQueueNode::RemoveFromAsyncUnlockedWaiterList(WaiterQueueNode* node) {
  node->requester_->async_unlocked_waiter_queue_nodes().remove_if(
      [=](std::unique_ptr<WaiterQueueNode>& n) { return n.get() == node; });
}

void WaiterQueueNode::RemoveFromAsyncLockedWaiterList(WaiterQueueNode* node) {
  node->requester_->async_locked_waiter_queue_nodes().remove_if(
      [=](std::unique_ptr<WaiterQueueNode>& n) { return n.get() == node; });
}

void WaiterQueueNode::MoveToAsyncLockedWaiterList(WaiterQueueNode* node) {
  ...
  locked_list.emplace_back(nullptr);
  locked_list.back().swap(*it);
