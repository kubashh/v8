...
bool InsertOptionsIntoLocale(Isolate* isolate, Handle<JSObject> options,
                             char* icu_locale) {
  CHECK(isolate);
  CHECK(icu_locale);

  const std::map<std::string, std::string> option_to_bcp47_map = {
      {"calendar", "ca"},        {"collation", "co"}, {"currency", "cu"},
      {"hourCycle", "hc"},       {"caseFirst", "kf"}, {"numeric", "kn"},
      {"numberingSystem", "nu"}, {"timeZone", "tz"}};

  for (const auto& option_to_bcp47 : option_to_bcp47_map) {
    UErrorCode status = U_ZERO_ERROR;
    icu::UnicodeString value_unicode;
    if (!ExtractStringSetting(isolate, options, option_to_bcp47.first.c_str(),
                              &value_unicode)) {
      // Skip this key, user didn't specify it in options.
      continue;
    }
    char value_char[ULOC_FULLNAME_CAPACITY];
    value_unicode.extract(value_char, ULOC_FULLNAME_CAPACITY, nullptr, status);

    // Convert bcp47 key and value into legacy ICU format so we can use
    // uloc_setKeywordValue.
    const char* key = uloc_toLegacyKey(option_to_bcp47.second.c_str());
    if (!key) return false;

    // Overwrite existing, or insert new key-value to the locale string.
    const char* value = uloc_toLegacyType(key, value_char);
    if (value) {
...
bool PopulateResolvedLocaleWithUnicodeTags(Isolate* isolate,
  ...
  const std::map<std::string, std::string> bcp47_to_option_map = {
      {"ca", "calendar"},        {"co", "collation"}, {"cu", "currency"},
      {"hc", "hourCycle"},       {"kf", "caseFirst"}, {"kn", "numeric"},
      {"nu", "numberingSystem"}, {"tz", "timeZone"}};
      ...
      if (bcp47_value) {
        ...
        if (iterator != bcp47_to_option_map.end()) {
          if (iterator->second == "numeric") {
            ...
            JSObject::SetProperty(
                resolved,
                factory->NewStringFromAsciiChecked(iterator->second.c_str()),
                factory->ToBoolean(numeric), LanguageMode::kSloppy)
                .Assert();
          } else {
            JSObject::SetProperty(
                resolved,
...
bool Locale::InitializeLocale(Isolate* isolate, Handle<JSObject> locale_holder,
  ...
  if (!PopulateResolvedLocaleWithUnicodeTags(isolate, icu_canonical,
                                             resolved)) {
    return false;
  }
  ...
  Factory* factory = isolate->factory();
  JSObject::SetProperty(resolved, factory->NewStringFromStaticChars("language"),
                        factory->NewStringFromAsciiChecked(icu_language),
                        LanguageMode::kSloppy)
      .Assert();

  if (strlen(icu_script) != 0) {
    JSObject::SetProperty(resolved, factory->NewStringFromStaticChars("script"),
                          factory->NewStringFromAsciiChecked(icu_script),
                          LanguageMode::kSloppy)
        .Assert();
  }

  if (strlen(icu_region) != 0) {
    JSObject::SetProperty(resolved, factory->NewStringFromStaticChars("region"),
                          factory->NewStringFromAsciiChecked(icu_region),
                          LanguageMode::kSloppy)
        .Assert();
  }
  ...
  JSObject::SetProperty(resolved, factory->NewStringFromStaticChars("baseName"),
  ...
  JSObject::SetProperty(
      resolved, factory->NewStringFromStaticChars("canonicalized"),
      factory->NewStringFromAsciiChecked(bcp47_result), LanguageMode::kSloppy)
      .Assert();
