...
bool IntlUtil::RemoveLocaleScriptTag(const std::string& locale,
  ...
  const char* icu_script = icu_locale.getScript();
  if (!icu_script) {
  ...
  *locale_less_script = bcp47_result;
  return true;
}
...
std::set<std::string> IntlUtil::GetAvailableLocales(const IcuService& service) {
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    if (IntlUtil::RemoveLocaleScriptTag(locale, &shortened_locale)) {
