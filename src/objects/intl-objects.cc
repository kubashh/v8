...
void BuildLanguageTagRegexes(Isolate* isolate) {
  ...
  std::string language_tag_regex = std::string(language_tag);

  UErrorCode status = U_ZERO_ERROR;
  icu::RegexMatcher* language_singleton_regex_matcher = new icu::RegexMatcher(
      icu::UnicodeString::fromUTF8(language_singleton_regex), 0, status);
  icu::RegexMatcher* language_tag_regex_matcher = new icu::RegexMatcher(
      icu::UnicodeString::fromUTF8(language_tag_regex), 0, status);
  icu::RegexMatcher* language_variant_regex_matcher = new icu::RegexMatcher(
      icu::UnicodeString::fromUTF8(language_variant_regex), 0, status);
  if (!U_SUCCESS(status)) {
    return;
  }

  isolate->set_language_tag_regexe_matchers(language_singleton_regex_matcher,
                                            language_tag_regex_matcher,
                                            language_variant_regex_matcher);
}

icu::RegexMatcher* GetLanguageSingletonRegexMatcher(Isolate* isolate) {
  icu::RegexMatcher* language_singleton_regex_matcher =
      isolate->language_singleton_regex_matcher();
  if (language_singleton_regex_matcher == nullptr) {
    BuildLanguageTagRegexes(isolate);
    language_singleton_regex_matcher =
        isolate->language_singleton_regex_matcher();
  }
  return language_singleton_regex_matcher;
}

icu::RegexMatcher* GetLanguageTagRegexMatcher(Isolate* isolate) {
  icu::RegexMatcher* language_tag_regex_matcher =
      isolate->language_tag_regex_matcher();
  if (language_tag_regex_matcher == nullptr) {
    BuildLanguageTagRegexes(isolate);
    language_tag_regex_matcher = isolate->language_tag_regex_matcher();
  }
  return language_tag_regex_matcher;
}

icu::RegexMatcher* GetLanguageVariantRegexMatcher(Isolate* isolate) {
  icu::RegexMatcher* language_variant_regex_matcher =
      isolate->language_variant_regex_matcher();
  if (language_variant_regex_matcher == nullptr) {
    BuildLanguageTagRegexes(isolate);
    language_variant_regex_matcher = isolate->language_variant_regex_matcher();
  }
  return language_variant_regex_matcher;
}
