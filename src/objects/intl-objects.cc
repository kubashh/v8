...
bool Locale::InitializeLocale(Isolate* isolate, Handle<JSObject> locale_holder,
  ...
  Factory* factory = isolate->factory();
  JSObject::SetProperty(
      locale_holder, factory->NewStringFromStaticChars("language"),
      factory->NewStringFromAsciiChecked(icu_language), LanguageMode::kSloppy)
      .Assert();

  if (strlen(icu_script) != 0) {
    JSObject::SetProperty(
        locale_holder, factory->NewStringFromStaticChars("script"),
        factory->NewStringFromAsciiChecked(icu_script), LanguageMode::kSloppy)
        .Assert();
  }

  if (strlen(icu_region) != 0) {
    JSObject::SetProperty(
        locale_holder, factory->NewStringFromStaticChars("region"),
        factory->NewStringFromAsciiChecked(icu_region), LanguageMode::kSloppy)
        .Assert();
  }
  ...
  JSObject::SetProperty(
      locale_holder, factory->NewStringFromStaticChars("baseName"),
  ...
  JSObject::SetProperty(
      locale_holder, factory->NewStringFromStaticChars("canonicalized"),
      factory->NewStringFromAsciiChecked(bcp47_result), LanguageMode::kSloppy)
      .Assert();
