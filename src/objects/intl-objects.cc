...
std::string IntlUtil::removeLocaleScriptTag(std::string localeIn) {
  // Build the shortened locale; eg, convert xx-Yyyy-ZZ  to xx-ZZ.
  // Unfortunately, cannot use regex :-( so doing this the hard way.

  if (localeIn.length() != 10 && localeIn.length() != 11) {
    return localeIn;
  }
  char delim = '-';
  std::size_t firstLen = localeIn.find(delim);
  if (firstLen != 2 && firstLen != 3) {
    return localeIn;
  }

  std::size_t secondDelim = localeIn.find(delim, firstLen + 1);
  ...
  std::size_t thirdLen = localeIn.length() - secondDelim - 1;
  if (thirdLen != 2) {
    return localeIn;
  }
  std::string firstPart = localeIn.substr(0, firstLen);
  std::string secondPart = localeIn.substr(firstLen + 1, secondLen);
  std::string thirdPart = localeIn.substr(firstLen + secondLen + 2);

  for (std::size_t i = 0; i < firstLen; i++) {
    if (!std::islower(firstPart[i])) {
      return localeIn;
    }
  }

  if (!std::isupper(secondPart[0])) {
    return localeIn;
  }
  for (std::size_t i = 1; i < secondLen; i++) {
    if (!std::islower(secondPart[i])) {
      return localeIn;
    }
  }

  for (std::size_t i = 0; i < thirdLen; i++) {
    if (!std::isupper(thirdPart[i])) {
      return localeIn;
    }
  }
...
std::set<std::string> IntlUtil::getAvailableLocales(IcuService service) {
