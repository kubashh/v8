...
icu::UnicodeString Intl::ToUnicodeString(Isolate* isolate,
                                         Handle<String> string) {
  DisallowHeapAllocation no_gc;
  string = String::Flatten(isolate, string);
  std::unique_ptr<uc16[]> sap;
  return icu::UnicodeString(
      GetUCharBufferFromFlat(string->GetFlatContent(), &sap, string->length()),
      string->length());
}
...
Handle<Object> Intl::CompareStrings(Isolate* isolate,
  ...
  UErrorCode status = U_ZERO_ERROR;
  icu::UnicodeString string_val1 = Intl::ToUnicodeString(isolate, string1);
  icu::UnicodeString string_val2 = Intl::ToUnicodeString(isolate, string2);
...
Managed<icu::UnicodeString>* Intl::SetTextToBreakIterator(
  ...
  icu::UnicodeString* u_text =
      new icu::UnicodeString(Intl::ToUnicodeString(isolate, text));
