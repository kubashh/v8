...
Maybe<bool> CopyProperty(Isolate* isolate, Handle<JSObject> in,
                         Handle<JSObject> options, const char* key) {
  Factory* factory = isolate->factory();
  ...
  LookupIterator it = LookupIterator::PropertyOrElement(isolate, options, p,
                                                        LookupIterator::OWN);
  return JSReceiver::CreateDataProperty(&it, v, kThrowOnError);
}
...
MaybeHandle<JSObject> NumberFormat::ResolvedOptions(
  ...
  for (const auto& p : kResolvedOptionsOfNumberFormatInstances) {
    MAYBE_RETURN(CopyProperty(isolate, resolved, options, p), options);
  }
