...
bool IntlUtil::RemoveLocaleScriptTag(const std::string& icu_locale,
  ...
  // Build the shortened locale; eg, convert xx_Yyyy_ZZ  to xx-ZZ.
  ...
  const char* icu_base_name = short_locale.getBaseName();
  *locale_less_script = std::string(icu_base_name);
  return true;
}
...
std::set<std::string> IntlUtil::GetAvailableLocales(const IcuService& service) {
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    if (IntlUtil::RemoveLocaleScriptTag(std::string(icu_name),
                                        &shortened_locale)) {
      error = U_ZERO_ERROR;
