...
void BuildLanguageTagRegexes(Isolate* isolate) {
  ...
  isolate->set_language_tag_regexes(language_singleton_regex,
                                    language_tag_regex, language_variant_regex);
}

std::string GetLanguageSingletonRegex(Isolate* isolate) {
  std::string language_singleton_regex = isolate->language_singleton_regex();
  if (language_singleton_regex.empty()) {
    BuildLanguageTagRegexes(isolate);
    language_singleton_regex = isolate->language_singleton_regex();
  }
  return language_singleton_regex;
}

std::string GetLanguageTagRegex(Isolate* isolate) {
  std::string language_tag_regex = isolate->language_tag_regex();
  if (language_tag_regex.empty()) {
    BuildLanguageTagRegexes(isolate);
    language_tag_regex = isolate->language_tag_regex();
  }
  return language_tag_regex;
}

std::string GetLanguageVariantRegex(Isolate* isolate) {
  std::string language_variant_regex = isolate->language_variant_regex();
  if (language_variant_regex.empty()) {
    BuildLanguageTagRegexes(isolate);
    language_variant_regex = isolate->language_variant_regex();
  }
  return language_variant_regex;
}
