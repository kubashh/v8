...
void BuildLanguageTagRegexes(Isolate* isolate) {
  ...
  icu::RegexMatcher* language_singleton_regex_matcher =
      new icu::RegexMatcher(language_singleton_regex.c_str(), 0, status);
  icu::RegexMatcher* language_tag_regex_matcher =
      new icu::RegexMatcher(language_tag_regex.c_str(), 0, status);
  icu::RegexMatcher* language_variant_regex_matcher =
      new icu::RegexMatcher(language_variant_regex.c_str(), 0, status);
  if (!U_SUCCESS(status)) {
    return;
  }
