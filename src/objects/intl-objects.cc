...
bool IsValidCollation(icu::Locale& locale, const std::string& value) {
  UErrorCode status = U_ZERO_ERROR;
  const char* legacy_key = uloc_toLegacyKey("co");
  std::unique_ptr<icu::StringEnumeration> values(
      icu::Collator::getKeywordValuesForLocale(
          legacy_key, icu::Locale(locale.getBaseName()), false, status));
  if (U_SUCCESS(status)) {
    int32_t length;
    std::string input_legacy_type(uloc_toLegacyType(legacy_key, value.c_str()));
    for (const char* type = values->next(&length, status); type != nullptr;
         type = values->next(&length, status)) {
      if (U_SUCCESS(status) && input_legacy_type == type) {
        return true;
      }
    }
  }
