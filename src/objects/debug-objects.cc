...
void DebugInfo::ClearBreakInfo(Isolate* isolate) {
  if (HasInstrumentedBytecodeArray()) {
    ...
    // shared_function_info_access, because the background thread does not read
    // this field while the DebugInfo still holds the original and instrumented
    // bytecode array.
    ...
    shared().UnprepareBytecodeForDebugExecution(isolate);
  }
