...
class V8_NODISCARD WaiterQueueNode {
 public:
  ...
  // Manage the nodes in the isolated locked and unlocked waiter queue nodes
  // lists. Should only be called with AsyncWaiterQueueNodes as arguments.
  static void RemoveFromAsyncUnlockedWaiterList(WaiterQueueNode* node);
  static void RemoveFromAsyncLockedWaiterList(WaiterQueueNode* node);
  static void MoveToAsyncLockedWaiterList(WaiterQueueNode* node);

  // Async cleanup functions.
  virtual bool IsSameNativeContextForAsyncCleanup(Tagged<Context> context) = 0;
