...
#include "src/init/vm-cage.h"  // TODO(saelo) needed?
...
inline void* EmptyBackingStoreBuffer() {
  ...
  // When the sandbox is enabled, backing store buffers are referenced from V8
  // heap objects through caged pointers, which cannot represent nullptr. So
  // instead of nullptr, we use a special empty backing store buffer, located at
  // the end of the cage.
  return reinterpret_cast<void*>(GetProcessWideVirtualMemoryCage()->base() +
                                 kVirtualMemoryCageSize - 1);
#else
...
class V8_EXPORT_PRIVATE BackingStore : public BackingStoreBase {
 public:
  ...
  bool IsEmpty() const { return buffer_start_ == EmptyBackingStoreBuffer(); }
