...
static const PatternItem kPatternItems[] = {
...
void SetPropertyFromPattern(Isolate* isolate, const std::string& pattern,
  ...
  for (const PatternItem* item = kPatternItems;
       item != nullptr && item->property != nullptr; ++item) {
    for (const PatternMap* pair = item->pairs;
         pair != nullptr && pair->pattern != nullptr; ++pair) {
      if (pattern.find(pair->pattern) != std::string::npos) {
        // Afer we find the first pair in the item which matching the pattern,
  ...
  if (canonical_time_zone == UNICODE_STRING_SIMPLE("Etc/UTC") ||
    ...
    ASSIGN_RETURN_ON_EXCEPTION(
        ...
        factory->NewStringFromTwoByte(Vector<const uint16_t>(
            reinterpret_cast<const uint16_t*>(canonical_time_zone.getBuffer()),
