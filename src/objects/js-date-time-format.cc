...
void CreateDataPropertyForOptions(Isolate* isolate, Handle<JSObject> options,
                                  Handle<String> key, Handle<Object> value) {
...
void CreateDataPropertyForOptions(Isolate* isolate, Handle<JSObject> options,
  ...
  CreateDataPropertyForOptions(isolate, options,
                               factory->NewStringFromAsciiChecked(key),
                               factory->NewStringFromAsciiChecked(value));
}
...
void CreateDataPropertyForOptions(Isolate* isolate, Handle<JSObject> options,
  ...
  Factory* factory = isolate->factory();
  CreateDataPropertyForOptions(isolate, options,
                               factory->NewStringFromAsciiChecked(key), value);
}
...
const PatternItem patterm_items[] = {
...
MaybeHandle<JSObject> JSDateTimeFormat::ResolvedOptions(
  ...
  if (resolved_obj->IsJSReceiver()) {
    ...
    if (numbering_system_obj->IsString()) {
      ...
      CreateDataPropertyForOptions(isolate, options, numberingSystem_string,
                                   numbering_system);
    }
  }
  ...
  for (auto item = patterm_items; item != nullptr && item->property != nullptr;
    ...
    for (const PatternMap* pair = item->pairs;
      ...
      if (pattern.find(pair->pattern) != std::string::npos) {
        CreateDataPropertyForOptions(isolate, options, item->property,
                                     pair->value);
        break;
      }
    }
  ...
  if (pattern.find("h") != std::string::npos) {
    CreateDataPropertyForOptions(isolate, options, "hour12", true);
  } else if (pattern.find("H") != std::string::npos) {
    CreateDataPropertyForOptions(isolate, options, "hour12", false);
  }
  // calendar
  ...
  CreateDataPropertyForOptions(isolate, options, "calendar",
                               calendar_str.c_str());
