...
Intl::HourCycle get_hour_cycle(const char* str) {
  if (strlen(str) == 3) {
    switch (str[2]) {
      case '1':
        if (strcmp(str, "h11") == 0) return Intl::HourCycle::kH11;
        break;
      case '2':
        if (strcmp(str, "h12") == 0) return Intl::HourCycle::kH12;
        break;
      case '3':
        if (strcmp(str, "h23") == 0) return Intl::HourCycle::kH23;
        break;
      case '4':
        if (strcmp(str, "h24") == 0) return Intl::HourCycle::kH24;
        break;
    }
  }
  UNREACHABLE();
...
MaybeHandle<JSDateTimeFormat> JSDateTimeFormat::Initialize(
  ...
  if (hour_cycle == Intl::HourCycle::kUndefined) {
    if (r.extensions.find("hc") != r.extensions.end()) {
      hour_cycle = get_hour_cycle(r.extensions.at("hc").c_str());
    }
  }
