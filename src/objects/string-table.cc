...
Handle<String> StringTable::LookupString(Isolate* isolate,
  ...
  //
  //  - String::Flatten is not threadsafe and but is only called on non-shared
  //    strings, since non-flat strings are not shared.
  //
  //  - String::MakeThin is not threadsafe but is only called on non-shared
  //    strings, since all shared strings are in-place internalizable.
  //
  ...
  //
  //  - In-place internalizable strings' do not incur a copy regardless of
  //    string table sharing. The map mutation is threadsafe even with relaxed
  //    memory order, because for concurrent table lookups, the "losing" thread
  //    will be correctly ordered by LookupKey's write mutex and see the updated
  //    map during the re-lookup. Further, the internalized string map is the
  //    same map in RO space regardless of which thread is doing the lookup.
