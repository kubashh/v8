...
static constexpr int kStringTableMinShrinkCapacity = kStringTableMinCapacity;
...
int ComputeStringTableCapacityWithShrink(int current_capacity,
  ...
  if (current_capacity <= kStringTableMinCapacity) return current_capacity;
  ...
  if (new_capacity < kStringTableMinShrinkCapacity) return current_capacity;
  return new_capacity;
}
...
// by the elements themselves. These are accessessed as offsets from the
// elements_ field, which itself provides storage for the first element.
...
void* StringTable::Data::operator new(size_t size, int capacity) {
  ...
  return malloc(size + (capacity - 1) * sizeof(Object));
}

void StringTable::Data::operator delete(void* table) { free(table); }
  ...
  while (true) {
...

    {
      ...
      int capacity_after_shrinking = ComputeStringTableCapacityWithShrink(
          current_capacity, current_nof + 1);
...
void StringTable::Print() { data_->Print(); }
