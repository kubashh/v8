...
inline Address ExternalPointerArray::get(int index, Isolate* isolate) {
  return ReadExternalPointerField<kWasmIndirectFunctionTargetTag>(
      OffsetOfElementAt(index), isolate);
}

inline void ExternalPointerArray::set(int index, Isolate* isolate,
                                      Address value) {
  WriteLazilyInitializedExternalPointerField<kWasmIndirectFunctionTargetTag>(
      OffsetOfElementAt(index), isolate, value);
}
