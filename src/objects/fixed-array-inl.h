...
int TaggedArrayBase<Derived, Shape>::capacity() const {
  return Smi::ToInt(TaggedField<Smi, Derived::kCapacityOffset>::load(*this));
}

template <class Derived, class Shape>
int TaggedArrayBase<Derived, Shape>::capacity(AcquireLoadTag tag) const {
  return Smi::ToInt(
      TaggedField<Smi, Derived::kCapacityOffset>::Acquire_Load(*this));
}

template <class Derived, class Shape>
void TaggedArrayBase<Derived, Shape>::set_capacity(int value) {
  TaggedField<Smi, Derived::kCapacityOffset>::store(*this, Smi::FromInt(value));
}

template <class Derived, class Shape>
void TaggedArrayBase<Derived, Shape>::set_capacity(int value,
                                                   ReleaseStoreTag tag) {
  TaggedField<Smi, Derived::kCapacityOffset>::Release_Store(
      *this, Smi::FromInt(value));
}

int FixedArray::length() const { return capacity(); }

int FixedArray::length(AcquireLoadTag tag) const { return capacity(tag); }

void FixedArray::set_length(int value) { set_capacity(value); }

void FixedArray::set_length(int value, ReleaseStoreTag tag) {
...
int FixedDoubleArray::length() const { return capacity(); }

int FixedDoubleArray::length(AcquireLoadTag tag) const { return capacity(tag); }

void FixedDoubleArray::set_length(int value) { set_capacity(value); }

void FixedDoubleArray::set_length(int value, ReleaseStoreTag tag) {
  set_capacity(value, tag);
}
...
typename S::ElementT* PrimitiveArrayBase<D, S>::end() const {
  return AddressOfElementAt(capacity());
}

template <class D, class S>
int PrimitiveArrayBase<D, S>::DataSize() const {
  int data_size = SizeFor(capacity()) - S::kHeaderSize;
  DCHECK_EQ(data_size, OBJECT_POINTER_ALIGN(capacity() * S::kElementSize));
  return data_size;
}
...
// static
template <class IsolateT>
Handle<FixedArrayBase> FixedDoubleArray::New(IsolateT* isolate, int capacity,
                                             AllocationType allocation) {
  if (V8_UNLIKELY(static_cast<unsigned>(capacity) > kMaxCapacity)) {
    FATAL("Fatal JavaScript invalid size error %d (see crbug.com/1201626)",
          capacity);
  } else if (V8_UNLIKELY(capacity == 0)) {
    return isolate->factory()->empty_fixed_array();
...
// static
template <class IsolateT>
Handle<ByteArray> ByteArray::New(IsolateT* isolate, int capacity,
                                 AllocationType allocation) {
  if (V8_UNLIKELY(static_cast<unsigned>(capacity) > kMaxCapacity)) {
  ...
  Handle<ByteArray> result =
      Handle<ByteArray>::cast(Allocate(isolate, capacity, &no_gc, allocation));

  int padding_size = SizeFor(capacity) - OffsetOfElementAt(capacity);
  memset(result->AddressOfElementAt(capacity), 0, padding_size);
...
uint32_t ByteArray::get_int(int offset) const {
  ...
  DCHECK_LE(offset + sizeof(uint32_t), capacity());
...
FixedIntegerArray<T>::FixedIntegerArray(Address ptr) : ByteArray(ptr) {
  DCHECK_EQ(ByteArray::capacity() % sizeof(T), 0);
}
...
int FixedIntegerArray<T>::length() const {
  DCHECK_EQ(ByteArray::capacity() % sizeof(T), 0);
  return ByteArray::capacity() / sizeof(T);
}
