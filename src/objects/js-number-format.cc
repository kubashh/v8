...
Maybe<icu::Formattable> IntlMathematicalValue::ToFormattable(Isolate* isolate) {
  ...
  DCHECK(!obj->IsNaN());
  return Just(icu::Formattable(value_->Number()));
}
  ...
  if (numeric_obj->IsBigInt()) {
    ...
    *formatted = FormatDecimalString(isolate, number_format, string, status);
    ...
    if (FLAG_harmony_intl_number_format_v3 && numeric_obj->IsString()) {
      Handle<String> string = Handle<String>::cast(numeric_obj);
      *formatted = FormatDecimalString(isolate, number_format, string, status);
    } else {
...
MaybeHandle<T> PartitionNumberRangePattern(Isolate* isolate,
  ...
  if (x.IsNaN()) {
    THROW_NEW_ERROR_RETURN_VALUE(
        isolate,
        NewRangeError(MessageTemplate::kInvalid,
                      factory->NewStringFromStaticChars("start"), x_obj),
        MaybeHandle<T>());
  }
  if (y.IsNaN()) {
    THROW_NEW_ERROR_RETURN_VALUE(
        isolate,
        NewRangeError(MessageTemplate::kInvalid,
                      factory->NewStringFromStaticChars("end"), y_obj),
        MaybeHandle<T>());
  }
  ...
  if (x.IsMathematicalValue()) {
    ...
    if (y.IsMathematicalValue() && y.IsLessThan(isolate, x)) {
      THROW_NEW_ERROR_RETURN_VALUE(
          isolate, NewRangeError(MessageTemplate::kInvalid, x_obj, y_obj),
          MaybeHandle<T>());
    }
