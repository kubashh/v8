...
Handle<JSObject> JSNumberFormat::ResolvedOptions(
  ...
  /*
  JSNumberFormat::Usage usage = number_format->usage();
  CreateDataPropertyForOptions(isolate, options,
                               isolate->factory()->usage_string(),
                               JSNumberFormat::UsageToString(usage));

  icu::NumberFormat* icu_number_format =
  number_format->icu_number_format()->raw(); CHECK_NOT_NULL(icu_number_format);

  UErrorCode status = U_ZERO_ERROR;
  bool numeric =
      icu_number_format->getAttribute(UCOL_NUMERIC_COLLATION, status) ==
  UCOL_ON; CHECK(U_SUCCESS(status)); CreateDataPropertyForOptions(isolate,
  options, isolate->factory()->numeric_string(), numeric);

  const char* case_first = nullptr;
  status = U_ZERO_ERROR;
  switch (icu_number_format->getAttribute(UCOL_CASE_FIRST, status)) {
  ...
  status = U_ZERO_ERROR;
  switch (icu_number_format->getAttribute(UCOL_STRENGTH, status)) {
    case UCOL_PRIMARY: {
      CHECK(U_SUCCESS(status));
      status = U_ZERO_ERROR;
      // case level: true + s1 -> case, s1 -> base.
      if (UCOL_ON == icu_number_format->getAttribute(UCOL_CASE_LEVEL, status)) {
        sensitivity = "case";
      } else {
        sensitivity = "base";
      }
  ...
  icu_number_format->getAttribute(UCOL_ALTERNATE_HANDLING, status) ==
  UCOL_SHIFTED; CHECK(U_SUCCESS(status)); CreateDataPropertyForOptions(isolate,
  options, isolate->factory()->ignorePunctuation_string(), ignore_punctuation);

  status = U_ZERO_ERROR;
  const char* collation;
  std::unique_ptr<icu::StringEnumeration> collation_values(
      icu_number_format->getKeywordValues("co", status));
  ...
  status = U_ZERO_ERROR;
  icu::Locale icu_locale = icu_number_format->getLocale(ULOC_VALID_LOCALE,
  status); CHECK(U_SUCCESS(status));
...
/*
void SetCaseFirstOption(icu::NumberFormat* icu_number_format, const char* value)
{ CHECK_NOT_NULL(icu_number_format); UErrorCode status = U_ZERO_ERROR; if
(strncmp(value, "upper", 5) == 0) {
    icu_number_format->setAttribute(UCOL_CASE_FIRST, UCOL_UPPER_FIRST, status);
  } else if (strncmp(value, "lower", 5) == 0) {
    icu_number_format->setAttribute(UCOL_CASE_FIRST, UCOL_LOWER_FIRST, status);
  } else {
    icu_number_format->setAttribute(UCOL_CASE_FIRST, UCOL_OFF, status);
  }
  CHECK(U_SUCCESS(status));
}
*/
...
MaybeHandle<JSNumberFormat> JSNumberFormat::InitializeNumberFormat(
  ...
  JSNumberFormat::Usage usage = JSNumberFormat::Usage::SORT;
  Maybe<bool> found_usage = Intl::GetStringOption(
      isolate, options, "usage", values, "Intl.NumberFormat", &usage_str);
  MAYBE_RETURN(found_usage, MaybeHandle<JSNumberFormat>());

  if (found_usage.FromJust()) {
    DCHECK_NOT_NULL(usage_str.get());
    if (strncmp(usage_str.get(), "search", 6) == 0) {
      usage = JSNumberFormat::Usage::SEARCH;
    }
  }
  */

  // 5. Set number_format.[[Usage]] to usage.
  // number_format->set_usage(usage);

  // 6. If usage is "sort", then
  //    a. Let localeData be %NumberFormat%.[[SortLocaleData]].
  // 7. Else,
  //    a. Let localeData be %NumberFormat%.[[SearchLocaleData]].
  //
  ...
  Maybe<bool> found_numeric = Intl::GetBoolOption(isolate, options, "numeric",
                                                  "Intl.NumberFormat",
  &numeric); MAYBE_RETURN(found_numeric, MaybeHandle<JSNumberFormat>());
  ...
  */
  ...
  Maybe<bool> found_case_first = Intl::GetStringOption(
      isolate, options, "caseFirst", values, "Intl.NumberFormat",
  &case_first_str); MAYBE_RETURN(found_case_first,
  MaybeHandle<JSNumberFormat>());
  */

  // The relevant unicode extensions accepted by NumberFormat as specified here:
  // https://tc39.github.io/ecma402/#sec-intl-number_format-internal-slots
  //
  // 16. Let relevantExtensionKeys be %NumberFormat%.[[RelevantExtensionKeys]].
  ...
  // For example: in the case of `new Intl.NumberFormat('en-u-kn-true', {
  ...
  //
  // 17. Let r be ResolveLocale(%NumberFormat%.[[AvailableLocales]],
  // requestedLocales, opt, %NumberFormat%.[[RelevantExtensionKeys]],
  // localeData).
  // 18. Set number_format.[[Locale]] to r.[[locale]].
  Handle<JSObject> r;
  ASSIGN_RETURN_ON_EXCEPTION(
      isolate, r,
      Intl::ResolveLocale(isolate, "number_format", requested_locales, options),
      JSNumberFormat);
  ...
  //
  // As per
  // https://tc39.github.io/ecma402/#sec-intl-number_format-internal-slots: The
  // values "standard" and "search" must not be used as elements in any
  ...
  */
  ...
  // 21. Set number_format.[[Collation]] to collation.
  //
  // We don't store the collation value as per the above two steps
  // here. The collation value can be looked up from icu::NumberFormat on
  // demand, as part of Intl.NumberFormat.prototype.resolvedOptions.
  ...
  //     a. Set number_format.[[Numeric]] to ! SameValue(r.[[kn]], "true").
  //
  ...
  if (found_numeric.FromJust()) {
    icu_number_format->setAttribute(UCOL_NUMERIC_COLLATION,
  ...
  */
  ...
  //     a. Set number_format.[[CaseFirst]] to r.[[kf]].
  //
  ...
  if (found_case_first.FromJust()) {
    ...
    DCHECK_NOT_NULL(case_first_cstr);
    SetCaseFirstOption(icu_number_format.get(), case_first_cstr);
  } else if (extensions.find("kf") != extensions.end()) {
    const char* value = extensions.at("kf");
    DCHECK_NOT_NULL(value);
    SetCaseFirstOption(icu_number_format.get(), value);
  }
  */
  ...
  /*
  status = U_ZERO_ERROR;
  icu_number_format->setAttribute(UCOL_NORMALIZATION_MODE, UCOL_ON, status);
  CHECK(U_SUCCESS(status));
  */
  ...
  Maybe<bool> found_sensitivity =
      Intl::GetStringOption(isolate, options, "sensitivity", values,
                            "Intl.NumberFormat", &sensitivity_str);
  MAYBE_RETURN(found_sensitivity, MaybeHandle<JSNumberFormat>());
  ...
  */
  ...
  if (!found_sensitivity.FromJust()) {
    ...
    if (usage == Usage::SORT) {
      ...
      // 26. Set number_format.[[Sensitivity]] to sensitivity.
      icu_number_format->setStrength(icu::NumberFormat::TERTIARY);
    }
  } else {
    ...
    if (strncmp(sensitivity_cstr, "base", 4) == 0) {
      icu_number_format->setStrength(icu::NumberFormat::PRIMARY);
    } else if (strncmp(sensitivity_cstr, "accent", 6) == 0) {
      icu_number_format->setStrength(icu::NumberFormat::SECONDARY);
    } else if (strncmp(sensitivity_cstr, "case", 4) == 0) {
      icu_number_format->setStrength(icu::NumberFormat::PRIMARY);
      status = U_ZERO_ERROR;
      icu_number_format->setAttribute(UCOL_CASE_LEVEL, UCOL_ON, status);
      CHECK(U_SUCCESS(status));
    } else {
  ...
  */
  ...
  Maybe<bool> found_ignore_punctuation =
      Intl::GetBoolOption(isolate, options, "ignorePunctuation",
                          "Intl.NumberFormat", &ignore_punctuation);
  MAYBE_RETURN(found_ignore_punctuation, MaybeHandle<JSNumberFormat>());
  */

  // 28. Set number_format.[[IgnorePunctuation]] to ignorePunctuation.
  /*
  if (found_ignore_punctuation.FromJust() && ignore_punctuation) {
    status = U_ZERO_ERROR;
    icu_number_format->setAttribute(UCOL_ALTERNATE_HANDLING, UCOL_SHIFTED,
  status); CHECK(U_SUCCESS(status));
  }
  */
...
// static
/*
const char* JSNumberFormat::UsageToString(Usage usage) {
  switch (usage) {
    case Usage::SORT:
      return "sort";
    case Usage::SEARCH:
