...
MaybeHandle<JSNumberFormat> JSNumberFormat::UnwrapNumberFormat(
  ...
  Handle<String> method_name_str =
      isolate->factory()->NewStringFromStaticChars("UnwrapNumberFormat");

  Handle<JSObject> object;
  ASSIGN_RETURN_ON_EXCEPTION(
      isolate,
      object,
      Intl::UnwrapReceiver(isolate, format_holder, constructor,
                           Intl::Type::kNumberFormat, method_name_str, true),
      JSNumberFormat);
  CHECK(object->IsJSNumberFormat());
  ...
  if (!format_holder->IsJSNumberFormat()) {
    ...
    ASSIGN_RETURN_ON_EXCEPTION(
        ...
        Object::InstanceOf(isolate, format_holder, number_format_constructor),
        JSNumberFormat);
    ...
    if (is_instance_of_obj->BooleanValue(isolate)) {
      ...
      if (!object->IsJSReceiver()) {
        // 4. If Type(nf) is not Object ....,  then
        // a. Throw a TypeError exception.
        THROW_NEW_ERROR(
            isolate,
            NewTypeError(
                MessageTemplate::kIncompatibleMethodReceiver,
                factory->NewStringFromAsciiChecked("UnwrapNumberFormat")),
            JSNumberFormat);
      }
      format_holder = Handle<JSObject>::cast(object);
    }
  }
  ...
  // 5. Return nf.
  return Handle<JSNumberFormat>::cast(format_holder);
}
