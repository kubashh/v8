...
class StrDecimalLiteral {
 public:
  explicit StrDecimalLiteral(Handle<String> value);
...
StrDecimalLiteral::StrDecimalLiteral(Handle<String> value)
    : string_(value),
      negative_(false),
      exp_(0),
      integer_start_(0),
  ...
  if (string_->length() > 0) {
    ...
    // Skip leading '0'
    while (i < string_->length() && string_->Get(i) == '0') {
      i++;
    }
    integer_start_ = integer_end_ = i;
    // Scan 0-9
    while (i < string_->length() && '0' <= string_->Get(i) &&
           string_->Get(i) <= '9') {
      i++;
    }
    integer_end_ = i;
    ...
    if (i >= string_->length()) {
      ...
      while (integer_start_ < integer_end_ &&
             string_->Get(integer_end_ - 1) == '0') {
        integer_end_--;
      }
      return;
    }
    ...
    // Scan Exponment
    if (i < string_->length() &&
        (string_->Get(i) == 'e' || string_->Get(i) == 'E')) {
      i++;
      if (i < string_->length()) {
    ...
    if (integer_start_ == integer_end_) {
      while (fraction_start_ < fraction_end_ &&
             string_->Get(fraction_start_) == '0') {
        fraction_start_++;
        exp_--;
      }
    }
...
int StrDecimalLiteral::Digit(int index) const {
  if (index < integer_end_ - integer_start_) {
    return string_->Get(index - integer_start_) - '0';
  }
  if (index < integer_end_ - integer_start_ + fraction_end_ - fraction_start_) {
    return string_->Get((index - (integer_end_ - integer_start_)) -
                        fraction_start_) -
           '0';
  }
  return 0;
...
Maybe<bool> IntlMathematicalValue::IsLessThan(
  ...
  ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, y_str, y.ToString(isolate),
                                   Nothing<bool>());
  return Just(StrDecimalLiteral(x_str).IsLessThan(StrDecimalLiteral(y_str)));
}
...
Maybe<IntlMathematicalValue> IntlMathematicalValue::From(Isolate* isolate,
  ...
  // NonDecimalIntegerLiteral, we get non_decimal either as BigInt or Number.
  int32_t i = 0;
  // Skip over StrWhiteSpaceChar
  while (i < string->length()) {
    if (!IsWhiteSpaceOrLineTerminator(string->Get(i))) {
      break;
    }
    i++;
  }
  // The length of the StrWhiteSpaceChar_opt before StrNumericLiteral
  int prefix_white_space_length = i;
  ...
  if (i + 2 < string->length() && string->Get(i) == '0') {
    base::uc16 ch = string->Get(i + 1);
  ...
  int32_t postfix_white_space_start = string->length();
  // Trim StrWhiteSpaceChar from the back
  while (postfix_white_space_start > prefix_white_space_length) {
    if (!IsWhiteSpaceOrLineTerminator(
            string->Get(postfix_white_space_start - 1))) {
      break;
    }
    postfix_white_space_start--;
  }
  if (prefix_white_space_length == 0 &&
      postfix_white_space_start == string->length()) {
    result.value_ = string;
    return Just(result);
  }

  // Trim the StrWhiteSpace_opt before and after StrNumericLiteral.
  result.value_ = factory->NewSubString(string, prefix_white_space_length,
                                        postfix_white_space_start);
  return Just(result);
}
