...
Maybe<bool> SetNumberFormatDigitOptions(Isolate* isolate,
                                        icu::NumberFormat* icu_number_format,
                                        Handle<JSNumberFormat> number_format,
                                        Handle<JSReceiver> options,
                                        int mnfd_default, int mxfd_default) {
  icu::DecimalFormat* icu_decimal_format =
      static_cast<icu::DecimalFormat*>(icu_number_format);
  Factory* factory = isolate->factory();
  int32_t mnid, mnfd, mxfd;

  // 5. Let mnid be ? GetNumberOption(options, "minimumIntegerDigits,", 1, 21,
  //    1).
  Maybe<int> found_mnid =
      Intl::GetNumberOption(isolate, options, "minimumIntegerDigits", 1, 21, 1);
  MAYBE_RETURN(found_mnid, Nothing<bool>());
  mnid = found_mnid.FromJust();

  // 6. Let mnfd be ? GetNumberOption(options, "minimumFractionDigits", 0, 20,
  //    mnfdDefault).
  Maybe<int> found_mnfd = Intl::GetNumberOption(
      isolate, options, "minimumFractionDigits", 0, 20, mnfd_default);
  MAYBE_RETURN(found_mnfd, Nothing<bool>());
  mnfd = found_mnfd.FromJust();

  // 7. Let mxfdActualDefault be max( mnfd, mxfdDefault ).
  int32_t mxfd_actual_default = mnfd > mxfd_default ? mnfd : mxfd_default;

  // 8. Let mxfd be ? GetNumberOption(options, "maximumFractionDigits", mnfd,
  //    20, mxfdActualDefault).
  Maybe<int> found_mxfd = Intl::GetNumberOption(
      isolate, options, "maximumFractionDigits", mnfd, 20, mxfd_actual_default);
  MAYBE_RETURN(found_mxfd, Nothing<bool>());
  mxfd = found_mxfd.FromJust();

  // 9. Let mnsd be ? Get(options, "minimumSignificantDigits").
  Handle<String> minimum_significant_digits =
      factory->NewStringFromStaticChars("minimumSignificantDigits");
  Handle<Object> mnsd_obj;
  ASSIGN_RETURN_ON_EXCEPTION_VALUE(
      isolate, mnsd_obj,
      JSObject::GetProperty(isolate, options, "minimumSignificantDigits"),
      Nothing<bool>());

  // 10. Let mxsd be ? Get(options, "maximumSignificantDigits").
  Handle<String> maximum_significant_digits =
      factory->NewStringFromStaticChars("maximumSignificantDigits");
  Handle<Object> mxsd_obj;
  ASSIGN_RETURN_ON_EXCEPTION_VALUE(
      isolate, mxsd_obj,
      JSObject::GetProperty(isolate, options, maximum_significant_digits),
      Nothing<bool>());

  // 11. Set intlObj.[[MinimumIntegerDigits]] to mnid.
  icu_decimal_format->setMinimumIntegerDigits(mnid);

  // 12. Set intlObj.[[MinimumFractionDigits]] to mnfd.
  icu_decimal_format->setMinimumFractionDigits(mnfd);

  // 13. Set intlObj.[[MaximumFractionDigits]] to mxfd.
  icu_decimal_format->setMaximumFractionDigits(mxfd);

  // 14. If mnsd is not undefined or mxsd is not undefined, then
  if (!mnsd_obj->IsUndefined() || !mxsd_obj->IsUndefined()) {
    int32_t mnsd, mxsd;
    // a. Let mnsd be ? DefaultNumberOption(mnsd, 1, 21, 1).
    Maybe<int> found_mnsd = Intl::DefaultNumberOption(
        isolate, mnsd_obj, 1, 21, 1, minimum_significant_digits);
    MAYBE_RETURN(found_mnsd, Nothing<bool>());
    mnsd = found_mnsd.FromJust();

    // b. Let mxsd be ? DefaultNumberOption(mxsd, mnsd, 21, 21).
    Maybe<int> found_mxsd = Intl::DefaultNumberOption(
        isolate, mxsd_obj, mnsd, 21, 21, maximum_significant_digits);
    MAYBE_RETURN(found_mxsd, Nothing<bool>());
    mxsd = found_mxsd.FromJust();

    // c. Set intlObj.[[MinimumSignificantDigits]] to mnsd.
    icu_decimal_format->setMinimumSignificantDigits(mnsd);

    // d. Set intlObj.[[MaximumSignificantDigits]] to mxsd.
    icu_decimal_format->setMaximumSignificantDigits(mnsd);
    number_format->set_significant_digits_are_set();
  }
  return Just(true);
}
...
Handle<JSObject> JSNumberFormat::ResolvedOptions(
  ...
  Handle<String> locale_str = factory->NewStringFromAsciiChecked(result);
  ...
  if (number_format_holder->significant_digits_are_set()) {
    CreateDataPropertyForOptions(isolate, options,
...
MaybeHandle<JSNumberFormat> JSNumberFormat::InitializeNumberFormat(
  ...
  Maybe<bool> maybe_set_number_for_digit_options = SetNumberFormatDigitOptions(
      isolate, icu_number_format.get(), number_format, options, mnfd_default,
      mxfd_default);
