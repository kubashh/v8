...
HeapObjectReference HeapObjectReference::ClearedValue(Isolate* isolate) {
  ...
  Address raw_value = kLower32bitClearedWeakHeapObject;
  ...
  DCHECK_EQ(
      raw_value & (~static_cast<Address>(kLower32bitClearedWeakHeapObject)),
      isolate_root);
#endif
  // The rest of the code will check only the lower 32-bits.
  DCHECK_EQ(kLower32bitClearedWeakHeapObject, static_cast<uint32_t>(raw_value));
