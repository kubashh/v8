...
void JSTypedArray::RemoveExternalPointerCompensationForSerialization() {
  ...
  if (V8_HEAP_SANDBOX_BOOL) {
    ...
    // TODO(v8:10391): This const_cast is ok, because when pointer compression
    // in enabled GetIsolateForPtrCompr() returns valid Isolate pointer.
    Isolate* rw_isolate = const_cast<Isolate*>(isolate);
    set_external_pointer(
        rw_isolate, external_pointer() -
                        ExternalPointerCompensationForOnHeapArray(isolate));
  } else {
    set_external_pointer(
        nullptr, external_pointer() -
                     ExternalPointerCompensationForOnHeapArray(isolate));
  }
}
...
void JSTypedArray::SetOnHeapDataPtr(HeapObject base, Address offset) {
  ...
  if (V8_HEAP_SANDBOX_BOOL) {
    ...
    set_external_pointer(
        rw_isolate,
        offset + ExternalPointerCompensationForOnHeapArray(isolate));
  } else {
    set_external_pointer(
        nullptr, offset + ExternalPointerCompensationForOnHeapArray(isolate));
  }
