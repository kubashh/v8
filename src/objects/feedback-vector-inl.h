...
std::pair<MaybeObject, MaybeObject> FeedbackNexus::GetFeedbackPair() const {
  if (config()->config() == NexusConfig::BackgroundThread &&
      cache_.is_active()) {
    return std::make_pair(cache_.slot1(config()), cache_.slot2(config()));
  }
...
void FeedbackNexus::SetFeedback(FeedbackType feedback, WriteBarrierMode mode) {
  static_assert(std::is_base_of<MaybeObject, FeedbackType>::value ||
                    std::is_same<FeedbackType, Smi>::value ||
                    std::is_base_of<Object, FeedbackType>::value,
...
void FeedbackNexus::SetFeedback(FeedbackType feedback, WriteBarrierMode mode,
  ...
  static_assert(std::is_base_of<MaybeObject, FeedbackType>::value ||
                    ...
                    std::is_base_of<Object, FeedbackType>::value,
                "feedbacks need to be Smi, Object or MaybeObject");
  static_assert(std::is_base_of<MaybeObject, FeedbackExtraType>::value ||
                    std::is_same<FeedbackExtraType, Smi>::value ||
                    std::is_base_of<Object, FeedbackExtraType>::value,
                "feedbacks need to be Smi, Object or MaybeObject");
  MaybeObject fmo = FeedbackConvert(feedback);
