...
class JSLocale : public JSObject {
 public:
  ...
  static const int kCurrencyOffset = kCollationOffset + kPointerSize;
  static const int kHourCycleOffset = kCurrencyOffset + kPointerSize;
  ...
  static const int kTimeZoneOffset = kNumberingSystemOffset + kPointerSize;
  // Final size.
  static const int kSize = kTimeZoneOffset + kPointerSize;

 private:
  // Extracts value of a given property key in the Object.
  static bool ExtractStringSetting(Isolate* isolate, Handle<JSReceiver> options,
                                   const char* key,
                                   icu::UnicodeString* setting);

  // Inserts tags from options into locale string.
  static bool InsertOptionsIntoLocale(Isolate* isolate,
                                      Handle<JSReceiver> options,
                                      char* icu_locale);

  // Fills in the JSLocale object slots with Unicode tag/values.
  static bool PopulateLocaleWithUnicodeTags(Isolate* isolate,
