...
void MapUpdater::UpdateFieldType(Isolate* isolate, Handle<Map> map,
  ...
  PropertyConstness cur_constness = new_constness;
  Representation cur_representation = new_representation;
  MaybeObjectHandle cur_wrapped_type = new_wrapped_type;
  ...
  while (!backlog.empty() || !subtree_backlog.empty()) {
    ...
    if (!backlog.empty()) {
      ...
      cur_constness = std::get<PropertyConstness>(work);
      cur_representation = std::get<Representation>(work);
      cur_wrapped_type = std::get<MaybeObjectHandle>(work);
    }
    ...
    DCHECK(details.representation().Equals(cur_representation) ||
           details.representation().CanBeInPlaceChangedTo(cur_representation));

    // Skip if already updated the shared descriptor.
    if (cur_constness != details.constness() ||
        !cur_representation.Equals(details.representation()) ||
        descriptors->GetFieldType(descriptor) != *cur_wrapped_type.object()) {
      Descriptor d = Descriptor::DataField(
          name, descriptors->GetFieldIndex(descriptor), details.attributes(),
          cur_constness, cur_representation, cur_wrapped_type);
