...
Handle<Map> MapUpdater::ReconfigureToDataField(InternalIndex descriptor,
  ...
  if (old_details.constness() == PropertyConstness::kConst &&
    ...
    // Ensure we'll be updating constness of the up-to-date map.
    Handle<Map> old_map = UpdateMapNoLock(isolate_, old_map_);
    PropertyDetails details =
        old_map->instance_descriptors().GetDetails(modified_descriptor_);
    Handle<FieldType> field_type(
        old_map->instance_descriptors().GetFieldType(modified_descriptor_),
        isolate_);
    Map::GeneralizeField(isolate_, old_map, descriptor,
                         PropertyConstness::kMutable, details.representation(),
                         field_type);
    DCHECK_EQ(PropertyConstness::kMutable, old_map->instance_descriptors()
                                               .GetDetails(modified_descriptor_)
                                               .constness());
