...
BytecodeArray* SharedFunctionInfo::GetBytecodeArray() const {
  DCHECK(HasBytecodeArray());
  if (HasDebugInfo() && GetDebugInfo()->HasDebugBytecodeArray()) {
...
BytecodeArray* SharedFunctionInfo::GetDebugBytecodeArray() const {
  DCHECK(HasBytecodeArray());
  DCHECK(HasDebugInfo() && GetDebugInfo()->HasDebugBytecodeArray());
...
bool SharedFunctionInfo::HasDebugInfo() const {
  return function_identifier_or_debug_info()->IsStruct();
}
...
Object* SharedFunctionInfo::function_identifier() const {
  if (HasDebugInfo())
    return GetDebugInfo()->function_identifier();
  else
    return function_identifier_or_debug_info();
}
