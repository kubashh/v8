...
Smi ScriptHash(String source, const ScriptDetails& script_details) {
  DisallowGarbageCollection no_gc;
  size_t hash = base::hash_combine(
      source.EnsureHash(), script_details.line_offset,
      script_details.column_offset, script_details.origin_options.Flags());
  if (Handle<Object> name;
      script_details.name_obj.ToHandle(&name) && name->IsString()) {
    hash = base::hash_combine(hash, String::cast(*name).EnsureHash());
  }
...
ScriptCacheKey::ScriptCacheKey(Handle<String> source,
                               ...
                               Isolate* isolate)
    : HashTableKey(
          static_cast<uint32_t>(ScriptHash(*source, *script_details).value())),
      source_(source),
