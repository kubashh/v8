...
#undef TYPED_ARRAY_CASE
  ...
  static Object CopyElementsHandleSlow(Handle<Object> source,
    ...
    for (size_t i = 0; i < length; i++) {
      ...
      if (V8_UNLIKELY(new_length <= offset + i)) {
        // Proceed with the loop so that we call get getters for the values even
        // though we don't set them.
        continue;
      }
  ...
  static Object CopyElementsHandleImpl(Handle<Object> source,
    ...
    DCHECK_LE(offset + length, destination_ta->GetLength());
    ...
    if (source->IsJSTypedArray()) {
      ...
      // FIXME: also check out of boundness
