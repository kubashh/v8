...
enum class SimdKinds { kSSENeon, kAVX2, kNone };
...
#ifdef NEON64
// Returns the first non-zero index in |v|. |v| is a Neon vector that can
// be either 32x4 (the return is then 0, 1, 2 or 3) or 64x2 (the return is then
// 0 or 1). This is more or less equivalent to doing a movemask followed by a
// tzcnt on Intel.
template <typename T>
inline int extract_first_nonzero_index(T v) {
  // There is a chance that the vmax here should be vmin.
  if constexpr (std::is_same<T, int32x4_t>::value) {
    int32x4_t mask = {1, 2, 3, 4};
    mask = vandq_u32(mask, v);
    return vmaxvq_u32(mask) - 1;
  }
  if constexpr (std::is_same<T, int64x2_t>::value) {
    int64x2_t mask = {1, 2};
    mask = vandq_u64(mask, v);
    return vmaxvq_u64(mask);
  }
  if constexpr (std::is_same<T, float64x2_t>::value) {
    float64x2_t mask = {1, 2};
    mask = vandq_f64(mask, v) - 1;
    return vmaxvq_f64(mask) - 1;
  }
...
#define VECTORIZED_LOOP_Neon(type_load, type_eq, set1, cmp, movemask, extract) \
  {                                                                            \
...
// Since we don't compile with -mavx or -mavx2 (or /arch:AVX2 or MSVC), Clang
...
// ArrayIndexOfIncludesPacked only handles cases that can be efficiently
// vectorized:
//
...
template <ElementsKind kind>
uintptr_t ArrayIndexOfIncludesPacked(Address array_start, uintptr_t array_len,
                                     uintptr_t from_index,
                                     Address search_element) {
  static_assert(kind == PACKED_SMI_ELEMENTS || kind == PACKED_DOUBLE_ELEMENTS ||
                kind == PACKED_ELEMENTS);

  array_len = Smi::ToInt(static_cast<Smi>(array_len));
  from_index = Smi::ToInt(static_cast<Smi>(from_index));

  uintptr_t fail_value = Smi::FromInt(-1).ptr();
...
#ifdef V8_COMPRESS_POINTERS
#define COMPRESS(v) CompressTagged(v)
#else
#define COMPRESS(v) (v)
#endif

  if constexpr (kind == PACKED_DOUBLE_ELEMENTS) {
    DCHECK(Object(array_start).IsFixedDoubleArray());
  ...
  if constexpr (kind == PACKED_SMI_ELEMENTS) {
    DCHECK(Object(array_start).IsFixedArray());
    FixedArray fixed_array = FixedArray::cast(Object(array_start));
    Tagged_t* array =
        static_cast<Tagged_t*>(fixed_array.data_start().ToVoidPtr());

    DCHECK(Object(search_element).IsSmi());

    Tagged_t search_num = static_cast<Tagged_t>(search_element);

    return search<Tagged_t>(array, array_len, from_index, search_num);
  }

  if constexpr (kind == PACKED_ELEMENTS) {
    DCHECK(Object(array_start).IsFixedArray());
    FixedArray fixed_array = FixedArray::cast(Object(array_start));
    Tagged_t* array =
        static_cast<Tagged_t*>(fixed_array.data_start().ToVoidPtr());

    DCHECK(!Object(search_element).IsNumber());
    DCHECK(!Object(search_element).IsBigInt());
    DCHECK(!Object(search_element).IsString());

    Tagged_t search_element_compressed = COMPRESS(search_element);

    return search<Tagged_t>(array, array_len, from_index,
                            search_element_compressed);
  }
}

}  // namespace

uintptr_t ArrayIndexOfIncludesPackedSmi(Address array_start,
                                        uintptr_t array_len,
                                        uintptr_t from_index,
                                        Address search_element) {
  return ArrayIndexOfIncludesPacked<PACKED_SMI_ELEMENTS>(
      array_start, array_len, from_index, search_element);
}

uintptr_t ArrayIndexOfIncludesPackedObject(Address array_start,
                                           uintptr_t array_len,
                                           uintptr_t from_index,
                                           Address search_element) {
  return ArrayIndexOfIncludesPacked<PACKED_ELEMENTS>(
      array_start, array_len, from_index, search_element);
}

uintptr_t ArrayIndexOfIncludesPackedDouble(Address array_start,
                                           uintptr_t array_len,
                                           uintptr_t from_index,
                                           Address search_element) {
  return ArrayIndexOfIncludesPacked<PACKED_DOUBLE_ELEMENTS>(
      array_start, array_len, from_index, search_element);
}
