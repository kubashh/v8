...
void TransitionsAccessor::Insert(Isolate* isolate, Handle<Map> map,
  ...
  Encoding encoding = GetEncoding(isolate, raw_transitions);
  ...
  if (encoding == kUninitialized || encoding == kMigrationTarget) {
    if (flag == SIMPLE_PROPERTY_TRANSITION) {
      map->set_raw_transitions(HeapObjectReference::Weak(*target));
      return;
    }
    ...
    map->set_raw_transitions(MaybeObject::FromObject(*result));
    DCHECK_EQ(kFullTransitionArray,
              GetEncoding(isolate, MaybeObject::FromObject(*result)));
    return;
  }

  if (encoding == kWeakRef) {
    Map simple_transition =
    ...
    Handle<Map> simple_map(simple_transition, isolate);
    Handle<TransitionArray> result =
        isolate->factory()->NewTransitionArray(1, 1);
    if (GetEncoding(isolate, raw_transitions) != kWeakRef) {
      result->Set(0, *name, HeapObjectReference::Weak(*target));
      map->set_raw_transitions(MaybeObject::FromObject(*result));
      DCHECK_EQ(kFullTransitionArray,
                GetEncoding(isolate, MaybeObject::FromObject(*result)));
    ...
    result->Set(0, GetSimpleTransitionKey(*simple_map),
                HeapObjectReference::Weak(*simple_map));
    ...
    if (insertion_index == 0) {
      ...
      result->Set(1, GetSimpleTransitionKey(*simple_map),
                  HeapObjectReference::Weak(*simple_map));
    }
  ...
  // Reload();
  DisallowGarbageCollection no_gc;
  raw_transitions = map->raw_transitions(isolate, kAcquireLoad);
  TransitionArray array =
      TransitionArray::cast(raw_transitions->GetHeapObjectAssumeStrong());
...
bool TransitionsAccessor::CanHaveMoreTransitions(Isolate* isolate,
  ...
  if (GetEncoding(isolate, raw_transitions) == kFullTransitionArray) {
    return transitions(raw_transitions).number_of_transitions() <
...
WeakFixedArray TransitionsAccessor::GetPrototypeTransitions(Isolate* isolate,
  ...
  TransitionArray transition_array = transitions(raw_transitions);
  if (!transition_array.HasPrototypeTransitions()) {
    return ReadOnlyRoots(isolate).empty_weak_fixed_array();
  }
  return transitions(raw_transitions).GetPrototypeTransitions();
}
...
void TransitionsAccessor::SetPrototypeTransitions(
  ...
  transitions(map->raw_transitions(isolate, kAcquireLoad))
...
void TransitionsAccessor::EnsureHasFullTransitionArray(Isolate* isolate,
  ...
  if (nof == 1) {
    if (encoding == kUninitialized) {
      ...
      Map target = encoding == kWeakRef
                       ? Map::cast(map->raw_transitions(isolate, kAcquireLoad)
  ...
  map->set_raw_transitions(MaybeObject::FromObject(*result), kReleaseStore);
  // ReplaceTransitions(MaybeObject::FromObject(
  //     *result));  // TODO: I should rewrite to static ReplaceTransitions
}
