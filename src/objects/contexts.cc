...
Tagged<ContextSlotReprCell> Context::GetOrCreateContextSlotRepr(
  ...
  if (!IsContextSlotReprCell(object)) {
    CHECK(IsSmi(object));
    object = *isolate->factory()->NewContextSlotReprCell(object.ToSmi());
    side_data->set(side_data_index, object);
  }
...
namespace {
ContextSlotRepresentation GeneralizeContextSlotRepr(
    ContextSlotRepresentation old_repr, ContextSlotRepresentation new_repr) {
  if (old_repr == ContextSlotRepresentation::kMutableHeapNumber &&
      new_repr == ContextSlotRepresentation::kSmi) {
  ...
  return static_cast<ContextSlotRepresentation>(
      std::max(static_cast<int>(old_repr), static_cast<int>(new_repr)));
}
}  // namespace

void Context::StoreScriptContextElementAndUpdateSideData(
  ...
  if (IsTheHole(*old_value)) {
    ...
    ContextSlotRepresentation repr =
        ContextSlotReprCell::RepresentationOf(*new_value);
    side_data->set(context_slot_repr_index,
                   Smi::FromInt(static_cast<int>(repr)));
    if (repr == ContextSlotRepresentation::kMutableHeapNumber) {
  ...
  ContextSlotRepresentation expected_repr;
  ContextSlotRepresentation new_value_repr =
      ContextSlotReprCell::RepresentationOf(*new_value);
  ...
  if (IsContextSlotReprCell(context_slot_repr)) {
    ...
    expected_repr = static_cast<ContextSlotRepresentation>(
        Cast<Smi>(context_slot_repr).value());
  }

  // Update representation.
  ContextSlotRepresentation updated_repr =
      GeneralizeContextSlotRepr(expected_repr, new_value_repr);
  side_data->set(context_slot_repr_index,
                 Smi::FromInt(static_cast<int>(updated_repr)));

  // Update slot value.
  if (expected_repr == ContextSlotRepresentation::kMutableHeapNumber) {
    DCHECK(Is<HeapNumber>(*old_value));
    if (updated_repr == ContextSlotRepresentation::kMutableHeapNumber) {
      if (Is<Smi>(*new_value)) {
      ...
      DCHECK_EQ(updated_repr, ContextSlotRepresentation::kOther);
      script_context->set(index, *new_value);
    }
  } else if (updated_repr == ContextSlotRepresentation::kMutableHeapNumber) {
