...
namespace {
ContextSlotReprCell::Representation GeneralizeContextSlotRepr(
    ContextSlotReprCell::Representation old_repr,
    ContextSlotReprCell::Representation new_repr) {
  if (old_repr == ContextSlotReprCell::kMutableHeapNumber &&
      new_repr == ContextSlotReprCell::kSmi) {
    // We remain in mutable heap number if we see a Smi.
    return ContextSlotReprCell::kMutableHeapNumber;
  }
  // Otherwise representation rank always grow.
  return static_cast<ContextSlotReprCell::Representation>(
...
void Context::StoreContextElementAndUpdateSideData(
  ...
  ContextSlotReprCell::Representation expected_repr;
  ContextSlotReprCell::Representation new_value_repr =
      ContextSlotReprCell::RepresentationOf(*new_value);
  ...
  if (IsContextSlotReprCell(context_slot_repr)) {
    ...
    expected_repr = static_cast<ContextSlotReprCell::Representation>(
        Cast<Smi>(context_slot_repr).value());
  }

  // Update representation.
  ContextSlotReprCell::Representation updated_repr =
      GeneralizeContextSlotRepr(expected_repr, new_value_repr);
  side_data->set(context_slot_repr_index, Smi::FromInt(updated_repr));

  // Update slot value.
  if (expected_repr == ContextSlotReprCell::kMutableHeapNumber) {
    DCHECK(Is<HeapNumber>(*old_value));
    if (updated_repr == ContextSlotReprCell::kMutableHeapNumber) {
      if (Is<Smi>(*new_value)) {
      ...
      DCHECK_EQ(updated_repr, ContextSlotReprCell::kOther);
      script_context->set(index, *new_value);
    }
  } else if (updated_repr == ContextSlotReprCell::kMutableHeapNumber) {
