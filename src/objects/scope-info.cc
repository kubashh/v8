...
void ScopeInfo::SetFunctionName(Object name) {
  ...
  TqSetterStructFunctionNameInfo function_name_info;
  function_name_info.function_variable_name = name;
  function_name_info.function_variable_context_or_stack_slot_index =
      kTqNoChange;
  set_function_name_info(0, function_name_info);
}
...
void ScopeInfo::SetPositionInfo(int start, int end) {
  DCHECK(HasPositionInfo());
  DCHECK_LE(start, end);
  TqSetterStructPositionInfo position_info;
  position_info.start = start;
  position_info.end = end;
  set_position_info(0, position_info);
}
...
void ScopeInfo::ModuleVariable(int i, String* name, int* index,
                               ...
                               InitializationFlag* init_flag,
                               MaybeAssignedFlag* maybe_assigned_flag) {
  TqGetterStructModuleVariable variable = module_variables(i);
