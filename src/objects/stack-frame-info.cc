...
void SerializeJSStackFrame(Isolate* isolate, Handle<StackTraceFrame> frame,
  ...
  if (is_promise_all) {
    ...
    Handle<String> index_string = isolate->factory()->NumberToString(
        handle(Smi::FromInt(StackTraceFrame::GetPromiseAllIndex(frame)),
               isolate),
        isolate);
...
void SerializeWasmStackFrame(Isolate* isolate, Handle<StackTraceFrame> frame,
  ...
  char buffer[16];
  SNPrintF(ArrayVector(buffer), "%u]", StackTraceFrame::GetLineNumber(frame));
  builder.AppendCString(buffer);

  // Column numbers are 1-based for API back-wards compatibility.
  SNPrintF(ArrayVector(buffer), ":%d",
           StackTraceFrame::GetColumnNumber(frame) - 1);
  builder.AppendCString(buffer);
