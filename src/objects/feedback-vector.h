...
class V8_EXPORT_PRIVATE NexusConfig {
 public:
  ...
  enum Mode { Writer, Reader };

  Mode mode() const { return local_heap_ == nullptr ? Writer : Reader; }
  ...
  bool can_write() const { return mode() == Writer; }
...
class V8_EXPORT_PRIVATE FeedbackNexus final {
 public:
  // The Isolate-default NexusConfig is used for these two constructors.
  ...
  int ExtractMapsAndHandlers(
      ...
      base::Optional<TryUpdateHandler> map_handler = base::nullopt) const;
