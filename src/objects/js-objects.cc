...
#undef TYPED_ARRAY_CONSTRUCTORS_SWITCH
    ...
    default: {
      ...
      if (IsJSPrimitiveWrapper(*this)) {
        ...
        Object::ShortPrint(JSPrimitiveWrapper::cast(*this)->value(),
                           accumulator);
      }
      accumulator->Put('>');
      break;
...
void JSObject::PrintElementsTransition(FILE* file, Handle<JSObject> object,
  ...
  if (from_kind != to_kind) {
    ...
    PrintF(file, " for ");
    Object::ShortPrint(*object, file);
    PrintF(file, " from ");
    Object::ShortPrint(*from_elements, file);
    PrintF(file, " to ");
    Object::ShortPrint(*to_elements, file);
    PrintF(file, "\n");
  }
}
