...
// static
bool JSReceiver::CheckPrivateStore(LookupIterator* it, bool is_define) {
  DCHECK(it->GetName()->IsPrivate());
  bool is_private_name = it->GetName()->IsPrivateName();
  ...
  for (; it->IsFound(); it->Next()) {
    switch (it->state()) {
      ...
      case LookupIterator::ACCESS_CHECK:
        ...
        break;
      case LookupIterator::DATA:
        if (is_private_name && is_define && should_throw) {
