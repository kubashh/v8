...
MaybeHandle<JSObject> JSLocale::WeekInfo(Isolate* isolate,
  ...
  int32_t week_start = weekdayFromEDaysOfWeek(calendar->getFirstDayOfWeek());
  ...
  int32_t minimal_days_in_first_week = calendar->getMinimalDaysInFirstWeek();

  Maybe<bool> maybe_create_week_start = JSReceiver::CreateDataProperty(
      isolate, info, factory->weekStart_string(),
      factory->NewNumberFromInt(week_start), Just(kDontThrow));
  DCHECK(maybe_create_week_start.FromJust());
  USE(maybe_create_week_start);
  ...
  Maybe<bool> maybe_create_minimal_days_in_first_week =
      JSReceiver::CreateDataProperty(
          isolate, info, factory->minimalDaysInFirstWeek_string(),
          factory->NewNumberFromInt(minimal_days_in_first_week),
          Just(kDontThrow));
  DCHECK(maybe_create_minimal_days_in_first_week.FromJust());
  USE(maybe_create_minimal_days_in_first_week);
