...
bool ExtractStringSetting(Isolate* isolate, Handle<JSReceiver> options,
...
bool InsertOptionsIntoLocale(Isolate* isolate, Handle<JSReceiver> options,
                             char* icu_locale) {
  CHECK(isolate);
  CHECK(icu_locale);
...
bool JSLocale::InitializeLocale(Isolate* isolate,
  ...
  if (!InsertOptionsIntoLocale(isolate, options, icu_result)) {
    return false;
  }
