...
bool IsExtensionSingleton(const std::string& value) {
  return IsAlpha(value, 1, 1) || IsDigit(value, 1, 1);
}

// TODO(ftang) Replace the following check w/ icu::LocaleBuilder
// once ICU64 land in March 2019.
bool StartWithUnicodeLanguageId(const std::string& value) {
  // unicode_language_id =
  ...
  if (!(tokens.size() >= 1 && IsUnicodeLanguageSubtag(tokens[0]))) return false;
  if (tokens.size() == 1) return true;
  if (!(IsUnicodeScriptSubtag(tokens[1]) || IsUnicodeRegionSubtag(tokens[1]) ||
        IsUnicodeVariantSubtag(tokens[1]) || IsExtensionSingleton(tokens[1])))
    return false;
  if (tokens.size() == 2 || IsExtensionSingleton(tokens[1])) return true;
  if (!((IsUnicodeScriptSubtag(tokens[1]) &&
         IsUnicodeRegionSubtag(tokens[2])) ||
        IsUnicodeVariantSubtag(tokens[2]) || IsExtensionSingleton(tokens[2])))
    return false;
  if (tokens.size() == 3 || IsExtensionSingleton(tokens[2])) return true;
  for (size_t i = 3; i < tokens.size(); i++) {
    if (!(IsUnicodeVariantSubtag(tokens[i]) || IsExtensionSingleton(tokens[i])))
      return false;
    if (IsExtensionSingleton(tokens[i])) return true;
  }
  return true;
}
...
Maybe<std::string> ApplyOptionsToTag(Isolate* isolate, Handle<String> tag,
  ...
  // exception.
  if (!StartWithUnicodeLanguageId(*bcp47_tag)) {
    THROW_NEW_ERROR_RETURN_VALUE(
        isolate, NewRangeError(MessageTemplate::kLocaleBadParameters),
        Nothing<std::string>());
  }
  ...
  std::vector<std::string> tokens;
  std::string token;
  std::istringstream tokenStream(maybe_canonicalized.FromJust());
  while (std::getline(tokenStream, token, '-')) {
    tokens.push_back(token);
  }
  ...
  if (maybe_region.FromJust()) {
    ...
    if (IsUnicodeRegionSubtag(tokens[1])) {
      tokens[1] = region_str.get();
    } else if (IsUnicodeScriptSubtag(tokens[1]) &&
               IsUnicodeRegionSubtag(tokens[2])) {
