...
bool JSLocale::InitializeLocale(Isolate* isolate,
  ...
  locale_holder->set_language(
      *factory->NewStringFromAsciiChecked(icu_language));
  if (strlen(icu_script) != 0) {
    locale_holder->set_script(*factory->NewStringFromAsciiChecked(icu_script));
  }
  if (strlen(icu_region) != 0) {
    locale_holder->set_region(*factory->NewStringFromAsciiChecked(icu_region));
  }
  ...
  locale_holder->set_locale(*factory->NewStringFromAsciiChecked(bcp47_result));
