...
namespace {
void SetAsyncUnlockThen(Isolate* isolate, Handle<JSAtomicsMutex> mutex,
...
class V8_NODISCARD WaiterQueueNode {
 public:
  ...
  // Enqueues {new_head} at the fron of the queue.
...
void AsyncWaiterNotifyTask::RunInternal() {
  JSAtomicsMutex::HandleAsyncNotify(node_);
}
...
MaybeHandle<Object> JSAtomicsMutex::RunCallable(Handle<JSAtomicsMutex> mutex,
  ...
  // If result is a promise, set the mutex release as a then callback.
  // Otherwise, release the lock.
  if (IsJSPromise(*result)) {
    SetAsyncUnlockThen(isolate, mutex, promise);
  } else {
    mutex->Unlock(isolate);
  }
...
void JSAtomicsMutex::HandleAsyncNotify(AsyncWaiterQueueNode* node) {
  ...
  RunOrQueueCallable(mutex_lock, isolate, callback, promise);
}
...
void JSAtomicsMutex::LockSlowPath(Isolate* requester,
  ...
  for (;;) {
    ...
    if (!MaybeEnqueueNode(requester, mutex, state, this_waiter.get())) return;
