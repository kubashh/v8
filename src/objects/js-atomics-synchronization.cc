...
// static
void JSAtomicsMutex::UnlockWaiterQueue(std::atomic<StateT>* state) {
  // Change only the `kIsWaiterQueueLockedBit` bit.
  StateT expected = state->load(std::memory_order_relaxed);
  StateT desired;
  do {
    desired = (expected & ~kIsWaiterQueueLockedBit);
...
bool JSAtomicsMutex::LockJSMutexOrDequeueTimedOutWaiter(
  ...
  if (waiter_head == nullptr) {
    ...
    UnlockWaiterQueue(state);
    return false;
  }

  WaiterQueueNode::DequeueMatching(&waiter_head, [&](WaiterQueueNode* node) {
    return node == timed_out_waiter;
  });
  ...
  // At this point this thread holds the queue lock, but there is a chance that
  // timed_out_waiter was notified between the time this thread woke up and the
  // time it acquired the queue lock, so there is a risk that the next
  // queue head is never notified. Try to take the js mutex lock here, if we
  // succeed, the next node will be notified by this thread, otherwise, it will
  // be notified by the thread holding the lock now.

  // Since we use strong CAS below, we know that the js mutex lock will be held
  // by either this thread or another thread that can't go through the unlock
  // fast path because this thread is holding the waiter queue lock.
