...
Handle<JSPromise> SetPromiseThen(
    Isolate* isolate, Handle<JSPromise> promise, Handle<JSFunction> callable,
  ...
  if (reject_handler.is_null()) {
    local_then_promise =
        local_promise->Then(local_native_context, local_callable);
  } else {
    local_then_promise =
        local_promise->Then(local_native_context, local_callable,
...
Handle<JSPromise> SetAsyncUnlockThen(Isolate* isolate,
                                     ...
                                     Handle<JSPromise> unlockPromise) {
  ...
  JSObject::AddProperty(isolate, resolver_callback, "unlockPromise",
                        Handle<Object>::cast(unlockPromise),
  ...
  Handle<JSPromise> then_promise =
      SetPromiseThen(isolate, promise, resolver_callback, reject_callback);
  return then_promise;
}
...
Handle<JSPromise> JSAtomicsMutex::LockOrQueuePromise(
  ...
  // Set the callable to be run a mirotask after acquiring the lock.
  ...
  Handle<JSPromise> unlock_promise = isolate->factory()->NewJSPromise();
  Handle<JSPromise> unlock_then =
      SetAsyncUnlockThen(isolate, mutex, callable_then, unlock_promise);
