...
void JSAtomicsMutex::UnlockAsyncLockedMutex(
  ...
  WaiterQueueNode::RemoveFromAsyncWaiterQueueList(waiter_node);
  Unlock(requester);
}
...
void JSAtomicsMutex::HandleAsyncTimeout(LockAsyncWaiterQueueNode* waiter) {
  ...
  if (V8_UNLIKELY(waiter->native_context_.IsEmpty())) {
    ...
    // list.
    WaiterQueueNode::RemoveFromAsyncWaiterQueueList(waiter);
    return;
  }
...
// 1. `wait_promise`, which will be resolved either in the notify task or in the
//    timeout task.
...
Handle<JSPromise> JSAtomicsCondition::WaitAsync(
  ...
  WaitAsyncWaiterQueueNode* this_waiter =
      WaitAsyncWaiterQueueNode::NewAsyncWaiterStoredInIsolate(requester, cv,
                                                              wait_promise);
