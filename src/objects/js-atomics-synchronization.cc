...
class V8_NODISCARD AsyncWaiterQueueNode final : public WaiterQueueNode {
 public:
  explicit AsyncWaiterQueueNode(Isolate* requester, Handle<JSObject> mutex,
    ...
    if (!callback.is_null()) {
      v8::Local<v8::Object> local_callback =
          Utils::ToLocal(callback.ToHandleChecked());
  ...
  bool HasEmptyCallback() { return callback_.IsEmpty(); }
...
bool JSAtomicsMutex::AsyncLockSlowPath(Handle<JSAtomicsMutex> mutex,
  ...
  std::unique_ptr<AsyncWaiterQueueNode> this_waiter =
      std::make_unique<AsyncWaiterQueueNode>(isolate, mutex, promise, callback);
...
// static
Handle<JSPromise> JSAtomicsCondition::WaitAsync(Isolate* requester,
                                                Handle<JSAtomicsCondition> cv,
                                                Handle<JSAtomicsMutex> mutex) {
  Handle<JSPromise> promise = requester->factory()->NewJSPromise();
  std::unique_ptr<AsyncWaiterQueueNode> this_waiter =
      std::make_unique<AsyncWaiterQueueNode>(requester, mutex, promise,
...
// static
void JSAtomicsCondition::HandleAsyncNotify(detail::AsyncWaiterQueueNode* node) {
  if (node->GetRequester()->cancelable_task_manager()->canceled()) return;
  Isolate* isolate = node->GetRequester();
  HandleScope scope(isolate);
  ...
  Handle<JSGlobalObject> global(isolate->native_context()->global_object(),
                                isolate);
  ...
  Handle<SharedFunctionInfo> info =
      isolate->factory()->NewSharedFunctionInfoForBuiltin(
          ...
          Builtin::kAtomicsMutexAsyncUnlock);
  info->set_language_mode(LanguageMode::kStrict);
