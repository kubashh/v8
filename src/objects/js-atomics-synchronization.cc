...
class AsyncLockTimeoutTask : public CancelableTask {
 public:
  AsyncLockTimeoutTask(CancelableTaskManager* cancelable_task_manager,
                       AsyncLockWaiterQueueNode* node)
      : CancelableTask(cancelable_task_manager), node_(node) {}

  void RunInternal() override;

 private:
  AsyncLockWaiterQueueNode* node_;
};
...
class AsyncLockWaiterQueueNode final : public AsyncWaiterQueueNode {
 public:
  AsyncLockWaiterQueueNode(Isolate* requester, Handle<JSObject> mutex,
                           Handle<JSPromise> promise, Handle<JSObject> callback)
  ...
  ~AsyncLockWaiterQueueNode() { callback_.Reset(); }
...
class AsyncWaitTimeoutTask : public CancelableTask {
 public:
  AsyncWaitTimeoutTask(CancelableTaskManager* cancelable_task_manager,
                       AsyncWaitWaiterQueueNode* node,
                       Handle<JSAtomicsCondition> cv)
      : CancelableTask(cancelable_task_manager), node_(node) {
    v8::Isolate* v8_isolate =
        reinterpret_cast<v8::Isolate*>(node->GetRequester());
 ...
 private:
  AsyncWaitWaiterQueueNode* node_;
...
void AsyncWaitTimeoutTask::RunInternal() {
  if (node_->GetRequester()->cancelable_task_manager()->canceled()) return;
  Isolate* isolate = node_->GetRequester();
  HandleScope scope(isolate);
  if (node_->should_wait) {
    Handle<JSAtomicsCondition> cv = Handle<JSAtomicsCondition>::cast(
        Utils::OpenHandle(*cv_.Get(reinterpret_cast<v8::Isolate*>(isolate))));
    JSAtomicsCondition::RemoveTimedOutWaiter(cv, node_);
    JSAtomicsCondition::HandleAsyncNotify(node_);
    // v8::Context::Scope contextScope(node_->native_context());
    // MaybeHandle<Object> resolve_result =
    //   JSPromise::Resolve(node_->GetPromise(),
...
using detail::AsyncLockTimeoutTask;
using detail::AsyncLockWaiterQueueNode;
using detail::AsyncWaitTimeoutTask;
...
void JSAtomicsMutex::LockSlowPath(Isolate* requester,
  ...
  for (;;) {
    ...
    if (!MaybeEnqueueNode(requester, mutex, state, &this_waiter,
                          WaiterQueueNode::Enqueue))
      return;
...
Handle<JSPromise> JSAtomicsCondition::WaitAsync(
  ...
  std::unique_ptr<AsyncWaitWaiterQueueNode> this_waiter =
      std::make_unique<AsyncWaitWaiterQueueNode>(requester, mutex, promise);
  QueueWaiter(requester, cv, this_waiter.get());
  if (timeout) {
    auto task = std::make_unique<AsyncWaitTimeoutTask>(
        requester->cancelable_task_manager(), this_waiter.get(), cv);
    this_waiter->task_runner()->PostNonNestableDelayedTask(
        std::move(task), timeout->InSecondsF());
  }
  this_waiter.release();
  mutex->Unlock(requester);
  return promise;
...
void JSAtomicsCondition::RemoveTimedOutWaiter(
