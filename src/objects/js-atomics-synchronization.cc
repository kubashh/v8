...
void SetAsyncUnlockHandlers(Isolate* isolate, Handle<JSAtomicsMutex> mutex,
  ...
  handlers_context->set(JSAtomicsMutex::kLockAsyncPromiseAsyncContextSlot,
                        *lock_async_promise);

  if (async_locked_waiter) {
    Handle<JSExternalObject> wrapper = Handle<JSExternalObject>::cast(
        isolate->factory()->NewExternal(async_locked_waiter));
...
void JSAtomicsMutex::UnlockAsyncLockedMutex(
    Isolate* requester,
    MaybeHandle<JSExternalObject> maybe_async_locked_waiter_wrapper) {
  if (!maybe_async_locked_waiter_wrapper.is_null()) {
    Handle<JSExternalObject> async_locked_waiter_wrapper =
        maybe_async_locked_waiter_wrapper.ToHandleChecked();
    WaiterQueueNode* waiter_node =
        static_cast<WaiterQueueNode*>(async_locked_waiter_wrapper->value());
  ...
  if (V8_UNLIKELY(waiter->native_context_.IsEmpty())) {
    ...
    if (!waiter->synchronization_primitive_.IsEmpty()) {
      ...
      if (HasWaitersField::decode(current_state)) {
        ...
        // waiter, so manually realease the queue lock without changing the
        // IsLockedField bit.
        while (!TryLockWaiterQueueExplicit(state, current_state)) {
          YIELD_PROCESSOR;
        }
