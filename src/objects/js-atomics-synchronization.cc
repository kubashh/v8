 ...
 private:
  ...
  explicit AsyncWaiterQueueNode(Isolate* requester,
    ...
    synchronization_primitive_ = GetWeakGlobal(
        requester,
        Utils::ToLocal(Handle<JSObject>::cast(synchronization_primitive)));
    AddWaitingData(requester, internal_waiting_promise, unlocked_promise);
  }
...
bool JSAtomicsMutex::LockAsyncSlowPath(
  ...
  if (!MaybeEnqueueNode(isolate, mutex, state, waiter_node)) {
    // The mutex is locked, the waiter node was not enqueued, so we don't need
    // the waiting data in the node.
...
MaybeHandle<JSPromise> JSAtomicsMutex::SetAsyncUnlockHandlers(
  ...
  if (promise_then.is_null()) {
    // If the `promise_then` operation failed then the lockAsync all will throw.
