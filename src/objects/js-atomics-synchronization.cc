...
void AsyncWaiterQueueTask::RunInternal() {
  ...
  Tagged<Oddball> junk =
      JSAtomicsMutex::RunOrQueueCallable(mutex, isolate, callback, promise);
  USE(junk);
  // node_->Call();
  delete node_;
}
...
// static
Tagged<Oddball> JSAtomicsMutex::RunOrQueueCallable(
    Handle<JSAtomicsMutex> mutex, Isolate* isolate,
    Handle<JSObject> run_under_lock, Handle<JSObject> promise) {
