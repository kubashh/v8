...
void SetAsyncUnlockHandlers(Isolate* isolate, Handle<JSAtomicsMutex> mutex,
                            ...
                            Handle<JSPromise> lock_async_promise,
  ...
  handlers_context->set(JSAtomicsMutex::kUnlockPromiseAsyncContextSlot,
                        *lock_async_promise);

  if (async_locked_waiter) {
    Handle<AsyncLockedWaiterWrapper> wrapper =
        Handle<AsyncLockedWaiterWrapper>::cast(isolate->factory()->NewStruct(
            ASYNC_LOCKED_WAITER_WRAPPER_TYPE, AllocationType::kOld));
    wrapper->SetAsyncLockedWaiter(isolate, async_locked_waiter);
    handlers_context->set(JSAtomicsMutex::kAsyncLockedWaiterSlot, *wrapper);
  } else {
    handlers_context->set(JSAtomicsMutex::kAsyncLockedWaiterSlot,
  ...
  PerformPromiseThen(isolate, lock_promise, resolver_callback, reject_callback);
}
...
class V8_NODISCARD SyncWaiterQueueNode final : public WaiterQueueNode {
 public:
  ...
  bool IsSameNativeContextForAsyncCleanup(Tagged<Context> context) override {
    return false;
  }
...
class V8_NODISCARD AsyncWaiterQueueNode : public WaiterQueueNode {
 public:
  ~AsyncWaiterQueueNode() {
    ...
    synchronization_primitive_.Reset();
    lock_async_promise_.Reset();
  }

  static AsyncWaiterQueueNode<T>* NewUnlockedWaiterInIsolate(
      Isolate* requester, Handle<T> synchronization_primitive,
      Handle<JSPromise> waiting_promise,
      MaybeHandle<JSPromise> lock_async_promise = MaybeHandle<JSPromise>()) {
    auto waiter = std::unique_ptr<AsyncWaiterQueueNode<T>>(
        new AsyncWaiterQueueNode<T>(requester, synchronization_primitive,
                                    waiting_promise, lock_async_promise));
    AsyncWaiterQueueNode<T>* raw_waiter = waiter.get();
    requester->async_unlocked_waiter_queue_nodes().push_back(std::move(waiter));
    return raw_waiter;
  }

  static AsyncWaiterQueueNode<T>* NewLockedWaiterInIsolate(
      Isolate* requester, Handle<T> synchronization_primitive) {
    auto waiter = std::unique_ptr<AsyncWaiterQueueNode<T>>(
        new AsyncWaiterQueueNode<T>(requester, synchronization_primitive));
    AsyncWaiterQueueNode<T>* raw_waiter = waiter.get();
    requester->async_locked_waiter_queue_nodes().push_back(std::move(waiter));
    return raw_waiter;
  }
  ...
  Handle<JSPromise> GetWaitingPromise() {
    v8::Isolate* v8_isolate = reinterpret_cast<v8::Isolate*>(requester_);
    Handle<JSPromise> waiting_promise =
        Utils::OpenHandle(*waiting_promise_.Get(v8_isolate));
  ...
  bool IsSameNativeContextForAsyncCleanup(Tagged<Context> context) override {
    return *Utils::OpenDirectHandle(*GetNativeContext()) == *context;
  }
 ...
 private:
  friend JSSynchronizationPrimitive;
  ...
  explicit AsyncWaiterQueueNode(Isolate* requester,
    ...
    synchronization_primitive_ = GetWeakGlobal(
        ...
        Utils::ToLocal(Handle<JSObject>::cast(synchronization_primitive)));
    waiting_promise_ =
        GetWeakGlobal(requester, Utils::PromiseToLocal(waiting_promise));
  ...
  // `waiting_promise_` is the internal first promise of the chain. See
  // comments in `JSAtomicsMutex` and `JSAtomicsCondition`.
  Global<v8::Promise> waiting_promise_;
  Global<v8::Object> synchronization_primitive_;
  // `lock_async_promise_` is the user exposed promise used to handle timeouts,
...
// static
void JSSynchronizationPrimitive::RealmDisposed(Isolate* isolate) {
  Tagged<Context> context = isolate->raw_native_context();
  CleanupWaiters(isolate, [=](WaiterQueueNode* waiter) {
    return waiter->IsSameNativeContextForAsyncCleanup(context);
  });
}
...
void JSSynchronizationPrimitive::CleanupWaiters(Isolate* isolate,
                                                DequeueMatcher matcher) {
  std::list<std::unique_ptr<WaiterQueueNode>>& async_unlocked_list =
      isolate->async_unlocked_waiter_queue_nodes();
  if (!async_unlocked_list.empty()) {
    HandleScope handle_scope(isolate);
    auto it = async_unlocked_list.begin();
    while (it != async_unlocked_list.end()) {
      ...
      it = async_unlocked_list.erase(it);
    }
  }

  std::list<std::unique_ptr<WaiterQueueNode>>& async_locked_list =
      isolate->async_locked_waiter_queue_nodes();
  if (async_locked_list.empty()) return;
  HandleScope handle_scope(isolate);
  auto it = async_locked_list.begin();
  while (it != async_locked_list.end()) {
    LockAsyncWaiterQueueNode* async_node =
        static_cast<LockAsyncWaiterQueueNode*>(it->get());
    if (!matcher(async_node)) {
      it++;
      continue;
    }
    Handle<JSAtomicsMutex> js_mutex = async_node->GetSynchronizationPrimitive();
    if (js_mutex->IsCurrentThreadOwner()) {
      js_mutex->Unlock(isolate);
    }
    it = async_locked_list.erase(it);
  }
}
...
Tagged<Object> JSSynchronizationPrimitive::NumWaitersForTesting(
  ...
  int num_waiters;
  {
    ...
    if (!HasWaitersField::decode(current_state)) {
      ...
      StateT desired;
      do {
        desired = IsWaiterQueueLockedField::update(expected, false);
        desired = HasWaitersField::update(desired, false);
    ...
    new_state = IsWaiterQueueLockedField::update(current_state, false);
...
void JSAtomicsMutex::CleanupMatchingAsyncWaiters(Isolate* isolate,
  ...
  if (waiter_head) {
    // It is safe to query the notify_task_id_ while holding the queue lock
    // because the Notify is only called while holding the queue lock.
    if (async_node->notify_task_id_ != CancelableTaskManager::kInvalidTaskId) {
      if (!IsLockedField::decode(current_state)) {
        // The node is not in the queue, so the HandleNotify task for it is
        // already in the event loop. Notify the next head only if the queue
        // is not empty.
        WaiterQueueNode* old_head = WaiterQueueNode::Dequeue(&waiter_head);
        old_head->Notify();
      }
    } else {
      // Dequeue and mark all the nodes that match the matcher for cleanup.
...
void JSAtomicsMutex::UnlockAsyncLockedMutex(
    Isolate* requester,
    MaybeHandle<AsyncLockedWaiterWrapper> maybe_async_locked_waiter_wrapper) {
  if (!maybe_async_locked_waiter_wrapper.is_null()) {
    Handle<AsyncLockedWaiterWrapper> async_locked_waiter_wrapper =
        maybe_async_locked_waiter_wrapper.ToHandleChecked();
    WaiterQueueNode* waiter_node = reinterpret_cast<WaiterQueueNode*>(
        async_locked_waiter_wrapper->DestructivelyGetAsyncLockedWaiter(
            requester));
    WaiterQueueNode::RemoveFromLockedWaiterList(waiter_node);
  }
  Unlock(requester);
}
...
// lock_async_promise being the last in the chain and the return value of the
...
//     lock_async_promise with the result of the callback.
// 3.2 The reject unlock handler, releases the lock and rejects
//     lock_async_promise with the error from the callback.

// static
Handle<JSPromise> JSAtomicsMutex::LockOrQueuePromise(
    Isolate* requester, Handle<JSAtomicsMutex> mutex,
    Handle<Object> run_under_lock, base::Optional<base::TimeDelta> timeout) {
  Handle<JSPromise> lock_promise = requester->factory()->NewJSPromise();
  Handle<JSPromise> callback_promise = PerformPromiseThen(
      requester, lock_promise, Handle<JSFunction>::cast(run_under_lock));
  Handle<JSPromise> lock_async_promise = requester->factory()->NewJSPromise();
  LockAsyncWaiterQueueNode* waiter_node = nullptr;
  bool locked = LockAsync(requester, mutex, lock_promise, lock_async_promise,
                          &waiter_node, timeout);
  if (locked) {
    // Create an AsyncWaiterQueueNode to be queued in the async locked waiter
    // queue.
    LockAsyncWaiterQueueNode::NewLockedWaiterInIsolate(requester, mutex);
  }
  SetAsyncUnlockHandlers(requester, mutex, callback_promise, lock_async_promise,
                         waiter_node);
  return lock_async_promise;
}
...
bool JSAtomicsMutex::LockAsync(Isolate* requester, Handle<JSAtomicsMutex> mutex,
  ...
  bool locked =
      LockImpl(requester, mutex, timeout, [=](std::atomic<StateT>* state) {
        return LockAsyncSlowPath(requester, mutex, state, promise,
                                 unlock_promise, waiter_node, timeout);
      });
  if (locked) {
...
Handle<JSPromise> JSAtomicsMutex::LockAsyncWrapperForWait(
  ...
  LockAsync(requester, mutex, lock_promise, MaybeHandle<JSPromise>(),
            &waiter_node);
  return lock_promise;
}
...
bool JSAtomicsMutex::LockAsyncSlowPath(
    ...
    Handle<JSPromise> promise, MaybeHandle<JSPromise> lock_async_promise,
    LockAsyncWaiterQueueNode** waiter_node,
    base::Optional<base::TimeDelta> timeout) {
  // Spin for a little bit to try to acquire the lock, so as to be fast under
  // microcontention.
  ...
  // `async_unlocked_waiter_queue_nodes` while they are waiting and by the
  // requester's `async_locked_waiter_queue_nodes` while they are holding the
  // lock.
  LockAsyncWaiterQueueNode* this_waiter =
      LockAsyncWaiterQueueNode::NewUnlockedWaiterInIsolate(
...
// static
void JSAtomicsMutex::HandleAsyncTimeout(LockAsyncWaiterQueueNode* node) {
  Isolate* requester = node->requester_;
  HandleScope scope(requester);
  v8::Context::Scope contextScope(node->GetNativeContext());
  Handle<JSAtomicsMutex> js_mutex = node->GetSynchronizationPrimitive();

  bool dequeued = JSAtomicsMutex::DequeueTimedOutAsyncWaiter(
      requester, js_mutex,
      node->GetSynchronizationPrimitive()->AtomicStatePtr(), node);
  ...
  if (!dequeued) {
    TryAbortResult abort_result =
        requester->cancelable_task_manager()->TryAbort(node->notify_task_id_);
    DCHECK_EQ(abort_result, TryAbortResult::kTaskAborted);
    USE(abort_result);
  }

  Handle<JSPromise> lock_promise = node->GetWaitingPromise();
...
// static
void JSAtomicsMutex::HandleAsyncNotify(LockAsyncWaiterQueueNode* node) {
  Isolate* requester = node->requester_;
  HandleScope scope(requester);
  v8::Context::Scope contextScope(node->GetNativeContext());
  Handle<JSAtomicsMutex> js_mutex = node->GetSynchronizationPrimitive();
  Handle<JSPromise> promise = node->GetWaitingPromise();
  bool locked = LockOrQueueAsyncNode(requester, js_mutex, node);
  if (locked) {
    if (node->timeout_task_id_ != CancelableTaskManager::kInvalidTaskId) {
      TryAbortResult abort_result =
          requester->cancelable_task_manager()->TryAbort(
              node->timeout_task_id_);
      DCHECK_EQ(abort_result, TryAbortResult::kTaskAborted);
      USE(abort_result);
    }
    if (node->lock_async_promise_.IsEmpty()) {
      // This node came from a lockAsync notify giving control back to an async
      // lock call, so we don't need to put the node in the locked waiter list
      // because the corresponding LockAsycWaiterQueueNode
      // is already in the locked waiter list.
      WaiterQueueNode::RemoveFromUnlockedWaiterList(node);
    } else {
      WaiterQueueNode::MoveToLockedWaiterList(node);
    }
...
WaiterQueueNode* JSAtomicsCondition::RemoveTimedOutWaiter(
    Handle<JSAtomicsCondition> cv, WaitAsyncWaiterQueueNode* async_waiter) {
  Isolate* requester = async_waiter->requester_;
  std::atomic<StateT>* state = cv->AtomicStatePtr();
...
Handle<JSPromise> JSAtomicsCondition::WaitAsync(
  ...
  Handle<SharedFunctionInfo> info(
      requester->heap()->atomics_condition_get_lock_sfi(), requester);
  ...
  Handle<JSPromise> lock_promise =
      PerformPromiseThen(requester, wait_promise, lock_function);

  // At this point the lock is considered contended, create a new async waiter
  // node in the C++ heap. It's lifetime is managed by the requester's
  // `async_unlocked_waiter_queue_nodes`.
  WaitAsyncWaiterQueueNode* this_waiter =
      WaitAsyncWaiterQueueNode::NewUnlockedWaiterInIsolate(requester, cv,
                                                           wait_promise);
...
// static
void JSAtomicsCondition::HandleAsyncTimeout(WaitAsyncWaiterQueueNode* node) {
  HandleScope scope(node->requester_);
  WaiterQueueNode* waiter = JSAtomicsCondition::RemoveTimedOutWaiter(
      node->GetSynchronizationPrimitive(), node);
  // If the waiter is not in the queue, the notify task is already in the event
  // loop, so cancel the notify task.
  if (!waiter) {
    TryAbortResult abort_result =
        node->requester_->cancelable_task_manager()->TryAbort(
            node->notify_task_id_);
    DCHECK_EQ(abort_result, TryAbortResult::kTaskAborted);
    USE(abort_result);
  }
...
// static
void JSAtomicsCondition::HandleAsyncNotify(WaitAsyncWaiterQueueNode* node) {
  Isolate* requester = node->requester_;
  HandleScope scope(requester);
  if (node->timeout_task_id_ != CancelableTaskManager::kInvalidTaskId) {
    TryAbortResult abort_result =
        requester->cancelable_task_manager()->TryAbort(node->timeout_task_id_);
    DCHECK_EQ(abort_result, TryAbortResult::kTaskAborted);
    USE(abort_result);
  }
  v8::Context::Scope contextScope(node->GetNativeContext());
