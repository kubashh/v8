...
void JSAtomicsMutex::CleanupMatchingAsyncWaiters(Isolate* isolate,
  ...
  if (waiter_head) {
    ...
    if (!async_node->ready_for_async_cleanup_) {
      // The node was not in the queue, so the HandleNotify task for it is
      // already in the event loop, we may need to notify the next head or
      // the queue may stall.
  ...
  if (!maybe_async_locked_waiter_wrapper.is_null()) {
    ...
    WaiterQueueNode* waiter_node = reinterpret_cast<WaiterQueueNode*>(
        async_locked_waiter_wrapper->DestructivelyGetAsyncLockedWaiter(
            requester));
    WaiterQueueNode::RemoveFromAsyncLockedWaiterList(waiter_node);
  }
  Unlock(requester);
}
...
void JSAtomicsMutex::HandleAsyncTimeout(LockAsyncWaiterQueueNode* waiter) {
  ...
  Handle<JSPromise> lock_promise = waiter->GetWaitingPromise();
  Handle<JSPromise> lock_async_promise = waiter->GetLockAsyncPromise();
...
void JSAtomicsMutex::HandleAsyncNotify(LockAsyncWaiterQueueNode* waiter) {
  ...
  v8::Context::Scope contextScope(waiter->GetNativeContext());
  Handle<JSAtomicsMutex> js_mutex = waiter->GetSynchronizationPrimitive();
  Handle<JSPromise> promise = waiter->GetWaitingPromise();
  bool locked = LockOrQueueAsyncNode(requester, js_mutex, waiter);
  if (locked) {
    ...
    if (waiter->lock_async_promise_.IsEmpty()) {
      ...
      WaiterQueueNode::RemoveFromAsyncUnlockedWaiterList(waiter);
    } else {
      WaiterQueueNode::MoveToAsyncLockedWaiterList(waiter);
    }
