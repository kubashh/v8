...
MaybeHandle<Object> ConcurrentLookupIterator::TryGetOwnCowElement(
  ...
  // Check against both JSArray::length and FixedArray::length. The former is
  // the source of truth, but due to concurrent reads it may not match the
  // given `array_elements`.
  ...
  // Filter out holes irrespective of the elements kind, which may not be
  // consistent with the given elements backing store.
