...
class MaybeObject : public ObjectImpl<HeapObjectReferenceType::WEAK> {
 public:
  constexpr MaybeObject() : ObjectImpl(kNullAddress) {}
  constexpr explicit MaybeObject(Address ptr) : ObjectImpl(ptr) {}

  // These operator->() overloads are required for handlified code.
  constexpr MaybeObject* operator->() { return this; }
