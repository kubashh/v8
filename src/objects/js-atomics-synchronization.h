...
class JSAtomicsMutex
 ...
 public:
  ...
  // Try to lock the mutex, if it's currently owned by another thread, queues a
  // LockAsyncWaiterQueueNode.
  // `waiter_node` should be nullptr only when this is called by a waitAsync
  // call returning control to the lockAsync callback. In such case, a new
  // waiter node will be allocated on the C++ stack if the lock is not acquired.
