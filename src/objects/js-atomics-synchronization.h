...
class JSSynchronizationPrimitive
 ...
 public:
  ...
  static void RealmDisposed(Isolate* isolate);
 ...
 protected:
  ...
  static inline void SetWaiterQueueStateOnly(std::atomic<StateT>* state,
                                             StateT new_state);
...
class JSAtomicsMutex
 ...
 public:
  ...
  void UnlockAsyncLockedMutex(
      Isolate* requester,
      MaybeHandle<AsyncLockedWaiterWrapper> async_locked_waiter_wrapper);
  ...
  enum {
    ...
    kUnlockPromiseAsyncContextSlot,
    kAsyncLockedWaiterSlot,
    kAsyncContextLength
  };
...
// The AsyncLockedWaiterWrapper is a wrapper around the WaiterQueueNode that
// is kept in the context of the async unlock handlers.
// Consider the following code:
// Atomics.Mutex.lockAsync(mutex, async function { await SomePromise() }),
// this will keep the lock held until SomePromise is resolved. It is possible
// to terminate the worker or realm where this was called, so we need to make
// sure to release the lock on isolate deinitialization and realm disposal. In
// order to implement this we repurpose the LockAsyncWaiterQueueNodes after the
// lock is taken and:
// 1. Keep a list of locked waiters in the isolate to release on cleanup.
// 2. Keep a reference to the waiter in the unlock promise so the waiter can
//    be removed from the list when the lock is released naturally.

class AsyncLockedWaiterWrapper
    : public TorqueGeneratedAsyncLockedWaiterWrapper<AsyncLockedWaiterWrapper,
                                                     Struct> {
 public:
  DECL_CAST(AsyncLockedWaiterWrapper)
  using BodyDescriptor = StructBodyDescriptor;

  inline WaiterQueueNode* DestructivelyGetAsyncLockedWaiter(Isolate* requester);
  inline void SetAsyncLockedWaiter(Isolate* requester, WaiterQueueNode* waiter);

  TQ_OBJECT_CONSTRUCTORS(AsyncLockedWaiterWrapper)
 ...
 private:
  ...
  // is stored in the external pointer table and the object itself only contains
  // a 32-bit external pointer handles.
  inline ExternalPointerHandle* async_locked_waiter_handle_location() const;
#else
  inline WaiterQueueNode** async_locked_waiter_location() const;
#endif
};
