...
class JSAtomicsMutex
 ...
 public:
  ...
  void UnlockAsyncLockedMutex(
      Isolate* requester,
      MaybeHandle<JSExternalObject> async_locked_waiter_wrapper);
  ...
  enum {
    ...
    // mutex is released.
    kLockAsyncPromiseAsyncContextSlot,
    // The isolate keeps a list of WaiterQueueNodes for each mutex locked
    // asynchronously, this is so that the lock can be released in case worker
    // termination. The kAsyncLockedWaiterSlot slot is used to store
    // a JSExternalObject wrapping aroung and ExternalPointerHandle (or raw
