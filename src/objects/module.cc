  ...
  if (FLAG_trace_module_status) {
    ...
    if (this->IsJSModule()) {
      Handle<Script> script(
          Handle<JSModule>::cast(Handle<Module>(*this, GetIsolate()))->script(),
          GetIsolate());
      os << " for ";
...
void Module::RecordError(Isolate* isolate) {
  ...
  if (this->IsJSModule()) {
    Handle<JSModule> self =
        Handle<JSModule>::cast(Handle<Module>(*this, GetIsolate()));
...
#endif  // DEBUG
  if (module->IsJSModule()) {
    JSModule::Reset(isolate, Handle<JSModule>::cast(module));
  } else {
    UNREACHABLE();
  }
...
MaybeHandle<Cell> Module::ResolveExport(Isolate* isolate, Handle<Module> module,
  ...
  if (module->IsJSModule()) {
    return JSModule::ResolveExport(isolate, Handle<JSModule>::cast(module),
                                   module_specifier, export_name, loc,
...
bool Module::Instantiate(Isolate* isolate, Handle<Module> module,
  ...
  if (FLAG_trace_module_status) {
    ...
    if (module->IsJSModule()) {
      Handle<JSModule>::cast(module)->script().GetNameOrSourceURL().Print(os);
    }
#ifndef OBJECT_PRINT
    os << "\n";
#endif  // OBJECT_PRINT
  ...
  if (!Module::PrepareInstantiate(isolate, module, context, callback)) {
    ResetGraph(isolate, module);
    return false;
  }
  Zone zone(isolate->allocator(), ZONE_NAME);
  ZoneForwardList<Handle<JSModule>> stack(&zone);
  unsigned dfs_index = 0;
  if (!Module::FinishInstantiate(isolate, module, &stack, &dfs_index, &zone)) {
...
bool Module::PrepareInstantiate(Isolate* isolate, Handle<Module> module,
                                ...
                                v8::Module::ResolveCallback callback) {
  if (module->IsJSModule()) {
    return JSModule::PrepareInstantiate(isolate, Handle<JSModule>::cast(module),
                                        context, callback);
  } else {
    UNREACHABLE();
  }
...
bool Module::FinishInstantiate(Isolate* isolate, Handle<Module> module,
  ...
  if (module->IsJSModule()) {
    return JSModule::FinishInstantiate(isolate, Handle<JSModule>::cast(module),
                                       stack, dfs_index, zone);
  } else {
    UNREACHABLE();
  }
}
...
MaybeHandle<Object> Module::Evaluate(Isolate* isolate, Handle<Module> module,
                                     ...
                                     unsigned* dfs_index) {
  if (module->IsJSModule()) {
    return JSModule::Evaluate(isolate, Handle<JSModule>::cast(module), stack,
                              dfs_index);
  } else {
    UNREACHABLE();
  }
...
Handle<JSModuleNamespace> Module::GetModuleNamespace(Isolate* isolate,
  ...
  if (module->IsJSModule())
    JSModule::FetchStarExports(isolate, Handle<JSModule>::cast(module), &zone,
                               &visited);
