...
template <class type>
inline constexpr std::optional<RootIndex> UniqueMapOfInstanceType() {
  return {};
}

#define INSTANCE_TYPE_MAP(V, rootIndexName, rootAccessorName, class_name) \
  template <>                                                             \
  inline constexpr std::optional<RootIndex>                               \
...
#define INSTANCE_TYPE_CHECKER2(type, forinstancetype)                          \
  V8_INLINE bool Is##type(Map map_object) {                                    \
    if (auto expected = UniqueMapOfInstanceType<InstanceTypeTraits::type>()) { \
      bool res = CheckInstanceMap(*expected, map_object);                      \
      SLOW_DCHECK(Is##type(map_object.instance_type()) == res);                \
      return res;                                                              \
    }                                                                          \
    if (auto range =                                                           \
            StaticReadOnlyRootRange(forinstancetype, forinstancetype)) {       \
...
#define INSTANCE_TYPE_CHECKER_RANGE2(type, first_instance_type,     \
                                     last_instance_type)            \
  V8_INLINE bool Is##type(Map map_object) {                         \
    if (auto range = StaticReadOnlyRootRange(first_instance_type,   \
                                             last_instance_type)) { \
...
V8_INLINE bool IsInternalizedString(Map map_object) {
  ...
  return CheckInstanceMapRange(
      *StaticReadOnlyRootRange(INTERNALIZED_STRING_TYPE,
