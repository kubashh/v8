...
template <class type>
inline constexpr std::pair<Tagged_t, Tagged_t> StaticMapRangeOfInstanceType() {
  return {kNullAddress, kNullAddress};
}
...
inline bool CheckInstanceMapRange(std::pair<Tagged_t, Tagged_t> range,
                                  Map map) {
...
template <>
inline constexpr std::pair<Tagged_t, Tagged_t>
StaticMapRangeOfInstanceType<InstanceTypeTraits::String>() {
  return {StaticReadOnlyRoot::kFirstStringMap,
          StaticReadOnlyRoot::kLastStringMap};
}

#else

inline bool CheckInstanceMap(Tagged_t expected, Map map) { UNREACHABLE(); }
inline bool CheckInstanceMapRange(std::pair<Tagged_t, Tagged_t> expected,
                                  Map map) {
  UNREACHABLE();
}
...
#define INSTANCE_TYPE_CHECKER_RANGE(type, first_instance_type,                \
  ...
  V8_INLINE bool Is##type(Map map_object) {                                   \
    auto expected = StaticMapRangeOfInstanceType<InstanceTypeTraits::type>(); \
    if (expected.first != kNullAddress) {                                     \
      return CheckInstanceMapRange(expected, map_object);                     \
    }                                                                         \
    return Is##type(map_object.instance_type());                              \
  }
...
#if V8_STATIC_ROOTS_BOOL
  return CheckInstanceMapRange({StaticReadOnlyRoot::kFirstInternalizedStringMap,
                                StaticReadOnlyRoot::kLastInternalizedStringMap},
                               map_object);
#else
...
#if V8_STATIC_ROOTS_BOOL
  return CheckInstanceMapRange({StaticReadOnlyRoot::kFirstFreeSpaceOrFillerMap,
                                StaticReadOnlyRoot::kLastFreeSpaceOrFillerMap},
                               map_object);
#else
