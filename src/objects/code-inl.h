...
DEF_GETTER(BytecodeArray, ConstantPool, FixedArray) {
  return FixedArray::cast(constant_pool());
}

DEF_GETTER(BytecodeArray, HandlerTable, ByteArray) {
  return ByteArray::cast(handler_table());
}
...
DEF_GETTER(BytecodeArray, SizeIncludingMetadata, int) {
  ...
  Object maybe_constant_pool = constant_pool(cage_base);
  ...
  Object maybe_handler_table = handler_table(cage_base);
