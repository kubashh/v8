...
void SwissNameDictionary::Rehash(Isolate* isolate) {
  ...
  for (int enum_index = 0; enum_index < UsedCapacity(); ++enum_index) {
    ...
    data[enum_index] = std::make_tuple(
        KeyAt(entry).ptr(), ValueAtRaw(entry).ptr(), DetailsAt(entry).ToByte());
  }
  ...
  for (D& triple : data) {
    ...
    Object value = Object(std::get<1>(triple));
    PropertyDetails details = PropertyDetails::FromByte(std::get<2>(triple));

    int new_entry = AddInternal(name, value, details);
