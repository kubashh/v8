...
bool DescriptorArrayMarkingState::TryUpdateIndicesToMark(
  ...
  while (true) {
    ...
    if (current_epoch != epoch_from_state) {
      ...
      const DescriptorIndex to_be_processed = already_marked + delta;
      if (to_be_processed >= index_to_mark) {
        return false;
      }
      new_raw_gc_state = NewState(current_epoch, already_marked,
                                  index_to_mark - to_be_processed);
    }
...
DescriptorArrayMarkingState::UpdateMarkedDescriptorsForProcessing(
  ...
  while (true) {
    ...
    // check are:
    // - Epoch mismatch;
    // - Epoch matches but marked/delta is 0;
