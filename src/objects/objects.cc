...
bool Script::GetPositionInfoInternal(const LineEndsContainer& ends,
                                     int position,
                                     Script::PositionInfo* info) const {
...
template bool Script::GetPositionInfoInternal<Tagged<FixedArray>>(
    const Tagged<FixedArray>& ends, int position,
    Script::PositionInfo* info) const;
...
bool Script::GetPositionInfo(int position, PositionInfo* info,
  ...
  if (!has_line_ends()) {
    ...
    if (!GetPositionInfoInternal(ends, position, info)) return false;
  }
...
bool Script::GetPositionInfoWithLineEnds(
    ...
    OffsetFlag offset_flag) const {
  if (!GetPositionInfoInternal(line_ends, position, info)) return false;
