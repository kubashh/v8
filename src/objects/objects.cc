...
bool HeapObject::NeedsRehashing(InstanceType instance_type) const {
  ...
  switch (instance_type) {
    ...
    case NAME_DICTIONARY_TYPE:
    case NAME_TO_INDEX_DICTIONARY_TYPE:
    case GLOBAL_DICTIONARY_TYPE:
    case NUMBER_DICTIONARY_TYPE:
    case SIMPLE_NUMBER_DICTIONARY_TYPE:
    case HASH_TABLE_TYPE:
...
void HeapObject::RehashBasedOnMap(IsolateT* isolate) {
  switch (map(isolate).instance_type()) {
    ...
    case NAME_DICTIONARY_TYPE:
      ...
      break;
    case NAME_TO_INDEX_DICTIONARY_TYPE:
      NameToIndexDictionary::cast(*this).Rehash(isolate);
      break;
    case SWISS_NAME_DICTIONARY_TYPE:
...
int32_t NameToIndexDictionary::Lookup(Handle<Name> key) {
  DisallowGarbageCollection no_gc;
...
Handle<NameToIndexDictionary> NameToIndexDictionary::Add(
    Isolate* isolate, Handle<NameToIndexDictionary> dictionary,
    Handle<Name> key, int32_t index) {
  DCHECK_GE(index, 0);
  // Validate that the key is absent.
  SLOW_DCHECK(dictionary->FindEntry(isolate, key).is_not_found());
  // Check whether the dictionary should be extended.
  dictionary = EnsureCapacity(isolate, dictionary);

  // Compute the key object.
  InternalIndex entry = dictionary->FindInsertionEntry(isolate, key->hash());
  dictionary->set(EntryToIndex(entry), *key);
  dictionary->set(EntryToIndex(entry) + 1, Smi::FromInt(index));
  dictionary->ElementAdded();
  return dictionary;
}
...
Object NameToIndexDictionary::ValueAt(InternalIndex entry) {
  return this->get(EntryToValueIndex(entry));
}
...
EXTERN_DEFINE_HASH_TABLE(NameToIndexDictionary, NameToIndexShape)
