...
inline void JSNumberFormat::set_significant_digits_are_set() {
  int hints = flags();
  hints = SignificantDigitsAreSetBits::update(hints, true);
  set_flags(hints);
}

inline bool JSNumberFormat::significant_digits_are_set() const {
  return SignificantDigitsAreSetBits::decode(flags());
}
