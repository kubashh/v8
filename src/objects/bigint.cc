...
MaybeHandle<BigInt> BigInt::Multiply(Isolate* isolate, Handle<BigInt> x,
  ...
  for (int i = 0; i < x->length(); i++) {
    ...
    // Multiplication can take a long time. Check for interrupt requests. We use
    // the unsafe accessor here because 1. the check must be fast, and 2. it's
    // perfectly fine to read a wrong value due to races now and then.
...
bool MutableBigInt::AbsoluteDivLarge(Isolate* isolate,
  ...
  for (int j = m; j >= 0; j--) {
    ...
    // Division can take a long time. Check for interrupt requests. We use
    // the unsafe accessor here because 1. the check must be fast, and 2. it's
    // perfectly fine to read a wrong value due to races now and then.
    if (isolate->stack_guard()->unsafe_has_pending_interrupts()) {
      StackLimitCheck interrupt_check(isolate);
...
MaybeHandle<String> MutableBigInt::ToStringGeneric(Isolate* isolate,
  ...
  if (length == 1) {
    ...
    do {
      ...
      // We use the unsafe accessor here because 1. the check must be fast,
      // and 2. it's perfectly fine to read a wrong value due to races now and
      // then.
      if (isolate->stack_guard()->unsafe_has_pending_interrupts()) {
        StackLimitCheck interrupt_check(isolate);
        if (interrupt_check.InterruptRequested()) {
