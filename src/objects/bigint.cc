...
Handle<BigInt> BigInt::AsIntN(uint64_t n, Handle<BigInt> x) {
  ...
  bool need_subtract;
  {
    ...
    if (static_cast<uint64_t>(result->length()) > power_length) {
      need_subtract = true;
    } else if (static_cast<uint64_t>(result->length()) < power_length) {
      need_subtract = false;
    } else {
      // TODO(neis): Ideally we shouldn't allocate here at all.
      need_subtract = AbsoluteCompare(result, PowerOfTwo(isolate, n - 1)) >= 0;
    }
  }

  if (!x->sign()) {
    // x is positive.  Note that result == x mod 2^n.
    if (!need_subtract) return result;
  ...
  //    need_subtract || (abs(result) == 2^n)
  // c) Performing the subtraction does the right thing even in the case where
  //    abs(result) == 2^n, so we can simplify a little bit and mirror the
  //    conditional of the positive case above.
  if (!need_subtract) {
