...
Handle<BigInt> BigInt::AsUintN(uint64_t n, Handle<BigInt> x) {
  ...
  // x is negative. We use the following facts:
  // Let r = -x % 2^n. Note that this is {result}.
  // If r == 0, then x % 2^n == 0.
  // If r != 0, then x % 2^n == 2^n - r.
  if (result->is_zero()) return result;
  return Subtract(PowerOfTwo(x->GetIsolate(), n), result);
}
...
Handle<BigInt> BigInt::AbsoluteAsUintN(uint64_t n, Handle<BigInt> x) {
  ...
  int needed_digits = (N / kDigitBits) + (N % kDigitBits != 0);
...
Handle<BigInt> BigInt::PowerOfTwo(Isolate* isolate, uint64_t n) {
  ...
  int needed_digits =
      (needed_bits / kDigitBits) + (needed_bits % kDigitBits != 0);
