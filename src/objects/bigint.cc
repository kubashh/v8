...
MaybeHandle<BigInt> MutableBigInt::LeftShiftByAbsolute(Isolate* isolate,
  ...
  const int result_length = bigint::LeftShiftByAbsolute_ResultLength(
  ...
  bigint::LeftShiftByAbsolute(GetRWDigits(result), GetDigits(x), shift);
  result->set_sign(x->sign());
  return MakeImmutable(result);
}
...
Handle<BigInt> MutableBigInt::RightShiftByAbsolute(Isolate* isolate,
  ...
  bigint::RightShiftByAbsoluteState state;
  const int result_length = bigint::RightShiftByAbsolute_ResultLength(
      GetDigits(x), sign, shift, &state);
  DCHECK_LE(result_length, length);
  if (result_length <= 0) {
    return RightShiftByMaximum(isolate, sign);
  }
  Handle<MutableBigInt> result = New(isolate, result_length).ToHandleChecked();
  bigint::RightShiftByAbsolute(GetRWDigits(result), GetDigits(x), shift, state);
