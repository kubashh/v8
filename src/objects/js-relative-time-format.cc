...
JSRelativeTimeFormat::InitializeRelativeTimeFormat(
  ...
  // Just some random non standard algorithm to decide the locale for now.
  // The following parts need to be replaced by code w/ bstell
  ...
  if (input_options->IsNull(isolate)) {
    // TypeError
    return MaybeHandle<JSRelativeTimeFormat>();
  } else if (input_options->IsUndefined(isolate)) {
...
Object* JSRelativeTimeFormat::ResolvedOptions(
