...
void Context::initialize_length(int len, bool expand) {
  DCHECK(LengthField::is_valid(len));
  set_length_and_extension_flag(
      HasExtensionField::update(LengthField::encode(len), expand));
}
...
void Context::set_extension(HeapObject object) {
  ...
  // TODO(victor): we temporaraly always set the extension bit
  set_length_and_extension_flag(
