...
class FormattedDuration {
 public:
  ...
  FormattedDuration(FormattedDuration&& src);
  virtual ~FormattedDuration() {}
  icu::UnicodeString toString(UErrorCode& status) const {
...
class DurationFormatInternal {
 public:
  DurationFormatInternal(
      Field smallest_unit, Field largest_unit,
  ...
  Field smallest_unit() const { return smallest_; }
  Field largest_unit() const { return largest_; }
...
class ListDurationFormat : public DurationFormatInternal {
 public:
  ListDurationFormat(
      Field smallest_unit, Field largest_unit,
      icu::ListFormatter* list_formatter,
      const icu::number::LocalizedNumberFormatter& number_formatter,
      UErrorCode& status)
      : DurationFormatInternal(smallest_unit, largest_unit, number_formatter),
...
Maybe<bool> FieldsToUnits(Isolate* isolate, Handle<Object> fields,
                          Field* smallest_unit, Field* largest_unit) {
  bool fields_bool[Field::kCount];
  // If fields is undefined, then
  if (fields->IsUndefined(isolate)) {
    for (int i = 0; i < Field::kCount; i++) {
      fields_bool[i] = true;
    }
  } else {
    ...
    if (is_array.FromJust()) {
      ...
      std::map<std::string, Field> kProperties = {
          {"years", Field::kYears},
          {"months", Field::kMonths},
          {"weeks", Field::kWeeks},
          {"days", Field::kDays},
          {"hours", Field::kHours},
          {"minutes", Field::kMinutes},
          {"seconds", Field::kSeconds},
          {"milliseconds", Field::kMilliseconds},
          {"microseconds", Field::kMicroseconds},
          {"nanoseconds", Field::kNanoseconds}};
  ...
  std::string units;
  *largest_unit = Field::kNanoseconds;
  for (int i = Field::kYears; i < Field::kCount; i++) {
    if (fields_bool[i]) {
      *largest_unit = static_cast<Field>(i);
      break;
    }
  }
  *smallest_unit = Field::kYears;
  for (int i = Field::kNanoseconds; i >= Field::kYears; i--) {
    if (fields_bool[i]) {
      *smallest_unit = static_cast<Field>(i);
      break;
    }
  }
  return Just(true);
...
Handle<JSArray> GetFields(Isolate* isolate, Field smallest_unit,
                          Field largest_unit) {
  Factory* factory = isolate->factory();
