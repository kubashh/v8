...
bool EmbedderDataSlot::store_aligned_pointer(Isolate* isolate, void* ptr) {
  ...
  // external pointer handle, and only once an external pointer is stored in
  // them are they properly initialized.
  Address slot_address = address() + kExternalPointerOffset;
  if (!IsLazilyInitializedExternalPointerFieldInitialized(slot_address)) {
    InitExternalPointerField<kEmbedderDataSlotPayloadTag>(slot_address, isolate,
                                                          value);
  } else {
    WriteExternalPointerField<kEmbedderDataSlotPayloadTag>(slot_address,
                                                           isolate, value);
  }
