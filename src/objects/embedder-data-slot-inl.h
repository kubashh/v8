...
bool EmbedderDataSlot::GetAndClearExternalPointerForSerialization(
    ExternalPointer_t* out_ptr, const DisallowGarbageCollection& no_gc) {
  auto* location =
      reinterpret_cast<ExternalPointer_t*>(address() + kExternalPointerOffset);
#ifdef V8_COMPRESS_POINTERS
  ExternalPointerHandle handle = base::AsAtomic32::Relaxed_Load(location);
  if (handle == kNullExternalPointerHandle) return false;
  base::AsAtomic32::Relaxed_Store(location, kNullExternalPointerHandle);
  *out_ptr = handle;
#else   // !V8_COMPRESS_POINTERS
  Address raw_value = *location;
  if (raw_value == kNullExternalPointer) return false;
  if (!HAS_SMI_TAG(raw_value)) return false;
  *out_ptr = raw_value;
  *location = kNullExternalPointer;
#endif  // !V8_COMPRESS_POINTERS
  DCHECK_EQ(*location, kNullExternalPointer);
  return true;
}

void EmbedderDataSlot::RestoreExternalPointerAfterSerialization(
    ExternalPointer_t ptr, const DisallowGarbageCollection& no_gc) {
  auto* location =
      reinterpret_cast<ExternalPointer_t*>(address() + kExternalPointerOffset);
  DCHECK_NE(ptr, kNullExternalPointer);
  DCHECK_EQ(*location, kNullExternalPointer);
#ifdef V8_COMPRESS_POINTERS
  // Release store, as needed for lazily initialized slots.
  base::AsAtomic32::Release_Store(location, ptr);
#else   // !V8_COMPRESS_POINTERS
