...
MaybeHandle<JSTemporalPlainTime> ToTemporalTime(Isolate* isolate,
  ...
  if (item_obj->IsJSReceiver()) {
    ...
    if ((result.calendar->length() > 0) &&
        !result.calendar->Equals(*(isolate->factory()->iso8601_string()))) {
      // i. Throw a RangeError exception.
      THROW_NEW_ERROR(isolate, NEW_TEMPORAL_INVALD_ARG_RANGE_ERROR(),
                      JSTemporalPlainTime);
    }
...
MaybeHandle<JSTemporalZonedDateTime> ToTemporalZonedDateTime(
  ...
  if (item_obj->IsJSReceiver()) {
    ...
    CHECK_GT(result2.time_zone_name->length(), 0);

    // e. Let offsetString be result.[[TimeZoneOffsetString]].
    offset_string = result2.offset_string;
    ...
    } else if (result2.offset_string->length() == 0) {
      // i. Set offsetBehaviour to wall.
      offset_behaviour = OffsetBehaviour::kWall;
    }
    ...
    ASSIGN_RETURN_ON_EXCEPTION(
        isolate, time_zone,
        temporal::CreateTemporalTimeZone(isolate, result2.time_zone_name),
        JSTemporalZonedDateTime);

    Handle<Object> calendar_string;
    if (result2.calendar->length() == 0) {
      calendar_string = factory->undefined_value();
    } else {
      calendar_string = result2.calendar;
    }
...
MaybeHandle<Object> ToRelativeTemporalObject(Isolate* isolate,
  ...
  if (value_obj->IsJSReceiver()) {
    ...
    if (relative_result.calendar->length() == 0) {
      result_calendar = factory->undefined_value();
    } else {
      result_calendar = relative_result.calendar;
    }
    ASSIGN_RETURN_ON_EXCEPTION(
        isolate, calendar,
    ...
    if (relative_result.offset_string->length() > 0) {
      offset_string_obj = relative_result.offset_string;
    }

    // e. Let timeZone be result.[[TimeZoneIANAName]].
    if (relative_result.time_zone_name->length() > 0) {
      time_zone_obj = relative_result.time_zone_name;
    }
    // f. If result.[[TimeZoneZ]] is true, then
...
Maybe<DateRecord> ParseTemporalMonthDayString(Isolate* isolate,
  ...
  DateRecord ret({year, month, day, isolate->factory()->empty_string()});
  return Just(ret);
}
...
Maybe<ZonedDateTimeRecord> ParseTemporalRelativeToString(
  ...
  ZonedDateTimeRecord record;
  if (maybe_parsed_time_zone.IsJust()) {
    ...
    record.offset_string = isolate->factory()->empty_string();
    // c. Let timeZone be undefined.
    record.time_zone_name = isolate->factory()->empty_string();
  }
...
#define GET_PROP(name)                                                        \
  {                                                                           \
    ...
    if (!value->IsUndefined()) {                                              \
      ...
      ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, number,                             \
...
MaybeHandle<JSTemporalDuration> JSTemporalDuration::With(
  ...
  Maybe<bool> maybe_partial = temporal::ToPartialDuration(
    isolate, temporal_duration_like, &years, &months, &weeks, &days,
