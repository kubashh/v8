...
Maybe<DateTimeRecord> ParseTemporalDateTimeString(Isolate* isolate,
  ...
  // (see 13.33), then
  bool satisfy = false;
  Maybe<ParsedResult> maybe_parsed =
  ...
  // 4. Return result.
  return ParseISODateTime(isolate, maybe_parsed.FromJust());
}
...
Maybe<DateRecord> ParseTemporalDateString(Isolate* isolate,
  ...
  // 3. Let result be ? ParseISODateTime(isoString).
  Maybe<DateTimeRecord> maybe_result =
      ParseISODateTime(isolate, maybe_parsed.FromJust());
...
Maybe<DateRecord> ParseTemporalMonthDayString(Isolate* isolate,
  ...
  ParsedResult parsed = maybe_parsed.FromJust();
  // 3. Let result be ? ParseISODateTime(isoString).
...
Maybe<std::string> ParseTemporalCalendarString(Isolate* isolate,
  ...
  Maybe<ParsedResult> maybe_parsed =
      TemporalParser::ParseTemporalCalendarString(isolate, iso_string,
                                                  &satisfy);
  MAYBE_RETURN(maybe_parsed, Nothing<std::string>());
  if (!satisfy) {
    THROW_NEW_ERROR_RETURN_VALUE(isolate, NEW_TEMPORAL_INVALD_ARG_RANGE_ERROR(),
                                 Nothing<std::string>());
  }
...
Maybe<ZonedDateTimeRecord> ParseTemporalRelativeToString(
  ...
  // (see 13.33), then
  satisfy = false;
  Maybe<ParsedResult> maybe_parsed_time_zone =
      TemporalParser::ParseTemporalZonedDateTimeString(isolate, iso_string,
                                                       &satisfy);
  ParsedResult parsed_time_zone = maybe_parsed_time_zone.FromJust();
