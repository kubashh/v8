...
MaybeHandle<JSReceiver> GetTemporalCalendarWithISODefault(
  ...
  if (item->IsJSTemporalPlainDate()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalPlainDate>::cast(item)->calendar(), isolate);
  }
  if (item->IsJSTemporalPlainDateTime()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalPlainDateTime>::cast(item)->calendar(), isolate);
  }
  if (item->IsJSTemporalPlainMonthDay()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalPlainMonthDay>::cast(item)->calendar(), isolate);
  }
  if (item->IsJSTemporalPlainTime()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalPlainTime>::cast(item)->calendar(), isolate);
  }
  if (item->IsJSTemporalPlainYearMonth()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalPlainYearMonth>::cast(item)->calendar(), isolate);
  }
  if (item->IsJSTemporalZonedDateTime()) {
    return Handle<JSReceiver>(
        Handle<JSTemporalZonedDateTime>::cast(item)->calendar(), isolate);
  }
