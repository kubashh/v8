  ...
  if (from_result2) {
    ASSIGN_RETURN_ON_EXCEPTION(
        ...
        InterpretISODateTimeOffset(
            isolate, result1.year, result2.month, result2.day, result2.hour,
...
MaybeHandle<JSTemporalInstant> ToTemporalInstant(Isolate* isolate,
  ...
  if (item->IsJSTemporalInstant()) {
    // i. Return item.
    return Handle<JSTemporalInstant>(JSTemporalInstant::cast(*item), isolate);
  }
...
MaybeHandle<String> TemporalInstantToString(Isolate* isolate,
  ...
  Handle<JSTemporalTimeZone> output_time_zone;
  ...
  if (time_zone_obj->IsUndefined()) {
    ...
    CHECK(time_zone_obj->IsJSTemporalTimeZone());
    output_time_zone = Handle<JSTemporalTimeZone>::cast(time_zone_obj);
  }
...
MaybeHandle<BigInt> DifferenceInstant(Isolate* isolate, Handle<BigInt> ns1,
  ...
  Handle<BigInt> diff;
  ASSIGN_RETURN_ON_EXCEPTION(isolate, diff, BigInt::Subtract(isolate, ns1, ns2),
                             BigInt);
...
MaybeHandle<JSTemporalDuration> UntilOrSince(
  ...
  Handle<JSTemporalPlainDateTime> first = (sign == -1) ? other : date_time;
  Handle<JSTemporalPlainDateTime> second = (sign == -1) ? date_time : other;
  ...
  round_result.days *= sign;
  round_result.hours *= sign;
  round_result.minutes *= sign;
  round_result.seconds *= sign;
  round_result.milliseconds *= sign;
  round_result.microseconds *= sign;
  round_result.nanoseconds *= sign;
  Maybe<bool> maybe_result = BalanceDuration(
      isolate, &round_result.days, &round_result.hours, &round_result.minutes,
      &round_result.seconds, &round_result.milliseconds,
      &round_result.microseconds, &round_result.nanoseconds, largest_unit);
  MAYBE_RETURN(maybe_result, Handle<JSTemporalDuration>());
  CHECK(maybe_result.FromJust());
  ...
  return temporal::CreateTemporalDuration(
      isolate, sign * round_result.years, sign * round_result.months,
      sign * round_result.weeks, round_result.days, round_result.hours,
      round_result.minutes, round_result.seconds, round_result.milliseconds,
      round_result.microseconds, round_result.nanoseconds);
}
