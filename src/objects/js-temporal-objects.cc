...
MaybeHandle<Object> ToRelativeTemporalObject(Isolate* isolate,
  ...
  if (value_obj->IsJSReceiver()) {
    ...
    if (!time_zone_name.is_null()) {
      ...
      ASSIGN_RETURN_ON_EXCEPTION(
          isolate, time_zone,
          temporal::CreateTemporalTimeZone(isolate, time_zone_name), Object);
      time_zone_obj = time_zone;
      // g. Else,
    } else {
...
Handle<BigInt> RoundTemporalInstant(Isolate* isolate, Handle<BigInt> ns,
  ...
  UNIMPLEMENTED();  // will be check in by
                    // https://chromium-review.googlesource.com/c/v8/v8/+/3566671
}
...
Maybe<DurationRecord> AddDuration(Isolate* isolate, const DurationRecord& dur1,
  ...
  if (relative_to_obj->IsUndefined()) {
    ...
    // result.[[Nanoseconds]]).
    return CreateDurationRecord(isolate, {0, 0, 0, result.time_duration});
    ...
    return CreateDurationRecord(
        isolate,
        {date_difference->years().Number(), date_difference->months().Number(),
  ...
  if (!(largest_unit == Unit::kYear || largest_unit == Unit::kMonth ||
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(
        ...
        DifferenceInstant(isolate, handle(relative_to->nanoseconds(), isolate),
                          end_ns, 1, Unit::kNanosecond,
                          RoundingMode::kHalfExpand),
        Nothing<DurationRecord>());
    // ii. Let result be ! BalanceDuration(0, 0, 0, 0, 0, 0, diffNs,
    // largestUnit).
    // Note: We may have issue with int64_t here.
    bool loss_less;
    int64_t diff_ns_int = diff_ns->AsInt64(&loss_less);
    CHECK(!loss_less);
    MAYBE_ASSIGN_RETURN_ON_EXCEPTION_VALUE(
        isolate, result.time_duration,
        BalanceDuration(isolate, largest_unit,
                        {0, 0, 0, 0, 0, 0, static_cast<double>(diff_ns_int)},
                        method_name),
        Nothing<DurationRecord>());
