...
MaybeHandle<JSTemporalDuration> CreateTemporalDuration(
  ...
  Handle<Object> years_obj = factory->NewNumberFromInt64(years);
  Handle<Object> months_obj = factory->NewNumberFromInt64(months);
  Handle<Object> weeks_obj = factory->NewNumberFromInt64(weeks);
  Handle<Object> days_obj = factory->NewNumberFromInt64(days);
  Handle<Object> hours_obj = factory->NewNumberFromInt64(hours);
  Handle<Object> minutes_obj = factory->NewNumberFromInt64(minutes);
  Handle<Object> seconds_obj = factory->NewNumberFromInt64(seconds);
  Handle<Object> milliseconds_obj = factory->NewNumberFromInt64(milliseconds);
  Handle<Object> microseconds_obj = factory->NewNumberFromInt64(microseconds);
  Handle<Object> nanoseconds_obj = factory->NewNumberFromInt64(nanoseconds);
...
Maybe<DurationRecord> DifferenceZonedDateTime(
  ...
  Maybe<DurationRecord> maybe_date_difference = DifferenceISODateTime(
      ...
      calendar, largest_unit, factory->undefined_value(), method);
...
Maybe<bool> NanosecondsToDays(Isolate* isolate, Handle<BigInt> nanoseconds,
  ...
  Maybe<DurationRecord> maybe_date_difference = DifferenceISODateTime(
      ...
      end_date_time->iso_second(), end_date_time->iso_millisecond(),
      end_date_time->iso_microsecond(), end_date_time->iso_nanosecond(),
      calendar, Unit::kDay, factory->undefined_value(), method);
...
MaybeHandle<JSTemporalPlainDate> JSTemporalCalendar::DateAdd(
  ...
  Maybe<bool> maybe_result = AddISODate(
      ...
      NumberToInt64(duration->years()), NumberToInt64(duration->months()),
      NumberToInt64(duration->weeks()), days, maybe_overflow.FromJust(), &year,
      &month, &day);
  MAYBE_RETURN(maybe_result, Handle<JSTemporalPlainDate>());
  CHECK(maybe_result.FromJust());
...
#define TO_ISO(name, Name)                                                     \
  MaybeHandle<Smi> JSTemporalCalendar::Name(                                   \
    ...
    return Handle<Smi>(                                                        \
        Smi::FromInt(ToISO##Name(isolate, date->iso_year(), date->iso_month(), \
                                 date->iso_day())),                            \
        isolate);                                                              \
  }
...
MaybeHandle<Smi> JSTemporalCalendar::DaysInMonth(
  ...
  return Handle<Smi>(Smi::FromInt(ISODaysInMonth(isolate, iso_year, iso_month)),
                     isolate);
}
...
MaybeHandle<Smi> JSTemporalCalendar::DaysInYear(
  ...
  return Handle<Smi>(Smi::FromInt(ISODaysInYear(isolate, iso_year)), isolate);
}
...
MaybeHandle<Oddball> JSTemporalCalendar::InLeapYear(
  ...
  Factory* factory = isolate->factory();
  return IsISOLeapYear(isolate, iso_year) ? factory->true_value()
                                          : factory->false_value();
}
...
MaybeHandle<JSArray> GetIANATimeZoneEpochValueAsArrayOfInstant(
  ...
  if (time_zone_index == 0) {
    ...
    // TODO(ftang) need to handle the size 0 and 2 case for the exact moment of
    // transition later.
    fixed_array = factory->NewFixedArray(1);
    Handle<BigInt> time_in_milliseconds;
    ASSIGN_RETURN_ON_EXCEPTION(
    ...
    Maybe<int64_t> maybe_offset_in_milliseconds =
        Intl::GetTimeZoneOffsetMilliseconds(isolate, time_zone_index,
                                            time_in_milliseconds->AsInt64());
    MAYBE_RETURN(maybe_offset_in_milliseconds, Handle<JSArray>());
    int64_t offset_in_milliseconds = maybe_offset_in_milliseconds.FromJust();
    ...
    ASSIGN_RETURN_ON_EXCEPTION(
        isolate, epoch_nanoseconds,
        BigInt::Subtract(isolate, nanoseconds_in_local, offset_in_nanoseconds),
        JSArray);
