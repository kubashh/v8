...
class SourceTextModule::AsyncParentCompletionSet
    : public std::set<Handle<SourceTextModule>,
                      SourceTextModule::AsyncEvaluatingOrdinalCompare,
                      ZoneAllocator<Handle<SourceTextModule>>> {
 public:
  explicit AsyncParentCompletionSet(Zone* zone)
      : std::set<Handle<SourceTextModule>, AsyncEvaluatingOrdinalCompare,
                 ZoneAllocator<Handle<SourceTextModule>>>(
            AsyncEvaluatingOrdinalCompare(),
            ZoneAllocator<Handle<SourceTextModule>>(zone)),
        zone_(zone) {}
  ...
  for (int i = 0; i < module->AsyncParentModuleCount(); i++) {
    ...
    if (exec_list->find(m) == exec_list->end() &&
      ...
      if (!m->HasPendingAsyncDependencies()) {
        ...
        if (!m->async()) GatherAsyncParentCompletions(isolate, m, exec_list);
      }
    }
  }
...
void SourceTextModule::AsyncModuleExecutionFulfilled(
  ...
  // 4. Let execList be a new empty List.
  Zone zone(isolate->allocator(), ZONE_NAME);
  AsyncParentCompletionSet exec_list(&zone);

  // 5. Perform ! GatherAsyncParentCompletions(module, _execList).
  GatherAsyncParentCompletions(isolate, module, &exec_list);

  // 6. Let sortedExecList be a List of elements that are the elements of
  ...
  Handle<SourceTextModule> top_level_parent = module;

  // 7. For each Module m of sortedExecList, do
  for (Handle<SourceTextModule> m : exec_list) {
    ...
    if (!m->IsAsyncEvaluating()) {
      ...
      if (!ExecuteModule(isolate, m).ToHandle(&unused_result)) {
        ...
        // c. Otherwise, set m.[[AsyncEvaluating]] to false.
  ...
  // TODO(syg): There's a spec bug here.

  // 8. If module.[[TopLevelCapability]] is not undefined, then
  if (!top_level_parent->top_level_capability().IsUndefined(isolate)) {
    //  a. Assert: module.[[DFSIndex]] is equal to module.[[DFSAncestorIndex]].
    DCHECK_EQ(top_level_parent->dfs_index(),
              top_level_parent->dfs_ancestor_index());

    //  b. Perform ! Call(module.[[TopLevelCapability]].[[Resolve]],
    //                    undefined, «undefined»).
    Handle<JSPromise> capability(
        JSPromise::cast(top_level_parent->top_level_capability()), isolate);
    JSPromise::Resolve(capability, isolate->factory()->undefined_value())
        .ToHandleChecked();
  }
...
MaybeHandle<Object> SourceTextModule::InnerModuleEvaluation(
  ...
  // 14. If module.[[PendingAsyncDependencies]] > 0 or _module_.[[Async]] is
  //     true, then
