  ...
  for (DirectHandle<SourceTextModule> m : exec_list) {
    ...
    if (m->async()) {
      ...
      if (!ExecuteModule(isolate, m, &exception).ToHandle(&unused_result)) {
        ...
        // (Even though the spec does not set [[AsyncEvaluation]] to false
        // because it doesn't use the field again for this module, the ordinal
        // still needs to be reset because we also use the [[AsyncEvaluation]]
        // field to find stalled modules. See GetStalledTopLevelAwaitMessages.)
...
void SourceTextModule::AsyncModuleExecutionRejected(
  ...
  // 1. If module.[[Status]] is EVALUTED, then
  if (module->status() == kErrored) {
    // a. Assert: module.[[EvaluationError]] is not empty.
    DCHECK(!IsTheHole(module->exception(), isolate));
    // b. Return UNUSED.
