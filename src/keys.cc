...
void FilterForEnumerableProperties(Handle<JSReceiver> receiver,
  ...
  for (uint32_t i = 0; i < length; i++) {
    ...
    if (type == kIndexed) {
      ...
      attributes = args.CallIndexedPropertyQuery(interceptor->query(), number);
    } else {
      CHECK(element->IsName());
      attributes = args.CallNamedPropertyQuery(interceptor->query(),
...
Maybe<bool> CollectInterceptorKeysInternal(Handle<JSReceiver> receiver,
  ...
  if (!interceptor->enumerator()->IsUndefined(isolate)) {
    if (type == kIndexed) {
      result =
          enum_args.CallIndexedPropertyEnumerator(interceptor->enumerator());
    } else {
      DCHECK_EQ(type, kNamed);
      result = enum_args.CallNamedPropertyEnumerator(interceptor->enumerator());
    }
  }
