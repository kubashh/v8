...
// Sending multiple notifications per file would reset previously sent
// diagnostics. Those, two steps are needed:
...
class DiagnosticCollector {
 public:
  void AddTorqueError(const TorqueError& error) {
    ...
    auto& notification = GetNotificationForSource(id);
  ...
  void AddLintError(const LintError& error) {
    auto& notification = GetNotificationForSource(error.position.source);
 ...
 private:
  PublishDiagnosticsNotification& GetNotificationForSource(SourceId id) {
