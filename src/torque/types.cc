...
std::vector<RuntimeType> AbstractType::GetRuntimeTypes() const {
  if (this == TypeOracle::GetWeakHeapObjectType()) {
    return {{"MaybeObject", "HeapObject"}};
  }
...
void ClassType::SetInstanceTypes(
...
void ComputeSlotKindsHelper(std::vector<ObjectSlotKind>* slots,
  ...
  for (const Field& field : fields) {
    ...
    offset += std::get<0>(field.GetFieldSizeInformation());
  }
}
}  // namespace
...
std::vector<ObjectSlotKind> ClassType::ComputeHeaderSlotKinds() const {
  ...
  DCHECK_EQ(header_size() / TargetArchitecture::TaggedSize(), result.size());
  return result;
}
