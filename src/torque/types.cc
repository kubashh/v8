...
bool StructType::HasConstFields() const {
  return GetConstexprValueCount() != 0;
}
    ...
    if (field.name_and_type.name == fieldname) {
      ...
      if (field_constexpr_count != 0) {
        ...
        if (field_type->IsConstexpr()) {
          std::cout << type->name() << "." << field.name_and_type.name
                    << std::endl;
...
VisitResult::VisitResult(const Type* type, std::string constexpr_value)
    : type_(type) {
  constexpr_values_.push_back(constexpr_value);
  DCHECK(type->IsConstexpr());
  DCHECK(!type->IsStructType());
}
VisitResult::VisitResult(const Type* type, StackRange stack_range)
