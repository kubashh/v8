...
VisitResult ImplementationVisitor::Visit(NumberLiteralExpression* expr) {
  ...
  Type result_type = LookupType(expr->pos, CONST_FLOAT64_TYPE_STRING);
  if (i == d) {
    if (Internals::IsValidSmi(i)) {
      if (sizeof(void*) == sizeof(double) && ((i >> 30) != (i >> 31))) {
        result_type = LookupType(expr->pos, CONST_INT32_TYPE_STRING);
      } else {
        result_type = LookupType(expr->pos, CONST_INT31_TYPE_STRING);
      }
    }
  }
