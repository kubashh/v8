...
void ImplementationVisitor::BeginDebugMacrosFile() {
  ...
  source << "#define READ_FIELD_OR_FAIL(Type, destination, accessor, object, "
            "offset) \\\n"
         << "  do { \\\n"
         << "    Type value{}; \\\n"
         << "    d::MemoryAccessResult validity = accessor( \\\n"
         << "      object - kHeapObjectTag + offset, \\\n"
  ...
  source << "#define READ_TAGGED_FIELD_OR_FAIL(destination, accessor, object, "
            "offset) \\\n"
         << "  do { \\\n"
         << "    Tagged_t value{}; \\\n"
         << "    d::MemoryAccessResult validity = accessor( \\\n"
  ...
  source << "#define ASSIGN_OR_RETURN(dest, val)\\\n"
         << "  do {\\\n"
         << "    if ((val).validity != d::MemoryAccessResult::kOk) return "
            "{(val).validity, {}};\\\n"
         << "    dest = (val).value;\\\n"
         << "  } while (false) \n";
  source << "\n";
...
void ImplementationVisitor::VisitMacroCommon(Macro* macro) {
  ...
  if (output_type_ == OutputType::kCC) {
    ...
    // TODO(duongn): Use correct generator.
