...
LocationReference ImplementationVisitor::GenerateFieldReference(
  ...
  if (field.offset) {
    ...
    for (const Field& f : class_type->ComputeAllFields()) {
      if (f.offset) {
        offset = VisitResult(TypeOracle::GetConstInt31Type(),
...
VisitResult ImplementationVisitor::GenerateArrayLength(VisitResult object,
  ...
  CurrentScope::Scope current_scope_scope(class_type->nspace());
  ...
  for (Field f : class_type->ComputeAllFields()) {
    ...
    field_bindings.Add(
        ...
        LocalValue{GenerateFieldReference(object, f, class_type)}, true);
  }
  return stack_scope.Yield(GenerateArrayLength(*field.index));
}
...
LayoutForInitialization ImplementationVisitor::GenerateLayoutForInitialization(
  ...
  if (class_type->size().KnownValue()) {
    layout.size = VisitResult(TypeOracle::GetConstInt31Type(),
                              ToString(*class_type->size().KnownValue()));
  } else {
    layout.size = offset;
  }
  if ((1 << class_type->size().AlignmentLog2()) <
...
VisitResult ImplementationVisitor::Visit(NewExpression* expr) {
  ...
  return stack_scope.Yield(
      GenerateCall("%RawDownCast", {{allocate_result}, {}}, {class_type}));
}
