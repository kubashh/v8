...
VisitResult ImplementationVisitor::Visit(NewExpression* expr) {
  ...
  if (class_type->IsExtern()) {
    ...
    base::Optional<const Field*> index_field;
    for (auto f : class_type->fields()) {
      if (f.index) {
        index_field = f.index;
      }
    }
    auto index_result = initializer_results.results.rbegin();
