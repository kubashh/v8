...
base::Optional<std::string> MatchSimpleBodyDescriptor(const ClassType* type) {
  ...
  // We pick a flexible body descriptor even in cases where the object size is
  // fixed, to reduce the amount of code executed for object visitation.
  if (end_index == slots.size()) {
    return ToString("Flexible", has_weak_pointers ? "Weak" : "",
                    "BodyDescriptor<", start_offset, ">");
  }
  if (!has_weak_pointers) {
    return ToString("FixedBodyDescriptor<", start_offset, ", ", end_offset,
