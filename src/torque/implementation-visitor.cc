  ...
  if (can_return) {
    ...
    if (!return_type->IsConstexpr()) {
      if (return_type->IsStructType()) {
        ...
        const_values.resize(return_type->GetConstexprValueCount());
  ...
  if (current_callable->HasReturnValue()) {
    ...
    if (current_callable->IsMacro()) {
      if (return_result.IsOnStack()) {
        ...
        if (return_result.HasConstexprValues() ||
            return_result.type()->IsStructType()) {
...
VisitResult ImplementationVisitor::TemporaryUninitializedStruct(
  ...
  for (const Field& f : struct_type->fields()) {
    if (const StructType* struct_type =
      ...
      VisitResult field_struct_result =
          TemporaryUninitializedStruct(struct_type, reason);
      range.Extend(field_struct_result.stack_range());
      if (field_struct_result.HasConstexprValues()) {
        for (auto value : field_struct_result.constexpr_values()) {
          constexpr_values.push_back(value);
        }
      }
    } else {
  ...
  return VisitResult(struct_type, range, constexpr_values);
}
...
VisitResult ImplementationVisitor::InitializeAggregate(
  ...
  if (constexpr_values.size() == 0) {
    if (aggregate_type->IsStructType()) {
...
VisitResult ImplementationVisitor::GenerateCopy(const VisitResult& to_copy) {
  if (to_copy.IsOnStack()) {
    if (to_copy.HasConstexprValues() || to_copy.type()->IsStructType()) {
      return VisitResult(
          to_copy.type(),
          assembler().Peek(to_copy.stack_range(), to_copy.type()),
          to_copy.constexpr_values());
    } else {
      return VisitResult(to_copy.type(), assembler().Peek(to_copy.stack_range(),
                                                          to_copy.type()));
    }
  }
  return to_copy;
...
std::pair<StackRange, std::vector<std::string>>
ImplementationVisitor::LowerParameter(
  ...
  if (type->IsConstexpr()) {
    ...
    for (auto& field : struct_type->fields()) {
      StackRange parameter_range = lowered_parameters->TopRange(0);
      std::vector<std::string> constexpr_values;
