...
VisitResult ImplementationVisitor::InlineMacro(
  ...
  if (can_return) {
    ...
    if (!return_type->IsConstexpr()) {
      ...
      size_t const_value_count = return_type->GetConstexprValueCount();
      for (size_t i = 0; i < const_value_count; ++i) {
        const_values.push_back("dummy");
      }
...
VisitResult ImplementationVisitor::TemporaryUninitializedStruct(
  ...
  std::vector<std::string> const_values;
  size_t const_value_count = struct_type->GetConstexprValueCount();
  for (size_t i = 0; i < const_value_count; ++i) {
    const_values.push_back("dummy");
  }
  return VisitResult(struct_type, range, const_values);
}
...

  {
    ...
    for (size_t i = 0; i < parameter_count; ++i) {
      ...
      if (type->IsStructType() && type->GetConstexprValueCount() != 0) {
        ReportError("struct label parameters cannot have constexpr values");
      }
...
size_t ImplementationVisitor::InitializeAggregateHelper(
  ...
  for (auto f : aggregate_type->fields()) {
    ...
    VisitResult current_value = initializer_results.results.at(current);
...
void ImplementationVisitor::InitializeFieldFromSpread(
  ...
  GenerateAssert(
      [&]() {
        return GenerateCall(
            "==",
            {{index, initializer_results.field_value_map.at(*(field.index))},
             {}},
            {}, false);
      },
...
VisitResult ImplementationVisitor::Visit(StructExpression* decl) {
  ...
  VisitResult uninitialized_result = TemporaryUninitializedStruct(
      struct_type, "it's not initialized in the struct " + struct_type->name());

  VisitResult initialized_result = InitializeAggregate(
      struct_type, uninitialized_result, initialization_results);
...
void ImplementationVisitor::GenerateAssignToLocation(
  ...
  if (reference.IsCallAccess()) {
    ...
    if (variable.type()->IsStructType() &&
        StructType::cast(variable.type())->HasConstexprFields()) {
      ReportError(
          "cannot assign to struct with const fields except in an initializer");
    }
...
ImplementationVisitor::LowerParameter(
  ...
  if (type->IsConstexpr()) {
    ...
    std::vector<std::string> struct_constexpr_values;
    for (auto& field : struct_type->fields()) {
      base::Optional<StackRange> parameter_range =
          lowered_parameters->TopRange(0);
      std::vector<std::string> field_struct_values;
      std::tie(parameter_range, field_struct_values) =
      ...
      for (auto constexpr_value : field_struct_values) {
