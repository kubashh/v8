...
void CppClassGenerator::GenerateFieldAccessor(const Field& f) {
  ...
  if (const StructType* struct_type = StructType::DynamicCast(field_type)) {
    ...
    EmitLoadFieldStatement(field_type, get_type, constexpr_offset, 0,
  ...
  if (const StructType* struct_type = StructType::DynamicCast(field_type)) {
    for (const Field& struct_field : struct_type->fields()) {
      ...
      EmitStoreFieldStatement(
          ...
          f.relaxed_write, *struct_field.offset, "    ",
          "(*value." + struct_field.name_and_type.name + ")");
      inl_ << "  }\n";
    }
  } else {
...
void CppClassGenerator::EmitLoadFieldStatement(
    const Type* field_type, const std::string& get_type,
    base::Optional<std::string> constexpr_offset, size_t struct_offset,
...
void CppClassGenerator::EmitStoreFieldStatement(
    const Type* field_type, const std::string& set_type, std::string offset,
    bool relaxed_write, size_t struct_offset, const char* indent,
    const std::string& value) {
  DCHECK(!field_type->IsStructType());
