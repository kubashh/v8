...
void CppClassGenerator::EnsureStructGenerated(const Type* t) {
  ...
  bool first = generated_structs_.empty();

  // Only generate each struct value type once within the containing class.
  if (!generated_structs_.insert(s).second) return;

  // Emit a value that can be used in setters to indicate that the field
  // shouldn't be updated.
  if (first) {
    hdr_ << "  static constexpr base::nullopt_t kTqNoChange = "
            "base::nullopt;\n\n";
  }
  ...
  hdr_ << "  struct TqGetterStruct" << CamelifyString(s->name()) << " {\n";
  for (const Field& f : s->fields()) {
  ...
  hdr_ << "  struct TqSetterStruct" << CamelifyString(s->name()) << " {\n";
  for (const Field& f : s->fields()) {
...
std::string CppClassGenerator::GetTypeNameForAccessor(const Field& f,
  ...
  if (const StructType* s = StructType::DynamicCast(field_type)) {
    return (kind == kGet ? "TqGetterStruct" : "TqSetterStruct") + s->name();
  }
  if (!field_type->IsSubtypeOf(TypeOracle::GetTaggedType())) {
