...
void ImplementationVisitor::Visit(TypeAlias* alias) {
  ...
  header_out() << "\n    std::tuple<";
  bool first = true;
  for (const Type* type : LowerType(struct_type, false)) {
    if (!first) {
      header_out() << ", ";
    }
    first = false;
...
VisitResult ImplementationVisitor::InlineMacro(
  ...
  if (can_return) {
    ...
    if (!return_type->IsConstexpr()) {
      base::Optional<std::vector<std::string>> const_values;
      SetReturnValue(VisitResult(
          return_type, stack.TopRange(lowered_return_types.size()), {}));
    }
...
void ImplementationVisitor::VisitMacroCommon(Macro* macro) {
  ...
  if (Method* method = Method::DynamicCast(macro)) {
    ...
    std::vector<VisitResult> this_arguments;
    LowerParameter(this_type, ExternalParameterName(kThisParameterName),
                   &lowered_parameters, &lowered_parameter_types,
                   &this_arguments);
    VisitResult this_result = this_arguments[0];
  ...
  for (size_t i = 0; i < macro->signature().parameter_names.size(); ++i) {
    ...
    LowerParameter(type, external_name, &lowered_parameters,
                   &lowered_parameter_types, &arguments);
  }
...
const Type* ImplementationVisitor::Visit(
  ...
  if (stmt->type) {
    ...
    if ((*type)->IsConstexpr() != 0 && !stmt->const_qualified) {
      ReportError(
          "cannot declare 'let' variable with or containing constexpr "
          "type. Use 'const' instead.");
    }
  }
  ...
  if (stmt->initializer) {
    ...
    if (type) {
      ...
      if (init_result->type()->IsConstexpr() && !stmt->const_qualified) {
        ReportError(
            "cannot initialize 'let' variable with and expression that "
            "is or contains a constexpr type. Use 'const' instead.");
      }
    }
...
const Type* ImplementationVisitor::Visit(ReturnStatement* stmt) {
    ...
    if (current_callable->IsMacro()) {
      if (return_result.IsOnStack()) {
        ...
        SetReturnValue(VisitResult(return_result.type(), return_value_range,
                                   return_result.constexpr_values()));
      } else {
  ...
  current_callable->MarkHasReturns();
  if (current_callable->signature().return_type->GetConstexprValueCount()) {
    size_t& return_count = CurrentReturnCount::Get();
    if (return_count++ > 1) {
      ReportError(
...
VisitResult ImplementationVisitor::Visit(TryLabelExpression* expr) {
...

  {
    ...
    for (size_t i = 0; i < parameter_count; ++i) {
      ...
      if (type->IsStructType() && type->GetConstexprValueCount() != 0) {
        ReportError("struct label paraeters cannot have constexpr values");
      }
      // Verify that structs for label arguments don't have constexpr values
...
size_t ImplementationVisitor::InitializeAggregateHelper(
  ...
  if (current_class) {
    ...
    if (super) {
      current = InitializeAggregateHelper(
          super, allocate_result, initializer_results, constexpr_values);
    }
  }
  ...
  for (auto f : aggregate_type->fields()) {
    ...
    } else if (f.name_and_type.type->IsConstexpr()) {
      if (!current_value.type()->IsSubtypeOf(f.name_and_type.type)) {
        ReportError("initialzier for constexpr-typed field '",
                    f.name_and_type.name, "' doesn't have a compatible type");
      }
      constexpr_values->push_back(current_value.constexpr_value());
    } else {
...
void ImplementationVisitor::InitializeFieldFromSpread(VisitResult object,
                                                      const Field& field,
  ...
  assembler().Bind(body_block);
  {
    ...
    Binding<LocalLabel> no_more{&LabelBindingsManager::Get(), "Done",
                                LocalLabel{exit_block}};
    Arguments next_arguments;
    next_arguments.labels.push_back(&no_more);
    Callable* callable = nullptr;
    callable = LookupMethod("Next", target, next_arguments, {});
...
#if 0
  AssertStatement* assert = MakeNode<AssertStatement>(true,
  MakeNode < Bin
  )

    Arguments compare_arguments;
    compare_arguments.parameters.push_back(length);
    compare_arguments.parameters.push_back(index);
    VisitResult compare_result = GenerateCall("==", assign_arguments);
#endif

  assembler().Bind(exit_block);
}

VisitResult ImplementationVisitor::InitializeAggregate(
    const AggregateType* aggregate_type, VisitResult allocate_result,
    const InitializerResults& initializer_results) {
  std::vector<std::string> constexpr_values;
  size_t consumed_initializers = InitializeAggregateHelper(
      aggregate_type, allocate_result, initializer_results, &constexpr_values);
  if (consumed_initializers != initializer_results.results.size()) {
    ReportError("more initializers than fields present in ",
                aggregate_type->name());
  }
  base::Optional<std::vector<std::string>> optional_constexpr_values;
  if (constexpr_values.size() != 0)
    optional_constexpr_values = constexpr_values;

  return VisitResult(allocate_result.type(), allocate_result.stack_range(),
                     optional_constexpr_values);
}

FieldValueMap ImplementationVisitor::GetFieldValueMap(
    const ClassType* class_type,
    const InitializerResults& initializer_results) {
  FieldValueMap field_values;
  auto initializer_result = initializer_results.results.rbegin();
  auto current_class = class_type;
  while (current_class != nullptr) {
    auto current_field = current_class->fields().rbegin();
    while (current_field != current_class->fields().rend()) {
      field_values[&*current_field] = *initializer_result;
      ++current_field;
      ++initializer_result;
    }
    ...
    while (current_field != current_class->fields().end()) {
      if (current_field->index) {
        ...
        VisitResult initializer_value =
            (*field_values)[*(current_field->index)];
...
VisitResult ImplementationVisitor::Visit(NewExpression* expr) {
  ...
  if (class_type->IsExtern()) {
    ...
    FieldValueMap field_values =
        GetFieldValueMap(class_type, initializer_results);

    object_size = AddVariableObjectSize(object_size, class_type, &field_values);
  ...
  VisitResult initialzed_result =
      InitializeAggregate(class_type, allocate_result, initializer_results);

  return stack_scope.Yield(initialzed_result);
}
...
VisitResult ImplementationVisitor::Visit(StructExpression* decl) {
  ...
  VisitResult unitialized_result = TemporaryUninitializedStruct(
      struct_type, "it's not initialized in the struct " + struct_type->name());

  VisitResult initialized_result = InitializeAggregate(
      struct_type, unitialized_result, initialization_results);

  return stack_scope.Yield(initialized_result);
}
...
void ImplementationVisitor::GenerateAssignToLocation(
  ...
  if (reference.IsCallAccess()) {
    ...
    if (variable.type()->IsStructType() &&
        StructType::cast(variable.type())->HasConstFields()) {
      ReportError(
          "cannot assign to struct with const fields except in a variable "
          "initializer");
    }
...
VisitResult ImplementationVisitor::GenerateCall(
  ...
  if (auto* builtin = Builtin::DynamicCast(callable)) {
    ...
    if (intrinsic->ExternalName() == "%RawConstexprCast") {
      ...
      if (intrinsic->ExternalName() == "%GetFrameArguments") {
        std::string args = assembler().NewFrameArguments();
        constexpr_arguments.push_back(args);
        assembler().Emit(CallIntrinsicInstruction{
            intrinsic, specialization_types, constexpr_arguments});
        return VisitResult(return_type, "&*" + args);
      } else {
        assembler().Emit(CallIntrinsicInstruction{
            intrinsic, specialization_types, constexpr_arguments});
...
std::pair<base::Optional<StackRange>, base::Optional<std::vector<std::string>>>
ImplementationVisitor::LowerParameter(
  ...
  if (type->IsConstexpr()) {
    std::vector<std::string> constexpr_values{parameter_name};
    if (arguments) (*arguments)->push_back(VisitResult(type, parameter_name));
    return std::make_pair(base::Optional<StackRange>{}, constexpr_values);
  } else if (const StructType* struct_type = StructType::DynamicCast(type)) {
    StackRange range = lowered_parameters->TopRange(0);
    base::Optional<std::vector<std::string>> struct_constexpr_values;
    for (auto& field : struct_type->fields()) {
      base::Optional<StackRange> parameter_range =
          lowered_parameters->TopRange(0);
      base::Optional<std::vector<std::string>> constexpr_values;
      std::tie(parameter_range, constexpr_values) =
          LowerParameter(field.name_and_type.type,
                         parameter_name + "." + field.name_and_type.name,
                         lowered_parameters, lowered_parameters_types, {});
      if (parameter_range) range.Extend(*parameter_range);
      if (!struct_constexpr_values.has_value()) {
        struct_constexpr_values = constexpr_values;
      } else {
        if (constexpr_values) {
          for (auto constexpr_value : *constexpr_values) {
            struct_constexpr_values->push_back(constexpr_value);
          }
        }
      }
    }
    if (arguments) {
      (*arguments)
          ->push_back(VisitResult(type, range, struct_constexpr_values));
    }
    return std::make_pair(range, struct_constexpr_values);
  } else {
    lowered_parameters->Push(parameter_name);
    if (arguments)
      (*arguments)
          ->push_back(VisitResult(type, lowered_parameters->TopRange(1)));
    lowered_parameters_types->Push(type);
    return std::make_pair(lowered_parameters->TopRange(1),
...
DEFINE_CONTEXTUAL_VARIABLE(ImplementationVisitor::CurrentReturnCount)
...
void ImplementationVisitor::Visit(Declarable* declarable) {
  ...
  CurrentReturnCount::Scope return_count(0);
