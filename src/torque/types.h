...
class VisitResult {
 public:
  ...
  VisitResult(const Type* type, std::string constexpr_value) : type_(type) {
    constexpr_values_.push_back(constexpr_value);
  ...
  VisitResult(const Type* type, StackRange stack_range,
    ...
    DCHECK_IMPLIES(type->IsConstexpr(), constexpr_values_.size() > 0);
    DCHECK_IMPLIES(!type->IsStructType() && !type->IsConstexpr(),
                   constexpr_values.size() == 0);
    DCHECK(type->GetConstexprValueCount() == constexpr_values.size());
  }
  static VisitResult NeverResult();
  const Type* type() const { return type_; }
  const std::string& constexpr_value() const { return constexpr_values_[0]; }
  const std::vector<std::string>& constexpr_values() const {
  ...
  bool HasConstexprValues() const { return constexpr_values_.size() != 0; }
 ...
 private:
  ...
  std::vector<std::string> constexpr_values_;
