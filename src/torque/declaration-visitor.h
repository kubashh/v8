...
class DeclarationVisitor : public FileVisitor {
 public:
  ...
  void Visit(ExternalRuntimeDeclaration* decl) {
    ...
    Type return_type = LookupType(decl->pos, decl->return_type);
    ...
    Signature signature{{},
                        ...
                        return_type,
                        {}};
    declarations()->DeclareRuntime(decl->pos, decl->name, std::move(signature));
  }
  ...
  void Visit(VarDeclarationStatement* stmt) {
    ...
    Type type = LookupType(stmt->pos, stmt->type);
  ...
  void Visit(ConstDeclaration* decl) {
    declarations()->DeclareConstant(decl->pos, decl->name,
                                    LookupType(decl->pos, decl->type),
                                    decl->literal);
  }
 ...
 private:
  ...
  void PushControlSplit() {
    ...
    live_and_changed.live = declarations()->GetLiveTypeVariables();
