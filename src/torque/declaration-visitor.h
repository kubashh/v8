...
class DeclarationVisitor : public FileVisitor {
 public:
  ...
  Signature MakeSpecializedSignature(Generic* generic,
                                     const TypeVector& specialized_types);
  Callable* SpecializeImplicit(Generic* generic,
                               const TypeVector& specialized_types);
  Callable* Specialize(Generic* generic, const TypeVector& specialized_types,
                       CallableNode* declaration,
                       base::Optional<const CallableNodeSignature*> signature,
                       Statement* body);

 private:
  void DeclareSpecializedTypes(Generic* generic,
                               const TypeVector& specialized_types);
};
