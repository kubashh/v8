...
TypeArgumentInference::TypeArgumentInference(
  ...
  for (size_t i = 0; i < type_parameters.size(); i++) {
    if (!inferred_[i]) {
      Fail("failed to infer arguments for all types parameters");
      return;
    }
  }
}
...
TypeVector TypeArgumentInference::GetResult() const {
  ...
  std::transform(inferred_.begin(), inferred_.end(), result.begin(),
                 [](auto& maybe_type) { return *maybe_type; });
  return result;
}
...
void TypeArgumentInference::Match(TypeExpression* parameter,
  ...
  if (BasicTypeExpression* basic =
    ...
    // here
  } else if (FunctionTypeExpression* basic =
                 FunctionTypeExpression::DynamicCast(parameter)) {
    Fail("argument type inference involving function types is unsupported");
  } else if (UnionTypeExpression* basic =
                 UnionTypeExpression::DynamicCast(parameter)) {
    Fail("argument type inference involving union types is unsupported");
  } else {
    UNREACHABLE();
  }
}
...
void TypeArgumentInference::MatchGeneric(BasicTypeExpression* parameter,
  ...
  DCHECK_EQ(parameters.size(), argument_types.size());
