...
base::Optional<ParseResult> MakeDouble(ParseResultIterator* child_results) {
  auto value = child_results->NextAs<std::string>();
  size_t num_chars_converted = 0;
  double result = 0;
  try {
  ...
  if (index) {
    if (optional) {
      ...
      index = MakeNode<ConditionalExpression>(
          *index, MakeNode<IntegerLiteralExpression>(1),
...
struct TorqueGrammar : Grammar {
  ...
  Symbol floatingPointLiteral = {
      Rule({Pattern(MatchFloatingPointLiteral)}, MakeDouble)};
