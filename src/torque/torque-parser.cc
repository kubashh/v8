...
template <>
V8_EXPORT_PRIVATE const ParseResultTypeId
    ParseResultHolder<ConditionalAnnotationType>::id =
        ParseResultTypeId::kConditionalAnnotationType;
template <>
V8_EXPORT_PRIVATE const ParseResultTypeId
...
base::Optional<ParseResult> MakeConditionalAnnotation(
    ParseResultIterator* child_results) {
  auto type = child_results->NextAs<ConditionalAnnotationType>();
...
struct TorqueGrammar : Grammar {
  ...
  // Result: ConditionalAnnotationType
  Symbol conditionalAnnotationLabel = {
      Rule({Token("@if")},
           YieldIntegralConstant<ConditionalAnnotationType,
  ...
  Symbol conditionalAnnotation = {
      Rule({&conditionalAnnotationLabel, Token("("), &identifier, Token(")")},
