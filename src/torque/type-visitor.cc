...
const AbstractType* TypeVisitor::ComputeType(AbstractTypeDeclaration* decl) {
  bool enforce_tnode = !decl->non_constexpr_version;
  std::string generates = ComputeGeneratesType(decl->generates, enforce_tnode);
  ...
  if (decl->non_constexpr_version) {
    const Type* type = Declarations::LookupType(*decl->non_constexpr_version);
