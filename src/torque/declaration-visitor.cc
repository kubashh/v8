...
void DeclarationVisitor::FinalizeStructFieldsAndMethods(
  ...
  for (auto& field : struct_declaration->fields) {
    ...
    if (field_type->IsConstexpr() && !field.is_const) {
      ReportError("struct field '", field.name_and_type.name->value,
                  "' with constexpr type must be const");
    }
...
void DeclarationVisitor::FinalizeClassFieldsAndMethods(
  ...
  for (ClassFieldExpression& field_expression : class_declaration->fields) {
    ...
    if (field_type->IsConstexpr()) {
      ReportError("class field '", field_expression.name_and_type.name->value,
                  "' cannot have a constexpr type");
    }
