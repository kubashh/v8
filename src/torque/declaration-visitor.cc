...
void DeclarationVisitor::FinalizeClassFieldsAndMethods(
  ...
  for (size_t field_index = 0; field_index < class_declaration->fields.size();
    ...
    const Type* field_type = TypeOracle::GetVoidType();
    ...
    if (padding_expression) {
      ...
      padding_size = GetPaddingSize(class_offset, following_data_size);
    ...
    const Field& field = class_type->RegisterField({field_expression.pos,
                                                    class_type,
                                                    index_field,
  ...
  for (auto& field : class_type->fields()) {
    if (field.index || field.padding_size.has_value()) continue;
