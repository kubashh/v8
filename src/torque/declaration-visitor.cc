...
void TypeDeclarationVisitor::Visit(Declaration* decl) {
...
#undef ENUM_ITEM
    case AstNode::Kind::kNamespaceDeclaration:
      return Visit(NamespaceDeclaration::cast(decl));
    default:
...
void TypeDeclarationVisitor::Visit(AbstractTypeDeclaration* decl) {
  Declarations::PreDeclareTypeAlias(decl->name, decl, false);
  if (decl->constexpr_generates) {
    // Here we create a second AbstractTypeDeclaration for the associated
    // constexpr type.
    if (decl->transient) {
    ...
    AbstractTypeDeclaration* constexpr_decl = MakeNode<AbstractTypeDeclaration>(
        constexpr_name, false, constexpr_extends, decl->constexpr_generates,
        base::nullopt);
    constexpr_decl->pos = decl->pos;
    constexpr_decl->non_constexpr_version = decl->name;
    Declarations::PreDeclareTypeAlias(constexpr_name, constexpr_decl, false);
  }
}
...
void TypeDeclarationVisitor::FinalizeClasses() {
  for (auto current_class_info : class_declarations_) {
    ClassDeclaration* class_declaration;
    const TypeAlias* alias;
    std::tie(class_declaration, alias) = current_class_info;
    CurrentScope::Scope scope_activator(alias->ParentScope());
    CurrentSourcePosition::Scope position_activator(class_declaration->pos);
    const ClassType* class_type = ClassType::cast(
        Declarations::ResolveType(class_declaration->name)->type());
    TypeVisitor::FinalizeClassFieldsAndMethods(
        const_cast<ClassType*>(class_type), class_declaration);
  }
}

void TypeDeclarationVisitor::ResolveAliases() {
  for (auto& p : GlobalContext::AllDeclarables()) {
    if (const TypeAlias* alias = TypeAlias::DynamicCast(p.get())) {
      CurrentScope::Scope scope_activator(alias->ParentScope());
      CurrentSourcePosition::Scope position_activator(alias->pos());
      alias->Resolve();
    }
  }
}
