...
// Constants
kythe_entity_t KytheData::AddConstantDefinition(
...
kythe_entity_t KytheData::AddFunctionDefinition(Callable* callable) {
  ...
  kythe_entity_t callable_id = that->consumer_->AddDefinition(
    KytheConsumer::Kind::Function,
      callable->ExternalName(), MakeKythePosition(ident_pos));
...
kythe_entity_t KytheData::AddBindingDefinitionImpl(uint64_t binding_index,
