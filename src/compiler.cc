...
bool ProduceCodeCache(ScriptCompiler::CompileOptions options) {
...
Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScript(
  ...
  if (compile_options == ScriptCompiler::kNoCompileOptions) {
    cached_data = NULL;
             ...
             ProduceCodeCache(compile_options)) {
  ...
  if (FLAG_profile_deserialization && ProduceCodeCache(compile_options)) {
    timer.Start();
  }

  if (result.is_null() || ProduceCodeCache(compile_options)) {
    ...
    if (ProduceCodeCache(compile_options)) {
      parse_info.set_will_serialize();
      parse_info.set_eager(compile_options ==
                           ScriptCompiler::kProduceExhaustiveCodeCache);
    }
    ...
    if (extension == NULL && !result.is_null()) {
      ...
      compilation_cache->PutScript(source, context, language_mode, result,
                                   vector);
      if (ProduceCodeCache(compile_options) && !ContainsAsmModule(script)) {
