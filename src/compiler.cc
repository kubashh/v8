...
bool Compiler::CollectSourcePositions(Isolate* isolate,
  ...
  if (GetCurrentStackPosition() < isolate->stack_guard()->real_climit()) {
    // Stack is already exhausted. Setting source_position_table to exception
    // allows the source positions to be later recalculated, but if anything
    // tries to immediately access them they will just get empty_byte_array.
    bytecode->set_source_position_table(ReadOnlyRoots(isolate).exception());
    return false;
  }
  ...
  if (!parsing::ParseAny(&parse_info, shared_info, isolate)) {
    // Parsing failed probably as a result of stack exhaustion. Setting
    // source_position_table to exception allows the source positions to be
    // later recalculated, but if anything tries to immediately access them they
    // will just get empty_byte_array.
    bytecode->set_source_position_table(ReadOnlyRoots(isolate).exception());
  ...
  if (!outer_function_job) {
    // Recompiling failed probably as a result of stack exhaustion. Setting
    // source_position_table to exception allows the source positions to be
    // later recalculated, but if anything tries to immediately access them they
    // will just get empty_byte_array.
    bytecode->set_source_position_table(ReadOnlyRoots(isolate).exception());
