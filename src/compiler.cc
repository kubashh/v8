...
MaybeHandle<JSFunction> Compiler::GetFunctionFromEval(
  ...
  if (eval_result.has_cell()) {
    cell = Handle<FeedbackCell>(eval_result.cell(), isolate);
  }
  ...
  if (eval_result.has_shared()) {
    if (eval_result.has_cell()) {
      result = isolate->factory()->NewFunctionFromSharedFunctionInfo(
          shared_info, context, cell, NOT_TENURED);
    } else {
      result = isolate->factory()->NewFunctionFromSharedFunctionInfo(
...
MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScript(
  ...
  if (extension == nullptr) {
    ...
    if (can_consume_code_cache && !pair.has_shared()) {
      ...
      if (pair.has_cell()) {
        feedback_cell = Handle<FeedbackCell>(pair.cell(), isolate);
      }
    }
  }
