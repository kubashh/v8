...
bool IsInterpreterFramePc(Isolate* isolate, Address pc) {
  Code* fist_execution_stub =
      isolate->builtins()->builtin(Builtins::kFunctionFirstExecution);
  ...
  return (pc >= fist_execution_stub->instruction_start() &&
          pc < fist_execution_stub->instruction_end()) ||
