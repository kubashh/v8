...
CompilerDispatcherJob::CompilerDispatcherJob(
    ...
    : status_(CompileJobStatus::kInitialParse),
      isolate_(nullptr),
      tracer_(tracer),
...
void CompilerDispatcherJob::ParseInitial() {
  DCHECK(status() == CompileJobStatus::kInitialParse);
  COMPILER_DISPATCHER_TRACE_SCOPE_WITH_NUM(
      tracer_, kParse,
      parse_info_->end_position() - parse_info_->start_position());
  if (trace_compiler_dispatcher_jobs_) {
    PrintF("CompilerDispatcherJob[%p]: Inital Parsing\n",
           static_cast<void*>(this));
  }
  ...
  DCHECK_NOT_NULL(result_slot_);
  if (finish_callback_) {
    finish_callback_->ParseInitialFinished(std::move(parse_info_));
  }
