...
void MicrotaskQueue::SetUpDefaultMicrotaskQueue(Isolate* isolate) {
  DCHECK(!isolate->default_microtask_queue());
...
// static
std::unique_ptr<MicrotaskQueue> MicrotaskQueue::NewMicrotaskQueue(
    Isolate* isolate) {
  DCHECK(isolate->default_microtask_queue());
...
void MicrotaskQueue::IterateMicrotasks(RootVisitor* visitor) {
  ...
  visitor->VisitRootPointers(Root::kStrongRoots, nullptr, ring_buffer_,
                             ring_buffer_ + std::max(start_ + size_ - capacity_,
