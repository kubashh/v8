...
#undef NATIVE_CONTEXT_FIELD_ACCESSOR
  ...
  // efficiency mode. The decision when to enable efficiency can be steered by
  // the embedder. Currently the only signal that can be considered is if an
  // isolate is in foreground or background mode.
  bool UseEfficiencyMode() {
    if (V8_UNLIKELY(v8_flags.efficiency_mode.value().has_value())) {
      return *v8_flags.efficiency_mode.value();
    }
    return v8_flags.efficiency_mode_for_background_isolate &&
