...
class V8_EXPORT_PRIVATE ExternalPointerTable {
 public:
  ExternalPointerTable() : buffer_(0), length_(0), capacity_(0) {}

  Address get(ExternalPointer_t ptr) const {
  ...
  void set(ExternalPointer_t ptr, Address value) {
    DCHECK(HAS_SMI_TAG(ptr));
    size_t idx = ptr >> 1;
    if (idx < length_) {
      buffer_[idx] = value;
