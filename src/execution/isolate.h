...
class V8_EXPORT_PRIVATE Isolate final : private HiddenFactory {
 ...
 public:
  ...
  V8_INLINE static Isolate* GlobalCurrent();
...
#undef NATIVE_CONTEXT_FIELD_ACCESSOR
  ...
  void IncrementHandlesDeref() {
    ...
    std::string desc = std::string(active_handles_count_scope_ != nullptr
                                       ...
                                       : "unknown");
    /*
    if (handles_deref_map_.find(desc) == handles_deref_map_.end()) {
      std::cout << "new entry " << desc << std::endl;
      std::cout << "i am your puppet!\n";
      IAmADummy();
    }
    */
...
extern Isolate* g_global_main_isolate_ V8_CONSTINIT;
