...
// 4+cp  |  BytecodeArray  |   |                        |
//       |- - - - - - - - -|   |                        |
// 4+cp  |  offset or FBV  |   v                        |
//       +-----------------+----                        |
// 5+cp  |   register 0    |   ^                     Callee
//       |- - - - - - - - -|   |                   frame slots
// 6+cp  |   register 1    | Register file         (slot >= 0)
//  ...  |       ...       |   |                        |
//       |  register n-1   |   |                        |
//       |- - - - - - - - -|   |                        |
// 5+cp+n|   register n    |   v                        v
//  -----+-----------------+----- <-- stack ptr -------------
//
...
class BaselineFrameConstants : public UnoptimizedFrameConstants {
 public:
  ...
  static constexpr int kFeedbackVectorOffset =
      kExpressionsOffset + kFeedbackVectorExpressionIndex * kSystemPointerSize;
};
