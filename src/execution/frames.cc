...
CodeLookupResult StackFrame::LookupCodeT() const {
  ...
  if (DEBUG_BOOL) {
    ...
    if (!result.IsEmbeddedBuiltinCode()) {
...
void InternalFrame::Iterate(RootVisitor* v) const {
  ...
  if (code.ToCodeT().has_tagged_outgoing_params()) IterateExpressions(v);
}
...
InnerPointerToCodeCache::GetCacheEntry(Address inner_pointer) {
  ...
  if (entry->inner_pointer == inner_pointer) {
    if (DEBUG_BOOL) {
      ...
      CHECK(lookup_result.IsFound());
      CHECK_EQ(entry->code.IsEmbeddedBuiltinCode(),
               lookup_result.IsEmbeddedBuiltinCode());
      if (lookup_result.IsEmbeddedBuiltinCode()) {
        CHECK_EQ(entry->code.code_data_container(),
                 lookup_result.code_data_container());
      } else {
        CHECK_EQ(entry->code.code(), lookup_result.code());
      }
    }
  } else {
