...
CodeLookupResult StackFrame::LookupCodeT() const {
  ...
  if (DEBUG_BOOL) {
    ...
    if (result.IsCode()) {
      ...
      CodeDataContainer code = result.codet();
      CHECK_GE(pc(), code.InstructionStart(isolate(), pc()));
      CHECK_LT(pc(), code.InstructionEnd(isolate(), pc()));
#endif
    }
...
void JsToWasmFrame::Iterate(RootVisitor* v) const {
  ...
  CHECK(lookup_result.IsFound());
  Builtin builtin;
  // It's not allowed to use lookup_result.ToCodeT() here because we might be
  // in the middle of GC.
  if (lookup_result.IsCodeT()) {
    builtin = lookup_result.codet().builtin_id();
  } else {
    builtin = lookup_result.code().builtin_id();
  }
