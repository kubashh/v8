...
void TypedFrame::IterateParamsOfWasmToJSWrapper(RootVisitor* v) const {
  ...
  for (size_t i = 0; i < parameter_count; i++) {
    ...
    if (l.IsRegister()) {
      ...
      // means that the wrapper pushed an odd number. If the number is odd, and
      // the and the size of a double on the stack is two words, then there is
      // an alignment word between the pushed gp registers and the pushed fp
      // registers, so that the whole spill area is double-size aligned.
