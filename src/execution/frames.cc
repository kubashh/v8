...
InnerPointerToCodeCache::GetCacheEntry(Address inner_pointer) {
  ...
  if (entry->inner_pointer == inner_pointer) {
    MapWord map_word = entry->code->map_word(kRelaxedLoad);
    if (map_word.IsForwardingAddress()) {
      entry->code = GcSafeCode::unchecked_cast(
          map_word.ToForwardingAddress(entry->code.value()));
    }
    ...
    // - the cache is cleared as part of the GC epilogue.
