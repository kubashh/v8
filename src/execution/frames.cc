...
DebuggableFrameIterator::DebuggableFrameIterator(Isolate* isolate)
    : iterator_(isolate) {
...
DebuggableFrameIterator::DebuggableFrameIterator(Isolate* isolate,
                                                 StackFrameId id)
    : DebuggableFrameIterator(isolate) {
  while (!done() && frame()->id() != id) Advance();
}

void DebuggableFrameIterator::Advance() {
  do {
    iterator_.Advance();
  } while (!done() && !IsValidFrame(iterator_.frame()));
}

int DebuggableFrameIterator::FrameFunctionCount() const {
  DCHECK(!done());
...
FrameSummary DebuggableFrameIterator::GetTopValidFrame() const {
  DCHECK(!done());
  // Like FrameSummary::GetTop, but additionally observes
  // DebuggableFrameIterator filtering semantics.
...
// static
bool DebuggableFrameIterator::IsValidFrame(StackFrame* frame) {
...
SafeStackFrameIterator::SafeStackFrameIterator(Isolate* isolate, Address pc,
  ...
  if (fast_c_fp != kNullAddress) {
    ...
    if (IsValidStackAddress(state.fp +
      ...
      type = StackFrame::kTypeForAdvance;
    }
...
StackFrame::Type SafeStackFrameIterator::ComputeStackFrameType(
    StackFrame::State* state) const {
#if V8_ENABLE_WEBASSEMBLY
  if (state->fp == kNullAddress) {
    DCHECK(v8_flags.experimental_wasm_stack_switching);
...
void IrregexpFrame::Iterate(RootVisitor* v) const {
  UNREACHABLE();  // These frames should not be visited by anything.
}
