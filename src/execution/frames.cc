...
#if V8_ENABLE_WEBASSEMBLY
#if !V8_WASM_INTERPRETER || !USE_SIMULATOR
...
#endif  // !V8_WASM_INTERPRETER || !USE_SIMULATOR
...
class DrumBrakeWasmCode {
 public:
  DrumBrakeWasmCode() : wasm_code_(nullptr) {}

  void set_wasm_code(const wasm::WasmCode* wasm_code) {
...
void WasmFrame::Iterate(RootVisitor* v) const {
  ...
  DrumBrakeWasmCode interpreter_wasm_code;
  ...
  if (!is_wasm_interpreter_frame) {
    ...
    DCHECK(wasm_code);
    interpreter_wasm_code.set_wasm_code(wasm_code);
  }
  ...
  DrumBrakeWasmCode* wasm_code = &interpreter_wasm_code;
#endif  // !V8_WASM_INTERPRETER
...
void WasmInterpreterEntryFrame::Summarize(
  ...
  std::vector<WasmInterpreterStackEntry> interpreted_stack =
      WasmInterpreterObject::GetInterpretedStack(
          instance->trusted_data(isolate())->interpreter_object(), fp());
...
Tagged<Tuple2> WasmInterpreterEntryFrame::interpreter_object() const {
  return wasm_instance()->trusted_data(isolate())->interpreter_object();
}

Tagged<WasmModuleObject> WasmInterpreterEntryFrame::module_object() const {
  return wasm_instance()->trusted_data(isolate())->module_object();
}
...
int WasmInterpreterEntryFrame::function_index(
  ...
  return WasmInterpreterObject::GetFunctionIndex(
      wasm_instance()->trusted_data(isolate())->interpreter_object(), fp(),
      inlined_function_index);
}

Tagged<Script> WasmInterpreterEntryFrame::script() const {
...
Tagged<Object> WasmInterpreterEntryFrame::context() const {
  return wasm_instance()->trusted_data(isolate())->native_context();
}
