...
class FutexWaitList {
 public:
  ...
  static int8_t* ToWaitLocation(
      const std::shared_ptr<BackingStore>& backing_store, size_t addr) {
 ...
 private:
  ...
  // Location inside a shared buffer -> linked lost of Nodes waiting on that
  // location.
...
// part of the lists. It must be the mutex used together with the `cond_`
// condition variable of such nodes.
base::LazyMutex g_mutex = LAZY_MUTEX_INITIALIZER;
// A map (location -> FutexWaitList) of FutexWaitLists.
...
Object FutexEmulation::WaitSync(Isolate* isolate,
  ...
  do {  // Not really a loop, just makes it easier to break out early.
    ...
    auto wait_location = FutexWaitList::ToWaitLocation(backing_store, addr);
...
FutexWaitListNode::FutexWaitListNode(
    ...
    : isolate_for_async_waiters_(isolate),
      ...
      wait_location_(FutexWaitList::ToWaitLocation(backing_store, wait_addr)),
      waiting_(true) {
...
Object FutexEmulation::Wake(Handle<JSArrayBuffer> array_buffer, size_t addr,
  ...
  base::MutexGuard lock_guard(g_mutex.Pointer());
  auto it = g_wait_list.Pointer()->location_lists_.find(wait_location);
  if (it == g_wait_list.Pointer()->location_lists_.end()) {
    return Smi::zero();
  }
  ...
  while (node && num_waiters_to_wake > 0) {
    ...
    // Checking wait_location_ here is not enough, since we need to guard
    ...
    if (node_backing_store.get() == nullptr &&
      ...
      // This cleanup code is not very efficient, since it only kicks in when a
      // new BackingStore has been created in the same memory area where the
...
void FutexEmulation::IsolateDeinit(Isolate* isolate) {
  ...
  {
    auto it = g_wait_list.Pointer()->location_lists_.begin();
    while (it != g_wait_list.Pointer()->location_lists_.end()) {
      ...
      if (head == nullptr) {
        g_wait_list.Pointer()->location_lists_.erase(it++);
      } else {
        ++it;
      }
    }
