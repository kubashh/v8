...
class ResolveAsyncWaiterPromisesTask : public v8::Task {
 public:
  explicit ResolveAsyncWaiterPromisesTask(Isolate* isolate)
      : isolate_(isolate) {}

  void Run() override { FutexEmulation::ResolveAsyncWaiterPromises(isolate_); }

 private:
  Isolate* isolate_;
};

class AsyncWaiterTimeoutTask : public v8::Task {
 public:
  AsyncWaiterTimeoutTask() {}

  void Run() override { FutexEmulation::HandleAsyncWaiterTimeouts(); }
};
  ...
  do {  // Not really a loop, just makes it easier to break out early.
    ...
    if (use_timeout) {
      ...
      if (new_low_timeout) {
        ...
        node->task_runner_->PostDelayedTask(std::move(task),
...
FutexWaitListNode* FutexEmulation::DeleteAsyncWaiterNode(
  ...
  if (!node->promise_.IsEmpty()) {
    Handle<JSPromise> promise =
        Handle<JSPromise>::cast(Utils::OpenPersistent(node->promise_));

    if (!node->native_context_.IsEmpty()) {
      Handle<NativeContext> native_context = Handle<NativeContext>::cast(
          Utils::OpenPersistent(node->native_context_));
...
void FutexEmulation::HandleAsyncWaiterTimeouts() {
  ...
  while (node) {
    ...
    if (current_time > node->timeout_time_) {
      ...
      if (node->waiting_) {
        node->waiting_ = false;
        // Will schedule ResolveAsyncWaiterPromisesTask.
        node->Notify(false);
      }
