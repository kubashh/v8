...
MaybeHandle<String> ErrorUtils::ToString(Isolate* isolate,
  ...
  if (message_source == ToStringMessageSource::kCurrentMessageProperty) {
    ...
    // was constructed. This is a V8-specific extension to keep Error.stack
    // consistent.
...
MaybeHandle<Object> ErrorUtils::CaptureStackTrace(Isolate* isolate,
  ...
  // "error_message_symbol" property if none is set yet. The stack should
  // consistently contain the error message at the time |captureStackTrace|
  // was called.
  LookupIterator it(isolate, LookupIterator::PROTOTYPE_CHAIN_SKIP_INTERCEPTOR,
