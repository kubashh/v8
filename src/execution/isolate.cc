...
void Isolate::AddCrashKeysForIsolateAndHeapPointers() {
  if (add_crash_key_callback_ == nullptr) {
    return;
  }

  const uintptr_t iso_address = reinterpret_cast<uintptr_t>(this);
  add_crash_key_callback_(v8::CrashKeyId::kIsolateAddress,
                          AddressToString(iso_address));
