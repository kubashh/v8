...
#define TRACE_CONTEXTS(...) \
  STREAM_TRACE_IF(v8_flags.trace_detached_contexts, __VA_ARGS__)
#ifdef DEBUG
#define TRACE_ISOLATES(...)                                                 \
  STREAM_TRACE_IF(v8_flags.trace_isolates, "Isolate ", this, " (id ", id(), \
                  ") ", __VA_ARGS__)
#else
#define TRACE_ISOLATES(...)
#endif
#define TRACE_RAIL(...) \
  STREAM_TRACE_IF(v8_flags.trace_rail, ToString(), ' ', __VA_ARGS__)
#define TRACE_STATS(...) STREAM_TRACE_IF(v8_flags.trace_zone_stats, __VA_ARGS__)
#define TRACE_TURBO_STACK(...) \
  STREAM_TRACE_IF(v8_flags.trace_turbo_stack_accesses, __VA_ARGS__)
#ifdef V8_ENABLE_PRECISE_ZONE_STATS
#define TRACE_TYPE_STATS(...) \
  STREAM_TRACE_IF(v8_flags.trace_zone_type_stats, __VA_ARGS__)
#else
#define TRACE_TYPE_STATS(...)
#endif
#define TRACE_WASM_STACK(...) \
  TRACE_IF(v8_flags.trace_wasm_stack_switching, __VA_ARGS__)
