...
char* Isolate::Iterate(RootVisitor* v, char* thread_storage) {
  ...
  Iterate(v, thread);
  // Normally, ThreadLocalTop::caller_context_ is visited weakly but in order
  // to simplify handling of frozen threads we just clear it. Otherwise, we'd
  // need to traverse the thread_storage again just to find this one field.
  thread->caller_context_ = Context();
...
Tagged<Object> Isolate::UnwindAndFindHandler() {
  ...
  // The caller context value becomes outdated after frames unwinding.
...
Handle<NativeContext> Isolate::GetIncumbentContextSlow() {
  ...
  if (!it.done() &&
    ...
    // If the caller_context is set then it must be correct.
    if (DEBUG_BOOL && !caller_context().is_null()) {
      DCHECK_EQ(caller_context()->native_context(), context->native_context());
    }
    return Handle<NativeContext>(context->native_context(), this);
  }
  DCHECK(caller_context().is_null());
...
void Isolate::CheckIsolateLayout() {
  ...
  static_assert(kCEntryFPCacheLine ==
                RoundDown<kCacheLineSize>(
                    OFFSET_OF(IsolateData, thread_local_top_.caller_context_)));
  static_assert(kCEntryFPCacheLine ==
                RoundDown<kCacheLineSize>(
...
SaveContext::SaveContext(Isolate* isolate) : isolate_(isolate) {
  ...
  if (!isolate->caller_context().is_null()) {
    caller_context_ = Handle<Context>(isolate->caller_context(), isolate);
  }
}

SaveContext::~SaveContext() {
  isolate_->set_context(context_.is_null() ? Tagged<Context>() : *context_);
  isolate_->set_caller_context(caller_context_.is_null() ? Tagged<Context>()
                                                         : *caller_context_);
}
...
AssertNoContextChange::AssertNoContextChange(Isolate* isolate)
    : isolate_(isolate),
      ...
      caller_context_(isolate->caller_context(), isolate) {}
