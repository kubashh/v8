...
void VerifyCheckedFlags() {
  ...
  CheckedFlags current_flags = CheckedFlag::kInitialized;
  if (FLAG_jitless) current_flags |= CheckedFlag::kJitless;
  if (FLAG_regexp_interpret_all) {
    current_flags |= CheckedFlag::kRegexpInterpretAll;
  }
#if V8_ENABLE_WEBASSEMBLY
  if (FLAG_expose_wasm) current_flags |= CheckedFlag::kExposeWasm;
#endif
  if (FLAG_opt) current_flags |= CheckedFlag::kOpt;

  uint32_t initial_flags = 0;
  bool is_initial_isolate =
      initial_checked_flag_values_.compare_exchange_strong(
          initial_flags, static_cast<uint32_t>(current_flags),
          std::memory_order_relaxed, std::memory_order_relaxed);

  if (is_initial_isolate) return;  // Nothing to check.

  if (static_cast<uint32_t>(current_flags) != initial_flags) {
    FATAL(
