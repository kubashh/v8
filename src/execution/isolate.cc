...
// function.
using map_of_loads_and_stores_per_function =
    std::map<std::string /* function_name */,
             std::pair<uint64_t /* loads */, uint64_t /* stores */>>;
map_of_loads_and_stores_per_function* stack_access_count_map = nullptr;
}  // namespace
...
void Isolate::DumpAndResetStats() {
  if (FLAG_trace_turbo_stack_accesses) {
    ...
    if (!stack_access_count_map) {
      os << "None of optimized/wasm stack-access function founds.";
    } else {
...
Address Isolate::load_from_stack_count_address(const char* function_name) {
  ...
  if (!stack_access_count_map) {
    stack_access_count_map = new map_of_loads_and_stores_per_function{};
  }
