...
#define TRACE_CONTEXTS(...) \
  StreamTraceIf(v8_flags.trace_detached_contexts, __VA_ARGS__)
#ifdef DEBUG
#define TRACE_ISOLATES(...)                                               \
  StreamTraceIf(v8_flags.trace_isolates, "Isolate ", this, " (id ", id(), \
                ") ", __VA_ARGS__)
#else
#define TRACE_ISOLATES(...)
#endif
#define TRACE_RAIL(...) \
  StreamTraceIf(v8_flags.trace_rail, ToString(), ' ', __VA_ARGS__)
#define TRACE_STATS(...) StreamTraceIf(v8_flags.trace_zone_stats, __VA_ARGS__)
#define TRACE_TURBO_STACK(...) \
  StreamTraceIf(v8_flags.trace_turbo_stack_accesses, __VA_ARGS__)
#ifdef V8_ENABLE_PRECISE_ZONE_STATS
#define TRACE_TYPE_STATS(...) \
  StreamTraceIf(v8_flags.trace_zone_type_stats, __VA_ARGS__)
#else
#define TRACE_TYPE_STATS(...)
#endif
#define TRACE_WASM_STACK(...) \
  TraceIf(v8_flags.trace_wasm_stack_switching, __VA_ARGS__)
...
void Isolate::SyncStackLimit() {
  ...
  TRACE_WASM_STACK("Switch to stack #%d", stack->id());
 ...
 private:
  void UpdateMemoryTrafficAndReportMemoryUsage(size_t memory_traffic_delta) {
...

    {
      ...
      TRACE_STATS("{\"type\": \"v8-zone-trace\", \"stats\": ", trace_str, '}');
...
void Isolate::WasmInitJSPIFeature() {
  if (IsUndefined(root(RootIndex::kActiveContinuation))) {
    ...
    stack->set_index(0);
    TRACE_WASM_STACK("Set up native stack object (limit: %p, base: %p)",
...
#undef TRACE_WASM_STACK
#undef TRACE_TYPE_STATS
#undef TRACE_TURBO_STACK
#undef TRACE_STATS
#undef TRACE_RAIL
#undef TRACE_ISOLATES
#undef TRACE_CONTEXTS
