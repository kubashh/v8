...
#if V8_ENABLE_WEBASSEMBLY
MaybeHandle<WasmSuspenderObject> TryGetWasmSuspenderForHandler(
  ...
  if (IsBuiltinFunction(isolate, handler, Builtin::kWasmResume)) {
    // Now peek into the handlers' AwaitContext to get to
    // the JSGeneratorObject for the async function.
    Handle<SharedFunctionInfo> shared(JSFunction::cast(handler)->shared(),
                                      isolate);
...
MaybeHandle<WasmSuspenderObject> TryGetWasmSuspender(
    Isolate* isolate, Handle<PromiseReaction> reaction) {
  return TryGetWasmSuspenderForHandler(isolate, reaction->fulfill_handler());
}
#endif  // V8_ENABLE_WEBASSEMBLY
...
MaybeHandle<JSPromise> TryGetCurrentTaskPromise(Isolate* isolate) {
  ...
  if (IsPromiseReactionJobTask(*current_microtask)) {
    ...
    if (IsBuiltinAsyncFulfillHandler(isolate,
      ...
      if (TryGetWasmSuspenderForHandler(isolate,
                                        promise_reaction_job_task->handler())
              .ToHandle(&suspender)) {
...
#if V8_ENABLE_WEBASSEMBLY
    // If the microtask is undefined, we need to check
    // if there is a wasm suspender and get the promise from that.
    Tagged<Object> suspender = isolate->root(RootIndex::kActiveSuspender);
    if (IsWasmSuspenderObject(suspender)) {
      // For some reason this is a no-op -- if we are running in JSPI wasm that
      // has never suspended we end up here, but the promise we get has no
      // reactions so it is not useful.
      return handle(WasmSuspenderObject::cast(suspender)->promise(), isolate);
    }
#endif  // V8_ENABLE_WEBASSEMBLY
  }
