...
#endif
std::atomic<bool> initial_FLAG_opt_;
...
#endif
    // Keep this last to avoid data races (see below).
    initial_FLAG_opt_.store(FLAG_opt, std::memory_order_release);
  } else {
    // It is invalid to create multiple isolates in the same process with
    // differing jitless configurations.

    // Keep this first to avoid data races (see above).
    CHECK_EQ(initial_FLAG_opt_.load(std::memory_order_acquire), FLAG_opt);
#if V8_ENABLE_WEBASSEMBLY
