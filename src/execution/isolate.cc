...
void Isolate::PumpHeapBrokerTaskQueues() {
  static constexpr size_t kMaxTasksToProcess = 512;  // Totally random.
  const size_t size = broker_task_queues_.size();
  size_t iters_since_last_work = 0;
  size_t processed_tasks = 0;
  size_t i = 0;
  while (iters_since_last_work < size && processed_tasks < kMaxTasksToProcess) {
    ...
    do {
      ...
      processed_tasks++;  // Yes we can exceed the limit here.
