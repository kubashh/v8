...
struct FormattedStringPart<int> {
  // Integer range: [-2,147,483,647, 2,147,483,647]. Representable in 11
  // characters.
...
struct FormattedStringPart<size_t> {
  // size_t range: [0, 4,294,967,295] on 32-bit, [0,
  // +18,446,744,073,709,551,615] on 64-bit. Needs 10 or 20 characters.
...
template <size_t N>
struct FormattedStringPart<const char[N]> {
...
//   V8::FatalProcessOutOfMemory(nullptr, message.PrintToArray().begin());
//
...
//   V8::FatalProcessOutOfMemory(nullptr, message_arr.begin());
 ...
 public:
  ...
  V8_WARN_UNUSED_RESULT auto operator<<(T&& t) const&& {
    ...
    return FormattedString<Ts..., PlainT>{
        std::tuple_cat(parts_, std::make_tuple(Part<PlainT>{.value = t}))};
  }
