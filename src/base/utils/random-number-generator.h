...
class V8_BASE_EXPORT RandomNumberGenerator final {
 public:
  ...
  // Returns the next pseudorandom set of n unique int64 values from the range 0
  // (inclusive) to max (exclusive). n must be less or equal to max.
  std::vector<int64_t> NextSample(int64_t max, size_t n) WARN_UNUSED_RESULT;
  ...
  std::vector<int64_t> NextSampleSlow(
      int64_t max, size_t n,
      const std::unordered_set<int64_t>& excluded = {}) WARN_UNUSED_RESULT;
