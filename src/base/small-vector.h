 ...
 public:
  ...
  SmallVector& operator=(const SmallVector& other) V8_NOEXCEPT {
    ...
    if (capacity() < other_size) {
      ...
      if (is_big()) FreeDynamicStorage();
      begin_ =
          reinterpret_cast<T*>(allocator_.allocate(sizeof(T) * other_size));
  ...
  // Clear without reverting back to inline storage. Dynamic storage is *not*
  // freed.
  void clear() { end_ = begin_; }

  // Clear and go back to inline storage. Dynamic storage is *not* freed.
  void reset() {
