...
namespace v8 {
namespace base {
static int16_t CharToDigit[128] = {
...
static int16_t charToDigitDecode(unsigned char c) { return CharToDigit[c]; }

int16_t charToDigitDecodeForTesting(unsigned char c) {
  return charToDigitDecode(c);
}

int32_t VLQBase64Decode(const std::string& s, size_t* pos) {
  ...
  while (toContinue) {
    ...
    int digit = static_cast<int>(charToDigitDecode(s[*pos]));
    if (digit == -1 ||
        (shift + CONTINUE_SHIFT >= CHAR_BIT * sizeof(int32_t) && digit >> 2)) {
      return std::numeric_limits<int32_t>::min();
    }
