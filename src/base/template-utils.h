...
// A variant of std::array that checks the number of constructor arguments.
template <class T, size_t n>
struct Array : std::array<T, n> {
  template <class... Args>
  Array(Args... args)  // NOLINT(runtime/explicit)
      : std::array<T, n>{std::move(args)...} {
...
template <class T>
struct Array<T, 0> : std::array<T, 0> {
  Array() = default;
};
