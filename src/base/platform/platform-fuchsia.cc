...
void* AllocateInternal(const zx::vmar& vmar, void* vmar_base, size_t page_size,
  ...
  DCHECK_IMPLIES(placement != kAnywhere, address != nullptr);
  ...
  if (status != ZX_OK && placement == kUseHint) {
...
void* OS::Allocate(void* address, size_t size, size_t alignment,
  ...
  PlacementMode placement = address != nullptr ? kUseHint : kAnywhere;
...
Optional<AddressSpaceReservation> OS::CreateAddressSpaceReservation(
  ...
  zx::vmar child;
  zx_vaddr_t child_addr;
  PlacementMode placement = hint != nullptr ? kUseHint : kAnywhere;
...
Optional<AddressSpaceReservation> AddressSpaceReservation::CreateSubReservation(
  ...
  zx_vaddr_t child_addr;
  zx_status_t status = CreateAddressSpaceReservationInternal(
      *zx::unowned_vmar(vmar_), base(), OS::AllocatePageSize(), address, kFixed,
      size, OS::AllocatePageSize(), max_permission, &child, &child_addr);
  if (status != ZX_OK) return {};
...
bool AddressSpaceReservation::Allocate(void* address, size_t size,
  ...
  DCHECK(Contains(address, size));
  void* allocation =
      AllocateInternal(*zx::unowned_vmar(vmar_), base(), OS::AllocatePageSize(),
                       address, kFixed, size, OS::AllocatePageSize(), access);
