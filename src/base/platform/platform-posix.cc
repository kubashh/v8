...
int PKU::GetPermissionsProtectionKey() { return pku_key_; }

DEFINE_LAZY_LEAKY_OBJECT_GETTER(PKU, GetPKU)

PKU::PKU() {
  InitializeMemoryProtectionKeySupport();
  pku_key_ = AllocateMemoryProtectionKey();
  is_support_pku_ = pku_key_ != base::OS::kNoMemoryProtectionKey;
}
...
bool PKU::SetPermissionsAndMemoryProtectionKey(
  ...
  if (pkey_mprotect) {
    ...
    DCHECK_EQ(0, reinterpret_cast<uintptr_t>(address) % OS::CommitPageSize());
    DCHECK_EQ(0, size % OS::CommitPageSize());

    int protection = GetProtectionFromMemoryPermission(permissions);

    int ret = pkey_mprotect(address, size, protection, pku_key_);
...
void PKU::SetWriteableForMemoryProtectionKey(bool writeable) {
  ...
  CHECK_EQ(0, pkey_set(pku_key_, permissions));
}

OS::MemoryProtectionKeyPermission PKU::GetMemoryProtectionKeyPermission() {
  int permission = pkey_get(pku_key_);
...
bool PKU::HasMemoryProtectionKeySupport() { return is_support_pku_; }

bool OS::SetPermissionsAndMemoryProtectionKey(
    void* address, size_t size, OS::MemoryPermission permissions) {
  return GetPKU()->SetPermissionsAndMemoryProtectionKey(address, size,
                                                        permissions);
}

void OS::SetWriteableForMemoryProtectionKey(bool writeable) {
  return GetPKU()->SetWriteableForMemoryProtectionKey(writeable);
}

int OS::GetPermissionsProtectionKey() {
  return GetPKU()->GetPermissionsProtectionKey();
}

bool OS::HasMemoryProtectionKeySupport() {
  return GetPKU()->HasMemoryProtectionKeySupport();
}

OS::MemoryProtectionKeyPermission OS::GetMemoryProtectionKeyPermission() {
  return GetPKU()->GetMemoryProtectionKeyPermission();
}
