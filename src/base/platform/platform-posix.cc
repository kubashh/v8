...
// static
bool OS::CommitPages(void* address, size_t size, MemoryPermission access) {
  DCHECK_NE(access, OS::MemoryPermission::kNoAccess);
#if defined(V8_OS_DARWIN)
  DCHECK_EQ(0, reinterpret_cast<uintptr_t>(address) % CommitPageSize());
  DCHECK_EQ(0, size % CommitPageSize());

  if (kTraceOS) {
    printf("OS::CommitPages: %p-%p, %#x\n", address,
           reinterpret_cast<char*>(address) + size, static_cast<int>(access));
  }
...
bool AddressSpaceReservation::CommitPages(void* address, size_t size,
                                          OS::MemoryPermission access) {
  DCHECK(Contains(address, size));
  return OS::CommitPages(address, size, access);
}
