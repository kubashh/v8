...
uint8_t* RandomizedVirtualAlloc(size_t size, DWORD flags, DWORD protect,
  ...
  if (use_aslr &&
    ...
    base = reinterpret_cast<uint8_t*>(VirtualAlloc(hint, size, flags, protect));
  }

  // On failure, let the OS find an address to use.
  if (base == nullptr) {
    base =
        reinterpret_cast<uint8_t*>(VirtualAlloc(nullptr, size, flags, protect));
  }
...
void* OS::Allocate(void* address, size_t size, size_t alignment,
  ...
  uint8_t* base = RandomizedVirtualAlloc(request_size, flags, protect, address);
