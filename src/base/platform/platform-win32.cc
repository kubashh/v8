...
void* OS::Allocate(void* address, size_t size, size_t alignment,
  ...
  for (int i = 0; i < kMaxAttempts; ++i) {
    ...
    // Try reducing the size by freeing and then re-allocating at aligned_base.
    CHECK(Free(reinterpret_cast<uint8_t*>(base), padded_size));
    ...
    if (base != nullptr) break;
    // Don't keep trying to allocate at the hinted address.
    address = nullptr;
  }
