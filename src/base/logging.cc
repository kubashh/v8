...
void FatalOOM(OOMType type, const char* msg) {
#ifdef V8_FUZZILLI
  // When fuzzing, we generally want to ignore OOM failures. It's important
  // that we exit with a non-zero exit status here so that the fuzzer treats it
  ...
  FATAL("Fatal %s out of memory: %s", type_str, msg);
#endif  // V8_FUZZILLI
}
