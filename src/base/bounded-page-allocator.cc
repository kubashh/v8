...
BoundedPageAllocator::BoundedPageAllocator(
    ...
    size_t allocate_page_size, InitializationBehavior initialization_behavior)
    : allocate_page_size_(allocate_page_size),
      ...
      initialization_behavior_(initialization_behavior) {
...
bool BoundedPageAllocator::FreePages(void* raw_address, size_t size) {
  ...
  if (initialization_behavior_ == kAllocatedPagesMustBeZeroInitialized) {
    ...
    DCHECK_EQ(initialization_behavior_, kAllocatedPagesCanBeUninitialized);
