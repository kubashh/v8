...
// values. The Flags<T, M, S> class is a template class, where T is an enum
// type, and M and S define the underlying storage type (usually int).
// Separate M and S parameters allows using std::atomic<M> as a storage type.
//
...
// int.
template <typename T, typename MaskType = int, typename StorageType = MaskType>
class Flags final {
 public:
  using flag_type = T;
