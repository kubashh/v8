...
class SharedMemory : public ::v8::PageAllocator::SharedMemory {
 public:
  ...
  std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping> RemapTo(
      ::v8::PageAllocator* allocator, void* new_address) const override {
    // Allocate the memory with the passed in allocator detaching it so it never
    // tries to use that memory again.
    if (!allocator->AllocatePagesAtAndDetach(new_address, size_,
                                             PageAllocator::kNoAccess)) {
      return {};
    }
    // Now remap the shared memory using the SharedMemory's original allocator.
