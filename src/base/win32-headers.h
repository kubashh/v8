...
struct V8_SRWLOCK {
  // SRWLOCK contains a void*.
  std::aligned_storage<sizeof(void*), alignof(void*)>::type storage;
};

struct V8_CONDITION_VARIABLE {
  // CONDITION_VARIABLE contains a void*.
  std::aligned_storage<sizeof(void*), alignof(void*)>::type storage;
};

struct V8_CRITICAL_SECTION {
  // CRITICALSECTION contains three pointers, two 32-bit integers, and a pointer
  // sized integer.
  std::aligned_storage<sizeof(void*) * 4 + 8, alignof(void*)>::type storage;
};
