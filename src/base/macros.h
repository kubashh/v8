...
V8_INLINE A implicit_cast(A x) {
  // If this is ever being used for non-trivially-copyable types, we have to
  // think about proper move semantics.
  // TODO(tebbi): Fix SloppyTNode<T> to be trivially copyable.
  // ASSERT_TRIVIALLY_COPYABLE(A);
  return x;
}
