...
class CombinationAssertScope : public Scopes... {
 public:
  // Constructor for per-thread scopes.
  V8_EXPORT_PRIVATE CombinationAssertScope() : Scopes()... {}
  // Constructor for per-isolate scopes.
  V8_EXPORT_PRIVATE explicit CombinationAssertScope(Isolate* isolate)
      : Scopes(isolate)... {}
  V8_EXPORT_PRIVATE ~CombinationAssertScope() = default;

  CombinationAssertScope(const CombinationAssertScope&) = delete;
  CombinationAssertScope& operator=(const CombinationAssertScope&) = delete;

  V8_EXPORT_PRIVATE static bool IsAllowed() {
    bool is_allowed = true;
    ITERATE_PACK(is_allowed &= Scopes::IsAllowed());
    return is_allowed;
  }
...
namespace detail {
// Helper for CombinationAssertScope<Scopes>::Release, which runs
// Scope::Release() for all Scopes in reverse order.
template <typename... Scopes>
struct ReleaseHelper;
template <>
struct ReleaseHelper<> {
  static void Release(CombinationAssertScope<>* scope) {}
};
template <typename Scope, typename... Scopes>
struct ReleaseHelper<Scope, Scopes...> {
  static void Release(CombinationAssertScope<Scope, Scopes...>* scope) {
    ReleaseHelper<Scopes...>::Release(scope);
    scope->Scope::Release();
  }
};
}  // namespace detail
template <typename... Scopes>
void CombinationAssertScope<Scopes...>::Release() {
  detail::ReleaseHelper<Scopes...>::Release(this);
}
