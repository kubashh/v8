...
#define PER_ISOLATE_ASSERT_TYPE_DEBUG_ONLY(V)                           \
  ...
  /* Scope to document where we do not expect deoptimization. */        \
  /* Scope to introduce an exception to DisallowDeoptimization. */      \
  V(AllowCompilation, DisallowCompilation, compilation_assert)          \
  /* Scope to document where we do not expect exceptions. */            \
  /* Scope to introduce an exception to DisallowExceptions. */          \
  V(AllowExceptions, DisallowExceptions, no_exception_assert)

#define PER_ISOLATE_ASSERT_TYPE(V)                                           \
  ...
  PER_ISOLATE_ASSERT_TYPE_DEBUG_ONLY(V)

#define PER_ISOLATE_ASSERT_ENABLE_SCOPE(EnableType, DisableType, field) \
  class V8_NODISCARD EnableType {                                       \
   public:                                                              \
    V8_EXPORT_PRIVATE explicit EnableType(Isolate* isolate);            \
    EnableType(const EnableType&) = delete;                             \
    EnableType& operator=(const EnableType&) = delete;                  \
    V8_EXPORT_PRIVATE ~EnableType();                                    \
                                                                        \
...
#define PER_ISOLATE_ASSERT_DISABLE_SCOPE(EnableType, DisableType, field) \
  class V8_NODISCARD DisableType {                                       \
   public:                                                               \
    V8_EXPORT_PRIVATE explicit DisableType(Isolate* isolate);            \
    DisableType(const DisableType&) = delete;                            \
    DisableType& operator=(const DisableType&) = delete;                 \
    V8_EXPORT_PRIVATE ~DisableType();                                    \
                                                                         \
    static bool IsAllowed(Isolate* isolate);                             \
                                                                         \
    V8_EXPORT_PRIVATE static void Open(Isolate* isolate,                 \
...
PER_ISOLATE_ASSERT_TYPE(PER_ISOLATE_ASSERT_ENABLE_SCOPE)
PER_ISOLATE_ASSERT_TYPE(PER_ISOLATE_ASSERT_DISABLE_SCOPE)
