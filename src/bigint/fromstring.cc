...
void ProcessorImpl::FromStringClassic(RWDigits Z,
  ...
  // few parts. All multipliers are the same, except possibly for the last.
  ...
  if (num_heap_parts == 0) {
    // If there are no heap parts, then the last stack part must use the
    // {last_multiplier}, and we are done.
    MultiplySingle(Z, already_set, accumulator->last_multiplier_);
    Add(Z, accumulator->stack_parts_[num_stack_parts - 1]);
    return;
  }
  // Otherwise, the last stack part still uses the max multiplier, and we
  // proceed with processing the heap parts.
  MultiplySingle(Z, already_set, max_multiplier);
  Add(Z, accumulator->stack_parts_[num_stack_parts - 1]);
  already_set.set_len(already_set.len() + 1);
  for (int i = 0; i < num_heap_parts - 1; i++) {
    MultiplySingle(Z, already_set, max_multiplier);
