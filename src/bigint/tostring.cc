...
char* BasecaseFixedLast(digit_t chunk, char* out) {
  while (chunk != 0) {
    DCHECK(*(out - 1) == '?');  // NOLINT(readability/check)
    if (radix <= 10) {
      *(--out) = '0' + (chunk % radix);
    } else {
...
template <int radix>
char* BasecaseFixedMiddle(digit_t chunk, char* out) {
  ...
  DCHECK(chunk == 0);  // NOLINT(readability/check)
  return out;
}
...
// The price we pay is having to work on half digit; the technique doesn't
...
class ToStringFormatter {
 ...
 private:
  ...
  char* BasecaseMiddle(digit_t digit, char* out) {
    if (radix_ == 10) return BasecaseFixedMiddle<10>(digit, out);
...
void ProcessorImpl::ToString(char* out, int* out_length, Digits X, int radix,
                             bool sign) {
#if DEBUG
  for (int i = 0; i < *out_length; i++) out[i] = '?';
#endif
