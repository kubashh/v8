...
// Defined here in the header so that it can be inlined. For best performance,
// stack-allocating instances is recommended.
class FromStringAccumulator {
 public:
  ...
  // Step 1: Call this method to read all characters. {Char} should be
  // a character type, such as uint8_t or uint16_t.
  ...
  // Step 2: Check if a result is available, and determine its required
  // allocation size.
  Result result() { return result_; }
  int ResultLength() {
    return stack_parts_used_ + static_cast<int>(heap_parts_.size());
  }
