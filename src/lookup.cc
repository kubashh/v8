...
void LookupIterator::InternalUpdateProtector() {
  ...
  if (*name_ == ReadOnlyRoots(heap()).constructor_string()) {
  ...
  } else if (*name_ == ReadOnlyRoots(heap()).next_string()) {
  ...
  } else if (*name_ == ReadOnlyRoots(heap()).species_symbol()) {
    if (!isolate_->IsArraySpeciesLookupChainIntact() &&
        !isolate_->IsTypedArraySpeciesLookupChainIntact() &&
        !isolate_->IsPromiseSpeciesLookupChainIntact())
      return;
  ...
  } else if (*name_ == ReadOnlyRoots(heap()).is_concat_spreadable_symbol()) {
    if (!isolate_->IsIsConcatSpreadableLookupChainIntact()) return;
    isolate_->InvalidateIsConcatSpreadableProtector();
  } else if (*name_ == ReadOnlyRoots(heap()).iterator_symbol()) {
    if (!isolate_->IsArrayIteratorLookupChainIntact()) return;
    if (holder_->IsJSArray()) {
      isolate_->InvalidateArrayIteratorProtector();
    }
  } else if (*name_ == ReadOnlyRoots(heap()).resolve_string()) {
  ...
  } else if (*name_ == ReadOnlyRoots(heap()).then_string()) {
