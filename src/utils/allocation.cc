...
bool VirtualMemoryCage::InitReservation(const ReservationParams& params) {
  ...
  DCHECK_LE(params.base_bias_size, params.reservation_size);

  const size_t base_alignment =
      RoundUp(params.base_alignment, params.page_allocator->AllocatePageSize());
  Address hint = RoundDown(params.requested_start_hint, base_alignment) -
                 RoundUp(params.base_bias_size, base_alignment);

  if (IsAligned(params.base_bias_size, base_alignment)) {
    // When the base bias is itself aligned to the base alignment, reserve
    // virtual memory using the base alignment.
    VirtualMemory reservation(params.page_allocator, params.reservation_size,
                              reinterpret_cast<void*>(hint), base_alignment);
    ...
    for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
      ...
      Address address =
          RoundUp(padded_reservation.address() + params.base_bias_size,
                  base_alignment) -
      ...
      if (overreserve) {
        ...
        Address address = RoundUp(reservation.address() + params.base_bias_size,
                                  base_alignment) -
                          params.base_bias_size;
  ...
  CHECK(IsAligned(base_, base_alignment));
