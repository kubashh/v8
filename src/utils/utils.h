...
namespace {
constexpr uint16_t kSIMDMatched16Mask = UINT16_MAX;
constexpr uint32_t kSIMDMatched32Mask = UINT32_MAX;
}  // namespace

template <typename Char>
TARGET_AVX2 bool SimdMemEqualAVX2(const Char* lhs, const Char* rhs,
                                  size_t count, size_t order) {
  // Don't bother with AVX2 for strings shorter than 33 chars.
  DCHECK_GT(count, 32);
  DCHECK_GE(order, 6);
  ...
  const uint32_t res_start_mask = _mm256_movemask_epi8(res_start);

  if (res_start_mask != kSIMDMatched32Mask) return false;

  // count: [33, 64]
  if (order == 6) {
    // Check end of string (potentially overlapping with start).
    ...
    const uint32_t res_end_mask = _mm256_movemask_epi8(res_end);
    return res_end_mask == kSIMDMatched32Mask;
  }

  for (size_t i = count % sizeof(__m256i); i < count; i += sizeof(__m256i)) {
    const __m256i lhs256 =
        _mm256_loadu_si256(reinterpret_cast<const __m256i*>(lhs + i));
    const __m256i rhs256 =
        _mm256_loadu_si256(reinterpret_cast<const __m256i*>(rhs + i));
    const __m256i res = _mm256_cmpeq_epi8(lhs256, rhs256);
    const uint32_t res_mask = _mm256_movemask_epi8(res);
    if (res_mask != kSIMDMatched32Mask) return false;
  }
  return true;
}
  ...
  switch (order) {
    ...
    default:  // count: [33, ...]
    {
      if (CpuFeatures::IsSupported(AVX2)) {
        return SimdMemEqualAVX2(lhs, rhs, count, order);
      }
