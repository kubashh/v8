...
TARGET_SSE4 V8_INLINE bool SimdMemEqual(const Char* lhs, const Char* rhs,
  ...
  if (order == 5) {
    ...
    */
    const int res_start = _mm_testc_si128(lhs128_start, rhs128_start);
    const int res_end = _mm_testc_si128(lhs128_end, rhs128_end);
    return (res_start & res_end) != 0;
  }
  ...
  */
  int res_unrolled = _mm_testc_si128(lhs128_unrolled, rhs128_unrolled);
  if (res_unrolled == 0) return false;
  ...
  for (size_t i = count % sizeof(__m128i); i < count; i += sizeof(__m128i)) {
    ...
    */
    int res = _mm_testc_si128(lhs128, rhs128);
...
template <typename Char>
TARGET_SSE4 V8_INLINE bool FastMemEqual(const Char* lhs, const Char* rhs,
                                        size_t count) {
  static_assert(sizeof(Char) == 1);
  if (count == 0) {
    return true;
...
#else
      // memcmp compares byte-by-byte, but for equality it doesn't matter
      // whether two-byte char comparison is little- or big-endian.
...
TARGET_SSE4 inline bool CompareCharsEqualUnsigned(const lchar* lhs,
                                                  const rchar* rhs,
                                                  size_t chars) {
  ...
  if constexpr (sizeof(*lhs) == sizeof(*rhs)) {
    if constexpr (sizeof(*lhs) == 1) {
      return FastMemEqual(lhs, rhs, chars);
    } else {
      // memcmp compares byte-by-byte, but for equality it doesn't matter
      // whether two-byte char comparison is little- or big-endian.
      return memcmp(lhs, rhs, chars * sizeof(*lhs)) == 0;
...
template <typename lchar, typename rchar>
TARGET_SSE4 inline bool CompareCharsEqual(const lchar* lhs, const rchar* rhs,
                                          size_t chars) {
