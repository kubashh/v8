...
void JSPluralRules::JSPluralRulesVerify(Isolate* isolate) {
  ...
  JSObjectVerify(isolate);
  VerifyObjectField(isolate, kLocaleOffset);
  CHECK(locale()->IsUndefined(isolate) || locale()->IsString());
  VerifyObjectField(isolate, kTypeOffset);
  CHECK(locale()->IsUndefined(isolate) || type()->IsString());
  VerifyObjectField(isolate, kICUPluralRulesOffset);
  CHECK(icu_plural_rules()->IsUndefined(isolate) ||
        icu_plural_rules()->IsForeign());
  VerifyObjectField(isolate, kICUDecimalFormatOffset);
  CHECK(icu_decimal_format()->IsUndefined(isolate) ||
        icu_decimal_format()->IsForeign());
}
