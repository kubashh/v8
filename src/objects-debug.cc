...
void Oddball::OddballVerify(Isolate* isolate) {
  ...
  if (map() == ReadOnlyRoots(heap).undefined_map()) {
    CHECK(this == ReadOnlyRoots(heap).undefined_value());
  } else if (map() == ReadOnlyRoots(heap).the_hole_map()) {
    CHECK(this == ReadOnlyRoots(heap).the_hole_value());
  } else if (map() == ReadOnlyRoots(heap).null_map()) {
    CHECK(this == ReadOnlyRoots(heap).null_value());
  } else if (map() == ReadOnlyRoots(heap).boolean_map()) {
    CHECK(this == ReadOnlyRoots(heap).true_value() ||
          this == ReadOnlyRoots(heap).false_value());
  } else if (map() == ReadOnlyRoots(heap).uninitialized_map()) {
    CHECK(this == ReadOnlyRoots(heap).uninitialized_value());
  } else if (map() == ReadOnlyRoots(heap).arguments_marker_map()) {
    CHECK(this == ReadOnlyRoots(heap).arguments_marker());
  } else if (map() == ReadOnlyRoots(heap).termination_exception_map()) {
    CHECK(this == ReadOnlyRoots(heap).termination_exception());
  } else if (map() == ReadOnlyRoots(heap).exception_map()) {
    CHECK(this == ReadOnlyRoots(heap).exception());
  } else if (map() == ReadOnlyRoots(heap).optimized_out_map()) {
    CHECK(this == ReadOnlyRoots(heap).optimized_out());
  } else if (map() == ReadOnlyRoots(heap).stale_register_map()) {
    CHECK(this == ReadOnlyRoots(heap).stale_register());
  } else if (map() == ReadOnlyRoots(heap).self_reference_marker_map()) {
