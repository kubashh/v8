...
#undef BAILOUT
  if (bailout) {
    CHECK(
        BuildAbort(AbortReason::kUnexpectedReturnFromThrow).IsDoneWithAbort());
  }
}
...
void MaglevGraphBuilder::VisitThrow() {
  ...
  CHECK(BuildCallRuntime(Runtime::kThrow, {exception}).IsDoneWithAbort());
}
void MaglevGraphBuilder::VisitReThrow() {
  ValueNode* exception = GetAccumulatorTagged();
  CHECK(BuildCallRuntime(Runtime::kReThrow, {exception}).IsDoneWithAbort());
}
...
void MaglevGraphBuilder::VisitThrowReferenceErrorIfHole() {
  ...
  if (IsConstantNode(value->opcode())) {
    if (IsTheHoleValue(value)) {
      ...
      CHECK(BuildCallRuntime(Runtime::kThrowAccessedUninitializedVariable,
                             {constant})
                .IsDoneWithAbort());
    }
    return;
...
void MaglevGraphBuilder::VisitThrowSuperNotCalledIfHole() {
  ...
  if (IsConstantNode(value->opcode())) {
    if (IsTheHoleValue(value)) {
      CHECK(BuildCallRuntime(Runtime::kThrowSuperNotCalled, {})
                .IsDoneWithAbort());
    }
    return;
  }
...
void MaglevGraphBuilder::VisitThrowSuperAlreadyCalledIfNotHole() {
  ...
  if (IsConstantNode(value->opcode())) {
    if (!IsTheHoleValue(value)) {
      CHECK(BuildCallRuntime(Runtime::kThrowSuperAlreadyCalledError, {})
                .IsDoneWithAbort());
    }
    return;
  }
...
void MaglevGraphBuilder::VisitAbort() {
  ...
  CHECK(BuildAbort(reason).IsDoneWithAbort());
}
