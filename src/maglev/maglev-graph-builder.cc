...
ReduceResult MaglevGraphBuilder::TryReduceArrayPrototypePush(
  ...
  auto build_elements_block =
      [&](base::Optional<MaglevSubGraphBuilder::Label>& label,
          ElementsKind kind) {
        if (label.has_value()) {
          sub_graph.Bind(&*label);
          ValueNode* new_array_length = build_array_push(kind);
          sub_graph.set(var_new_array_length, new_array_length);
          sub_graph.Goto(&return_value);
        }
      };

  build_elements_block(double_elements, PACKED_DOUBLE_ELEMENTS);
  build_elements_block(smi_elements, PACKED_SMI_ELEMENTS);
  build_elements_block(object_elements, PACKED_ELEMENTS);
