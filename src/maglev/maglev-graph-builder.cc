...
void MaglevGraphBuilder::VisitGetKeyedProperty() {
  ...
  if (current_for_in_index_ != nullptr && current_for_in_receiver_ == object &&
    ...
    // TODO(leszeks): Cache the enum cache node.
    auto* descriptor_array = AddNewNode<LoadTaggedField>(
        {current_for_in_cache_type_}, Map::kInstanceDescriptorsOffset);
    auto* enum_cache = AddNewNode<LoadTaggedField>(
        {descriptor_array}, DescriptorArray::kEnumCacheOffset);
    auto* cache_array =
        AddNewNode<LoadTaggedField>({enum_cache}, EnumCache::kIndicesOffset);
    //  TODO(leszeks): Cache the field index.
    auto* field_index =
