...
ReduceResult MaglevGraphBuilder::ReduceCallWithArrayLikeForArgumentsObject(
  ...
  if (arguments_object.IsTopLevelArgumentsObject()) {
    ...
    if (arguments_object.IsRestParameter()) {
      CapturedValue elements = arguments_object.get(JSArray::kElementsOffset);
      DCHECK_EQ(elements.type, CapturedValue::kArgumentsElements);
      start_index = elements.arguments_elements->formal_parameter_count();
    }
    return AddNewCallNode<CallForwardVarargs>(args, target_node, GetContext(),
                                              start_index, target_type);
  }

  DCHECK(arguments_object.IsInlinedArgumentsObject());
...
MaglevGraphBuilder::TryGetNonEscapingArgumentsObject(ValueNode* value) {
  ...
  //   compiler::NativeContextRef native_context =
  //   broker()->target_native_context(); if
  //   (native_context.strict_arguments_map(broker()).equals(map)) {
  //     return true;
  //   }
  //   // TODO(victorgomes): We can loose the mapped_count == 0 requirement if
  //   there
  //   // is no stores to  the mapped arguments.
  //   CapturedValue elements_value =
  //   object.get(JSArgumentsObject::kElementsOffset);
  ...
  //   const CapturedObject& elements =
  //   elements_node->captured_allocation().object; compiler::MapRef
  //   elements_map = elements.GetMap();
  ...
  //   elements.get(SloppyArgumentsElements::kLengthOffset);
  ...
  switch (type) {
      ...
      if (parameter_count_without_receiver() == 0) {
        ...
        if (is_inline()) {
          ...
          return CapturedObject::CreateArgumentsObject(
              zone(), CapturedObject::kInlinedSloppyUnmappedArgumentsObject,
          ...
          return CapturedObject::CreateArgumentsObject(
              zone(), CapturedObject::kTopLevelSloppyUnmappedArgumentsObject,
              broker()->target_native_context().sloppy_arguments_map(broker()),
        ...
        if (is_inline()) {
          ...
          return CapturedObject::CreateArgumentsObject(
              zone(), CapturedObject::kInlinedSloppyMappedArgumentsObject,
          ...
          return CapturedObject::CreateArgumentsObject(
              zone(), CapturedObject::kTopLevelSloppyMappedArgumentsObject,
              broker()->target_native_context().fast_aliased_arguments_map(
                  broker()),
              CapturedValue(GetTaggedValue(length)), CapturedValue(elements),
              GetClosure());
    ...
    case CreateArgumentsType::kUnmappedArguments:
      if (is_inline()) {
        ...
        CapturedValue elements = BuildInlinedArgumentsElements(0, length);
        return CapturedObject::CreateArgumentsObject(
            zone(), CapturedObject::kInlinedStrictArgumentsObject,
        ...
        ArgumentsElements* elements = AddNewNode<ArgumentsElements>(
            ...
            parameter_count_without_receiver());
        return CapturedObject::CreateArgumentsObject(
            zone(), CapturedObject::kTopLevelStrictArgumentsObject,
            broker()->target_native_context().strict_arguments_map(broker()),
            CapturedValue(GetTaggedValue(length)), CapturedValue(elements));
      }
    case CreateArgumentsType::kRestParameter:
      if (is_inline()) {
        ...
        return CapturedObject::CreateArgumentsObject(
            zone(), CapturedObject::kInlinedRestParameter,
        ...
        return CapturedObject::CreateArgumentsObject(
            zone(), CapturedObject::kTopLevelRestParameter,
            broker()->target_native_context().js_array_packed_elements_map(
                broker()),
