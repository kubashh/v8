...
ValueNode* MaglevGraphBuilder::GetTaggedValue(
  ...
  if (auto alt = alternative.tagged()) {
    return alt;
  }
...
ValueNode* MaglevGraphBuilder::GetTruncatedInt32ForToNumber(ValueNode* value,
  ...
  // too.
  if (auto alt = alternative.int32()) {
    return alt;
  }
  if (auto alt = alternative.truncated_int32_to_number()) {
    return alt;
  }
...
ValueNode* MaglevGraphBuilder::GetFloat64ForToNumber(ValueNode* value,
  ...
  if (auto alt = alternative.float64()) {
    return alt;
  }
...
bool CanInlineArrayIteratingBuiltin(compiler::JSHeapBroker* broker,
                                    const NodeInfo::PossibleMapsAccessor& maps,
                                    ElementsKind* kind_return) {
  DCHECK_NE(0, maps.size());
  *kind_return = (*maps.begin()).elements_kind();
...
ReduceResult MaglevGraphBuilder::TryReduceArrayForEach(
  ...
  PossibleMaps receiver_maps_before_loop(node_info->possible_maps().begin(),
                                         node_info->possible_maps().end(),
                                         zone());
    ...
    if (current_block_ == nullptr) {
      ...
      if (auto receiver_maps_after_call =
        ...
        recheck_maps_after_call = receiver_maps_before_loop.contains(
            receiver_maps_after_call->begin(), receiver_maps_after_call->end());
      }
    }
  }
...
compiler::HolderLookupResult MaglevGraphBuilder::TryInferApiHolderValue(
  ...
  compiler::MapRef first_receiver_map = *receiver_maps->begin();
