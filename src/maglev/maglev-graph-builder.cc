...
bool MaglevGraphBuilder::CanElideWriteBarrier(ValueNode* object,
  ...
  switch (value->value_representation()) {
    ...
    case ValueRepresentation::kInt32:
      return true;
    case ValueRepresentation::kUint32: {
      // If its alternative conversion node is CheckedSmiTagUint32 or
      // UnsafeSmiTagUint32, value will be converted to a smi when tagged.
      NodeInfo* node_info = GetOrCreateInfoFor(value);
      if (ValueNode* tagged_alt = node_info->alternative().tagged()) {
        return tagged_alt->Is<CheckedSmiTagUint32>() ||
               tagged_alt->Is<UnsafeSmiTagUint32>();
      }
      return false;
    }
    case ValueRepresentation::kFloat64:
    case ValueRepresentation::kHoleyFloat64: {
      // If its alternative conversion node is CheckedSmiTagFloat64, value will
      // be converted to a smi when tagged.
      NodeInfo* node_info = GetOrCreateInfoFor(value);
      if (ValueNode* tagged_alt = node_info->alternative().tagged()) {
        return tagged_alt->Is<CheckedSmiTagFloat64>();
      }
      return false;
    }
    case ValueRepresentation::kIntPtr:
...
ReduceResult MaglevGraphBuilder::TryReduceArrayForEach(
  ...
  sub_builder.GotoIfFalse<BranchIfInt32Compare>(
      &loop_end, {index_int32, original_length}, Operation::kLessThan);
  ...
  if (!result.IsDoneWithAbort()) {
    ...
    if (current_length != original_length) {
      AddNewNode<CheckInt32Condition>({original_length, current_length},
