...
ValueNode* MaglevGraphBuilder::GetTaggedValue(
  ...
  if (auto alt = alternative.try_get<AlternativeNode::kTagged>()) {
    return alt;
  }
  ...
  switch (representation) {
    case ValueRepresentation::kInt32: {
      if (NodeTypeIsSmi(node_info->type())) {
        return alternative.set<AlternativeNode::kTagged>(
...
ValueNode* MaglevGraphBuilder::GetTruncatedInt32ForToNumber(ValueNode* value,
  ...
  // too.
  if (auto alt =
          alternative.try_get<AlternativeNode::kInt32,
                              AlternativeNode::kTruncatedInt32ToNumber>()) {
    return alt;
  }
  ...
  switch (representation) {
    case ValueRepresentation::kTagged: {
      ...
      if (NodeTypeIsSmi(old_type)) {
        ...
        return alternative.set<AlternativeNode::kInt32>(
      ...
      if (NodeTypeIs(old_type, desired_type)) {
        return alternative.set<AlternativeNode::kTruncatedInt32ToNumber>(
...
ValueNode* MaglevGraphBuilder::GetInt32(ValueNode* value) {
  ...
  if (auto alt = alternative.try_get<AlternativeNode::kInt32>()) {
    return alt;
  }
  ...
  switch (representation) {
    case ValueRepresentation::kTagged: {
      ...
      return alternative.set<AlternativeNode::kInt32>(BuildSmiUntag(value));
    }
    case ValueRepresentation::kUint32: {
      if (node_info->is_smi()) {
        return alternative.set<AlternativeNode::kInt32>(
...
ValueNode* MaglevGraphBuilder::GetFloat64ForToNumber(ValueNode* value,
  ...
  if (auto alt = alternative.try_get<AlternativeNode::kFloat64>()) {
    return alt;
  }
    ...
    case ValueRepresentation::kTagged: {
      switch (hint) {
        ...
        case ToNumberHint::kAssumeNumber:
          ...
          return alternative.set<AlternativeNode::kFloat64>(
        ...
        case ToNumberHint::kAssumeNumberOrOddball: {
          ...
          if (NodeTypeIsNumber(node_info->type())) {
            alternative.set<AlternativeNode::kFloat64>(float64_node);
          }
          return float64_node;
        }
      }
...
compiler::OptionalHeapObjectRef MaglevGraphBuilder::TryGetConstant(
  ...
  if (info) {
    if (auto c = info->alternative().try_get<AlternativeNode::kConstant>()) {
...
void MaglevGraphBuilder::SetKnownValue(ValueNode* node,
  ...
  known_info->alternative().set<AlternativeNode::kConstant>(GetConstant(ref));
}
...
class KnownMapsMerger {
 public:
  ...
  void IntersectWithKnownNodeAspects(
    ...
    auto node_info = known_node_aspects.FindInfo(object);
    bool has_node_info = known_node_aspects.IsValid(node_info);
    NodeType type =
...
ReduceResult MaglevGraphBuilder::BuildCheckMaps(
  ...
  if (merger.known_maps_are_subset_of_requested_maps()) {
    ...
    for (compiler::MapRef map :
         *known_node_aspects().TryGetPossibleMapsFor(object)) {
  ...
  switch (object->properties().value_representation()) {
    ...
    case ValueRepresentation::kTagged:
      ...
      if (SmiConstant* constant = object->TryCast<SmiConstant>()) {
        ...
        return alternative.get_or_set<AlternativeNode::kInt32>(
...
BasicBlock* MaglevGraphBuilder::BuildSpecializedBranchIfCompareNode(
  ...
  if (node->value_representation() == ValueRepresentation::kInt32) {
    ...
    if (auto as_int32 =
            node_info->alternative().try_get<AlternativeNode::kInt32>()) {
      return make_specialized_branch_if_compare(ValueRepresentation::kInt32,
                                                as_int32);
    } else if (auto as_float64 = node_info->alternative()
                                     .try_get<AlternativeNode::kFloat64>()) {
      return make_specialized_branch_if_compare(ValueRepresentation::kFloat64,
                                                as_float64);
    } else {
...
void MaglevGraphBuilder::VisitThrowReferenceErrorIfHole() {
  ...
  if (const NodeInfo* info = known_node_aspects().TryGetInfoFor(value)) {
    if (auto alt = info->alternative()
                       .try_get<AlternativeNode::kInt32,
                                AlternativeNode::kTruncatedInt32ToNumber,
                                AlternativeNode::kFloat64>()) {
      return;
    }
  }
