...
void MaglevGraphBuilder::StoreAndCacheContextSlot(ValueNode* context,
  ...
  if (known_node_aspects().may_have_aliasing_contexts ==
    ...
    for (auto& cache : loaded_context_slots) {
      if (cache.first.index == offset && cache.first.context != context) {
        if (ContextMayAlias(cache.first.context, scope_info) &&
            cache.second != value) {
          if (v8_flags.trace_maglev_graph_building) {
            std::cout << "  * Clearing probably aliasing value "
                      << PrintNodeLabel(graph_labeller(), cache.first.context)
                      << "[" << offset
  ...
  if (updated.second) {
    ...
    if (updated.first->second != value) {
      ...
      DCHECK_EQ(loaded_context_slots.at(key), value);
      if (WithinAnEffectRecordingPeelingIteration()) {
        GetCurrentPeeledLoopEffects().context_slot_written.insert(key);
      }
    }
