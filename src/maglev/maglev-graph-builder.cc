...
ValueNode* MaglevGraphBuilder::GetTaggedValue(
  ...
  if (auto alt = alternative.try_get<Alternative::kTagged>()) {
    return *alt;
  }
  ...
  switch (representation) {
    case ValueRepresentation::kInt32: {
      if (NodeTypeIsSmi(node_info->type())) {
        return alternative.set<Alternative::kTagged>(
...
ValueNode* MaglevGraphBuilder::GetTruncatedInt32ForToNumber(ValueNode* value,
  ...
  // too.
  if (auto alt = alternative.try_get<Alternative::kInt32,
                                     Alternative::kTruncatedInt32ToNumber>()) {
    return *alt;
  }
  ...
  switch (representation) {
    case ValueRepresentation::kTagged: {
      ...
      if (NodeTypeIsSmi(old_type)) {
        ...
        return alternative.set<Alternative::kInt32>(
      ...
      if (NodeTypeIs(old_type, desired_type)) {
        return alternative.set<Alternative::kTruncatedInt32ToNumber>(
...
ValueNode* MaglevGraphBuilder::GetInt32(ValueNode* value) {
  ...
  switch (representation) {
    case ValueRepresentation::kTagged: {
      ...
      return alternative.set<Alternative::kInt32>(BuildSmiUntag(value));
    }
    case ValueRepresentation::kUint32: {
      if (node_info->is_smi()) {
        return alternative.set<Alternative::kInt32>(
...
ValueNode* MaglevGraphBuilder::GetFloat64ForToNumber(ValueNode* value,
  ...
  if (auto alt = alternative.try_get<Alternative::kFloat64>()) {
    return *alt;
  }
    ...
    case ValueRepresentation::kTagged: {
      switch (hint) {
        ...
        case ToNumberHint::kAssumeNumber:
          ...
          return alternative.set<Alternative::kFloat64>(
        ...
        case ToNumberHint::kAssumeNumberOrOddball: {
          ...
          if (NodeTypeIsNumber(node_info->type())) {
            alternative.set<Alternative::kFloat64>(float64_node);
          }
          return float64_node;
        }
      }
...
compiler::OptionalHeapObjectRef MaglevGraphBuilder::TryGetConstant(
  ...
  if (info) {
    if (auto c = info->alternative().try_get<Alternative::kConstant>()) {
...
void MaglevGraphBuilder::SetKnownValue(ValueNode* node,
  ...
  known_info->alternative().set<Alternative::kConstant>(GetConstant(ref));
}
 ...
 public:
  explicit KnownMapsMerger(compiler::JSHeapBroker* broker,
      ...
      : broker_(broker),
        ...
        intersect_set_(std::make_unique<PossibleMaps>()) {}
  ...
  void IntersectWithKnownNodeAspects(
    ...
    if (has_node_info && node_info->second.try_get_possible_maps()) {
  ...
  void UpdateKnownNodeAspects(ValueNode* object,
    ...
    node_info->set_possible_maps(
        std::make_unique<PossibleMaps>(*intersect_set_), any_map_is_unstable_);
  ...
  switch (object->properties().value_representation()) {
    ...
    case ValueRepresentation::kTagged:
      ...
      if (SmiConstant* constant = object->TryCast<SmiConstant>()) {
        ...
        return alternative.get_or_set<Alternative::kInt32>(
...
ReduceResult MaglevGraphBuilder::TryReduceArrayForEach(
  ...
  if (!node_info || !node_info->try_get_possible_maps()) {
    if (v8_flags.trace_maglev_graph_building) {
  ...
  if (receiver_maps_were_unstable) {
    node_info->set_possible_maps(
        std::make_unique<PossibleMaps>(receiver_maps_before_loop),
  ...
  if (recheck_maps_after_call) {
    if (current_block_ == nullptr) {
      ...
      if (auto receiver_maps_after_call =
              known_node_aspects().TryGetPossibleMapsFor(receiver)) {
...
ReduceResult MaglevGraphBuilder::TryReduceArrayPrototypePush(
  ...
  auto possible_maps = known_node_aspects().TryGetPossibleMapsFor(receiver);
...
compiler::HolderLookupResult MaglevGraphBuilder::TryInferApiHolderValue(
  ...
  auto receiver_maps = known_node_aspects().TryGetPossibleMapsFor(receiver);
  if (!receiver_maps) {
    // No info about receiver, can't infer API holder.
    return not_found;
  }
  DCHECK(!receiver_maps->is_empty());
...
BasicBlock* MaglevGraphBuilder::BuildSpecializedBranchIfCompareNode(
  ...
  if (node->value_representation() == ValueRepresentation::kInt32) {
    ...
    if (auto as_int32 =
            node_info->alternative().try_get<Alternative::kInt32>()) {
      return make_specialized_branch_if_compare(ValueRepresentation::kInt32,
                                                *as_int32);
    } else if (auto as_float64 =
                   node_info->alternative().try_get<Alternative::kFloat64>()) {
      return make_specialized_branch_if_compare(ValueRepresentation::kFloat64,
                                                *as_float64);
    } else {
...
void MaglevGraphBuilder::VisitThrowReferenceErrorIfHole() {
  ...
  if (const NodeInfo* info = known_node_aspects().TryGetInfoFor(value)) {
    if (auto alt = info->alternative()
                       .try_get<Alternative::kInt32,
                                Alternative::kTruncatedInt32ToNumber,
                                Alternative::kFloat64>()) {
      return;
    }
  }
