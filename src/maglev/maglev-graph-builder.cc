...
ReduceResult MaglevGraphBuilder::TryBuildInlinedCall(
  ...
  if (result.IsDoneWithoutValue()) {
    ...
    // The current block was inherited from the inlined function (or passed
    // through to the outer function, in which case there is nothing left to
    // process in the current function).
    DCHECK_IMPLIES(
