...
enum class BlockProcessResult {
  ...
  kSkip,      // Skip processing this block (for all processors!).
};
...
enum class ProcessResult {
  ...
  kSkipBlock,  // Stop processing this block and skip the rest for all
               // processors.
};
 ...
 public:
  ...
  ProcessResult Process(Node* node, const ProcessingState& state) {
    ...
    switch (res) {
      ...
      case ProcessResult::kHoist:
      case ProcessResult::kSkipBlock:
        // TODO(olivf): Combining these with multi-node processors seems
        // dangerous and probably needs some more thinking.
        UNREACHABLE();
    }
  }
  void PreProcessGraph(Graph* graph) {
