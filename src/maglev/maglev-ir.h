...
class VirtualObject : public FixedInputValueNodeT<0, VirtualObject> {
 ...
 public:
  explicit VirtualObject(uint64_t bitfield, compiler::MapRef map,
      ...
      : Base(bitfield), map_(map), slot_count_(slot_count), slots_(slots) {}

  explicit VirtualObject(uint64_t bitfield, compiler::MapRef map,
                         Float64 number)
      : Base(bitfield), map_(map), number_(number) {}

  explicit VirtualObject(uint64_t bitfield, compiler::MapRef map,
                         compiler::FixedDoubleArrayRef elements)
      : Base(bitfield), map_(map), double_elements_(elements) {}
  ...
  Float64 number() const { return number_; }

  compiler::FixedDoubleArrayRef double_elements() const {
    return double_elements_;
  }

  uint32_t slot_count() const {
    DCHECK(!map().IsHeapNumberMap());
    DCHECK(!map().IsFixedDoubleArrayMap());
    return slot_count_;
  }
  ...
  size_t size() const {
    ...
    if (map().IsFixedDoubleArrayMap()) {
      return FixedDoubleArray::SizeFor(double_elements_.length());
    }
    return (slot_count_ + 1) * kTaggedSize;
  }
  ...
  void set(uint32_t offset, ValueNode* value) {
    DCHECK_NE(offset, 0);  // Don't try to get the map through this setter.
    DCHECK(!map().IsHeapNumberMap());
    DCHECK(!map().IsFixedDoubleArrayMap());
    offset -= kTaggedSize;
    SBXCHECK_LT(offset / kTaggedSize, slot_count_);
  ...
  void ClearSlots(int last_init_slot, ValueNode* clear_value) {
    ...
    for (uint32_t i = last_init_index; i < slot_count_; i++) {
      slots_[i] = clear_value;
    }
  }
 ...
 private:
  ...
  union {
    Float64 number_;  // If a heap number, we don't use any slots.
    compiler::FixedDoubleArrayRef double_elements_;  // If a double fixed array.
    struct {
      uint32_t slot_count_;  // Does not count the map.
      ValueNode** slots_;    // Does not contain the map.
    };
  };
};
