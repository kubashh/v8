        ...
        if (input->Is<Identity>()) {
          ...
          if (!n->properties().is_conversion()) {
            if constexpr (std::is_same_v<NodeT, CheckedStoreSmiField>) {
              ...
              if constexpr (std::is_same_v<NodeT,
                                           StoreTaggedFieldNoWriteBarrier>) {
                if (i == StoreTaggedFieldNoWriteBarrier::kValueIndex &&
                    phi->value_representation() !=
                        ValueRepresentation::kTagged) {
                  // Since we re-tagged {phi}, it's possible that it became a
                  // HeapObject (either because {phi} is a Float64 phi, or
                  // because it's a Int32/Uint32 phi that doesn't fit on 31
                  // bits), so we can't drop the write barrier.
                  node->OverwriteWith(
                      Opcode::kStoreTaggedFieldWithWriteBarrier,
                      StoreTaggedFieldWithWriteBarrier::kProperties);
                }
              }
            }
          }
