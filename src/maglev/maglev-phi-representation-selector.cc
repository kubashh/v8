...
void MaglevPhiRepresentationSelector::UpdateNodePhiInput(
    ...
    const ProcessingState& state) {
  if (input_index == StoreFixedArrayElementNoWriteBarrier::kElementsIndex) {
    // The elements array should always be tagged.
    DCHECK_NE(phi->value_representation(), ValueRepresentation::kTagged);
    return;
  }
  if (input_index == StoreFixedArrayElementNoWriteBarrier::kIndexIndex) {
    UpdateNodePhiInput(static_cast<NodeBase*>(node), phi, input_index, state);
    return;
  }
  DCHECK_EQ(input_index, StoreFixedArrayElementNoWriteBarrier::kValueIndex);
...
void MaglevPhiRepresentationSelector::UpdateNodePhiInput(
  ...
  DCHECK_EQ(input_index, CheckedStoreFixedArraySmiElement::kValueIndex);
  node->change_input(input_index, SmiTagPhi(phi, node, state));
}
...
ValueNode* MaglevPhiRepresentationSelector::SmiTagPhi(
    Phi* phi, FromNodeT* node, const ProcessingState& state) {
...
#undef TAG_INPUT
  ...
  tagged->CopyEagerDeoptInfoOf(node, builder_->zone());
...
#endif
  node->template OverwriteWith<ToNodeT>();
  return tagged;
}
