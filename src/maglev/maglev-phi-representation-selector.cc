...
ProcessResult MaglevPhiRepresentationSelector::Process(Phi* node,
  ...
  for (int i = 0; i < node->input_count(); i++) {
    ...
    if (input->Is<SmiConstant>()) {
      ...
      // TODO(olivf): Unless we untag OSR values, speculatively untagging could
      // end us in deopt loops. To enable this by default we need to add some
      // feedback to be able to back off. Or, ideally find the respective
      // checked conversion form within the loop to wire up the feedback
      // collection.
      // TODO(olivf): Currently there is no hard guarantee that the phi merge
      // state has a checkpointed jump.
      if (v8_flags.maglev_speculative_hoist_phi_untagging) {
        if (node->is_loop_phi() &&
            node->merge_state()
                ...
                ->control_node()
                ->Is<CheckpointedJump>() &&
            (input->Is<InitialValue>() || i == 0 ||
             (input->has_id() && input->id() < node->merge_state()
                                                   ->predecessor_at(0)
                                                   ->control_node()
                                                   ->id()))) {
...
#define TRACE_INPUT_LABEL \
  "    @ Input " << i << " (" << PrintNodeLabel(graph_labeller(), input) << ")"
    ...
    if (input->Is<SmiConstant>()) {
      ...
      DeoptFrame* deopt_frame = &block->control_node()
                                     ->Cast<CheckpointedJump>()
                                     ->eager_deopt_info()
                                     ->top_frame();
      ValueNode* untagged;
      auto static_type = StaticTypeForNode(builder_->broker(),
