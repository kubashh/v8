...
class Alternative {
 public:
  ...
  template <AlternativeKind kind>
  bool has() const {
    static_assert(kind < AlternativeKind::kNumberOfAlternativeKinds);
  ...
  template <AlternativeKind... kinds>
  base::Optional<ValueNode*> try_get() const {
  ...
  Alternative() { values_.fill(nullptr); }

  bool is_empty() const { return values_ == Alternative().values_; }

  void MergeWith(const Alternative& other) {
    for (int i = 0;
         i < static_cast<int>(AlternativeKind::kNumberOfAlternativeKinds);
         ++i) {
 ...
 private:
  ...
  template <>
  struct GetAnyImpl<> {
    static base::Optional<ValueNode*> result(const Alternative& a) {
      return {};
    }
  };
  template <AlternativeKind kind, AlternativeKind... kinds>
  struct GetAnyImpl<kind, kinds...> {
    static base::Optional<ValueNode*> result(const Alternative& a) {
  ...
  std::array<ValueNode*,
             static_cast<int>(AlternativeKind::kNumberOfAlternativeKinds)>
      values_;

  // Prevent callers from copying these when they try to update the
  // alternatives by making these private.
  Alternative(const Alternative&) V8_NOEXCEPT = default;
  Alternative& operator=(const Alternative&) V8_NOEXCEPT = default;
...
class NodeInfo {
 public:
  ...
  struct StripAllMapsOnCopy {
    const NodeInfo& val;
  };
  explicit NodeInfo(const StripAllMapsOnCopy& other) V8_NOEXCEPT {
    type_ = other.val.type_;
    alternative_ = other.val.alternative_;
  }

  struct StripUnstableMapsOnCopy {
    const NodeInfo& val;
  };
  explicit NodeInfo(const StripUnstableMapsOnCopy& other) V8_NOEXCEPT {
    type_ = other.val.type_;
    alternative_ = other.val.alternative_;
  ...
  const Alternative& alternative() const { return alternative_; }
  Alternative& alternative() { return alternative_; }
  ...
  void MergeWith(const NodeInfo& other, bool& any_merged_map_is_unstable,
    ...
    if (other.possible_maps_) {
      if (!possible_maps_) {
        possible_maps_.reset(new PossibleMaps());
      }
  ...
  void set_possible_maps(std::unique_ptr<PossibleMaps> possible_maps,
                         bool any_map_is_unstable) {
    possible_maps_ = std::move(possible_maps);
    any_map_is_unstable_ = any_map_is_unstable;
  }

  void set_possible_maps(compiler::MapRef map, bool any_map_is_unstable) {
    set_possible_maps(std::make_unique<PossibleMaps>(map), any_map_is_unstable);
  }
    ...
    for (const auto& it : node_infos) {
      if (any_map_for_any_node_is_unstable) {
        clone->node_infos.emplace(it.first,
                                  NodeInfo::StripAllMapsOnCopy{it.second});
      } else {
        ...
        // map.
        clone->node_infos.emplace(it.first,
                                  NodeInfo::StripUnstableMapsOnCopy{it.second});
      }
    }
   ...
   public:
    ...
    explicit Alternatives(const NodeInfo* node_info)
        : node_type_(node_info ? node_info->type() : NodeType::kUnknown),
          tagged_alternative_(node_info ? node_info->alternative()
                                              .try_get<Alternative::kTagged>()
                                              .value_or(nullptr)
                                        : nullptr) {}
