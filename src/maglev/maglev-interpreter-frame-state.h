 ...
 public:
  ...
  class AlternativeNodes {
   public:
    ...
    static constexpr size_t number_of_alternatives = 5;
#define ALTERNATIVES(V)           \
  V(tagged, 0)                    \
  V(int32, 1)                     \
  V(truncated_int32_to_number, 2) \
  V(float64, 3)                   \
  V(constant, 4)

#define API(kind, idx)                                                      \
  ValueNode* kind() const { return store_[idx]; }                           \
  ValueNode* set_##kind(ValueNode* val) { return store_[idx] = val; }       \
  ValueNode* get_or_set_##kind(const std::function<ValueNode*()>& create) { \
    if (store_[idx]) return store_[idx];                                    \
    return store_[idx] = create();                                          \
  }
    ALTERNATIVES(API)
#undef API
#undef ALTERNATIVES

    bool is_empty() const { return store_ == AlternativeNodes().store_; }
   ...
   private:
    std::array<ValueNode*, number_of_alternatives> store_;
  ...
  struct PossibleMapsAccessor {
    const PossibleMaps::const_iterator begin_;
    const PossibleMaps::const_iterator end_;

    PossibleMaps::const_iterator begin() const { return begin_; }
    PossibleMaps::const_iterator end() const { return end_; }
    size_t size() const { return end_ - begin_; }
    bool is_empty() const { return begin_ == end_; }
  };

  PossibleMapsAccessor possible_maps() const {
    CHECK(possible_maps_are_known_);
    return PossibleMapsAccessor{possible_maps_.begin(), possible_maps_.end()};
  }

  base::Optional<PossibleMapsAccessor> try_get_possible_maps() const {
    if (possible_maps_are_known_) {
...
struct KnownNodeAspects {
  ...
  base::Optional<NodeInfo::PossibleMapsAccessor> TryGetPossibleMapsFor(
      ValueNode* node) const {
    auto info_it = FindInfo(node);
    if (!IsValid(info_it)) return {};
