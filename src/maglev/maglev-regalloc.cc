...
void StraightForwardRegisterAllocator::AllocateNodeResult(ValueNode* node) {
  ...
  if (operand.basic_policy() == compiler::UnallocatedOperand::FIXED_SLOT) {
    ...
    if (idx > 0) {
      ...
      CHECK_GE(tagged_.top, tagged_.pinned_slots_high_watermark_);
      CHECK_GE(idx, tagged_.pinned_slots_high_watermark_);
      for (int i = tagged_.pinned_slots_high_watermark_; i < idx; ++i) {
        tagged_.free_slots.emplace_back(i, node->live_range().start);
      }
      tagged_.top = tagged_.pinned_slots_high_watermark_ = idx + 1;
    }
    return;
  }
