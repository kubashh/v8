...
void Call::GenerateCode(MaglevAssembler* masm, const ProcessingState& state) {
  ...
  __ PushArguments(PushArgumentsIterator<Input*>{args_rbegin(), args_rend()});
...
void CallKnownJSFunction::GenerateCode(MaglevAssembler* masm,
  ...
  if (actual_parameter_count < expected_parameter_count_) {
    ...
    __ PushArguments(
        PushArgumentRepeat<Register>{kScratchRegister, number_of_undefineds},
        PushArgumentsIterator<Input*>{args_rbegin(), args_rend()}, receiver());
  } else {
    __ PushArguments(PushArgumentsIterator<Input*>{args_rbegin(), args_rend()},
                     receiver());
  }
