...
void Float64Box::GenerateCode(MaglevCodeGenState* code_gen_state,
                              const ProcessingState& state) {
  // TODO(victorgomes): Check if it fits on a Smi and return the Smi instead.
...
void Float64Unbox::GenerateCode(MaglevCodeGenState* code_gen_state,
  ...
  __ CompareRoot(Operand(value, -kHeapObjectTag), RootIndex::kHeapNumberMap);
  EmitEagerDeoptIf(not_equal, code_gen_state, this);
  __ Movsd(ToDoubleRegister(result()),
           Operand(value, kTaggedSize - kHeapObjectTag));
  __ bind(&done);
}
