  ...
  __ JumpToDeferredIf(
      ...
      [](MaglevAssembler* masm, CheckValueEqualsString* node,
        ...
        AddDeoptRegistersToSnapshot(&snapshot, node->eager_deopt_info());
        {
          ...
          // // Move target into arg_reg_2 first, in case target aliases
          // arg_reg_1.
          // __ Move(arg_reg_2, target);
          // __ Move(arg_reg_1,
          ...
          // // If internalization succeeded, we can now again compare by
          // // reference. Do this before restoring registers, so that the deopt
          // // below has the correct register set.
          // __ Cmp(target, node->value().object());
          // Left and Length should already be in the right places.
