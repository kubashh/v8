...
enum class TypedArrayKind { Int, Uint, Float };
...
void GenerateTypedArrayLoad(MaglevAssembler* masm, NodeT* node, Register object,
  ...
  if constexpr (std::is_same_v<ResultReg, Register>) {
    ...
    DCHECK_EQ(type, TypedArrayKind::Float);
    switch (kind) {
...
#define DEF_OPERATION(Name, ResultReg, ToResultReg, typed_array_kind,         \
                      check_detached)                                         \
