...
#define DEFINE_BUILTIN_REDUCER(Name) \
  bool TryReduce##Name(const CallArguments& args);
  MAGLEV_REDUCED_BUILTIN(DEFINE_BUILTIN_REDUCER)
#undef DEFINE_BUILTIN_REDUCER

  bool TryReduceBuiltin(Builtin builtin, const CallArguments& args);

  bool TryBuildCallKnownJSFunction(compiler::JSFunctionRef function,
                                   const CallArguments& args);

  void BuildGenericCall(ValueNode* target, Call::TargetType target_type,
                        const CallArguments& args,
                        compiler::FeedbackSource& feedback_source);
