 ...
 private:
  ...
  void VisitSingleBytecode() {
    ...
    if (V8_UNLIKELY(merge_state != nullptr)) {
      // Clear available context slot stores when there is any controlflow.
      // TODO(olivf): More precision could be acchieved by tracking dominating
...
#undef DEFINE_BUILTIN_REDUCER
  ...
  inline void DeleteNode(Node* node) {
    for (int i = 0; i < node->input_count(); ++i) {
      node->change_input(i, GetSmiConstant(0));
    }
    node->OverwriteWith(Opcode::kDead);
  }

  ZoneMap<KnownNodeAspects::LoadedContextSlotsKey, Node*>
