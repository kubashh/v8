...
class MaglevFrameTranslationBuilder {
 ...
 private:
  ...
  void BuildFixedDoubleArray(compiler::FixedDoubleArrayRef array) {
  ...
  void BuildVirtualObject(VirtualObject* object,
                          const InputLocation*& input_location) {
    if (object->map().IsHeapNumberMap()) {
      BuildHeapNumber(object->number());
      return;
    }
    if (object->map().IsFixedDoubleArrayMap()) {
      BuildFixedDoubleArray(object->double_elements());
      return;
    }
  ...
  void BuildDeoptFrameSingleValue(const ValueNode* value,
    ...
    if (const InlinedAllocation* alloc = value->TryCast<InlinedAllocation>()) {
      if (alloc->HasBeenElided()) {
        input_location++;
        BuildVirtualObject(alloc->object(), input_location);
        return;
      }
