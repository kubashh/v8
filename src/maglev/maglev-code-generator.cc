 ...
 private:
  ...
  void RecordMoves(const MaglevCompilationUnit& unit, BasicBlock* catch_block,
    ...
    for (Phi* phi : *catch_block->phis()) {
      DCHECK_EQ(phi->input_count(), 0);  // Since it's an ExceptionPhi.
  ...
  void EmitMaterialisationsAndPushResults() const {
    ...
    if (save_accumulator_) __ Push(kReturnRegister0);
    for (const Move& move : materialising_moves_) {
      MaterialiseTo(move.source, kReturnRegister0);
      __ Push(kReturnRegister0);
    }
  }

  void EmitPopMaterialisedResults() const {
    if (materialising_moves_.size() == 0) return;
    __ RecordComment("EmitPopMaterialisedResults");
    for (auto it = materialising_moves_.rbegin();
    ...
    if (save_accumulator_) __ Pop(kReturnRegister0);
  }
