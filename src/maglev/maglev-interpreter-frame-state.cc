...
bool TypeMatches(const NodeInfo& before, const NodeInfo& after) {
...
bool KnownNodeAspects::IsCompatibleWithLoopHeader(
  ...
  if (!had_effects) {
    if (!AspectIncludes(loop_header.node_infos, node_infos, TypeMatches)) {
      return false;
    }
...
void MergePointInterpreterFrameState::InitializeLoop(
  ...
  MergeVirtualObjects(builder, compilation_unit, unmerged, predecessor);
  ...
  auto UpdateLoopPhiType = [&](Phi* result, NodeType unmerged_type) {
    ...
    if (predecessors_so_far_ == 0) {
      ...
      if (optimistic_loop_phis) {
        ...
        // generalized here.
        NodeType initial = unmerged_type == NodeType::kSmi ? NodeType::kNumber
