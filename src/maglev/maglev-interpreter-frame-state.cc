...
__attribute__((optnone)) void error1() {
  std::cout << "Error 1\n" << std::flush;
  std::cerr << "Error 1\n" << std::flush;
  UNREACHABLE();
}

__attribute__((optnone)) void error2() {
  std::cout << "Error 2\n" << std::flush;
  std::cerr << "Error 2\n" << std::flush;
  UNREACHABLE();
}

__attribute__((optnone)) void error3() {
  std::cout << "Error 3\n" << std::flush;
  std::cerr << "Error 3\n" << std::flush;
  UNREACHABLE();
}

__attribute__((optnone)) void error4() {
  std::cout << "Error 4\n" << std::flush;
  std::cerr << "Error 4\n" << std::flush;
  UNREACHABLE();
}
...
ValueNode* MergePointInterpreterFrameState::MergeValue(
  ...
  if (!per_predecessor_alternatives->first()) {
    error2();
    // std::cout << "Error 2\n" << std::flush;
    // std::cerr << "Error 2\n" << std::flush;
    // UNREACHABLE();
  }
  // CHECK_NOT_NULL(per_predecessor_alternatives);
  // CHECK_NOT_NULL(per_predecessor_alternatives->first());
  ...
  NodeType type = merged_type != NodeType::kUnknown
                      ? merged_type
                      : per_predecessor_alternatives->first()->node_type();
  int i = 0;
