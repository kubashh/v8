...
NodeType GetNodeType(compiler::JSHeapBroker* broker, LocalIsolate* isolate,
  ...
  // If this node has no NodeInfo, we fall back to its static type.
  return StaticTypeForNode(broker, isolate, node);
}

NodeType GetNodeType(
    compiler::JSHeapBroker* broker, LocalIsolate* isolate,
    const MergePointInterpreterFrameState::Alternatives* alternatives,
    ValueNode* node) {
  // We first check the alternatives in order to return the most precise
  // type possible.
  if (alternatives->node_type() != NodeType::kUnknown) {
    return alternatives->node_type();
  }
  // If this node has no NodeInfo, we fall back to its static type.
  return StaticTypeForNode(broker, isolate, node);
}
...
ValueNode* MergePointInterpreterFrameState::MergeValue(
  ...
  for (const Alternatives* alt : *per_predecessor_alternatives) {
    ...
    if (i == 0) {
      type =
          GetNodeType(builder->broker(), builder->local_isolate(), alt, merged);
    } else {
      type = IntersectType(
          type, GetNodeType(builder->broker(), builder->local_isolate(), alt,
                            merged));
    }
    i++;
  }
