...
class V8_EXPORT_PRIVATE BytecodeRegisterOptimizer final
 ...
 public:
  ...
  Variable* GetVariableInEquivalenceSetOfRegister(Register re);

  // Return true if the var is the variable which was stored in the accumulator
  // now.
  bool IsRegisterInEquivalenceSetOfVariable(Variable* var, Register re);
 ...
 private:
  ...
  uint32_t NextEquivalenceId() {
    equivalence_id_++;
    variable_equivalence_set_map_.resize(equivalence_id_ + 1);
