...
void BytecodeGenerator::VisitNot(UnaryOperation* expr) {
  if (execution_result()->IsEffect()) {
    ...
    VisitInSameExecutionScope(expr->expression());
  } else {
...
void BytecodeGenerator::BuildLogicalTest(Token::Value token, Expression* left,
  ...
  TestFallthrough fallthrough = test_result->fallthrough();
  {
    ...
    VisitInSameExecutionScope(left);
...
void BytecodeGenerator::VisitInSameExecutionScope(Expression* expr) {
  {
    RegisterAllocationScope reg_scope(this);
    Visit(expr);
  }
  if (execution_result()->IsTest() &&
      !execution_result()->AsTest()->result_consumed_by_test()) {
