...
size_t BytecodeGenerator::AllocateSharedFunctionInfoEntry(
...
void BytecodeGenerator::VisitFunctionLiteral(FunctionLiteral* expr) {
  ...
  size_t entry = AllocateSharedFunctionInfoEntry(expr);
...
void BytecodeGenerator::BuildClassLiteral(ClassLiteral* expr) {
...

  {
    RegisterAllocationScope register_scope(this);
    RegisterList args =
        register_allocator()->NewRegisterList(3 + dynamic_arguments_count);
...
void BytecodeGenerator::VisitClassLiteralProperties(ClassLiteral* expr,
  ...
  // Create nodes to compute method names that are not known at compile time.
  for (int i = 0; i < expr->properties()->length(); i++) {
    ClassLiteral::Property* property = expr->properties()->at(i);
    if (property->is_computed_name()) {
