...
void BytecodeGenerator::GenerateBytecodeBody() {
  ...
  if (literal->kind() == FunctionKind::kDerivedConstructor) {
    ...
    BytecodeLabels load_this(zone());
    BytecodeLabel do_return;
    if (check_return_value.empty() && !builder()->RemainderOfBlockIsDead()) {
      BuildThisVariableLoad();
      BuildReturn(literal->return_position());
    } else {
      if (!builder()->RemainderOfBlockIsDead()) {
        builder()->Jump(load_this.New());
      }

      check_return_value.Bind(builder());
      builder()->JumpIfUndefined(load_this.New()).Jump(&do_return);

      {
        load_this.Bind(builder());
        BuildThisVariableLoad();
      }

      {
        builder()->Bind(&do_return);
        BuildReturn(literal->return_position());
      }
    }
  } else {
