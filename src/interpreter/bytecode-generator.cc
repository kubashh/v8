...
class BytecodeGenerator::ExpressionResultScope {
 public:
  ...
  virtual ~ExpressionResultScope() { generator_->set_execution_result(outer_); }
...
void BytecodeGenerator::BuildCreateArrayLiteral(
  ...
  for (; current != end; ++current) {
    ...
    if (subexpr->IsSpread()) {
      ...
      BuildFillArrayWithIterator(iterator, array, index, value, Register(),
...
//   var array = []
//   var index = 0
//   %BuildFillArrayWithIterator(iterator, array, index, done)
//   done = true
//   b = array
//
// } catch(e) {
...
void BytecodeGenerator::BuildDestructuringArrayAssignment(
  ...
  Register iteration_exception = register_allocator()->NewRegister();
  builder()->LoadTheHole();
  builder()->StoreAccumulatorInRegister(iteration_exception);
  ...
  // We want to close the iterator if it is not done already, but we suppress
  // any exceptions it might throw if there was already an exception during
  // the destructuring.
  //
  // finally {
  //   try {
  ...
  // }
  {
    RegisterAllocationScope inner_try_scope(this);
    TryCatchBuilder inner_try_control_builder(builder(), nullptr, nullptr,
                                              HandlerTable::DESUGARING);
    ...
    // Evaluate the try-block inside a control scope. This simulates a handler
    // that is intercepting 'throw' control commands.
    inner_try_control_builder.BeginTry(context);
    {
      ControlScopeForTryCatch scope(this, &inner_try_control_builder);

      // if (!done) iterator.close()
      BytecodeLabel iterator_is_done;
      if (!pattern->values()->is_empty()) {
        // Empty destructuring closes unconditionally, so only need to check
        // for done if there was some destructuring.
    ...
    inner_try_control_builder.EndTry();
    ...
    commands.JumpIfRethrow(&suppress_close_exception);
    builder()
        ->LoadAccumulatorWithRegister(close_exception)
        .ReThrow()
        .Bind(&suppress_close_exception);
    inner_try_control_builder.EndCatch();
  }
