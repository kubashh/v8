...
void BytecodeGenerator::VisitCall(Call* expr) {
  ...
  const bool build_reflect_apply = first_spread_index < arguments_count - 1;
  ...
  switch (call_type) {
    ...
    case Call::GLOBAL_CALL: {
      ...
      if (first_spread_index == arguments_count && !optimize_as_one_shot) {
  ...
  if (build_reflect_apply) {
    // If there is a non-final spread, we rewrite calls like
    //     callee(1, ...x, 2)
    // to
    //     %reflect_apply(callee, receiver, [1, ...x, 2])
    //
    // The callee and receiver code above already loaded them into the correct
    // registers in args.
    ...
    CHECK_EQ(receiver_arg_count + arguments_count, args.register_count());
  }

  // Resolve callee for a potential direct eval call. This block will mutate the
  // callee value.
  if (expr->is_possibly_eval() && arguments_count > 0) {
    RegisterAllocationScope inner_register_scope(this);
  ...
  if (first_spread_index == arguments_count - 1) {
    // If there is only one spread and it's the last argument, there is a
    // specialized bytecode.
    DCHECK(!implicit_undefined_receiver);
    builder()->CallWithSpread(callee, args,
                              feedback_index(feedback_spec()->AddCallICSlot()));
  } else if (build_reflect_apply) {
