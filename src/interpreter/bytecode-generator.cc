...
void BytecodeGenerator::BuildClassLiteral(ClassLiteral* expr, Register name) {
  ...
  AccessorTable<ClassLiteral::Property> private_accessors(zone());
  {
    ...
    // Assign holes to private names for TDZ access checks.
    VariableMap* private_names = expr->scope()->private_names();
    if (private_names != nullptr && private_names->occupancy() != 0) {
      for (auto p = private_names->Start(); p != nullptr;
           p = private_names->Next(p)) {
        Variable* var = reinterpret_cast<Variable*>(p->value);
        DCHECK_EQ(var->location(), VariableLocation::CONTEXT);
        builder()->LoadTheHole().StoreContextSlot(execution_context()->reg(),
                                                  var->index(), 0);
      }
    }
