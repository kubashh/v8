...
class BytecodeGenerator::DeferredCommands final {
 public:
  ...
  struct Entry {
    // The command type being applied on this path.
    ControlScope::Command command;
    // The target statement for the command or {nullptr}.
    Statement* statement;
    // A token identifying this particular path.
    int token;
  };
  ...
  void RecordCommand(ControlScope::Command command, Statement* statement) {
  ...
  void RecordHandlerReThrowPath() {
    ...
    RecordCommand(ControlScope::CMD_RETHROW, nullptr);
  }
  ...
  void JumpIfRethrow(BytecodeLabel* label) {
    builder()
        ->LoadLiteral(Smi::FromInt(GetRethrowToken()))
        .CompareReference(token_register_)
        .JumpIfTrue(ToBooleanMode::kAlreadyBoolean, label);
  }
 ...
 private:
  int GetTokenForCommand(ControlScope::Command command, Statement* statement) {
    switch (command) {
      case ControlScope::CMD_RETURN:
        return GetReturnToken();
      case ControlScope::CMD_ASYNC_RETURN:
        return GetAsyncReturnToken();
      case ControlScope::CMD_RETHROW:
  ...
  int GetReturnToken() {
    if (return_token_ == -1) {
      return_token_ = GetNewTokenForCommand(ControlScope::CMD_RETURN, nullptr);
    }
  ...
  int GetAsyncReturnToken() {
    if (async_return_token_ == -1) {
      async_return_token_ =
          GetNewTokenForCommand(ControlScope::CMD_ASYNC_RETURN, nullptr);
    }
  ...
  int GetRethrowToken() {
    if (rethrow_token_ == -1) {
      rethrow_token_ =
          GetNewTokenForCommand(ControlScope::CMD_RETHROW, nullptr);
    }
    return rethrow_token_;
  }

  int GetNewTokenForCommand(ControlScope::Command command,
                            Statement* statement) {
...
void BytecodeGenerator::VisitTryFinallyStatement(TryFinallyStatement* stmt) {
  ...
  DeferredCommands commands(this, token, result);
...
void BytecodeGenerator::BuildFinalizeIteration(
  ...
  try_control_builder.BeginTry(context);
  {
    ...
    // TODO(leszeks): In some cases (e.g. empty destructuring) we know that done
    // is false.
  ...
  deferred_iteration_commands->JumpIfRethrow(&suppress_close_exception);
  builder()
...
Expression* BytecodeGenerator::GetDestructuringDefaultValue(
  ...
  if ((*target)->IsAssignment()) {
    ...
    *target = default_init->target();
    DCHECK((*target)->IsValidReferenceExpression() ||
           (*target)->IsObjectLiteral() || (*target)->IsArrayLiteral());
  }
...
void BytecodeGenerator::BuildDestructuringArrayAssignment(
  ...
  try_control_builder.BeginTry(context);
  {
    ...
    if (spread) {
      ...
      // Reaching here means that BuildFillArrayWithIterator was exited because
      // of next_result.done being true, so set 'done' to true.
      builder()
          ->LoadTrue()
          .StoreAccumulatorInRegister(done)
  ...
  BuildFinalizeIteration(iterator, done, &commands);
