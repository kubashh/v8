...
BytecodeArrayBuilder& BytecodeArrayBuilder::JumpIfTrue(
  ...
  DCHECK(!label->is_bound());
  if (branch_slot.IsInvalid()) {
    if (mode == ToBooleanMode::kAlreadyBoolean) {
      OutputJumpIfTrueNoFeedback(label, 0);
    } else {
      DCHECK_EQ(mode, ToBooleanMode::kConvertToBoolean);
      OutputJumpIfToBooleanTrueNoFeedback(label, 0);
    }
  } else {
    if (mode == ToBooleanMode::kAlreadyBoolean) {
...
BytecodeArrayBuilder& BytecodeArrayBuilder::JumpIfFalse(
  ...
  if (branch_slot.IsInvalid()) {
    if (mode == ToBooleanMode::kAlreadyBoolean) {
      OutputJumpIfFalseNoFeedback(label, 0);
    } else {
      DCHECK_EQ(mode, ToBooleanMode::kConvertToBoolean);
      OutputJumpIfToBooleanFalseNoFeedback(label, 0);
    }
  } else {
    if (mode == ToBooleanMode::kAlreadyBoolean) {
