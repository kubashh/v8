...
TNode<Object> InterpreterAssembler::ConstructForwardArgs(
    TNode<Object> target, TNode<Context> context, TNode<Object> new_target,
    TNode<UintPtrT> slot_id) {
  DCHECK(Bytecodes::MakesCallAlongCriticalPath(bytecode_));
  TVARIABLE(Object, var_result);
  ...
  Callable callable =
      CodeFactory::ForwardStandardFrameArgsThenConstruct(isolate());
  return CallStub(callable, context, GetInterpretedFramePointer(), target,
                  new_target);
}
