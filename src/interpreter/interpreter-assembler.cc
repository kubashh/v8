...
TNode<IntPtrT> InterpreterAssembler::GetInterpretedFramePointer() {
  if (!interpreted_frame_pointer_.IsBound()) {
    interpreted_frame_pointer_ =
        UncheckedCast<IntPtrT>(LoadParentFramePointer());
...
void InterpreterAssembler::SaveBytecodeOffset() {
  TNode<IntPtrT> value_offset = BytecodeOffset();
  if (operand_scale() != OperandScale::kSingle) {
    // Subtract one from the value_offset such that it points to the Wide /
    // ExtraWide prefix bytecode.
    value_offset = IntPtrSub(BytecodeOffset(), IntPtrConstant(1));
  }
  // Note that 'offset' is the offset in which to store 'value_offset'
  int offset = Register::bytecode_offset().ToOperand() * kSystemPointerSize;
  TNode<IntPtrT> base = GetInterpretedFramePointer();
  ...
  if (SmiValuesAre32Bits()) {
    ...
    StoreNoWriteBarrier(MachineRepresentation::kWord32, base,
                        ...
                        TruncateIntPtrToInt32(value_offset));
  } else {
    StoreNoWriteBarrier(MachineRepresentation::kTaggedSigned, base,
                        IntPtrConstant(offset), SmiTag(value_offset));
  }
}
