...
void InterpreterAssembler::CollectCallableFeedback(Node* target, Node* context,
  ...
  TNode<Object> feedback_value = RemoveWeakBit(feedback_element);
  ...
  BIND(&extra_checks);
  {
    ...
    CSA_ASSERT(this, IsWeakOrClearedHeapObject(feedback_element));

    // If the weak reference is cleared, we have a new chance to become
    // monomorphic.
    Comment("check if weak reference is cleared");
    Node* is_cleared = IsClearedWeakHeapObject(feedback_element);
    Branch(is_cleared, &initialize, &mark_megamorphic);
...
Node* InterpreterAssembler::Construct(Node* target, Node* context,
  ...
  BIND(&extra_checks);
  {
    ...
    Comment("check if weak reference");
    GotoIfNot(IsWeakOrClearedHeapObject(feedback_element),
              &check_allocation_site);
    ...
    BIND(&check_allocation_site);
    {
      ...
      TNode<HeapObject> strong_feedback_element =
          ToStrongHeapObject(feedback_element);
      GotoIfNot(IsAllocationSiteMap(LoadMap(strong_feedback_element)),
                &check_initialized);
      ...
      var_site.Bind(strong_feedback_element);
...
Node* InterpreterAssembler::ConstructWithSpread(Node* target, Node* context,
  ...
  BIND(&extra_checks);
  {
    ...
    Comment("check if weak reference");
    Node* is_weak_reference = IsWeakOrClearedHeapObject(feedback_element);
    GotoIfNot(is_weak_reference, &check_initialized);
    ...
    BIND(&initialize);
    {
      ...
      BIND(&done_loop);
      StoreWeakReferenceInFeedbackVector(feedback_vector, slot_id, new_target);
      ReportFeedbackUpdate(feedback_vector, slot_id,
