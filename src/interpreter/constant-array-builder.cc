...
namespace {
bool IsJavaScriptValue(Handle<Object> object) {
  return object->IsNumber() || object->IsName() || object->IsOddball() ||
         object->IsBigInt() || object->IsJSReceiver();
}

struct ObjectHandleHash {
  size_t operator()(Handle<Object> object) const {
    if (!IsJavaScriptValue(object)) return 0;
    if (object->IsJSReceiver()) {
      Handle<JSReceiver> receiver = Handle<JSReceiver>::cast(object);
      return receiver->GetOrCreateHash(receiver->GetIsolate())->value();
    }
    return Smi::cast(object->GetHash())->value();
  }
};

struct ObjectHandleSameValue {
  bool operator()(Handle<Object> a, Handle<Object> b) {
    if (!IsJavaScriptValue(a) || !IsJavaScriptValue(b)) return *a == *b;
...
void ConstantArrayBuilder::ConstantArraySlice::CheckAllElementsAreUnique(
  ...
  std::unordered_set<Handle<Object>, debug_check::ObjectHandleHash,
                     debug_check::ObjectHandleSameValue>
      objects;
  for (const Entry& entry : constants_) {
    ...
    Handle<Object> object = entry.ToHandle(isolate);
