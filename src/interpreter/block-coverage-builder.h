...
class BoundBlockCoverageBuilder final {
 public:
  BoundBlockCoverageBuilder(AstNode* node,
                            BlockCoverageBuilder* block_coverage_builder)
      : node_(node), block_coverage_builder_(block_coverage_builder) {
    DCHECK_NOT_NULL(node);
  }

  int AllocateBlockCoverageSlot(SourceRangeKind kind) {
    return (block_coverage_builder_ == nullptr)
               ? BlockCoverageBuilder::kNoCoverageArraySlot
               : block_coverage_builder_->AllocateBlockCoverageSlot(node_,
                                                                    kind);
  }
