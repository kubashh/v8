...
// kVariable means that the variable is definitely in the register.
// kMaybeVariable means that the variable might be in the register.
enum class VariableHintMode { kVariable, kMaybeVariable };
 ...
 public:
  RegisterInfo(Register reg, uint32_t equivalence_id, bool materialized,
      ...
      : register_(reg),
        ...
        variable_hint_({nullptr, VariableHintMode::kVariable}),
        next_(this),
        prev_(this) {}
...
void BytecodeRegisterOptimizer::SetVariableInRegister(Variable* var,
  ...
  do {
    ...
    it->set_variable_hint({var, VariableHintMode::kVariable});
    it = it->next();
  } while (it != info);
}
...
Variable* BytecodeRegisterOptimizer::GetVariableInRegister(Register reg) {
  ...
  return hint.mode == VariableHintMode::kVariable ? hint.variable : nullptr;
}
...
bool BytecodeRegisterOptimizer::IsVariableInRegister(Variable* var,
  ...
  RegisterInfo* info = GetRegisterInfo(reg);
  VariableHint hint = info->variable_hint();
  return hint.mode == VariableHintMode::kVariable && hint.variable == var;
}
