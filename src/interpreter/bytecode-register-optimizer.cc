...
void BytecodeRegisterOptimizer::RegisterInfo::MoveToNewEquivalenceSet(
    uint32_t equivalence_id, bool materialized, bool reset_variable_hint) {
  next_->prev_ = prev_;
  prev_->next_ = next_;
  next_ = prev_ = this;
  equivalence_id_ = equivalence_id;
  materialized_ = materialized;
  flush_variable_hint(reset_variable_hint);
  type_hint_ = TypeHint::kAny;
}
...
Variable* BytecodeRegisterOptimizer::GetMightHaveVariableInRegister(
    Register reg) {
  RegisterInfo* info = GetRegisterInfo(reg);
  ...
  for (RegisterInfo* reg_info : registers_needing_flushed_) {
    ...
    if (materialized != nullptr) {
      ...
      while ((equivalent = materialized->GetEquivalent()) != materialized) {
        ...
        equivalent->MoveToNewEquivalenceSet(NextEquivalenceId(), true, false);
      ...
      reg_info->MoveToNewEquivalenceSet(NextEquivalenceId(), false, false);
    }
  }
...
void BytecodeRegisterOptimizer::PrepareOutputRegister(Register reg) {
  ...
  reg_info->MoveToNewEquivalenceSet(NextEquivalenceId(), true, true);
  max_register_index_ =
...
void BytecodeRegisterOptimizer::AllocateRegister(RegisterInfo* info) {
  ...
  if (!info->materialized()) {
    info->MoveToNewEquivalenceSet(NextEquivalenceId(), true, true);
  }
}
