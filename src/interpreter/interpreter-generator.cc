...
class InterpreterStoreNamedPropertyAssembler : public InterpreterAssembler {
 public:
  ...
  void StaNamedProperty(Callable ic) {
    ...
    Node* feedback_vector = LoadFeedbackVector();
    Node* context = GetContext();
    Label call_runtime(this), call_side_effect_check(this);
    Branch(NeedsSideEffectCheck(), &call_side_effect_check, &call_runtime);
    BIND(&call_runtime);
    {
    ...
    {
      CallRuntime(Runtime::kDebugSideEffectCheck, context, object);
      Goto(&call_runtime);
    }
  }
};
...
IGNITION_HANDLER(CreateArrayLiteral, InterpreterAssembler) {
  ...
  {
    CallRuntime(Runtime::kDebugMarkAsSideEffectFree, context,
                var_result.value());
    Dispatch();
  }
}
...
IGNITION_HANDLER(CreateEmptyArrayLiteral, InterpreterAssembler) {
  ...
  BIND(&call_debug_receiver_created);
  {
    CallRuntime(Runtime::kDebugMarkAsSideEffectFree, context, result);
    Dispatch();
  }
}
