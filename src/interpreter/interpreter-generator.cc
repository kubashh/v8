...
class UnaryNumberOpAssembler : public InterpreterAssembler {
 public:
  UnaryNumberOpAssembler(CodeAssemblerState* state, Bytecode bytecode,
                         OperandScale operand_scale)
      : InterpreterAssembler(state, bytecode, operand_scale) {}
  ...
  void UnaryOpWithFeedback(
    ...
    Label start(this, 2, loop_vars), end(this);
    ...
    BIND(&start);
    {
      ...
      BIND(&if_bigint);
      {
        var_result.Bind(bigint_op(value, context));
  ...
  void IncDecWithFeedback(Token::Value op) {
    UnaryOpWithFeedback(
        ...
        [=](Node* bigint_value, Node* context) {
...
IGNITION_HANDLER(Negate, UnaryNumberOpAssembler) {
  UnaryOpWithFeedback(
...
IGNITION_HANDLER(Inc, UnaryNumberOpAssembler) {
...
IGNITION_HANDLER(Dec, UnaryNumberOpAssembler) {
