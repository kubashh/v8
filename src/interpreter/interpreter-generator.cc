...
IGNITION_HANDLER(JumpLoop, InterpreterAssembler) {
  ...
  TNode<Word32T> actual =
      Word32And(Word32Shl(Int32Sub(TruncateIntPtrToInt32(BytecodeOffset()),
                                   kFirstBytecodeOffset),
                          kShift),
                kMask);
  // Note: 0 is treated as 1, see Code::OsrInstallTargetFor.
  actual = Select<Word32T>(
      Word32Equal(actual, 0), [&]() { return Int32Constant(1 << kShift); },
      [&]() { return actual; });
