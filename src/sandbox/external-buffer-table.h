...
struct ExternalBufferTableEntry {
  ...
  // Returns the old external buffer. This entry must be an external buffer
  ...
  inline void UnmarkAndMigrateInto(ExternalBufferTableEntry& other);
 ...
 private:
  ...
  struct Payload {
    Payload(std::pair<Address, size_t> buffer, ExternalPointerTag tag)
        : encoded_word_(Tag(buffer.first, tag)), raw_size_(buffer.second) {}
    Payload(Address value, ExternalPointerTag tag)
        : encoded_word_(Tag(value, tag)), raw_size_(0) {}

    std::pair<Address, size_t> Untag(ExternalPointerTag tag) const {
      return std::make_pair(encoded_word_ & ~tag, raw_size_);
    }
    ...
    Address ExtractEvacuationEntryHandleLocation() const {
      return Untag(kExternalPointerEvacuationEntryTag).first;
    }
...
class V8_EXPORT_PRIVATE ExternalBufferTable
 ...
 public:
  ...
  using CompactibleSpace = CompactibleExternalEntityTable<
      ExternalBufferTableEntry, kExternalBufferTableReservationSize>::Space;
