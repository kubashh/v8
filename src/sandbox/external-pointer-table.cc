...
uint32_t ExternalPointerTable::SweepAndCompact(Isolate* isolate) {
  ...
  for (uint32_t i = table_end - 1; i > 0; i--) {
    ...
    if (payload.ContainsEvacuationEntry()) {
      ...
      // TODO: ExtraceHandleLocation instead?
      ExternalPointerHandle* handle_location =
          reinterpret_cast<ExternalPointerHandle*>(
              payload.Untag(kExternalPointerEvacuationEntryTag));
...
void ExternalPointerTable::ResolveEvacuationEntryDuringSweeping(
  ...
  auto payload = at(old_index).GetRawPayload();
  DCHECK(payload.ContainsExternalPointer());
  payload.ClearMarkBit();
  at(new_index).SetRawPayload(payload);
  *handle_location = new_handle;
  ...
  const  kClobberedEntry = Entry::Decode(kClobberedEntryMarker);
  DCHECK_NE(payload, kClobberedEntry);
  at(old_index).SetRawPayload(kClobberedEntry
  */
#endif  // DEBUG
}
