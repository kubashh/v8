 ...
 *  - A regular entry, containing the external pointer together with a type tag
 * and the marking bit in the unused upper bits, or
 *  - A freelist entry, tagged with the kExternalPointerFreeEntryTag and
 * containing the index of the next entry on the freelist in the lower 32 bits,
 * or
 *  - An evacuation entry, tagged with the kExternalPointerEvacuationEntryTag
 * and containing the address of the ExternalPointerSlot that is to be evacuated
 * into this entry. See the compaction algorithm overview for more details about
 * these entries.
 */
struct ExternalPointerTableEntry {
  // Make this entry a external pointer entry containing the given pointer
  ...
  // Make this entry a freelist entry, storing the index of the next entry on
  // the freelist.
  ...
  inline Address GetEvacuationEntryHandleLocation() const;
 ...
 private:
  ...
  inline void SetRawPayload(Payload payload) {
...
class V8_EXPORT_PRIVATE ExternalPointerTable
 ...
 private:
...
#endif  // DEBUG
  uint32_t Sweep(Isolate* isolate, uint32_t table_end);

  uint32_t FinishCompactingDuringSweeping(Isolate* isolate);
  ...
  // This happens when the freelist becomes empty but we still need to allocate
  // evacuation entries.
