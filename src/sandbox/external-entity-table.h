 ...
 *
 * The ExternalEntityTable class is effectively an abstract class and needs to
 * be extended by a concrete implementation class, such as the
 * ExternalPointerTable class.
 *
 * The ExternalEntityTable class implement the basic table memory management:
 * the reservation of the backing storage as well as (in-place) growing and
 * shrinking of the backing memory and entry allocation routines.
 * Free entries are managed through a singly-linked list, which is built up
 * during sweeping and will be sorted top-to-bottom so that entries with a
 * lower index are allocated first, thereby automatically defragmenting the
 * table to some degree.
 */
...
class V8_EXPORT_PRIVATE ExternalEntityTable {
 protected:
  ...
  // return zero. The threshold index must be below the current capacity. This
  // method will therefore never grow the table.
  // This method is atomic and can be called from background threads.
