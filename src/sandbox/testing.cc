...
void SandboxMemoryView(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  Handle<JSDataView> view = factory->NewJSDataView(buffer, 0, size);

  args.GetReturnValue().Set(Utils::ToLocal(view));
}
...
// Sandbox.getSizeInBytesOf(object) -> Number
void SandboxGetSizeInBytesOf(const v8::FunctionCallbackInfo<v8::Value>& args) {
  v8::Isolate* isolate = args.GetIsolate();
...
void MemoryCorruptionApi::Install(Isolate* isolate) {
  ...
  Handle<JSObject> sandbox_object =
      factory->NewJSObject(isolate->object_function(), AllocationType::kOld);

  // Install the various members of the Sandbox object, such as the MemoryView
  // constructor.
  InstallGetter(isolate, sandbox_object, SandboxGetByteLength, "byteLength");
  InstallConstructor(isolate, sandbox_object, SandboxMemoryView, "MemoryView",
                     2);
  InstallFunction(isolate, sandbox_object, SandboxGetAddressOf, "getAddressOf",
                  1);
  InstallFunction(isolate, sandbox_object, SandboxGetSizeInBytesOf,
                  "getSizeInBytesOf", 1);

  // Install the Sandbox object as property on the global object.
  Handle<JSGlobalObject> global = isolate->global_object();
  Handle<String> name = factory->NewStringFromAsciiChecked("Sandbox");
  JSObject::AddProperty(isolate, global, name, sandbox_object, DONT_ENUM);
}
