...
// WARNING: this function should be kept behind V8_EXPOSE_MEMORY_CORRUPTION_API
// so that the code is never available in any shipping build, where it could
// potentially be abused to facilitate exploitation.
...
#ifdef V8_OS_LINUX
struct sigaction g_orig_sigbus_handler, g_orig_sigsegv_handler;
void SandboxSignalHandler(int signal, siginfo_t* info, void* void_context) {
  ...
  sigaction(SIGBUS, &g_orig_sigbus_handler, nullptr);
  sigaction(SIGSEGV, &g_orig_sigsegv_handler, nullptr);
}
#endif  // V8_OS_LINUX
...
void SandboxTesting::InstallSandboxCrashFilter() {
  ...
  bool success = true;
  success &= (sigaction(SIGBUS, &action, &g_orig_sigbus_handler) == 0);
  success &= (sigaction(SIGSEGV, &action, &g_orig_sigsegv_handler) == 0);
  CHECK(success);
#else
