...
#define VALID_INDIRECT_POINTER_TAG_CASE(tag_name, tag_id) case tag_name:
  switch (tag) {
    PER_ISOLATE_INDIRECT_POINTER_TAG_LIST(VALID_INDIRECT_POINTER_TAG_CASE)
    return true;
    default:
      break;
  }
#undef VALID_INDIRECT_POINTER_TAG_CASE
#define VALID_SHARED_TRUSTED_POINTER_TAG_CASE(tag_name, tag_id) \
  || (tag == tag_name)
  if (false SHARED_TRUSTED_POINTER_TAG_LIST(
          VALID_SHARED_TRUSTED_POINTER_TAG_CASE)) {
    return true;
  }
  return false;
}
