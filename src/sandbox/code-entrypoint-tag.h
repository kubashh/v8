...
//
// When the sandbox is enabled, the code pointer table achieves a form of
...
// incompatible signatures. Further, some internal builtins may not expect to be
// called indirectly in this way at all. CodeEntrypointTags are therefore used
// to achieve fine-grained CFI: used appropriately, they guarantee that the
// callee and caller of such control-flow transfers are compatible. As such,
// two code objects should use the same tag iff they can safely be interchanged
// at all (indirect) callsites.
//
