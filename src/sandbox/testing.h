...
//
// If an access violation (i.e. a SIGSEGV or SIGBUS) happens inside the sandbox
// address space it is considered harmless for the sandbox and so the process
// is simply terminated through _exit. On the other hand, an access violation
// outside the sandbox address space indicates a security issue with the
// sandbox. In that case, the signal is forwarded to the original signal
