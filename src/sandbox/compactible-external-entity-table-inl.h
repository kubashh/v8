...
void CompactibleExternalEntityTable<Entry, size>::MaybeCreateEvacuationEntry(
  ...
  if (index >= start_of_evacuation_area) {
    ...
    uint32_t new_index =
        Super::AllocateEntryBelow(space, start_of_evacuation_area);
    if (new_index) {
      ...
      Super::at(new_index).MakeEvacuationEntry(handle_location);
    } else {
...
void CompactibleExternalEntityTable<Entry,
  ...
  uint32_t num_segments_to_evacuate =
      (num_free_entries / 2) / Super::kEntriesPerSegment;
  uint32_t space_size = num_total_entries * Super::kEntrySize;
  ...
  if (v8_flags.stress_compaction) {
    should_compact = num_free_entries > Super::kEntriesPerSegment;
  }
