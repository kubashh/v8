...
uint32_t ExternalEntityTable<Entry>::AllocateEntryBelow(
  ...
  DCHECK(is_initialized());
  DCHECK_LT(threshold_index, capacity());
...
bool ExternalEntityTable<Entry>::TryAllocateEntryFromFreelist(
  ...
  Entry& entry_to_allocate = at(freelist.next());
  uint32_t next_free_entry = entry_to_allocate.GetNextFreelistEntryIndex();
  FreelistHead new_freelist(next_free_entry, freelist.length() - 1);
...
template <typename Entry>
ExternalEntityTable<Entry>::FreelistHead ExternalEntityTable<Entry>::Grow(
    Isolate* isolate) {
  // Freelist should be empty when calling this method.
  DCHECK(freelist_head_.load(std::memory_order_relaxed).is_empty());
  ...
  if (!root_space->SetPagePermissions(
          buffer_start + old_capacity * sizeof(Address), kBlockSize,
  ...
  // Build the freelist bottom to top. This way, the table becomes
  // self-compacting to some degree.
...
void ExternalEntityTable<Entry>::Shrink(uint32_t new_capacity) {
  uint32_t old_capacity = capacity();
  DCHECK(IsAligned(kBlockSize, new_capacity));
