...
class ExecArgs {
 public:
  ...
  bool Init(Isolate* isolate, Local<Value> arg0, Local<Array> command_info) {
    ...
    int i = 1;
    for (unsigned j = 0; j < command_info->Length(); i++, j++) {
      Local<Value> arg(
          command_info
...
void Shell::System(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  Local<Array> command_info;
  if (info.Length() > 1) {
    if (!info[1]->IsArray()) {
      info.GetIsolate()->ThrowError("system: Argument 2 must be an array");
      return;
    }
    command_info = info[1].As<Array>();
  } else {
    command_info = Array::New(info.GetIsolate(), 0);
  }
  if (command_info->Length() > ExecArgs::kMaxArgs) {
  ...
  ExecArgs exec_args;
  if (!exec_args.Init(info.GetIsolate(), info[0], command_info)) {
    return;
  }
