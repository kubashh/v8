...
class TerminateTask : public i::CancelableTask {
 public:
  ...
  void RunInternal() override { worker_->TerminateInternal(); }
...
void Worker::TerminateInternal() {
  auto expected = Worker::State::kTerminating;
  CHECK(state_.compare_exchange_strong(expected, Worker::State::kTerminated));
}
