...
MaybeLocal<Context> Shell::CreateEvaluationContext(Isolate* isolate) {
  ...
  if (options.include_arguments) {
    const std::vector<const char*>& info = options.arguments;
    int size = static_cast<int>(info.size());
    Local<Array> array = Array::New(isolate, size);
    for (int i = 0; i < size; i++) {
      Local<String> arg =
          v8::String::NewFromUtf8(isolate, info[i]).ToLocalChecked();
 ...
 private:
  ...
  void Send(const v8_inspector::StringView& string) {
    ...
    if (callback->IsFunction()) {
      ...
      Local<Value> info[] = {message};
      USE(callback.As<Function>()->Call(context, Undefined(isolate_), 1, info));
#ifdef DEBUG
      if (try_catch.HasCaught()) {
...
int Shell::Main(int argc, char* argv[]) {
  ...
  if (options.trace_enabled && !i::v8_flags.verify_predictable) {
    ...
    init_info.backends = perfetto::BackendType::kInProcessBackend;
    perfetto::Tracing::Initialize(init_info);
