...
void Shell::RealmTakeWebSnapshot(
  ...
  Local<FunctionTemplate> snapshot_ctor = data->GetSnapshotObjectCtor();
  Local<ObjectTemplate> snapshot_template = snapshot_ctor->InstanceTemplate();
...
Local<FunctionTemplate> Shell::CreateSnapshotTemplate(Isolate* isolate) {
  Local<FunctionTemplate> snapshot_obj_ctor = FunctionTemplate::New(isolate);
  snapshot_obj_ctor->InstanceTemplate()->SetInternalFieldCount(1);
  PerIsolateData::Get(isolate)->SetSnapshotObjectCtor(snapshot_obj_ctor);
  return snapshot_obj_ctor;
}
