...
int Shell::Main(int argc, char* argv[]) {
  ...
  D8Console console(isolate);
  {
    ...
    Local<String> profile_name =
        String::NewFromUtf8(isolate, "d8profile", NewStringType::kNormal)
            .FromMaybe(Local<String>());
    if (options.cpu_profiler) {
      cpu_profiler = CpuProfiler::New(isolate);
      CpuProfilingOptions profile_options;
      cpu_profiler->StartProfiling(profile_name, profile_options);
    }
    ...
    if (options.cpu_profiler) {
      CpuProfile* profile = cpu_profiler->StopProfiling(profile_name);
