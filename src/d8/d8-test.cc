...
class FastCApiObject {
 public:
  ...
  static AnyCType EnforceRangeCompareI32Patch(AnyCType receiver,
                                              ...
                                              AnyCType options) {
    AnyCType ret;
    ret.double_value = EnforceRangeCompare<int32_t>(
        receiver.object_value, should_fallback.bool_value,
        real_arg.double_value, checked_arg.int32_value, *options.options_value);
    return ret;
  }
  static AnyCType EnforceRangeCompareI64Patch(AnyCType receiver,
                                              ...
                                              AnyCType options) {
    AnyCType ret;
    ret.double_value = EnforceRangeCompare<int64_t>(
        receiver.object_value, should_fallback.bool_value,
        real_arg.double_value, checked_arg.int64_value, *options.options_value);
    return ret;
  }
#endif  //  V8_USE_SIMULATOR_WITH_GENERIC_C_CALLS

  template <typename IntegerT>
  static bool EnforceRangeCompare(Local<Object> receiver, bool should_fallback,
    ...
    int64_t real_int = static_cast<int64_t>(real_arg);
    // Number is in range.
    if (real_int <=
            static_cast<int64_t>(std::numeric_limits<IntegerT>().max()) &&
        real_int >=
            static_cast<int64_t>(std::numeric_limits<IntegerT>().min())) {
      return checked_arg == static_cast<IntegerT>(real_int);
    }
    CHECK(false && "Number range should have been enforced");
    UNREACHABLE();
  }
  ...
  static void EnforceRangeCompareSlowCallback(
    ...
    int64_t real_int = 0;
    IntegerT checked_arg = 42;
    if (args.Length() > 1 && args[1]->IsNumber()) {
      double real_arg =
          args[1]->NumberValue(isolate->GetCurrentContext()).FromJust();
      real_int = static_cast<int64_t>(real_arg);
    }
    if (args.Length() > 2 && args[2]->IsNumber()) {
      checked_arg =
    ...
    bool in_range =
        (real_int <=
         ...
         real_int >=
             static_cast<int64_t>(std::numeric_limits<IntegerT>().min()));
    if (in_range) {
      CHECK_EQ(static_cast<IntegerT>(real_int), checked_arg);
      args.GetReturnValue().Set(Boolean::New(isolate, in_range));
    } else {
    ...
    CFunction enforce_range_compare_i32_c_func =
        CFunctionBuilder()
            .Fn(FastCApiObject::EnforceRangeCompare<int32_t>
                    V8_IF_USE_SIMULATOR(
                        FastCApiObject::EnforceRangeCompareI32Patch))
    ...
    CFunction enforce_range_compare_i64_c_func =
        CFunctionBuilder()
            .Fn(FastCApiObject::EnforceRangeCompare<int64_t>
                    V8_IF_USE_SIMULATOR(
                        FastCApiObject::EnforceRangeCompareI64Patch))
            .Arg<3, v8::CTypeInfo::Flags::kEnforceRangeBit>()
            .Build();
    api_obj_ctor->PrototypeTemplate()->Set(
