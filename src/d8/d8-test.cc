...
class FastCApiObject {
 public:
  ...
  typedef double Type;
  static constexpr uint8_t type_info =
      static_cast<uint8_t>(CTypeInfo::Type::kFloat64);
#else
  typedef int32_t Type;
  static constexpr uint8_t type_info =
      static_cast<uint8_t>(CTypeInfo::Type::kInt32);
  ...
  static Type AddAllSequenceFastCallback(Local<Object> receiver,
    ...
    if (seq_arg->Length() < 0 || seq_arg->Length() > 1024) {
      options.fallback = 1;
      return 0;
    }
    ...
    Type buffer[1024];
    bool result =
        seq_arg->CopyAndConvertArrayToCppBuffer<Type, type_info>(buffer, 1024);
    if (!result) {
      options.fallback = 1;
      return 0;
    }
    ...
    Type sum = 0;
    for (uint32_t i = 0; i < seq_arg->Length(); ++i) {
      sum += buffer[i];
    }
