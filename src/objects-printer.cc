...
void Map::MapPrint(std::ostream& os) {  // NOLINT
  ...
  TransitionsAccessor transitions(this);
...
void TransitionsAccessor::PrintTransitionTree(std::ostream& os, int level) {
  ...
  for (int i = 0; i < num_transitions; i++) {
    ...
    TransitionsAccessor transitions(target);
    transitions.PrintTransitionTree(os, level + 1);
  }
}

void JSObject::PrintTransitions(std::ostream& os) {  // NOLINT
  TransitionsAccessor ta(map());
