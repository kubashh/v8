...
Handle<Object> ScopeIterator::GetFunctionName() const {
  if (HasNestedScopeChain()) {
    return JSFunction::GetDebugName(function_);
  }
  if (!context_->scope_info()->HasFunctionName()) {
    return isolate_->factory()->undefined_value();
  }
  // TODO(verwaest): Make sure we put the name into the scope-info.
  return handle(context_->scope_info()->FunctionName());
}
...
MUST_USE_RESULT MaybeHandle<JSObject> ScopeIterator::MaterializeScopeDetails() {
  ...
  if (Type() != ScopeTypeGlobal && Type() != ScopeTypeScript && HasContext()) {
    Handle<Object> closure_name = GetFunctionName();
...
int ScopeIterator::start_position() {
  ...
  // TODO(verwaest): Fetch from scope_info.
  return 0;
}
...
void ScopeIterator::GetNestedScopeChain(Isolate* isolate, Scope* scope,
  ...
  if (scope->is_function_scope()) {
    ...
    if (scope->start_position() > function_->shared()->start_position() &&
        scope->end_position() <= function_->shared()->end_position()) {
      return;
    }
  }
  if (scope->is_hidden()) {
