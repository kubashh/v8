...
bool ScopeIterator::NeedsAndHasContext() const {
  ...
  // for the case where the function is executing in its parent context. Note
  // that modules do not begin by pushing their own context; for a module scope,
  // function_->context() refers to the contents of the module itself rather
  // than to the scope in which the module is defined.
  if (current_scope_ == closure_scope_ && current_scope_->is_function_scope() &&
      !function_.is_null() && function_->context() == *context_) {
    return false;
  }
  return true;
}
