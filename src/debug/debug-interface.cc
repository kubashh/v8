...
bool Script::GetPossibleBreakpoints(
  ...
  if (!GetSourceOffset(start).To(&start_offset)) {
    return false;
  }
  if (end.IsEmpty()) {
    end_offset = std::numeric_limits<int>::max();
  } else if (!GetSourceOffset(end).To(&end_offset)) {
    return false;
  }
  ...
  std::sort(v8_locations.begin(), v8_locations.end(),
            [](const i::BreakLocation& loc1, const i::BreakLocation& loc2) {
...
Maybe<int> Script::GetSourceOffset(const Location& location) const {
  ...
  if (line < 0 || line >= line_ends->length() || column < 0) {
    return Nothing<int>();
  }
  if (line > 0) {
    column += GetSmiValue(line_ends, line - 1) + 1;
  }
  return column <= GetSmiValue(line_ends, line) ? Just(column) : Nothing<int>();
}
