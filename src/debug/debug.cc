...
class Debug::TemporaryObjectsTracker : public HeapObjectAllocationTracker {
 ...
 private:
  ...
  // multiple). If {include_touching} is true, it will also consider regions
  // that have no overlap but are directly connected.
  std::map<Address, Address>::iterator FindOverlappingRegion(
      Address start, Address end, bool include_touching) {
    // Region A = [start, end) overlaps with an existing region X = [xstart,
    // xend) iff (start <= xend) && (xstart <= end). Since we index {regions_}
    // by end address, we can find a candidate that satisfies the first
    // condition using lower_bound.
    if (include_touching) {
  ...
  void AddRegion(Address start, Address end) {
    ...
    // Region [start, end) can be combined with an existing region if the
    // overlap.
    auto it = FindOverlappingRegion(start, end, true);
    if (it == regions_.end()) {
      // If there is no such region, we can just add the new one and are done.
      regions_.emplace(end, start);
      return;
    }

    // Otherwise, we found an overlapping region. We remove the old one and add
    // the new region recursively (to handle cases here the new region overlaps
    // multiple existing ones).
    Address new_start = std::min(start, it->second);
    Address new_end = std::max(end, it->first);
    regions_.erase(it);
    AddRegion(new_start, new_end);
  }
  ...
  bool RemoveFromRegions(Address start, Address end) {
    ...
    for (; it != regions_.end();
      ...
      Address xstart = it->second;
      Address xend = it->first;
      // If we remove the region [start, end) from the region X = [xstart, xend)
      // at it, there can be at most 2 regions left:
      regions_.erase(it);
      // The one before {start} is: [xstart, start)
      if (xstart < start) AddRegion(xstart, start);
      // And the one after {end} is: [end, xend)
      if (end < xend) AddRegion(end, xend);
    }
    return true;
  }
