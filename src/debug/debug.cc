...
bool Debug::AllFramesOnStackAreBlackboxed(bool include_async,
  ...
  for (StackFrameIterator it(isolate_); !it.done(); it.Advance()) {
    ...
    if (stop_at_caught || include_async) {
      ...
      if (include_async && prediction == Isolate::CAUGHT_BY_ASYNC_AWAIT) {
        ...
        if (IsPromiseOnStack(*promise_stack)) {
          ...
          if (!maybe_promise.is_null()) {
            Handle<JSObject> object_handle = maybe_promise.ToHandleChecked();
            if (IsJSPromise(*object_handle)) {
              ...
              isolate_->WalkPromiseTree(
                  ...
                  [this, &is_caught,
                   &is_blackboxed](Isolate::PromiseHandler handler) {
                    ...
                    if (!ReceiverIsBlackboxed(isolate_, handler.receiver)) {
                      is_blackboxed = false;
                    }
