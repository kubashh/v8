...
void BreakIterator::SetDebugBreak() {
  ...
  DCHECK(debug_break_type >= DEBUGGER_STATEMENT);
...
void Debug::ThreadInit() {
  ...
  thread_local_.muted_at_location_ = false;
  thread_local_.muted_code_ = Smi::zero();
  thread_local_.muted_offset_ = -1;
}
...
void Debug::Iterate(RootVisitor* v, ThreadLocal* thread_local_data) {
  ...
  v->VisitRootPointer(Root::kDebug, nullptr,
                      FullObjectSlot(&thread_local_data->muted_code_));
}
...
bool Debug::IsMutedAtBreakLocation(Handle<SharedFunctionInfo> function,
                                   const BreakLocation& location) {
  ...
  return thread_local_.muted_at_location_ &&
         *function == thread_local_.muted_code_ &&
         location.position() == thread_local_.muted_offset_;
}
...
void Debug::SetMutedLocation(Handle<SharedFunctionInfo> function,
                             const BreakLocation& location) {
  thread_local_.muted_at_location_ = true;
  thread_local_.muted_code_ = *function;
  thread_local_.muted_offset_ = location.position();
}
...
void Debug::OnException(Handle<Object> exception,
  ...
  // Handle<SharedFunctionInfo> debuggable_function;
  bool uncaught = !isolate_->WalkCallStackAndPromiseTree(
      promise, [this, &all_frames_ignored,  //&debuggable_function,
                &is_debuggable](Isolate::PromiseHandler handler) {
        if (!handler.async) {
          if (!is_debuggable) {
            is_debuggable = true;
            // debuggable_function = handle(handler.function_info, isolate_);
          }
...

  {
    ...
    for (BreakLocation location : break_locations) {
      if (IsMutedAtBreakLocation(shared, location)) {
        return;
      }
    }
  }
  // bool found_debuggable_function = false;
  // for (JavaScriptStackFrameIterator it(isolate_); !it.done() &&
  // !found_debuggable_function; it.Advance()) {
  //   std::vector<Handle<SharedFunctionInfo>> infos;
  //   it.frame()->GetFunctions(&infos);
  //   for (Handle<SharedFunctionInfo> info : infos) {
  //     if (*info == *debuggable_function) {
  //       // Don't use this debug info; use the top info for this frame.
  //       FrameSummary summary = FrameSummary::GetTop(it.frame());
  //       Handle<JSFunction> function = summary.AsJavaScript().function();
  //       Handle<DebugInfo>
  //       debug_info(TryGetDebugInfo(function->shared()).value(), isolate_);
  //         // Enter the debugger.
  //       DebugScope debug_scope(this);
  //       std::vector<BreakLocation> break_locations;
  //       BreakLocation::AllAtCurrentStatement(debug_info, it.frame(),
  //       &break_locations); for (BreakLocation location : break_locations) {
  //         if (IsMutedAtBreakLocation(location)) {
  //           return;
  //         }
  //       }
  //       found_debuggable_function = true;
  //       break;
  //     }
  //   }
  // }
  // DCHECK(found_debuggable_function);
  ...
  MaybeHandle<FixedArray> break_points;
  {
    ...
    if (frame && IsJSFunction(frame->function())) {
      ...
      if (ToHandle(isolate_, TryGetDebugInfo(*shared), &debug_info) &&
        ...
        if (thread_local_.muted_at_location_) {
          for (BreakLocation location : break_locations) {
            if (IsMutedAtBreakLocation(shared, location)) {
