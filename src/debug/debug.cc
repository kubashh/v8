...
void Debug::ThreadInit() {
  ...
  thread_local_.break_on_function_call_ = false;
...
void Debug::Break(JavaScriptFrame* frame, Handle<JSFunction> break_target) {
  ...
  if (!break_points_hit.is_null() || break_on_function_call()) {
...
void Debug::SetBreakOnFunctionCall() {
  // This method forces V8 to break on next function call regardless current
  // last_step_action_. If any break happens between SetBreakOnFunctionCall and
  // ClearBreakOnFunctionCall, we will clear this flag and stepping. If break
  // does not happen, e.g. all called functions are blackboxed or no function
  // is called, then we will clear this flag and let stepping continue its
  // normal business.
  thread_local_.break_on_function_call_ = true;
  UpdateHookOnFunctionCall();
}

void Debug::ClearBreakOnFunctionCall() {
  thread_local_.break_on_function_call_ = false;
  UpdateHookOnFunctionCall();
}

void Debug::PrepareStepIn(Handle<JSFunction> function) {
  CHECK(last_step_action() >= StepIn || break_on_function_call());
...
void Debug::RunPromiseHook(PromiseHookType hook_type, Handle<JSPromise> promise,
  ...
  if (hook_type == PromiseHookType::kBefore) {
    debug_delegate_->AsyncEventOccurred(debug::kDebugWillHandle, id, false);
  } else if (hook_type == PromiseHookType::kAfter) {
    debug_delegate_->AsyncEventOccurred(debug::kDebugDidHandle, id, false);
  } else {
...
void Debug::UpdateHookOnFunctionCall() {
  ...
  hook_on_function_call_ =
      ...
      thread_local_.break_on_function_call_;
}
