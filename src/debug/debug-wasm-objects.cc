...
Handle<String> CreateTypeName(MaybeHandle<WasmModuleObject> module_object,
                              wasm::ValueType type, Isolate* isolate) {
  if (!module_object.is_null()) {
    return GetRefTypeName(isolate, type,
  ...
  switch (value.type().kind()) {
    ...
    case wasm::kRef: {
      ...
      if (ref->IsWasmStruct()) {
        ...
        // instance object. If the function was created
        // in JavaScript using `new WebAssembly.Function(...)`, a module for
        // name resolution is not available.
        if (module_object.is_null() &&
        ...
        t = CreateTypeName(module_object, value.type(), isolate);
