  ...
  if (info->HasBytecodeArray()) {
    ...
    for (interpreter::BytecodeArrayIterator it(bytecode_array); !it.done();
      ...
      if (BytecodeRequiresRuntimeCheck(bytecode)) {
        if (FLAG_trace_side_effect_free_debug_evaluate) {
          PrintF("[debug-evaluate] function requires runtime check for %s.\n",
    ...
    return info->GetIsolate()->debug()->PrepareFunctionForSideEffectChecks(
        info, require_runtime_check);
  } else {
...
// static
void DebugEvaluate::PatchWithSideEffectChecks(
