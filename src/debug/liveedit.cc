...
class CollectDataOnArchivedThreads : public ThreadVisitor {
 public:
  explicit CollectDataOnArchivedThreads(FunctionDataMap* map)
      : data_map_(map) {}
  void VisitThread(Isolate* isolate, ThreadLocalTop* top) override {
    for (JavaScriptFrameIterator it(isolate, top); !it.done(); it.Advance()) {
      ...
      for (auto& sfi : sfis) {
        ...
        if (!FunctionDataEntryForSharedFunctionInfo(data_map_, *sfi, &data)) {
          continue;
        }
...
void FillFunctionData(Isolate* isolate, FunctionDataMap* map, Zone* frames_zone,
  ...
  CollectDataOnArchivedThreads visitor(map);
