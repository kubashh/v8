...
struct SourcePositionEvent {
  enum Type { LITERAL_STARTS, DIFF_STARTS, DIFF_ENDS, LITERAL_ENDS };
  ...
  static bool LessThen(const SourcePositionEvent& a,
    ...
    // If more then one event have the same position:
    // - DIFF_START will go after everything except DIFF_ENDS,
    // - DIFF_ENDS will go before everything except DIFF_START,
    // - LITERAL_START will go before LITERALS_END,
    // - for two similar LITERAL event we compare its positions.
    switch (a.type) {
      case DIFF_STARTS:
        return b.type == DIFF_ENDS;
      case DIFF_ENDS:
        return b.type != DIFF_STARTS;
      default: {}
    }
    switch (b.type) {
      case DIFF_STARTS:
    ...
    if (a.type != b.type) return a.type == LITERAL_STARTS;
    if (a.type == LITERAL_STARTS) {
      return b.literal->end_position() > a.literal->end_position();
    } else {
...
struct FunctionLiteralChange {
  // If any of start/end position is kNoSourcePosition,
  // this literal is considered damaged and will not be mapped and edited at
  // all.
...
// Function which has not changed itself, but any variable in it's
