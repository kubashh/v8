...
MaybeHandle<ScopeInfo> DetermineOuterScopeInfo(Isolate* isolate,
  ...
  while (!scope_info->IsEmpty() && !scope_info->HasContext() &&
         scope_info->HasOuterScopeInfo()) {
    scope_info = handle(scope_info->OuterScopeInfo(), isolate);
  }
  if (scope_info->IsEmpty() || !scope_info->HasContext()) {
    return kNullMaybeHandle;
  }

#ifdef DEBUG
  ScopeInfo other_scope_info = FindOuterScopeInfoFromScriptSfi(isolate, script);
