...
void MapLiterals(const FunctionLiteralChanges& changes,
  ...
  for (const auto& mapping : mappings) {
    if (change_state[mapping.first] == ChangeState::UNCHANGED_CODE) {
...
class FunctionDataMap : public ThreadVisitor {
 public:
  void Set(int script_id, FunctionLiteral* literal, bool should_restart) {
    map_[script_id].emplace(literal->function_literal_id(),
 ...
 private:
  bool Lookup(int script_id, int function_literal_id, FunctionData** data) {
    auto inner_map_it = map_.find(script_id);
    if (inner_map_it == map_.end()) return false;
  ...
  // script_id -> literal_id -> data.
  std::unordered_map<int, std::unordered_map<int, FunctionData>> map_;
};
...
void LiveEdit::PatchScript(Isolate* isolate, Handle<Script> script,
  ...
  FunctionDataMap function_data_map;
  for (const auto& mapping : changed) {
    function_data_map.Set(script->id(), mapping.first, true);
    function_data_map.Set(new_script->id(), mapping.second, false);
  }
  for (const auto& mapping : unchanged_code) {
    function_data_map.Set(script->id(), mapping.first, false);
  }
