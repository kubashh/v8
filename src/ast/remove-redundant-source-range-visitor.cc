...
void RemoveRedundantSourceRangeVisitor::MaybeRemoveLastRange(
  ...
  if (stmts->last()->IsIfStatement()) {
    IfStatement* ifstmt = reinterpret_cast<IfStatement*>(stmts->last());
    IfStatementSourceRanges* ranges =
        reinterpret_cast<IfStatementSourceRanges*>(
            source_range_map_->Find(stmts->last()));
    if (ranges) {
      source_range_map_->Erase(ifstmt->then_statement());
      source_range_map_->Erase(ifstmt->else_statement());
      ranges->DisableContinuation();
    }
  } else if (stmts->last()->IsJump()) {
    AstNodeSourceRanges* ranges = source_range_map_->Find(stmts->last());
    if (ranges != nullptr) {
      source_range_map_->Erase(stmts->last());
    }
  }
}
