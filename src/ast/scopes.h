...
class V8_EXPORT_PRIVATE Scope : public NON_EXPORTED_BASE(ZoneObject) {
 public:
  ...
  bool ForceContextForLanguageMode() const {
    // For function scopes we need not force a context.
    if (scope_type_ == FUNCTION_SCOPE) return false;
    return (outer_scope_ != nullptr &&
            language_mode() > outer_scope_->language_mode());
  }
