...
class Suspend final : public Expression {
 public:
  ...
  enum Flags {
    ...
    kGenerator = 0,
    kAsyncGenerator = 1 << 2,
    kGeneratorTypeMask = kGenerator | kAsyncGenerator,
  ...
  Flags generator_type() const {
    return static_cast<Flags>(FlagsField::decode(bit_field_) &
                              kGeneratorTypeMask);
  }
  bool is_yield() const { return suspend_type() == kYield; }
  bool is_yield_star() const { return suspend_type() == kYieldStar; }
  bool is_await() const { return suspend_type() == kAwait; }

  bool is_async_generator() const {
    return generator_type() == kAsyncGenerator;
  }
  ...
  void set_suspend_type(Flags type) {
    DCHECK_EQ(0, type & ~kSuspendTypeMask);
    bit_field_ = FlagsField::update(bit_field_, type);
  }

  static const char* TypeString(Flags flags);
