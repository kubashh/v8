...
{
  outer_scope_str.setPtr(scope);
  outer_scope_str.setStorage(scope->scope_calls_eval_);
  // Reset in order to record eval calls during this Snapshot's lifetime.
  outer_scope_str.getPtr()->scope_calls_eval_ = false;
}

Scope::Snapshot::~Snapshot() {
  // Restore previous calls_eval bit if needed.
  //if (outer_scope_calls_eval_) {
  if (outer_scope_str.getStorage()) {
    outer_scope_str.getPtr()->scope_calls_eval_ = true;
  }
}
...
void Scope::Snapshot::Reparent(DeclarationScope* new_parent) const {
  DCHECK_EQ(new_parent, outer_scope_str.getPtr()->inner_scope_);
  DCHECK_EQ(new_parent->outer_scope_, outer_scope_str.getPtr());
  ...
  Scope *outer_scope_ = outer_scope_str.getPtr();
