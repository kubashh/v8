  ...
  if (is_class_scope()) {
    ...
    if (class_scope->class_variable() != nullptr) {
      ...
      PrintF("%s%s:\n",
             ...
             class_scope->ShouldSaveClassVariableIndex() ? ", index saved"
                                                         : ", index not saved");
...
bool Scope::MustAllocate(Variable* var) {
  ...
  // The anonymous class variable has a special invisible name '.', it's only
  // possible to reference it through static private methods.
  if (is_class_scope() && var == AsClassScope()->class_variable() &&
      var->raw_name()->IsOneByteEqualTo(".")) {
    // The static private methods are referenced directly, in that case
    // the class variable is marked as used during private name resolution.
    if (var->is_used()) {
      DCHECK(AsClassScope()->has_static_private_methods());
      return true;
    }
    // The static private methods may be referenced through eval.
    if (inner_scope_calls_eval_ &&
        AsClassScope()->has_static_private_methods()) {
      var->set_is_used();
      return true;
    }
    return false;
  ...
  for (VariableProxy* proxy = unresolved.first(); proxy != nullptr;) {
    ...
    if (has_private_names) {
      ...
      if (var != nullptr) {
        var->set_is_used();
        proxy->BindTo(var);
        // If the variable being accsssed is a static private method, we need to
        ...
        if (var->is_static() &&
          ...
          class_variable_->set_is_used();
          class_variable_->ForceContextAllocation();
        }
      }
    }
