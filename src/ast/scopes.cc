...
void DeclarationScope::HoistSloppyBlockFunctions(AstNodeFactory* factory) {
  ...
  // so accumulate declared names and sort them and declare at the end.
  ZoneVector<std::pair<int, const AstRawString*>> names_to_declare(zone());
  ...
  for (ZoneHashMap::Entry* p = map->Start(); p != nullptr; p = map->Next(p)) {
    ...
    for (SloppyBlockFunctionMap::Delegate* delegate = delegates;
      ...
      if (!declaration_queued) {
        ...
        names_to_declare.push_back({delegate->index(), name});
      }
  ...
  // Sort by index so as to declare in source order; because index comes
  // first in the pair and the indices are unique, the default comparator
  // is sufficient.
  std::sort(names_to_declare.begin(), names_to_declare.end());
