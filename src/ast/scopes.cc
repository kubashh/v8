...
Scope* Scope::GetHomeObjectScope() {
  ...
  // "super" in arrow functions binds outside the arrow function.
  // Arrow functions are also never receiver scopes since they close over the
  // receiver.
  DCHECK(!IsArrowFunction(kind));
  // If we find
  // a function which doesn't bind "super" (is not a method etc.), we know
  // "super" here doesn't bind anywhere and we can return nullptr.
