...
Node* CodeStubAssembler::AllocateJSObjectFromMapWithSlackTracking(
    Node* context, Node* map, Node* properties, Node* elements,
    AllocationFlags flags) {
  return AllocateJSObjectFromMap(map, properties, elements, flags, context);
}
...
Node* CodeStubAssembler::AllocateJSObjectFromMap(Node* map, Node* properties,
                                                 ...
                                                 Node* context) {
  CSA_ASSERT(this, IsMap(map));
  CSA_ASSERT(this, Word32Not(IsJSFunctionMap(map)));
  CSA_ASSERT(this, Word32Not(InstanceTypeEqual(LoadMapInstanceType(map),
...
void CodeStubAssembler::InitializeJSObjectFromMap(Node* object, Node* map,
  ...
  // We perform slack tracking if a valid context is passed in.
  if (context == nullptr) {
...
void CodeStubAssembler::InitializeJSObjectBodyWithSlackTracking(
    Node* object, Node* map, Node* instance_size, Node* context) {
  CSA_SLOW_ASSERT(this, IsMap(map));
...
void CodeStubAssembler::InitializeFieldsWithRoot(
  ...
  CSA_SLOW_ASSERT(this, IntPtrLessThanOrEqual(start_offset, end_offset));
