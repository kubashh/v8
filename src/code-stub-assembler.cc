...
TNode<FixedArray> CodeStubAssembler::ExtractToFixedArrayWithCOW(
    ...
    Node* to_fixed_array_map, ElementsKind from_kind, AllocationFlags flags,
    ...
    if (handle_old_space) {
      BIND(&old_space);
      {
        ...
        to_elements = AllocateFixedArray(to_kind, capacity, parameter_mode,
                                         flags, var_to_fixed_array_map.value());
        var_result.Bind(to_elements);
        CopyFixedArrayElements(from_kind, source, to_kind, to_elements, first,
  ...
  BIND(&cow);
  {
    // We shouldn't use COW if |source| is a FixedDoubleArray.
