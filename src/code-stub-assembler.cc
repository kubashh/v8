...
TNode<FixedArray> CodeStubAssembler::ExtractToFixedArrayWithCOW(
    Node* from_fixed_array, Node* first, Node* count, Node* capacity,
  ...
  var_fixed_array_map.Bind(to_fixed_array_map);
  ...
  BIND(&new_space_check);
  {
    ...
    CopyFixedArrayElements(from_kind, from_fixed_array, to_kind, to_elements,
                           first, count, capacity, SKIP_WRITE_BARRIER,
                           parameter_mode);
    Goto(&done);
    ...
    if (handle_old_space) {
      BIND(&old_space);
      {
        ...
        var_result.Bind(to_elements);
        CopyFixedArrayElements(from_kind, from_fixed_array, to_kind,
                               to_elements, first, count, capacity,
                               UPDATE_WRITE_BARRIER, parameter_mode);
        Goto(&done);
      }
    }
  }
  ...
  BIND(&cow);
  {
    if (extract_flags & ExtractFixedArrayFlag::kDontCopyCOW) {
      Branch(WordNotEqual(IntPtrOrSmiConstant(0, parameter_mode), first),
             &new_space_check, [&] {
               var_result.Bind(from_fixed_array);
               Goto(&done);
             });
    } else {
