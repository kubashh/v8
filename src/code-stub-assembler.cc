...
template <>
CodeStubAssembler::TNode<IntPtrT>
CodeStubAssembler::ParameterToWord<CodeStubAssembler::SMI_PARAMETERS>(
    TNode<Smi> value) {
  return SmiUntag(value);
}
template <>
CodeStubAssembler::TNode<IntPtrT>
CodeStubAssembler::ParameterToWord<CodeStubAssembler::INTPTR_PARAMETERS>(
    TNode<IntPtrT> value) {
  return value;
}
...
TNode<Int32T> CodeStubAssembler::LoadHashForJSObject(
  ...
  {
    var_hash.Bind(SmiToWord32(CAST(LoadFixedArrayElement(
...
TNode<FixedArrayBase> CodeStubAssembler::LoadElements(
    SloppyTNode<JSObject> object) {
  return LoadObjectField<FixedArrayBase>(object, JSObject::kElementsOffset);
}

TNode<Object> CodeStubAssembler::LoadJSArrayLength(SloppyTNode<JSArray> array) {
  CSA_ASSERT(this, IsJSArray(array));
  return LoadObjectField<Object>(array, JSArray::kLengthOffset);
}
...
TNode<Number> CodeStubAssembler::ChangeFloat64ToTagged(
  ...
  BIND(&if_valueisint32);
  {
    if (Is64()) {
      ...
      TNode<PairNode<Int32T, BoolT>> pair =
          Int32AddWithOverflow(value32, value32);
...
TNode<Number> CodeStubAssembler::ChangeInt32ToTagged(
  ...
  TVARIABLE(Number, var_result);
  TNode<PairNode<Int32T, BoolT>> pair = Int32AddWithOverflow(value, value);
  TNode<BoolT> overflow = Projection<1>(pair);
  Label if_overflow(this, Label::kDeferred), if_notoverflow(this),
      if_join(this);
  Branch(overflow, &if_overflow, &if_notoverflow);
...
TNode<Number> CodeStubAssembler::ChangeUint32ToTagged(
  ...
  BIND(&if_not_overflow);
  {
    if (Is64()) {
      ...
      TNode<PairNode<Int32T, BoolT>> pair = Int32AddWithOverflow(
...
Node* CodeStubAssembler::NumberToString(Node* context, Node* argument) {
  ...
  BIND(&smi);
  {
    ...
    Node* smi_key = LoadFixedArrayElement<SMI_PARAMETERS>(number_string_cache,
                                                          smi_index, 0);
    GotoIf(WordNotEqual(smi_key, argument), &runtime);

    // Smi match, return value from cache entry.
    IncrementCounter(isolate()->counters()->number_to_string_native(), 1);
    result.Bind(LoadFixedArrayElement<SMI_PARAMETERS>(number_string_cache,
                                                      smi_index, kPointerSize));
    Goto(&done);
  }
...
TNode<Number> CodeStubAssembler::ToInteger(SloppyTNode<Context> context,
  ...
  BIND(&out);
  CSA_SLOW_ASSERT(this, IsNumber(var_arg));
  return CAST(var_arg);
}
...
Node* CodeStubAssembler::EmitKeyedSloppyArguments(Node* receiver, Node* key,
  ...
  Node* mapped_index =
      LoadFixedArrayElement(elements, Signed(IntPtrAdd(key, intptr_two)));
