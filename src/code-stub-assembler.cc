...
Node* ExtractJSArray(Node* array, Node* first_element, Node* count,
                     Node* capacity, Node* allocation_site) {
  return nullptr;
}
...
Node* CodeStubAssembler::CopyFixedArray(Node* fixed_array, Node* first,
                                        Node* count, Node* copy_capacity,
                                        CopyFixedArrayFlags copy_flags,
                                        ParameterMode parameter_mode) {
  VARIABLE(result, MachineRepresentation::kTagged);
  ...
  if (copy_capacity == nullptr) {
    copy_capacity = count;
  } else {
    CSA_ASSERT(this, Word32Not(IntPtrOrSmiGreaterThan(
                         IntPtrOrSmiAdd(first, count, parameter_mode),
                         copy_capacity, parameter_mode)));
  }
  ...
  if (copy_flags & CopyFixedArrayFlag::kCopyFixedDoubleArrays) {
    if (copy_flags & CopyFixedArrayFlag::kCopyFixedArrays) {
      GotoIf(IsFixedDoubleArrayMap(fixed_array_map), &if_fixed_double_array);
    } else {
      CSA_ASSERT(this, IsFixedDoubleArrayMap(fixed_array_map));
    }
  } else {
    DCHECK(copy_flags & CopyFixedArrayFlag::kCopyFixedArrays);
    CSA_ASSERT(this, Word32BinaryNot(IsFixedDoubleArrayMap(fixed_array_map)));
  }

  if (copy_flags & CopyFixedArrayFlag::kCopyFixedArrays) {
    Label new_space_check(this), old_space(this);

    Branch(
        WordEqual(fixed_array_map, LoadRoot(Heap::kFixedCOWArrayMapRootIndex)),
        &cow, &new_space_check);

    BIND(&new_space_check);
    GotoIfFixedArraySizeDoesntFitInNewSpace(copy_capacity, &old_space,
    ...
    Node* to_elements =
        AllocateFixedArray(kind, copy_capacity, parameter_mode,
    ...
    CopyFixedArrayElements(kind, fixed_array, kind, to_elements, first, count,
                           copy_capacity, SKIP_WRITE_BARRIER, parameter_mode);
    Goto(&done);
    ...
    BIND(&old_space);
    {
      ...
      to_elements = AllocateFixedArray(kind, copy_capacity, parameter_mode,
                                       flags, fixed_array_map);
      result.Bind(to_elements);
      CopyFixedArrayElements(kind, fixed_array, kind, to_elements, first, count,
                             copy_capacity, UPDATE_WRITE_BARRIER,
                             parameter_mode);
      Goto(&done);
    }
  ...
  if (copy_flags & CopyFixedArrayFlag::kCopyFixedDoubleArrays) {
    ...
    Node* to_elements = AllocateFixedArray(kind, copy_capacity, parameter_mode,
                                           flags, fixed_array_map);
    result.Bind(to_elements);
    CopyFixedArrayElements(kind, fixed_array, kind, to_elements, first, count,
                           copy_capacity, SKIP_WRITE_BARRIER, parameter_mode);
...
void CodeStubAssembler::CopyFixedArrayElements(
  ...
  Node* to_array_adjusted = element_offset_matches
                                ? IntPtrSub(to_array, first_from_element_offset)
                                : to_array;
