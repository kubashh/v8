...
void CodeStubAssembler::GenerateEqual_Same(Node* value, Label* if_equal,
  ...
  if (var_type_feedback != nullptr) {
    ...
    BIND(&if_string);
    {
      ...
      var_type_feedback->Bind(CollectFeedbackForString(instance_type));
      Goto(if_equal);
    }
    ...
    BIND(&if_bigint);
    {
      ...
      OverwriteFeedback(var_type_feedback, CompareOperationFeedback::kBigInt);
      Goto(if_equal);
    }
  ...
  BIND(&if_heapnumber);
  {
    OverwriteFeedback(var_type_feedback, CompareOperationFeedback::kNumber);
