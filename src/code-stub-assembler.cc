...
void CodeStubAssembler::GotoIfHeapNumberToSmi(TNode<HeapNumber> number,
                                              TVariable<Smi>* output,
                                              Label* if_smi) {
  TNode<Float64T> value = LoadHeapNumberValue(number);
  GotoIfFloat64ToSmi(value, output, if_smi);
}

void CodeStubAssembler::GotoIfFloat64ToSmi(TNode<Float64T> value,
                                           TVariable<Smi>* output,
                                           Label* if_smi) {
  ...
  {
    if (SmiValuesAre32Bits()) {
      *output = SmiTag(ChangeInt32ToIntPtr(value32));
    } else {
      DCHECK(SmiValuesAre31Bits());
...
TNode<Number> CodeStubAssembler::ChangeFloat64ToTagged(
  ...
  TVARIABLE(Number, var_result);
  GotoIfFloat64ToSmi(value, &var_smi_result, &if_smi);
...
TNode<String> CodeStubAssembler::NumberToString(TNode<Number> input) {
  ...
  BIND(&if_heap_number);
  {
    ...
    GotoIfHeapNumberToSmi(heap_number_input, &smi_input, &if_smi);
