...
TNode<Map> CodeStubAssembler::LoadMap(SloppyTNode<HeapObject> object) {
  return CAST(LoadObjectField(object, HeapObject::kMapOffset));
}
...
TNode<Float64T> CodeStubAssembler::ChangeNumberToFloat64(
  ...
  result = UncheckedCast<Float64T>(LoadObjectField(
      CAST(value), HeapNumber::kValueOffset, MachineType::Float64()));
  Goto(&done);
