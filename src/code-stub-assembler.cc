...
void CodeStubAssembler::StoreFixedTypedArrayElementFromTagged(
    ...
    TNode<RawPtrT> data_pointer, TNode<Object> index_node, TNode<Object> value,
    ElementsKind elements_kind, ParameterMode parameter_mode) {
  switch (elements_kind) {
    case UINT8_ELEMENTS:
    case UINT8_CLAMPED_ELEMENTS:
    case INT8_ELEMENTS:
...
Node* CodeStubAssembler::LoadFixedDoubleArrayElement(
  ...
  Node* offset = ElementOffsetFromIndex(index_node, HOLEY_DOUBLE_ELEMENTS,
                                        parameter_mode, header_size);
  CSA_SLOW_ASSERT(
      this, IsOffsetInBounds(offset, LoadAndUntagFixedArrayBaseLength(object),
...
TNode<BoolT> CodeStubAssembler::IsOffsetInBounds(SloppyTNode<IntPtrT> offset,
  ...
  int correction = header_size - kHeapObjectTag - kPointerSize;
  TNode<IntPtrT> last_offset = ElementOffsetFromIndex(
      length, HOLEY_ELEMENTS, INTPTR_PARAMETERS, correction);
