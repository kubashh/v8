...
Node* CodeStubAssembler::WordIsWordAligned(Node* word) {
  return WordEqual(IntPtrConstant(0),
                   WordAnd(word, IntPtrConstant(kBitsPerPointer - 1));
}
...
Node* CodeStubAssembler::LoadContextElement(Node* context, Node* slot_index) {
  Node* offset =
      IntPtrAdd(WordToByteOffset(slot_index),
...
Node* CodeStubAssembler::AllocateNameDictionary(Node* at_least_space_for) {
  ...
  Node* store_size = IntPtrAdd(WordToByteOffset(length),
...
Node* CodeStubAssembler::AllocateJSObjectFromMap(Node* map, Node* properties,
                                                 ...
                                                 AllocationFlags flags) {
  CSA_ASSERT(this, IsMap(map));
  Node* size = WordToByteOffset(LoadMapInstanceSize(map));
...
Node* WordToByteOffset(Node* value) {
...
void CodeStubAssembler::LoadPropertyFromFastObject(Node* object, Node* map,
  ...
  BIND(&if_in_field);
  {
    ...
    {
      Comment("if_inobject");
      Node* field_offset = WordToByteOffset(
...
void CodeStubAssembler::HandleSlackTracking(Node* context, Node* object,
  ...
  Node* instance_size = WordToByteOffset(instance_size_words);
...

  {
    ...
    Node* used_size = IntPtrSub(
        instance_size, WordTobyteOffset(ChangeUint32ToWord(unused_fields)));
