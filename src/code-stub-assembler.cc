...
TNode<FixedArray> CodeStubAssembler::ExtractToFixedArray(
  ...
  VARIABLE(var_result, MachineRepresentation::kTagged);
  VARIABLE(var_to_fixed_array_map, MachineRepresentation::kTagged, source_map);

  Label set_map(this), done(this, {&var_result}), cow_check(this), cow(this),
      new_space_check(this, {&var_to_fixed_array_map});

  Branch(IsFixedDoubleArrayMap(source_map), &set_map, &cow_check);

  BIND(&set_map);
  {
    CSA_ASSERT(this, IsDoubleElementsKind(Int32Constant(from_kind)));
    var_to_fixed_array_map.Bind(LoadRoot(RootIndex::kFixedArrayMap));
  ...
  {
    CSA_ASSERT(this,
               Word32BinaryNot(IsDoubleElementsKind(Int32Constant(from_kind))));
...
TNode<FixedArrayBase> CodeStubAssembler::ExtractFixedArray(
  ...
  if (extract_flags & ExtractFixedArrayFlag::kFixedArrays) {
    ...
    // Here we extract a FixedArray (possibliy FixedCOWArray) to a FixedArray
    // (also possible FixedCOWArray).
