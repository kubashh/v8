...
Node* CodeStubAssembler::NonNumberToNumberOrNumeric(Node* context, Node* input,
  ...
  BIND(&loop);
  {
    ...
    if (mode == ConversionMode::kToNumeric) {
    ...
    BIND(&if_inputisreceiver);
    {
      ...
      if (mode == ConversionMode::kToNumber) {
        Branch(IsNumber(result), &if_done, &if_notdone);
      } else {
        DCHECK(mode == ConversionMode::kToNumeric);
        Branch(IsNumeric(result), &if_done, &if_notdone);
      }
    ...
    BIND(&if_inputisother);
    {
      ...
      // object here.
      auto function_id = mode == ConversionMode::kToNumber
                             ? Runtime::kToNumber
                             : Runtime::kToNumeric;
...
Node* CodeStubAssembler::NonNumberToNumber(Node* context, Node* input) {
  return NonNumberToNumberOrNumeric(context, input, ConversionMode::kToNumber);
}

Node* CodeStubAssembler::NonNumberToNumeric(Node* context, Node* input) {
  return NonNumberToNumberOrNumeric(context, input, ConversionMode::kToNumeric);
}
