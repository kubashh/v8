...
TNode<FixedArray> CodeStubAssembler::ExtractToFixedArrayWithCOW(
    Node* source, Node* first, Node* count, Node* capacity,
    Node* to_fixed_array_map, ElementsKind from_kind,
  ...
  CSA_ASSERT(this, Word32BinaryNot(IsFixedDoubleArrayMap(to_fixed_array_map)));
  CSA_ASSERT(this,
             WordNotEqual(IntPtrOrSmiConstant(0, parameter_mode), capacity));

  VARIABLE(var_result, MachineRepresentation::kTagged);
  VARIABLE(var_to_fixed_array_map, MachineRepresentation::kTagged);
  var_to_fixed_array_map.Bind(to_fixed_array_map);

  Label done(this, {&var_result}), cow(this),
      new_space_check(this, {&var_to_fixed_array_map});
  Branch(WordEqual(var_to_fixed_array_map.value(),
                   LoadRoot(Heap::kFixedCOWArrayMapRootIndex)),
         &cow, &new_space_check);

  BIND(&cow);
  {
    // We shouldn't skip the copy for COW if |source| is a FixedDoubleArray,
    // because our target is to be a FixedArray.
    if (extract_flags & ExtractFixedArrayFlag::kDontCopyCOW &&
        !IsDoubleElementsKind(from_kind)) {
      // We only skip the copy if the target array starts with the first element
      // of |source|. If target starts somewhere in between of |source|, we'd
      // better copy.
      ...
      var_to_fixed_array_map.Bind(LoadRoot(Heap::kFixedArrayMapRootIndex));
  ...
  BIND(&new_space_check);
  {
    ...
    Node* to_elements = AllocateFixedArray(to_kind, capacity, parameter_mode,
                                           AllocationFlag::kNone,
                                           var_to_fixed_array_map.value());
...
TNode<FixedArrayBase> CodeStubAssembler::ExtractFixedArray(
  ...
  if (extract_flags & ExtractFixedArrayFlag::kFixedArrays) {
    ...
    Node* to_elements = ExtractToFixedArrayWithCOW(
