...
class ConstantPool {
 public:
  ...
  void EmitAndClear(bool require_jump);
  bool ShouldEmitNow(bool require_jump) const;
  void Check(bool force_emit, bool require_jump);
  ...
  class BlockScope {
   public:
    explicit BlockScope(Assembler* pool);
    ~BlockScope();
 ...
 private:
  ...
  int first_use_32_ = -1;
  int first_use_64_ = -1;
  // Maps entries to the pc-offset of the instruction that uses them.
