...
bool RegExpParser::CreateNamedCaptureAtIndex(const ZoneVector<uc16>* name,
  ...
  if (named_captures_ == nullptr) {
    named_captures_ = new (zone_->New(sizeof(*named_captures_)))
        ZoneSet<RegExpCapture*, ZoneRegExpCaptureComparator>(zone());
  } else {
  ...
  named_captures_->emplace(capture);  // insert or empl
...
void RegExpParser::PatchNamedBackReferences() {
  ...
  for (int i = 0; i < named_back_references_->length(); i++) {
    ...
    int index = -1;
    RegExpCapture* capture = new (zone()) RegExpCapture(0);
    DCHECK_NULL(capture->name());

    capture->set_name(ref->name());
    const auto& capture_it = named_captures_->find(capture);
