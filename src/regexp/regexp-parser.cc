...
class RegExpBuilder {
 ...
 private:
  ...
  bool unicode() const {
    // /v implies /u (https://tc39.es/ecma262/#sec-parsepattern)
...
class RegExpParserImpl final {
 private:
  ...
  void ParseClassRanges(ZoneList<CharacterRange>* ranges,
  ...
  bool unicodeSets() const { return IsUnicodeSets(top_level_flags_); }
...
bool RegExpParserImpl<CharT>::AddPropertyClassRange(
  ...
  const bool needs_case_folding = unicodeSets() && ignore_case();
  if (name_2.empty()) {
...
base::uc32 RegExpParserImpl<CharT>::ParseCharacterEscape(
  ...
  if (unicodeSets() && in_class_escape_state == InClassEscapeState::kInClass) {
...
template <class CharT>
void RegExpParserImpl<CharT>::ParseClassRanges(
    ...
    if (current() == '-') {
      ...
      if (is_class_1 || is_class_2) {
        ...
        if (unicode()) {
          ...
          ReportError(RegExpError::kInvalidCharacterClass);
          return;
        }
        if (!is_class_1) ranges->Add(CharacterRange::Singleton(char_1), zone());
        ranges->Add(CharacterRange::Singleton('-'), zone());
      ...
      if (char_1 > char_2) {
        ReportError(RegExpError::kOutOfOrderCharacterClass);
        return;
      }
...
RegExpTree* RegExpParserImpl<CharT>::ParseClassSetOperand(
  ...
  if (c == '\\') {
    ...
    if (IsClassSetReservedPunctuator(next)) {
      *type_out = ClassSetOperandType::kClassSetCharacter;
      ranges->Add(CharacterRange::Singleton(next), zone());
      Advance(2);
      return nullptr;
    }
...
RegExpTree* RegExpParserImpl<CharT>::ParseClassUnion(
  ...
  while (has_more() && current() != ']') {
    if (current() == '-') {
      ...
      operand_type = ClassSetOperandType::kClassSetRange;
    } else {
      RegExpTree* operand =
          ParseClassSetOperand(builder, &operand_type, ranges CHECK_FAILED);
      if (operand != nullptr) {
