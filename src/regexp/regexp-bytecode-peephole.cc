...
void RegExpBytecodePeephole::EmitArgument(int start_pc, const byte* bytecode,
  ...
  switch (arg.length) {
    ...
    case 2:
      ...
      break;
    case 3: {
      // We load 4 bytes from position - 1, mask out unneded bits and shift the
      // result. This assumes little-endian.
      DCHECK_EQ(0, (arg_pos - 1) & 3);
      int32_t val = GetValue<int32_t>(bytecode, arg_pos - 1, 0xFFFFFF00) >> 8;
