...
bool RegExpImpl::CompileIrregexp(Isolate* isolate, Handle<JSRegExp> re,
  ...
  // compile_data.bytecode is either bytecode (if we interpret the regexp) or
  // kUninitializedValue (if we compile all regexp or tier-up has happened).
  data->set(JSRegExp::bytecode_index(is_one_byte), compile_data.bytecode);
