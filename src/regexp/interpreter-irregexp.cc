...
class BacktrackStack {
 public:
  ...
  void push(int v) {
    if (has_static_data()) {
      if (static_data_top_ == static_data_end_) {
        SwitchToDynamicData();
  ...
  int sp() const {
    if (has_static_data()) {
      return static_data_top_ - static_data_;
    } else {
 ...
 private:
  ...
  int* static_data_top_ = static_data_;
  int* const static_data_end_ = static_data_ + kStaticCapacity;
