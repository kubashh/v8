...
struct Flag {
  ...
  bool ShouldCheckFlagContradictions() {
  ...
  bool CheckFlagChange(SetBy new_set_by, bool change_flag,
    ...
    // For bool flags, specifying the flag multiple times is allowed as long as
    // the values coincide. For non-bool flags, we disallow specifying them
    // multiple times or explicitly as well as implied because otherwise the
    // presence of a conflict depends on the precise value.
    if (ShouldCheckFlagContradictions() &&
        (change_flag || (type_ != TYPE_MAYBE_BOOL && type_ != TYPE_BOOL))) {
      const char* hint =
      ...
      switch (set_by_) {
        ...
        case SetBy::kWeakImplication:
          if (new_set_by == SetBy::kWeakImplication && change_flag) {
            FATAL(
        ...
        case SetBy::kImplication:
          if (new_set_by == SetBy::kImplication && change_flag) {
            FATAL(
