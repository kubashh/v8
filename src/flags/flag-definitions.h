...
DEFINE_BOOL(
    ...
    "inline JS->Wasm calls (specifically: inline wrappers and the full Wasm "
...
DEFINE_EXPERIMENTAL_FEATURE(
    ...
    "TurboFan). Only the Wasm code is inlined in Turboshaft, the JS->Wasm "
    "wrappers are still inlined in TurboFan. For controlling whether to inline "
    "at all, see --turbo-inline-js-wasm-calls.")
// Can't use Turboshaft Wasm-in-JS inlining without the Turboshaft _JavaScript_
// pipeline. Note however, that this feature is independent of the Turboshaft
// _Wasm_ pipeline (since the inlinee gets compiled with the JS pipeline).
