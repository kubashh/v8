...
EVALUATE(VL) {
  ...
  switch (m3) {
    case 3:
      DCHECK_EQ(0x7&addr, 0);
      for (int i = 0; i < 2; i++)
        set_simd_register_by_lane<int64_t>(r1, i, ptr->int64[i]);
      break;
    case 5:
      DCHECK_EQ(0xf&addr, 0);
...
EVALUATE(VLGV) {
  ...
  int8_t* src = reinterpret_cast<int8_t*>(&get_simd_register(r3))
                + index * size_by_byte;
  memcpy(&registers_[r1], src, size_by_byte);
  return length;
}
...
EVALUATE(VLVG) {
  ...
  int64_t index = b2_val + d2;
  const int size_by_byte = 1 << m4;
  int8_t* dst = reinterpret_cast<int8_t*>(&get_simd_register(r1))
                + index * size_by_byte;
  memcpy(dst, &registers_[r3], size_by_byte);
  return length;
}
