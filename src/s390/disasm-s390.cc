...
int Decoder::FormatRegister(Instruction* instr, const char* format) {
  ...
  if (format[1] == '1') {  // 'r1: register resides in bit 8-11
    int reg = instr->Bits<TwoByteInstr, int>(7, 4);
    PrintRegister(reg);
    return 2;
  } else if (format[1] == '2') {  // 'r2: register resides in bit 12-15
    int reg = instr->Bits<TwoByteInstr, int>(3, 0);
    ...
    int reg = instr->Bits<FourByteInstr, int>(15, 12);
    PrintRegister(reg);
    return 2;
  } else if (format[1] == '4') {  // 'r4: register resides in bit 20-23
    int reg = instr->Bits<FourByteInstr, int>(11, 8);
    PrintRegister(reg);
    return 2;
  } else if (format[1] == '5') {  // 'r5: register resides in bit 24-28
    int reg = instr->Bits<FourByteInstr, int>(7, 4);
    PrintRegister(reg);
    return 2;
  } else if (format[1] == '6') {  // 'r6: register resides in bit 29-32
    int reg = instr->Bits<FourByteInstr, int>(3, 0);
    PrintRegister(reg);
    return 2;
