...
void IC::SetCache(LookupIterator* lookup, const MaybeObjectHandle& handler) {
  DCHECK(IsHandler(*handler));
  // Currently only load and store ICs support non-code handlers.
  DCHECK(IsAnyLoad() || IsAnyStore() || IsAnyHas());
  Handle<Name> name = lookup->GetName();
  switch (state()) {
    case NO_FEEDBACK:
      UNREACHABLE();
    case UNINITIALIZED:
...
MaybeHandle<Object> KeyedStoreIC::StoreTransition(Handle<Object> object,
                                                  Handle<Object> key,
                                                  Handle<Object> value) {
  if (MigrateDeprecated(isolate(), object)) {
    Handle<Object> result;
    if (Runtime::SetObjectProperty(isolate(), object, key, value,
                                   StoreOrigin::kMaybeKeyed,
    ...
    if (key_type == kName) {
      if (!IsNullOrUndefined(*object, isolate())) {
        PropertyKey key(isolate(), maybe_name);
        LookupIterator it(isolate(), object, key, LookupIterator::DEFAULT);
        LookupIterator::State original_state = it.state();
        if (original_state == LookupIterator::NOT_FOUND &&
            StoreIC::LookupForWrite(&it, value, StoreOrigin::kMaybeKeyed)) {
          LookupIterator::State new_state = it.state();
          if (new_state == LookupIterator::TRANSITION &&
              !it.transition_map()->is_dictionary_map()) {
            if (!Object::SetProperty(&it, value, StoreOrigin::kMaybeKeyed,
                                     Just(ShouldThrow::kDontThrow))
                     .IsNothing()) {
              return value;
            }
...
RUNTIME_FUNCTION(Runtime_KeyedStoreTransitionIC_Miss) {
  ...
  // We don't throw on exception. Instead, we return the
  // |mega_transition_failed_symbol| to trigger an eager deoptimization.
  KeyedStoreIC ic(isolate, vector, vector_slot, kind);
  DirectHandle<Object> result;
  if (!ic.StoreTransition(receiver, key, value).ToHandle(&result)) {
    return ReadOnlyRoots(isolate).mega_transition_failed_symbol();
  }
  return *result;
}
