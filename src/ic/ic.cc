...
static FastCloneObjectMode CanFastCloneObject(Handle<Map> map) {
  DisallowGarbageCollection no_gc;
  if (map->IsNullOrUndefinedMap()) return FastCloneObjectMode::kDifferentMap;
    ...
    if (maybe_vector->IsFeedbackVector()) {
      ...
      if (!source->IsSmi() && !nexus.IsMegamorphic()) {
        Handle<Map> source_map(Handle<HeapObject>::cast(source)->map(),
                               isolate);
        FastCloneObjectMode clone_mode = CanFastCloneObject(source_map);
        if (clone_mode == FastCloneObjectMode::kIdenticalMap) {
          ...
          if (source_map->IsNullOrUndefinedMap() ||
              source_map->GetInObjectProperties() -
                      source_map->UnusedInObjectProperties() ==
                  0) {
            Handle<Map> empty = isolate->factory()->ObjectLiteralMapFromCache(
                isolate->native_context(), 0);
            nexus.ConfigureCloneObject(source_map, empty);
            return *empty;
          }
          // TODO(olivf, chrome:1204540) The case where the spread literal
          // would create a literal with a different map (i.e.,
          // FastCloneObjectMode::kDifferentMap) is currently not supported when
          // we have in object properties. To
          // extend the support we would need to ensure that when the source map
          // representation is generalized the result map is too (or that the
          // result map is already as generic as possible to start with).
        }
        DCHECK(clone_mode == FastCloneObjectMode::kDifferentMap ||
               clone_mode == FastCloneObjectMode::kNotSupported);
