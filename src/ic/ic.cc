...
RUNTIME_FUNCTION(Runtime_CloneObjectIC_Miss) {
  ...
  // To clone objects with non-root maps we have to finalize their size.
  auto CompleteSlackTrackingIfNonRoot = [](Handle<Map> map, Isolate* isolate) {
    if (!map->GetBackPointer().is_null() &&
        map->IsInobjectSlackTrackingInProgress()) {
      Map initial_map = map->FindRootMap(isolate);
      MapUpdater::CompleteInobjectSlackTracking(isolate, initial_map);
    }
    DCHECK_IMPLIES(!map->GetBackPointer().is_null(),
                   !map->IsInobjectSlackTrackingInProgress());
  };
    ...
    if (maybe_vector->IsFeedbackVector()) {
      ...
      if (!source->IsSmi() && !nexus.IsMegamorphic()) {
        ...
        if (clone_mode == FastCloneObjectMode::kIdenticalMap) {
          CompleteSlackTrackingIfNonRoot(source_map, isolate);
          ...
          if (CanFastCloneObjectWithDifferentMaps(source_map, result_map)) {
            CompleteSlackTrackingIfNonRoot(result_map, isolate);
