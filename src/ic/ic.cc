...
Handle<Map> FastCloneObjectMap(Isolate* isolate, Handle<HeapObject> source,
  ...
  if (source_map->IsJSObjectMap() &&
      ...
      source_map->prototype() == ((flags & ObjectLiteral::kHasNullPrototype)
                                      ? ReadOnlyRoots(isolate).null_value()
                                      : initial_map->prototype())) {
    // TODO(bmeurer): We could also just transition to the appropriate
    // prototype from {source_map} instead of the hard check above.
