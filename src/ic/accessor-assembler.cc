...
void AccessorAssembler::StoreDataPropertyWithMapTransition(
    const StoreICParameters* p, TNode<Map> transition_map, Label* miss) {
  // Transition to a new Map to add a data property WITH NONE attributes.
  // Ignores prototype validity cell, and ignores existing accessors and
  // attributes. This method should only be used for StoreInLiteral builtins,
  // with the understanding that it's impossible for there to be any
  // non-writable/non-configurable own properties at that time.
  TNode<Uint32T> bitfield3 = LoadMapBitField3(transition_map);
  CSA_ASSERT(this, IsClearWord32<Map::IsDictionaryMapBit>(bitfield3));
  GotoIf(IsSetWord32<Map::IsDeprecatedBit>(bitfield3), miss);
  ...
  CSA_ASSERT(this, WordEqual(BitcastMaybeObjectToWord(LoadWeakFixedArrayElement(
                                 descriptors, last_key_index)),
                             p->name));
...
void AccessorAssembler::GenerateCloneObjectIC_Slow() {
  ...
  {
    Label cont(this);
    GotoIf(IsJSReceiverInstanceType(type), &cont);
  ...
  GotoIfNot(IsJSObjectInstanceType(type), &call_runtime);
