...
#undef ASSERT_CONSECUTIVE
  ...
  BIND(&if_nonsmi_handler);
  {
    ...
    {
      if (p->IsAnyDefineOwn()) {
        // Normally, proto handlers are not created for DefineNamedOwnIC and
        // DefineKeyedOwnIC, however such a handler might be fetched from the
        // StoreIC's megamorphic stub cache. Treat such handlers as slow ones.
        Goto(&if_slow);
