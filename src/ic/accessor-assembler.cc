...
void AccessorAssembler::EmitElementLoad(
  ...
  BIND(&if_nonfast);
  {
    ...
    BIND(&if_typed_array);
    {
      ...
      // in intptr_t representation are considered out of bounds for
      // TypedArrays, that's why we need the special branch in case of
      // failing the UintPtrLessThan test.
      STATIC_ASSERT(JSTypedArray::kMaxLength <=
      ...
      BIND(&if_oob);
      {
        // We get here if either {intptr_index} is
        //
        //  a.) negative, or
        //  b.) positive and outside the bounds of the {object}.
        //
        // In case of a negative {intptr_index} we have to miss to
        // avoid a deopt loop in the optimized code generated by
        // TurboFan for TypedArray accesses.
