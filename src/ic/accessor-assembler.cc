...
void AccessorAssembler::HandleLoadAccessor(
  ...
  TVARIABLE(HeapObject, lookup_start_object, CAST(p->lookup_start_object()));
  Label load(this);
...

  {
    ...
    BIND(&if_add_normal);
    {
      ...
      TNode<Map> lookup_start_object_map =
          LoadMap(CAST(p->receiver_and_lookup_start_object()));
      InvalidateValidityCellIfPrototype(lookup_start_object_map);

      TNode<NameDictionary> properties =
          CAST(LoadSlowProperties(CAST(p->receiver_and_lookup_start_object())));
...
void AccessorAssembler::GenericPropertyLoad(
    ...
    TNode<Int32T> instance_type, const LoadICParameters* p, Label* slow,
    UseStubCache use_stub_cache) {
  CSA_ASSERT(this, TaggedEqual(lookup_start_object, p->lookup_start_object()));
  CSA_ASSERT(this, TaggedEqual(lookup_start_object, p->receiver()));
  ExitPoint direct_exit(this);
  ...
  BIND(&lookup_prototype_chain);
  {
    ...
    BIND(&loop);
    {
      ...
      TryGetOwnProperty(p->context(),
                        lookup_start_object /* == p->receiver() */, CAST(proto),
  ...
  BIND(&special_receiver);
  {
    ...
    direct_exit.ReturnCallStub(
        Builtins::CallableFor(isolate(), Builtins::kProxyGetProperty),
        p->context(), p->receiver_and_lookup_start_object(), name,
...
void AccessorAssembler::LoadIC_BytecodeHandler(const LazyLoadICParameters* p,
  ...
  TNode<Map> lookup_start_object_map =
      LoadLookupStartObjectMap(p->receiver_and_lookup_start_object());
...
void AccessorAssembler::LoadIC_NoFeedback(const LoadICParameters* p,
  ...
  GenericPropertyLoad(CAST(p->receiver()), lookup_start_object_map,
...
void AccessorAssembler::KeyedLoadICGeneric(const LoadICParameters* p) {
  ...
  TNode<Object> receiver_and_lookup_start_object =
      p->receiver_and_lookup_start_object();
  GotoIf(TaggedIsSmi(receiver_and_lookup_start_object), &if_runtime);
  GotoIf(IsNullOrUndefined(receiver_and_lookup_start_object), &if_runtime);

  {
    ...
    BIND(&if_unique_name);
    {
      ...
      TNode<Map> lookup_start_object_map =
          LoadMap(CAST(receiver_and_lookup_start_object));
      TNode<Uint16T> instance_type =
          LoadMapInstanceType(lookup_start_object_map);
      GenericPropertyLoad(CAST(receiver_and_lookup_start_object),
                          lookup_start_object_map, instance_type, &pp,
                          &if_runtime);
    }
    ...
    BIND(&if_index);
    {
      ...
      TNode<Uint16T> instance_type =
          LoadMapInstanceType(lookup_start_object_map);
      GenericElementLoad(CAST(receiver_and_lookup_start_object),
                         lookup_start_object_map, instance_type,
...
void AccessorAssembler::KeyedLoadICPolymorphicName(const LoadICParameters* p,
  ...
  BIND(&miss);
  {
    ...
    TailCallRuntime(access_mode == LoadAccessMode::kLoad
                        ...
                        : Runtime::kKeyedHasIC_Miss,
                    context, p->receiver(), name, slot, vector);
  }
}
...
void AccessorAssembler::StoreIC(const StoreICParameters* p) {
  ...
  BIND(&try_megamorphic);
  {
    ...
    TryProbeStubCache(isolate()->store_stub_cache(), p->lookup_start_object(),
                      CAST(p->name()), &if_handler, &var_handler, &miss);
  }

  BIND(&no_feedback);
  {
    TailCallBuiltin(Builtins::kStoreIC_NoFeedback, p->context(),
                    p->receiver_and_lookup_start_object(), p->name(),
                    p->value(), p->slot());
  }
  ...
  BIND(&miss);
  {
    TailCallRuntime(Runtime::kStoreIC_Miss, p->context(), p->value(), p->slot(),
                    p->vector(), p->receiver_and_lookup_start_object(),
                    p->name());
  }
}
...
void AccessorAssembler::GenerateLoadIC() {
  ...
  LoadICParameters p(context, receiver_and_lookup_start_object, name, slot,
                     vector);
  LoadIC(&p);
}
...
void AccessorAssembler::GenerateLoadIC_Megamorphic() {
  ...
  TryProbeStubCache(isolate()->load_stub_cache(),
                    receiver_and_lookup_start_object, CAST(name), &if_handler,
                    &var_handler, &miss);

  BIND(&if_handler);
  LazyLoadICParameters p(
      // lazy_context
      [=] { return context; }, receiver_and_lookup_start_object,
      // lazy_name
      [=] { return name; },
      // lazy_slot
      [=] { return slot; }, vector);
  ...
  BIND(&miss);
  direct_exit.ReturnCallRuntime(Runtime::kLoadIC_Miss, context,
                                receiver_and_lookup_start_object, name, slot,
                                vector);
}
...
void AccessorAssembler::GenerateLoadIC_Noninlined() {
  ...
  TNode<Map> lookup_start_object_map =
      LoadLookupStartObjectMap(receiver_and_lookup_start_object);
...
void AccessorAssembler::GenerateLoadIC_NoFeedback() {
  ...
  LoadICParameters p(context, receiver_and_lookup_start_object, name,
...
void AccessorAssembler::GenerateLoadICTrampoline() {
  ...
  TailCallBuiltin(Builtins::kLoadIC, context, receiver_and_lookup_start_object,
                  name, slot, vector);
}
...
void AccessorAssembler::GenerateLoadICTrampoline_Megamorphic() {
  ...
  TailCallBuiltin(Builtins::kLoadIC_Megamorphic, context,
...
void AccessorAssembler::GenerateKeyedLoadICTrampoline() {
  ...
  TailCallBuiltin(Builtins::kKeyedLoadIC, context,
                  receiver_and_lookup_start_object, name, slot, vector);
}
...
void AccessorAssembler::GenerateKeyedLoadICTrampoline_Megamorphic() {
  ...
  TailCallBuiltin(Builtins::kKeyedLoadIC_Megamorphic, context,
                  receiver_and_lookup_start_object, name, slot, vector);
}
...
void AccessorAssembler::GenerateStoreIC() {
  ...
  StoreICParameters p(context, receiver_and_lookup_start_object, name, value,
                      slot, vector);
  StoreIC(&p);
}
...
void AccessorAssembler::GenerateStoreICTrampoline() {
  ...
  TailCallBuiltin(Builtins::kStoreIC, context, receiver_and_lookup_start_object,
...
void AccessorAssembler::GenerateKeyedStoreICTrampoline() {
  ...
  TailCallBuiltin(Builtins::kKeyedStoreIC, context,
                  receiver_and_lookup_start_object, name, value, slot, vector);
}
...
void AccessorAssembler::GenerateCloneObjectIC() {
  ...
  TNode<Map> source_map = LoadLookupStartObjectMap(source);
...
void AccessorAssembler::GenerateKeyedHasIC_Megamorphic() {
  ...
  Return(HasProperty(context, receiver_and_lookup_start_object, name,
