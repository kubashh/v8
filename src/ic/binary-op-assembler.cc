...
TNode<Object> BinaryOpAssembler::Generate_AddWithFeedback(
    ...
    TNode<IntPtrT> slot_id, TNode<HeapObject> maybe_feedback_vector,
    bool rhs_is_smi) {
  ...
  BIND(&if_lhsisnotnumber);
  {
    ...
    BIND(&if_lhsisoddball);
    {
      ...
      Branch(TaggedIsNotSmi(rhs), &call_with_oddball_feedback,
             &check_rhsisoddball);
    }
...
TNode<Object> BinaryOpAssembler::Generate_BinaryOperationWithFeedback(
  ...
  BIND(&if_lhsissmi);
  {
    ...
    if (!rhs_is_smi) {
      ...
      {
        // Check if {rhs} is a HeapNumber.
        GotoIfNot(TaggedIsSmi(rhs), &check_rhsisoddball);

        // Perform a floating point operation.
        var_float_lhs = SmiToFloat64(lhs_smi);
        var_float_rhs = LoadHeapNumberValue(CAST(rhs));
