...
// When the key is know to be a UniqueName (e.g. from enum cache), the type
// would be kName, otherwise it would be kKey.
enum class KeyType { kKey, kName };
...
void KeyedStoreGenericAssembler::KeyedStoreTransition(
  ...
  if (key_type == KeyType::kKey) {
    ...
    DCHECK_EQ(key_type, KeyType::kName);
