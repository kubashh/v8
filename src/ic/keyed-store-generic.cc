...
class KeyedStoreGenericAssembler : public AccessorAssembler {
 ...
 private:
  ...
  typedef std::function<void(TNode<Map>)> ValidateAndApplyTransitionFunction;
  void TryMigrateToCachedTransitionMap(
      TNode<Map> map, TNode<Name> name,
      const ValidateAndApplyTransitionFunction& callback, Label* slow);
};
...
void KeyedStoreGenericAssembler::TryMigrateToCachedTransitionMap(
    TNode<Map> map, TNode<Name> name,
    const ValidateAndApplyTransitionFunction& callback, Label* slow) {
  ...
  // weak reference -> simple_tranition
  ...
  BIND(&found_handler_candidate);
  {
    ...
    callback(var_transition_map.value());
  }
}
...
void KeyedStoreGenericAssembler::EmitGenericPropertyStore(
  ...
  BIND(&fast_properties);
  {
    ...
    BIND(&lookup_transition);
    {
      ...
      TryMigrateToCachedTransitionMap(receiver_map, CAST(p->name),
                                      [=](TNode<Map> transition_map) {
                                        // Validate the transition handler
                                        // candidate and apply the transition.
                                        HandleStoreICTransitionMapHandlerCase(
                                            p, transition_map, slow, true);
                                        exit_point->Return(p->value);
                                      },
                                      slow);
    }
...
void KeyedStoreGenericAssembler::EmitGenericPropertyStoreInLiteral(
  ...
  BIND(&fast_properties);
  {
    ...
    {
      Comment("lookup transition");
      TryMigrateToCachedTransitionMap(
          map, key,
          [=, &call_runtime](TNode<Map> transition_map) {
            // Validate the transition handler candidate and apply the
            // transition.
            StoreICParameters p(context, receiver, key, value, nullptr,
                                nullptr);
            HandleStoreICTransitionMapHandlerCase(&p, transition_map,
                                                  &call_runtime, false);
            exit_point->Return(value);
          },
          &call_runtime);
  ...
  BIND(&dictionary_properties);
  {
    ...
    BIND(&not_found);
    {
      ...
      Branch(IsSetWord32<Map::IsExtensibleBit>(bit_field2), &extensible,
             &call_runtime);

      BIND(&extensible);
      Label add_dictionary_property_slow(this);
      InvalidateValidityCellIfPrototype(map, bit_field2);
  ...
  BIND(&call_runtime);
  TNode<Smi> smi_language_mode = SmiConstant(language_mode);
  exit_point->ReturnCallRuntime(Runtime::kStoreDataPropertyInLiteral, context,
                                receiver, key, value, smi_language_mode);
}
