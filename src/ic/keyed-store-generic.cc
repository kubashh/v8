...
void KeyedStoreGenericAssembler::EmitGenericPropertyStore(
  ...
  BIND(&fast_properties);
  {
    ...
    Label lookup_descriptor(this);
    Comment("lookup transition");
    CheckForAssociatedProtector(name, &lookup_descriptor);

    TNode<Map> transition_map = FindCandidateStoreICTransitionMapHandler(
        receiver_map, name, &lookup_descriptor);
    ...
    HandleStoreICTransitionMapHandlerCase(p, transition_map, &lookup_descriptor,
                                          flags);
    exit_point->Return(p->value());

    BIND(&lookup_descriptor);
    {
