...
void V8FileLogger::SetEtwCodeEventHandler(uint32_t options) {
  ...
  if (options & kJitCodeEventEnumExisting) {
    // TODO: Here we log the existing code to all the listeners registered to
    // this Isolate logger, while we should only log to the newly created
    // ETWJitLogger. This should not generally be a problem because it is quite
    // unlikely to have both file logger and ETW tracing both enabled by
    // default.
    HandleScope scope(isolate_);
    LogBuiltins();
    LogCodeObjects();
