...
void V8Provider::LogInitializePlatform() {
  constexpr static auto event_desc = etw::EventDescriptor(kInitializePlatformEvent);
...
void V8Provider::LogShutdownPlatform() {
  constexpr static auto event_desc = etw::EventDescriptor(kShutdownPlatformEvent);
...
void V8Provider::LogSnapshotInitStart(void* isolate) {
  constexpr static auto event_desc = etw::EventDescriptor(kSnapshotInitStartEvent);
  constexpr static auto event_meta =
      etw::EventMetadata("SnapshotInitStart", etw::Field("isolate", kTypePointer));

  LogEventData(State(), &event_desc, &event_meta, isolate);
}

void V8Provider::LogSnapshotInitStop(void* isolate) {
  constexpr static auto event_desc = etw::EventDescriptor(kSnapshotInitStopEvent);
  constexpr static auto event_meta =
      etw::EventMetadata("SnapshotInitStop", etw::Field("isolate", kTypePointer));
...
void V8Provider::LogJitExecuteStart() {
  constexpr static auto event_desc = etw::EventDescriptor(kJitExecuteStartEvent);
...
void V8Provider::LogJitFinalizeStart() {
  constexpr static auto event_desc = etw::EventDescriptor(kJitFinalizeStartEvent);
...
void V8Provider::LogJitFinalizeStop() {
  constexpr static auto event_desc = etw::EventDescriptor(kJitFinalizeStopEvent);
...
void V8Provider::LogDeopt(const std::string& reason, const std::string& kind,
  ...
  constexpr static auto event_meta = etw::EventMetadata(
      "Deopt", etw::Field("reason", kTypeAnsiStr), etw::Field("kind", kTypeAnsiStr),
      etw::Field("src", kTypeAnsiStr), etw::Field("fn", kTypeAnsiStr),
      etw::Field("line", kTypeInt32), etw::Field("column", kTypeInt32));
...
void V8Provider::LogDisableOpt(const std::string& fn_name,
  ...
  LogEventData(State(), &event_desc, &event_meta, etw::Field("fn", kTypeAnsiStr),
...
void V8Provider::LogCodeEvent(const JitCodeEvent* event) {
  ...
  // TODO(sartang@microsoft.com): There are events for CODE_ADD_LINE_POS_INFO and
