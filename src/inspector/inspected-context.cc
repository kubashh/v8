...
void InspectedContext::resetContext(
  ...
  data.SetSecondPassCallback(&InspectedContext::callConsoleDestroyed);
}

void InspectedContext::callConsoleDestroyed(
    const v8::WeakCallbackInfo<InspectedContext::WeakCallbackData>& data) {
  ...
  callbackData->inspector->contextDestroyed(callbackData->groupId,
...
InspectedContext::InspectedContext(V8InspectorImpl* inspector,
  ...
  if (info.destroyedWhenCollected) {
    m_context.SetWeak(
