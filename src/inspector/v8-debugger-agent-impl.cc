...
void adjustBreakpointLocation(const V8DebuggerScript& script,
  ...
  v8::debug::Location hintPosition = script.location((uint32_t)bestMatch);
...
void V8DebuggerAgentImpl::didParseSource(
  ...
  if (success) {
    m_frontend.scriptParsed(
        ...
        isModuleParam, (uint32_t)scriptRef->source().length(),
        std::move(stackTrace));
  } else {
    m_frontend.scriptFailedToParse(
        ...
        (uint32_t)scriptRef->source().length(), std::move(stackTrace));
  }
