...
static void getDebugSymbols(
  ...
  std::vector<v8::debug::WasmScript::DebugSymbols> v8ScriptDebugSymbols;
  script.getDebugSymbols(&v8ScriptDebugSymbols);

  *debugSymbols = std::make_unique<Array<protocol::Debugger::DebugSymbols>>();
  for (size_t i = 0; i < v8ScriptDebugSymbols.size(); ++i) {
    v8::debug::WasmScript::DebugSymbols& symbol = v8ScriptDebugSymbols[i];
    ...
    (*debugSymbols)->emplace_back(std::move(protocolDebugSymbol));
  }
}
#endif  // V8_ENABLE_WEBASSEMBLY
