...
#include "src/inspector/gdb-server/packet.h"
#include "src/inspector/gdb-server/transport.h"

namespace v8_inspector {

Session::Session(Transport* transport)
    : io_(transport), flags_(0), seq_(0), connected_(true) {}

Session::~Session() {}

void Session::WaitForDebugStubEvent(bool ignore_gdb) {
  io_->WaitForDebugStubEvent(ignore_gdb);
}

void Session::SetFlags(uint32_t flags) { flags_ |= flags; }

void Session::ClearFlags(uint32_t flags) { flags_ &= ~flags; }

uint32_t Session::GetFlags() { return flags_; }

bool Session::IsDataAvailable() { return io_->IsDataAvailable(); }

bool Session::Connected() { return connected_; }
...
retry:
  ...
  while (1) {
    ...
    if (ch == '$') {
      GdbRemoteLog(LOG_INFO, "RX Missing $, retry.\n");
      goto retry;
    }
    // Keep a running XSUM
    run_xsum += ch;
  ...
  // if (GetFlags() & DEBUG_RECV) GdbRemoteLog(1, "RX %s\n", in.c_str());
  ...
  if (fin_xsum == run_xsum) {
    ...
    GdbRemoteLog(LOG_INFO, "RX Bad XSUM, retry\n");
    goto retry;
  }
