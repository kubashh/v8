...
void Transport::WaitForDebugStubEvent(bool ignore_input_from_gdb) {
  bool wait = true;
  ...
  handles[0] = socket_event_;
  int count = 1;  // size_ < kBufSize ? 2 : 1;
  int result =
  ...
  if (result == WAIT_TIMEOUT || result == WAIT_OBJECT_0) return;
  GdbRemoteLog(LOG_FATAL,
