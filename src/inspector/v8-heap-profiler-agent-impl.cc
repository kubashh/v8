...
struct V8HeapProfilerAgentImpl::AsyncCallbacks {
  ...
  std::vector<std::unique_ptr<TakeHeapSnapshotCallback>>
...
class V8HeapProfilerAgentImpl::HeapSnapshotTask : public v8::Task {
 public:
  ...
  void Run() override {
    ...
    DCHECK_GT(asyncCallbacks->m_heapSnapshotCallbacks.size(), 0);
    std::unique_ptr<TakeHeapSnapshotCallback> callback =
        std::move(asyncCallbacks->m_heapSnapshotCallbacks.front());
    asyncCallbacks->m_heapSnapshotCallbacks.erase(
        asyncCallbacks->m_heapSnapshotCallbacks.begin());

    Response response = m_agent->takeHeapSnapshotNow(
        m_protocolOptions, cppgc::EmbedderStackState::kNoHeapPointers);
...
void V8HeapProfilerAgentImpl::takeHeapSnapshot(
  ...
  m_asyncCallbacks->m_heapSnapshotCallbacks.push_back(std::move(callback));
  task_runner->PostNonNestableTask(std::make_unique<HeapSnapshotTask>(
