...
struct V8HeapProfilerAgentImpl::AsyncCallbacks {
  ...
  // Heap snapshot tasks are owned by the Isolate on which they are queued, but
  // the associated callbacks are owned by the profiler agent via this
  // AsyncCallbacks structure.
  std::vector<std::pair<V8HeapProfilerAgentImpl::HeapSnapshotTask*,
                        std::unique_ptr<TakeHeapSnapshotCallback>>>
      m_heapSnapshotCallbacks;
};
...
class V8HeapProfilerAgentImpl::HeapSnapshotTask : public v8::Task {
 public:
  ...
  void Run(cppgc::EmbedderStackState stackState) {
    // If the async callbacks object still exists and is not canceled, then the
    // V8HeapProfilerAgentImpl still exists, so we can safely take a snapshot.
    std::shared_ptr<AsyncCallbacks> asyncCallbacks = m_asyncCallbacks.lock();
    if (!asyncCallbacks) return;
    v8::base::MutexGuard lock(&asyncCallbacks->m_mutex);
    if (asyncCallbacks->m_canceled) return;

    auto& heapSnapshotCallbacks = asyncCallbacks->m_heapSnapshotCallbacks;
    auto it =
        std::find_if(heapSnapshotCallbacks.begin(), heapSnapshotCallbacks.end(),
                     [&](const auto& pair) { return pair.first == this; });
    if (it == heapSnapshotCallbacks.end()) {
      // This task must have already been run. This can happen because the task
      // was queued with PostNonNestableTask but then got run early by
      // takePendingHeapSnapshots.
      return;
    }
    std::unique_ptr<TakeHeapSnapshotCallback> callback = std::move(it->second);
    heapSnapshotCallbacks.erase(it);

    Response response =
        m_agent->takeHeapSnapshotNow(m_protocolOptions, stackState);
...
V8HeapProfilerAgentImpl::~V8HeapProfilerAgentImpl() {
  ...
  m_asyncCallbacks->m_heapSnapshotCallbacks.clear();
}
...
void V8HeapProfilerAgentImpl::takeHeapSnapshot(
  ...
  std::unique_ptr<HeapSnapshotTask> task = std::make_unique<HeapSnapshotTask>(
      this, m_asyncCallbacks, protocolOptions);
  m_asyncCallbacks->m_heapSnapshotCallbacks.emplace_back(task.get(),
                                                         std::move(callback));
...
void V8HeapProfilerAgentImpl::takePendingHeapSnapshots() {
  // Each task will remove itself from m_heapSnapshotCallbacks.
  while (m_asyncCallbacks->m_heapSnapshotCallbacks.size() > 0) {
    m_asyncCallbacks->m_heapSnapshotCallbacks.front().first->Run(
        cppgc::EmbedderStackState::kMayContainHeapPointers);
  }
}
