...
void V8InspectorSessionImpl::sendProtocolResponse(
  ...
  m_channel->sendResponse(
      callId, MessageBuffer::create(std::move(message), uses_binary_protocol_));
}

void V8InspectorSessionImpl::sendProtocolNotification(
    std::unique_ptr<protocol::Serializable> message) {
  m_channel->sendNotification(
      MessageBuffer::create(std::move(message), uses_binary_protocol_));
}
...
void V8InspectorSessionImpl::dispatchProtocolMessage(
  ...
  bool binary_protocol =
      message.is8Bit() && message.length() && message.characters8()[0] != '{';
  if (binary_protocol) uses_binary_protocol_ = true;
  int callId;
