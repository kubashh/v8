...
class NoOpEvaluateCallback
    : public protocol::Runtime::Backend::EvaluateCallback {
 public:
  void sendSuccess(
      std::unique_ptr<protocol::Runtime::RemoteObject> result,
      Maybe<protocol::Runtime::ExceptionDetails> exceptionDetails) override {}
  void sendFailure(const protocol::DispatchResponse&) override {}
  void fallThrough() override {}
  ~NoOpEvaluateCallback() = default;
};
...
V8InspectorSession::EvaluateResult V8InspectorSessionImpl::evaluate(
  ...
  scope.tryCatch().SetVerbose(true);
