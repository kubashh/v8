...
void V8Console::ScheduleAsyncTask(
  ...
  V8Console* console =
      static_cast<V8Console*>(v8::External::Cast(*info.Data())->Value());

  v8::debug::ConsoleCallArguments args(info);
  ConsoleHelper helper(args, v8::debug::ConsoleContext(), console->m_inspector);
  ...
  auto it = console->m_asyncTaskIds.find(argId);
  if (it != console->m_asyncTaskIds.end()) {
  ...
  int* taskPtr = new int();
  console->m_asyncTaskIds.emplace(argId, taskPtr);
  console->m_inspector->asyncTaskScheduled(taskName, taskPtr, false);
}
...
void V8Console::StartAsyncTask(
  ...
  console->m_inspector->asyncTaskStarted(taskPtr);
}
...
void V8Console::FinishAsyncTask(
  ...
  int* taskPtr = it->second;
  console->m_inspector->asyncTaskFinished(taskPtr);

  delete taskPtr;
  console->m_asyncTaskIds.erase(argId);
}
...
void V8Console::CancelAsyncTask(
  ...
  int* taskPtr = it->second;
  console->m_inspector->asyncTaskCanceled(taskPtr);
...
void V8Console::installAsyncStackTaggingAPI(v8::Local<v8::Context> context,
  ...
  v8::Local<v8::External> data = v8::External::New(isolate, this);
  console
      ->Set(v8::String::NewFromUtf8Literal(isolate, "scheduleAsyncTask"),
            v8::FunctionTemplate::New(isolate, &ScheduleAsyncTask), data)
      .Check();
  console
      ->Set(v8::String::NewFromUtf8Literal(isolate, "startAsyncTask"),
            v8::FunctionTemplate::New(isolate, &StartAsyncTask), data)
      .Check();
  console
      ->Set(v8::String::NewFromUtf8Literal(isolate, "finishAsyncTask"),
            v8::FunctionTemplate::New(isolate, &FinishAsyncTask), data)
      .Check();
  console
      ->Set(v8::String::NewFromUtf8Literal(isolate, "cancelAsyncTask"),
            v8::FunctionTemplate::New(isolate, &CancelAsyncTask), data)
      .Check();
}
