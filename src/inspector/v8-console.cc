...
class ConsoleHelper {
 public:
  ...
  v8::Maybe<int64_t> firstArgToInteger(int64_t defaultValue) {
    ...
    if (m_info[0]->IsNumber())
      return v8::Just(m_info[0].As<v8::Integer>()->Value());
    return m_info[0]->IntegerValue(m_context);
  }
...
void V8Console::ScheduleAsyncTask(
  ...
  int64_t id = m_asyncTaskIds.size();
...
void V8Console::StartAsyncTask(
  ...
  v8::debug::ConsoleCallArguments args(info);
  ConsoleHelper helper(args, v8::debug::ConsoleContext(), m_inspector);
  v8::Maybe<int64_t> maybeArgId = helper.firstArgToInteger(0);
