...
class ActualScript : public V8DebuggerScript {
 ...
 public:
  ...
  v8::debug::WasmScript::DebugSymbolsType getDebugSymbolsType() const override {
    auto script = this->script();
    if (!script->IsWasm()) return v8::debug::WasmScript::DebugSymbolsType::None;
    return v8::debug::WasmScript::Cast(*script)->GetDebugSymbolType();
  }
