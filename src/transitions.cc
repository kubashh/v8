...
bool TransitionsAccessor::CanStoreSimpleTransition() {
  return encoding() == kUninitialized;
}

bool TransitionsAccessor::IsSimpleTransition() {
  switch (encoding()) {
    case kWeakCell:
    case kTuple3Handler:
...
void TransitionsAccessor::Insert(Handle<Name> name, Handle<Map> target,
  ...
  if (IsSimpleTransition()) {
    Map* old_target = GetSimpleTransition();
    ...
    Reload();
    if (IsSimpleTransition()) {
      old_target = GetSimpleTransition();
      Object* value = raw_transitions_->IsWeakCell()
                          ? WeakCell::cast(raw_transitions_)->value()
                          : raw_transitions_;
      result->Set(0, GetSimpleTransitionKey(old_target), value);
    } else {
...
// TODO: move all this to StoreHandler in new file handler-configuration.cc

bool IsMatchingMap(WeakCell* target_cell, Name* name, PropertyKind kind,
...
Object* ValidTuple3HandlerOrNull(Object* handler, Name* name,
                                 Handle<Map>* out_transition) {
  DCHECK(handler->IsTuple3());
  if (!StoreHandler::IsTuple3StillValid(handler)) return nullptr;
...
// |name| can be nullptr if no name/details check needs to be performed.
Object* ValidFixedArrayHandlerOrNull(Object* handler, Name* name,
                                     Handle<Map>* out_transition) {
  DCHECK(handler->IsFixedArray());
  if (!StoreHandler::IsArrayStillValid(handler, name)) return nullptr;
...
Object* TransitionsAccessor::SearchHandler(Name* name,
  ...
  switch (encoding()) {
    ...
    case kWeakCell:
      return nullptr;
    case kTuple3Handler:
      return ValidTuple3HandlerOrNull(raw_transitions_, name, out_transition);
    case kFixedArrayHandler:
      return ValidFixedArrayHandlerOrNull(raw_transitions_, name,
                                          out_transition);
    case kFullTransitionArray: {
      ...
      if (raw_handler->IsTuple3()) {
        return ValidTuple3HandlerOrNull(raw_handler, nullptr, out_transition);
      }
      if (raw_handler->IsFixedArray()) {
        return ValidFixedArrayHandlerOrNull(raw_handler, nullptr,
                                            out_transition);
      }
      return nullptr;
    }
...
Handle<String> TransitionsAccessor::ExpectedTransitionKey() {
  ...
  DCHECK(!cell->cleared());
  //if (cell->cleared()) return Handle<String>::null();
...
void TransitionsAccessor::EnsureHasFullTransitionArray() {
  ...
  int nof = IsSimpleTransition() ? 1 : 0;
  Handle<TransitionArray> result =
      TransitionArray::Allocate(map_->GetIsolate(), nof);
  DisallowHeapAllocation no_gc;
  Reload();
  int new_nof = IsSimpleTransition() ? 1 : 0;
  if (new_nof != nof) {
    DCHECK(new_nof == 0);
...
void TransitionsAccessor::TraverseTransitionTreeInternal(
    TraverseCallback callback, void* data) {
    ...
    case kFullTransitionArray: {
      if (transitions()->HasPrototypeTransitions()) {
        ...
        for (int i = 0; i < length; ++i) {
          ...
          TransitionsAccessor(Map::cast(cell->value()))
              .TraverseTransitionTreeInternal(callback, data);
        }
      }
      for (int i = 0; i < transitions()->number_of_transitions(); ++i) {
        TransitionsAccessor(transitions()->GetTarget(i))
            .TraverseTransitionTreeInternal(callback, data);
      }
      break;
    }
  ...
  if (simple_target != nullptr) {
    TransitionsAccessor(simple_target)
        .TraverseTransitionTreeInternal(callback, data);
  }
  callback(map_, data);
}
