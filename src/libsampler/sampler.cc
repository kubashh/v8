...
void Sampler::DoSample() {
  ...
  // Wait for the target thread to become suspended.
  zx_signals_t signals = 0;
  zx_status_t suspended = zx_object_wait_one(
      profiled_thread, ZX_THREAD_SUSPENDED, ZX_TIME_INFINITE, &signals);
...
// Fetch a copy of its "general register" states.
  ...
  if (zx_thread_read_state(profiled_thread, ZX_THREAD_STATE_REGSET0,
    ...
    v8::RegisterState state;
#if V8_HOST_ARCH_X64
    state.pc = thread_state.rip;
    state.sp = thread_state.rsp;
    state.fp = thread_state.rbp;
#elif V8_HOST_ARCH_ARM64
    state.pc = thread_state.pc;
    state.sp = thread_state.sp;
    state.fp = thread_state.r[29];
#endif
    SampleStack(state);
  }
