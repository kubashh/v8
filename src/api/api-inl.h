  ...
  ~CallDepthScope() {
    ...
    if (do_callback) {
      if (microtask_queue && microtask_queue->microtasks_policy() ==
        ...
        // If this DCHECK fires, code that could trigger microtask execution is
        // not correctly wrapped in a `MicrotasksScope`. Most code will want to
        // simply allow microtasks to run. However, code that does something
        // more unusual (e.g. simulating a sync call by making an async call and
        // waiting on the result) may want to defer microtask execution to avoid
        // unexpected reordering of tasks.
