...
Local<String> ModuleRequest::GetSpecifier() const {
  ...
  i::Isolate* i_isolate = self->GetIsolate();
  i::DirectHandle<i::Object> specifier(self->specifier(), i_isolate);
  return ToApiHandle<String>(specifier, i_isolate);
}
...
Local<FixedArray> ModuleRequest::GetImportAssertions() const {
  i::DirectHandle<i::ModuleRequest> self = Utils::OpenDirectHandle(this);
  i::Isolate* i_isolate = self->GetIsolate();
  i::DirectHandle<i::Object> import_assertions(self->import_assertions(),
                                               i_isolate);
  return ToApiHandle<FixedArray>(import_assertions, i_isolate);
}
...
Local<Value> Module::GetException() const {
  ...
  i::Isolate* i_isolate = self->GetIsolate();
  ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
  i::DirectHandle<i::Object> exception(self->GetException(), i_isolate);
  return ToApiHandle<Value>(exception, i_isolate);
}
