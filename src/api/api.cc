...
#define SET_FIELD_WRAPPED(i_isolate, obj, setter, external_ptr_type, ptr) \
  do {                                                                    \
    external_ptr_type external_ptr(ptr);                                  \
    i::Handle<i::Object> foreign = FromCData(i_isolate, external_ptr);    \
    (obj)->setter(*foreign);                                              \
  } while (false)

// Interceptor Info callbacks currently all use the same ExternalPointerTag.
#define SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, setter, ptr)            \
  do {                                                                       \
    external_ptr<decltype(ptr), internal::kInterceptorInfoTag> external_ptr( \
        ptr);                                                                \
    i::Handle<i::Object> foreign = FromCData(i_isolate, external_ptr);       \
    (obj)->setter(*foreign);                                                 \
  } while (false)
  ...
  if (c_function_overloads.size() > 0) {
    ...
    for (int i = 0; i < function_count; i++) {
      ...
      i::Handle<i::Object> address =
          FromCData(i_isolate, external::CFunction(c_function.GetAddress()));
      function_overloads->set(
          i::FunctionTemplateInfo::kFunctionOverloadEntrySize * i, *address);
      i::Handle<i::Object> signature = FromCData(
          i_isolate, external::CFunctionInfo(c_function.GetTypeInfo()));
      function_overloads->set(
...
i::Handle<i::InterceptorInfo> CreateInterceptorInfo(
  ...
  if (getter) SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_getter, getter);
  if (setter) SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_setter, setter);
  if (query) SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_query, query);
  if (descriptor)
    SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_descriptor, descriptor);
  if (remover)
    SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_deleter, remover);
  if (enumerator)
    SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_enumerator, enumerator);
  if (definer)
    SET_WRAPPED_INTERCEPTOR_INFO(i_isolate, obj, set_definer, definer);
...
void ObjectTemplate::SetAccessCheckCallback(AccessCheckCallback callback,
  ...
  SET_FIELD_WRAPPED(i_isolate, info, set_callback,
                    external::AccessCheckCallback, callback);
...
void Context::SetAbortScriptExecution(
  ...
  if (callback == nullptr) {
    ...
    SET_FIELD_WRAPPED(i_isolate, context, set_script_execution_callback,
                      external::AbortScriptExecutionCallback, callback);
  }
}
...
#undef SET_WRAPPED_INTERCEPTOR_INFO
#undef NEW_STRING
