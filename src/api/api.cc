...
Local<AsyncContext::Variable> v8::AsyncContext::Variable::New(
    Isolate* v8_isolate) {
  Utils::ApiCheck(i::v8_flags.js_async_context,
...
Local<String> v8::AsyncContext::Variable::Name() {
  CHECK(i::v8_flags.js_async_context);
...
MaybeLocal<Value> v8::AsyncContext::Variable::GetValue() {
  ...
  i::Handle<i::HeapObject> snapshot =
      i_isolate->factory()->async_context_store();
...
v8::AsyncContext::VariableScope::VariableScope(
  ...
  i::Handle<i::HeapObject> previous_snapshot =
      isolate_->factory()->async_context_store();
  ...
  if (!i::IsUndefined(*previous_snapshot)) {
    ...
    new_snapshot = i::OrderedHashMap::Allocate(
                       isolate_, previous_snapshot_map->NumberOfElements() + 1)
                       .ToHandleChecked();
    ...
    new_snapshot = i::OrderedHashMap::Allocate(isolate_, 1).ToHandleChecked();
  }
...
bool v8::AsyncContext::Snapshot::IsEqual(Local<Snapshot> other) {
  CHECK(i::v8_flags.js_async_context);
  i::Handle<i::JSAsyncContextSnapshot> js_this = Utils::OpenHandle(this);
  i::Handle<i::JSAsyncContextSnapshot> js_other = Utils::OpenHandle(*other);

  i::Isolate* i_isolate = js_this->GetIsolate();
  CHECK_EQ(js_other->GetIsolate(), i_isolate);

  if (js_this->snapshot() == js_other->snapshot()) {
    return true;
  }

  if (i::IsUndefined(js_this->snapshot()) ||
      i::IsUndefined(js_other->snapshot())) {
    return false;
  }

  DCHECK(i::IsOrderedHashMap(js_this->snapshot()));
  DCHECK(i::IsOrderedHashMap(js_other->snapshot()));

  i::Handle<i::OrderedHashMap> this_map(
      i::Tagged<i::OrderedHashMap>::cast(js_this->snapshot()), i_isolate);
  i::Handle<i::OrderedHashMap> other_map(
      i::Tagged<i::OrderedHashMap>::cast(js_other->snapshot()), i_isolate);
  DCHECK_NE(this_map->NumberOfElements(), 0);
  DCHECK_NE(other_map->NumberOfElements(), 0);

  if (this_map->NumberOfElements() != other_map->NumberOfElements()) {
    return false;
  }

  i::ReadOnlyRoots roots(i_isolate);
  for (i::InternalIndex i : this_map->IterateEntries()) {
    i::Tagged<i::Object> key = this_map->KeyAt(i);
    if (!this_map->IsKey(roots, key)) continue;

    i::InternalIndex other_i = other_map->FindEntry(i_isolate, key);
    if (other_i.is_not_found() ||
        this_map->ValueAt(i) != other_map->ValueAt(other_i)) {
      return false;
    }
  }
