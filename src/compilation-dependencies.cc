...
void InstallDependency(Isolate* isolate, Handle<WeakCell> source,
  ...
  DCHECK_NOT_NULL(isolate->handle_scope_data()->canonical_scope);
...
class FieldTypeDependency final : public CompilationDependencies::Dependency {
 public:
  FieldTypeDependency(const Isolate* isolate_, Handle<Map> owner,
  ...
  bool IsValid() const override {
    ...
    DCHECK_EQ(*owner_, owner_->FindFieldOwner(isolate_, descriptor));
