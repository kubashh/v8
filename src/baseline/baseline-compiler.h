...
class BaselineAssembler final {
 public:
  explicit BaselineAssembler(Isolate* isolate)
  ...
  void j(Condition cc, Label* target, Label::Distance distance = Label::kFar);
  void jmp(Label* target, Label::Distance distance = Label::kFar);
  ...
  void ComparePointer(Register value, Operand operand);
  void CompareRoot(Register value, RootIndex index);
  Condition CheckSmi(Register value);
  void SmiCompare(Register lhs, Register rhs);
  // cmp_tagged
  void CompareTagged(Register value, Operand operand);
  void CompareTagged(Operand operand, Register value);
  void CompareByte(Register value, Immediate immediate);
  ...
  void AddSmi(MemOperand operand, Immediate immediate);
  ...
  void BitwiseOrInt(Register lhs, Immediate immediate);
  ...
  class ScratchRegisterScope {
   public:
    explicit ScratchRegisterScope(BaselineAssembler* assembler)
        : assembler_(&assembler->masm_) {
      USE(assembler_);
    }
    explicit ScratchRegisterScope(MacroAssembler* assembler)
