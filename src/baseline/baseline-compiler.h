...
#undef DECLARE_VISITOR
  ...
  // for CFI.
  enum class IndirectJumpTarget { kNo, kYes };
  ...
  Label* EnsureLabel(int i, IndirectJumpTarget indirect_jump_target =
                                IndirectJumpTarget::kNo) {
    if (labels_[i].GetPointer() == nullptr) {
      labels_[i].SetPointer(zone_.New<Label>());
    }
    if (indirect_jump_target == IndirectJumpTarget::kYes) {
