...
template <typename... Args>
struct CountPushHelper<ForceAlignmentMarker, Args...> {
  // TODO(leszeks): Should we count this somehow?
  static int Count(ForceAlignmentMarker, Args... args) {
    return CountPushHelper<Args...>::Count(args...);
  }
};
...
//       PrintF("%c%d", reg_char, reg.code());
//     }
//   } else if (reg.Aliases(sp)) {
//     // Disassemble w31/x31 as stack pointer wsp/sp.
//     PrintF("%s", reg.Is64Bits() ? "sp" : "wsp");
//   } else {
...
// void Print(interpreter::Register reg) { PrintF("%s",
// reg.ToString(1).c_str()); }

// void Print(TaggedIndex value) {
//   PrintF("<idx:%d>", static_cast<int>(value.value()));
// }
// // void Print(ExternalReference reference) {
// //   PrintF("<%p", reinterpret_cast<const void*>(reference.address()));
// //   const Runtime::Function* fn =
// //   Runtime::FunctionForEntry(reference.address()); if (fn)
// //   PrintF("<%s.entry>", fn->name); PrintF(">");
// // }
// void Print(Handle<HeapObject> value) {
//   PrintF("<");
//   value->ShortPrint();
//   PrintF(">");
// }
// void Print(Smi value) { PrintF("<smi:%d>", value.value()); }
// void Print(RootIndex idx) { PrintF("%s", RootsTable::name(idx)); }
// void Print(MemOperand op) {
//   PrintF("[");
//   Print(op.base());
//   PrintF(" + ");
//   if (op.regoffset().is_valid()) {
//     Print(op.regoffset());
//   } else {
//     PrintF("%d", static_cast<int>(op.offset()));
//   }
//   PrintF(" << %d", static_cast<int>(op.shift_amount()));
//   PrintF("]");
// }
// void Print(int32_t value) { PrintF("<int:%d>", value); }
...
template <typename Arg, typename... Args>
struct PushAllHelper<ForceAlignmentMarker, Arg, Args...> {
  static void Push(BaselineAssembler* basm, ForceAlignmentMarker, Arg arg,
                   Args... args) {
    // PrintF("-Alignment marker -- already aligned\n");
    PushAll(basm, arg, args...);
  }
  static void PushReverse(BaselineAssembler* basm, ForceAlignmentMarker,
                          Arg arg, Args... args) {
    // PrintF("-Alignment marker -- already aligned\n");
    PushAllReverse(basm, arg, args...);
  }
};
template <typename Arg, typename... Args>
struct PushAllHelper<Arg, ForceAlignmentMarker, Args...> {
  static void Push(BaselineAssembler* basm, Arg arg, ForceAlignmentMarker,
                   Args... args) {
    // PrintF("-Alignment marker -- pushing padding\n");
    PushAll(basm, arg, padreg, args...);
  }
  static void PushReverse(BaselineAssembler* basm, Arg arg,
                          ForceAlignmentMarker, Args... args) {
    // PrintF("-Alignment marker -- pushing padding\n");
    PushAllReverse(basm, arg, padreg, args...);
  }
};
...
struct PushAllHelper<Arg, interpreter::RegisterList> {
  static void Push(BaselineAssembler* basm, Arg arg,
    ...
    // PrintF("-Pushing list of %d with pop left\n", list.register_count());
  ...
  static void PushReverse(BaselineAssembler* basm, Arg arg,
    ...
    if (list.register_count() == 0) {
      // PrintF("-Reverse pushing empty list\n");
      PushAllReverse(basm, arg);
    } else {
      // PrintF("-Reverse pushing list of %d with pop left\n",
      //  list.register_count());
...
struct PushAllHelper<interpreter::RegisterList> {
  ...
  static void PushReverse(BaselineAssembler* basm,
                         interpreter::RegisterList list) {
    // PrintF("-Reverse pushing list of %d\n", list.register_count());
...
int BaselineAssembler::Push(T... vals) {
  ...
  // the first push.
  int push_count = CountPushHelper<T...>::Count(vals...);
  // PrintF("---Push %d\n", push_count);
  if (push_count % 2 == 0) {
    PushAll(this, vals...);
  } else {
    PushAll(this, padreg, vals...);
  }
  return push_count;
}
...
void BaselineAssembler::PushReverse(T... vals) {
  // PrintF("---Push reverse %d\n", CountPushHelper<T...>::Count(vals...));
  PushAllReverse(this, vals...);
}

template <typename... T>
void BaselineAssembler::Pop(T... registers) {
  PopAllHelper<T...>::Pop(this, registers...);
}
...
void BaselineCompiler::Prologue() {
  ...
  // Push lr and fp here, since CallBuiltin will override lr.
...
void BaselineCompiler::PrologueFillFrame() {
  ...
  // BaselinePrologue already pushed one undefined.
  int i = 1;
  if (has_new_target) {
    if (new_target_index == 0) {
      // Oops, need to fix up that undefined that BaselinePrologue pushed.
  ...
  if (register_count < 2 * kLoopUnrollSize) {
    ...
    // We enter the loop unconditionally, so
