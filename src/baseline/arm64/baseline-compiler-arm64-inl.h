...
struct PushAllHelper<Arg> {
  ...
  static void PushReverse(BaselineAssembler* basm, Arg) {
...
struct PushAllHelper<interpreter::RegisterList> {
  ...
  static void PushReverse(BaselineAssembler* basm,
                          interpreter::RegisterList list) {
    DCHECK_EQ(list.register_count() % 2, 0);
    for (int reg_index = list.register_count() - 2; reg_index >= 0;
         reg_index -= 2) {
      PushAllReverse(basm, list[reg_index], list[reg_index + 1]);
    }
  }
};
...
template <typename... T>
int BaselineAssembler::PushReverse(T... vals) {
  int push_count = CountPushHelper<T...>::Count(vals...);
  if (push_count % 2 == 0) {
    PushAllReverse(this, vals...);
  } else {
    PushAllReverse(this, padreg, vals...);
  }
  return push_count;
}
...
void BaselineAssembler::EmitReturn(MacroAssembler* masm) {
  ...
  __ masm()->lsl(actual_params_size, actual_params_size,
                 kSystemPointerSizeLog2);
  __ masm()->Add(actual_params_size, actual_params_size,
