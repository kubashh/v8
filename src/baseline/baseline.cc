...
class BaselineCompiler {
 public:
  ...
  void Epilogue() {
    ...
    __ SaveRegisters(kJSCallerSaved);
    __ Push(kJSFunctionRegister);
    __ CallRuntime(Runtime::kEnsureFeedbackVectorForFunction);
    __ RestoreRegisters(kJSCallerSaved);
    __ jmp(&not_optimized_);
  }
  ...
  void LoadClosureFeedbackArray(Register output, Register closure) {
    ...
    Label done;
    DCHECK_NE(output, kScratchRegister);
    __ CmpObjectType(output, CLOSURE_FEEDBACK_CELL_ARRAY_TYPE,
                     kScratchRegister);
    __ j(equal, &done);
