...
//       *isolate_->factory()->name() ==
  ...
  void BuildCall(ConvertReceiverMode mode, uint32_t slot, uint32_t arg_count,
                 Args... args);
  void BuildCallRuntime(Runtime::FunctionId fid, Operand first_argument,
                        uint32_t arg_count);
...
void BaselineCompiler::VisitCallRuntime() {
  BuildCallRuntime(accessor().GetRuntimeIdOperand(0), RegisterOperand(1),
...
void BaselineCompiler::VisitInvokeIntrinsic() {
  BuildCallRuntime(accessor().GetIntrinsicIdOperand(0), RegisterOperand(1),
                   accessor().GetRegisterCountOperand(2));
}
...
void BaselineCompiler::VisitCreateMappedArguments() {
  ...
  // TODO(pthier): This is not working because GetCallerArguments
