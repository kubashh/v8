...
class BaselineAssembler : public MacroAssembler {
 public:
  ...
  /*
   * TODO(verwaest): This is smaller but slower with sandbox :(
...
class BaselineCompiler {
 public:
  ...
  void PrologueHandleOptimizationState(Register feedback_vector) {
    ...
    __ j(not_equal, &no_feedback_vector_);
  ...
  void Epilogue() {
    ...
    __ RecordComment("[ Feedback vector init");
    __ bind(&no_feedback_vector_);
    // TODO(leszeks): Make this less ugly, possibly move into the optimization
    // marker check handling.
    __ TailCallBuiltin(Builtins::kPrepareForBaseline);
  ...
  void LoadFeedbackVector(Register output) {
    ...
    if (__ emit_debug_code()) {
      ...
      __ Assert(equal, AbortReason::kNoReason);
    }
    __ RecordComment("]");
  }
 ...
 private:
  ...
  void AddPosition() {
    bytecode_offset_table_builder_.AddPosition(__ pc_offset(), accessor().current_offset());
  }
...
void BaselineCompiler::VisitJumpLoop() {
  ...
  CallBuiltin(Builtins::kInterpreterOnStackReplacement);
  __ RecordComment("]");
...
Handle<Code> CompileWithBaseline(Isolate* isolate,
  ...
  Handle<Code> code =
      CompileWithBaseline(isolate, shared->internal_formal_parameter_count(),
