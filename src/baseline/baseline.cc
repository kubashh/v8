...
//       *isolate_->factory()->name() ==
  ...
  void BuildBinopWithSmi(Builtins::Name builtin_name, bool fast_path,
                         bool check_overflow,
                         std::function<void(Register, Smi)> instruction);
...
void BaselineCompiler::BuildBinopWithSmi(
  ...
  if (fast_path) {
    ...
    instruction(kInterpreterAccumulatorRegister, IndexAsSmi(0));
    if (check_overflow) {
...
void BaselineCompiler::VisitAddSmi() {
  BuildBinopWithSmi(Builtins::kAdd_WithFeedback, true, true,
                    [&](Register lhs, Smi rhs) {
                      __ Move(kScratchRegister, rhs);
                      __ addq(lhs, kScratchRegister);
                    });
}
...
void BaselineCompiler::VisitBitwiseOrSmi() {
  BuildBinopWithSmi(Builtins::kBitwiseOr_WithFeedback, false, true,
                    [&](Register lhs, Smi rhs) {
                      __ Move(kScratchRegister, rhs);
                      __ orq(lhs, kScratchRegister);
                    });
}
void BaselineCompiler::VisitBitwiseXorSmi() {
  BuildBinopWithSmi(Builtins::kBitwiseXor_WithFeedback, false, true,
                    [&](Register lhs, Smi rhs) {
                      __ Move(kScratchRegister, rhs);
                      __ xorq(lhs, kScratchRegister);
                    });
}
void BaselineCompiler::VisitBitwiseAndSmi() {
  BuildBinopWithSmi(Builtins::kBitwiseAnd_WithFeedback, false, true,
                    [&](Register lhs, Smi rhs) {
                      __ Move(kScratchRegister, rhs);
                      __ andq(lhs, kScratchRegister);
                    });
}
void BaselineCompiler::VisitShiftLeftSmi() {
  BuildBinopWithSmi(
      Builtins::kShiftLeft_WithFeedback, false, true,
      [&](Register lhs, Smi rhs) { __ shlq(lhs, Immediate(rhs.value())); });
}
void BaselineCompiler::VisitShiftRightSmi() {
  BuildBinopWithSmi(
      Builtins::kShiftRight_WithFeedback, false, true,
      [&](Register lhs, Smi rhs) { __ sarq(lhs, Immediate(rhs.value())); });
}
void BaselineCompiler::VisitShiftRightLogicalSmi() {
  BuildBinopWithSmi(
      Builtins::kShiftRightLogical_WithFeedback, false, true,
      [&](Register lhs, Smi rhs) { __ shrq(lhs, Immediate(rhs.value())); });
}
