...
class BaselineCompiler {
 public:
  ...
  void LoadIndexAsSmi(Register output, int operand_index) {
    __ Move(output, Smi::FromInt(Index(operand_index)));
  }
...
void BaselineCompiler::VisitLdaNamedProperty() {
  ...
  LoadTaggedPointerField(LoadWithVectorDescriptor::VectorRegister(),
                         kScratchRegister, FeedbackCell::kValueOffset);
...
void BaselineCompiler::VisitLdaKeyedProperty() {
  ...
  __ LoadTaggedPointerField(
      kScratchRegister,
      FieldOperand(kScratchRegister, JSFunction::kFeedbackCellOffset));
  __ LoadTaggedPointerField(
      LoadWithVectorDescriptor::VectorRegister(),
      FieldOperand(kScratchRegister, FeedbackCell::kValueOffset));
  ...
  __ Move(LoadDescriptor::NameRegister(), kInterpreterAccumulatorRegister);
...
void BaselineCompiler::VisitCreateBlockContext() {
  ...
  __ movq(__ ContextOperand(), kInterpreterAccumulatorRegister);
}
...
void BaselineCompiler::VisitJumpIfToBooleanTrueConstant() {
  Label if_false;
  __ JumpIfToBoolean(kInterpreterAccumulatorRegister,
...
void BaselineCompiler::VisitJumpIfToBooleanFalseConstant() {
  Label if_true;
  __ JumpIfToBoolean(kInterpreterAccumulatorRegister, &if_true,
                     BuildJumpLabel(accessor().GetJumpTargetOffset()));
  __ bind(&if_true);
}
