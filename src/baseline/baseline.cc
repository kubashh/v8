...
class BaselineSourcePositionTableBuilder {
 public:
...
class BaselineCompiler {
 public:
  ...
  void SetFunctionEntrySourcePosition(int position) {
    ...
    //     code_offset, SourcePosition(position), false);
  }
...
//       *isolate_->factory()->name() ==
  ...
  void BuildCall(ConvertReceiverMode mode, uint32_t slot, uint32_t arg_count,
                 Args... args);
  void BuildCallRuntime(Runtime::FunctionId fid,
                                        interpreter::Register first_register,
                        uint32_t arg_count);
  ...
  BaselineSourcePositionTableBuilder source_position_table_builder_;
...
void BaselineCompiler::BuildCallRuntime(Runtime::FunctionId fid,
                                        interpreter::Register first_register,
                                        uint32_t arg_count) {
  for (uint32_t i = 0; i < arg_count; i++) {
    __ Push(__ RegisterFrameOperand(
        interpreter::Register(first_register.index() + i)));
  }
  LoadContext(kContextRegister);
  __ CallRuntime(fid);
}
void BaselineCompiler::VisitCallRuntime() {
  BuildCallRuntime(accessor().GetRuntimeIdOperand(0), accessor().GetRegisterOperand(1),
                   accessor().GetRegisterCountOperand(2));
}
...
void BaselineCompiler::VisitInvokeIntrinsic() {
  ...
  interpreter::Register first_arg_register = accessor().GetRegisterOperand(1);
  uint32_t arg_count = accessor().GetRegisterCountOperand(2);
  switch (intrinsic_id) {
    // TODO(leszeks): Extract into per-inline function helpers.
    case Runtime::kInlineCreateJSGeneratorObject:
      DCHECK_EQ(arg_count, 2);
      CallBuiltin(Builtins::kCreateGeneratorObject,
    ...
    case Runtime::kInlineGeneratorGetResumeMode:
      LoadRegister(kInterpreterAccumulatorRegister, first_arg_register);
    ...
    default:
      BuildCallRuntime(intrinsic_id, first_arg_register, arg_count);
      break;
  }
}
...
void BaselineCompiler::VisitTestInstanceOf() {
  ...
  CallBuiltin(Builtins::kInstanceOf_WithFeedback, RegisterOperand(0),  // object
              kInterpreterAccumulatorRegister,  // callable
              Immediate(Index(1)),              // slot
              kScratchRegister);                // feedback vector
}
...
void BaselineCompiler::VisitCreateArrayFromIterable() {
  CallBuiltin(Builtins::kIterableToListWithSymbolLookup, kInterpreterAccumulatorRegister);
}
