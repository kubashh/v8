...
void BaselineCompiler::VisitJumpLoop() {
  int weight = accessor().GetRelativeJumpTargetOffset();
  DCHECK_EQ(back_jumps_.count(accessor().GetJumpTargetOffset()), 1);
  Label* label = &back_jumps_[accessor().GetJumpTargetOffset()];
...
void BaselineCompiler::VisitJumpIfJSReceiver() {
  UpdateInterruptBudgetAndDoInterpreterJump();
  Label dont_jump;
  __ JumpIfSmi(kInterpreterAccumulatorRegister, &dont_jump, Label::kNear);
