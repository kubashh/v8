...
class BaselineAssembler : public MacroAssembler {
 public:
  ...
  void BitwiseOrSmi(Register lhs, int32_t rhs) {
    orq(lhs, AsSmiImmediate(rhs));
  }
  void BitwiseOrSmi(Register lhs, Register rhs) { orq(lhs, rhs); }
  void BitwiseXorSmi(Register lhs, int32_t rhs) {
    xorq(lhs, AsSmiImmediate(rhs));
  }
  void BitwiseXorSmi(Register lhs, Register rhs) { xorq(lhs, rhs); }
  void BitwiseAndSmi(Register lhs, int32_t rhs) {
    andq(lhs, AsSmiImmediate(rhs));
  }
  void BitwiseAndSmi(Register lhs, Register rhs) { andq(lhs, rhs); }
...
#undef DECLARE_VISITOR
  ...
  void CallBuiltin(Builtins::Name builtin, Args... args) {
    ...
    #ifdef DEBUG
    if (FLAG_sparkplug_gc_after_call) {
      ...
      CallRuntime(Runtime::kCollectGarbage, UndefinedOperand());
      if (Builtins::CallInterfaceDescriptorFor(builtin).GetReturnCount() == 2) {
  ...
  void JumpIfToBoolean(bool do_jump_if_true, Register reg, Label* label,
    ...
    #ifdef DEBUG
    if (FLAG_sparkplug_gc_after_call) {
      // Because we add a runtime call to GC.
      distance = Label::kFar;
...
void BaselineCompiler::BuildBinop(
  ...
  if (fast_path) {
    ...
    instruction(kInterpreterAccumulatorRegister, right);
    if (check_overflow) {
  ...
  // This duplicates the load from the fast path above since the instruction may
  // have clobbered the value after which we decide to jump to the builtin due
  // to overflow or updating feedback.
  LoadRegister(kInterpreterAccumulatorRegister, 0);
...
void BaselineCompiler::BuildBinopWithSmi(
  ...
  if (fast_path) {
    ...
    __ bind(&builtin);
    __ Move(kInterpreterAccumulatorRegister, rcx);
  }
  CallBuiltin(builtin_name, kInterpreterAccumulatorRegister,  // lhs
              IntAsSmi(0),                                    // rhs
              Immediate(Index(1)),                            // slot
...
void BaselineCompiler::VisitIntrinsicGeneratorClose(
    interpreter::RegisterList args) {
  LoadRegister(kInterpreterAccumulatorRegister, args[0]);
  __ Move(kScratchRegister, Smi::FromInt(JSGeneratorObject::kGeneratorClosed));
  StoreTaggedField(kInterpreterAccumulatorRegister,
                   JSGeneratorObject::kContinuationOffset, kScratchRegister);
  __ LoadRoot(kInterpreterAccumulatorRegister, RootIndex::kUndefinedValue);
}
...
Handle<Code> CompileWithBaseline(Isolate* isolate,
  ...
  return CompileWithBaseline(isolate, shared->internal_formal_parameter_count(),
