...
class BaselineCompiler {
 public:
  ...
  void LoadRegister(Register output, interpreter::Register source) {
    __ movq(output, __ RegisterFrameOperand(source));
  }
...
#undef DECLARE_VISITOR
  ...
  Label* BuildForwardJumpLabel() {
    ...
    std::vector<Label>& labels = forward_jumps_[target_offset];
    labels.emplace_back();
    return &labels.back();
  }
...
void BaselineCompiler::BuildCall(ConvertReceiverMode mode, Args... args) {
  ...
  uint32_t arg_count = sizeof...(args) - 1;  // Remove receiver.
  uint32_t slot_index = sizeof...(args) + 1;
  switch (mode) {
    ...
    case ConvertReceiverMode::kNullOrUndefined:
      ...
      slot_index--;  // No receiver in the bytecode argument.
      break;
  ...
  MoveArgumentsForDescriptor(&masm_, descriptor,
                             ...
                             Immediate(Index(slot_index)),  // kSlot
...
void BaselineCompiler::VisitCallProperty0() {
  BuildCall(ConvertReceiverMode::kNotNullOrUndefined, RegisterOperand(1));
}
...
void BaselineCompiler::VisitCallUndefinedReceiver() {
  // TODO(victor): not sure if we should use kInterpreterPushArgsThenCall!
  // Missing feedback!
  ...
  __ movq(descriptor.GetRegisterParameter(
              InterpreterPushArgsThenCallDescriptor::kFunction),
          RegisterOperand(0));
  __ leaq(descriptor.GetRegisterParameter(
              InterpreterPushArgsThenCallDescriptor::kFirstArgument),
          RegisterOperand(1));
  __ movq(descriptor.GetRegisterParameter(
              InterpreterPushArgsThenCallDescriptor::kNumberOfArguments),
          Immediate(arg_count));
  if (descriptor.HasContextParameter()) {
    LoadContext(descriptor.ContextRegister());
  }
  __ CallBuiltin(Builtins::kInterpreterPushUndefinedAndArgsThenCall);
}
void BaselineCompiler::VisitCallUndefinedReceiver0() {
  BuildCall(ConvertReceiverMode::kNullOrUndefined, UndefinedOperand());
}
...
void BaselineCompiler::VisitCreateRegExpLiteral() {
  // TODO(verwaest): Call a builtin with fast path instead.
  Register feedback_vector = kScratchRegister;
  LoadFeedbackVector(feedback_vector);
  CallRuntime(Runtime::kCreateRegExpLiteral,
...
void BaselineCompiler::VisitJump() { __ j(always, BuildForwardJumpLabel()); }
void BaselineCompiler::VisitJumpConstant() {
  __ j(always, BuildForwardJumpLabel());
}
...
void BaselineCompiler::VisitJumpIfNotUndefinedConstant() {
  __ JumpIfNotRoot(kInterpreterAccumulatorRegister, RootIndex::kUndefinedValue,
                   BuildForwardJumpLabel());
}
...
void BaselineCompiler::VisitJumpIfTrueConstant() {
  __ CompareRoot(kInterpreterAccumulatorRegister, RootIndex::kTrueValue);
  __ j(equal, BuildForwardJumpLabel());
}
...
void BaselineCompiler::VisitJumpIfJSReceiverConstant() {
  ...
  __ j(greater_equal, BuildForwardJumpLabel());
