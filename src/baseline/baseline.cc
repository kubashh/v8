...
class BaselineCompiler {
 public:
  ...
  void LoadRegister(Register output, int operand_index) {
    __ movq(output, __ RegisterFrameOperand(
                        accessor().GetRegisterOperand(operand_index)));
  }
  void PushRegister(int operand_index) {
    __ Push(
        __ RegisterFrameOperand(accessor().GetRegisterOperand(operand_index)));
  }
...
#undef DECLARE_VISITOR
  ...
  void ToBoolean(Register reg) {
    __ Move(CallDescriptors::call_descriptor_data(CallDescriptors::ToBoolean)
                ->register_param(0),
            reg);
    LoadContext(ToBooleanDescriptor::ContextRegister());
    __ CallBuiltin(Builtins::kToBoolean);
  }
...
void BaselineCompiler::VisitLdaLookupSlot() {
  PushConstant<Name>(0);
  LoadContext(CallInterfaceDescriptor::ContextRegister());
...
void BaselineCompiler::VisitLdaNamedProperty() {
  ...
  LoadContext(LoadDescriptor::ContextRegister());
  LoadRegister(LoadDescriptor::ReceiverRegister(), 0);
  LoadConstant<Name>(LoadDescriptor::NameRegister(), 1);
  LoadIndexAsSmi(LoadDescriptor::SlotRegister(), 2);
...
void BaselineCompiler::VisitLdaNamedPropertyNoFeedback() {
  ...
  LoadRegister(LoadDescriptor::ReceiverRegister(), 0);
  LoadConstant<Name>(LoadDescriptor::NameRegister(), 1);
  __ CallBuiltin(Builtins::kGetProperty);
}
...
void BaselineCompiler::VisitLdaKeyedProperty() {
  ...
  LoadContext(LoadDescriptor::ContextRegister());
  LoadRegister(LoadDescriptor::ReceiverRegister(), 0);
  LoadAccumulator(LoadDescriptor::NameRegister());
  LoadIndexAsSmi(LoadDescriptor::SlotRegister(), 1);
...
void BaselineCompiler::VisitStaNamedPropertyNoFeedback() {
  ...
  PushConstant<Name>(1);
  PushAccumulator();
  __ CallRuntime(Runtime::kSetNamedProperty);
}
...
void BaselineCompiler::VisitStaKeyedProperty() {
  ...
  __ CallBuiltin(Builtins::kKeyedStoreIC);
}
...
void BaselineCompiler::VisitStaDataPropertyInLiteral() {
  LoadContext(kContextRegister);
  PushRegister(0);    // object
  PushRegister(1);    // name
  PushAccumulator();  // value
  PushIntAsSmi(2);    // flags
  PushIndexAsSmi(3);  // slot
