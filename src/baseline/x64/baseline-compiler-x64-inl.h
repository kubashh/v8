...
void BaselineAssembler::Move(interpreter::Register output, Register source) {
  if (output.is_parameter()) {
    // Parameters aren't guaranteed to have the isolate root in the top 32 bits,
    // so we have to write the full slot.
    movq(RegisterFrameOperand(output), source);
  } else {
    // All other registers are pre-initialised with an isolate-rooted value, so
    // we don't need to write the full slot each time.
    mov_tagged(RegisterFrameOperand(output), source);
  }
}
