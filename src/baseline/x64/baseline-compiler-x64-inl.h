...
Register BaselineAssembler::ScratchRegisterScope::AcquireScratch() {
...
void BaselineAssembler::Decrement(Register) {
  ScratchRegisterScope scope(this);
  __ decl(scope.AcquireScratch());
}
...
void BaselineAssembler::Switch(Register reg, int case_value_base,
  ...
  __ leaq(scratch, MemOperand(&jump_table));
  __ jmp(MemOperand(scratch, reg, times_8, 0));
...
void BaselineCompiler::Prologue() {
  ...
  Register scratch = scope.AcquireScratch();
  __ Move(scratch, rsp);
  __ SubPointer(scratch, Immediate(bytecode_->frame_size()));
  __ ComparePointer(
      scratch, __ StackLimitAsOperand(StackLimitKind::kInterruptStackLimit));
...
void BaselineCompiler::PrologueFillFrame() {
  ...
  if (register_count < 2 * kLoopUnrollSize) {
    ...
    BaselineAssembler::ScratchRegisterScope scope(&masm_);
    Register scratch = scope.AcquireScratch();
    __ Move(scratch, Immediate(register_count / kLoopUnrollSize));
    Label loop;
    __ bind(&loop);
...
void BaselineAssembler::EmitReturn(MacroAssembler* masm) {
  ...
  __ Move(scratch, Operand(rbp, InterpreterFrameConstants::kFunctionOffset));
  __ LoadTaggedPointerField(
      scratch, FieldOperand(scratch, JSFunction::kFeedbackCellOffset));

  __ addl(FieldOperand(scratch, FeedbackCell::kInterruptBudgetOffset), weight);
