...
void BaselineCompiler::GenerateCode() {
  {
    ...
    for (int i = 0; i < table.NumberOfRangeEntries(); ++i) {
      EnsureLabels(table.GetRangeHandler(i))->MarkAsIndirectJumpTarget();
    }
...
void BaselineCompiler::VisitSingleBytecode() {
  ...
  bool needs_indirect_jump_target = FLAG_deopt_to_baseline;
  if (labels_[offset]) {
    ...
    if (labels_[offset]->indirect_jump_target) {
      needs_indirect_jump_target = true;
    }
  }

  if (needs_indirect_jump_target) {
    __ JumpTarget();
  }
