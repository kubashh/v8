...
void BaselineCompiler::BuildBinop(
  ...
  if (FLAG_sparkplug_inline_smi && fast_path) {
    BaselineAssembler::ScratchRegisterScope scratch_scope(&basm_);
    Register scratch = scratch_scope.AcquireScratch();
    __ MoveMaybeSmi(scratch, right);
    __ BitwiseOrSmi(scratch, left);
    __ JumpIfNotSmi(scratch, &builtin);

    instruction(left, right);
    if (check_overflow) __ JumpIf(Condition::kOverflow, &builtin_reload_left);
...
void BaselineCompiler::BuildBinopWithConstant(
  ...
  if (FLAG_sparkplug_inline_smi && fast_path) {
    ...
    if (check_overflow) {
      __ JumpIf(Condition::kOverflow, &builtin_restore_accumulator,
                Label::kNear);
    }
  ...
  if (FLAG_sparkplug_inline_smi) {
    ...
    SelectBooleanConstant(kInterpreterAccumulatorRegister,
                          [&](Label* is_true, Label::Distance distance) {
                            ...
                            __ JumpIf(condition, is_true, distance);
                          });
...
void BaselineCompiler::VisitTestEqual() {
  BuildCompare(Builtins::kEqual_WithFeedback, Condition::kEqual);
}
void BaselineCompiler::VisitTestEqualStrict() {
  BuildCompare(Builtins::kStrictEqual_WithFeedback, Condition::kEqual);
}
void BaselineCompiler::VisitTestLessThan() {
  BuildCompare(Builtins::kLessThan_WithFeedback, Condition::kLessThan);
}
void BaselineCompiler::VisitTestGreaterThan() {
  BuildCompare(Builtins::kGreaterThan_WithFeedback, Condition::kGreaterThan);
}
