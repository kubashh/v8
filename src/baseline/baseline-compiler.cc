...
void BaselineCompiler::GenerateCode() {
  {
    ...
    for (int i = 0; i < table.NumberOfRangeEntries(); ++i) {
      MarkAsIndirectJumpTarget(table.GetRangeHandler(i));
    }
...
void BaselineCompiler::VisitSingleBytecode() {
  ...
  if (labels_[offset].GetPointer()) __ Bind(labels_[offset].GetPointer());
  ...
  if (FLAG_deopt_to_baseline || IsIndirectJumpTarget(offset)) {
    __ JumpTarget();
  }
