...
void BaselineCompiler::LoadFeedbackVector(Register output) {
  ...
  if (__ emit_debug_code()) {
    BaselineAssembler::ScratchRegisterScope scope(&masm_);
...
void BaselineCompiler::TraceBytecode(Runtime::FunctionId function_id) {
  SaveAccumulatorScope accumulator_scope(&masm_);
  CallRuntime(
...
void BaselineCompiler::VisitStaModuleVariable() {
  ...
  if (cell_index > 0) {
    ...
    cell_index -= 1;
    __ LoadFixedArrayElement(kScratchRegister, kScratchRegister, cell_index);
    SaveAccumulatorScope save_accumulator(&masm_);
...
void BaselineCompiler::UpdateFeedback(int operand_index, int bit, Label* done) {
  ...
  __ TestAndBranchIfAnySet(kScratchRegister, bit, done);
  __ BitwiseOrInt(kScratchRegister, Immediate(bit));
...
void BaselineCompiler::BuildBinopWithConstant(
  ...
  if (fast_path) {
    ...
    if (check_overflow) {
      __ MoveSmi(rcx, kInterpreterAccumulatorRegister);
    }
    ...
    if (check_overflow) {
      ...
      __ MoveSmi(kInterpreterAccumulatorRegister, rcx);
    }
    __ bind(&builtin);
  }
...
void BaselineCompiler::BuildCompare(Builtins::Name builtin_name,
  ...
  // Fast path for Smi.
  Label builtin, done;
  __ MoveMaybeSmi(rcx, kScratchRegister);
  __ BitwiseOrSmi(rcx, kInterpreterAccumulatorRegister);
  __ JumpIfNotSmi(rcx, &builtin);
...
void BaselineCompiler::VisitTestUndetectable() {
  ...
  __ TestAndBranchIfAllClear(kScratchRegister,
...
void BaselineCompiler::VisitSetPendingMessage() {
  ...
  __ Move(rcx, kInterpreterAccumulatorRegister);
  __ Move(kInterpreterAccumulatorRegister, MemOperand(kScratchRegister, 0));
  __ Move(MemOperand(kScratchRegister, 0), rcx);
}
