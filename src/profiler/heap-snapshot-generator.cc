...
void HeapObjectsMap::RemoveDeadEntries() {
  ...
  std::unordered_map<size_t, NativeObject> reverse_merged_native_entries_map_;
  for (const auto& it : merged_native_entries_map_) {
    auto result =
        reverse_merged_native_entries_map_.insert({it.second, it.first});
  ...
  for (size_t i = 1; i < entries_.size(); ++i) {
    ...
    auto merged_reverse_it = reverse_merged_native_entries_map_.find(i);
    if (entry_info.accessed) {
      ...
      if (merged_reverse_it != reverse_merged_native_entries_map_.end()) {
