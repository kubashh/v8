...
uint32_t V8HeapExplorer::EstimateObjectsCount() {
  ...
  while (!it.Next().is_null() &&
         objects_count != std::numeric_limits<int32_t>::max())
    ++objects_count;
...
void HeapSnapshotGenerator::ProgressStep() {
  // Avoid reporting more progress done than
  // there is in total. When we are done,
  // we will report a progress_counter_ of
  // progress_total_, so take care of it there.
