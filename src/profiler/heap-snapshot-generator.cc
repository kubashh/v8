...
void V8HeapExplorer::ExtractJSObjectReferences(
  ...
  SetPropertyReference(obj, entry, ReadOnlyRoots(heap_).proto_string(),
                       iter.GetCurrent());
      ...
      if (!proto_or_map->IsTheHole(heap_->isolate())) {
        if (!proto_or_map->IsMap()) {
          SetPropertyReference(
              obj, entry, ReadOnlyRoots(heap_).prototype_string(), proto_or_map,
              nullptr, JSFunction::kPrototypeOrInitialMapOffset);
        } else {
          SetPropertyReference(obj, entry,
                               ReadOnlyRoots(heap_).prototype_string(),
                               js_fun->prototype());
...
bool V8HeapExplorer::IsEssentialObject(Object* object) {
  return object->IsHeapObject() && !object->IsOddball() &&
         object != ReadOnlyRoots(heap_).empty_byte_array() &&
         object != ReadOnlyRoots(heap_).empty_fixed_array() &&
         object != ReadOnlyRoots(heap_).empty_weak_fixed_array() &&
         object != ReadOnlyRoots(heap_).empty_descriptor_array() &&
         object != ReadOnlyRoots(heap_).fixed_array_map() &&
         object != ReadOnlyRoots(heap_).cell_map() &&
         object != ReadOnlyRoots(heap_).global_property_cell_map() &&
         object != ReadOnlyRoots(heap_).shared_function_info_map() &&
         object != ReadOnlyRoots(heap_).free_space_map() &&
         object != ReadOnlyRoots(heap_).one_pointer_filler_map() &&
         object != ReadOnlyRoots(heap_).two_pointer_filler_map();
}
