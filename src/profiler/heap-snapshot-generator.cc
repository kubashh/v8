...
class EmbedderGraphImpl : public EmbedderGraph {
 public:
  ...
  explicit EmbedderGraphImpl(DisallowHeapAllocation& promise) {}
  ~EmbedderGraphImpl() final {
  ...
  Node* V8Node(const v8::Local<v8::Value>& value) final {
    ...
    return new V8NodeImpl(*object);
  }

  void AddEdge(Node* from, Node* to) final {
    nodes_.insert(from);
    nodes_.insert(to);
    edges_.push_back({from, to});
  }

  const std::set<Node*>& nodes() { return nodes_; }
...
HeapEntry* NativeObjectsExplorer::EntryForEmbedderGraphNode(
  ...
  if (node->IsEmbedderNode()) {
    return filler_->FindOrAddEntry(node, embedder_graph_entries_allocator_);
  } else {
...
bool NativeObjectsExplorer::IterateAndExtractReferences(
  ...
  if (FLAG_heap_profiler_embedder_graph) {
    v8::HandleScope scope(reinterpret_cast<v8::Isolate*>(isolate_));
    DisallowHeapAllocation no_allocation;
    EmbedderGraphImpl graph(no_allocation);
    ...
    for (auto node : graph.nodes()) {
      if (node->IsRootNode()) {
        filler_->SetIndexedAutoIndexReference(HeapGraphEdge::kElement,
                                              snapshot_->root()->index(),
                                              EntryForEmbedderGraphNode(node));
      }
    }
