...
void CpuProfiler::StartProfiling(const char* title, bool record_samples,
  ...
  if (!context.is_null()) {
    ...
    if (is_profiling_) StopProcessor();
    context_filter = std::make_shared<ContextFilter>();
  }
  ...
  if (profiles_->StartProfiling(title, record_samples, mode, context_filter)) {
    ...
    if (context_filter) {
      // Since starting the processor may cause heap allocations, associate the
      // native context address with the processor after initialization.
      // TODO(acomminos): This shouldn't be necessary once we make the lifetime
      //                  of the CodeMap equal to that of the CpuProfiler.
      DisallowHeapAllocation no_gc;
      Handle<Context> handle = context.ToHandleChecked();
      Address context_address = handle->native_context().ptr();
      context_filter->SetNativeContextAddress(context_address);
    }
  }
}
