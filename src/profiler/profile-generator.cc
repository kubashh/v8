...
CpuProfile::CpuProfile(CpuProfiler* profiler, const char* title,
  ...
  if (!options_.impl()->filter_context.is_null()) {
    DisallowHeapAllocation no_gc;
    Handle<Context> filter_context =
        options_.impl()->filter_context.ToHandleChecked();
    context_filter_ = base::make_unique<ContextFilter>(
        filter_context->native_context().address());
  }
}
...
void CpuProfile::AddPath(base::TimeTicks timestamp,
  ...
  bool should_record_sample =
      ...
      (options_.max_samples() == v8::CpuProfilingOptions::kNoSampleLimit ||
