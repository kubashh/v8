...
CpuProfile::CpuProfile(CpuProfiler* profiler, const char* title,
  ...
  if (!options.filter_context().IsEmpty()) {
    ...
    Local<v8::Context> context_local =
        options.filter_context().ToLocalChecked();
    Address context_address =
        Utils::OpenHandle(*context_local)->native_context().ptr();
    context_filter_.reset(new ContextFilter(context_address));
  }
}
...
void CpuProfilesCollection::UpdateNativeContextAddressForCurrentProfiles(
  ...
  for (const std::unique_ptr<CpuProfile>& profile : current_profiles_) {
    if (auto* context_filter = profile->context_filter()) {
      ...
      context_filter->set_native_context_address(to);
    }
  }
  current_profiles_semaphore_.Signal();
}
