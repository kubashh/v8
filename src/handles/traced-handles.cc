...
namespace {
bool ComputeWeaknessForYoungObject(
...
void TracedHandlesImpl::ProcessYoungObjects(
  ...
  for (TracedNode* node : young_nodes_) {
    if (!node->is_in_use()) continue;
    DCHECK_IMPLIES(node->is_weak(), node->markbit<AccessMode::NON_ATOMIC>());
...
bool TracedHandles::IsWeak(Address* location,
  ...
  if (node->is_in_young_list()) {
    return ComputeWeaknessForYoungObject(embedder_root_handler, node,
