...
// static
bool TracedHandles::IsWeak(Address* location, Heap* heap,
                           WeaknessComputationMode weakness_computation_mode) {
  auto* node = TracedNode::FromLocation(location);
  DCHECK(node->is_in_use<AccessMode::ATOMIC>());
  if (weakness_computation_mode == WeaknessComputationMode::kRecompute) {
    auto* handler = heap->GetEmbedderRootsHandler();
