...
namespace {
template <TracedHandles::WeaknessComputationMode weakness_computation_mode>
bool ComputeWeaknessForYoungObject(EmbedderRootsHandler* handler,
                                   TracedNode* node) {
  bool is_unmodified_api_object;
  if constexpr (weakness_computation_mode ==
                TracedHandles::WeaknessComputationMode::kAtomic) {
    is_unmodified_api_object =
        JSObject::IsUnmodifiedApiObject(node->location());
  } else {
    is_unmodified_api_object =
        JSObject::IsUnmodifiedApiObjectThreadSafe(node->location());
  }
...
void TracedHandlesImpl::ComputeWeaknessForYoungObjects() {
  ...
  for (TracedNode* node : young_nodes_) {
    ...
    ComputeWeaknessForYoungObject<
        TracedHandles::WeaknessComputationMode::kAtomic>(handler, node);
  }
}
...
void TracedHandlesImpl::ProcessYoungObjects(
  ...
  DCHECK_IMPLIES(v8_flags.minor_ms ||
                 ...
                 !is_marking_);
  if ((garbage_collector == GarbageCollector::SCAVENGER) && is_marking_) return;
...
namespace {
std::pair<Tagged<Object>, bool> MarkObject(Tagged<Object> obj, TracedNode& node,
  ...
  bool was_marked = node.set_markbit<AccessMode::ATOMIC>();
  ...
  if (mark_mode == TracedHandles::MarkMode::kOnlyYoung &&
    ...
    return std::make_pair(Smi::zero(), was_marked);
  return std::make_pair(obj, was_marked);
}
}  // namespace

// static
std::pair<Tagged<Object>, bool> TracedHandles::Mark(Address* location,
                                                    MarkMode mark_mode) {
...
Tagged<Object> TracedHandles::MarkConservatively(
  ...
  return MarkObject(node.object(), node, mark_mode).first;
}

// static
bool TracedHandles::IsWeak(
    Address* location, EmbedderRootsHandler* embedder_root_handler,
    TracedHandles::WeaknessComputationMode weakness_computation_mode) {
  if (!embedder_root_handler) return false;
  auto* node = TracedNode::FromLocation(location);
  DCHECK(node->is_in_use<AccessMode::ATOMIC>());
  if (node->is_in_young_list<AccessMode::ATOMIC>()) {
    if (weakness_computation_mode == WeaknessComputationMode::kAtomic) {
      return ComputeWeaknessForYoungObject<WeaknessComputationMode::kAtomic>(
          embedder_root_handler, node);
    } else {
      return ComputeWeaknessForYoungObject<
          WeaknessComputationMode::kConcurrent>(embedder_root_handler, node);
    }
  }
  return false;
}
