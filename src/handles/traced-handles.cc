...
void TracedHandlesImpl::ComputeWeaknessForYoungObjects() {
  ...
  for (TracedNode* node : young_nodes_) {
    ...
    ComputeWeaknessForYoungObject(
        handler, node, TracedHandles::WeaknessComputationMode::kAtomic);
  }
}
...
void TracedHandlesImpl::ProcessYoungObjects(
  ...
  for (TracedNode* node : young_nodes_) {
    ...
    if (!is_minor_gc && !node->markbit<AccessMode::NON_ATOMIC>()) {
      // The node is not reachable so there's no need to reset it's associated
      // root since it will be destroyed anyway.
      continue;
    }
...
bool TracedHandles::IsWeak(Address* location,
  ...
  if (node->is_in_young_list()) {
    ComputeWeaknessForYoungObject(embedder_root_handler, node,
                                  weakness_computation_mode);
  }
  return node->is_weak();
}
