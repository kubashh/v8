...
TracedNodeBlock* TracedNodeBlock::Create(TracedHandles& traced_handles,
  ...
  auto* block = new TracedNodeBlock(traced_handles, overall_list, usable_list);
  return block;
}

// static
void TracedNodeBlock::Delete(TracedNodeBlock* block) { delete block; }
...
void TracedHandles::FreeNode(TracedNode* node) {
  ...
  if (V8_UNLIKELY(block.IsFull() && !usable_blocks_.Contains(&block))) {
  ...
  if (V8_UNLIKELY(block.IsEmpty())) {
    ...
    if (block_has_young_nodes) {
      CHECK(young_blocks_.Contains(&block));
    ...
    for (auto* node : *block) {
      ...
      if (should_reset) {
        ...
        if (ObjectInYoungGeneration(node->object())) {
          ...
          block->ClearYoung(node->index());
          if (!block->HasYoungNodes()) {
            CHECK(young_blocks_.Contains(block));
            young_blocks_.Remove(block);
          }
        }
