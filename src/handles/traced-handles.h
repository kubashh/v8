...
class TracedNode final {
 ...
 private:
  ...
  using IsInYoungList_UNUSED = IsInUse::Next<bool, 1>;
  using IsWeak = IsInYoungList_UNUSED::Next<bool, 1>;
...
class TracedNodeBlock final {
  ...
  struct YoungListNode {
    static auto* GetListNode(TracedNodeBlock* block) {
      return &block->young_list_node_;
    }
  };
 ...
 public:
  ...
  using YoungList = DoublyLinkedList<TracedNodeBlock, YoungListNode>;
  ...
  const void* nodes_end_address() const { return at(kCapacity); }
  ...
  bool IsYoung(TracedNode::IndexType index) const {
    return young_nodes_bits_[index / CHAR_BIT] & (index % CHAR_BIT);
  }
 ...
 private:
  ...
  YoungList::ListNode young_list_node_;
  ...
  uint8_t young_nodes_bits_[kCapacity / CHAR_BIT] = {0};
