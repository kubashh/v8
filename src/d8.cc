...
PerIsolateData::PerIsolateData(Isolate* isolate)
    : isolate_(isolate),
      realms_(nullptr),
      unhandled_rejects_(10, PromiseHash(isolate)) {
...
static std::string ExceptionToString(Isolate* isolate, Local<Value> exception,
  ...
  if (message.IsEmpty()) {
    ...
    snprintf(buffer, sizeof(buffer), "wasm-function[%d]:%d: %s\n",
...
void Shell::ReportException(Isolate* isolate, v8::TryCatch* try_catch) {
  ...
  std::string exception_string =
      ExceptionToString(isolate, try_catch->Exception(), try_catch->Message());
...
void Shell::ReportUnhandledRejections() {
  Isolate* isolate = Isolate::GetCurrent();
  PerIsolateData* idata = PerIsolateData::Get(isolate);
  if (!idata->unhandled_rejects_.size()) return;
  for (auto pair : idata->unhandled_rejects_) {
    std::string message = pair.second;
    printf("Unhandled Promise Rejection: %s\n", message.c_str());
  }
  idata->unhandled_rejects_.clear();

  // TODO(sbc): Perhaps we should have an option to exit with non-zero in this
  // case since ignoring unhandled rejections in tests can hide real issues.
}

void Shell::PromiseRejectHandler(v8::PromiseRejectMessage data) {
  Isolate* isolate = Isolate::GetCurrent();
  PromiseRejectEvent event = data.GetEvent();

  Local<Promise> promise = data.GetPromise();
  Persistent<Promise, CopyablePersistentTraits<Promise>> p(isolate, promise);
  PerIsolateData* idata = PerIsolateData::Get(isolate);

  if (event == v8::kPromiseHandlerAddedAfterReject) {
    idata->unhandled_rejects_.erase(p);
  } else if (event == v8::kPromiseRejectWithNoHandler) {
    Local<Value> exception = data.GetValue();
    Local<Message> message = Exception::CreateMessage(isolate, exception);
    idata->unhandled_rejects_.emplace(
        std::make_pair(p, ExceptionToString(isolate, exception, message)));
  }
}
