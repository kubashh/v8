...
class SnapshotImpl : public AllStatic {
 public:
  static v8::StartupData CreateSnapshotBlob(
      ...
      const SnapshotData* shareable_snapshot_in,
  ...
  static base::Vector<const byte> ExtractReadOnlyData(
      const v8::StartupData* data);
  static base::Vector<const byte> ExtractShareableData(
      const v8::StartupData* data);
  ...
  static const uint32_t kShareableOffsetOffset =
      kReadOnlyOffsetOffset + kUInt32Size;
  static const uint32_t kFirstContextOffsetOffset =
      kShareableOffsetOffset + kUInt32Size;
...
bool Snapshot::Initialize(Isolate* isolate) {
  ...
  base::Vector<const byte> shareable_data =
      SnapshotImpl::ExtractShareableData(blob);
  ...
  SnapshotData shareable_snapshot_data(MaybeDecompress(shareable_data));

  bool success = isolate->InitWithSnapshot(
      &startup_snapshot_data, &read_only_snapshot_data,
      &shareable_snapshot_data, ExtractRehashability(blob));
...
v8::StartupData Snapshot::Create(
  ...
  ShareableSerializer shareable_serializer(isolate, flags,
  ...
  can_be_rehashed = can_be_rehashed && shareable_serializer.can_be_rehashed();
  ...
  if (FLAG_serialization_statistics) {
    ...
    DCHECK_NE(shareable_serializer.TotalAllocationSize(), 0);
    ...
    PrintF("%10d bytes per isolate\n",
           read_only_serializer.TotalAllocationSize() +
               shareable_serializer.TotalAllocationSize() +
  ...
  v8::StartupData result = SnapshotImpl::CreateSnapshotBlob(
      &startup_snapshot, &read_only_snapshot, &shareable_snapshot,
...
v8::StartupData SnapshotImpl::CreateSnapshotBlob(
  ...
  const SnapshotData* shareable_snapshot;
  ...
  SnapshotData shareable_compressed(
      SnapshotCompression::Compress(shareable_snapshot_in));
  startup_snapshot = &startup_compressed;
  read_only_snapshot = &read_only_compressed;
  shareable_snapshot = &shareable_compressed;
  ...
  shareable_snapshot = shareable_snapshot_in;
  ...
  total_length += static_cast<uint32_t>(shareable_snapshot->RawData().length());
  ...
  // Shareable.
  SnapshotImpl::SetHeaderValue(data, SnapshotImpl::kShareableOffsetOffset,
                               payload_offset);
  payload_length = shareable_snapshot->RawData().length();
  CopyBytes(
      data + payload_offset,
      reinterpret_cast<const char*>(shareable_snapshot->RawData().begin()),
      payload_length);
  if (FLAG_serialization_statistics) {
    PrintF("%10d bytes for shareable\n", payload_length);
  }
  payload_offset += payload_length;
...
base::Vector<const byte> SnapshotImpl::ExtractReadOnlyData(
  ...
  return ExtractData(data, GetHeaderValue(data, kReadOnlyOffsetOffset),
                     GetHeaderValue(data, kShareableOffsetOffset));
}

base::Vector<const byte> SnapshotImpl::ExtractShareableData(
    const v8::StartupData* data) {
  DCHECK(Snapshot::SnapshotIsValid(data));

  return ExtractData(data, GetHeaderValue(data, kShareableOffsetOffset),
