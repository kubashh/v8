...
uint32_t SnapshotImpl::ExtractNumContexts(const v8::StartupData* data) {
  CHECK_LT(SnapshotImpl::kNumberOfContextsOffset, data->raw_size);
  uint32_t num_contexts =
      SnapshotImpl::GetHeaderValue(data, SnapshotImpl::kNumberOfContextsOffset);
  return num_contexts;
}
...
uint32_t SnapshotImpl::ExtractContextOffset(const v8::StartupData* data,
  ...
  uint32_t context_offset =
      SnapshotImpl::GetHeaderValue(data, ContextSnapshotOffsetOffset(index));
...
Vector<const byte> SnapshotImpl::ExtractStartupData(
  ...
  uint32_t num_contexts = SnapshotImpl::ExtractNumContexts(data);
  return ExtractData(
      data, StartupSnapshotOffset(num_contexts),
      SnapshotImpl::GetHeaderValue(data, SnapshotImpl::kReadOnlyOffsetOffset));
}
...
Vector<const byte> SnapshotImpl::ExtractReadOnlyData(
  ...
  return ExtractData(
      data,
      SnapshotImpl::GetHeaderValue(data, SnapshotImpl::kReadOnlyOffsetOffset),
      SnapshotImpl::GetHeaderValue(data, ContextSnapshotOffsetOffset(0)));
}
...
Vector<const byte> SnapshotImpl::ExtractContextData(const v8::StartupData* data,
  ...
  uint32_t context_offset = SnapshotImpl::ExtractContextOffset(data, index);
  uint32_t next_context_offset;
  if (index == num_contexts - 1) {
    next_context_offset = data->raw_size;
  } else {
    next_context_offset = SnapshotImpl::ExtractContextOffset(data, index + 1);
...
void SnapshotImpl::CheckVersion(const v8::StartupData* data) {
  ...
  CHECK_LT(SnapshotImpl::kVersionStringOffset + kVersionStringLength,
           static_cast<uint32_t>(data->raw_size));
  Version::GetString(Vector<char>(version, kVersionStringLength));
  if (strncmp(version, data->data + SnapshotImpl::kVersionStringOffset,
    ...
    FATAL(
        ...
        data->data + SnapshotImpl::kVersionStringOffset, data->raw_size,
        SnapshotImpl::ExtractNumContexts(data));
  }
}
