...
bool Snapshot::InitializeStage1(Isolate* isolate) {
  ...
  return DecompressSnapshotInternal(
      isolate, [](Isolate* isolate, SnapshotData* startup_snapshot_data,
...
bool Snapshot::Initialize(Isolate* isolate) {
  ...
  bool success = DecompressSnapshotInternal(
      isolate,
      [&bytes_read](Isolate* isolate, SnapshotData* startup_snapshot_data,
                    SnapshotData* read_only_snapshot_data,
                    SnapshotData* shared_heap_snapshot_data, size_t bytes) {
        bytes_read = bytes;
...
bool Snapshot::DecompressSnapshotInternal(
    Isolate* isolate,
  ...
  return initialization_function(
      isolate, &startup_snapshot_data, &read_only_snapshot_data,
      &shared_heap_snapshot_data,
...
#ifdef V8_STATIC_ROOTS
  const bool make_initial_two_stage_snapshot =
      (!i::Snapshot::DefaultSnapshotBlob() ||
       i::Snapshot::DefaultSnapshotBlob()->raw_size == 0);

  v8::StartupData stage1;
  if (make_initial_two_stage_snapshot) {
