...
class SnapshotWriter {
 public:
  ...
  void WriteEmbedded(i::EmbeddedBlob blob) const {
#ifdef V8_EMBEDDED_BUILTINS
    i::Vector<const i::byte> embedded_vector(blob.data, blob.size);
 ...
 private:
  ...
  static void WriteEmbeddedFileData(FILE* fp,
                                    const i::Vector<const i::byte>& blob) {
    // TODO(jgruber,v8:6666): Restructure this to create the following symbols
    // for each off-heap builtin:
    //
    // kBuiltins##Name##Start
    // kBuiltins##Name##End
    // kBuiltins##Name##Size
    //
    // And additionally kOffHeapCodeStart and kOffHeapCodeEnd to mark boundaries
    // of the entire region. This will give us symbolization of builtin stack
    // frames for free and we can skip the EmbeddedData logic. The downside is
    // that it'll become more difficult to switch between embedded blobs if that
...
int main(int argc, char** argv) {
...

  {
    ...
    v8::StartupData blob;
    {
      ...
      i::EmbeddedBlob embedded_blob = i::Snapshot::CreateEmbeddedBlob(isolate);
