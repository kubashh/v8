...
v8::StartupData Snapshot::CreateSnapshotBlob(
  ...
  result.data = data;
  result.raw_size = static_cast<int>(total_length);
...
EmbeddedData EmbeddedData::FromIsolate(Isolate* isolate) {
  ...
  for (int i = 0; i < Builtins::builtin_count; i++) {
    ...
    if (Builtins::IsOffHeapSafe(i)) {
      ...
      uint32_t offset = RoundUp<kCodeAlignment>(raw_data_size);
...
// static
EmbeddedBlob Snapshot::CreateEmbeddedBlob(Isolate* isolate) {
  DisallowHeapAllocation no_gc;
  EmbeddedData embedded_data = EmbeddedData::FromIsolate(isolate);
  return {embedded_data.data(), embedded_data.size()};
}
#endif
