...
bool Snapshot::HasContextSnapshot(Isolate* isolate, size_t index) {
  ...
  size_t num_contexts = ExtractNumContexts(blob);
...
v8::StartupData Snapshot::CreateSnapshotBlob(
  ...
  size_t num_contexts = context_snapshots.size();
  size_t startup_snapshot_offset = StartupSnapshotOffset(num_contexts);
  size_t total_length = startup_snapshot_offset;
  ...
  size_t payload_offset = StartupSnapshotOffset(num_contexts);
  size_t payload_length =
      static_cast<size_t>(startup_snapshot->RawData().length());
  memcpy(data + payload_offset, startup_snapshot->RawData().start(),
         payload_length);
  if (FLAG_profile_deserialization) {
    PrintF("Snapshot blob consists of:\n%10zu bytes for startup\n",
           payload_length);
  }
  ...
  for (size_t i = 0; i < num_contexts; i++) {
    ...
    payload_length = static_cast<size_t>(context_snapshot->RawData().length());
    memcpy(data + payload_offset, context_snapshot->RawData().start(),
           payload_length);
    if (FLAG_profile_deserialization) {
      PrintF("%10zu bytes for context #%zu\n", payload_length, i);
    }
    payload_offset += payload_length;
  }
...
size_t Snapshot::ExtractNumContexts(const v8::StartupData* data) {
  CHECK_LT(kNumberOfContextsOffset, data->raw_size);
  uint32_t num_contexts;
  memcpy(&num_contexts, data->data + kNumberOfContextsOffset, kUInt32Size);
  return num_contexts;
}

size_t Snapshot::ExtractContextOffset(const v8::StartupData* data,
                                      size_t index) {
  ...
  memcpy(&context_offset, data->data + ContextSnapshotOffsetOffset(index),
         kUInt32Size);
  CHECK_LT(context_offset, data->raw_size);
...
bool Snapshot::ExtractRehashability(const v8::StartupData* data) {
  ...
  int rehashability;
  memcpy(&rehashability, data->data + kRehashabilityOffset, kIntSize);
  return rehashability != 0;
}

Vector<const byte> Snapshot::ExtractStartupData(const v8::StartupData* data) {
  size_t num_contexts = ExtractNumContexts(data);
  size_t startup_offset = StartupSnapshotOffset(num_contexts);
  CHECK_LT(startup_offset, data->raw_size);

  size_t first_context_offset = ExtractContextOffset(data, 0);
  size_t startup_length = first_context_offset - startup_offset;
...
Vector<const byte> Snapshot::ExtractContextData(const v8::StartupData* data,
  ...
  size_t context_offset = ExtractContextOffset(data, index);
  // Calculate the length of the requested context by taking the offset of the
  // next context (or the end of the data), and subtracting the offset of this
  // context.
  size_t context_length;
  if (index == num_contexts - 1) {
    context_length = data->raw_size - context_offset;
  } else {
    size_t next_context_offset = ExtractContextOffset(data, index + 1);
    context_length = next_context_offset - context_offset;
  }
...
SnapshotData::SnapshotData(const Serializer* serializer) {
  ...
  // Calculate sizes.
  size_t reservation_size = reservations.size() * kInt32Size;
  size_t size = kHeaderSize + reservation_size + payload->size();
...
Vector<const byte> SnapshotData::Payload() const {
  ...
  size_t payload_length = GetHeaderValue(kPayloadLengthOffset);
  DCHECK_EQ(data_ + size_, payload + payload_length);
  return Vector<const byte>(payload, payload_length);
}
