...
class EmbeddedBuiltinsStatistics final {
 public:
  void Print() const {}

  void CollectFromBlob(const i::EmbeddedData* blob) {}

  /*
   *
   * size of the blob on disk (embedded.cc)
   * size of the blob in memory (i.e. total builtin size + metadata)
   * maybe stats about builtin sizes (max,min,avg,stddev,percentiles)
   */
 private:
};
}  // namespace
...
EmbeddedData EmbeddedData::FromIsolate(Isolate* isolate) {
  ...
  if (FLAG_embedded_builtins_statistics) d.PrintStatistics();
...
// static
void EmbeddedData::PrintStatisticsForTest() {
  int size = 0;
...
void EmbeddedData::PrintStatistics() const {
  DCHECK(FLAG_embedded_builtins_statistics);
  ...
  PrintF("Embedded builtins statistics\n");
  PrintF("----------------------------\n");
