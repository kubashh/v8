...
MaybeObject** Deserializer<AllocatorT>::ReadDataCase(
  ...
  if (where == kNewObject && how == kPlain && within == kStartOfObject) {
    ...
    if (how == kFromCode) {
      ...
      // If new_object is coming from the "where == kExternalReference" branch
      // above, the weak bit might already be set -> don't assert here that it's
      // not set.
      MaybeObject* new_maybe_object =
          reinterpret_cast<MaybeObject*>(new_object);
      if (allocator()->GetAndClearNextReferenceIsWeak()) {
