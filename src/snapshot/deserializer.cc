...
int Deserializer<IsolateT>::ReadAllocateJSDispatchEntry(
  ...
  // shared. This should only happen in certain testing modes (e.g. snapshot
  // stress mode) so isn't much of a problem in practice. If we wanted to avoid
  // that, we could serialize an entry ID (e.g. just the index) and then
  // maintain a map of these IDs to JSDispatchHandles during deserialization.
