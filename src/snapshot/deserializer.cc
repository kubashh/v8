...
void Deserializer<IsolateT>::PostProcessNewObject(Handle<Map> map,
  ...
  if (deserializing_user_code()) {
    if (InstanceTypeChecker::IsInternalizedString(instance_type)) {
      ...
      StringTableInsertionKey key(isolate(), string, true);
...
int Deserializer<IsolateT>::ReadSingleBytecodeData(byte data,
  ...
  switch (data) {
    ...
    // Find an object in the shareable object cache and write a pointer to it
    // to the current object.
    case kShareableObjectCache: {
      int cache_index = source_.GetInt();
      // TODO(leszeks): Could we use the address of the shareable_object_cache
      // entry as a Handle backing?
      HeapObject heap_object = HeapObject::cast(
          main_thread_isolate()->shareable_object_cache()->at(cache_index));
      DCHECK(
          ShareableSerializer::IsShareable(main_thread_isolate(), heap_object));
      return slot_accessor.Write(heap_object, GetAndResetNextReferenceType());
    }
