...
TSlot Deserializer::WriteExternalPointer(TSlot dest, Address value) {
  DCHECK(!allocator()->next_reference_is_weak());
  // TODO(saelo) this probably shouldn't need to initialize the slot as well?
#ifdef V8_HEAP_SANDBOX
  Address slot = isolate()->external_pointer_table().allocate();
  isolate()->external_pointer_table().set(slot, value);
  memcpy(dest.ToVoidPtr(), &slot, kExternalPointerSize);
#else
...
HeapObject Deserializer::PostProcessNewObject(HeapObject obj,
  ...
  if (obj.IsScript()) {
    ...
    string.allocate_external_pointer_entries(isolate());
    ...
    data_view.allocate_external_pointer_entries(isolate());
    ...
    if (typed_array.is_on_heap()) {
      ...
      typed_array.allocate_external_pointer_entries(isolate());
