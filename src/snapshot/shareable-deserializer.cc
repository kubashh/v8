...
void ShareableDeserializer::DeserializeIntoIsolate() {
  // Don't deserialize into client Isolates. If there are client Isolates, the
  // shareable object cache should already be populated.
  DCHECK_IMPLIES(isolate()->shared_isolate() != nullptr,
                 !isolate()->shareable_object_cache()->empty());
  if (isolate()->shared_isolate() != nullptr) return;
  DCHECK(isolate()->shareable_object_cache()->empty());
  HandleScope scope(isolate());

  IterateShareableObjectCache(isolate(), this);
...
void ShareableDeserializer::DeserializeStringTable() {
  // See ShareableSerializer::SerializeStringTable.
  ...
  for (int i = 0; i < string_table_size; ++i) {
    ...
    StringTableInsertionKey key(isolate(), string, false);
