...
EmbeddedData EmbeddedData::FromIsolate(Isolate* isolate) {
  ...
  const uint32_t blob_metadata_size = MetadataOffset() + MetadataTableSize();
...
void EmbeddedData::PrintStatistics() const {
  ...
  const int blob_metadata_size = static_cast<int>(
      EmbeddedBlobHashSize() + IsolateHashSize() + MetadataTableSize());
  ...
  PrintF("  Padding:                            %d\n",
         static_cast<int>(code_size() + metadata_size() - instruction_size -
                          blob_metadata_size));
