...
bool BuiltinAliasesOffHeapTrampolineRegister(Isolate* isolate, Code code) {
  DCHECK(Builtins::IsIsolateIndependent(code.builtin()));
  switch (Builtins::KindOf(code.builtin())) {
    case Builtins::CPP:
    case Builtins::TFC:
    case Builtins::TFH:
    case Builtins::TFJ:
  ...
  Callable callable = Builtins::CallableFor(isolate, code.builtin());
  ...
  for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast;
    ...
    while (!on_heap_it.done()) {
      ...
      off_heap_it.rinfo()->set_target_address(
          blob->InstructionStartOfBuiltin(target.builtin()),
          SKIP_WRITE_BARRIER);
