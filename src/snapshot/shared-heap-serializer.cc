...
// static
bool SharedHeapSerializer::ShouldBeInSharedHeap(Isolate* isolate,
                                                HeapObject obj) {
  if (obj.IsString()) {
...
void SharedHeapSerializer::FinalizeSerialization() {
  ...
  // which entries are added to the shared space object cache. Terminate the
  ...
  // Check that all serialized object are in shared space and not RO. RO objects
  ...
  for (auto it = it_scope.begin(); it != it_scope.end(); ++it) {
    ...
    CHECK(ShouldBeInSharedHeap(isolate(), obj));
...
bool SharedHeapSerializer::SerializeUsingSharedHeapObjectCache(
    SnapshotByteSink* sink, Handle<HeapObject> obj) {
  if (!ShouldBeInSharedHeap(isolate(), *obj)) return false;
...
void SharedHeapSerializer::SerializeObjectImpl(Handle<HeapObject> obj) {
  // Objects in the shared space cannot depend on per-Isolate roots but can
  // depend on RO roots since sharing objects requires sharing the RO space.
  DCHECK(ShouldBeInSharedHeap(isolate(), *obj) || ReadOnlyHeap::Contains(*obj));
