...
void Serializer::ObjectSerializer::Serialize() {
  ...
  if (object_.IsExternalString()) {
    ...
    return;
  } else if (!in_read_only_space(object_)) {
...
SnapshotSpace get_snapshot_space(HeapObject object) {
  if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL) {
    if (third_party_heap::Heap::InCodeSpace(object.address()))
      return SnapshotSpace::kCode;
    else if (in_read_only_space(object))
...
void Serializer::ObjectSerializer::SerializeObject() {
  ...
  Map map = object_.map();
  SnapshotSpace space = get_snapshot_space(object_);
