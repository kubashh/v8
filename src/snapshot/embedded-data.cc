...
Code* InstructionStream::TryLookupCode(Isolate* isolate, Address address) {
  ...
  if (address <
      d.InstructionStartOfBuiltin(MapEmbeddedIndexToBuiltinIndex(0))) {
    return nullptr;
  }
  ...
  int l = 0, r = EmbeddedData::kBuiltinCount;
  while (l < r) {
    const int mid = (l + r) / 2;
    const int builtin_index = MapEmbeddedIndexToBuiltinIndex(mid);
    Address start = d.InstructionStartOfBuiltin(builtin_index);
    Address end = start + d.PaddedInstructionSizeOfBuiltin(builtin_index);
...
void FinalizeEmbeddedCodeTargets(Isolate* isolate, EmbeddedData* blob) {
  ...
  for (int i = 0; i < EmbeddedData::kBuiltinCount; i++) {
...
EmbeddedData EmbeddedData::FromIsolate(Isolate* isolate) {
  ...
  for (int i = 0; i < EmbeddedData::kBuiltinCount; i++) {
    ...
    if (Builtins::IsIsolateIndependent(builtin_index)) {
      ...
      if (Builtins::IsWasmRuntimeStub(builtin_index) &&
  ...
  for (int i = 0; i < EmbeddedData::kBuiltinCount; i++) {
    ...
    if (!Builtins::IsIsolateIndependent(builtin_index)) continue;
    Code* code = builtins->builtin(builtin_index);
...
Address EmbeddedData::InstructionStartOfBuiltin(int i) const {
  ...
  const int embedded_index = MapBuiltinIndexToEmbeddedIndex(i);
  const struct Metadata* metadata = Metadata();
  const uint8_t* result =
      RawData() + metadata[embedded_index].instructions_offset;
...
uint32_t EmbeddedData::InstructionSizeOfBuiltin(int i) const {
  ...
  const int embedded_index = MapBuiltinIndexToEmbeddedIndex(i);
  const struct Metadata* metadata = Metadata();
  return metadata[embedded_index].instructions_length;
}
...
void EmbeddedData::PrintStatistics() const {
  ...
  constexpr int kCount = EmbeddedData::kBuiltinCount;
