...
void PlatformDependentEmbeddedFileWriter::DeclareSymbolGlobal(
    const char* name) {
  // TODO(jgruber): Investigate switching to .globl.
...
#else
  fprintf(fp_, ".global _%s\n", name);
#endif
}

void PlatformDependentEmbeddedFileWriter::DeclareSymbol(const char* name) {
  // TODO(jgruber): Investigate non-global declarations on windows.
  DeclareSymbolGlobal(name);
}
