...
void CodeSerializer::SerializeObject(HeapObject* obj, HowToCode how_to_code,
  ...
  if (ElideObject(obj)) {
    return SerializeObject(ReadOnlyRoots(isolate()).undefined_value(),
  ...
  if (obj->IsScript()) {
    ...
    script_obj->set_wrapper(ReadOnlyRoots(isolate()).undefined_value());
    ...
    if (context_data != ReadOnlyRoots(isolate()).undefined_value() &&
        context_data != ReadOnlyRoots(isolate()).uninitialized_symbol()) {
      script_obj->set_context_data(ReadOnlyRoots(isolate()).undefined_value());
    }
