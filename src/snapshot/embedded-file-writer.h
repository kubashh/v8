...
class EmbeddedFileWriter {
 public:
  ...
  void SetEmbeddedManifestFile(const char* embedded_manifest_src_path) {
    embedded_manifest_src_path_ = embedded_manifest_src_path;
  }
  ...
  void SetEmbeddedTarget(const char* embedded_target) {
    embedded_target_ = embedded_target;
    UpdateSymbols();
  }

  void WriteEmbedded(const i::EmbeddedData* blob) const {
    MaybeWriteEmbeddedFile(blob);
    MaybeWriteEmbeddedVariantFile();
  }
 ...
 private:
  ...
  void MaybeWriteEmbeddedVariantFile() const {
    if (embedded_manifest_src_path_ == nullptr) return;

    FILE* fp = GetFileDescriptorOrDie(embedded_manifest_src_path_);

    fprintf(fp, "// Autogenerated file. Do not edit.\n");
    fprintf(fp, "\n");
    fprintf(fp, "#include \"src/v8.h\"\n");
    fprintf(fp, "#include \"src/embedded-blob-variant.h\"\n");
    fprintf(fp, "\n");
    fprintf(fp, "extern \"C\" const uint8_t* %s;\n", embedded_blob_symbol_);
    fprintf(fp, "extern \"C\" uint32_t %s;\n", embedded_blob_size_symbol_);
    fprintf(fp, "\n");
    fprintf(fp, "namespace {\n");
    fprintf(fp, "  v8::internal::%sEmbeddedBlobVariant variant(\n",
            embedded_variant_);
    fprintf(fp, "      %s,\n", embedded_blob_symbol_);
    fprintf(fp, "      %s,\n", embedded_blob_size_symbol_);
    fprintf(fp, "      0x%x);\n", i::CpuFeatures::SupportedFeatures());
    fprintf(fp, "}  // namespace\n");
  ...
  void UpdateSymbols() {
    if (embedded_target_ == nullptr) {
      i::SNPrintF(i::Vector<char>(embedded_blob_symbol_),
                  "v8_%s_embedded_blob_", embedded_variant_);
      i::SNPrintF(i::Vector<char>(embedded_blob_data_symbol_),
                  "v8_%s_embedded_blob_data_", embedded_variant_);
      i::SNPrintF(i::Vector<char>(embedded_blob_size_symbol_),
                  "v8_%s_embedded_blob_size_", embedded_variant_);
    } else {
      i::SNPrintF(i::Vector<char>(embedded_blob_symbol_),
                  "v8_%s_%s_embedded_blob_", embedded_variant_,
                  embedded_target_);
      i::SNPrintF(i::Vector<char>(embedded_blob_data_symbol_),
                  "v8_%s_%s_embedded_blob_data_", embedded_variant_,
                  embedded_target_);
      i::SNPrintF(i::Vector<char>(embedded_blob_size_symbol_),
                  "v8_%s_%s_embedded_blob_size_", embedded_variant_,
                  embedded_target_);
    }
  }
  ...
  void WriteMetadataSection(PlatformDependentEmbeddedFileWriter* w,
    ...
    w->DeclareLabel(embedded_blob_data_symbol_);
  ...
  void WriteInstructionStreams(PlatformDependentEmbeddedFileWriter* w,
    ...
    for (int i = 0; i < i::Builtins::builtin_count; i++) {
      ...
      char builtin_symbol[kMaxSymbolLength];
  ...
  void WriteFileEpilogue(PlatformDependentEmbeddedFileWriter* w,
    ...
    w->SectionData();
    w->DeclarePointerToSymbol(embedded_blob_symbol_,
                              embedded_blob_data_symbol_);
    w->Newline();

    w->Comment("The size of the embedded blob in bytes.");
    w->SectionRoData();
    w->DeclareUint32(embedded_blob_size_symbol_, blob->size());
    w->Newline();
...
#undef V8_COMPILER_IS_MSVC
  ...
  const char* embedded_manifest_src_path_ = nullptr;
  const char* embedded_variant_ = kDefaultEmbeddedVariant;
  const char* embedded_target_ = nullptr;  // For multi-target snapshots.

  // Fairly arbitrary but should fit all symbol names.
  static constexpr int kMaxSymbolLength = 256;
  char embedded_blob_symbol_[kMaxSymbolLength] = "";
  char embedded_blob_data_symbol_[kMaxSymbolLength] = "";
  char embedded_blob_size_symbol_[kMaxSymbolLength] = "";
};
