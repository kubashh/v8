...
class ReadOnlyHeapImageSerializer {
 ...
 private:
  ...
  void WritePage(const ReadOnlyPage* page,
                 const std::vector<MemoryRegion>& unmapped_regions) {
    sink_->Put(Bytecode::kReadOnlyPage, "page begin");
    ...
    sink_->Put(Bytecode::kFinalizeReadOnlyPage, "page end");
  }

  void WriteSegment(const ReadOnlySegmentForSerialization* segment) {
    sink_->Put(Bytecode::kReadOnlySegment, "segment begin");
    ...
    if (!V8_STATIC_ROOTS_BOOL) {
      // Non-static roots builds need the relocation table:
