...
void Generate_DeoptimizationEntry(MacroAssembler* masm,
  ...
  __ mov(Operand(esp, 2 * kSystemPointerSize), esi);  // Bailout id.
  __ mov(Operand(esp, 3 * kSystemPointerSize), ecx);  // Code address or 0.
  __ mov(Operand(esp, 4 * kSystemPointerSize), edx);  // Fp-to-sp delta.

  masm->IndirectLoadExternalReference(
      eax, ExternalReference::isolate_address(isolate));
  __ mov(Operand(esp, 5 * kSystemPointerSize), eax);
  {
