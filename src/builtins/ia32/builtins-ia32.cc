...
void NewImplicitReceiver(MacroAssembler* masm) {
  // ----------- S t a t e -------------
  // -- eax : argument count
  // -- edi : constructor to call (checked to be a JSFunction)
...
void Builtins::Generate_InterpreterPushArgsThenFastConstructFunction(
  ...
  //  -- esp[0]  : return address
  //  -- esp[4]  : allocation site feedback (if available or undefined)
  //  -- esp[8]  : the new target (checked to be a JSFunction)
  ...
  // Number of the arguments minus receiver.
  __ lea(esi, Operand(eax, -1));
  // Find the address of the last argument.
  __ shl(esi, kSystemPointerSizeLog2);
  __ neg(esi);
  __ add(esi, ecx);
