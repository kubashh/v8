...
static void AdvanceBytecodeOffsetOrReturn(MacroAssembler* masm,
                                          ...
                                          Register scratch1, Label* if_return) {
  ...
  __ push(ebx);  // Save the root register.
...
// Bailout to the return label if this is a return bytecode.
  Label advance_to_next_bytecode, goto_if_return;
#define JUMP_IF_EQUAL(NAME)                                             \
  __ cmpb(bytecode,                                                     \
          Immediate(static_cast<int>(interpreter::Bytecode::k##NAME))); \
  __ j(equal, &goto_if_return);
  RETURN_BYTECODE_LIST(JUMP_IF_EQUAL)
#undef JUMP_IF_EQUAL
  __ jmp(&advance_to_next_bytecode, Label::kNear);

  __ bind(&goto_if_return);
  __ pop(ebx);  // Restore the root register.
  __ jmp(if_return);

  __ bind(&advance_to_next_bytecode);
