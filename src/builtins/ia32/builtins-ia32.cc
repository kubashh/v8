...
static void Generate_StackOverflowCheck(MacroAssembler* masm, Register num_args,
  ...
  __ mov(scratch, __ ExternalOperand(real_stack_limit, scratch));
  __ sub(scratch, esp);
...
static void Generate_JSEntryTrampolineHelper(MacroAssembler* masm,
...

  {
    ...
    __ mov(esi, __ ExternalOperand(context_address, scratch1));
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm) {
  ...
  __ cmpb(__ ExternalOperand(debug_hook, ecx), Immediate(0));
  __ j(not_equal, &prepare_step_in_if_stepping);

  // Flood function if we need to continue stepping in the suspended generator.
  ExternalReference debug_suspended_generator =
      ExternalReference::debug_suspended_generator_address(masm->isolate());
  __ cmp(edx, __ ExternalOperand(debug_suspended_generator, ecx));
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size,
  ...
  if (FLAG_debug_code) {
    ...
    __ cmp(edx, __ ExternalOperand(pending_exception_address, ecx));
  ...
  __ mov(esp, __ ExternalOperand(pending_handler_sp_address, esi));
  __ mov(ebp, __ ExternalOperand(pending_handler_fp_address, esi));
  __ mov(esi, __ ExternalOperand(pending_handler_context_address, esi));
  ...
  __ mov(edi, __ ExternalOperand(pending_handler_entrypoint_address, edi));
  __ jmp(edi);
}
