...
void BaseCollectionsAssembler::LoadKeyValue(
  ...
  BIND(&if_fast);
  {
    ...
    BIND(&if_smiorobjects);
    {
      ...
      BIND(&if_two);
      {
        TNode<FixedArray> elements_fa = CAST(elements);
        *key =
            LoadAndNormalizeFixedArrayElement(elements_fa, IntPtrConstant(0));
        *value =
            LoadAndNormalizeFixedArrayElement(elements_fa, IntPtrConstant(1));
        Goto(&exit);
      }
    }
    BIND(&if_doubles);
...
class CollectionsBuiltinsAssembler : public BaseCollectionsAssembler {
 ...
 protected:
  ...
  std::tuple<TNode<TableType>, TNode<IntPtrT>> Transition(
      TNode<TableType> const table, TNode<IntPtrT> const index,
      UpdateInTransition const& update_in_transition);
  template <typename IteratorType, typename TableType>
  std::tuple<TNode<TableType>, TNode<IntPtrT>> TransitionAndUpdate(
...
template <typename TableType>
std::tuple<TNode<TableType>, TNode<IntPtrT>>
...
CollectionsBuiltinsAssembler::NextSkipHoles(TNode<TableType> table,
  ...
  BIND(&done_loop);
  return {entry_key, entry_start_position, var_index.value()};
}
