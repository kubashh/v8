...
void CollectionsBuiltinsAssembler::AddToOrderedHashTable(
    ...
    const StoreAtEntryCallback<CollectionType>& store_existing_entry_callback) {
  ...
  BIND(&entry_found);
  {
    ...
    store_existing_entry_callback(table, entry_start_position_or_hash.value());
    Goto(&done);
  }
  ...
  BIND(&store_new_entry);
  {
    StoreOrderedHashTableNewEntry(
        ...
        number_of_buckets.value(), occupancy.value(), store_new_entry_callback);
    Goto(&done);
  }
...
TF_BUILTIN(MapPrototypeSet, CollectionsBuiltinsAssembler) {
  ...
  StoreAtEntryCallback<OrderedHashMap> store_existing_entry =
  ...
  AddToOrderedHashTable(table, key, grow, store_new_entry,
                        store_existing_entry);
  Return(receiver);
}
...
void CollectionsBuiltinsAssembler::StoreOrderedHashTableNewEntry(
  ...
  const TNode<IntPtrT> entry_start = IntPtrAdd(
      ...
      number_of_buckets);
  store_new_entry_callback(table, entry_start);
