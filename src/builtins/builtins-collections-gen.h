...
class CollectionsBuiltinsAssembler : public BaseCollectionsAssembler {
 ...
 protected:
  ...
  template <typename CollectionType>
  using GrowCallback = std::function<const TNode<CollectionType>()>;
  template <typename CollectionType>
  using StoreAtEntryCallback = std::function<void(
      const TNode<CollectionType> table, const TNode<IntPtrT> entry_start)>;
  template <typename CollectionType>
  void AddToOrderedHashTable(
      const TNode<CollectionType> table, const TNode<Object> key,
      const GrowCallback<CollectionType>& grow,
      const StoreAtEntryCallback<CollectionType>& store_new_entry_callback,
      const StoreAtEntryCallback<CollectionType>&
          store_existing_entry_callback);
