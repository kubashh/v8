...
void Builtins::Generate_BaselinePrologue(MacroAssembler* masm) {
  ...
  // r12 is callee saved... we're just overriding this here :/
  ...
  __ Push(descriptor.GetRegisterParameter(
      BaselinePrologueDescriptor::kJavaScriptCallArgCount));  // Actual argument
                                                              // count.
  ...
  __ bind(&has_optimized_code_or_marker);
  {
    // TODO(verwaest): Overwrite return address instead.
