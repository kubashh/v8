...
static void GetSharedFunctionInfoBytecode(MacroAssembler* masm,
                                          Register sfi_data,
                                          Register scratch1) {
  Label check_for_interpreter_data, done;

  __ CmpObjectType(sfi_data, BASELINE_DATA_TYPE, scratch1);
  __ j(not_equal, &check_for_interpreter_data, Label::kNear);

  __ LoadTaggedPointerField(sfi_data,
                            FieldOperand(sfi_data, BaselineData::kDataOffset));

  __ bind(&check_for_interpreter_data);
...
// static
void Builtins::Generate_BaselineHandleOptimizationMarker(MacroAssembler* masm) {
  Register feedback_vector = rbx;
  Register optimization_state = rcx;
  Label maybe_has_optimized_code;
  __ testl(
