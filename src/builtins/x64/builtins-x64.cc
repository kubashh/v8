...
static void MaybeTailCallOptimizedCodeSlot(
    MacroAssembler* masm, Register closure, Register feedback_vector,
    Register scratch1, Register scratch2, Register scratch3,
    Label* found_deoptimized_code, bool leave_on_call) {
...

  {
    ...
    // given label.
    // TODO(leszeks): Just do the evict runtime call here?
    __ testl(
    ...
    // Re-use the feedback_vector register as a scratch register.
    ReplaceClosureEntryWithOptimizedCode(masm, optimized_code_entry, closure,
                                         feedback_vector, scratch3, scratch2);
    if (leave_on_call) {
      __ leave();
    }
...

  {
    ...
    __ SmiCompare(optimized_code_entry, Smi::kZero);
    ...
    // Check 1 (compile optimized non-concurrent)
    {
      ...
      __ movp(FieldOperand(feedback_vector, kOptimizedCodeCellOffset),
              Immediate(0));
      if (leave_on_call) {
        __ leave();
      }
    ...
    __ bind(&check_in_optimization_queue);
    {
      ...
      __ CompareRoot(rsp, Heap::kStackLimitRootIndex);
      __ j(above_equal, &optimized_code_cell_is_clear);
      if (leave_on_call) {
        __ leave();
      }
      GenerateTailCallToReturnedCode(masm, Runtime::kTryInstallOptimizedCode);
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  // Clobbers rcx, r14 and r15.
  MaybeTailCallOptimizedCodeSlot(masm, closure, feedback_vector, rcx, r14, r15,
                                 &found_deoptimized_code, true);
  ...
  // Optimized code is marked for deopt, bailout to the EvictOptimizedCodeSlot
  // runtime function which will clear the feedback vector's optimized code
  // slot and return the function's code object.
