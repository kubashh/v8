...
<<<<<<< Updated upstream
  ...
  Label post_instantiation_deopt_entry, not_create_implicit_receiver;
=======
  __ PushRoot(RootIndex::kTheHoleValue);  // Implicit receiver again?
...
=======
  __ Call(BUILTIN_CODE(masm->isolate(), FastNewObject), RelocInfo::CODE_TARGET);
  __ movq(rcx, rax);  // Save it in rcx
  // Restore registers
  __ movq(rdx,
          Operand(rbp,
                  ConstructFrameConstants::kNewTargetOrImplicitReceiverOffset));
  __ movq(rdi, Operand(rbp, ConstructFrameConstants::kConstructorOffset));
  __ movq(rax, Operand(rbp, ConstructFrameConstants::kLengthOffset));
  __ SmiUntagUnsigned(rax);
  __ movq(rsi, Operand(rbp, ConstructFrameConstants::kContextOffset));

  // Patch implicit receiver
  __ movq(Operand(rsp, 0), rcx);
  // Patch second implicit receivier
  __ movq(Operand(rsp, rax, times_system_pointer_size, 0), rcx);
  // Recover context
  __ movq(rsi,
