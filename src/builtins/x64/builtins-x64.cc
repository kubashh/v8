...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  Label fast_baseline;
  __ LoadTaggedPointerField(
      kScratchRegister, FieldOperand(closure, JSFunction::kFeedbackCellOffset));
  __ LoadTaggedPointerField(
      kScratchRegister,
      FieldOperand(kScratchRegister, FeedbackCell::kValueOffset));
  __ CmpObjectType(kScratchRegister, FEEDBACK_VECTOR_TYPE, kScratchRegister);
  __ j(equal, &fast_baseline);
  GenerateTailCallToReturnedCode(masm, Runtime::kPrepareForBaseline);

  __ bind(&fast_baseline);
