...
TF_BUILTIN(ObjectGetOwnPropertyDescriptor, ObjectBuiltinsAssembler) {
  ...
  Node* obj = args.AtIndex(arg_index);
  Increment(&arg_index);
  Node* key = args.AtIndex(arg_index);
  ...
  ReturnIf(IsUndefined(desc), UndefinedConstant());
...
Node* ObjectBuiltinsAssembler::FromPropertyDescriptor(Node* context,
  ...
  BIND(&if_generic_desc);
  {
    ...
    Node* get = LoadObjectField(desc, FixedArray::OffsetOfElementAt(
                                          PropertyDescriptorObject::kGetIndex));
    AddToDictionaryIf(IsNotTheHole(get), properties, factory->get_string(), get,
                      &bailout);
    Node* set = LoadObjectField(desc, FixedArray::OffsetOfElementAt(
                                          PropertyDescriptorObject::kSetIndex));
