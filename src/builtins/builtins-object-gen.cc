...
TF_BUILTIN(ObjectProtoToString, ObjectBuiltinsAssembler) {
  Label checkstringtag(this), if_receiverisapiobject(this, Label::kDeferred),
      if_receiverisarguments(this), if_receiverisarray(this),
      if_receiverisboolean(this), if_receiverisdate(this),
      if_receiveriserror(this), if_receiverisfunction(this),
      if_receiverisnumber(this, Label::kDeferred), if_receiverisobject(this),
      if_receiverisprimitive(this), if_receiverisproxy(this, Label::kDeferred),
      if_receiverisregexp(this), if_receiverisstring(this),
      if_receiverissymbol(this, Label::kDeferred), if_receiverisvalue(this);
  ...
  GotoIf(TaggedIsSmi(receiver), &if_receiverisnumber);
  Node* receiver_map = LoadMap(receiver);
  Node* receiver_instance_type = LoadMapInstanceType(receiver_map);
  GotoIf(IsPrimitiveInstanceType(receiver_instance_type),
         &if_receiverisprimitive);
  const struct {
    InstanceType value;
    Label* label;
  } kJumpTable[] = {{JS_OBJECT_TYPE, &if_receiverisobject},
                    {JS_ARRAY_TYPE, &if_receiverisarray},
                    {JS_FUNCTION_TYPE, &if_receiverisfunction},
                    {JS_REGEXP_TYPE, &if_receiverisregexp},
                    {JS_ARGUMENTS_TYPE, &if_receiverisarguments},
                    {JS_DATE_TYPE, &if_receiverisdate},
                    {JS_BOUND_FUNCTION_TYPE, &if_receiverisfunction},
                    {JS_API_OBJECT_TYPE, &if_receiverisapiobject},
                    {JS_SPECIAL_API_OBJECT_TYPE, &if_receiverisapiobject},
                    {JS_PROXY_TYPE, &if_receiverisproxy},
                    {JS_ERROR_TYPE, &if_receiveriserror},
                    {JS_VALUE_TYPE, &if_receiverisvalue}};
  ...
  Switch(receiver_instance_type, &if_receiverisobject, case_values, case_labels,
         arraysize(case_values));

  BIND(&if_receiverisapiobject);
  {
  ...
  BIND(&if_receiverisprimitive);
  {
    Label return_null(this), return_undefined(this);

    GotoIf(IsStringInstanceType(receiver_instance_type), &if_receiverisstring);
    GotoIf(IsBooleanMap(receiver_map), &if_receiverisboolean);
    GotoIf(IsHeapNumberMap(receiver_map), &if_receiverisnumber);
    GotoIf(IsSymbolMap(receiver_map), &if_receiverissymbol);
  ...
  BIND(&if_receiverisvalue);
  {
    Node* receiver_value = LoadJSValueValue(receiver);
    GotoIf(TaggedIsSmi(receiver_value), &if_receiverisnumber);
    Node* receiver_value_map = LoadMap(receiver_value);
    GotoIf(IsHeapNumberMap(receiver_value_map), &if_receiverisnumber);
    GotoIf(IsBooleanMap(receiver_value_map), &if_receiverisboolean);
    Branch(IsSymbolMap(receiver_value_map), &if_receiverissymbol,
           &if_receiverisstring);
  }
  ...
  BIND(&checkstringtag);
  {
    ...
    BIND(&loop);
    {
      ...
      GotoIfNot(IsSetWord32<Map::HasNoInterestingSymbols>(holder_bit_field3),
                &return_generic);
