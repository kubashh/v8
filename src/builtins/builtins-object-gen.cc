...
TF_BUILTIN(ObjectAssign, ObjectBuiltinsAssembler) {
  ...
  args.ForEach(
      [=](Node* next_source_) {
        ...
        BIND(&slow);
        {
          Print("Runtime::kSetDataProperties");
...
TNode<Uint32T> ObjectBuiltinsAssembler::EnsureOnlyHasSimpleProperties(
  ...
  GotoIf(IsStringWrapperElementsKind(map), bailout);
  GotoIf(IsSpecialReceiverInstanceType(instance_type), bailout);
      ...
      [=, &var_stable](TNode<UintPtrT> descriptor_key_index) {
...

        {
          ...
          BIND(&if_found_dict);
          {
            ...
            LoadPropertyFromNameDictionary(dictionary, entry, &var_details,
                                           &var_value);
        ...
        BIND(&do_store);
        {
          KeyedStoreGenericGenerator::EmitAssignNamedProperty(
              state(), context, to, next_key, var_value.value());
