...
BUILTIN(AsyncContextVariablePrototypeRun) {
  ...
  if (!receiver->IsJSAsyncContextVariable()) {
  ...
  InternalIndex store_index = InternalIndex::NotFound();
  if (!snapshot->IsUndefined()) {
    ...
    store_index = async_context_store->FindEntry(isolate, *async_variable);
  }

  if (store_index.is_found()) {
    async_context_store->SetEntry(store_index, *async_variable, *value);
  } else {
    async_context_store =
...
BUILTIN(AsyncContextSnapshotPrototypeRun) {
  ...
  if (!receiver->IsJSAsyncContextSnapshot()) {
    THROW_NEW_ERROR_RETURN_FAILURE(
        isolate, NewTypeError(MessageTemplate::kIncompatibleMethodReceiver));
  }
