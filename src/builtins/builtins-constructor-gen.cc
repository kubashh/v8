...
TF_BUILTIN(ConstructBaseline, CallOrConstructBuiltinsAssembler) {
  ...
  // TODO(verwaest): Make sure CollectConstructFeedback knows we have a feedback
  // vector.
...
TF_BUILTIN(ConstructWithSpreadBaseline, CallOrConstructBuiltinsAssembler) {
  auto target = Parameter<Object>(Descriptor::kTarget);
  auto new_target = Parameter<Object>(Descriptor::kNewTarget);
  auto spread = Parameter<Object>(Descriptor::kSpread);
  auto args_count =
