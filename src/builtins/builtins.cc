...
const char* Builtins::Lookup(Address pc) {
  ...
  if (initialized_) {
    for (int i = 0; i < kBuiltinCount; i++) {
      Builtin builtin = Builtins::FromInt(i);
      if (isolate_->heap()->builtin(builtin).contains(isolate_, pc)) {
        return name(builtin);
      }
    }
  }
...
void Builtins::set_code(Builtin builtin, Code code) {
  DCHECK_EQ(builtin, code.builtin());
...
bool Builtins::IsBuiltin(const Code code) {
  return Builtins::IsBuiltinId(code.builtin());
}
...
bool Builtins::IsIsolateIndependentBuiltin(const Code code) {
  const Builtin builtin = code.builtin();
...
void Builtins::InitializeBuiltinEntryTable(Isolate* isolate) {
  ...
  for (int i = 0; i < kBuiltinCount; i++) {
    ...
    // resolved.
    Builtin builtin = Builtins::FromInt(i);
    CHECK(Builtins::IsBuiltinId(isolate->heap()->builtin(builtin).builtin()));
    DCHECK(isolate->heap()->builtin(builtin).is_off_heap_trampoline());
    builtin_entry_table[i] = d.InstructionStartOfBuiltin(builtin);
  }
}
