...
void CallOrConstructBuiltinsAssembler::CallOrConstructDoubleVarargs(
    Node* target, Node* new_target, Node* elements, Node* length, Node* kind,
  ...
  Node* new_elements = AllocateFixedArray(FAST_ELEMENTS, intptr_length);
...
void CallOrConstructBuiltinsAssembler::CallOrConstructWithSpread(
  ...
  BIND(&if_done);
  {
    ...
    BIND(&if_double);
    {
      CallOrConstructDoubleVarargs(target, new_target, elements, length, kind,
                                   args_count, context);
    }
  }
}
