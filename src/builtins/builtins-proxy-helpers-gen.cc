...
Node* ProxyAssembler::ProxyGetProperty(Node* context, Node* proxy, Node* name,
  ...
  Label throw_proxy_handler_revoked(this, Label::kDeferred),
      trap_undefined(this), no_target_desc(this),
  ...
  VARIABLE(target, MachineRepresentation::kTagged);
  target.Bind(LoadObjectField(proxy, JSProxy::kTargetOffset));

  // 6. Let trap be ? GetMethod(handler, "get").if_found_value
  ...
  CheckGetTrapResult(context, target.value(), proxy, name, trap_result,
                     &return_result, &no_target_desc);
  ...
  BIND(&no_target_desc);
  {
    result.Bind(CallRuntime(Runtime::kGetPropertyWithReceiver, context, proxy,
                            name, receiver));
    Goto(&done);
  }
