...
Node* ProxyHelpersCodeStubAssembler::ProxyGetProperty(Node* context,
  ...
  BIND(&loop);
  {
    ...
    result.Bind(GetPropertySingleRun(context, target.value(), name, receiver,
                                     &target, &traverse));
    Goto(&loop);
  }
...
Node* ProxyHelpersCodeStubAssembler::GetPropertySingleRun(
  ...
  Label done(this), if_found_value(this, Label::kDeferred), if_not_found(this);
  ...
  BIND(&trap_undefined_or_slow_call_runtime);
  {
    ...
    traverse->Bind(FalseConstant());
    result.Bind(CallRuntime(Runtime::kGetTargetProperty, context,
