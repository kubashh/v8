...
TF_BUILTIN(FastArrayExtract, HigherOrderArrayBuiltinCodeStubAssembler) {
  ...
  Node* new_elements =
      CopyFixedArray(LoadElements(array), begin, count, count,
                     CopyFixedArrayFlag::kCopyAllFixedArrays, SMI_PARAMETERS);

  Node* result =
      AllocateUninitializedJSArrayWithoutElements(array_map, count, nullptr);
