...
TF_BUILTIN(FastArraySlice, CodeStubAssembler) {
  ...
  // 3. Let len be ToLength(Get(O, "length")).
  // 4. ReturnIfAbrupt(len).
  Node* len = SmiUntag(LoadJSArrayLength(receiver));
#endif

  // 1. Let O be ToObject(this value).
  // 2. ReturnIfAbrupt(O).
  Node* o = CallBuiltin(Builtins::kToObject, context, receiver);

  // 3. Let len be ToLength(Get(O, "length")).
  // 4. ReturnIfAbrupt(len).
  Node* len = ToLength_Inline(
      context, GetProperty(context, o, isolate()->factory()->length_string()));
  ...
  k.Bind(NumberMax(NumberAdd(len, relative_start), NumberConstant(0)));
  Goto(&relative_start_done);
  BIND(&relative_start_positive);
  k.Bind(NumberMin(relative_start, len));
  Goto(&relative_start_done);
  ...
  final.Bind(
      NumberMax(NumberAdd(len, relative_end.value()), NumberConstant(0)));
  Goto(&relative_end_done);
  BIND(&relative_end_positive);
  final.Bind(NumberMin(relative_end.value(), len));
  Goto(&relative_end_done);
