...
class FastArraySliceCodeStubAssembler : public CodeStubAssembler {
 public:
  ...
  Node* HandleFastElementsSlice(Node* context, Node* array, Node* from,
    ...
    Label fast(this), check_double(this);
    ...
    GotoIf(IsElementsKindGreaterThan(elements_kind, HOLEY_ELEMENTS),
           &check_double);
    ...
    int max_fast_elements =
        ...
        kPointerSize;
    Label fixed_array_in_new_space(this);
    Branch(SmiAboveOrEqual(count, SmiConstant(max_fast_elements)), slow,
           &fixed_array_in_new_space);

    BIND(&fixed_array_in_new_space);
    Node* new_array;
    Node* to_elements;
    std::tie(new_array, to_elements) = AllocateUninitializedJSArrayWithElements(
        HOLEY_ELEMENTS, array_map, count, nullptr, count,
        CodeStubAssembler::SMI_PARAMETERS);

    Node* additional_source_offset =
        ElementOffsetFromIndex(from, HOLEY_ELEMENTS, SMI_PARAMETERS);

    Node* from_elements = LoadElements(array);
    BuildFastFixedArrayForEach(
        to_elements, HOLEY_ELEMENTS, SmiConstant(0), count,
        [this, from_elements, additional_source_offset](Node* to_elements,
                                                        Node* offset) {
          Node* combined_offset = IntPtrAdd(additional_source_offset, offset);
          Node* element =
              Load(MachineType::AnyTagged(), from_elements, combined_offset);
          StoreNoWriteBarrier(MachineRepresentation::kTagged, to_elements,
                              offset, element);
        },
        SMI_PARAMETERS);
    result.Bind(new_array);
    Goto(&done);

    BIND(&check_double);

    max_fast_elements =
        (kMaxRegularHeapObjectSize - FixedDoubleArray::kHeaderSize -
         JSArray::kSize - AllocationMemento::kSize) /
        kDoubleSize;
    Label fixed_double_array_in_new_space(this);
    Branch(SmiAboveOrEqual(count, SmiConstant(max_fast_elements)), slow,
           &fixed_double_array_in_new_space);
    ...
    additional_source_offset =
        ElementOffsetFromIndex(from, HOLEY_DOUBLE_ELEMENTS, SMI_PARAMETERS);

    from_elements = LoadElements(array);
    BuildFastFixedArrayForEach(
        to_elements, HOLEY_DOUBLE_ELEMENTS, SmiConstant(0), count,
        [this, from_elements, additional_source_offset](Node* to_elements,
                                                        Node* offset) {
          Node* combined_offset = IntPtrAdd(additional_source_offset, offset);
          if (Is64()) {
            Node* element =
                Load(MachineType::Int64(), from_elements, combined_offset);
            StoreNoWriteBarrier(MachineRepresentation::kWord64, to_elements,
                                offset, element);
          } else {
            Node* element =
                Load(MachineType::Int32(), from_elements, combined_offset);
            StoreNoWriteBarrier(MachineRepresentation::kWord32, to_elements,
                                offset, element);
            combined_offset =
                IntPtrAdd(combined_offset, IntPtrConstant(kPointerSize));
            offset = IntPtrAdd(offset, IntPtrConstant(kPointerSize));
            element =
                Load(MachineType::Int32(), from_elements, combined_offset);
            StoreNoWriteBarrier(MachineRepresentation::kWord32, to_elements,
                                offset, element);
...
TF_BUILTIN(FastArraySlice, FastArraySliceCodeStubAssembler) {
  ...
  Label clone(this);
  BranchIfFastJSArray(o.value(), context, FastJSArrayAccessMode::INBOUNDS_READ,
                      &clone, &length_done);

  BIND(&clone);
  Node* array_map = LoadMap(o.value());
  Node* elements_kind = LoadMapElementsKind(array_map);
  GotoIf(IsElementsKindGreaterThan(elements_kind, HOLEY_DOUBLE_ELEMENTS),
         &length_done);

  args.PopAndReturn(
      CallStub(CodeFactory::ArrayClone(isolate()), context, receiver));
  ...
  Label non_fast(this);
  Node* fast_result =
      HandleFastElementsSlice(context, o.value(), k.value(), count, &non_fast);
...
TF_BUILTIN(FastArrayClone, HigherOrderArrayBuiltinCodeStubAssembler) {
