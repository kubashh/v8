...
TF_BUILTIN(ArrayFrom, ArrayPopulatorAssembler) {
  ...
  // Use fast path if:
  // - there is only one argument |items|.
  // - the receiver is Array.
  Branch(Word32And(Word32Equal(argc, Int32Constant(1)),
                   WordEqual(array_function, receiver)),
         &fast_iterate, &normal_iterate);

  BIND(&fast_iterate);
  {
    IteratorBuiltinsAssembler iterator_assembler(state());

    iterator_assembler.FastIterableToList(context, items, &normal_iterate);
  }
