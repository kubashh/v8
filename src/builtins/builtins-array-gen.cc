...
TF_BUILTIN(ArrayPrototypeShift, CodeStubAssembler) {
  ...
  BIND(&fast);
  {
    ...
    BIND(&fast_elements_tagged);
    {
      ...
      Node* value = LoadFixedArrayElement(elements_fa, 0);
      BuildFastLoop(
          IntPtrConstant(0), new_length,
          [&](Node* index) {
            StoreFixedArrayElement(
                elements_fa, index,
                LoadFixedArrayElement(elements_fa,
                                      IntPtrAdd(index, IntPtrConstant(1))));
          },
          1, ParameterMode::INTPTR_PARAMETERS, IndexAdvanceMode::kPost);
      StoreFixedArrayElement(elements_fa, new_length, TheHoleConstant());
      GotoIf(WordEqual(value, TheHoleConstant()), &return_undefined);
      args.PopAndReturn(value);
    }
                    ...
                    [&](Node* index) {
                      StoreFixedArrayElement(
                          ...
                          LoadFixedArrayElement(
                              elements_fa, IntPtrAdd(index, IntPtrConstant(1))),
                          SKIP_WRITE_BARRIER);
                    },
