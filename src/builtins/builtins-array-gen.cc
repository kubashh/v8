...
TF_BUILTIN(FastArraySlice, CodeStubAssembler) {
  ...
  BranchIfFastJSArray(receiver, context,
                      CodeStubAssembler::FastJSArrayAccessMode::INBOUNDS_READ,
                      &fast_elements_kind, &slow);
  BIND(&fast_elements_kind);
  ...
  Node* target = LoadFromFrame(StandardFrameConstants::kFunctionOffset,
                               MachineType::TaggedPointer());
  TailCallStub(CodeFactory::ArraySlice(isolate()), context, target,
               UndefinedConstant(), argc);
#else
  Node* const native_context = LoadNativeContext(context);
  Node* const js_slice_function =
      LoadContextElement(native_context, Context::ARRAY_SLICE_INDEX);
  Node* const js_slice_code =
      LoadObjectField(js_slice_function, JSFunction::kCodeOffset);
  Node* const js_slice_context =
      LoadObjectField(js_slice_function, JSFunction::kContextOffset);
  TailCallStub(BuiltinDescriptor(isolate()), js_slice_code, js_slice_context,
               js_slice_function, UndefinedConstant(), argc);
#endif
}
