...
void ArrayIncludesIndexofAssembler::Generate(SearchVariant variant,
  ...
  Label if_smi(this), if_objects(this), if_smiorobjects(this),
  ...
  GotoIf(IsElementsKindLessThanOrEqual(elements_kind, HOLEY_SMI_ELEMENTS),
         &if_smi);
  GotoIf(ElementsKindEqual(elements_kind, Int32Constant(HOLEY_ELEMENTS)),
         &if_objects);
  ...
  {
    Callable callable = Builtins::CallableFor(
        isolate(), (variant == kIncludes) ? Builtin::kArrayIncludesObject
                                          : Builtin::kArrayIndexOfObject);
    TNode<Object> result = CallStub(callable, context, elements, search_element,
                                    array_length, SmiTag(index_var.value()));
    args.PopAndReturn(result);
  }
...
// Note that if |array_kind| is HOLEY_SMI_ELEMENTS, then the actual kind of the
// array can be either HOLEY_SMI_ELEMENTS or PACKED_SMI_ELEMENTS, and when it's
// HOLEY_ELEMENTS then the actual kind of the array if either HOLEY_ELEMENTS or
// PACKED_ELEMENTS. This parameter is used only to call the SIMD code, and we
// don't about HOLEYness, we just care about SMI_ELEMENTS vs ELEMENTS vs
// DOUBLE_ELEMENTS.
void ArrayIncludesIndexofAssembler::GenerateSmiOrObject(
  ...
  if (kCanVectorize &&
      (array_kind == PACKED_ELEMENTS || array_kind == HOLEY_ELEMENTS)) {
...
TF_BUILTIN(ArrayIncludesObject, ArrayIncludesIndexofAssembler) {
  auto context = Parameter<Context>(Descriptor::kContext);
...
TF_BUILTIN(ArrayIncludesSmi, ArrayIncludesIndexofAssembler) {
  ...
  GenerateSmiOrObject(kIncludes, context, elements, search_element,
                      array_length, from_index, HOLEY_SMI_ELEMENTS);
}
...
TF_BUILTIN(ArrayIndexOfObject, ArrayIncludesIndexofAssembler) {
  auto context = Parameter<Context>(Descriptor::kContext);
  auto elements = Parameter<FixedArray>(Descriptor::kElements);
  auto search_element = Parameter<Object>(Descriptor::kSearchElement);
  auto array_length = Parameter<Smi>(Descriptor::kLength);
