...
Node* ArrayBuiltinsAssembler::FindProcessor(Node* k_value, Node* k) {
  ...
  void ArrayBuiltinsAssembler::MapFirstHoleAction(Node* index) {
    TNode<Int32T> from_kind = LoadElementsKind(a_.value());
    ...
    VariableList list({&a_, &target_map}, zone());
    ...
    BIND(&smi_transition);
    TransitionElementsKind(a_.value(), target_map.value(), PACKED_SMI_ELEMENTS,
    ...
    TransitionElementsKind(a_.value(), target_map.value(),
                           PACKED_DOUBLE_ELEMENTS, HOLEY_DOUBLE_ELEMENTS, true,
                           &transition_in_runtime);
    Goto(&done);

    BIND(&object_transition);
    target_map = LoadJSArrayElementsMap(HOLEY_ELEMENTS, native_context);
    TransitionElementsKind(a_.value(), target_map.value(), PACKED_ELEMENTS,
                           HOLEY_ELEMENTS, true, &transition_in_runtime);
    Goto(&done);

    BIND(&transition_in_runtime);
    CallRuntime(Runtime::kTransitionElementsKind, context(), a_.value(),
                target_map.value());
    Goto(&done);
