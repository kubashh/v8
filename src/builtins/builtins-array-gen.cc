...
Node* ArrayBuiltinsAssembler::FindProcessor(Node* k_value, Node* k) {
  ...
  void ArrayBuiltinsAssembler::GenerateIteratingTypedArrayBuiltinBody(
    ...
    // TODO(delphick): len_ will always be a SMI, but will be treated as a
    // Number by the rest of the code leading to redundant code-generation.
    // We should either create len_smi_ or store the type information
    // explicitly.
