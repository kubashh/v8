...
BUILTIN(NumberFormatConstructor) {
  ...
  // 3. Perform ? InitializeNumberFormt(numberFormat, locales, options).
  ...
  if (new_target->IsUndefined() && is_instance_of) {
    ...
    MAYBE_RETURN(JSReceiver::CreateDataProperty(
                     ...
                     isolate->factory()->intl_fallback_symbol(), number_format,
                     kThrowOnError),
...
BUILTIN(NumberFormatPrototypeFormatNumber) {
  ...
  CHECK_RECEIVER(JSNumberFormat, format_holder, method);

  // 3. Let nf be ? UnwrapNumberFormat(nf).
  Handle<Object> bound_format =
      Handle<Object>(format_holder->bound_format(), isolate);
  ...
  Handle<Context> context = isolate->factory()->NewBuiltinContext(
      native_context, JSNumberFormat::ContextSlot::kLength);

  // 4. b. Set F.[[NumberFormat]] to nf.
  context->set(JSNumberFormat::ContextSlot::kNumberFormat, *format_holder);
  ...
  format_holder->set_bound_format(*new_bound_format_function);
...
BUILTIN(NumberFormatInternalFormatNumber) {
  ...
  Handle<JSObject> number_format_holder = Handle<JSObject>(
      JSObject::cast(context->get(JSNumberFormat::ContextSlot::kNumberFormat)),
      isolate);
