...
BUILTIN(DurationFormatPrototypeFormat) {
  HandleScope scope(isolate);
  CHECK_RECEIVER(JSDurationFormat, format_holder,
                 "Intl.DurationFormat.prototype.format");
  Handle<Object> value_obj = args.atOrUndefined(isolate, 1);

  RETURN_RESULT_OR_FAILURE(
      isolate, JSDurationFormat::Format(isolate, value_obj, format_holder));
}
...
BUILTIN(DurationFormatPrototypeFormatToParts) {
  ...
  Handle<Object> value_obj = args.atOrUndefined(isolate, 1);
  RETURN_RESULT_OR_FAILURE(isolate, JSDurationFormat::FormatToParts(
                                        isolate, value_obj, format_holder));
}
