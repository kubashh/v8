...
BUILTIN(StringPrototypeNormalizeI18N) {
  ...
  if (form_input->IsUndefined(isolate)) {
    ...
    if (String::Equals(form, factory->NewStringFromStaticChars("NFC"))) {
      form_name = "nfc";
      form_mode = UNORM2_COMPOSE;
    } else if (String::Equals(form, factory->NewStringFromStaticChars("NFD"))) {
      form_name = "nfc";
      form_mode = UNORM2_DECOMPOSE;
    } else if (String::Equals(form,
                              factory->NewStringFromStaticChars("NFKC"))) {
      form_name = "nfkc";
      form_mode = UNORM2_COMPOSE;
    } else if (String::Equals(form,
                              factory->NewStringFromStaticChars("NFKD"))) {
      form_name = "nfkc";
      form_mode = UNORM2_DECOMPOSE;
    } else {
