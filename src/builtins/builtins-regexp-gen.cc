...
TNode<HeapObject> RegExpBuiltinsAssembler::RegExpExecInternal(
  ...
  Label if_success(this), if_exception(this, Label::kDeferred);
  {
    ...
    // Argument 5: Number of capture registers. This used to be set to zero to
    // force global regexp to behave as non-global. Setting it to the number of
    // capture registers should have the same effect and the actual value is
    ...
    // Argument 7: Indicate that this is a direct call from JavaScript. This
    // argument is ignored in the interpreter.
...
void RegExpBuiltinsAssembler::BranchIfFastRegExp(
  ...
  TNode<Map> initial_map = CAST(
      LoadObjectField(regexp_fun, JSFunction::kPrototypeOrInitialMapOffset));
  TNode<BoolT> has_initialmap = WordEqual(map, initial_map);
