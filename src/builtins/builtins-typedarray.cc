...
TF_BUILTIN(TypedArrayInitialize, TypedArrayBuiltinsAssembler) {
  ...
  Bind(&buffer_not_null);
  {
    // CSA_ASSERT(this, IsNull(maybe_buffer));
    Print("Buffer not null", maybe_buffer);
    Goto(&check_alignment);
  }
