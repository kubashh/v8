...
void TypedArrayBuiltinsAssembler::LoadMapAndElementsSize(Node* array,
  ...
  ExternalArrayType types[] = {
      kExternalUint8Array,  kExternalUint8ClampedArray, kExternalInt8Array,
      kExternalUint16Array, kExternalInt16Array,        kExternalUint32Array,
      kExternalInt32Array,  kExternalFloat32Array,      kExternalFloat64Array};

  const size_t kTypedElementsKindCount = LAST_FIXED_TYPED_ARRAY_ELEMENTS_KIND -
                                         FIRST_FIXED_TYPED_ARRAY_ELEMENTS_KIND +
                                         1;
  DCHECK_EQ(kTypedElementsKindCount, arraysize(types));
  ...
  for (size_t i = 0; i < kTypedElementsKindCount; i++) {
    Bind(elements_kind_labels[i]);
    {
      Handle<Map> map(isolate()->heap()->MapForFixedTypedArray(types[i]));
      typed_map->Bind(HeapConstant(map));
      size->Bind(SmiConstant(
          isolate()->factory()->GetExternalArrayElementSize(types[i])));
      Goto(&done);
    }
  }
