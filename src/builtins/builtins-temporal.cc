...
#define TEMPORAL_TO_STRING(T)                                  \
  BUILTIN(Temporal##T##PrototypeToString) {                    \
    ...
    return *JSTemporal##T::ToString(isolate, t, method);       \
  }
...
#define TEMPORAL_ZONED_DATE_TIME_GET_PREPARE(M)                               \
  ...
  ASSIGN_RETURN_FAILURE_ON_EXCEPTION(                                         \
      ...
      temporal::BuiltinTimeZoneGetPlainDateTimeFor(isolate, time_zone,        \
                                                   instant, calendar));
...
TEMPORAL_ZONED_DATE_TIME_GET_INT_BY_FORWARD_TIME_ZONE(Nanosecond,
                                                      iso_nanosecond)
TEMPORAL_GET_AFTER_DIVID(ZonedDateTime, EpochSeconds, nanoseconds, 1000000000,
                         epochSeconds)
TEMPORAL_GET_AFTER_DIVID(ZonedDateTime, EpochMilliseconds, nanoseconds, 1000000,
                         epochMilliseconds)
