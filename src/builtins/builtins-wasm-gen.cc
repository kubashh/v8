...
class WasmBuiltinsAssembler : public CodeStubAssembler {
 ...
 protected:
  ...
  TNode<HeapNumber> CreateHeapNumber() {
    TNode<Code> target = LoadBuiltinFromFrame(Builtins::kAllocateHeapNumber);
    return UncheckedCast<HeapNumber>(
        CallStub(AllocateHeapNumberDescriptor(), target, NoContextConstant()));
  }

  TNode<HeapNumber> ToHeapNumber(TNode<Uint32T> value) {
    TNode<HeapNumber> heap_value = CreateHeapNumber();
    StoreHeapNumberValue(heap_value, ChangeUint32ToFloat64(value));
    return heap_value;
  }

  TNode<HeapNumber> ToHeapNumber(TNode<Int32T> value) {
    TNode<HeapNumber> heap_value = CreateHeapNumber();
    StoreHeapNumberValue(heap_value, ChangeInt32ToFloat64(value));
    return heap_value;
  }

  TNode<HeapNumber> ToHeapNumber(TNode<Float64T> value) {
    TNode<HeapNumber> heap_value = CreateHeapNumber();
    StoreHeapNumberValue(heap_value, value);
    return heap_value;
  }
};
...
TF_BUILTIN(WasmAtomicWake, WasmBuiltinsAssembler) {
  ...
  TNode<HeapNumber> address =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kAddress)));
  TNode<HeapNumber> count =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kCount)));
...
TF_BUILTIN(WasmI64AtomicWait, WasmBuiltinsAssembler) {
  ...
  TNode<HeapNumber> address =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kAddress)));
  TNode<HeapNumber> expected_value_high = ToHeapNumber(
      UncheckedCast<Uint32T>(Parameter(Descriptor::kExpectedValueHigh)));
  TNode<HeapNumber> expected_value_low = ToHeapNumber(
  ...
  TNode<Smi> result_smi = UncheckedCast<Smi>(CallRuntimeWithCEntry(
      ...
      address, expected_value_high, expected_value_low, timeout));
  ReturnRaw(SmiToInt32(result_smi));
}

TF_BUILTIN(WasmMemoryCopy, WasmBuiltinsAssembler) {
  TNode<HeapNumber> dest =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kDest)));
  TNode<HeapNumber> source =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kSource)));
  TNode<HeapNumber> size =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kSize)));

  TNode<Object> instance = LoadInstanceFromFrame();
  TNode<Code> centry = LoadCEntryFromInstance(instance);
  TailCallRuntimeWithCEntry(Runtime::kWasmMemoryCopy, centry,
                            NoContextConstant(), instance, dest, source, size);
}

TF_BUILTIN(WasmMemoryFill, WasmBuiltinsAssembler) {
  TNode<HeapNumber> dest =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kDest)));
  TNode<Smi> value =
      SmiFromInt32(UncheckedCast<Int32T>(Parameter(Descriptor::kValue)));
  TNode<HeapNumber> size =
      ToHeapNumber(UncheckedCast<Uint32T>(Parameter(Descriptor::kSize)));

  TNode<Object> instance = LoadInstanceFromFrame();
  TNode<Code> centry = LoadCEntryFromInstance(instance);
  TailCallRuntimeWithCEntry(Runtime::kWasmMemoryFill, centry,
                            NoContextConstant(), instance, dest, value, size);
}
