...
TF_BUILTIN(WasmNewBigInt_32, WasmBuiltinsAssembler) {
  if (Is32()) {
    TNode<Code> target = LoadBuiltinFromFrame(Builtins::kNewBigInt_32);
    Node* context = IntPtrConstant(0);  // null context

    TNode<Object> low = UncheckedParameter(Descriptor::kLow);
    TNode<Object> high = UncheckedParameter(Descriptor::kHigh);

    TailCallStub(WasmToJavaScriptTypeConversion_32Descriptor(), target, context,
                 low, high);
  } else {
    Unreachable();
  }
