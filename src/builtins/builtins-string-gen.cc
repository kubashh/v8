...
TNode<WordT> StringBuiltinsAssembler::DirectStringData(
    TNode<String> string, TNode<Word32T> string_instance_type) {
  // Compute the effective offset of the first character.
  TVARIABLE(WordT, var_data);
  ...
  BIND(&if_external);
  {
    ...
    var_data =
        LoadObjectField<WordT>(string, ExternalString::kResourceDataOffset);
    Goto(&if_join);
  }
...
void StringBuiltinsAssembler::StringEqual_Loop(
  ...
  // Compute the effective offset of the first character.
  TNode<WordT> lhs_data = DirectStringData(lhs, lhs_instance_type);
  TNode<WordT> rhs_data = DirectStringData(rhs, rhs_instance_type);
...
TNode<String> StringBuiltinsAssembler::SubString(TNode<String> string,
  ...
  BIND(&external_string);
  {
    ...
    var_result = AllocAndCopyStringCharacters<RawPtrT>(
