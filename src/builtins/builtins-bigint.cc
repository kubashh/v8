...
BUILTIN(BigIntConstructor_ConstructStub) {
  ...
  Handle<Object> bigint(isolate->heap()->undefined_value(), isolate);
  Handle<JSValue>::cast(result)->set_value(*bigint);
  return *result;
}
...
BUILTIN(BigIntPrototypeToLocaleString) {
  ...
  return *isolate->factory()->NewStringFromAsciiChecked("UNIMPLEMENTED");
}
