...
TF_BUILTIN(ShadowRealmGetWrappedValue, ShadowRealmBuiltinsAssembler) {
  ...
  BIND(&slow_wrap);
  {
    TNode<JSFunction> fn = CAST(LoadContextElement(
        context, Context::SHADOW_REALM_WRAPPED_FUNCTION_CREATE_FUN_INDEX));
    Return(CallBuiltin(Builtin::kShadowRealmWrappedFunctionCreate, context, fn,
                       UndefinedConstant(), JSParameterCount(2),
