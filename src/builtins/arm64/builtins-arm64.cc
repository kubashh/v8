...
void Builtins::Generate_BaselinePrologue(MacroAssembler* masm) {
  ...
  // We already entered the frame in the caller Prologue
  ...
  // TODO(leszeks): Consider guaranteeing that this call leaves `undefined` in
  // the accumulator register, to skip the load in the baseline code.
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  __ bind(&is_baseline);
  {
    ...
    // Read off the optimization state in the feedback vector.
    // TODO(leszeks): Is this worth doing here? Baseline code will check it
    // anyway...
    __ Ldr(optimization_state,
           FieldMemOperand(feedback_vector, FeedbackVector::kFlagsOffset));
