...
void Generate_JSBuiltinsConstructStubHelper(MacroAssembler* masm) {
  ...
  // Enter a construct frame.
  {
    ...
    constexpr int additional_slots = kArgcIncludesReceiver ? 1 : 2;
    ...
    __ Str(padreg,
           MemOperand(x2, kArgcIncludesReceiver ? 0 : kSystemPointerSize));
    ...
    // Copy arguments to the expression stack.
    {
      ...
      if (kArgcIncludesReceiver) {
        __ Sub(count, argc, kArgcReceiverSlots);
      } else {
        __ Mov(count, argc);
      }
  ...
  __ DropArguments(x1, kArgcIncludesReceiver
...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm) {
  ...
  if (kArgcIncludesReceiver) {
    ...
    __ Sub(argc_without_receiver, x12, kArgcReceiverSlots);
  }
  __ Bic(x10, x12, 1);
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm) {
  ...
  if (kArgcIncludesReceiver) {
    __ Sub(x10, x10, kArgcReceiverSlots);
  }
...
static void Generate_JSEntryTrampolineHelper(MacroAssembler* masm,
...

  {
    ...
    // padding.
    constexpr int additional_slots = kArgcIncludesReceiver ? 2 : 3;
    ...
    __ Str(function,
           MemOperand(scratch, kArgcIncludesReceiver ? 0 : kSystemPointerSize));
    ...
    if (kArgcIncludesReceiver) {
      __ Cmp(argc, kArgcReceiverSlots);
      __ B(eq, &done);
    } else {
      __ Cbz(argc, &done);
    }
...
static void GenerateInterpreterPushArgs(MacroAssembler* masm, Register num_args,
  ...
  if (kArgcIncludesReceiver && skip_receiver) {
    __ Sub(slots_to_copy, num_args, kArgcReceiverSlots);
  } else if (!kArgcIncludesReceiver && !skip_receiver) {
...
void Builtins::Generate_FunctionPrototypeApply(MacroAssembler* masm) {
  ...
  // present) instead.
  {
    ...
    __ Peek(receiver, 0);
    __ Cmp(argc, Immediate(kArgcReceiverSlots + 1));
    __ B(lt, &done);
  ...
  __ DropArguments(argc, kArgcIncludesReceiver
                             ? TurboAssembler::kCountIncludesReceiver
                             : TurboAssembler::kCountExcludesReceiver);
...
void Builtins::Generate_FunctionPrototypeCall(MacroAssembler* masm) {
  ...
  // That is if padding exists or not.
  {
    ...
    if (kArgcIncludesReceiver) {
      ...
      __ Sub(argc_without_receiver, argc, kArgcReceiverSlots);
    }
...
void Builtins::Generate_ReflectApply(MacroAssembler* masm) {
  ...
  // thisArgument (if present) instead.
  {
    ...
    __ Cmp(argc, Immediate(kArgcReceiverSlots + 3));
    __ B(lt, &done);
...
void Builtins::Generate_CallOrConstructForwardVarargs(MacroAssembler* masm,
  ...
  if (kArgcIncludesReceiver) {
    __ Subs(len, len, kArgcReceiverSlots);
  }
...
void Generate_PushBoundArguments(MacroAssembler* masm) {
  ...
  __ Cbz(bound_argc, &no_bound_arguments);
  {
    ...
    if (kArgcIncludesReceiver) {
      __ Sub(argc, argc, kArgcReceiverSlots);
    }
    ...
    // Update argc.
    if (kArgcIncludesReceiver) {
      __ Add(argc, total_argc, kArgcReceiverSlots);
    } else {
