...
TF_BUILTIN(CopyFastSmiOrObjectElements, CodeStubAssembler) {
  ...
  Node* elements = LoadObjectField(object, JSObject::kElementsOffset);
  ExtractFixedArrayFlags flags(ExtractFixedArrayFlag::kFixedArrays);
  flags |= ExtractFixedArrayFlag::kForceCOWCopy;
  Node* new_elements = CloneFixedArray(elements, flags);
