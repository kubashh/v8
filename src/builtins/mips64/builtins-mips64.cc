...
void Builtins::Generate_InterpreterForwardAllArgsThenConstruct(
  ...
  // Load the interpreted frame into a4.
...
void Builtins::Generate_ConstructForwardAllArgs(MacroAssembler* masm) {
  ...
  // Load the current frame pointer into r4.
  __ Move(a4, fp);
  Generate_ConstructForwardAllArgsImpl(masm);
}
...
void Builtins::Generate_CallApiCallbackImpl(MacroAssembler* masm,
...

  {
    ...
    __ Sd(holder, ExitFrameStackSlotOperand(FCA::kImplicitArgsOffset));
    ...
    __ Sd(holder, ExitFrameStackSlotOperand(FCA::kValuesOffset));
