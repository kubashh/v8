...
TNode<UintPtrT> TypedArrayBuiltinsAssembler::CalculateExternalPointer(
  ...
  TNode<UintPtrT> external_pointer_orig = Unsigned(
      IntPtrAdd(backing_store, ChangeNonnegativeNumberToUintPtr(byte_offset)));

#ifdef DEBUG
  TNode<Number> external_pointer_orig_num =
      ChangeUintPtrToTagged(external_pointer_orig);
  TNode<Number> backing_store_num = ChangeUintPtrToTagged(backing_store);
  TNode<Number> external_pointer_num =
      NumberAdd(backing_store_num, byte_offset);
  CSA_ASSERT(this,
             IsNumberEqual(external_pointer_orig_num, external_pointer_num),
             external_pointer_orig_num, external_pointer_num);
#endif  // DEBUG

  return external_pointer_orig;
}
