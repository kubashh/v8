...
V8_WARN_UNUSED_RESULT Object* GenericArrayPush(Isolate* isolate,
  ...
  double len = raw_length_number->Number();
  if (arg_count > kMaxSafeInteger - len) {
  ...
  for (int i = 0; i < arg_count; ++i) {
    ...
    if (len <= JSArray::kMaxArrayIndex) {
      RETURN_FAILURE_ON_EXCEPTION(
          isolate, Object::SetElement(isolate, receiver, len, element,
                                      LanguageMode::kStrict));
    } else {
      bool success;
      LookupIterator it = LookupIterator::PropertyOrElement(
          isolate, Handle<Object>::cast(receiver),
          isolate->factory()->NewNumber(len), &success);
  ...
  Handle<Object> final_len = isolate->factory()->NewNumber(len);
  RETURN_FAILURE_ON_EXCEPTION(
      isolate,
      Object::SetProperty(receiver, isolate->factory()->length_string(),
                          final_len, LanguageMode::kStrict));
...
BUILTIN(ArrayPush) {
  ...
  if (!EnsureJSArrayWithWritableFastElements(isolate, object, &args, 1)) {
