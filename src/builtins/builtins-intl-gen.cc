...
class IntlBuiltinsAssembler : public CodeStubAssembler {
 public:
  ...
  void ToLowerCaseImpl(TNode<String> string, TNode<Int32T> argc,
                       TNode<Context> context, ToLowerCaseKind kind);
};

TF_BUILTIN(StringToLowerCaseIntl, IntlBuiltinsAssembler) {
  const auto string = Parameter<String>(Descriptor::kString);
  ToLowerCaseImpl(string, TNode<Int32T>() /*argc*/, TNode<Context>(),
...
TF_BUILTIN(StringPrototypeToLocaleLowerCase, IntlBuiltinsAssembler) {
  ...
  auto context = Parameter<Context>(Descriptor::kContext);
  TNode<String> string =
      ToThisString(context, receiver, "String.prototype.toLocaleLowerCase");
  ToLowerCaseImpl(string, argc, context, ToLowerCaseKind::kToLocaleLowerCase);
}

void IntlBuiltinsAssembler::ToLowerCaseImpl(TNode<String> string,
                                            TNode<Int32T> argc,
  ...
  if (kind == ToLowerCaseKind::kToLocaleLowerCase) {
    ...
    TailCallBuiltin(Builtin::kStringPrototypeToLocaleLowerCaseSlow, context,
                    target, UndefinedConstant(), argc);
  } else {
    DCHECK_EQ(kind, ToLowerCaseKind::kToLowerCase);
    TailCallRuntime(Runtime::kStringToLowerCaseIntl, NoContextConstant(),
                    string);
  }
}
