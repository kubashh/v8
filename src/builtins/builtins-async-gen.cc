...
Node* AsyncBuiltinsAssembler::TaggedIsGeneratorObject(Node* tagged_object) {
  Node* const tagged_is_not_smi = TaggedIsNotSmi(tagged_object);
  VARIABLE(var_phi, MachineRepresentation::kBit, tagged_is_not_smi);
  Label done(this), if_false(this);

  // If tagged is an Smi, return false
  GotoIfNot(tagged_is_not_smi, &done);

  Node* const instance_type = LoadInstanceType(tagged_object);

  // If instance_type < JS_GENERATOR_OBJECT_TYPE, return false
  Node* const maybe_generator = Int32GreaterThanOrEqual(
      instance_type, Int32Constant(JS_GENERATOR_OBJECT_TYPE));
  var_phi.Bind(maybe_generator);
  GotoIfNot(maybe_generator, &done);

  // Return instance_type <= JS_ASYNC_GENERATOR_OBJECT_TYPE
  var_phi.Bind(Int32LessThanOrEqual(
      instance_type, Int32Constant(JS_ASYNC_GENERATOR_OBJECT_TYPE)));
  Goto(&done);

  BIND(&done);
  return var_phi.value();
}
