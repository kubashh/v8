...
class AsyncGeneratorBuiltinsAssembler : public AsyncBuiltinsAssembler {
 public:
  ...
  Node* HasCatchHandlerForPC(Node* generator);
...
Node* AsyncGeneratorBuiltinsAssembler::HasCatchHandlerForPC(Node* generator) {
  Node* has_catch_handler = ExternalConstant(
      ExternalReference::async_generator_has_catch_handler_for_pc(isolate()));
  Node* result = CallCFunction1(MachineType::Uint8(), MachineType::AnyTagged(),
                                has_catch_handler, generator);

  return SelectBooleanConstant(Word32And(result, Int32Constant(0xFF)));
}
}  // namespace
...
TF_BUILTIN(AsyncGeneratorResumeNext, AsyncGeneratorBuiltinsAssembler) {
  ...
  BIND(&if_abrupt);
  {
    ...
    TailCallBuiltin(Builtins::kAsyncGeneratorReturn, context, generator,
                    next_value, HasCatchHandlerForPC(generator));
