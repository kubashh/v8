...
Node* AsyncGeneratorBuiltinsAssembler::HasCatchHandlerForPC(Node* generator,
  ...
  GotoIf(IsGeneratorStateClosed(state), &done);
  Node* const function =
  ...
  Node* is_interpreted_function =
      ExternalConstant(ExternalReference::is_interpreted_function(isolate()));
  Branch(CallCFunction1(MachineType::Bool(), MachineType::AnyTagged(),
                        is_interpreted_function, function),
