...
TF_BUILTIN(DatePrototypeToPrimitive, CodeStubAssembler) {
  ...
  StringEqual(CAST(hint), hint_length, number_string, number_string_length,
              &hint_is_number, &hint_is_not_number);

  BIND(&hint_is_not_number);
  {
    TNode<IntPtrT> default_string_length =
        LoadStringLengthAsWord(default_string);
    StringEqual(CAST(hint), hint_length, default_string, default_string_length,
                &hint_is_string, &hint_is_not_default_string);
  }

  BIND(&hint_is_not_default_string);
  {
    TNode<IntPtrT> string_string_length = LoadStringLengthAsWord(string_string);
    StringEqual(CAST(hint), hint_length, string_string, string_string_length,
                &hint_is_string, &hint_is_invalid);
  }
