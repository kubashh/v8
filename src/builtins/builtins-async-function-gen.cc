...
TF_BUILTIN(AsyncFunctionReject, AsyncFunctionBuiltinsAssembler) {
  ...
  TNode<Object> reason = Parameter(Descriptor::kReason);
...
TF_BUILTIN(AsyncFunctionAwaitRejectClosure, AsyncFunctionBuiltinsAssembler) {
  ...
  const TNode<Object> sentError = Parameter(Descriptor::kSentError);
...
TF_BUILTIN(AsyncFunctionAwaitResolveClosure, AsyncFunctionBuiltinsAssembler) {
  CSA_ASSERT_JS_ARGC_EQ(this, 1);
  const TNode<Object> sentValue = Parameter(Descriptor::kSentValue);
  const auto context = Parameter<Context>(Descriptor::kContext);
