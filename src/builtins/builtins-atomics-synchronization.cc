...
Handle<JSPromise> UnlockAsyncLockedMutexFromHandlerContext(Isolate* isolate) {
  ...
  Handle<Object> unlock_promise = Handle<Object>(
      context->get(JSAtomicsMutex::kUnlockPromiseAsyncContextSlot), isolate);
  Handle<Object> waiter_or_undefined(
      context->get(JSAtomicsMutex::kAsyncLockedWaiterSlot), isolate);

  MaybeHandle<AsyncLockedWaiterWrapper> maybe_waiter;
  if (IsAsyncLockedWaiterWrapper(*waiter_or_undefined)) {
    maybe_waiter = Handle<AsyncLockedWaiterWrapper>::cast(waiter_or_undefined);
  }
  ...
  js_mutex->UnlockAsyncLockedMutex(isolate, maybe_waiter);
...
BUILTIN(AtomicsMutexAsyncUnlockResolveHandler) {
  ...
  Handle<JSPromise> js_unlock_promise =
      UnlockAsyncLockedMutexFromHandlerContext(isolate);
...
BUILTIN(AtomicsConditionGetLock) {
  DCHECK(v8_flags.harmony_struct);
  HandleScope scope(isolate);
