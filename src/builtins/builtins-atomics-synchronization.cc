...
Handle<JSPromise> UnlockAsyncLockedMutexFromHandlerContext(Isolate* isolate) {
  ...
  Handle<Object> unlock_promise = Handle<Object>(
      context->get(JSAtomicsMutex::kLockAsyncPromiseAsyncContextSlot), isolate);
  Handle<Object> waiter_or_undefined(
      context->get(JSAtomicsMutex::kAsyncLockedWaiterAsyncContextSlot),
      isolate);

  MaybeHandle<JSExternalObject> maybe_waiter;
  if (IsJSExternalObject(*waiter_or_undefined)) {
    maybe_waiter = Handle<JSExternalObject>::cast(waiter_or_undefined);
  }
