...
class BigIntBuiltinsAssembler : public CodeStubAssembler {
 public:
  ...
  void CppTrimMutableBigInt(TNode<BigInt> result, TNode<IntPtrT> new_length,
    ...
    TNode<ExternalReference> mutable_big_int_make_immutable = ExternalConstant(
        ExternalReference::mutable_big_int_make_immutable_function());
    CallCFunction(mutable_big_int_make_immutable, MachineType::AnyTagged(),
  ...
  void CppAbsoluteAdd(TNode<BigInt> result, TNode<BigInt> x, TNode<BigInt> y) {
    TNode<ExternalReference> mutable_big_int_absolute_add = ExternalConstant(
        ExternalReference::mutable_big_int_absolute_add_function());
    CallCFunction(mutable_big_int_absolute_add, MachineType::AnyTagged(),
                  std::make_pair(MachineType::AnyTagged(), result),
                  std::make_pair(MachineType::AnyTagged(), x),
                  std::make_pair(MachineType::AnyTagged(), y));
  }

  void CppAbsoluteSub(TNode<BigInt> result, TNode<BigInt> x, TNode<BigInt> y) {
    TNode<ExternalReference> mutable_big_int_absolute_sub = ExternalConstant(
        ExternalReference::mutable_big_int_absolute_sub_function());
    CallCFunction(mutable_big_int_absolute_sub, MachineType::AnyTagged(),
                  std::make_pair(MachineType::AnyTagged(), result),
                  std::make_pair(MachineType::AnyTagged(), x),
                  std::make_pair(MachineType::AnyTagged(), y));
  }
