...
void Builtins::Generate_WasmLiftoffFrameSetup(MacroAssembler* masm) {
  ...
  __ mflr(scratch);
  __ push(scratch);
  // Vector register {r8} is part of kGpParamRegisters which will get restored
  // at the end of the following block which will overwrite the vector value.
  // We need a temp regsiter that isn't part of the param register list.
  Register tmp = r11;
  __ push(tmp);
  {
    ...
    __ mr(tmp, kReturnRegister0);  // Make sure to transfer to vector after
                                   // param registers are restored.
    // Saved parameters are restored at the end of this block.
  }
  __ mr(vector, tmp);  // {r8} can be safely used again.
  __ pop(tmp);
  __ pop(scratch);
  __ mtlr(scratch);
