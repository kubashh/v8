...
void Builtins::Generate_WasmLiftoffFrameSetup(MacroAssembler* masm) {
  ...
  __ LoadTaggedPointerField(
      ...
      r0);
  __ ShiftLeftU64(r0, func_index, Operand(kTaggedSizeLog2));
  __ AddS64(vector, vector, r0);
  __ LoadTaggedPointerField(
      vector, FieldMemOperand(vector, FixedArray::kHeaderSize), r0);
  __ JumpIfSmi(vector, &allocate_vector);
  __ bind(&done);

  // restore r11 now that ip can be safely used.
  __ mr(ip, vector);
  __ pop(vector);
