...
class ProxyAssembler : public CodeStubAssembler {
 ...
 private:
  void ProxyGetProperty(Node* context, Node* proxy, Node* name, Node* receiver,
                        Variable* result, Variable* target,
                        Label* continue_loop, Label* end);
  void CheckTrapResult(Node* context, Node* target, Node* proxy, Node* name,
                       Node* trap_result, Label* if_found_value,
                       Label* if_not_found, Label* slow_call_runtime);
};
