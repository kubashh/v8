...
TNode<JSFunction> ProxiesCodeStubAssembler::AllocateProxyRevokeFunction(
  ...
  std::optional<TNode<JSDispatchHandleT>> dispatch_entry;
#ifdef V8_ENABLE_LEAPTIERING
  dispatch_entry = LoadBuiltinDispatchHandle(Builtin::kProxyRevoke);
#endif
  return AllocateFunctionWithContext(revoke_info, dispatch_entry,
                                     proxy_context);
}
