...
BUILTIN(DataViewConstructor) {
  const char* const kMethodName = "DataView Construct";
  ...
  dv->set_buffer(*array_buffer);

  // 12. Set O.[[ByteLength]] to viewByteLength.
  dv->set_byte_length(view_byte_length);

  // 13. Set O.[[ByteOffset]] to offset.
  dv->set_byte_offset(view_byte_offset);
  dv->set_data_pointer(
      isolate,
