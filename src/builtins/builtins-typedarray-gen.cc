...
TF_BUILTIN(TypedArrayConstructByArrayLike, TypedArrayBuiltinsAssembler) {
  ...
  Bind(&fast_copy);
  {
    ...
    // TODO(petermarshall): assert sizes.
    // SmiMul returns a heap number if it overflows.
    Node* byte_length = IntPtrMul(SmiUntag(length), SmiUntag(element_size));
    // CSA_ASSERT(this, SmiLessThanOrEqual(length,
    // SmiDiv(FixedTypedArrayBase::kMaxLength));
    Node* memcpy =
        ExternalConstant(ExternalReference::libc_memcpy_function(isolate()));
    CallCFunction3(MachineType::AnyTagged(), MachineType::Pointer(),
                   MachineType::Pointer(), MachineType::UintPtr(), memcpy,
                   holder_data_ptr, source_data_ptr, byte_length);
    Return(UndefinedConstant());
  }
