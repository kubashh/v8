...
void LazyBuiltinsAssembler::CompileLazy(TNode<JSFunction> function) {
  ...
  TNode<Uint32T> code_flags = LoadObjectField<Uint32T>(sfi_code, Code::kFlagsOffset);
  TNode<Uint32T> code_kind = DecodeWord32<Code::KindField>(code_flags);
  TNode<BoolT> is_baseline = IsEqualInWord32<Code::KindField>(code_kind, CodeKind::SPARKPLUG);
  ...
  code = Select<Code>(
      IsFeedbackVector(feedback_cell_value),
      [=]() { return sfi_code; },
      [=]() {
