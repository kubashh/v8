...
void SwitchStackPointerAndSimulatorStackLimit(MacroAssembler* masm,
  ...
  if (masm->options().enable_simulator_code) {
    UseScratchRegisterScope temps(masm);
    temps.Exclude(ip);
    __ ldr(ip, MemOperand(jmpbuf, wasm::kJmpBufStackLimitOffset));
    __ ldr(sp, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
    __ svc(kAdjustStackLimits);
  } else {
    __ ldr(sp, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
  }
}

void FillJumpBuffer(MacroAssembler* masm, Register jmpbuf, Label* tgt,
                    Register tmp1) {
  __ mov(tmp1, sp);
  __ str(tmp1, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
  __ str(fp, MemOperand(jmpbuf, wasm::kJmpBufFpOffset));
  __ LoadStackLimit(tmp1, StackLimitKind::kRealStackLimit);
  __ str(tmp1, MemOperand(jmpbuf, wasm::kJmpBufStackLimitOffset));

  // Save the jump buffer register, we need it temporarily.
  __ Push(jmpbuf);

  Register tmp2 = jmpbuf;

  // Compute tgt label address in register.
  Label here;
  __ mov(tmp2, Operand(pc));
  __ nop();  // need to account for pc overrun on Arm.
  __ bind(&here);
  __ mov_label_offset(tmp1, &here);
  __ sub(tmp1, tmp2, tmp1);
  __ mov_label_offset(tmp2, tgt);
  __ add(tmp1, tmp1, tmp2);  // Add in the offset to get address of tgt..
  __ Pop(jmpbuf);            // Finally stash the address in the jump buffer.
  __ str(tmp1, MemOperand(jmpbuf, wasm::kJmpBufPcOffset));
}
...
void RestoreParentSuspender(MacroAssembler* masm, Register tmp1,
  ...
  if (v8_flags.debug_code) {
    ...
    __ SmiUntag(state);
    __ cmp(state, Operand(WasmSuspenderObject::kActive));
...
void ResetStackSwitchFrameStackSlots(MacroAssembler* masm) {
  ...
  __ Zero(MemOperand(fp, StackSwitchFrameConstants::kInstanceOffset));
}
...
void Generate_WasmResumeHelper(MacroAssembler* masm, wasm::OnResume on_resume) {
  ...
  __ SmiUntag(state);
  __ cmp(state, Operand(WasmSuspenderObject::kSuspended));
...
void SwitchSimulatorStackLimit(MacroAssembler* masm) {
  if (masm->options().enable_simulator_code) {
    UseScratchRegisterScope temps(masm);
    temps.Exclude(ip);
    __ LoadStackLimit(ip, StackLimitKind::kRealStackLimit);
    __ svc(kAdjustStackLimits, al);
  }
}
...
void SwitchToTheCentralStackIfNeeded(MacroAssembler* masm, Register argc_input,
  ...
  __ Move(kSwitchFlagRegister, Operand(0));
  __ Move(kOldSPRegister, sp);

  // Using r2-r4 as temporary registers, because they will be rewritten
  ...
  const Register& on_central_stack_flag = r2;
  __ Move(on_central_stack_flag, on_central_stack_flag_loc);
  __ ldrb(on_central_stack_flag, MemOperand(on_central_stack_flag));

  Label do_not_need_to_switch;
  __ cmp(on_central_stack_flag, Operand(0));
  __ b(ne, &do_not_need_to_switch);
  // Switch to central stack.

  static constexpr Register central_stack_sp = ip;  // Super transitory.
  ...
  __ Move(kSwitchFlagRegister, Operand(1));
...
// r5: pointer to builtin function  (C callee-saved)
  ...
  if (switch_to_central_stack) {
    SwitchToTheCentralStackIfNeeded(masm, argc_input, target_fun, argv_input);
  }
#endif  // V8_ENABLE_WEBASSEMBLY
