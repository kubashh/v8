...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  // r4 as a temporary register for the smi tagged value.
  __ Move(r4, Operand(Smi::FromInt(BytecodeArray::kHeaderSize - kHeapObjectTag +
                                   kNoBytecodeOffset)));
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm) {
  ...
  __ cmp(
      kInterpreterBytecodeOffsetRegister,
      Operand(BytecodeArray::kHeaderSize - kHeapObjectTag + kNoBytecodeOffset));
  // Only update if it was kNoBytecodeOffset.
  __ mov(kInterpreterBytecodeOffsetRegister,
         Operand(BytecodeArray::kHeaderSize - kHeapObjectTag), LeaveCC, eq);
