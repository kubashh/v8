...
static void GenerateTailCallOptimizedCodeSlot(MacroAssembler* masm,
  ...
  // -----------------------------------
  DCHECK(!AreAliased(r1, r3, optimized_code_entry));
...
static void GenerateOptimizeCode(MacroAssembler* masm, Register feedback_vector,
                                 Register optimized_code_entry,
                                 Label* fallthrough) {
  // ----------- S t a t e -------------
  //  -- r3 : new target (preserved for callee if needed, and caller)
  ...
  // -----------------------------------
  DCHECK(!AreAliased(feedback_vector, r1, r3, optimized_code_entry));
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  // optimization marker. Otherwise, interpret it as a weak reference to a code
  // object.
  ...
  // optimization marker, carry onto the MaybeTailCallOptimizedCodeSlot path.
  __ cmp(optimized_code_entry,
         Operand(Smi::FromEnum(OptimizationMarker::kNone)));
  __ b(ne, &maybe_optimized);
  ...
  GenerateOptimizeCode(masm, feedback_vector, optimized_code_entry,
                       &not_optimized);

  __ bind(&optimized_code_slot_is_weak_ref);
  // Optimized code slot is a weak reference.
  __ LoadWeakValue(optimized_code_entry, optimized_code_entry, &not_optimized);
  GenerateTailCallOptimizedCodeSlot(masm, optimized_code_entry, r6);
