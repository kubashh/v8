...
void SwitchStackPointerAndSimulatorStackLimit(MacroAssembler* masm,
  ...
  if (masm->options().enable_simulator_code) {
    __ ldr(tmp, MemOperand(jmpbuf, wasm::kJmpBufStackLimitOffset));
    __ ldr(sp, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
    __ svc(kAdjustStackLimits | tmp.code());
  } else {
    __ ldr(sp, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
  }
}
...
namespace {
void SwitchSimulatorStackLimit(MacroAssembler* masm, Register tmp) {
  if (masm->options().enable_simulator_code) {
...
void SwitchToTheCentralStackIfNeeded(MacroAssembler* masm,
  ...
  {
    Register tmp = r3;
    DCHECK(!AreAliased(tmp, argc_input, argv_input, target_input));

    SwitchSimulatorStackLimit(masm, tmp);
  }
