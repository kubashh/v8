...
void SwitchStackState(MacroAssembler* masm, Register jmpbuf, Register tmp,
  ...
  if (v8_flags.debug_code) {
    ...
    Label ok;
    __ JumpIfEqual(tmp, Operand(old_state), &ok);
    __ Trap();
    __ bind(&ok);
  }
...
void RestoreParentSuspender(MacroAssembler* masm, Register tmp1,
  ...
  if (v8_flags.debug_code) {
    ...
    __ SmiUntag(state);
    __ JumpIfEqual(state, Operand(WasmSuspenderObject::kActive),
                   &parent_inactive);
    __ Trap();
...
void Generate_WasmResumeHelper(MacroAssembler* masm, wasm::OnResume on_resume) {
  ...
  __ SmiUntag(state);
  __ JumpIfEqual(state, Operand(WasmSuspenderObject::kSuspended),
