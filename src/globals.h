...
enum FunctionKind : uint8_t {
  ...
  kDerivedConstructor = 1 << 6,
  kDefaultConstructor = 1 << 7,
  kDefaultBaseConstructor = kBaseConstructor | kDefaultConstructor,
  kDefaultDerivedConstructor = kDerivedConstructor | kDefaultConstructor,
  kClassConstructor =
...
#define CONCISE_FUNCTION_LIST(V)    \
  V(AsyncConciseMethod)             \
  V(AsyncConciseGeneratorMethod)    \
  V(ClassFieldsInitializerFunction) \
  V(ConciseGeneratorMethod)         \
  V(ConciseMethod)

#define ARROW_FUNCTION_LIST(V) \
  V(AsyncArrowFunction)        \
  V(ArrowFunction)

#define ASYNC_FUNCTION_LIST(V)   \
  V(AsyncArrowFunction)          \
  V(AsyncConciseMethod)          \
  V(AsyncConciseGeneratorMethod) \
...
#define GENERATOR_FUNCTION_LIST(V) \
  V(AsyncConciseGeneratorMethod)   \
  V(AsyncGeneratorFunction)        \
  V(ConciseGeneratorMethod)        \
  V(GeneratorFunction)

#define ASYNC_GENERATOR_FUNCTION_LIST(V) \
  V(AsyncConciseGeneratorMethod)         \
  V(AsyncGeneratorFunction)

#define DEFAULT_CONSTRUCTOR_FUNCTION_LIST(V) \
  V(ClassConstructor)                        \
  V(DefaultBaseConstructor)                  \
  V(DefaultConstructor)                      \
  V(DefaultDerivedConstructor)

#define BASE_CONSTRUCTOR_FUNCTION_LIST(V) \
  V(BaseConstructor)                      \
  V(ClassConstructor)                     \
  V(DefaultBaseConstructor)

#define DERIVED_CONSTRUCTOR_FUNCTION_LIST(V) \
  V(ClassConstructor)                        \
  V(DefaultDerivedConstructor)               \
  V(DerivedConstructor)

#define GETTER_FUNCTION_LIST(V) \
  V(AccessorFunction)           \
  V(GetterFunction)

#define SETTER_FUNCTION_LIST(V) \
  V(AccessorFunction)           \
  V(SetterFunction)

#define ACCESSOR_FUNCTION_LIST(V) \
  V(GetterFunction)               \
  V(SetterFunction)

inline bool IsValidFunctionKind(FunctionKind kind) {
  CHECK_LT(kind, FunctionKind::kLastFunctionKind);
  CHECK_GE(kind, FunctionKind::kNormalFunction);
  return kind != FunctionKind::kLastFunctionKind;
}

#define FUNCTION_KIND_CASE(V) \
  case k##V:                  \
    return true;

#define FUNCTION_KIND_ACCESSORS(V, K)    \
  inline bool Is##V(FunctionKind kind) { \
    DCHECK(IsValidFunctionKind(kind));   \
    switch (kind) {                      \
      K(FUNCTION_KIND_CASE)              \
      default:                           \
        return false;                    \
    }                                    \
  }

FUNCTION_KIND_ACCESSORS(ArrowFunction, ARROW_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(AsyncFunction, ASYNC_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(AsyncGeneratorFunction, ASYNC_GENERATOR_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(BaseConstructor, BASE_CONSTRUCTOR_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(ConciseMethod, CONCISE_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(DefaultConstructor, DEFAULT_CONSTRUCTOR_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(DerivedConstructor, DERIVED_CONSTRUCTOR_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(GeneratorFunction, GENERATOR_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(GetterFunction, GETTER_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(SetterFunction, SETTER_FUNCTION_LIST);
FUNCTION_KIND_ACCESSORS(AccessorFunction, ACCESSOR_FUNCTION_LIST);

#undef FUNCTION_KIND_CASE
#undef FUNCTION_KIND_ACCESSORS
