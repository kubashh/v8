  ...
  void DebugPrintOutputObject(Object obj, unsigned output_offset,
    ...
    if (trace_scope_ != nullptr) {
      ...
      if (IsSmi(obj)) {
        ...
        Object::ShortPrint(obj, trace_scope_->file());
      }
...
void Deoptimizer::TraceDeoptBegin(int optimization_id,
  ...
  if (IsJSFunction(function_)) {
    Object::ShortPrint(function_, file);
    PrintF(file, ", ");
  }
  Object::ShortPrint(compiled_code_, file);
  PrintF(file,
         ", opt id %d, "
#ifdef DEBUG
         "node id %d, "
...
void Deoptimizer::TraceMarkForDeoptimization(Isolate* isolate, Code code,
  ...
  if (v8_flags.trace_deopt) {
    ...
    Object::ShortPrint(code, scope.file());
    PrintF(scope.file(), " (");
    Object::ShortPrint(deopt_data->SharedFunctionInfo(), scope.file());
...
void Deoptimizer::TraceEvictFromOptimizedCodeCache(Isolate* isolate,
  ...
  PrintF(scope.file(),
         ...
         reason);
  Object::ShortPrint(sfi, scope.file());
  PrintF(scope.file(), "]\n");
}
  ...
  for (auto& materialization : values_to_materialize_) {
    ...
    if (verbose_tracing_enabled()) {
      PrintF(trace_scope()->file(),
             ...
             value->ptr());
      Object::ShortPrint(*value, trace_scope()->file());
      PrintF(trace_scope()->file(), "\n");
    }
