 ...
 public:
  ...
  Handle<FixedArray> GetElementsAsStackTraceFrameArray() {
    ...
    for (int i = 0; i < frame_count; ++i) {
      int code_offset = Smi::ToInt(elements_->Offset(i));
      Handle<AbstractCode> code(elements_->Code(i), isolate_);
      ...
      if (!elements_->IsAnyWasmFrame(i)) {
        MaybeHandle<StackTraceFrame> maybe_frame =
            StackFrameCacheHelper::LookupCachedFrame(isolate_, code,
                                                     code_offset);
        if (!maybe_frame.is_null()) {
          stack_trace->set(i, *maybe_frame.ToHandleChecked());
          continue;
        }
      }
      ...
      if (!elements_->IsAnyWasmFrame(i)) {
        StackFrameCacheHelper::CacheFrameAndUpdateCache(isolate_, code,
                                                        code_offset, frame);
      }
    }
    return stack_trace;
