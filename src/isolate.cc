    ...
    switch (frame->type()) {
      ...
      case StackFrame::JAVA_SCRIPT_BUILTIN_CONTINUATION_WITH_CATCH: {
        ...
        JavaScriptBuiltinContinuationWithCatchFrame* js_frame =
            static_cast<JavaScriptBuiltinContinuationWithCatchFrame*>(frame);
        ...
        *exception_argument = exception;
        ...
        // Reconstructor stack pointer: point to last stack slot.
        Address return_sp = frame->fp() - height;

        // int offset = static_cast<int>(frame->pc() - code->entry());
