...
void Isolate::SetEmbeddedBlob(const uint8_t* blob, uint32_t blob_size) {
  ...
  if (d.EmbeddedBlobHash() != d.CreateEmbeddedBlobHash()) {
    FATAL(
        "Embedded blob checksum verification failed. "
        "Did you set a breakpoint in a builtin?");
  }
#endif  // DEBUG
}
...
#undef ASSIGN_ELEMENT
  ...
  if (!IsolateIsCompatibleWithEmbeddedBlob(this)) {
    FATAL(
        "The Isolate is incompatible with the embedded blob. This usually "
        "indicates a custom snapshot generated with inconsistent flags.");
  }
#endif  // DEBUG
