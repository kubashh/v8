...
bool ToPropertyDescriptorFastPath(Isolate* isolate, Handle<JSReceiver> obj,
  ...
  for (int i = 0; i < map->NumberOfOwnDescriptors(); i++) {
    ...
    if (key == ReadOnlyRoots(heap).enumerable_string()) {
      desc->set_enumerable(value->BooleanValue(isolate));
    } else if (key == ReadOnlyRoots(heap).configurable_string()) {
      desc->set_configurable(value->BooleanValue(isolate));
    } else if (key == ReadOnlyRoots(heap).value_string()) {
      desc->set_value(value);
    } else if (key == ReadOnlyRoots(heap).writable_string()) {
      desc->set_writable(value->BooleanValue(isolate));
    } else if (key == ReadOnlyRoots(heap).get_string()) {
      // Bail out to slow path to throw an exception if necessary.
      if (!value->IsCallable()) return false;
      desc->set_get(value);
    } else if (key == ReadOnlyRoots(heap).set_string()) {
      // Bail out to slow path to throw an exception if necessary.
      if (!value->IsCallable()) return false;
      desc->set_set(value);
    }
