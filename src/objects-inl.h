...
bool Object::FilterKey(PropertyFilter filter) {
  Object* key = IsPropertyCell() ? PropertyCell::cast(this)->name() : this;
  if (key->IsSymbol()) {
    if (filter & SKIP_SYMBOLS) return true;
    if (Symbol::cast(key)->is_private()) return true;
  } else {
...
Name* GlobalDictionary::NameAt(int entry) { return ValueAt(entry)->name(); }

PropertyCell* GlobalDictionary::ValueAt(int entry) {
...
PropertyDetails GlobalDictionaryShape::DetailsAt(Dictionary* dict, int entry) {
  ...
  return dict->ValueAt(entry)->property_details();
}
...
void GlobalDictionaryShape::DetailsAtPut(Dictionary* dict, int entry,
  ...
  DCHECK_LE(0, entry);  // Not found is -1, which is not caught by get().
  PropertyCell* cell = dict->ValueAt(entry);
