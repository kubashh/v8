...
// signal handler. This code can be found in handler-shared.cc.
...
bool TryHandleWasmTrap(LPEXCEPTION_POINTERS exception) {
  ...
  const PEXCEPTION_RECORD exception_record = exception->ExceptionRecord;

  if (exception_record->ExceptionCode != EXCEPTION_ACCESS_VIOLATION) {
    return false;
  }

  uintptr_t const fault_addr =
...
LONG HandleWasmTrap(LPEXCEPTION_POINTERS exception) {
  if (TryHandleWebAssemblyTrapWindows(exception)) {
