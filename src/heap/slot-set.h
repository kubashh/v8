...
class SlotSet {
 public:
  ...
  static SlotSet* Allocate(size_t buckets) {
    ...
    void* allocation =
        AlignedAlloc(kLengthSize + buckets_size, kSystemPointerSize);
    SlotSet* slot_set = reinterpret_cast<SlotSet*>(
        reinterpret_cast<uint8_t*>(allocation) + kLengthSize);
#ifdef DEBUG
  ...
  static void Delete(SlotSet* slot_set, size_t buckets) {
    ...
    AlignedFree(reinterpret_cast<uint8_t*>(slot_set) - kLengthSize);
  }
 ...
 private:
  ...
  static const int kLengthSize = sizeof(size_t);
#else
  static const int kLengthSize = 0;
#endif
};
