...
class SlotSet {
 public:
  ...
  static SlotSet* Allocate(size_t buckets) {
    size_t size = sizeof(size_t) + buckets * sizeof(Bucket*);
    void* allocation = AlignedAlloc(size, kSystemPointerSize);
    SlotSet* slot_set =
        reinterpret_cast<SlotSet*>(reinterpret_cast<size_t*>(allocation) + 1);
  ...
  static void Delete(SlotSet* slot_set, size_t buckets) {
    ...
    size_t* allocation = reinterpret_cast<size_t*>(slot_set) - 1;
