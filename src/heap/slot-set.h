...
class SlotSet {
 public:
  ...
  class Bucket {
   public:
    ...
    static Bucket* Allocate() {
      uint32_t* cells = NewArray<uint32_t>(kCellsPerBucket);

      for (int i = 0; i < kCellsPerBucket; i++) {
        cells[i] = 0;
      }

      return reinterpret_cast<Bucket*>(cells);
    }

    static void Delete(Bucket* bucket) {
      DeleteArray<uint32_t>(bucket->cells());
    }

    uint32_t* cells() { return reinterpret_cast<uint32_t*>(this); }
  ...
  STATIC_ASSERT(std::is_standard_layout<Bucket>::value);
  ...
  static SlotSet* Allocate(size_t buckets) {
    Bucket** array = NewArray<Bucket*>(buckets);
  ...
  static void Delete(SlotSet* slot_set, size_t buckets) {
    ...
    DeleteArray<Bucket*>(slot_set->buckets());
  }
  ...
  void Insert(size_t slot_offset) {
    ...
    Bucket* bucket = LoadBucket<access_mode>(bucket_ptr(bucket_index));
    if (bucket == nullptr) {
      bucket = Bucket::Allocate();
  ...
  static const int kBucketsRegularPage =
      kMaxSlots / kCellsPerBucket / kBitsPerCell;
 ...
 private:
  ...
  Bucket* LoadBucket(Bucket** bucket) {
    if (access_mode == AccessMode::ATOMIC)
      return reinterpret_cast<Bucket*>(base::AsAtomicPointer::Acquire_Load(
  ...
  Bucket* LoadBucket(size_t bucket_index) {
    return LoadBucket(bucket_ptr(bucket_index));
  }
  ...
  void StoreBucket(size_t bucket_index, Bucket* value) {
    StoreBucket(bucket_ptr(bucket_index), value);
  }

  bool IsEmptyBucket(Bucket* bucket) {
    for (int i = 0; i < kCellsPerBucket; i++) {
      if (bucket->LoadCell(i)) {
        return false;
      }
    }
    return true;
  ...
  bool SwapInNewBucket(Bucket** bucket, Bucket* value) {
    if (access_mode == AccessMode::ATOMIC) {
      return base::AsAtomicPointer::Release_CompareAndSwap(
                 reinterpret_cast<uint32_t**>(bucket), nullptr,
                 value->cells()) == nullptr;
    } else {
      DCHECK_NULL(*bucket);
      *bucket = value;
  ...
  Bucket** bucket_ptr(size_t bucket_index) { return buckets() + bucket_index; }
};
