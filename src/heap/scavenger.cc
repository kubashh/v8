...
void ScavengerCollector::CollectGarbage() {
  ...
  // Scavenger s[kMaxScavengerTasks];
  ...
  std::vector<std::unique_ptr<Scavenger>> scavengers;
  {
    ...
    for (int i = 0; i < num_scavenge_tasks; ++i) {
      ...
      // new (&s[i])
      //     Scavenger(this, heap_, is_logging, &empty_chunks, &copied_list,
      //               &promotion_list, &ephemeron_table_list, i);
    }
    Scavenger& main_thread_scavenger = *scavengers[kMainThreadId].get();
    // std::span<Scavenger> sp(&s[0], num_scavenge_tasks);
    // Scavenger& main_thread_scavenger = sp[0];
