...
// The class that implement memory balancer.
...
class MemoryBalancer {
 ...
 private:
  struct SmoothedBytesAndDuration {
    ...
    // Return memory(in bytes) per time(in nanoseconds).
  ...
  // A more principaled solution is to manage the external heap also using
  ...
  // HeartbeatTask use the diff between last observed time/memory and
