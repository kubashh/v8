...
class MemoryBalancer {
 public:
  ...
  MemoryBalancer(Heap* heap) : heap_(heap) {}
  ...
  std::atomic<size_t> live_memory_{0};
  std::atomic<double> major_allocation_bytes_{0};
  std::atomic<double> major_allocation_time_{0};
  std::atomic<double> major_gc_bytes_{0};
  std::atomic<double> major_gc_time_{0};
  std::atomic<bool> has_major_allocation_{false};
  std::atomic<bool> has_major_gc_{false};
  std::atomic<size_t> last_M_update_time_{0};
  std::atomic<double> last_M_memory_{0};
  std::atomic<size_t> concurrent_gc_time_{0};
