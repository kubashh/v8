...
class MemoryBalancer {
 public:
  ...
  explicit MemoryBalancer(Heap* heap, base::TimeTicks time);

  void NotifyGC(size_t major_allocation_bytes,
                base::TimeDelta major_allocation_duration,
                size_t major_gc_bytes, base::TimeDelta major_gc_duration);
