...
Handle<Code> FactoryBase<Impl>::NewCode(const NewCodeOptions& options) {
  ...
  // Set interpreter data or deopt data.
  if (CodeKindUsesBytecodeOrInterpreterData(options.kind)) {
    DCHECK(!CodeKindUsesDeoptimizationData(options.kind));
  ...
  // Set bytecode offsets or source position table.
  if (CodeKindUsesBytecodeOffsetTable(options.kind)) {
    ...
    if (options.source_position_table.is_null()) {
      code->set_source_position_table(*empty_byte_array());
    } else {
...
Handle<BytecodeArray> FactoryBase<Impl>::NewBytecodeArray(
  ...
  instance->clear_source_position_table();
