...
MaybeHandle<SeqStringT> FactoryBase<Impl>::NewRawStringWithMap(
  ...
  DCHECK(map == read_only_roots().string_map() ||
         map == read_only_roots().shared_string_map() ||
         map == read_only_roots().one_byte_string_map() ||
         map == read_only_roots().shared_one_byte_string_map());
  DCHECK_IMPLIES(map != read_only_roots().shared_string_map() &&
                     map != read_only_roots().shared_one_byte_string_map(),
