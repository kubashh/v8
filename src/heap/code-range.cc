...
uint8_t* CodeRange::RemapEmbeddedBuiltins(Isolate* isolate,
...
#if V8_OS_MACOS
  // By default, the embedded builtins are not remapped, but copied. This costs
  // memory, since builtins become private dirty anonymous memory, rather than
  // shared, clean, file-backed memory for the embedded version. On macOS, we
  // can remap the builtins *on top* of the space allocated in the code range,
  // making the "copy" shared, clean, file-backed memory, and thus saving
  // sizeof(builtins).
  //
  ...
  if ((reinterpret_cast<uintptr_t>(embedded_blob_code) &
    ...
    // Check (here and below) rather than fallback as this should never happen.
