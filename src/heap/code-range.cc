...
base::AddressRegion CodeRange::GetNonABICompliantCodeRange() {
  size_t reserved_area = GetWritableReservedAreaSize();
  DCHECK_LT(reserved_area, size());
  return {reserved_area, size() - reserved_area};
}
...
bool CodeRange::InitReservation(v8::PageAllocator* page_allocator,
  ...
  Address the_hint =
      GetCodeRangeAddressHint()->GetAddressHint(requested, kPageSize);
  ...
  if (reserved_area > 0) {
    ...
    if (win64_unwindinfo::CanRegisterUnwindInfoForNonABICompliantCodeRange()) {
      base::AddressRegion range = GetNonABICompliantCodeRange();
      win64_unwindinfo::RegisterNonABICompliantCodeRange(range.begin(),
                                                         range.size());
    }
#endif  // V8_OS_WIN64
  }
  ...
  if (V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT &&
    ...
    CHECK_EQ(reserved_area, 0);  // Otherwise the logic below must be changed.
...
void CodeRange::Free() {
  if (IsReserved()) {
    ...
    if (win64_unwindinfo::CanRegisterUnwindInfoForNonABICompliantCodeRange()) {
      base::AddressRegion range = GetNonABICompliantCodeRange();
      win64_unwindinfo::UnregisterNonABICompliantCodeRange(range.begin());
    }
#endif  // V8_OS_WIN64
