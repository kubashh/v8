...
bool CodeRange::InitReservation(v8::PageAllocator* page_allocator,
  ...
  DCHECK_LE(requested, kMaximalCodeRangeSize);

  VirtualMemoryCage::ReservationParams params;
  params.page_allocator = page_allocator;
  params.reservation_size = requested;
  params.base_alignment = MemoryAllocator::GetCommitPageSize();
...
// static
bool CodeRange::RequiresProcessWideCodeRange() {
  return COMPRESS_POINTERS_IN_SHARED_CAGE_BOOL && kPlatformRequiresCodeRange &&
         !FLAG_jitless;
}

// static
void CodeRange::InitializeOncePerProcess(v8::PageAllocator* page_allocator) {
  if (RequiresProcessWideCodeRange()) {
    *GetProcessWideCodeRangeCage() = std::make_shared<CodeRange>();
    // TODO(v8:11460) A shared code range's size is currently not configurable
    // and is always the maximal size.
...
std::shared_ptr<CodeRange> CodeRange::GetProcessWideCodeRange() {
  return RequiresProcessWideCodeRange() ? *GetProcessWideCodeRangeCage()
                                        : nullptr;
}
