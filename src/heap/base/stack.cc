...
void Stack::IteratePointers(StackVisitor* visitor) const {
  DCHECK_NOT_NULL(stack_start_);
  PushAllRegistersAndCallback(visitor, stack_start_, &IteratePointersImpl);
...
void Stack::StoreCalleeSavedRegisters(CalleeSavedRegisters* registers) {
  SaveCalleeSavedRegisters(registers->buffer.data());
}

V8_NOINLINE void Stack::PushAllRegistersAndCallback(StackVisitor* visitor,
                                                    const void* stack_start,
                                                    Callback callback) {
  Stack::CalleeSavedRegisters registers;
  Stack::StoreCalleeSavedRegisters(&registers);
