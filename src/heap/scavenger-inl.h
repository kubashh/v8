...
CopyAndForwardResult Scavenger::SemiSpaceCopyObject(Map* map,
  ...
  if (allocation.To(&target)) {
    ...
    if (!self_success) {
      ...
      return CopyAndForwardResult::MIGRATE_FAILURE;
    }
    HeapObjectReference::Update(slot, target);

    copied_list_.Push(ObjectAndSize(target, object_size));
    copied_size_ += object_size;
    return CopyAndForwardResult::SUCCESS;
  }
  return CopyAndForwardResult::COPY_FAILURE;
}
...
SlotCallbackResult Scavenger::HandleMigrationFailure(
    HeapObjectReference** slot) {
  MaybeObject* target = *slot;
  if (Heap::InToSpace(target)) return KEEP_SLOT;
...
SlotCallbackResult Scavenger::EvacuateObjectDefault(Map* map,
  ...
  if (!heap()->ShouldBePromoted(object->address())) {
    ...
    if (result == CopyAndForwardResult::SUCCESS) return KEEP_SLOT;
    if (result == CopyAndForwardResult::MIGRATE_FAILURE) {
      return HandleMigrationFailure(slot);
    }
    DCHECK(result == CopyAndForwardResult::COPY_FAILURE);
  }
  ...
  if (result == CopyAndForwardResult::SUCCESS) return REMOVE_SLOT;
  if (result == CopyAndForwardResult::MIGRATE_FAILURE) {
    return HandleMigrationFailure(slot);
  }
  DCHECK(result == CopyAndForwardResult::COPY_FAILURE);
...
SlotCallbackResult Scavenger::ScavengeObject(HeapObjectReference** p,
  ...
  if (first_word.IsForwardingAddress()) {
    ...
    CHECK(Heap::InToSpace(dest) || !Heap::InNewSpace((dest)));
...
SlotCallbackResult Scavenger::CheckAndScavengeObject(Heap* heap,
  ...
  if (Heap::InFromSpace(object)) {
    ...
    object = *slot;
    CHECK(result == REMOVE_SLOT || Heap::InNewSpace(object));
    return result;
