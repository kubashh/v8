...
#ifdef V8_TARGET_ARCH_64_BIT
  Address src_base = v8::internal::GetIsolateRoot(src[0]);
...
SharedReadOnlySpace* PointerCompressedReadOnlyArtifacts::CreateReadOnlySpace(
  ...
  for (size_t i = 0; i < pages_.size(); ++i) {
    ...
    auto shared_memory =
        RemapPageTo(memory_allocator, i, new_address, new_page);
...
PointerCompressedReadOnlyArtifacts::RemapPageTo(MemoryAllocator* allocator,
                                                size_t i, Address new_address,
                                                ReadOnlyPage*& new_page) {
  std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping> mapping =
      allocator->RemapSharedPage(shared_memory_[i].get(), new_address);
  if (mapping) {
...
void PointerCompressedReadOnlyArtifacts::Initialize(
  ...
  for (const ReadOnlyPage* page : pages) {
    ...
    uint32_t compressed_address = static_cast<uint32_t>(page->address());
#ifdef V8_COMPRESS_POINTERS
    DCHECK_EQ(compressed_address, CompressTagged(page->address()));
#endif
