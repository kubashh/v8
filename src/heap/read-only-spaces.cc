...
namespace {
// To avoid huge numbers of #ifdefs, this provides a GetIsolateRoot that can be
// used in all configurations. It is however only defined when
// V8_COMPRESS_POINTERS is defined.
Address GetIsolateRoot(Isolate* isolate) {
#ifdef V8_TARGET_ARCH_64_BIT
  return i::GetIsolateRoot(isolate);
#else
  UNREACHABLE();
...
void ReadOnlySpace::TearDown(MemoryAllocator* memory_allocator,
  ...
  for (ReadOnlyPage* chunk : pages_) {
    ...
    memory_allocator->FreeReadOnlyPage(
        chunk, !ReadOnlyHeap::IsReadOnlySpaceShared() ||
                   deletion_policy == kForceDeleteArtifacts);
  }
  pages_.resize(0);
...
void ReadOnlyArtifacts::MakeSharedCopy(
  ...
  for (const ReadOnlyPage* page : pages) {
    ...
    // 1. Allocate some new memory for a shared copy of the page. Doesn't need
    // to be V8 page aligned, since we'll never use it directly.
    void* address = page_allocator->AllocateSharedPages(
        size, PageAllocator::Permission::kReadWrite);
    ...
    // 3. Update the accounting stats stored so the allocated bytes are for the
    // new shared page rather than the original.
...
// static
ReadOnlyHeap* ReadOnlyArtifacts::GetReadOnlyHeapForIsolate(
    std::shared_ptr<ReadOnlyArtifacts> artifacts, Isolate* isolate) {
  DCHECK(ReadOnlyHeap::IsReadOnlySpaceShared());
  if (COMPRESS_POINTERS_BOOL) {
    auto* shared_read_only_space = new SharedReadOnlySpace(
    ...
    read_only_heap->InitializeReadOnlyRoots(artifacts, new_base_address);
...
SharedReadOnlySpace::SharedReadOnlySpace(
  ...
  if (new_base_address != 0) {
    // The previous ReadOnlyPages have now been reclaimed. So now we have to
    // get them back again so that it thinks it still owns them.
    for (size_t i = 0; i < pages.size(); ++i) {
      ...
      CHECK(success);
      void* old_address =
          const_cast<void*>(reinterpret_cast<const void*>(page));
      ReadOnlyPage* p =
          static_cast<ReadOnlyPage*>(page_allocator->RemapSharedPages(
              old_address, reinterpret_cast<void*>(new_address), size));
      CHECK_NOT_NULL(p);
...
void ReadOnlyPage::MakeHeaderRelocatable() {
  if (COMPRESS_POINTERS_BOOL) {
    // The pages must be relocatable at this point and area_start and area_end
    // are only used to calculate the area_size for capacity calculations so
    // offset them from kNullAddress.
