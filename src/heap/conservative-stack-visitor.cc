...
void measure_css::ObjectStats::AddObject(Address p) {
  ...
  objects_[p] = TO_CALCULATE;
}

void measure_css::ObjectStats::Calculate(Address p) const {
  if (objects_[p] != TO_CALCULATE) return;
  HeapObject obj = HeapObject::FromAddress(p);
...
Address measure_css::ObjectStats::LookupObject(Address p) const {
  base::MutexGuard lock(&mutex_);
  if (objects_.empty()) return kNullAddress;
  auto it = objects_.upper_bound(p);
  if (it == objects_.begin()) return kNullAddress;
  --it;
  CHECK_LE(it->first, p);
  Calculate(p);
  if (it->first + it->second <= p) return kNullAddress;
  return it->first;
}
...
void measure_css::PointerStats::PrintNVPOn(std::ostream& out) const {
  ...
  for (auto [pointer, times] : histogram_) {
...
void measure_css::ObjectStats::PrintNVPOn(std::ostream& out) const {
  for (auto it : objects_) Calculate(it.first);
  out << "{\"count\": " << objects_.size() << ",\"sum\": " << total_size_
      << "}";
}
...
void measure_css::Stats::AddPointer(Address p, CounterId id) {
  ...
  if (id != ALREADY_MARKED && id != FULL_ALREADY_MARKED &&
    ...
    return;
  Address base_ptr = marked_objects_.LookupObject(p);
  if (id == FULL_NOT_ALREADY_MARKED || id == YOUNG_NOT_ALREADY_MARKED) {
    // The object should definitely be marked now.
    CHECK_NE(kNullAddress, base_ptr);
    pointers_[FALSE_POSITIVE].AddSample(base_ptr);
  } else if (id == ALREADY_MARKED && base_ptr == kNullAddress) {
    pointers_[MYSTERIOUSLY_MARKED].AddSample(p);
  } else {
    CHECK_NE(kNullAddress, base_ptr);
    pointers_[WOULD_BE_PINNED].AddSample(base_ptr);
  }
}
...
void measure_css::Stats::PrintNVPOn(std::ostream& out) const {
  out << "{\"primary\": " << pointers_[PRIMARY]
      ...
      << ",\"mysteriously marked\": " << pointers_[MYSTERIOUSLY_MARKED] << "}";
}
