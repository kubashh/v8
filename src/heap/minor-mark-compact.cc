...
#include "src/heap/mark-compact-utilities-inl.h"
...
#include "src/heap/minor-mark-compact-inl.h"
...
class YoungGenerationMarkingVerifier
    : public MarkCompactUtilities::MarkingVerifierBase {
 public:
...
void YoungGenerationMarkingState::IncrementLiveBytes(MemoryChunk* chunk,
                                                     intptr_t by) {
...
bool YoungGenerationMainMarkingVisitor::ShortCutStrings(
...
void MinorMarkCompactCollector::PerformWrapperTracing() {
  ...
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_EMBEDDER_TRACING);
...
void MinorMarkCompactCollector::StartMarking() {
  ...
  if (cpp_heap && cpp_heap->generational_gc_supported()) {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_EMBEDDER_PROLOGUE);
...
void MinorMarkCompactCollector::Finish() {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_FINISH);

  {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_FINISH_ENSURE_CAPACITY);
...
#ifdef VERIFY_HEAP
  if (v8_flags.verify_heap) {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_VERIFY);
...
class YoungStringForwardingTableCleaner final
    : public MarkCompactUtilities::StringForwardingTableCleanerBase {
 public:
...
void MinorMarkCompactCollector::ClearNonLiveReferences() {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_CLEAR);
  ...
  if (external_string_table.HasYoung()) {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_CLEAR_STRING_TABLE);
    // Internalized strings are always stored in old space, so there is no
    // need to clean them here.
    MarkCompactUtilities::ExternalStringTableCleaner<
        MarkCompactUtilities::ExternalStringTableCleaningMode::kYoungOnly>
...
void MinorMarkCompactCollector::MarkRoots(RootMarkingVisitor* root_visitor) {
  Isolate* isolate = heap_->isolate();

  // Seed the root set (roots + old->new set).
  {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_SEED);
...
void MinorMarkCompactCollector::DoParallelMarking() {
  DCHECK(!v8_flags.concurrent_minor_mc_marking);
...
void MinorMarkCompactCollector::MarkRootsFromConservativeStack(
...
void MinorMarkCompactCollector::MarkLiveObjects() {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK);
...

  {
    ...
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_CLOSURE_PARALLEL);
    if (!v8_flags.concurrent_minor_mc_marking) {
      DoParallelMarking();
    } else {
      if (v8_flags.parallel_marking) {
  ...
  {
    TRACE_GC(heap_->tracer(),
             GCTracer::Scope::MINOR_MC_MARK_CONSERVATIVE_STACK);
    MarkRootsFromConservativeStack(&root_visitor);
  }

  {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_MARK_CLOSURE);
...
void MinorMarkCompactCollector::DrainMarkingWorklist(
  ...
  do {
    ...
    while (local_marking_worklists_->Pop(&heap_object) ||
           local_marking_worklists_->PopOnHold(&heap_object)) {
      ...
      if (visited_size) {
        marking_state_->IncrementLiveBytes(
           ...
           !MarkCompactUtilities::IsCppHeapMarkingFinished(
...
intptr_t NewSpacePageEvacuationThreshold() {
  return v8_flags.minor_mc_page_promotion_threshold *
...
bool MinorMarkCompactCollector::StartSweepNewSpace() {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_SWEEP_NEW);
  ...
  for (auto it = paged_space->begin(); it != paged_space->end();) {
    ...
    intptr_t live_bytes_on_page = non_atomic_marking_state_->live_bytes(p);
...
bool MinorMarkCompactCollector::SweepNewLargeSpace() {
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_SWEEP_NEW_LO);
...
void MinorMarkCompactCollector::Sweep() {
  DCHECK(!sweeper()->AreMinorSweeperTasksRunning());
  TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_SWEEP);
...
#ifdef DEBUG
  MarkCompactUtilities::VerifyRememberedSetsAfterEvacuation(
      heap_, GarbageCollector::MINOR_MARK_COMPACTOR);
  heap_->VerifyCountersBeforeConcurrentSweeping(
      GarbageCollector::MINOR_MARK_COMPACTOR);
#endif

  {
    TRACE_GC(heap_->tracer(), GCTracer::Scope::MINOR_MC_SWEEP_START_JOBS);
