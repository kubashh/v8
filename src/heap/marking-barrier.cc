...
void MarkingBarrier::Write(DescriptorArray descriptor_array,
  ...
  // Only major GC uses custom liveness in descriptor arrays.
  if (is_major()) {
  ...
  if (gc_epoch.has_value()) {
    ...
    if (DescriptorArrayMarkingState::TryUpdateIndicesToMark(
            gc_epoch.value(), descriptor_array, number_of_own_descriptors)) {
      current_worklist_->Push(descriptor_array);
    }
  } else {
    // Minor MC.
