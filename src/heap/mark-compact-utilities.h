...
#ifndef V8_HEAP_MARK_COMPACT_UTILITIES_H_
#define V8_HEAP_MARK_COMPACT_UTILITIES_H_
...
class MarkCompactUtilities final {
 public:
#ifdef VERIFY_HEAP
  class MarkingVerifierBase : public ObjectVisitorWithCageBases,
   ...
   protected:
    ...
    virtual void VerifyRootPointers(FullObjectSlot start,
                                    FullObjectSlot end) = 0;
  ...
  static bool IsCppHeapMarkingFinished(
      Heap* heap, MarkingWorklists::Local* local_marking_worklists);

#if DEBUG
  static void VerifyRememberedSetsAfterEvacuation(
...
#endif  // V8_HEAP_MARK_COMPACT_UTILITIES_H_
