...
#define TRACE(cond, ...) StreamTraceIf(cond, "ro-promotion: ", __VA_ARGS__)
#define TRACE_NORMAL(...) TRACE(v8_flags.trace_read_only_promotion, __VA_ARGS__)
#define TRACE_VERBOSE(...) \
  TRACE(v8_flags.trace_read_only_promotion_verbose, __VA_ARGS__)
 ...
 private:
  ...
  std::vector<Tagged<HeapObject>> DeterminePromotees(
    ...
    for (Tagged<HeapObject> o = it.Next(); !o.is_null(); o = it.Next()) {
      ...
      TRACE_NORMAL("accepted set {", CostlyTrace([&] {
  ...
  bool EvaluateSubgraph(Tagged<HeapObject> o, HeapObjectSet* accepted_subgraph,
    ...
    if (!IsPromoCandidate(isolate_, o)) {
      if (promo_rejected_.insert(o).second) {
        TRACE_NORMAL("rejected due to failed predicate ",
