...
static constexpr size_t kAllocationGranularity = sizeof(char);
...
class V8_EXPORT_PRIVATE ObjectStartBitmap {
 public:
  ...
  explicit inline ObjectStartBitmap(size_t offset);
...
ObjectStartBitmap::ObjectStartBitmap() = default;
...
Address ObjectStartBitmap::FindBasePtr(Address maybe_inner_ptr) const {
  ...
  uint8_t byte = load(cell_index) & ((1 << (bit + 1)) - 1);
  ...
  return reinterpret_cast<Address>(object_offset + offset());
}

void ObjectStartBitmap::SetBit(Address base_ptr) {
  size_t cell_index, object_bit;
  ObjectStartIndexAndBit(base_ptr, &cell_index, &object_bit);
  store(cell_index, static_cast<uint8_t>(load(cell_index) | (1 << object_bit)));
}
  ...
  for (size_t cell_index = 0; cell_index < kReservedForBitmap; cell_index++) {
    ...
    while (value) {
      ...
      const Address object_address =
          offset() + (kAllocationGranularity * object_start_number);
...
bool ObjectStartBitmap::IsClear() const {
  for (uint8_t cell : object_start_bit_map_) {
    if (cell != 0) {
      return false;
    }
  }
