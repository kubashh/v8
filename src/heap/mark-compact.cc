  ...
  Object RetainAs(Object object) override {
    ...
    if (marking_state_->IsBlack(heap_object)) {
      ...
      while (nested.IsAllocationSite()) {
        ...
        marking_state_->WhiteToGrey(current_site);
        if (marking_state_->GreyToBlack(current_site)) {
          marking_state_->IncrementLiveBytes(
              MemoryChunk::cast(BasicMemoryChunk::FromHeapObject(current_site)),
              ALIGN_TO_ALLOCATION_ALIGNMENT(current_site.Size()));
        }
      }
 ...
 private:
  void MarkForwardObject(StringForwardingTable::Record* record) {
    ...
    if (marking_state_->IsBlack(original_string)) {
      ...
      marking_state_->WhiteToGrey(forward_object);
      if (marking_state_->GreyToBlack(forward_object)) {
        marking_state_->IncrementLiveBytes(
            MemoryChunk::cast(BasicMemoryChunk::FromHeapObject(forward_object)),
            ALIGN_TO_ALLOCATION_ALIGNMENT(forward_object.Size()));
      }
    } else {
  ...
  void TryInternalize(String original_string,
    ...
    if (!forward_string.InReadOnlySpace()) {
      marking_state_->WhiteToGrey(forward_string);
      if (marking_state_->GreyToBlack(forward_string)) {
        marking_state_->IncrementLiveBytes(
            MemoryChunk::cast(BasicMemoryChunk::FromHeapObject(forward_string)),
            ALIGN_TO_ALLOCATION_ALIGNMENT(forward_string.Size()));
      }
    }
...
void MarkCompactCollector::FlushBytecodeFromSFI(
  ...
  marking_state_->WhiteToGrey(uncompiled_data);
  if (marking_state_->GreyToBlack(uncompiled_data)) {
    marking_state_->IncrementLiveBytes(
        MemoryChunk::cast(BasicMemoryChunk::FromHeapObject(uncompiled_data)),
        ALIGN_TO_ALLOCATION_ALIGNMENT(uncompiled_data.Size()));
  }
