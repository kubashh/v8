...
void MarkCompactCollector::SetUp() {
  ...
  for (int i = 0; i < 32; i++) {
    bitmap_pool_.Add(static_cast<Bitmap*>(calloc(1, Bitmap::kSize)));
  }
}
...
void MarkCompactCollector::CollectGarbageInYoungGeneration() {
  ...
  for (Page* p : PageRange(heap()->new_space()->FromSpaceStart(),
    ...
    bitmap_pool_.Add(p->ReleaseExternalBitmap());
    Bitmap* bm = bitmap_pool_.last();
    bm->Clear();
  }
}
 ...
 public:
  ...
  void VisitPointers(Object** start, Object** end) override {
    ...
    for (Object** p = start; p < end; p++) {
      Object* o = *p;
      if (o->IsHeapObject()) {
        if (ObjectMarking::IsWhite<mode>(HeapObject::cast(o))) {
...
class MarkCompactWeakObjectRetainer : public WeakObjectRetainer {
 public:
  ...
  virtual Object* RetainAs(Object* object) {
    ...
    DCHECK(!ObjectMarking::IsGrey<mode>(heap_object));
    if (ObjectMarking::IsBlack<mode>(heap_object)) {
      return object;
...
void MarkCompactCollector::DiscoverGreyObjectsOnPage(MemoryChunk* p) {
  ...
  while ((object = it.Next()) != NULL) {
    DCHECK(ObjectMarking::IsGrey<mode>(object));
    ObjectMarking::GreyToBlack<mode>(object);
 ...
 protected:
  ...
  inline void MigrateObject(HeapObject* dst, HeapObject* src, int size,
    ...
    if (mode == MarkingMode::YOUNG_GENERATION &&
      ...
      IncrementalMarking::TransferColor(src, dst, size);
    }
  }
 ...
 private:
  ...
  inline bool EvacuateWithoutCopying(HeapObject* object) {
    ...
    if (object->IsThinString()) {
      ...
      // TODO(mlippautz): Account for ThinString size as promoted.
      return true;
    }
    // TODO(mlippautz): Handle ConstString.
    return false;
  }
...
static bool IsUnmarkedObject(Heap* heap, Object** p) {
  ...
  return heap->InNewSpace(*p) &&
         !ObjectMarking::IsBlack<MarkingMode::YOUNG_GENERATION>(
             HeapObject::cast(*p));
}
...
void MarkCompactCollector::MarkLiveObjectsInYoungGeneration() {
...

  {
    ...
    for (Page* p : heap()->new_space()->to_space()) {
      p->AllocateExternalBitmap(bitmap_pool_.RemoveLast());
    }
  }
...
bool MarkCompactCollector::VisitLiveObjects(MemoryChunk* page, Visitor* visitor,
  ...
  while ((object = it.Next()) != nullptr) {
    DCHECK(ObjectMarking::IsBlack<M>(object));
