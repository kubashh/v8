...
static void VerifyAllBlackObjects(MemoryChunk* page) {
  LiveObjectIterator<kAllLiveObjects, mode> it(page);
  HeapObject* object = NULL;
  while ((object = it.Next()) != NULL) {
    CHECK(ObjectMarking::IsBlack<mode>(object));
  }
}
...
class RootMarkingVisitor : public ObjectVisitor {
 ...
 private:
  V8_INLINE void MarkObjectByPointer(Object** p) {
    ...
    // double start = collector_->heap()->MonotonicallyIncreasingTimeInMs();
    // {

    if (ObjectMarking::IsBlackOrGrey<mode>(object)) return;

    //   printf("took:
    //   %f\n",collector_->heap()->MonotonicallyIncreasingTimeInMs() - start );
    // }
...
class MarkCompactCollector::EvacuateVisitorBase
 ...
 protected:
  ...
  inline void MigrateObject(HeapObject* dst, HeapObject* src, int size,
    ...
    if (M == MarkingMode::YOUNG_GENERATION &&
...
class MarkCompactCollector::EvacuateNewSpaceVisitor final
    : public MarkCompactCollector::EvacuateVisitorBase<M> {
 public:
  ...
  explicit EvacuateNewSpaceVisitor(Heap* heap,
      ...
      : EvacuateVisitorBase<M>(heap, compaction_spaces),
...
#ifdef VERIFY_HEAP
  for (Page* p : heap()->new_space()->to_space()) {
    VerifyAllBlackObjects<MarkingMode::YOUNG_GENERATION>(p);
  }
#endif  // VERIFY_HEAP
}
...
#ifdef VERIFY_HEAP
  for (Page* p : heap()->new_space()->to_space()) {
    VerifyAllBlackObjects<MarkingMode::FULL>(p);
  }
#endif  // VERIFY_HEAP
}
...
void MarkCompactCollector::EvacuatePagesInParallel() {
  ...
  for (int i = 0; i < wanted_num_tasks; i++) {
    ...
    AllocationInfo info = evacuators[i]->CloseNewSpaceLAB();
