...
class YoungGenerationMarkingVisitor final
 ...
 public:
  ...
  YoungGenerationMarkingVisitor(Heap* heap, MarkingDeque* marking_deque)
      : heap_(heap), marking_deque_(marking_deque), recursion_depth_(0) {}
  ...
  void VisitPointer(HeapObject* host, Object** slot) final {
    ...
    if (heap_->InNewSpace(target)) {
      ...
      // TODO(mlippautz): Adopt MC recursive marking strategy.
 ...
 private:
  // For background tasks we cannot check the stack depth. Rely on recursion
  // depth for bailout.
  ...
  inline bool MarkRecursively(HeapObject* object) {
    if (recursion_depth_ > kMaxRecursionDepth) false;
    recursion_depth_++;
    if (ObjectMarking::WhiteToBlack<MarkBit::ATOMIC>(object,
                                                     marking_state(object))) {
      Visit(object);
