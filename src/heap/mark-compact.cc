...
class ClearTrivialWeakRefJobItem final
...
void MarkCompactCollector::ClearNonLiveReferences() {
  ...
  // A clearing job, used for all clearing that takes place in parallel.
  ...
  {
    auto clear_trivial_weakrefs_job_item =
        std::make_unique<ClearTrivialWeakRefJobItem>(this);
    const uint64_t trace_id = clear_trivial_weakrefs_job_item->trace_id();
    clearing_job->Add(std::move(clear_trivial_weakrefs_job_item));
...
void MarkCompactCollector::ClearTrivialWeakReferences() {
  ...
  while (local_weak_objects()->weak_references_trivial_local.Pop(&slot)) {
    ...
    if ((*location).GetHeapObjectIfWeak(&value)) {
      DCHECK(!IsCell(value));
      DCHECK(MainMarkingVisitor::IsTrivialWeakHeapObject(slot.first, value));
