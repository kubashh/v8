...
class RememberedSetUpdatingItem : public UpdatingItem {
 ...
 private:
  template <typename TSlot>
  inline void CheckOldToSharedUntypedSlot(MemoryChunk* chunk, TSlot slot) {
    HeapObject heap_object;
  ...
  inline void CheckOldToSharedTypedSlot(MemoryChunk* chunk, SlotType slot_type,
                                        Address addr) {
    HeapObject heap_object =
    ...
    if (chunk_->slot_set<OLD_TO_NEW, AccessMode::NON_ATOMIC>() != nullptr) {
      ...
      int slots = RememberedSet<OLD_TO_NEW>::Iterate(
          ...
          [this, &filter, has_shared_isolate](MaybeObjectSlot slot) {
            ...
            if (has_shared_isolate) CheckOldToSharedUntypedSlot(chunk_, slot);
            return result;
          },
          SlotSet::FREE_EMPTY_BUCKETS);
    ...
    if (chunk_->typed_slot_set<OLD_TO_NEW, AccessMode::NON_ATOMIC>() !=
      ...
      RememberedSet<OLD_TO_NEW>::IterateTyped(
          chunk_, [=](SlotType slot_type, Address slot) {
            ...
            CheckOldToSharedTypedSlot(chunk_, slot_type, slot);
            return result;
          });
    }
    if ((updating_mode_ == RememberedSetUpdatingMode::ALL) &&
