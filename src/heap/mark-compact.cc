...
class RememberedSetUpdatingItem : public UpdatingItem {
 public:
  explicit RememberedSetUpdatingItem(MemoryChunk* chunk) : chunk_(chunk) {}
...
int MarkCompactCollectorBase::CollectRememberedSetUpdatingItems(
  ...
  RememberedSet<type>::IterateMemoryChunks(
      heap(), [&job, &pages](MemoryChunk* chunk) {
        job->AddItem(new RememberedSetUpdatingItem<type>(chunk));
        pages++;
      });
...
void MarkCompactCollector::UpdatePointersAfterEvacuation() {
...

  {
    ...
    {
      TRACE_GC(heap()->tracer(),
               GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_TO_NEW_SLOTS);
      updating_job.Run();
    }
  }
...
void MinorMarkCompactCollector::UpdatePointersAfterEvacuation() {
...

  {
    ...
    {
      TRACE_GC(heap()->tracer(),
               GCTracer::Scope::MINOR_MC_EVACUATE_UPDATE_POINTERS_TO_NEW_SLOTS);
      updating_job.Run();
    }
  }
