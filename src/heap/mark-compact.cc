...
class FullMarkingVerifier : public MarkCompactUtilities::MarkingVerifierBase {
 public:
...
void MarkCompactCollector::VerifyMarkbitsAreClean(PagedSpaceBase* space) {
  for (Page* p : *space) {
    ...
    CHECK_EQ(0, non_atomic_marking_state_->live_bytes(p));
  }
}
...
void MarkCompactCollector::VerifyMarkbitsAreClean(LargeObjectSpace* space) {
  ...
  for (HeapObject obj = it.Next(); !obj.is_null(); obj = it.Next()) {
    ...
    CHECK_EQ(0, non_atomic_marking_state_->live_bytes(
...
void MarkCompactCollector::VerifyMarking() {
  ...
  if (v8_flags.verify_heap) {
    ...
    if (v8_flags.minor_mc && heap_->paged_new_space())
...
void MarkCompactCollector::MarkTransitiveClosureLinear() {
  ...
  while (work_to_do) {
    ...
    work_to_do = !local_marking_worklists_->IsEmpty() ||
                 !MarkCompactUtilities::IsCppHeapMarkingFinished(
                     heap_, local_marking_worklists_.get());
...
void MarkCompactCollector::MarkLiveObjects() {
...

  {
    ...
    CHECK(local_weak_objects()
              ->discovered_ephemerons_local.IsLocalAndGlobalEmpty());
    CHECK(MarkCompactUtilities::IsCppHeapMarkingFinished(
        heap_, local_marking_worklists_.get()));
...
void MarkCompactCollector::ClearNonLiveReferences() {
...

  {
    ...
    MarkCompactUtilities::ExternalStringTableCleaner<
        MarkCompactUtilities::ExternalStringTableCleaningMode::kAll>
...
void MarkCompactCollector::EvacuatePrologue() {
  ...
  if (new_space) {
    ...
    for (Page* p : *new_space) {
      if (non_atomic_marking_state_->live_bytes(p) > 0) {
...
void MarkCompactCollector::EvacuatePagesInParallel() {
  ...
  for (Page* page : new_space_evacuation_pages_) {
    intptr_t live_bytes_on_page = non_atomic_marking_state_->live_bytes(page);
  ...
  for (Page* page : old_space_evacuation_pages_) {
    ...
    live_bytes += non_atomic_marking_state_->live_bytes(page);
    evacuation_items.emplace_back(ParallelWorkItem{}, page);
  }
...

  {
    ...
    for (Page* p : new_space_evacuation_pages_) {
      if (p->IsFlagSet(Page::PAGE_NEW_OLD_PROMOTION)) {
        ...
        DCHECK_EQ(0, non_atomic_marking_state_->live_bytes(p));
    ...
    for (LargePage* p : promoted_large_pages_) {
      ...
      non_atomic_marking_state_->MarkBitFrom(object).Clear();
      p->ProgressBar().ResetIfEnabled();
      non_atomic_marking_state_->SetLiveBytes(p, 0);
    }
