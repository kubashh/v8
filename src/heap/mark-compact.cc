...
void MarkCompactCollector::SweepLargeSpace(LargeObjectSpace* space,
  ...
  for (auto it = space->begin(); it != space->end();) {
    ...
    if (liveness == SweepLargeSpaceLiveness::kClear) {
      Marking::MarkWhite(non_atomic_marking_state()->MarkBitFrom(object));
  ...
  {
    {
      ...
      SweepLargeSpace(heap()->lo_space(), SweepLargeSpaceLiveness::kClear);
    }
    {
      GCTracer::Scope sweep_scope(heap()->tracer(),
                                  GCTracer::Scope::MC_SWEEP_CODE_LO,
                                  ThreadKind::kMain);
      SweepLargeSpace(heap()->code_lo_space(), SweepLargeSpaceLiveness::kClear);
    }
    if (heap()->new_lo_space()) {
      ...
      GCTracer::Scope sweep_scope(heap()->tracer(),
                                  ...
                                  ThreadKind::kMain);
      // Keep liveness information around as it is used later during promotion
      // and cleared afterwards.
      SweepLargeSpace(heap()->new_lo_space(), SweepLargeSpaceLiveness::kKeep);
    }
    {
      GCTracer::Scope sweep_scope(
