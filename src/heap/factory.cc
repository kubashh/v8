...
Handle<WasmTypeInfo> Factory::NewWasmTypeInfo(
  ...
  result.AllocateExternalPointerEntry<kWasmTypeInfoForeignTag>(isolate());
  result.set_foreign_address<kWasmTypeInfoForeignTag>(isolate(), type_address);
...
Handle<WasmInternalFunction> Factory::NewWasmInternalFunction(
  ...
  DisallowGarbageCollection no_gc;
  result.AllocateExternalPointerEntry<kWasmInternalFunctionForeignTag>(
      isolate());
  result.set_foreign_address<kWasmInternalFunctionForeignTag>(isolate(),
                                                              opt_call_target);
  result.set_ref(*ref);
...
Handle<WasmExportedFunctionData> Factory::NewWasmExportedFunctionData(
  ...
  Handle<Foreign> sig_foreign =
      NewForeign<kWasmExportedFunctionDataSigForeignTag>(sig_address);
  Handle<WasmInternalFunction> internal =
...
Handle<WasmArray> Factory::NewWasmArrayFromMemory(uint32_t length,
  ...
  wasm::ValueType element_type =
      reinterpret_cast<wasm::ArrayType*>(
          map->wasm_type_info().foreign_address<kWasmTypeInfoForeignTag>())
          ->element_type();
