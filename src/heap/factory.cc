...
Handle<JSObject> Factory::NewJSObjectFromMap(
    DirectHandle<Map> map, AllocationType allocation,
    DirectHandle<AllocationSite> allocation_site, bool can_have_cpp_wrapper) {
  ...
  if (can_have_cpp_wrapper &&
      (IsJSApiObject(*js_obj) || IsJSGlobalObject(*js_obj) ||
       IsJSGlobalProxy(*js_obj) || IsJSSpecialApiObject(*js_obj) ||
       IsJSTypedArray(*js_obj) || IsJSPromise(*js_obj) ||
       IsJSArrayBuffer(*js_obj) || IsJSDataView(*js_obj) ||
       IsJSRabGsabDataView(*js_obj))) {
    InitializeCppHeapWrapper(*js_obj);
  }
