...
Handle<Script> Factory::NewScriptWithId(Handle<String> source, int script_id,
  ...
  script->set_name(ReadOnlyRoots(heap).undefined_value());
  script->set_id(script_id);
  script->set_line_offset(0);
  script->set_column_offset(0);
  script->set_context_data(ReadOnlyRoots(heap).undefined_value());
  script->set_type(Script::TYPE_NORMAL);
  script->set_wrapper(ReadOnlyRoots(heap).undefined_value());
  script->set_line_ends(ReadOnlyRoots(heap).undefined_value());
...
Handle<Module> Factory::NewModule(Handle<SharedFunctionInfo> code) {
  ...
  module->set_exception(ReadOnlyRoots(isolate()).the_hole_value());
  module->set_import_meta(ReadOnlyRoots(isolate()).the_hole_value());
