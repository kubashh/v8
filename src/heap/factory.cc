...
Handle<WasmTypeInfo> Factory::NewWasmTypeInfo(
  ...
  int allocated_size = static_cast<int>(WasmTypeInfo::kSupertypesOffset +
                                        supertypes.size() * kTaggedSize);
  WasmTypeInfo result = WasmTypeInfo::cast(
      AllocateRawWithImmortalMap(allocated_size, AllocationType::kOld, map));
  DisallowGarbageCollection no_gc;
  ...
  DCHECK_EQ(result.AllocatedSize(), allocated_size);
  return handle(result, isolate());
}
...
Handle<JSProxy> Factory::NewJSProxy(Handle<JSReceiver> target,
  ...
  if (target->IsCallable()) {
    if (target->IsConstructor()) {
      map = Handle<Map>::cast(isolate()->proxy_constructor_map());
    } else {
      map = Handle<Map>::cast(isolate()->proxy_callable_map());
    }
  } else {
    map = Handle<Map>::cast(isolate()->proxy_map());
  }
