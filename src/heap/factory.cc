...
MaybeHandle<String> NewStringFromBytes(Factory* factory,
  ...
  if (decoder.is_invalid()) {
    throw_invalid();
    DCHECK(factory->isolate()->has_pending_exception());
    return Handle<String>();
  }

  if (decoder.utf16_length() == 0) return factory->empty_string();

  if (decoder.is_one_byte()) {
    if (data.size() == 1) {
      return factory->LookupSingleCharacterStringFromCode(data[0]);
    }
    // Allocate string.
    Handle<SeqOneByteString> result;
    ASSIGN_RETURN_ON_EXCEPTION(
        factory->isolate(), result,
        factory->NewRawOneByteString(decoder.utf16_length(), allocation),
        String);
  ...
  ASSIGN_RETURN_ON_EXCEPTION(
      factory->isolate(), result,
      factory->NewRawTwoByteString(decoder.utf16_length(), allocation), String);
...
MaybeHandle<String> Factory::NewStringFromUtf8(
  ...
  return NewStringFromBytes<Utf8Decoder>(
      this, base::Vector<const uint8_t>::cast(string), allocation, handler);
}
...
MaybeHandle<String> Factory::NewStringFromWtf8(
  ...
  return NewStringFromBytes<Wtf8Decoder>(this, string, allocation, handler);
}
