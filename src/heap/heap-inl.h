...
void Heap::UpdateAllocationsHash(HeapObject object) {
  ...
  AllocationSpace allocation_space = memory_chunk->identity();
...
Heap* Heap::FromWritableHeapObject(const HeapObject obj) {
  ...
  SLOW_DCHECK(chunk->identity() != RO_SPACE ||
...
CodePageMemoryModificationScope::CodePageMemoryModificationScope(
  ...
  if (scope_active_) {
    DCHECK(chunk_->identity() == CODE_SPACE ||
           (chunk_->identity() == CODE_LO_SPACE));
