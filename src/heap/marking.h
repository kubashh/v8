...
class ConcurrentBitmap : public Bitmap {
 public:
  ...
  V8_EXPORT_PRIVATE bool AllBitsSetInRange(uint32_t start_index,
                                           uint32_t end_index);

  // Returns true if all bits in the range [start_index, end_index) are cleared.
  V8_EXPORT_PRIVATE bool AllBitsClearInRange(uint32_t start_index,
                                             uint32_t end_index);

  void Print();

  V8_EXPORT_PRIVATE bool IsClean();
