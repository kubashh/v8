...
Address LargePage::GetEndOfPayload(Address object_address,
                                   size_t object_size) const {
  if (executable() == EXECUTABLE) {
    return kNullAddress;
  }
  const size_t used_size = ::RoundUp((object_address - address()) + object_size,
                                     MemoryAllocator::GetCommitPageSize());
  return (used_size < CommittedPhysicalMemory()) ? address() + used_size
                                                 : kNullAddress;
}
...
#endif  // DEBUG
  Address free_start = page->GetEndOfPayload(object.address(), object_size);
