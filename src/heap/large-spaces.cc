...
void LargeObjectSpace::PromoteNewLargeObject(LargePage* page) {
  ...
  const bool is_shared_lo_space =
      identity() == SHARED_LO_SPACE || heap()->isolate()->is_shared();
  PtrComprCageBase cage_base(heap()->isolate());
  static_cast<LargeObjectSpace*>(page->owner())->RemovePage(page);
  page->ClearFlag(MemoryChunk::FROM_PAGE);
  if (is_shared_lo_space) page->SetFlag(MemoryChunk::IN_SHARED_HEAP);
  base::Optional<base::RecursiveMutexGuard> guard;
  if (is_shared_lo_space) guard.emplace(&allocation_mutex_);
  if (is_shared_lo_space) {
