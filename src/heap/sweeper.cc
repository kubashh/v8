...
int Sweeper::RawSweep(Page* p, FreeListRebuildingMode free_list_mode,
  ...
  // For regular sweeps, we increment the epoch counter on the MemoryChunk to
  // match the current MC epoch.
  p->increment_mark_compact_epoch();
  DCHECK_EQ(p->mark_compact_epoch(), heap_->mark_compact_collector()->epoch());
...
void Sweeper::PrepareToBeSweptPage(AllocationSpace space, Page* page) {
  ...
  DCHECK(!page->SweepingDone());
  page->set_mark_compact_epoch(heap_->mark_compact_collector()->epoch() - 1);
