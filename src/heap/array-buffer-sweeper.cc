...
void ArrayBufferSweeper::EnsureFinished() { EnsureFinished(false); }

void ArrayBufferSweeper::EnsureFinished(bool in_teardown) {
  ...
  switch (abort_result) {
    ...
    case TryAbortResult::kTaskRemoved: {
      ...
      if (job_.state == SweepingState::Prepared) {
        // This only happens when tearing down the heap. Avoid sweeping the list
        // in this case, because the next operation is going to release all
        // extensions anyway.
        CHECK(in_teardown);
