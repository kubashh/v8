...
void ArrayBufferSweeper::EnsureFinished() {
  ...
  if (heap_->isolate()->cancelable_task_manager()->TryAbort(job_.id) !=
...
void ArrayBufferSweeper::Prepare(SweepingScope scope) {
  CHECK_EQ(job_.state, SweepingState::None);
...
void ArrayBufferSweeper::Merge() {
  CHECK_EQ(job_.state, SweepingState::Done);
  young_.Append(&job_.young);
  old_.Append(&job_.old);
  job_.state = SweepingState::None;
}
...
ArrayBufferSweeper::SweepingJob::SweepingJob() : state(SweepingState::None) {}
...
ArrayBufferSweeper::SweepingJob ArrayBufferSweeper::SweepingJob::Prepare(
  ...
  job.old = old;
  job.scope = scope;
  job.id = CancelableTaskManager::kInvalidTaskId;
  job.state = SweepingState::Prepared;
  return job;
}

void ArrayBufferSweeper::SweepingJob::Sweep() {
  if (state == SweepingState::Done) return;
