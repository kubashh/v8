...
void ArrayBufferSweeper::RequestSweep(SweepingScope scope) {
  ...
  if (!heap_->IsTearingDown() && !heap_->ShouldReduceMemory() &&
    ...
    auto task = MakeCancelableTask(heap_->isolate(), [this] {
      ...
      base::MutexGuard guard(&sweeping_mutex_);
      // In case the main thread gets the mutex first and in consequence
      // completes the job itself, this job is already in uninitialized state.
      if (job_.state == SweepingState::Uninitialized) return;
      job_.Sweep();
    });
    job_.id = task->id();
...
void ArrayBufferSweeper::SweepingJob::Sweep() {
  if (state == SweepingState::Swept) return;
