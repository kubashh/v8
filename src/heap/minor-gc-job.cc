...
void MinorGCJob::ScheduleTask(Heap* heap) {
  ...
  // trigger, but may also occur before the trigger is reached.
  // For example, this method is called from the allocation observer for new
  // space. The observer step size is detemine based on the current task
  // trigger. However, due to refining allocated bytes after sweeping
  // (allocated bytes after sweeping may be less than live bytes during
  // marking), new space size may decrease while the observer step size remains
  // the same.
