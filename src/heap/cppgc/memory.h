...
V8_INLINE void CheckMemoryIsZapped(void* address, size_t size) {
  for (size_t i = 0; i < size; i++) {
    CHECK_EQ(kZappedValue, reinterpret_cast<ConstAddress>(address)[i]);
  }
}

V8_INLINE void CheckMemoryIsZero(void* address, size_t size) {
  for (size_t i = 0; i < size; i++) {
...
V8_INLINE void CheckMemoryIsInaccessible(void* address, size_t size) {
#if defined(V8_USE_MEMORY_SANITIZER)
