...
class V8_EXPORT_PRIVATE RawHeap final {
 public:
  ...
  // static constexpr size_t kFirstCustomSpaceIndex =
  //     static_cast<size_t>(SpaceType::kUserDefined1);
  // static_assert(kFirstCustomSpaceIndex + cppgc::Heap::kNumberOfCustomSpaces
  // ==
  //                   kMaxNumberOfSpaces,
  //               "Number of custom spaces inconsistent between cppgc::Heap and
  //               " "cppgc::internal::RawHeap.");

  // using SpaceType = cppgc::Heap::SpaceType;
  // using Spaces = std::array<std::unique_ptr<BaseSpace>, kMaxNumberOfSpaces>;
  ...
  BaseSpace* Space(size_t space_index) {
    const size_t index = static_cast<size_t>(space_index);
    DCHECK_GT(spaces_.size(), index);
    BaseSpace* space = spaces_[index].get();
    DCHECK(space);
    return space;
  ...
  size_t SpaceIndexForCustomSpace(size_t custom_space) {
    DCHECK_LT(custom_space, custom_spaces_);
    return kNumberOfRegularSpaces + custom_space;
  }
