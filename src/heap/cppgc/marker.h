...
class V8_EXPORT_PRIVATE MarkerBase {
 public:
  ...
  // trigger incremental/concurrent marking if needed. Returns true if
  // garbage collection finalization should be triggered.
  bool StartMarking();
 ...
 protected:
  class IncrementalMarkingTask : public v8::Task {
   public:
...
class V8_EXPORT_PRIVATE Marker final : public MarkerBase {
 ...
 protected:
  class IncrementalMarkingFinalizationTask : public v8::Task {
   public:
    using Handle = IncrementalMarkingTask::Handle;

    explicit IncrementalMarkingFinalizationTask(Heap*);
