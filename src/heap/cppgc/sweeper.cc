...
class Sweeper::SweeperImpl final {
 public:
  explicit SweeperImpl(RawHeap* heap, HeapStatsCollector* stats_collector)
      : heap_(heap),
...
Sweeper::Sweeper(RawHeap* heap, HeapStatsCollector* stats_collector)
    : impl_(std::make_unique<SweeperImpl>(heap, stats_collector)) {}
