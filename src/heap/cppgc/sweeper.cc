...
class Sweeper::SweeperImpl final {
 public:
  ...
  bool PerformSweepOnMutatorThread(double deadline_in_seconds,
                                   StatsCollector::ScopeId intenral_scope_id) {
    ...
    bool sweep_complete;
    {
      ...
      {
        StatsCollector::EnabledScope stats_scope(
            stats_collector_, intenral_scope_id, "deltaInSeconds",
            deadline_in_seconds - platform_->MonotonicallyIncreasingTime());
...
void Sweeper::PerformSweepOnMutatorThread(double deadline_in_seconds) {
  impl_->PerformSweepOnMutatorThread(deadline_in_seconds,
