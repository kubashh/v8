...
std::pair<bool, BasePage*> CanModifyObject(const void* object) {
  // object is guaranteed to be of type GarbageCollected, so getting the
  // BasePage is okay for regular and large objects.
  auto* base_page = BasePage::FromPayload(const_cast<void*>(object));
...
bool TryFree(const void* object) {
  bool can_modify;
  BasePage* base_page;
  std::tie(can_modify, base_page) = CanModifyObject(object);
  if (!can_modify) {
    return false;
  }

  auto& header = HeapObjectHeader::FromPayload(const_cast<void*>(object));
  header.Finalize();
  ...
  if (base_page->is_large()) {  // Large object.
    ...
    if (header.PayloadEnd() == lab.start()) {  // Returning to LAB.
