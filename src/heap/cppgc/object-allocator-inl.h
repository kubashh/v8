...
void* ObjectAllocator::AllocateObjectWithCustomSpacePolicy(size_t custom_space,
                                                           size_t size,
                                                           GCInfoIndex gcinfo) {
  const size_t allocation_size =
      RoundUp(size + sizeof(HeapObjectHeader), kAllocationGranularity);
  const size_t space_index = raw_heap_->SpaceIndexForCustomSpace(custom_space);
  return AllocateObjectOnSpace(
      NormalPageSpace::From(raw_heap_->Space(space_index)), allocation_size,
      gcinfo);
}
