...
void GCInfoTable::Resize() {
  ...
  const size_t size = current_table_end - reinterpret_cast<uint8_t*>(table_);
  if (size > 0 && (size % page_allocator_->CommitPageSize() == 0)) {
    CHECK(page_allocator_->SetPermissions(table_, size, PageAllocator::kRead));
  }
