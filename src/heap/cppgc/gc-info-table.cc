...
void GCInfoTable::Resize() {
  ...
  fprintf(stderr,
          "Resize: limit: %u new_limit: %u initial_limit: %u max_table_size: "
          "%zu allocate page size: %zu\n",
          limit_, new_limit, InitialTableLimit(), MaxTableSize(),
          page_allocator_->AllocatePageSize());
  CHECK(table_);
  ...
  if (limit_) {
    const size_t read_write_delta =
        kEntrySize *
        (limit_ == InitialTableLimit() ? InitialTableLimit() : limit_ / 2);
    CHECK(page_allocator_->SetPermissions(current_table_end - read_write_delta,
                                          read_write_delta,
                                          PageAllocator::kRead));
  }
