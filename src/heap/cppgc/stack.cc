...
void Stack::IterateAsanFakeFrameIfNecessary(StackVisitor* visitor,
  ...
  if (asan_fake_stack_) {
    ...
    if (real_stack_frame) {
      ...
      // of this stack.
      if (stack_start_ >= real_stack_frame && real_stack_frame >= stack_end) {
...
void Stack::IterateSafeStackIfNecessary(StackVisitor* visitor) const {
  ...
  CHECK_EQ(0u, stack_end & (kSafeStackAlignmentBytes - 1));
  CHECK_EQ(0u, stack_start & (kSafeStackAlignmentBytes - 1));
  void** current = reinterpret_cast<void**>(
