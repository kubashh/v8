...
MarkingVerifierBase::MarkingVerifierBase(
    HeapBase& heap, GCConfig::CollectionType collection_type,
...
void MarkingVerifierBase::Run(
    GCConfig::StackState stack_state, uintptr_t stack_end,
...
// before V8 compaction or compaction never runs with stack.
  ...
  if (stack_state == GCConfig::StackState::kMayContainHeapPointers) {
...
bool MarkingVerifierBase::VisitHeapObjectHeader(HeapObjectHeader& header) {
  ...
  if (collection_type_ == GCConfig::CollectionType::kMinor) {
...
MarkingVerifier::MarkingVerifier(HeapBase& heap_base,
                                 GCConfig::CollectionType collection_type)
