...
void Heap::ForceGarbageCollectionSlow(const char* source, const char* reason,
  ...
  internal::Heap::From(this)->CollectGarbage(
      {internal::GCConfig::CollectionType::kMajor, stack_state,
...
void CheckConfig(GCConfig config, HeapBase::MarkingType marking_support,
  ...
  CHECK_WITH_MSG(
      (config.collection_type != GCConfig::CollectionType::kMinor) ||
          (config.stack_state == GCConfig::StackState::kNoHeapPointers),
...
Heap::~Heap() {
  ...
  FinalizeIncrementalGarbageCollectionIfRunning(
      {GCConfig::CollectionType::kMajor,
       GCConfig::StackState::kMayContainHeapPointers,
...
void Heap::StartGarbageCollection(GCConfig config) {
  ...
  if (config.collection_type == GCConfig::CollectionType::kMajor)
...
void Heap::FinalizeGarbageCollection(GCConfig::StackState stack_state) {
  DCHECK(IsMarking());
  DCHECK(!in_no_gc_scope());
...
void Heap::FinalizeIncrementalGarbageCollectionIfNeeded(
    GCConfig::StackState stack_state) {
...
void Heap::StartIncrementalGarbageCollectionForTesting() {
  DCHECK(!IsMarking());
  DCHECK(!in_no_gc_scope());
  StartGarbageCollection({GCConfig::CollectionType::kMajor,
