...
void Heap::CollectGarbage(Config config) {
  ...
  FinalizeGarabageColllection(config.stack_state);
}

void Heap::StartIncrementalGarabageColllection(Config config) {
...
void Heap::FinalizeIncrementalGarabageColllectionIfRunning(Config config) {
...
void Heap::StartGarbageCollection(Config config) {
  ...
  marker_ = std::make_unique<Marker>(*this, platform_.get(), marking_config);
  marker_->StartMarking();
}

void Heap::FinalizeGarabageColllection(Config::StackState stack_state) {
  DCHECK(gc_in_progress_);
  DCHECK(!in_no_gc_scope());
...
Heap::IncrementalGarbageCollectionFinalizationTask::
    IncrementalGarbageCollectionFinalizationTask(Heap* heap)
    : heap_(heap), handle_(Handle::NonEmptyTag{}) {}

// static
Heap::IncrementalGarbageCollectionFinalizationTask::Handle
Heap::IncrementalGarbageCollectionFinalizationTask::Post(v8::TaskRunner* runner,
                                                         Heap* heap) {
  auto task =
      std::make_unique<IncrementalGarbageCollectionFinalizationTask>(heap);
  auto handle = task->handle_;
  runner->PostNonNestableTask(std::move(task));
  return handle;
}

void Heap::IncrementalGarbageCollectionFinalizationTask::Run() {
  if (handle_.IsCanceled()) return;

  heap_->FinalizeGarabageColllection(Heap::Config::StackState::kNoHeapPointers);
}
