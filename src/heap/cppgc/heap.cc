...
void Heap::CollectGarbage(Config config) {
  ...
  FinalizeGarbageColllection(config.stack_state);
}

void Heap::StartIncrementalGarbageColllection(Config config) {
...
void Heap::FinalizeIncrementalGarbageColllectionIfRunning(Config config) {
...
void Heap::FinalizeGarbageColllection(Config::StackState stack_state) {
  DCHECK(gc_in_progress_);
  DCHECK(!in_no_gc_scope());
...
void Heap::IncrementalGarbageCollectionFinalizationTask::Run() {
  ...
  heap_->FinalizeGarbageColllection(Heap::Config::StackState::kNoHeapPointers);
}
