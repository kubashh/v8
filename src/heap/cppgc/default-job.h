...
class DefaultJobImpl {
 public:
  ...
  explicit DefaultJobImpl(Key, std::unique_ptr<cppgc::JobTask> job_task)
      : job_task_(std::move(job_task)) {}

  ~DefaultJobImpl() {
    DCHECK_EQ(0, active_threads_.load(std::memory_order_relaxed));
    DCHECK(!CanRun());
    DCHECK(job_threads_.empty());
  }
  ...
  bool IsCompleted() { return !IsRunning(); }
  bool IsRunning() {
  ...
  bool CanRun() { return can_run_.load(std::memory_order_relaxed); }
 ...
 protected:
  ...
  void RunJobTask() {
    ...
    const uint8_t job_thread_id = NotifyJobThreadStart();
    JobDelegate delegate(this, job_thread_id);
    job_task_->Run(&delegate);
    NotifyJobThreadEnd(job_thread_id);
  }

  uint8_t NotifyJobThreadStart() {
    active_threads_.fetch_add(1, std::memory_order_relaxed);
    return 0;
  }
  void NotifyJobThreadEnd(uint8_t id) {
  ...
  // |active_threads_| doubles asa sync mechanism to let TSAN know all
  // threads terminated.
...
class DefaultJobImpl<Thread>::JobDelegate final : public cppgc::JobDelegate {
 public:
  explicit JobDelegate(DefaultJobImpl* job, uint8_t id)
      : job_(job), job_thread_id_(id) {}
