...
class MarkingStateBase {
 public:
  ...
  size_t marked_bytes() const {
    // |deferred_marked_bytes_| might temporarily exceed |marked_bytes_|. This
    // is because we increase |deferred_marked_bytes_| duriung a Trace method
    // but increase |marked_bytes_| only after returning from the Trace method.
    // However, since we never report marked bytes increasing
    // |deferred_marked_bytes_| and |marked_bytes_|, |deferred_marked_bytes_|
    // should never exceed |marked_bytes_| here.
    DCHECK_GE(marked_bytes_, deferred_marked_bytes_);
    return marked_bytes_ - deferred_marked_bytes_;
  }
