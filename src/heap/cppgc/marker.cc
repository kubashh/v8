...
void MarkerBase::IncrementalMarkingTask::Run() {
  ...
  if (mark_complete) {
    marker_->ScheduleIncrementalMarkingFinalizationIfNeeded();
  } else {
...
Marker::IncrementalMarkingFinalizationTask::IncrementalMarkingFinalizationTask(
    Heap* heap)
    : heap_(heap), handle_(Handle::NonEmptyTag{}) {}

// static
Marker::IncrementalMarkingFinalizationTask::Handle
Marker::IncrementalMarkingFinalizationTask::Post(v8::TaskRunner* runner,
                                                 Heap* heap) {
  auto task = std::make_unique<IncrementalMarkingFinalizationTask>(heap);
  auto handle = task->handle_;
  runner->PostNonNestableTask(std::move(task));
  return handle;
}

void Marker::IncrementalMarkingFinalizationTask::Run() {
  if (handle_.IsCanceled()) return;

  heap_->ForceFinalizeGarabageColllection(
      MarkingConfig::StackState::kNoHeapPointers);
}

Marker::Marker(Heap& heap, cppgc::Platform* platform, MarkingConfig config)
    : MarkerBase(heap, platform, config),
      marking_visitor_(heap, mutator_marking_state_),
      conservative_marking_visitor_(heap, mutator_marking_state_,
                                    marking_visitor_) {}

void Marker::ScheduleIncrementalMarkingFinalizationIfNeeded() {
  if (!platform_ || !foreground_task_runner_) return;
  DCHECK(!incremental_marking_handle_);
  incremental_marking_handle_ = IncrementalMarkingFinalizationTask::Post(
      foreground_task_runner_.get(), static_cast<Heap*>(&heap()));
}
