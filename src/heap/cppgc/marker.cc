...
void MarkerBase::IncrementalMarkingTask::Run() {
  ...
  if (marker_->IncrementalMarkingStep(stack_state_)) {
    // Incremental marking is done so should finalize GC. Incremental GC is
    // possible only via the GCInvoker, so getting here guarantees that either
    // non-nestable tasks or conservative stack scannnig are supported.
    DCHECK_IMPLIES(marker_->heap().stack_support() !=
                       HeapBase::StackSupport::kSupportsConservativeStackScan,
                   marker_->foreground_task_runner_->NonNestableTasksEnabled());
