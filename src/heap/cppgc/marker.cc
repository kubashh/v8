...
MarkerBase::IncrementalMarkingTask::IncrementalMarkingTask(
    MarkerBase* marker, bool is_finalization_task)
    : marker_(marker),
      is_finalization_task_(is_finalization_task),
...
MarkerBase::IncrementalMarkingTask::Post(v8::TaskRunner* runner,
  ...
  auto task =
      std::make_unique<IncrementalMarkingTask>(marker, is_finalization_task);
...
void MarkerBase::IncrementalMarkingTask::Run() {
  ...
  if (V8_UNLIKELY(is_finalization_task_)) {
    marker_->heap().ForceGarbageCollectionFinalizationIfSupported(
...
bool MarkerBase::StartMarking() {
  ...
  if (EnterIncrementalMarkingIfNeeded(config_, heap())) {
    ...
    // Performing atomic marking.
    FinishMarking(config_.stack_state);
    return true;
  }
}
...
void Marker::ScheduleIncrementalMarkingFinalizationIfNeeded() {
  if (!platform_ || !foreground_task_runner_) return;
  DCHECK(!incremental_marking_handle_);
  incremental_marking_handle_ = IncrementalMarkingTask::Post(
      foreground_task_runner_.get(), this, true /* is_finalization_task */);
}
