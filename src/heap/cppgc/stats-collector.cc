...
void StatsCollector::NotifySweepingCompleted(SweepingType sweeping_type) {
  ...
  DCHECK_IMPLIES(previous_.marking_type == StatsCollector::MarkingType::kAtomic,
                 previous_.scope_data[kIncrementalMark].InMicroseconds() == 0);
  DCHECK_IMPLIES(sweeping_type == StatsCollector::SweepingType::kAtomic,
                 previous_.scope_data[kIncrementalSweep].InMicroseconds() == 0);
  if (metric_recorder_) {
    MetricRecorder::GCCycle event = GetCycleEventForMetricRecorder(
        previous_.collection_type, previous_.marking_type, sweeping_type,
