...
#include "src/heap/cppgc/marker-utils.h"
    ...
    if (!DrainWorklistWithYielding(
            ...
            [&concurrent_marking_state,
             concurrent_marking_visitor](HeapObjectHeader* header) {
              ...
              TraceMarkedObject<HeapObjectHeader::AccessMode::kAtomic>(
...
ConcurrentMarkerBase::ConcurrentMarkerBase(
  ...
  concurrent_marking_handle_ =
      platform->PostJob(v8::TaskPriority::kUserVisible,
...
ConcurrentMarkerBase::~ConcurrentMarkerBase() {
  DCHECK(!concurrent_marking_handle_->IsRunning());
}

bool ConcurrentMarkerBase::MutatorIncrementalStep() {
