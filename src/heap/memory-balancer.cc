...
void MemoryBalancer::UpdateHeapLimit(size_t new_limit) {
  new_limit = std::max<size_t>(live_memory_ + min_heap_extra_space, new_limit) +
...
void MemoryBalancer::NotifyGC(size_t pre_gc_memory, size_t post_gc_memory) {
  ...
  if (heap_->tracer()->major_gc_time) {
    UpdateLiveMemoryMajorGC(post_gc_memory, pre_gc_memory,
                            (heap_->tracer()->major_gc_time.value() +
                             concurrent_gc_time_.exchange(0)) *
