...
void MemoryBalancer::RefreshLimit() {
  ...
  if (v8_flags.trace_memory_balancer) {
    heap_->isolate()->PrintWithTimestamp(
        "MemoryBalancer: allocation-rate=%.1lfb/ms gc-speed=%.1lfb/ms "
        "minium-limit=%.1lf computed-limit=%.1lfM new-limit=%.1lfM\n",
        major_allocation_rate_.value().rate(), major_gc_speed_.value().rate(),
