...
MemoryBalancer::MemoryBalancer(Heap* heap, base::TimeTicks time)
    : heap_(heap), last_measured_at_(time) {}
...
void MemoryBalancer::UpdateHeapLimit(size_t computed_limit) {
  size_t new_limit = std::max<size_t>(
      live_memory_after_gc_ + kMinHeapExtraSpace, computed_limit);
...
void MemoryBalancer::NotifyGC(size_t major_allocation_bytes,
                              base::TimeDelta major_allocation_duration,
                              size_t major_gc_bytes,
                              base::TimeDelta major_gc_duration) {
  UpdateAllocationRate(major_allocation_bytes, major_allocation_duration);
  UpdateGCSpeed(major_gc_bytes, major_gc_duration);
}
