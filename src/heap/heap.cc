...
void Heap::WriteBarrierForRange(HeapObject object, TSlot start_slot,
                                TSlot end_slot) {
  BasicMemoryChunk* source_chunk = BasicMemoryChunk::FromHeapObject(object);
  if (source_chunk->InReadOnlySpace()) return;
  MemoryChunk* source_page = MemoryChunk::cast(source_chunk);
