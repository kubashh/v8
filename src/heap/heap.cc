...
size_t Heap::ComputeMinorCollectionStepSize() {
  size_t interrupt_size =
      scavenge_job_ ? scavenge_job_->YoungGenerationTaskTriggerSize(this) : 0;

  if (new_space()->Size() + kTaggedSize < interrupt_size) {
    return RoundDown(interrupt_size - new_space()->Size(), kTaggedSize);
  } else {
    // Do not restrict LAB size in this case.
    return MemoryChunk::kPageSize;
  }
}

size_t Heap::ComputeMinorMarkingStepSize() {
  size_t interrupt_size = MinorMCTaskTriggerSize();
