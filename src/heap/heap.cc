...
void Heap::MembalancerUpdate() {
  if (UseMembalancer() && has_major_allocation && has_major_gc) {
    size_t new_limit =
        live_memory +
        sqrt(live_memory * (major_allocation_bytes / major_allocation_time) /
             (major_gc_bytes / major_gc_time) / CValue());
...
bool Heap::CollectGarbage(AllocationSpace space,
  ...
  if (major && UseMembalancer()) {
    live_memory = post_gc_memory;
    last_M_update_time = MonotonicallyIncreasingTimeInMs() * 1e6;
