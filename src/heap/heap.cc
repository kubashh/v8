...
StrongRootsEntry* Heap::RegisterStrongRoots(FullObjectSlot start,
  ...
  StrongRootsEntry* list = new StrongRootsEntry();
  list->prev = nullptr;
  list->next = strong_roots_head_;
  if (strong_roots_head_) {
    DCHECK_NULL(strong_roots_head_->prev);
    strong_roots_head_->prev = list;
  }
  list->start = start;
  list->end = end;
  strong_roots_head_ = list;
  return list;
}

void Heap::UnregisterStrongRoots(StrongRootsEntry* list) {
  base::MutexGuard guard(&strong_roots_mutex_);

  StrongRootsEntry* prev = list->prev;
  StrongRootsEntry* next = list->next;

  if (prev) prev->next = next;
  if (next) next->prev = prev;

  if (strong_roots_head_ == list) {
    DCHECK_NULL(prev);
