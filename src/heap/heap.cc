...
GcSafeCode Heap::GcSafeGetCodeFromInstructionStream(
  ...
  GcSafeCode code = GcSafeCode::unchecked_cast(istream.raw_code(kAcquireLoad));
  MapWord map_word = code.map_word(kRelaxedLoad);
  if (map_word.IsForwardingAddress()) {
    code = GcSafeCode::unchecked_cast(map_word.ToForwardingAddress(code));
  }
