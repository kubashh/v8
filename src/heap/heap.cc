...
void Heap::MembalancerUpdate() {
  if (v8_flags.memory_balancer && has_major_allocation && has_major_gc) {
    size_t new_limit =
        live_memory +
        sqrt(live_memory * (major_allocation_bytes / major_allocation_time) /
             (major_gc_bytes / major_gc_time) /
  ...
  if (major && v8_flags.memory_balancer) {
    ...
    if (tracer()->major_gc_time) {
      UpdateLiveMemoryMajorGC(
          ...
          (tracer()->major_gc_time.value() + concurrent_gc_time.exchange(0)) *
...
void Heap::UpdateHeapLimit(size_t new_limit) {
  new_limit = std::max<size_t>(live_memory + min_heap_extra_space, new_limit) +
              new_space_->Capacity();
