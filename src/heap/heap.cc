...
void Heap::IterateRoots(RootVisitor* v, base::EnumSet<SkipRoot> options) {
  ...
  if (!options.contains(SkipRoot::kUnserializable)) {
    ...
    // Iterate over the startup and shareable object caches unless serializing
    // or deserializing.
    ...
    if (isolate_->shared_isolate() == nullptr) {
      SerializerDeserializer::IterateShareableObjectCache(isolate_, v);
      v->Synchronize(VisitorSynchronization::kShareableObjectCache);
    }
  }
