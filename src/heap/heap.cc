  ...
  if (FLAG_stress_scavenge > 0 && stress_scavenge_observer_->HasRequestedGC()) {
    ...
    FinalizeIncrementalMarkingIncrementally(
        GarbageCollectionReason::kFinalizeMarkingViaStackGuard);
             ...
             AvailableAllocationSpace::kNoSpaceAvailable) {
    CollectAllGarbage(current_gc_flags_,
...
void Heap::update_backing_store_bytes(int64_t amount) {
  ...
  if (amount <= 0 || gc_state_ != NOT_IN_GC || isolate_->serializer_enabled())
    return;

  fprintf(stderr, "update_backing_store_bytes\n");

  AvailableAllocationSpace available = global_controller_->CheckAllocationLimit(
      GlobalSizeOfObjects(), global_memory_allocation_limit_);
  if (available == AvailableAllocationSpace::kNoSpaceAvailable) {
    fprintf(stderr, "update_backing_store_bytes - requesting GC\n");
    isolate()->stack_guard()->RequestGC();
  } else if (available == AvailableAllocationSpace::kSomeSpaceAvailable &&
             incremental_marking_->IsStopped() && FLAG_incremental_marking) {
    fprintf(stderr, "update_backing_store_bytes - requesting inc marking\n");
    StartIncrementalMarking(
        GCFlagsForIncrementalMarking(),
        GarbageCollectionReason::kGlobalAllocationLimit,
        GCCallbackFlags::kGCCallbackFlagCollectAllExternalMemory);
