...
void Heap::IterateWeakRoots(RootVisitor* v,
                            const RootIterationOptions& options) {
  DCHECK(!options.skip_weak);
  v->VisitRootPointer(Root::kStringTable, nullptr,
                      FullObjectSlot(&roots_table()[RootIndex::kStringTable]));
  v->Synchronize(VisitorSynchronization::kStringTable);
  if (!options.skip_external_string_table && !options.skip_unserializable) {
...
void Heap::IterateRoots(RootVisitor* v, const RootIterationOptions& options) {
  ...
  if (!options.skip_old_generation) {
    IterateBuiltins(v);
  ...
  if (!options.skip_unserializable) {
    if (!options.skip_global_handles) {
      if (options.skip_weak) {
