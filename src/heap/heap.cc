...
#ifdef DEBUG
  // There should be no invalidated old-to-new slots after each collection.
  for (SpaceIterator it(this); it.HasNext();) {
    Space* space = it.Next();
...
FixedArrayBase Heap::LeftTrimFixedArray(FixedArrayBase object,
  ...
  // Handle invalidated slots.
  if (MayContainRecordedSlots(new_object)) {
    // We have to clear slots in the free space to avoid stale slots.
...
void Heap::RemoveRecordedSlotsAfterObjectShrinking(HeapObject object,
  ...
  MemoryChunk::FromHeapObject(object)
      ->ResizeObjectWithInvalidatedSlots<OLD_TO_NEW>(object, new_size);
  if (incremental_marking()->IsCompacting()) {
    RememberedSet<OLD_TO_OLD>::RemoveRange(chunk, free_start, free_end,
                                           SlotSet::KEEP_EMPTY_BUCKETS);
    MemoryChunk::FromHeapObject(object)
        ->ResizeObjectWithInvalidatedSlots<OLD_TO_OLD>(object, new_size);
  }
}
