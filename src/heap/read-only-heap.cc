...
void ReadOnlyHeap::InitFromIsolate(Isolate* isolate) {
  ...
  if (IsReadOnlySpaceShared()) {
    ...
    if (COMPRESS_POINTERS_BOOL) {
      // With pointer compression each Isolate has its ReadOnlyHeap object.
      auto* read_only_heap =
      ...
      // Confirm the canonical versions of the ReadOnlySpace/ReadOnlyHeap from
      // the ReadOnlyArtifacts are not accidentally present in a real Isolate
      // (which might destroy them) and the ReadOnlyHeaps and Spaces are
