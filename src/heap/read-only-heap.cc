  ...
  if (IsReadOnlySpaceShared()) {
    ...
    if (des != nullptr) {
      ...
      if (!artifacts) {
        ...
        ro_heap->DeseralizeIntoIsolate(isolate, des, artifacts);
    ...
    // Shared ReadOnlyArtifacts is now created so re-assert that sharing is
    // allowed.
    DCHECK(IsReadOnlySpaceShared());
  } else {
    auto artifacts = std::make_shared<ReadOnlyArtifacts>();
    auto* ro_heap = CreateAndAttachToIsolate(isolate, artifacts);
    if (des != nullptr) ro_heap->DeseralizeIntoIsolate(isolate, des, artifacts);
  }
}

void ReadOnlyHeap::DeseralizeIntoIsolate(
    Isolate* isolate, ReadOnlyDeserializer* des,
    std::shared_ptr<ReadOnlyArtifacts> artifacts) {
  DCHECK_NOT_NULL(des);
