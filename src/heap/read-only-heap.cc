...
void ReadOnlyHeap::SetUp(Isolate* isolate, ReadOnlyDeserializer* des) {
  ...
  if (des != nullptr) {
    ...
    // ReadOnlyHeap intialization actually happens via the Heap so we have to
    // lock here and unlock in OnCreateHeapObjectsComplete.
    read_only_heap_creation_mutex_.Pointer()->Lock();
    std::shared_ptr<ReadOnlyArtifacts> artifacts =
        read_only_artifacts_.Get().lock();
    CHECK(!artifacts);
    artifacts = InitializeSharedReadOnlyArtifacts();
...
void ReadOnlyHeap::OnCreateHeapObjectsComplete(Isolate* isolate) {
  ...
  InitFromIsolate(isolate);
#ifdef V8_SHARED_RO_HEAP
  read_only_heap_creation_mutex_.Pointer()->Unlock();
#endif  // V8_SHARED_RO_HEAP
}
...
ReadOnlyHeap* ReadOnlyHeap::CreateAndAttachToIsolate(
  ...
  artifacts->set_read_only_heap(
      std::make_unique<ReadOnlyHeap>(new ReadOnlySpace(isolate->heap())));
  isolate->SetUpFromReadOnlyArtifacts(artifacts);
...
void ReadOnlyHeap::InitFromIsolate(Isolate* isolate) {
  ...
  shared_ro_heap_->read_only_space_ = artifacts->shared_read_only_space();
