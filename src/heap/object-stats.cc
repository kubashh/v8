...
bool ObjectStatsCollectorImpl::CanRecordFixedArray(FixedArrayBase* array) {
  return array != ReadOnlyRoots(heap_).empty_fixed_array() &&
         array != ReadOnlyRoots(heap_).empty_sloppy_arguments_elements() &&
         array != ReadOnlyRoots(heap_).empty_slow_element_dictionary() &&
         array != ReadOnlyRoots(heap_).empty_property_dictionary();
}
