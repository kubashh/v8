...
void PerContextObjectStats::ReportPerContext(PerContextObjectStats* precise,
                                             PerContextObjectStats* approx) {
  PrintF("---------------------------------------------\n");
  PrintF(
      "Precise, inferred KB\tPrecise, inferred %%\tApprox, inferred "
      "KB\tApprox, inferred %%\tPrecise, estimated KB\tApprox, estimated KB\n");
  double total_bytes = precise->total_stats_->total_size();
  double precise_inferred = 0;
  double approx_inferred = 0;
  for (const auto& it : precise->context_stats_) {
    precise_inferred += it.second->total_size();
  }

  for (const auto& it : approx->context_stats_) {
    approx_inferred += it.second->total_size();
  }

  for (const auto& it : precise->context_stats_) {
    auto ctx = it.first;
    double precise_bytes = precise->context_stats_[ctx]->total_size();
    double precise_pct = precise_bytes / total_bytes * 100;
    double approx_bytes = 0, approx_pct = 0;

    if (approx->context_stats_.count(ctx)) {
      approx_bytes = approx->context_stats_[ctx]->total_size();
      approx_pct = approx_bytes / total_bytes * 100;
    }

    double precise_estimate =
        total_bytes * (precise_bytes / precise_inferred) / KB;
    double approx_estimate =
        total_bytes * (approx_bytes / approx_inferred) / KB;

    PrintF("%012lx\t%3.2f\t%5.f\t%3.2f\t%5.f\t%5.f\t%5.f\t%3.2f\n", ctx,
           precise_pct, precise_bytes / KB, approx_pct, approx_bytes / KB,
           precise_estimate, approx_estimate,
           abs(approx_estimate - precise_estimate) / precise_estimate * 100);
  }
  PrintF("---------------------------------------------\n");
}
  ...
  IterateHeap(heap, [this, &heap, &total_size](HeapObject obj, int size) {
    ...
    if (context.type == ResultType::kKnownContext) {
      ...
      if (obj->IsJSWeakCollection()) {
        JSWeakCollection js = JSWeakCollection::cast(obj);
        context_map_[HeapObject::cast(js->table())->address()] = {
            ResultType::kInferredContext, context.context};
      }
      if (obj->IsJSCollection()) {
        JSCollection js = JSCollection::cast(obj);
        context_map_[HeapObject::cast(js->table())->address()] = {
            ResultType::kInferredContext, context.context};
      }
      if (obj->IsMap()) {
        ...
        if (map->owns_descriptors()) {
          context_map_[map->instance_descriptors()->address()] = {
              ResultType::kInferredContext, context.context};
        }
      }
      if (obj->IsJSFunction()) {
        JSFunction js = JSFunction::cast(obj);
        if (js->has_feedback_vector()) {
          context_map_[js->feedback_vector()->address()] = {
              ResultType::kInferredContext, context.context};
        }
      }
    }
