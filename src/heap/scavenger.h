...
class CopiedRangesList {
 public:
  ...
  bool Process(Callback callback) {
    ...
    do {
      Address start = current_start_;
      Address end = current_end_;
      current_start_ = current_end_ = nullptr;
      processed = ProcessRange(callback, start, end);
      AddressRange entry;
      while (copied_list_.Pop(&entry)) {
        ProcessRange(callback, entry.first, entry.second);
        processed = true;
      }
    } while (current_start_ != nullptr);
    return processed;
  }

 private:
  template <typename Callback>
  bool ProcessRange(Callback callback, Address start, Address end) {
    bool processed = false;
