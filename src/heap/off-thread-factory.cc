...
void OffThreadFactory::Publish(Isolate* isolate) {
  ...
  std::vector<Handle<Script>> script_handles;
  {
    ...
    for (RelativeSlot relative_slot : string_slots_) {
      ...
      String string =
          String::cast(ACQUIRE_READ_FIELD(obj, relative_slot.slot_offset));
