...
class MemoryAllocator::Unmapper::UnmapFreeMemoryJob : public JobTask {
 ...
 private:
  void RunImpl(JobDelegate* delegate) {
    unmapper_->PerformFreeMemoryOnQueuedChunks(FreeMode::kProtectPooled,
                                               delegate);
...
void MemoryAllocator::Unmapper::FreeQueuedChunks() {
  if (!heap_->IsTearingDown() && FLAG_concurrent_sweeping) {
    ...
    PerformFreeMemoryOnQueuedChunks(FreeMode::kProtectPooled);
  }
}
...
void MemoryAllocator::Unmapper::PerformFreeMemoryOnQueuedChunks(
  ...
  if (mode == MemoryAllocator::Unmapper::FreeMode::kUncommitPooled) {
    // The previous loop protected any pages marked as pooled and added them
    // to the pooled list. In case of kUncommitPooled we need to uncommit them
    // though.
    while ((chunk = GetMemoryChunkSafe(kPooled)) != nullptr) {
      allocator_->UncommitPooledChunk(chunk);
      if (delegate && delegate->ShouldYield()) return;
    }
  }
  PerformFreeMemoryOnQueuedNonRegularChunks();
...
bool MemoryAllocator::ProtectMemory(VirtualMemory* reservation) {
  size_t size = reservation->size();
...
void MemoryAllocator::UncommitMemory(v8::PageAllocator* page_allocator,
...
void MemoryAllocator::FreeReadOnlyPage(ReadOnlyPage* chunk) {
  ...
  if (reservation->IsReserved()) {
    ...
    UncommitMemory(allocator, chunk->address(),
...
void MemoryAllocator::UncommitPooledChunk(MemoryChunk* chunk) {
  // Pooled pages cannot be touched anymore as their memory is protected.
  // Pooled pages are not-executable.
  UncommitMemory(data_page_allocator(), chunk->address(),
