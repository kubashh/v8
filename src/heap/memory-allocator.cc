...
void MemoryAllocator::Pool::DiscardPooledChunks() {
  ...
  discarded_chunks_.insert(discarded_chunks_.end(), pooled_chunks_.begin(),
                           pooled_chunks_.end());
  fprintf(stderr, "DiscardPooledChunks, discarded_chunks: %lu\n", discarded_chunks_.size());
...
void MemoryAllocator::Free(MemoryAllocator::FreeMode mode, MemoryChunk* chunk) {
  ...
  switch (mode) {
    ...
    case FreeMode::kPostpone:
      ...
      fprintf(stderr, "Overall queued pages: %lu\n", queued_pages_to_be_freed_.size());
      break;
    case FreeMode::kPool:
...
MemoryAllocator::AllocateUninitializedPageFromPool(Space* space) {
  void* chunk = pool()->TryGetPooledMemoryChunk();
...
void MemoryAllocator::ReleaseQueuedPages() {
  fprintf(stderr, "Releasing %lu pages\n", queued_pages_to_be_freed_.size());
