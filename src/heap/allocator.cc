...
bool Allocator::IsLabEmpty() { return lab_.IsEmpty(); }
...
void Allocator::InvokeAllocationObservers(Address soon_object,
  ...
  DCHECK_NE(step, 0);
  DCHECK_GE(step, allocated_bytes);
  size_t rounded_step = ((step - 1) / object_alignment_) * object_alignment_;
  if (lab_.limit - lab_.published_top > rounded_step) {
    lab_.limit = lab_.published_top + rounded_step;
  }
}
