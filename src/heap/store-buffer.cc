...
void StoreBuffer::SetUp() {
  // Allocate the buffer memory aligned to kStoreBufferSize. This lets us use
  // a bit test to detect the ends of the buffers.
  const size_t requested_size = kStoreBufferSize * kStoreBuffers;
  void* hint = AlignedAddress(heap_->GetRandomMmapAddr(), kStoreBufferSize);
  VirtualMemory reservation;
  if (!AlignedAllocVirtualMemory(requested_size, kStoreBufferSize, hint,
                                 &reservation)) {
  ...
  // Set RW permissions only on the memory we use.
