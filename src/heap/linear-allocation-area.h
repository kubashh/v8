...
class V8_EXPORT_PRIVATE LabOriginalLimits final {
 ...
 public:
  ...
  void AdvanceTopsAndResetPendingObjectHandles(
      std::initializer_list<LabHandle*> labs,
      std::initializer_list<Address> new_tops,
      std::initializer_list<PendingObjectHandle*> objects);
 ...
 private:
  ...
  void UpdateSnapshot(Snapshot&, size_t current_version);
  void UpdatePartialSnapshot(std::initializer_list<BaseHandle*>, Snapshot&,
