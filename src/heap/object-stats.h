...
class ContextMapper {
 public:
  ...
  virtual Result ContextOf(HeapObject heap_object) = 0;
};

class PreciseContextMapper : public ContextMapper {
 public:
  explicit PreciseContextMapper(Heap* heap);
  Result ContextOf(HeapObject heap_object) override {
...
class ApproxContextMapper : public ContextMapper {
 public:
  explicit ApproxContextMapper(Heap* heap);
  Result ContextOf(HeapObject heap_object) override {
    auto it = context_map_.find(heap_object.address());
    if (it == context_map_.end())
      return {ResultType::kUnknown, NativeContext()};
...
class ObjectStatsImpl : public ObjectStats {
 ...
 private:
  ...
  std::unordered_set<Address> recorded_;
};
...
class PerContextObjectStats : public ObjectStats {
 public:
  ...
  static void Report(PerContextObjectStats* precise,
                     PerContextObjectStats* approx);

  static void ReportPerContext(PerContextObjectStats* precise,
                               PerContextObjectStats* approx);
 ...
 private:
  ...
  static void ReportInstanceType(PerContextObjectStats* precise,
                                 PerContextObjectStats* approx,
                                 const char* name, int index);
  static void ReportTotals(PerContextObjectStats* precise,
                           PerContextObjectStats* approx);

  static void ReportItem(const char* name, size_t precise_total,
                         const std::map<Address, size_t>& precise,
                         size_t approx_total,
                         const std::map<Address, size_t>& approx);
  std::map<Address, size_t> GetPerContext(int index);
  std::map<Address, size_t> GetTotalsPerContext();

  ContextMapper* context_mapper_ = nullptr;
  std::map<Address, std::unique_ptr<ObjectStatsImpl>> context_stats_;
