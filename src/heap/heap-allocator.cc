...
void HeapAllocator::Setup(LinearAllocationArea* new_allocation_info,
  ...
  if ((heap_->new_space() || v8_flags.sticky_mark_bits) &&
    ...
    new_space_allocator_.emplace(
        local_heap_, heap_->new_space(),
        MainAllocator::BlackAllocation::kAlwaysDisabled, new_allocation_info);
  }

  const MainAllocator::BlackAllocation black_allocation_for_non_new_objects =
      v8_flags.sticky_mark_bits
          ? MainAllocator::BlackAllocation::kAlwaysEnabled
          : MainAllocator::BlackAllocation::kEnabledOnMarking;
  old_space_allocator_.emplace(local_heap_, heap_->old_space(),
                               black_allocation_for_non_new_objects,
                               old_allocation_info);
