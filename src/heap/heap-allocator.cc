...
void HeapAllocator::UpdateAllocationTimeout() {
  ...
  if (interval >= 0) {
    ...
    // Interval might have gotten disabled through %SetAllocationTimeout at
    // runtime when running with --gc-interval=X. We need to set
    // allocation_timeout_ to something >0 in order for the next allocation to
    // succeed and not run into an OOM error. Since the GC interval is not
    // active anymore simply use the maximum number possible in order to not
    // trigger a GC early.
    allocation_timeout_ = INT_MAX;
  }
}
