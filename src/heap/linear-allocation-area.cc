...
void LabOriginalLimits::UpdateSnapshot(Snapshot& snapshot,
...
void LabOriginalLimits::UpdatePartialSnapshot(
...
void LabOriginalLimits::AdvanceTopsAndResetPendingObjectHandles(
    std::initializer_list<LabHandle*> labs,
    std::initializer_list<Address> lab_tops,
    std::initializer_list<PendingObjectHandle*> objects) {
  DCHECK_EQ(labs.size(), lab_tops.size());
  base::SharedMutexGuard<base::MutexSharedType::kExclusive> lock(&mutex_);
  for (size_t i = 0; i < labs.size(); ++i) {
    const auto* handle = *std::next(labs.begin(), i);
    if (!handle) continue;
    auto top = *std::next(lab_tops.begin(), i);
    DCHECK_LT(handle->node_index_, nodes_.size());
    auto& node = nodes_[handle->node_index_];
    DCHECK_LE(node.lab.top, top);
    DCHECK_GE(node.lab.limit, top);
