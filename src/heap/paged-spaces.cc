...
bool PagedSpace::RefillLabFromFreeList(size_t min_size, size_t max_size,
  ...
  FreeSpace new_node = free_list_->Allocate(min_size, &new_node_size);
...
bool PagedSpace::RefillLab(ThreadKind thread_kind, size_t min_size,
                           ...
                           Address* limit) {
  // TODO: aligment.
  if (RefillLabFromFreeList(min_size, max_size, top, limit)) {
    return true;
  }
  ...
  if (collector->sweeping_in_progress()) {
    ...
    if (ContributeToSweeping(min_size, kMaxPagesToSweep) &&
        RefillLabFromFreeList(min_size, max_size, top, limit)) {
      return true;
    }
  }
  if (is_compaction_space()) {
  ...
  if (heap()->ShouldExpandOldGenerationOnSlowAllocation(local_heap) &&
      heap()->CanExpandOldGeneration(thread_kind, AreaSize())) {
