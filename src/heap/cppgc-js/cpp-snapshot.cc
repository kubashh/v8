...
class GraphBuildingVisitor final : public JSVisitor {
 public:
  ...
  GraphBuildingVisitor(CppGraphBuilderImpl& graph_builder,
                       const ParentScope& parent_scope, std::string edge_name)
      : JSVisitor(cppgc::internal::VisitorFactory::CreateKey()),
        graph_builder_(graph_builder),
        parent_scope_(parent_scope),
...
void CppGraphBuilderImpl::Run() {
  ...
  states_.ForAllVisibleStates([this](StateBase* state_base) {
    ...
    {
      GraphBuildingVisitor object_visitor_for_eager_ephemerons(
          *this, parent_scope, "part of key -> value pair in ephemeron table");
      state.ForAllEagerEphemeronEdges(
          [&object_visitor_for_eager_ephemerons](
              const void* value, cppgc::TraceCallback callback) {
            callback(&object_visitor_for_eager_ephemerons, value);
          });
    }
  });
  // Add roots.
