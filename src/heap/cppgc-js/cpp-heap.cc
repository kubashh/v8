...
class V8_EXPORT_PRIVATE UnifiedHeapMarker : public cppgc::internal::Marker {
 public:
...
void CppHeap::RegisterV8References(
  ...
  for (auto& tuple : embedder_fields) {
    ...
    static_cast<UnifiedHeapMarker*>(marker_.get())->AddObject(tuple.first);
  }
}
