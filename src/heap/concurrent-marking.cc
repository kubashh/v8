...
class SlotSnapshot {
 public:
  ...
  void add(RelocInfo* reloc_info, HeapObject target) {
    typed_slots_.push_back({reloc_info, target});
  }
 ...
 private:
  ...
  class SlotSnapshottingVisitor final : public ObjectVisitor {
   public:
    ...
    void VisitCodeTarget(Code host, RelocInfo* rinfo) final {
      slot_snapshot_->add(rinfo, rinfo->target_object());
    }
  ...
  void VisitPointersInSnapshot(HeapObject host, const SlotSnapshot& snapshot) {
    ...
    for (const TypedSlot& slot : snapshot.typed_slots()) {
      MarkObject(host, slot.target);
      RecordRelocSlot(Code::cast(host), slot.rinfo, slot.target);
    }
  }
