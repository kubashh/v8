    ...
    while (marking_worklists_local.Pop(&heap_object)) {
      ...
      //                YoungGenerationMarkingVisitationMode::kConcurrent)) {
        if (current_marked_bytes >= kBytesUntilInterruptCheck ||
          ...
          if ((marking_mode == YoungGenerationMarkingVisitationMode::kConcurrent) && heap->minor_mark_sweep_collector()->is_in_atomic_pause()) {
            visitor.PublishWorklists();
            return marked_bytes +
                   MinorMarkingLoopImpl<
                       YoungGenerationMarkingVisitationMode::kParallel>(
                       heap, delegate, local_pretenuring_feedback);
          }
          objects_processed = 0;
