...
using YoungGenerationConcurrentMarkingVisitor =
...
V8_INLINE Map GetMapFromObject(HeapObject heap_object, Isolate* isolate) {
  if ((marking_mode == YoungGenerationMarkingVisitorMode::kConcurrent)) {
    return heap_object->map(isolate);
  } else {
    return Map::cast(*heap_object->map_slot());
  }
}
...
V8_INLINE size_t MinorMarkingLoopImpl(
  ...
  do {
    ...
    while (marking_worklists_local.Pop(&heap_object)) {
      if ((marking_mode == YoungGenerationMarkingVisitorMode::kConcurrent) &&
          IsYoungObjectInLab(heap, heap_object)) {
        DCHECK(!delegate->IsJoiningThread());
        visitor.marking_worklists_local().PushOnHold(heap_object);
      } else {
        Map map = GetMapFromObject<marking_mode>(heap_object, isolate);
  ...
  } while (!should_yield && remembered_sets.ProcessNextItem(&visitor));
...
void ConcurrentMarking::RunMinor(JobDelegate* delegate) {
...

  {
    ...
    //     "Marking a InstructionStream object requires write access to the "
    //     "Code page header");
    // TODO(v8:13012): This requires an atomic `IsInAtomicPause()` check.
    if (heap_->tracer()->IsInAtomicPause()) {
      marked_bytes =
          MinorMarkingLoopImpl<YoungGenerationMarkingVisitorMode::kParallel>(
              heap_, delegate, visitor, remembered_sets);
    } else {
      marked_bytes =
          MinorMarkingLoopImpl<YoungGenerationMarkingVisitorMode::kConcurrent>(
              heap_, delegate, visitor, remembered_sets);
    }
  }
