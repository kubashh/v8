 ...
 public:
  ...
  explicit ConcurrentMarkingVisitor(
    ...
    if (FLAG_stress_flush_bytecode) {
      bytecode_flush_mode_ = BytecodeFlushMode::StressFlushBytecode;
    } else if (FLAG_flush_bytecode) {
      bytecode_flush_mode_ = BytecodeFlushMode::FlushBytecode;
    } else {
      bytecode_flush_mode_ = BytecodeFlushMode::DoNotFlushBytecode;
    }
  }
  ...
  int VisitSharedFunctionInfo(Map map, SharedFunctionInfo shared_info) {
    ...
    if (bytecode_flush_mode_ != BytecodeFlushMode::DoNotFlushBytecode &&
        shared_info->ShouldFlushBytecode(
            bytecode_flush_mode_ == BytecodeFlushMode::StressFlushBytecode)) {
