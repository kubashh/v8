  ...
  class SlotSnapshottingVisitor final : public ObjectVisitor {
   public:
    ...
    void VisitCodeTarget(Code host, RelocInfo* rinfo) final {
      // We are not guaranteed to read a valid heap object pointer here.
      Object target = Code::GetObjectFromTargetAddress(rinfo->target_address());
      slot_snapshot_->add(rinfo, target);
    }

    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) final {
      // We are not guaranteed to read a valid heap object pointer here.
      Object target = rinfo->target_object_as_object();
      slot_snapshot_->add(rinfo, target);
    }
