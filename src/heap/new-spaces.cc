...
void NewSpace::VerifyImpl(Isolate* isolate, const Page* page,
  ...
  while (address != top()) {
    if (!Page::IsAlignedToPageSize(address)) {
      // The allocation pointer should not be in the middle of an object.
      CHECK(!Page::FromAddress(address)->ContainsLimit(top()) ||
            address < top());
      ...
      address = current_page->area_start();
    }
  }
