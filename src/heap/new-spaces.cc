...
bool SemiSpace::RefillLabFromCurrentPage(int min_size, int max_size,
  ...
  int aligned_size = min_size + filler_size;
  if (old_top + aligned_size <= high) {
    return false;
  }
  *top = old_top;
  *limit = old_top +
           std::max(static_cast<size_t>(aligned_size),
                    std::min(high - old_top, static_cast<size_t>(max_size)));
  current_top_ = *limit;
  return true;
}

bool SemiSpace::RefillLab(int min_size, int max_size,
                          AllocationAlignment alignment,
                          AllocationOrigin origin, Address* top,
                          Address* limit) {
  if (*top != kNullAddress) {
...
void SemiSpace::Free(Address* top, Address* limit) {
...
bool NewSpace::RefillLab(int min_size, int max_size,
                         AllocationAlignment alignment, AllocationOrigin origin,
                         Address* top, Address* limit) {
  return to_space_.RefillLab(min_size, max_size, alignment, origin, top, limit);
}

void NewSpace::Free(Address* top, Address* limit) {
  to_space_.Free(top, limit);
}
...
void NewSpace::Verify(Isolate* isolate) {
  ...
  while (true) {
    if (!Page::IsAlignedToPageSize(current)) {
      ...
      Page* current_page = Page::FromAllocationAreaAddress(current);
      if (current_page == to_space_.current_page()) break;
      Page* page = current_page->next_page();
