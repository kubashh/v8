...
class MemoryAllocator {
 public:
  ...
  class Unmapper {
   ...
   private:
    ...
    enum class FreeMode {
      // Protects pages before adding them to the pool.
      kProtectPooled,

      // Uncommits pooled pages. Only used on tear down and last-resort GCs.
      kUncommitPooled,
    };
  ...
  enum FreeMode {
    // Uncommits page immediately on the main thread.
    kImmediately,

    // Uncommits page on background thread.
    kConcurrently,

    // Protects page on background thread and adds it to pool. Avoids the
    // un-commit and re-commit cycle.
  ...
  void Free(MemoryAllocator::FreeMode mode, MemoryChunk* chunk);
 ...
 private:
  ...
  bool ProtectMemory(VirtualMemory* reservation);

  // Uncommits the given memory region.
  void UncommitMemory(v8::PageAllocator* page_allocator, Address addr,
                      size_t size);
  ...
  // Uncommits a pooled page. Only used on tear-down and last-resort GCs.
  void UncommitPooledChunk(MemoryChunk* chunk);
