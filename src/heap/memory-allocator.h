...
class MemoryAllocator {
 public:
  ...
  //
  // TODO(syg): Give code_range a better name since the CodeRange class exists.
  const base::AddressRegion& code_range() const {
#ifdef V8_COMPRESS_POINTERS_IN_SHARED_CAGE
    return CodeRange::GetProcessWideRange()->code_region();
#else
    return code_range_.code_region();
#endif
  }
 ...
 private:
  ...
  //
  // When V8_COMPRESS_POINTERS_IN_SHARED_CAGE, there is a process wide range
  // accessible via CodeRange::GetProcessWideRange().
  ...
  // or a BoundedPageAllocator from code_range_ (when pointer compression is
