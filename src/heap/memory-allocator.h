...
class MemoryAllocator {
 public:
  ...
  class V8_EXPORT_PRIVATE Pool {
   public:
    ...
    void AddMemoryChunk(MemoryChunk* chunk) {
      ...
      fprintf(stderr, "Overall pooled pages: %lu\n", pooled_chunks_.size());
    }

    MemoryChunk* TryGetPooledMemoryChunk() {
      if (!pooled_chunks_.empty()) {
        MemoryChunk* chunk = pooled_chunks_.back();
        pooled_chunks_.pop_back();
        return chunk;
      } else if (!discarded_chunks_.empty()) {
        MemoryChunk* chunk = discarded_chunks_.back();
        discarded_chunks_.pop_back();
        return chunk;
      }
      return nullptr;
    }

    void DiscardPooledChunks();
    void TearDown();
   ...
   private:
    ...
    std::vector<MemoryChunk*> discarded_chunks_;
 ...
 private:
  ...
  // Discards all the system pages inside the reservation. Returns true if it
  // succeeded and false otherwise.
  bool DiscardMemory(VirtualMemory* reservation);
