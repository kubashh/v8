...
size_t HeapController::CalculateOldGenerationAllocationLimit(
  ...
  size_t limit = static_cast<size_t>(old_gen_size * factor);
  limit = Max(limit,
              old_gen_size + MinimumAllocationLimitGrowingStep(growing_mode));
  limit += new_space_capacity;
  size_t halfway_to_the_max = (old_gen_size + max_old_generation_size) / 2;
  limit = Min(limit, halfway_to_the_max);

  if (FLAG_trace_gc_verbose) {
    heap_->isolate()->PrintWithTimestamp(
        "CalcOldGenAllocLimit: old size: %" PRIuS " KB, new limit: %" PRIuS
        " KB (%.1f)\n",
