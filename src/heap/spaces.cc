...
void Page::MergeOldToNewRememberedSets() {
  ...
  if (slot_set_[OLD_TO_NEW]) {
    // While there can be old-to-new slots when forcing promotion
    // of young live objects, there can't be slots both in the
    // old-to-new and the sweeping remembered set.
    DCHECK(!FLAG_always_promote_young_in_mc);
...
void MemoryChunk::InvalidateRecordedSlots(HeapObject object) {
  ...
  if (!FLAG_always_promote_young_in_mc || slot_set_[OLD_TO_NEW] != nullptr)
