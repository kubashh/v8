...
template V8_EXPORT_PRIVATE void
MemoryChunk::ResizeObjectWithInvalidatedSlots<OLD_TO_NEW>(HeapObject object,
                                                          int size);

template <RememberedSetType type>
void MemoryChunk::ResizeObjectWithInvalidatedSlots(HeapObject object,
                                                   int size) {
  if (!ShouldSkipEvacuationSlotRecording()) {
    if (invalidated_slots<type>() == nullptr) {
      return;
    ...
    if (it != invalidated_slots<type>()->end()) {
      DCHECK_LE(size, it->second);
      it->second = size;
    }
  }
}

template bool MemoryChunk::RegisteredObjectWithInvalidatedSlotsOrNotNeeded<
    OLD_TO_NEW>(HeapObject object);
template bool MemoryChunk::RegisteredObjectWithInvalidatedSlotsOrNotNeeded<
    OLD_TO_OLD>(HeapObject object);

template <RememberedSetType type>
bool MemoryChunk::RegisteredObjectWithInvalidatedSlotsOrNotNeeded(
    HeapObject object) {
  if (ShouldSkipEvacuationSlotRecording()) {
    // Invalidated slots do not matter if we are not recording slots.
    return true;
  }
  return RegisteredObjectWithInvalidatedSlots<type>(object);
}
