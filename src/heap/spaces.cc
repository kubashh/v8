...
Address SpaceWithLinearArea::ComputeLimit(Address start, Address end,
  ...
  if (identity() == NEW_SPACE && heap()->deserialization_complete()) {
    step_size = std::min(step_size, heap()->ComputeMinorCollectionStepSize());

    if (v8_flags.minor_mc && !heap()->incremental_marking()->IsMarking()) {
      step_size = std::min(step_size, heap()->ComputeMinorMarkingStepSize());
    }
  }

  DCHECK_LE(start + step_size, end);
  if (min_size == 0) {
    return start + step_size;
  } else {
