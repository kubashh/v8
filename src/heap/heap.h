...
#define RIGHT_TRIMMABLE_ARRAY_LIST(V) \
  ...
  int64_t external_memory_hard_limit() {
    if (v8_flags.external_memory_relaxed_limits) {
      return external_memory_limit();
    }
    return max_old_generation_size() / 2;
  }

  int64_t external_memory_limit() {
    size_t external_memory_at_last_gc =
        external_memory_at_last_gc_.load(std::memory_order_relaxed);
    if (v8_flags.external_memory_relaxed_limits) {
      return std::max(
          (external_memory_at_last_gc + max_global_memory_size_) / 2,
          external_memory_at_last_gc + kExternalAllocationSoftLimit);
    }
    return external_memory_at_last_gc + kExternalAllocationSoftLimit;
  }
...
#undef ROOT_ACCESSOR
  ...
  // The size of external memory after the last MarkCompact GC.
  std::atomic<size_t> external_memory_at_last_gc_ = 0;
