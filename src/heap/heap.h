...
class Heap {
 public:
  ...
  bool write_protect_code_memory() const {
    // On MacOs on ARM64 ("Apple M1"/Apple Silicon) code protection is achieved
    // by APRR/MAP_JIT machinery instead of changing memory protection flags.
    return !V8_HAS_PTHREAD_JIT_WRITE_PROTECT && write_protect_code_memory_;
  }
...
class V8_NODISCARD CodePageHeaderModificationScope {
 public:
  inline CodePageHeaderModificationScope();
  inline ~CodePageHeaderModificationScope();
};
