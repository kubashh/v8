...
class ItemParallelJob {
 public:
  ...
  void Run() {
    ...
    // first place.
    // TODO(gab): Figure out why |num_tasks == 0| breaks a lot of stuff, keep
    // running at least the main thread's task until then.
    const size_t num_tasks = Max(1U, Min(num_items, tasks_.size()));

    for (size_t i = num_tasks; i < tasks_.size(); i++) {
      // Abort unused tasks right away (or |cancelable_task_manager_| will leak
      // them).
      auto aborted = cancelable_task_manager_->TryAbort(tasks_[i]->id());
