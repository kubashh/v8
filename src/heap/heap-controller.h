...
class V8_EXPORT_PRIVATE MemoryController {
 public:
  MemoryController(Heap* heap, double min_growing_factor,
                   ...
                   double available_allocation_factor)
      : heap_(heap),
        ...
        kAvailableAllocationFactor(available_allocation_factor) {}
  ...
  AvailableAllocationSpace CheckAllocationLimit(size_t used_memory,
    ...
    if (used_memory > allocation_limit) {
      return AvailableAllocationSpace::kNoSpaceAvailable;
    } else if (used_memory > (kAvailableAllocationFactor * allocation_limit)) {
      return AvailableAllocationSpace::kSomeSpaceAvailable;
    }
    return AvailableAllocationSpace::kPlentySpaceAvailable;
  }
 ...
 protected:
  ...
  const double kAvailableAllocationFactor;
