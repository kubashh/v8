...
bool Deoptimizer::BuiltinContinuationModeIsJavaScript(
  ...
  return mode == BuiltinContinuationMode::JAVASCRIPT ||
         mode == BuiltinContinuationMode::JAVASCRIPT_WITH_CATCH ||
         mode == BuiltinContinuationMode::JAVASCRIPT_HANDLE_EXCEPTION;
}

bool Deoptimizer::BuiltinContinuationModeIsWithCatch(
    BuiltinContinuationMode mode) {
  return mode == BuiltinContinuationMode::JAVASCRIPT_WITH_CATCH ||
         mode == BuiltinContinuationMode::JAVASCRIPT_HANDLE_EXCEPTION;
}
...
Builtins::Name Deoptimizer::TrampolineForBuiltinContinuation(
  ...
  if (BuiltinContinuationModeIsJavaScript(mode)) {
    if (must_handle_result) {
      return Builtins::kContinueToJavaScriptBuiltinWithResult;
    } else {
      return Builtins::kContinueToJavaScriptBuiltin;
    }
  } else {
    if (must_handle_result) {
      return Builtins::kContinueToCodeStubBuiltinWithResult;
    } else {
      return Builtins::kContinueToCodeStubBuiltin;
    }
  }
  UNREACHABLE();
}
...
void Deoptimizer::DoComputeBuiltinContinuation(
  ...
  if (BuiltinContinuationModeIsWithCatch(mode)) {
    output_frame_offset -= kPointerSize;
    if (mode == BuiltinContinuationMode::JAVASCRIPT_HANDLE_EXCEPTION) {
      intptr_t accumulator_value =
          input_->GetRegister(kInterpreterAccumulatorRegister.code());
      WriteValueToOutput(reinterpret_cast<Object*>(accumulator_value), 0,
...
void Translation::BeginJavaScriptBuiltinContinuationWithCatchFrame(
  ...
  buffer_->Add(JAVA_SCRIPT_BUILTIN_CONTINUATION_FRAME_WITH_CATCH);
  buffer_->Add(bailout_id.ToInt());
  buffer_->Add(literal_id);
  buffer_->Add(height);
}
...
int Translation::NumberOfOperandsFor(Opcode opcode) {
  switch (opcode) {
    ...
    case JAVA_SCRIPT_BUILTIN_CONTINUATION_FRAME_WITH_CATCH:
      return 3;
  }
...
TranslatedFrame TranslatedState::CreateNextTranslatedFrame(
  ...
  switch (opcode) {
    ...
    case Translation::JAVA_SCRIPT_BUILTIN_CONTINUATION_FRAME_WITH_CATCH: {
      BailoutId bailout_id = BailoutId(iterator->Next());
      SharedFunctionInfo* shared_info =
          SharedFunctionInfo::cast(literal_array->get(iterator->Next()));
      int height = iterator->Next();
