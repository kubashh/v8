  ...
  for (size_t i = 0; i < count; ++i, ++frame_index) {
    ...
    switch (translated_frame->kind()) {
      ...
      case TranslatedFrame::kJavaScriptBuiltinContinuation:
        DoComputeBuiltinContinuation(translated_frame, frame_index, true, false,
                                     false);
        break;
      case TranslatedFrame::kJavaScriptBuiltinContinuationWithCatch:
        DoComputeBuiltinContinuation(translated_frame, frame_index, true, true,
                                     deoptimizing_throw_ && i == count - 1);
        break;
...
void Deoptimizer::DoComputeBuiltinContinuation(
    TranslatedFrame* translated_frame, int frame_index,
    bool java_script_builtin, bool with_catch, bool handle_exception) {
  TranslatedFrame::iterator value_iterator = translated_frame->begin();
  int input_index = 0;
  ...
  DCHECK_IMPLIES(handle_exception, with_catch);
  ...
  const int stack_param_count = translated_stack_parameters +
                                (must_handle_result ? 1 : 0) +
                                (with_catch ? 1 : 0);
  ...
  if (with_catch) {
    ...
    if (handle_exception) {
      intptr_t accumulator_value =
          input_->GetRegister(kInterpreterAccumulatorRegister.code());
      WriteValueToOutput(reinterpret_cast<Object*>(accumulator_value), 0,
                         frame_index, output_frame_offset, "accumulator ");
    } else {
      ...
      java_script_builtin
          ? (with_catch
                 ? StackFrame::TypeToMarker(
                       StackFrame::JAVA_SCRIPT_BUILTIN_CONTINUATION_WITH_CATCH)
                 : StackFrame::TypeToMarker(
