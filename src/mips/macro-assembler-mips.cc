...
void MacroAssembler::Ulw(Register rd, const MemOperand& rs) {
  ...
  if (IsMipsArchVariant(kMips32r6)) {
    ...
    DCHECK(kMipsLwrOffset <= kIntSize && kMipsLwlOffset <= kIntSize);
    MemOperand source = rs;
    // To simplify we set is_doubleword to true here to check if offset + 4 fits
    // into int16_t, instead of adding checks for offset + 3.
    AdjustBaseAndOffset(source, /* is_doubleword */ true);
    if (!rd.is(source.rm())) {
...
void MacroAssembler::Usw(Register rd, const MemOperand& rs) {
  ...
  if (IsMipsArchVariant(kMips32r6)) {
    ...
    DCHECK(IsMipsArchVariant(kMips32r2) || IsMipsArchVariant(kMips32r1) ||
           IsMipsArchVariant(kLoongson));
    DCHECK(kMipsSwrOffset <= kIntSize && kMipsSwlOffset <= kIntSize);
    MemOperand source = rs;
    // To simplify we set is_doubleword to true here to check if offset + 4 fits
    // into int16_t, instead of adding checks for offset + 3.
    AdjustBaseAndOffset(source, /* is_doubleword */ true);
...
void MacroAssembler::Ulh(Register rd, const MemOperand& rs) {
  ...
  if (IsMipsArchVariant(kMips32r6)) {
    ...
    MemOperand source = rs;
    // To simplify we set is_doubleword to true here to check if offset + 4 fits
    // into int16_t, instead of adding checks for offset + 1.
    AdjustBaseAndOffset(source, /* is_doubleword */ true);
    if (source.rm().is(at)) {
...
void MacroAssembler::Ldc1(FPURegister fd, const MemOperand& src) {
  ...
  MemOperand tmp = src;
  AdjustBaseAndOffset(tmp, /* is_doubleword */ true, /* is_float */ true);
