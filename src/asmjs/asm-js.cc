...
class AsmJsCompilationJob final : public CompilationJob {
 ...
 private:
  ...
  int module_size_;
  int64_t translation_time_micro_;
...
CompilationJob::Status AsmJsCompilationJob::ExecuteJobImpl() {
  ...
  translation_time_micro_ = translate_timer.Elapsed().InMicroseconds();
  module_size_ = compilation_info()->literal()->end_position() -
...
void AsmJsCompilationJob::RecordHistograms(Isolate* isolate) {
  ...
  int translation_throughput =
      translation_time_micro_ != 0
          ? static_cast<int>(static_cast<int64_t>(module_size_) /
                             translation_time_micro_)
          : 0;
