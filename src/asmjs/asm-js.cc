...
class AsmJsCompilationJob final : public CompilationJob {
 ...
 private:
  ...
  // TODO(asmjs) Replace with a bitvector.
  wasm::AsmJsParser::StdlibSet stdlib_uses_;

  double translate_time_;
...
CompilationJob::Status AsmJsCompilationJob::PrepareJobImpl() {
  ...
  // TODO(asmjs): In order to move translation to the non-main thread ExecuteJob
  // phase, the scanner stream needs to be off-heap.
...
CompilationJob::Status AsmJsCompilationJob::ExecuteJobImpl() {
  // TODO(asmjs): Move translation to the Execute phase (see comment about
