...
void Genesis::InitializeGlobal_harmony_temporal() {
  ...
  {
    Handle<JSGlobalObject> global(native_context()->global_object(), isolate());
    Handle<String> temporal_string =
        factory()->InternalizeUtf8String("Temporal");
    Handle<AccessorInfo> lazy_accessor = Accessors::MakeAccessor(
        isolate(), temporal_string, LazyInitializeGlobalThisTemporal, nullptr);
    lazy_accessor->set_replace_on_access(true);
  ...
  // Likewise Date.toTemporalInstant.
  {
    ...
    lazy_accessor->set_replace_on_access(true);
    JSObject::SetAccessor(date_prototype, name, lazy_accessor, DONT_ENUM)
        .Check();
  }
}
