...
void Genesis::InitializeGlobal_harmony_shadow_realm() {
  ...
  {  // --- W r a p p e d F u n c t i o n
    ...
    // Internal method for creating wrapped function.
    Handle<JSFunction> wrapped_function_create_fun = SimpleCreateFunction(
        isolate(), factory->empty_string(),
        Builtin::kShadowRealmWrappedFunctionCreate, 1, false);
    wrapped_function_create_fun->shared().set_native(false);
    native_context()->set_shadow_realm_wrapped_function_create_fun(
        *wrapped_function_create_fun);
  }
}
