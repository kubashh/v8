...
V8_INLINE bool EquivalentIndices(TypeIndex<kModuleRelative> index1,
                                 TypeIndex<kModuleRelative> index2,
                                 const WasmModule* module1,
                                 const WasmModule* module2) {
  DCHECK(index1 != index2 || module1 != module2);
  return module1->isorecursive_canonical_type_id(index1) ==
         module2->isorecursive_canonical_type_id(index2);
}

bool ValidStructSubtypeDefinition(TypeIndex<kModuleRelative> subtype_index,
                                  TypeIndex<kModuleRelative> supertype_index,
...
bool ValidArraySubtypeDefinition(TypeIndex<kModuleRelative> subtype_index,
                                 TypeIndex<kModuleRelative> supertype_index,
                                 const WasmModule* sub_module,
                                 const WasmModule* super_module) {
...
bool ValidFunctionSubtypeDefinition(TypeIndex<kModuleRelative> subtype_index,
                                    TypeIndex<kModuleRelative> supertype_index,
                                    const WasmModule* sub_module,
                                    const WasmModule* super_module) {
...
bool ValidSubtypeDefinition(TypeIndex<kModuleRelative> subtype_index,
                            TypeIndex<kModuleRelative> supertype_index,
                            const WasmModule* sub_module,
                            const WasmModule* super_module) {
...
V8_NOINLINE V8_EXPORT_PRIVATE bool IsHeapSubtypeOfImpl(
  ...
  TypeIndex<kModuleRelative> sub_index = sub_heap.ref_index();
  ...
  TypeIndex<kModuleRelative> super_index = super_heap.ref_index();
...
// {module1}.
HeapType::Representation CommonAncestor(TypeIndex<kModuleRelative> type_index1,
                                        TypeIndex<kModuleRelative> type_index2,
                                        const WasmModule* module1,
                                        const WasmModule* module2) {
  const TypeDefinition& type1 = module1->type(type_index1);
  const TypeDefinition& type2 = module2->type(type_index2);
  ...
  DCHECK_NE(type_index1.index, kNoSuperType);
  DCHECK_NE(type_index2.index, kNoSuperType);
  while (type_index1.index != kNoSuperType &&
         !(type_index1 == type_index2 && module1 == module2) &&
         !EquivalentIndices(type_index1, type_index2, module1, module2)) {
    type_index1 = module1->supertype(type_index1);
    type_index2 = module2->supertype(type_index2);
  }
  DCHECK_EQ(type_index1.index == kNoSuperType,
            type_index2.index == kNoSuperType);
  if (type_index1.index != kNoSuperType) {
