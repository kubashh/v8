...
class CompilationStateImpl {
 public:
  ...
  // Cancel all background compilation and wait for background jobs/tasks to
  // finish.
  // TODO(wasm): Ideally we would not need to wait for any tasks here.
  ...
  // Increase the {current_compile_concurrency_} counter. Returns whether it
  // increased towards {max_compile_concurrency_}.
  bool IncreaseNumOutstandingUnits(int new_units);
 ...
 private:
  ...
  // The atomic counter is shared with the compilation job.
...
class BgCompileScope {
 public:
  explicit BgCompileScope(WasmEngine* engine) : engine_(engine) {
    engine->RegisterBackgroundCompileTask();
  }

  ~BgCompileScope() { engine_->RemoveBackgroundCompileTask(); }

 private:
  WasmEngine* const engine_;
};

// Returns {true} if no more units are there, and {false} if stopped by the
// {JobDelegate}.
bool ExecuteJSToWasmWrapperCompilationUnits(
    const std::shared_ptr<BackgroundCompileToken>& token,
    JobDelegate* delegate) {
  std::shared_ptr<JSToWasmWrapperCompilationUnit> wrapper_unit = nullptr;
  base::Optional<BgCompileScope> bg_compile_scope_;
...

  {
    ...
    wrapper_unit = compile_scope.compilation_state()
                       ->GetNextJSToWasmWrapperCompilationUnit();
    if (!wrapper_unit) return true;
    bg_compile_scope_.emplace(compile_scope.native_module()->engine());
  }

  while (true) {
    wrapper_unit->Execute();
    ++num_processed_wrappers;
    must_yield = delegate->ShouldYield();
    if (must_yield) return false;
    BackgroundCompileScope compile_scope(token);
    if (compile_scope.cancelled()) return true;
    wrapper_unit = compile_scope.compilation_state()
                       ->GetNextJSToWasmWrapperCompilationUnit();
...
// Run by the {BackgroundCompileJob} (on any thread). Returns {true} if not more
// units are there, and {false} if stopped by the {JobDelegate}.
bool ExecuteCompilationUnits(
  ...
  // triggered.
  if (!ExecuteJSToWasmWrapperCompilationUnits(token, delegate)) return false;
...
class BackgroundCompileJob : public JobTask {
 public:
  ...
  void Run(JobDelegate* delegate) override {
    ...
    current_concurrency_->store(0, std::memory_order_relaxed);
    {
  ...
  size_t GetMaxConcurrency() const override {
    return std::min(max_concurrency_,
                    current_concurrency_->load(std::memory_order_relaxed));
  }
...
void CompilationStateImpl::CancelCompilation() {
  ...
  if (current_compile_job_ && current_compile_job_->IsRunning()) {
    // TODO(wasm): This sets a cancelled flag and waits for all background tasks
    // to finish. Try to avoid the waiting.
...
bool CompilationStateImpl::IncreaseNumOutstandingUnits(int new_units) {
  DCHECK_LT(0, new_units);
  int old_units = current_compile_concurrency_->load(std::memory_order_relaxed);
  while (true) {
    if (current_compile_concurrency_->compare_exchange_weak(
            old_units, old_units + new_units)) {
      return old_units < max_compile_concurrency_;
    }
    // Otherwise retry with updated {old_units}.
  }
}

void CompilationStateImpl::ScheduleCompileJobForNewUnits(int new_units) {
  bool concurrency_increased = IncreaseNumOutstandingUnits(new_units);
  base::MutexGuard guard(&mutex_);
  if (current_compile_job_ && current_compile_job_->IsRunning()) {
