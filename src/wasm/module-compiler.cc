...
class CompilationState {
 public:
  explicit CompilationState(internal::Isolate* isolate, ModuleEnv& env);
  ~CompilationState();
 ...
 private:
  ...
  std::unique_ptr<ModuleWireBytes> wire_bytes_;
        ...
        [compiled_module_deferred, deferred_handles](
          ...
          switch (event) {
            ...
            case CompilationEvent::kFailedCompilation:
              ...
              deferred_handles = nullptr;
              return;
            case CompilationEvent::kFinalize:
...
MaybeHandle<WasmModuleObject> CompileToModuleObjectInternal(
  ...
  // object.
  // Transfer ownership of {env} to the CompilationState, as the ModuleEnv
...
CompilationState::~CompilationState() {
  ...
  NotifyOnEvent(CompilationEvent::kFinalize, Handle<Object>::null());
}
...
std::unique_ptr<WasmCompilationUnit> CompilationState::GetNextExecutedUnit() {
  ...
  std::unique_ptr<WasmCompilationUnit> ret = std::move(finish_units.back());
