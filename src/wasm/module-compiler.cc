...
class CompilationStateImpl {
 public:
  ...
  // compilation.
  void SetNumberOfFunctionsToCompile(const WasmModule* module,
...
void InitializeCompilationUnits(NativeModule* native_module) {
  ...
  CompilationStateImpl* compilation_state =
      Impl(native_module->compilation_state());
  compilation_state->SetNumberOfFunctionsToCompile(module, enabled_features);
  ...
  for (uint32_t func_index = start; func_index < end; func_index++) {
    ...
    } else if (strategy == CompileStrategy ::kLazyBaseline) {
...
bool AsyncStreamingProcessor::ProcessCodeSectionHeader(
  ...
  compilation_state->SetNumberOfFunctionsToCompile(
      job_->native_module_->module(), job_->enabled_features_);
...
void CompilationStateImpl::SetNumberOfFunctionsToCompile(
  ...
  base::MutexGuard guard(&callbacks_mutex_);
  outstanding_baseline_functions_ = module->num_declared_functions -
                                    module->num_lazy_compilation_hints -
  ...
  DCHECK_LE(outstanding_baseline_functions_, outstanding_top_tier_functions_);
  highest_execution_tier_.assign(module->num_declared_functions,
                                 ExecutionTier::kNone);
...
void CompilationStateImpl::OnFinishedUnits(Vector<WasmCode*> code_vector) {
  ...
  for (WasmCode* code : code_vector) {
    ...
    if (strategy == CompileStrategy::kEager &&
      ...
      DCHECK_EQ(prior_tier, ExecutionTier::kNone);
