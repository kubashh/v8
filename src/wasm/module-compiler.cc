...
void ValidateSequentially(const WasmModule* module, NativeModule* native_module,
                          Counters* counters, AccountingAllocator* allocator,
                          ErrorThrower* thrower,
                          bool only_lazy_functions = false) {
...
void ValidateLazyFunctionsSequentially(const WasmModule* module,
                                       NativeModule* native_module,
                                       Counters* counters,
                                       AccountingAllocator* allocator,
                                       ErrorThrower* thrower) {
  const bool only_lazy_functions = true;
  ValidateSequentially(module, native_module, counters, allocator, thrower,
                       only_lazy_functions);
}
...
void CompileNativeModule(Isolate* isolate, ErrorThrower* thrower,
  ...
  if (native_module->enabled_features().compilation_hints) {
    ValidateLazyFunctionsSequentially(wasm_module, native_module,
  ...
  if (compilation_state->failed()) {
    ValidateSequentially(native_module->module(), native_module,
                         isolate->counters(), isolate->allocator(), thrower);
    CHECK(thrower->error());
  }
}
      ...
      if (enabled_features.compilation_hints && result.ok()) {
        ...
        for (int func_index = start; func_index < end; func_index++) {
          ...
          if (IsLazyCompilation(module, enabled_features, func_index)) {
            DecodeResult result = ValidateSingleFunction(
                module, func_index, code, counters_,
                job->isolate()->wasm_engine()->allocator(), enabled_features);
            if (result.failed()) {
...
bool AsyncStreamingProcessor::ProcessFunctionBody(Vector<const uint8_t> bytes,
  ...
  if (IsLazyCompilation(module, native_module, enabled_features, func_index)) {
    // TODO(frgossen): <<<<<check this coment>>>>>> Unify this code with
    // {ValidateSequentially}. Note, that the native moudle does not own the
    // wire bytes until {SetWireBytes} is called in {OnFinishedStream}.
    // Validation must threfore use the {bytes} parameter.
