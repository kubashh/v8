...
ModuleCompiler::CodeGenerationSchedule::GetNext() {
  ...
  if (available_work() != nullptr) {
    available_work()->Signal();
  }
  return ret;
}
...
bool ModuleCompiler::FetchAndExecuteCompilationUnit(
  ...
  if (executed_units_.available_work() != nullptr) {
    executed_units_.available_work()->Wait();
  }
...
uint32_t* ModuleCompiler::StartCompilationTasks() {
  ...
  executed_units_.InitAvailableWork(num_background_tasks_);
