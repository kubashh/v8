...
MaybeHandle<WasmModuleObject> ModuleCompiler::CompileToModuleObject(
  ...
  return CompileToModuleObjectInternal(isolate_, thrower, std::move(module),
...
MaybeHandle<WasmModuleObject> ModuleCompiler::CompileToModuleObjectInternal(
  ...
  auto env =
      CreateDefaultModuleEnv(isolate, module_, init_builtin, &globals_manager);
  ...
  Handle<WasmModuleWrapper> module_wrapper =
      WasmModuleWrapper::New(isolate_, module.release());
  ...
  for (auto exp : module_->export_table) {
    ...
    int export_index = static_cast<int>(module_->functions.size() + func_index);
...
std::shared_ptr<StreamingDecoder> AsyncCompileJob::CreateStream() {
  DCHECK_NULL(stream_.get());
  stream_.reset(
 ...
 private:
  ...
  void RunInForeground() override {
    ...
    job_->module_env_ = CreateDefaultModuleEnv(
        isolate, module_, illegal_builtin, &job_->globals_manager_);
 ...
 public:
  ...
  void RunInForeground() override {
    ...
    if (job_->outstanding_units_ == 0) {
      // Make sure all compilation tasks stopped running.
      job_->background_task_manager_.CancelAndWait();
      if (job_->ShouldFinish()) job_->DoSync<FinishCompile>();
    }
  }
...
class AsyncCompileJob::FinishCompile : public CompileStep {
  void RunInForeground() override {
    ...
    // and it will be destroyed when the GC reclaims the wrapper object.
    Handle<WasmModuleWrapper> module_wrapper =
        WasmModuleWrapper::New(job_->isolate_, job_->module_.release());
...
template <typename T>
bool AsyncStreamingProcessor::FinishAsyncCompileJobWithError(Result<T> error) {
...
bool AsyncStreamingProcessor::ProcessModuleHeader(Vector<const uint8_t> bytes,
  ...
  bool result = decoder_.DecodeModuleHeader(bytes, offset);
  if (!result) {
    return FinishAsyncCompileJobWithError(decoder_.FinishDecoding(false));
  }
  return result;
}
...
bool AsyncStreamingProcessor::ProcessSection(SectionCode section_code,
                                             ...
                                             uint32_t offset) {
  TRACE_STREAMING("Process section %d: %p - %p ...\n", section_code,
...
bool AsyncStreamingProcessor::StartCodeSection(size_t functions_count,
                                               uint32_t offset) {
  ...
  // Set required_finishers_ to 2, because both the AsyncCompileJob and the
  // AsyncStreamingProcessor have to finish.
  job_->required_finishers_.SetValue(2);
...
bool AsyncStreamingProcessor::ProcessFunctionBody(Vector<const uint8_t> bytes,
                                                  uint32_t offset) {
  TRACE_STREAMING("Process function body %d: %p - %p ...\n", next_function_,
                  bytes.begin(), bytes.end());
...
void AsyncStreamingProcessor::FinishChunk() {
  // TRACE_STREAMING("FinishChunk...\n");
  if (compilation_unit_builder_ != nullptr) {
...
void AsyncStreamingProcessor::Error(DecodeResult result) {
  TRACE_STREAMING("Stream error...\n");
  FinishAsyncCompileJobWithError(std::move(result));
}

// Finish the processing of the stream.
void AsyncStreamingProcessor::FinishStream(std::unique_ptr<uint8_t[]> bytes,
                                           size_t length) {
  ...
  DCHECK(result.ok());
  job_->module_ = std::move(result.val);
  if (job_->ShouldFinish()) job_->DoSync<AsyncCompileJob::FinishCompile>();
}

void AsyncStreamingProcessor::Abort() {
