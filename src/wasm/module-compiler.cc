...
ModuleCompiler::CodeGenerationSchedule::GetNext() {
  ...
  if (throttle_) {
    size_t prev_allocated_memory = allocated_memory_.Value();
    size_t currently_allocated_memory =
        allocated_memory_.Decrement(ret->memory_cost());
    USE(prev_allocated_memory);
    USE(currently_allocated_memory);
    DCHECK_LT(currently_allocated_memory, prev_allocated_memory);
    accept_work_.Signal();
  }
  return ret;
}
...
bool ModuleCompiler::FetchAndExecuteCompilationUnit(
  ...
  // Grab work before figuring if we need to wait, otherwise the thread may
  // hang waiting if all work is done elsewhere, and said thread starts
  // fetching again.
...
void ModuleCompiler::CompileInParallel(ModuleBytesEnv* module_env,
  ...
  while (finished_work < compilation_units_.size()) {
    // 3.b) If {executed_units} contains a compilation unit, the main thread
