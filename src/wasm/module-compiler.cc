...
class CompilationStateImpl {
 public:
  ...
  void SetNumberOfFunctionsToCompile(int num_functions, int num_lazy_functions,
                                     int num_lazy_baseline_functions);
  ...
  if (enabled_features.compilation_hints) {
    ...
    if (hint != nullptr) {
      switch (hint->strategy) {
        ...
        case WasmCompilationHintStrategy::kLazyBaseline:
          return CompileStrategy::kLazyBaseline;
...
class CompilationUnitBuilder {
 public:
  ...
  void AddTopTierUnit(int func_index) {
    DCHECK(native_module_->enabled_features().compilation_hints);
    ExecutionTierPair tiers = GetRequestedExecutionTiers(
        native_module_->module(), compilation_state()->compile_mode(),
        native_module_->enabled_features(), func_index);
    // In this case, the compilation unit is added even the baseline tier is the
    // same. The baseline is lazily compiled in this case.
    DCHECK(GetCompileStrategy(native_module_->module(),
                              native_module_->enabled_features(),
                              func_index) == CompileStrategy::kLazyBaseline);
    tiering_units_.emplace_back(CreateUnit(func_index, tiers.top_tier));
  }
  ...
  for (uint32_t func_index = start; func_index < end; func_index++) {
    ...
    if (only_lazy_functions == OnlyLazyFunctions::kYes) {
      ...
      if (strategy != CompileStrategy::kLazy &&
          strategy != CompileStrategy::kLazyBaseline)
        continue;
    }
...
void InitializeCompilationUnits(NativeModule* native_module) {
  ...
  compilation_state->SetNumberOfFunctionsToCompile(
      module->num_declared_functions, module->num_lazy_compilation_hints,
      module->num_lazy_baseline_compilation_hints);
  ...
  for (uint32_t func_index = start; func_index < end; func_index++) {
    CompileStrategy strategy =
        GetCompileStrategy(module, native_module, enabled_features, func_index);
    switch (strategy) {
      case CompileStrategy ::kLazyBaseline:
...
bool AsyncStreamingProcessor::ProcessCodeSectionHeader(
  ...
  // Set number of functions that must be compiled to consider the module fully
  // compiled.
  auto module = job_->native_module_->module();
...
bool AsyncStreamingProcessor::ProcessFunctionBody(Vector<const uint8_t> bytes,
  ...
  if (strategy == CompileStrategy::kLazy ||
    ...
    if (strategy == CompileStrategy::kLazyBaseline) {
...
void CompilationStateImpl::SetNumberOfFunctionsToCompile(
  ...
  outstanding_baseline_functions_ =
      num_functions - num_lazy_functions - num_lazy_baseline_functions;
  outstanding_top_tier_functions_ = num_functions - num_lazy_functions;
  DCHECK_LE(outstanding_baseline_functions_, outstanding_top_tier_functions_);
...
void CompilationStateImpl::OnFinishedUnits(Vector<WasmCode*> code_vector) {
  ...
  for (WasmCode* code : code_vector) {
    ...
    if (strategy == CompileStrategy::kEager &&
        prior_tier < requested_tiers.baseline_tier) {
      DCHECK(prior_tier == ExecutionTier::kNone);
      DCHECK_LE(requested_tiers.baseline_tier, code->tier());
    ...
    if ((strategy == CompileStrategy::kEager ||
         strategy == CompileStrategy::kLazyBaseline) &&
        prior_tier < requested_tiers.top_tier &&
        code->tier() >= requested_tiers.top_tier) {
      DCHECK_IMPLIES(strategy == CompileStrategy::kLazyBaseline,
                     prior_tier == ExecutionTier::kNone);
