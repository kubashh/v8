...
Handle<Script> CreateWasmScript(Isolate* isolate,
  ...
  // Script name is "wasm-<module_name>-hash" if name is available and
  // "wasm-hash" otherwise.
  ...
  if (name.is_set()) {
    Handle<String> module_name =
        WasmModuleObject::ExtractUtf8StringFromModuleBytes(
            ...
            .ToHandleChecked();
    name_prefix = isolate->factory()
                      ->NewConsString(isolate->factory()->InternalizeString(
                                          StaticCharVector("wasm-")),
                                      module_name)
                      .ToHandleChecked();
  } else {
    name_prefix =
        isolate->factory()->InternalizeString(StaticCharVector("wasm"));
  }
  Handle<String> name_str = isolate->factory()
                                ->NewConsString(name_prefix, name_hash)
                                .ToHandleChecked();
