...
Handle<Code> MakeWasmToWasmWrapper(
    Isolate* isolate, Handle<WasmExportedFunction> exported_function,
  ...
  Handle<WasmInstanceObject> imported_instance(exported_function->instance(),
                                               isolate);
  imported_instances->Set(imported_instance, imported_instance);
  Handle<Code> wasm_code = exported_function->GetWasmCode();
  ...
  *sig = imported_instance->module()
             ->functions[exported_function->function_index()]
             .sig;
  ...
  Handle<Code> wrapper_code = compiler::CompileWasmToWasmWrapper(
      isolate, wasm_code, *sig, exported_function->function_index(),
  ...
  deopt_data->set(1, Smi::FromInt(exported_function->function_index()));
...
void InstanceBuilder::WriteGlobalValue(WasmGlobal& global,
  ...
  TRACE("init [globals_start=%p + %u] = %lf, type = %s\n",
        raw_buffer_ptr(globals_, 0), global.offset, num,
