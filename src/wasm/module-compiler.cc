...
class CompilationStateImpl {
 public:
  ...
  void SetCallback(std::function<void(CompilationEvent, VoidResult*)> callback);
  ...
  void OnError(VoidResult* error_result);
  void OnFinishedUnit();
  ...
  VoidResult* compile_error() const {
    DCHECK_NOT_NULL(compile_error_);
    return compile_error_.get();
  }

 private:
  void NotifyOnEvent(CompilationEvent event, VoidResult* error_result);
  ...
  std::function<void(CompilationEvent, VoidResult*)> callback_;
...
void CompilationState::SetError(uint32_t func_index, ResultBase error_result) {
  Impl(this)->SetError(func_index, std::move(error_result));
}

const ResultBase& CompilationState::GetError() const {
  return *Impl(this)->compile_error();
}
...
void CompileSequentially(Isolate* isolate, NativeModule* native_module,
  ...
  for (uint32_t i = 0; i < module->functions.size(); ++i) {
    ...
    if (!success) {
      thrower->CompileFailed(native_module->compilation_state()->GetError());
      break;
    }
  }
  ...
  void RunInForeground() override {
        ...
        Impl(job_->native_module_->compilation_state());
    {
      ...
      compilation_state->SetCallback(
          [job](CompilationEvent event, VoidResult* error_result) {
...
void CompilationStateImpl::SetCallback(
    std::function<void(CompilationEvent, VoidResult*)> callback) {
...
void CompilationStateImpl::OnError(VoidResult* error_result) {
  DCHECK_NOT_NULL(error_result);
  Abort();
