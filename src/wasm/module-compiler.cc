...
enum class CompileStrategy : uint8_t {
  ...
  // execution, but only during its early phase (until top tier compilation
  // finishes).
...
class CompilationStateImpl {
 public:
  ...
  // Initialize compilation state. Set the number of compilation units to expect
  // for baseline and top tier compilation. Must be set before
  // {AddCompilationUnits} is invoked which triggers background compilation.
  void Initialize();
 ...
 private:
  ...
  std::vector<uint8_t> tiering_states_;

  // End of fields protected by {callbacks_mutex_}.
  //////////////////////////////////////////////////////////////////////////////

  // Encoding of fields in the {tiering_states_} vector.
  class BaselineTierField : public BitField8<ExecutionTier, 0, 2> {};
  class TopTierField : public BitField8<ExecutionTier, 2, 2> {};
  class ReachedTierField : public BitField8<ExecutionTier, 4, 2> {};
  class CompileStrategyField : public BitField8<CompileStrategy, 6, 2> {};
};
...
void CompilationStateImpl::Initialize() {
  auto enabled_features = native_module_->enabled_features();
  auto* module = native_module_->module();
  DCHECK_IMPLIES(!enabled_features.compilation_hints,
                 module->num_lazy_compilation_hints == 0);
  DCHECK_IMPLIES(!enabled_features.compilation_hints,
                 module->num_lazy_baseline_compilation_hints == 0);
  DCHECK(!failed());
  ...
  for (int func_index = start; func_index < end; func_index++) {
    ...
    // Initialize tiering state.
    uint8_t tiering_state = CompileStrategyField::encode(strategy);
    tiering_state =
        BaselineTierField::update(tiering_state, requested_tiers.baseline_tier);
    tiering_state =
        TopTierField::update(tiering_state, requested_tiers.top_tier);
    tiering_state =
        ReachedTierField::update(tiering_state, ExecutionTier::kNone);
    tiering_states_.push_back(tiering_state);
...
void CompilationStateImpl::OnFinishedUnits(Vector<WasmCode*> code_vector) {
  ...
  DCHECK_EQ(tiering_states_.size(),
  ...
  for (WasmCode* code : code_vector) {
    ...
    // Read current tiering state.
    int slot_index =
        code->index() - native_module_->module()->num_imported_functions;
    uint8_t tiering_state = tiering_states_[slot_index];
    ExecutionTier baseline_tier = BaselineTierField::decode(tiering_state);
    ExecutionTier top_tier = TopTierField::decode(tiering_state);
    ExecutionTier reached_tier = ReachedTierField::decode(tiering_state);
    CompileStrategy strategy = CompileStrategyField::decode(tiering_state);

    // Skip fully lazily compiled functions as we consider these neither for the
    // completion of baseline nor for the completion of top tier compilation.
    ...
    // Only eagerly compiled functions must reach their baseline tier before
    // overall baseline compilation is considered finished.
    if (strategy == CompileStrategy::kEager && reached_tier < baseline_tier) {
      DCHECK_EQ(ExecutionTier::kNone, reached_tier);
      DCHECK_LE(baseline_tier, code->tier());
    ...
    // Except for fully lazily compiled functions, all must reach their top tier
    // before overall top tier compilation is considered finished.
    DCHECK(strategy == CompileStrategy::kEager ||
           strategy == CompileStrategy::kLazyBaselineEagerTopTier);
    if (reached_tier < top_tier && top_tier <= code->tier()) {
      DCHECK_IMPLIES(strategy == CompileStrategy::kLazyBaselineEagerTopTier,
                     reached_tier == ExecutionTier::kNone);
    ...
    // Update tiering state.
    if (code->tier() > reached_tier) {
      tiering_states_[slot_index] =
          ReachedTierField::update(tiering_states_[slot_index], code->tier());
    }
