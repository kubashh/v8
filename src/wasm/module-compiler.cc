...
Handle<Code> UnwrapForeignExportOrCompileImportWrapper(
  ...
  WasmFunction* other_func = GetWasmFunctionForExportOrNull(isolate, target);
  if (other_func) {
  ...
  for (int index = 0; index < static_cast<int>(module_->import_table.size());
    ...
    switch (import.kind) {
      case kExternalFunction: {
        ...
        Handle<Code> import_wrapper = UnwrapForeignExportOrCompileImportWrapper(
      ...
      case kExternalTable: {
        ...
        for (int i = 0; i < table_size; ++i) {
          ...
          WasmFunction* function =
              GetWasmFunctionForExportOrNull(isolate_, val);
          if (function == nullptr) {
