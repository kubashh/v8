 ...
 public:
  ...
  void AddUnits(Vector<WasmCompilationUnit> baseline_units,
    ...
    for (auto pair : {std::make_pair(int{kBaseline}, baseline_units),
      ...
      queue->units[tier].insert(queue->units[tier].end(), units.begin(),
                                units.end());
      num_units_[tier].fetch_add(units.size(), std::memory_order_relaxed);
      // Now move all big units (should only be a small fraction of total
      // functions) to the {big_units_queue_}.
      auto end = queue->units[tier].end();
      for (auto it = end - units.size(); it < end;) {
        size_t func_size = module->functions[it->func_index()].code.length();
        if (func_size > kBigUnitsLimit) {
          ...
          big_units_queue_.units[tier].emplace(func_size, *it);
          *it = *--end;
        } else {
          ++it;
        }
      }
      if (end != queue->units[tier].end()) {
        queue->units[tier].erase(end, queue->units[tier].end());
      }
    }
  }
