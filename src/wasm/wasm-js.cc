...
uint32_t ToNonWrappingUint32(const char* argument_name, Local<v8::Value> v,
  ...
  if (!v->IsUndefined() && v->NumberValue(context).To(&double_number)) {
    if (std::isnan(double_number) || std::abs(double_number) == V8_INFINITY) {
      thrower->TypeError("%s must be a valid number", argument_name);
      return 0;
    }
    ...
    if (double_number > UINT32_MAX) {
      thrower->TypeError("%s must be in range", argument_name);
      return 0;
    }
...
void WebAssemblyTableGrow(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  uint32_t grow_by =
      ToNonWrappingUint32("Argument 0", args[0], context, &thrower);
