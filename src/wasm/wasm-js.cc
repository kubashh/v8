...
std::string ToString(const std::string& name) { return name; }
...
template <typename T>
bool EnforceUint32(const T argument_name, Local<v8::Value> v,
...
void WebAssemblyTableGrow(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  DCHECK(max_size64 <= std::numeric_limits<uint32_t>::max());
  ...
  // We used to check if the table would shrink here, but since the switch to
  // the Web IDL spec: negative grow_by is considered as a TypeError and thus
