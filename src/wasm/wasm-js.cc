...
const char* PrintArgumentOrPropertyName(const char* name) { return name; }

const char* PrintArgumentOrPropertyName(i::Handle<i::String> name) {
...
bool EnforceUint32(const T debug_name, Local<v8::Value> v,
  ...
  if (!v->NumberValue(context).To(&double_number)) {
    thrower->TypeError("%s must be convertible to a number",
                       PrintArgumentOrPropertyName(debug_name));
    return false;
  }
