...
void WebAssemblyException(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  i::JSObject::AddProperty(
      i_isolate, runtime_exception, "getArg",
      i::Handle<i::Object>(i_isolate->native_context()->wasm_exception_getarg(),
                           i_isolate),
      i::DONT_ENUM);
...
void WasmJs::Install(Isolate* isolate, bool exposed_on_global_object) {
  ...
  if (enabled_features.has_eh()) {
    ...
    Handle<HeapObject> instance_prototype(isolate->native_context()
                                              ...
                                              .instance_prototype(),
                                          isolate);
    auto getarg = CreateFunc(isolate, v8_str(isolate, "getArg"),
                             WebAssemblyExceptionGetArg, true,
                             SideEffectType::kHasNoSideEffect);
    context->set_wasm_exception_getarg(*getarg);
