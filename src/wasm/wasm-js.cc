...
bool ConvertToUint32(const char* argument_name, Local<v8::Value> v,
                     Local<Context> context, ErrorThrower* thrower,
                     uint32_t* res) {
  double double_number;

  if (v->IsUndefined() || !v->NumberValue(context).To(&double_number)) {
    thrower->TypeError("%s must be a number", argument_name);
    return false;
  }
  if (std::isnan(double_number) || double_number == V8_INFINITY) {
    thrower->TypeError("%s must be convertible to a valid number",
                       argument_name);
    return false;
  }
  if (double_number < 0) {
    thrower->TypeError("%s must be non-negative", argument_name);
    return false;
  }
  if (double_number > std::numeric_limits<uint32_t>::max()) {
    thrower->TypeError("%s must be in the unsigned long range", argument_name);
    return false;
  }
...
void WebAssemblyTableGrow(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  uint32_t grow_by;
  if (!ConvertToUint32("Argument 0", args[0], context, &thrower, &grow_by)) {
    return;
  }
...
void WebAssemblyTableGet(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  uint32_t index;
  if (!ConvertToUint32("Argument 0", args[0], context, &thrower, &index)) {
    return;
  }
