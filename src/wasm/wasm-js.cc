...
void WebAssemblySuspenderSuspendOnReturnedPromise(
  ...
  const i::wasm::FunctionSig* sig;
  i::Handle<i::Object> arg0 = Utils::OpenHandle(*args[0]);
  if (i::WasmJSFunction::IsWasmJSFunction(*arg0)) {
    sig = i::Handle<i::WasmJSFunction>::cast(arg0)->GetSignature(&zone);
  } else {
    // TODO(thibaudm): Clarify the spec for re-imported wasm functions.
    thrower.TypeError(
        "Argument 0 must be a WebAssembly.Function holding a JS function");
    return;
  }
