...
void WasmObjectToJSReturnValue(v8::ReturnValue<v8::Value>& return_value,
  ...
  switch (repr) {
    case i::wasm::HeapType::kExtern:
    case i::wasm::HeapType::kString:
      return_value.Set(Utils::ToLocal(wasm_value));
      break;
    case i::wasm::HeapType::kFunc: {
      if (!wasm_value->IsNull()) {
        DCHECK(wasm_value->IsWasmInternalFunction());
        wasm_value = handle(
            i::Handle<i::WasmInternalFunction>::cast(wasm_value)->external(),
            isolate);
      }
      return_value.Set(Utils::ToLocal(wasm_value));
      break;
    ...
    case i::wasm::HeapType::kEq:
    case i::wasm::HeapType::kAny: {
      if (!i::v8_flags.wasm_gc_js_interop && wasm_value->IsWasmObject()) {
