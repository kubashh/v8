...
bool CodeSpecialization::ApplyToWasmCode(Code* code,
  ...
  bool reloc_pointers = has_pointers_to_relocate;
  ...
  add_mode(reloc_pointers, RelocInfo::EXTERNAL_REFERENCE);
  ...
  for (RelocIterator it(code, reloc_mode); !it.done(); it.next()) {
    ...
    switch (mode) {
      ...
      case RelocInfo::EXTERNAL_REFERENCE: {
        ...
        Address old_ptr = it.rinfo()->target_external_reference();
        if (pointers_to_relocate.count(old_ptr) == 1) {
          Address new_ptr = pointers_to_relocate[old_ptr];
          *(reinterpret_cast<Address*>(it.rinfo()->target_address_address())) =
              new_ptr;
          changed = true;
        }
      } break;
