...
bool memory_copy_wrapper(Address data) {
  ...
  size_t dst = static_cast<size_t>(ReadUnalignedValue<uint32_t>(data + offset));
  offset += sizeof(uint32_t);
  size_t src = static_cast<size_t>(ReadUnalignedValue<uint32_t>(data + offset));
  offset += sizeof(uint32_t);
  size_t size =
      static_cast<size_t>(ReadUnalignedValue<uint32_t>(data + offset));

  size_t mem_size = instance.memory_size();
  if (dst > mem_size) return false;
  if (size > mem_size - dst) return false;

  if (src > mem_size) return false;
  if (size > mem_size - src) return false;
