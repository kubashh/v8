...
class TypeCanonicalizer {
 public:
  ...
  V8_EXPORT_PRIVATE void add_recursive_group(WasmModule* module, uint32_t size);
  ...
  V8_EXPORT_PRIVATE bool is_canonical_subtype(uint32_t sub_index,
 ...
 private:
  ...
  int find_canonical_group(CanonicalGroup&) const;

  CanonicalType canonicalize_type_def(const WasmModule* module,
                                      TypeDefinition type,
                                      uint32_t recursive_group_start);
  ValueType canonicalize_value_type(const WasmModule* module, ValueType type,
