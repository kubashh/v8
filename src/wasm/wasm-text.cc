  ...
  for (; i.has_next(); i.next()) {
    ...
    switch (opcode) {
      ...
      case kExprBrTable: {
        ...
        BranchTableIterator<Decoder::kUnchecked> iterator(&i, operand);
        os << "br_table";
      ...
      case kExprSetLocal:
      case kExprTeeLocal: {
        LocalIndexOperand<Decoder::kUnchecked> operand(&i, i.pc());
        os << WasmOpcodes::OpcodeName(opcode) << ' ' << operand.index;
        break;
      }
      case kExprThrow:
      case kExprCatch: {
        ExceptionIndexOperand<Decoder::kUnchecked> operand(&i, i.pc());
        os << WasmOpcodes::OpcodeName(opcode) << ' ' << operand.index;
        break;
      }
      case kExprGetGlobal:
      case kExprSetGlobal: {
        GlobalIndexOperand<Decoder::kUnchecked> operand(&i, i.pc());
        os << WasmOpcodes::OpcodeName(opcode) << ' ' << operand.index;
        break;
      }
#define CASE_CONST(type, str, cast_type)                           \
  case kExpr##type##Const: {                                       \
    Imm##type##Operand<Decoder::kUnchecked> operand(&i, i.pc());   \
...
#define CASE_OPCODE(opcode, _, __) case kExpr##opcode:
        ...
        FOREACH_STORE_MEM_OPCODE(CASE_OPCODE) {
          MemoryAccessOperand<Decoder::kUnchecked> operand(&i, i.pc(),
                                                           kMaxUInt32);
