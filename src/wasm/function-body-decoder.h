 ...
 public:
  ...
  WasmOpcode current() {
    return static_cast<WasmOpcode>(
        read_u8<Decoder::kUnchecked>(pc_, "expected bytecode"));
  }
  ...
  WasmOpcode prefixed_opcode() {
    byte prefix = read_u8<Decoder::kUnchecked>(pc_, "expected prefix");
    byte index = read_u8<Decoder::kUnchecked>(pc_ + 1, "expected index");
