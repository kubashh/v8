...
constexpr size_t kV8JSMaxWasmMemoryPages = 32767;  // = ~ 2 GiB
...
static_assert(
    kV8JSMaxWasmMemoryPages <= kV8MaxWasmMemoryPages,
    "v8's JS api shouldn't be more permissive than the internal limit");
static_assert(kV8MaxWasmTableEntries <= 4294967295,  // 2^32 - 1
              "v8 should not exceed WebAssembly non-web embedding limits");
