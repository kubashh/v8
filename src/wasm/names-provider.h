...
class NamesProvider {
 public:
  ...
  bool PrintFunctionName(StringBuilder& out, uint32_t function_index,
                         bool devtools_behavior = false,
                         bool index_as_comment = false);
  void PrintLocalName(StringBuilder& out, uint32_t function_index,
                      uint32_t local_index, bool index_as_comment = false);
  ...
  void PrintFieldName(StringBuilder& out, uint32_t struct_index,
                      uint32_t field_index, bool index_as_comment = false);
 ...
 private:
  ...
  std::unique_ptr<DecodedNameSection> names_{};
  std::unordered_map<uint32_t, std::string> import_export_function_names_;
  std::unordered_map<uint32_t, std::string> import_export_table_names_;
  std::unordered_map<uint32_t, std::string> import_export_memory_names_;
  std::unordered_map<uint32_t, std::string> import_export_global_names_;
  std::unordered_map<uint32_t, std::string> import_export_tag_names_;
};
