...
namespace {
class SaveContextScope {
 public:
  explicit SaveContextScope(Isolate* isolate, Handle<Context> context)
      : isolate_(isolate), prev_(isolate->context(), isolate) {
    isolate_->set_context(*context);
  }
  ~SaveContextScope() { isolate_->set_context(*prev_); }
...
class AsyncCompileJob {
 ...
 private:
  ...
  bool FinishCompile() {
    ...
    HandleScope scope(isolate_);
    SaveContextScope context_scope(isolate_, context_);
    // At this point, compilation has completed. Update the code table.
    for (size_t i = FLAG_skip_compiling_wasm_funcs;
