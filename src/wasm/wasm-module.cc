...
Handle<String> ToValueTypeString(Isolate* isolate, ValueType type) {
  return isolate->factory()->InternalizeUtf8String(type.type_name().c_str());
}
}  // namespace
...
Handle<JSObject> GetTypeForTable(Isolate* isolate, ValueType type,
  ...
  if (type == kWasmFuncRef) {
    // TODO(wasm): We should define the "funcref" string in one central
    // place and then use that constant everywhere.
    element = factory->InternalizeUtf8String("funcref");
  } else {
