...
constexpr int kMaxMemories = 32;
constexpr int kMaxMemorySize = 4;
...
class BodyGen {
  ...
  void memop(DataRange* data) {
    ...
    // Thanks to the LEB encoding and that the `memory_index` fits into 32 bits,
    // we can always emit 32 bits also for memory64.
  ...
  void memory_size(DataRange* data) {
    ...
    // The `memory_size` returns an I32. However, `kExprMemoryGrow` for memory64
  ...
  void grow_memory(DataRange* data) {
    ...
    // The `grow_memory` returns an I32. However, `kExprMemorySize` for memory64
    // returns an I64, so we should convert it.
    if (builder_->builder()->IsMemory64(memory_index)) {
      builder_->Emit(kExprI32ConvertI64);
    }
