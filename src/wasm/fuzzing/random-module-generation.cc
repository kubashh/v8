...
class WasmGenerator {
 ...
 public:
  ...
  template <>
  void Generate<kVoid>(DataRange* data) {
    ...
    constexpr GenerateFn alternatives[] = {
        ...
        &WasmGenerator::sequence<kVoid, kVoid, kVoid, kVoid, kVoid, kVoid,
                                 kVoid, kVoid>,
  ...
  template <>
  void Generate<kI32>(DataRange* data) {
    GeneratorRecursionScope rec_scope(this);
  ...
  template <>
  void Generate<kI64>(DataRange* data) {
    GeneratorRecursionScope rec_scope(this);
    if (recursion_limit_reached() || data->size() <= 1) {
  ...
  template <>
  void Generate<kF32>(DataRange* data) {
    GeneratorRecursionScope rec_scope(this);
  ...
  template <>
  void Generate<kF64>(DataRange* data) {
    GeneratorRecursionScope rec_scope(this);
  ...
  template <>
  void Generate<kS128>(DataRange* data) {
    GeneratorRecursionScope rec_scope(this);
    has_simd_ = true;
...
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE)
    base::Vector<uint8_t> GenerateRandomWasmModule<WasmModuleGenerationOptions{
        true}>(Zone*, base::Vector<const uint8_t> data);
