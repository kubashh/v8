...
class V8_EXPORT_PRIVATE StreamingDecoder {
 ...
 private:
  ...
  std::unique_ptr<DecodingState> Error(std::string message) {
    printf("StreamingDecoder error: %s\n", message.c_str());
    DecodeResult result(nullptr);
    result.error(module_offset_, std::move(message));
  ...
  bool StartCodeSection(size_t num_functions, uint32_t offset) {
    base::LockGuard<base::Mutex> guard(&processor_mutex_);
    if (!ok_) return ok_;
    ok_ &= processor_->StartCodeSection(num_functions, offset);
    return ok_;
  }
