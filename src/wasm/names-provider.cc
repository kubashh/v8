...
void NamesProvider::DecodeNames() {
  DCHECK(!has_decoded_);
  has_decoded_ = true;
  names_.reset(
      new DecodedNameSection(wire_bytes_, module_->name_section_offset));
...
void NamesProvider::ComputeNamesFromImportsExports() {
  ...
  DCHECK(has_decoded_);
  for (size_t i = 0; i < module_->import_table.size(); i++) {
...
void NamesProvider::ComputeImportName(
...
void NamesProvider::ComputeExportName(
    const WasmExport& ex, std::unordered_map<uint32_t, std::string>& target) {
...
// {devtools_behavior}: prepend '$', use import/export names as fallback,
// or "$funcN" as default. Otherwise only return raw name from name section.
bool NamesProvider::PrintFunctionName(StringBuilder& out,
                                      uint32_t function_index,
                                      bool devtools_behavior,
                                      bool index_as_comment) {
  // Function names are stored elsewhere, because we need to access them
  // during (streaming) compilation when the NamesProvides isn't ready yet.
  WireBytesRef ref = module_->lazily_generated_names.LookupFunctionName(
      ModuleWireBytes(wire_bytes_), function_index);
  if (ref.is_set()) {
    if (devtools_behavior) {
      out << '$';
      WriteRef(out, ref);
  ...
  if (!devtools_behavior) return false;
