...
#define INSTRUCTION_HANDLER_FUNC \
  static /*DISABLE_CFI_PERF*/ DISABLE_CFI_ICALL InstrHandlerRetType VECTORCALL
...
// Compacts two consecutive CopySlot32 or CopySlot64 instructions into a single
// instruction, to save one dispatch.
#define FLAG_drumbrake_compact_copyslot v8_flags.drumbrake_super_instructions;
...
class V8_EXPORT_PRIVATE WasmInterpreter {
 public:
  ...
  static bool HandleWasmTrap(uintptr_t fault_addr, uintptr_t* landing_pad);
...
class WasmBytecodeGenerator {
 ...
 private:
...
#ifdef FLAG_drumbrake_compact_copyslot
    if (TryCompactInstructionHandler(func)) return;
#endif  // FLAG_drumbrake_compact_copyslot
  ...
  template <>
  inline uint32_t CreateSlot<WasmRef>(ValueType value_type) {
    uint32_t slot_index = static_cast<uint32_t>(slots_.size());
  ...
  template <>
  inline uint32_t GetConstSlot(int32_t value) {
  ...
  template <>
  inline uint32_t GetConstSlot(int64_t value) {
  ...
  template <>
  inline uint32_t GetConstSlot(float value) {
  ...
  template <>
  inline uint32_t GetConstSlot(double value) {
  ...
  template <>
  inline uint32_t GetConstSlot(Simd128 value) {
    static_assert(sizeof(Simd128_DB) == sizeof(Simd128));
    auto it = s128_const_cache_.find(reinterpret_cast<Simd128_DB&>(value));
  ...
  template <>
  uint32_t CreateConstSlot<WasmRef>(WasmRef value) {
    UNREACHABLE();
  }
  ...
  // This purpose of this class is just to avoid modifying {Simd128}, in order
  // to minimize merge conflicts. {Simd128_DB} objects are stored in a map in
  // place of {Simd128}s, and this struct provides an additional equality
  // operator and hashing method.
  struct Simd128_DB : public Simd128 {
    bool operator==(const Simd128_DB& rhs) const {
      static_assert(sizeof(size_t) == sizeof(uint64_t));
      const int2 slhs = to_i64x2();
      const int2 srhs = rhs.to_i64x2();
      return (slhs.val[0] == srhs.val[0]) && (slhs.val[1] == srhs.val[1]);
    }

    struct hash {
      size_t operator()(const Simd128_DB& s128) const {
        return s128.hash_value();
      }
    };
  ...
  std::unordered_map<Simd128_DB, uint32_t, Simd128_DB::hash> s128_const_cache_;
