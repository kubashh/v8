...
class WasmInterpreterRuntime {
 public:
  ...
  bool WasmArrayCopy(WasmRef dest_wasm_array, uint32_t dest_index,
                     ...
                     uint32_t length);
  WasmRef WasmJSToWasmObject(WasmRef extern_ref, ValueType value_type) const;
...
#pragma clang diagnostic pop
  ...
  struct IndirectCallValue {
    ...
    static const uint32_t kInvalidSignatureIndex = UINT_MAX;
    static const uint32_t kInvalidFunctionIndex = UINT_MAX;

    IndirectCallValue()
        : mode(Mode::kInvalid),
          func_index(kInvalidFunctionIndex),
          sig_index(kInvalidSignatureIndex),
          signature(nullptr) {}
