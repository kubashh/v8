...
#define EMIT_INSTR_HANDLER_1(name) EmitFnId(k_##name);
#define EMIT_INSTR_HANDLER_2(name, pc) EmitFnId(k_##name, pc);
#define EMIT_INSTR_HANDLER_X(x, A, B, FUNC, ...) FUNC
#define EMIT_INSTR_HANDLER(...)                                            \
  EMIT_INSTR_HANDLER_X(, ##__VA_ARGS__, EMIT_INSTR_HANDLER_2(__VA_ARGS__), \
                       EMIT_INSTR_HANDLER_1(__VA_ARGS__))
  ...
  if (samples_.size() == slow_threshold_samples_count_) {
    ...
    if (average >= slow_threshold_) {
      ...
      if (isolate_ && !isolate_->context().is_null()) {
        ...
        v8::metrics::WasmModuleProcessResults event;
        ...
        isolate_->metrics_recorder()->DelayMainThreadEvent(event, context_id);
      }
    }
...
struct InstructionHandleCodeRange {
  Address begin;
  Address end;
};
...
InstructionHandleCodeRange
    kTrapCodeRanges[BoundsCheckedHandlersCounter::kTotalItems];
...
void InitTrapHandlersOnce() {
  ...
  size_t index = 0;
#define V(name)                                                \
  kTrapCodeRanges[index++] = {                                 \
      reinterpret_cast<Address>(kInstructionTable[k_##name]),  \
      reinterpret_cast<Address>(kInstructionTable[k_##name]) + \
          embedded_data.InstructionSizeOf(Builtin::k##name)};
...
WasmInterpreter::WasmInterpreter(Isolate* isolate, const WasmModule* module,
  ...
  base::CallOnce(&init_instruction_table_once, &InitInstructionTableOnce,
                 isolate);
  base::CallOnce(&init_trap_handlers_once, &InitTrapHandlersOnce);
#endif  // !V8_DRUMBRAKE_BOUNDS_CHECKS
}
...
#define NextOp()                                                       \
  ...
  [[clang::musttail]] return kInstructionTable[ReadFnId(code) &        \
                                               kInstructionTableMask]( \
      code, sp, wasm_runtime, r0, fp0)

#else  // DRUMBRAKE_ENABLE_PROFILING

// TODO(paolosev@microsoft.com) - MSVC support.
...
namespace {
static DISABLE_CFI_ICALL void VECTORCALL
Trap(const uint8_t* code, uint32_t* sp, WasmInterpreterRuntime* wasm_runtime,
     int64_t r0, double fp0) {
  TrapReason trap_reason = static_cast<TrapReason>(r0);
  wasm_runtime->SetTrap(trap_reason, code);
  [[clang::musttail]] return s_unwind_func_addr(code, sp, wasm_runtime,
                                                trap_reason, .0);
}
}  // namespace
#define TRAP(trap_reason) \
  [[clang::musttail]] return Trap(code, sp, wasm_runtime, trap_reason, fp0);
...
#include "src/wasm/interpreter/wasm-interpreter-simd-execute.h"
  ...
  for (uint32_t i = struct_type->field_count(); i > 0;) {
    ...
    switch (kind) {
      ...
      case kF32:
        ...
        break;
      case kF64:
        base::WriteUnalignedValue<double>(field_addr,
                                          pop<double>(sp, code, wasm_runtime));
        break;
      case kS128:
        base::WriteUnalignedValue<Simd128>(
...
INSTRUCTION_HANDLER_FUNC s2s_StructGet(const uint8_t* code, uint32_t* sp,
  ...
  Address field_addr = (*struct_obj).ptr() + offset;
  push<T>(sp, code, wasm_runtime,
          base::ReadUnalignedValue<U>(reinterpret_cast<Address>(field_addr)));
...
INSTRUCTION_HANDLER_FUNC s2s_RefStructGet(const uint8_t* code, uint32_t* sp,
  ...
  Tagged_t ref_tagged =
      base::ReadUnalignedValue<uint32_t>(reinterpret_cast<Address>(field_addr));
...
INSTRUCTION_HANDLER_FUNC s2s_ArrayNew(const uint8_t* code, uint32_t* sp,
  ...
  for (uint32_t i = 0; i < elem_count; i++) {
    base::WriteUnalignedValue<U>(reinterpret_cast<Address>(element_addr),
                                 value);
...
INSTRUCTION_HANDLER_FUNC s2s_RefArrayNew(const uint8_t* code, uint32_t* sp,
  ...
  for (uint32_t i = 0; i < elem_count; i++) {
    base::WriteUnalignedValue<Tagged_t>(
        element_addr, V8HeapCompressionScheme::CompressObject((*value).ptr()));
  ...
  for (uint32_t i = 0; i < elem_count; i++) {
    switch (kind) {
      ...
      case kF32:
        ...
        break;
      case kF64:
        base::WriteUnalignedValue<double>(element_addr,
                                          pop<double>(sp, code, wasm_runtime));
        break;
      case kS128:
        base::WriteUnalignedValue<Simd128>(
...
s2s_ArrayNewDefault(const uint8_t* code, uint32_t* sp,
  ...
  for (uint32_t i = 0; i < elem_count; i++) {
    switch (kind) {
      ...
      case kRefNull:
        base::WriteUnalignedValue<Tagged_t>(
            element_addr, static_cast<Tagged_t>(
...
INSTRUCTION_HANDLER_FUNC s2s_ArrayLen(const uint8_t* code, uint32_t* sp,
  ...
  Tagged<WasmArray> array = WasmArray::cast(*array_obj);
...
INSTRUCTION_HANDLER_FUNC s2s_ArrayCopy(const uint8_t* code, uint32_t* sp,
  ...
  if (V8_UNLIKELY(wasm_runtime->IsRefNull(src_array) ||
                  wasm_runtime->IsRefNull(dest_array))) {
    TRAP(TrapReason::kTrapNullDereference)
  }
...
INSTRUCTION_HANDLER_FUNC s2s_AnyConvertExtern(
  ...
  WasmRef result = wasm_runtime->WasmJSToWasmObject(extern_ref, kWasmAnyRef);
  if (V8_UNLIKELY(result.is_null())) {
    wasm::TrapReason reason = WasmInterpreterThread::GetRuntimeLastWasmError(
        wasm_runtime->GetIsolate());
    INLINED_TRAP(reason)
...
WasmInstruction WasmBytecodeGenerator::DecodeInstruction(pc_t pc,
  ...
  switch (orig) {
    ...
    case kExprTry: {
      ...
      if (imm.sig_index != kInvalidSignatureIndex) {
        ...
        optional.block.sig_index = kInvalidSignatureIndex;
...
#define LOAD_CASE(name, ctype, mtype, rep, type)                        \
  case kExpr##name: {                                                   \
    MemoryAccessImmediate imm(                                          \
        ...
        module_->memories.size() > 1, Decoder::kNoValidation);          \
    len = 1 + imm.length;                                               \
...
void WasmBytecodeGenerator::DecodeGCOp(WasmOpcode opcode,
  ...
  switch (opcode) {
    ...
    case kExprRefTestNull: {
      HeapTypeImmediate imm(WasmFeatures::All(), decoder, code->at(pc + *len),
                            Decoder::kNoValidation);
    ...
    case kExprBrOnCastFail: {
      ...
      *len += flags_imm.length;
      BranchDepthImmediate branch(decoder, code->at(pc + *len),
                                  Decoder::kNoValidation);
      *len += branch.length;
      HeapTypeImmediate source_imm(WasmFeatures::All(), decoder,
...
// static
bool WasmInterpreter::HandleWasmTrap(uintptr_t fault_addr,
                                     uintptr_t* landing_pad) {
  bool found = false;
#if !defined(V8_DRUMBRAKE_BOUNDS_CHECKS)
  for (size_t i = 0; i < BoundsCheckedHandlersCounter::kTotalItems; i++) {
    if (fault_addr > kTrapCodeRanges[i].begin &&
        fault_addr < kTrapCodeRanges[i].end) {
      found = true;
      break;
    }
  }
  ...
  if (found) {
    *landing_pad = reinterpret_cast<uintptr_t>(TrapMemOutOfBounds);
  }
#endif  // !V8_DRUMBRAKE_BOUNDS_CHECKS
  return found;
}
...
RegMode WasmBytecodeGenerator::EncodeInstruction(const WasmInstruction& instr,
  ...
  switch (instr.opcode) {
    case kExprUnreachable: {
      EMIT_INSTR_HANDLER(s2s_Unreachable, instr.pc);
      SetUnreachableMode();
      break;
    }
    case kExprNop:
    ...
    case kExprReturnCall: {
      ...
      if (is_imported) {
        if (is_tail_call) {
          EMIT_INSTR_HANDLER(s2s_ReturnCallImportedFunction, instr.pc);
          ...
          EMIT_INSTR_HANDLER(s2s_CallImportedFunction, instr.pc);
        }
      } else {
        if (is_tail_call) {
          EMIT_INSTR_HANDLER(s2s_ReturnCall, instr.pc);
          EmitI32Const(WasmBytecode::RetsSizeInSlots(sig) * kSlotSize);
          EmitI32Const(WasmBytecode::ArgsSizeInSlots(sig) * kSlotSize);
          EmitI32Const(WasmBytecode::RefRetsCount(sig));
          EmitI32Const(WasmBytecode::RefArgsCount(sig));
        } else {
          EMIT_INSTR_HANDLER(s2s_CallFunction, instr.pc);
        }
      }
    ...
    case kExprReturnCallIndirect: {
      ...
      if (is_tail_call) {
        EMIT_INSTR_HANDLER(s2s_ReturnCallIndirect, instr.pc);
        EmitI32Const(WasmBytecode::RetsSizeInSlots(sig) * kSlotSize);
        EmitI32Const(WasmBytecode::ArgsSizeInSlots(sig) * kSlotSize);
        EmitI32Const(WasmBytecode::RefRetsCount(sig));
        EmitI32Const(WasmBytecode::RefArgsCount(sig));
      } else {
        EMIT_INSTR_HANDLER(s2s_CallIndirect, instr.pc);
      }
    ...
    case kExprReturnCallRef: {
      ...
      if (is_tail_call) {
        EMIT_INSTR_HANDLER(s2s_ReturnCallRef, instr.pc);
        EmitI32Const(WasmBytecode::RetsSizeInSlots(sig) * kSlotSize);
        EmitI32Const(WasmBytecode::ArgsSizeInSlots(sig) * kSlotSize);
        EmitI32Const(WasmBytecode::RefRetsCount(sig));
        EmitI32Const(WasmBytecode::RefArgsCount(sig));
      } else {
        EMIT_INSTR_HANDLER(s2s_CallRef, instr.pc);
      }
    ...
    case kExprTableGet: {
      EMIT_INSTR_HANDLER(s2s_TableGet, instr.pc);
      EmitI32Const(instr.optional.index);
      I32Pop();
    ...
    case kExprTableSet: {
      EMIT_INSTR_HANDLER(s2s_TableSet, instr.pc);
      EmitI32Const(instr.optional.index);
      RefPop();
      I32Pop();
      break;
...
#define LOAD_CASE(name, ctype, mtype, rep, type)  \
  case kExpr##name: {                             \
    switch (mode) {                               \
      case kR2R:                                  \
        EMIT_INSTR_HANDLER(r2r_##name, instr.pc); \
        EmitI64Const(instr.optional.offset);      \
        return RegMode::k##type##Reg;             \
      case kR2S:                                  \
        EMIT_INSTR_HANDLER(r2s_##name, instr.pc); \
        EmitI64Const(instr.optional.offset);      \
        type##Push();                             \
        return RegMode::kNoReg;                   \
      case kS2R:                                  \
        EMIT_INSTR_HANDLER(s2r_##name, instr.pc); \
        EmitI64Const(instr.optional.offset);      \
        I32Pop();                                 \
        return RegMode::k##type##Reg;             \
      case kS2S:                                  \
        EMIT_INSTR_HANDLER(s2s_##name, instr.pc); \
        EmitI64Const(instr.optional.offset);      \
        I32Pop();                                 \
        type##Push();                             \
        return RegMode::kNoReg;                   \
...
#undef EXECUTE_UNOP
    ...
    case kExprRefAsNonNull: {
      EMIT_INSTR_HANDLER(s2s_RefAsNonNull, instr.pc);
      ValueType value_type = RefPop();
      RefPush(value_type);
      break;
    }
    ...
    case kExprStructGetU: {
      ...
      switch (kind) {
        case kI8:
          if (is_signed) {
            EMIT_INSTR_HANDLER(s2s_I8SStructGet, instr.pc);
          } else {
            EMIT_INSTR_HANDLER(s2s_I8UStructGet, instr.pc);
          }
          RefPop();
          EmitI32Const(offset);
          I32Push();
        ...
        case kI16:
          if (is_signed) {
            EMIT_INSTR_HANDLER(s2s_I16SStructGet, instr.pc);
          } else {
            EMIT_INSTR_HANDLER(s2s_I16UStructGet, instr.pc);
          }
          RefPop();
          EmitI32Const(offset);
          I32Push();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32StructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          I32Push();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64StructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          I64Push();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32StructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          F32Push();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64StructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          F64Push();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128StructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          S128Push();
          break;
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefStructGet, instr.pc);
          RefPop();
          EmitI32Const(offset);
          RefPush(value_type);
          break;
    ...
    case kExprStructSet: {
      ...
      switch (kind) {
        case kI8:
          EMIT_INSTR_HANDLER(s2s_I8StructSet, instr.pc);
          EmitI32Const(offset);
          I32Pop();
          break;
        case kI16:
          EMIT_INSTR_HANDLER(s2s_I16StructSet, instr.pc);
          EmitI32Const(offset);
          I32Pop();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32StructSet, instr.pc);
          EmitI32Const(offset);
          I32Pop();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64StructSet, instr.pc);
          EmitI32Const(offset);
          I64Pop();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32StructSet, instr.pc);
          EmitI32Const(offset);
          F32Pop();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64StructSet, instr.pc);
          EmitI32Const(offset);
          F64Pop();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128StructSet, instr.pc);
          EmitI32Const(offset);
          S128Pop();
          break;
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefStructSet, instr.pc);
          EmitI32Const(offset);
          RefPop();
          break;
        default:
    ...
    case kExprArrayNew: {
      ...
      switch (kind) {
        case kI8:
          EMIT_INSTR_HANDLER(s2s_I8ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();  // Array length.
          I32Pop();  // Initialization value.
          break;
        case kI16:
          EMIT_INSTR_HANDLER(s2s_I16ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          I32Pop();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          I32Pop();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          I64Pop();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          F32Pop();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          F64Pop();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128ArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          S128Pop();
          break;
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefArrayNew, instr.pc);
          EmitI32Const(array_index);
          I32Pop();
          RefPop();
          break;
    ...
    case kExprArrayNewFixed: {
      EMIT_INSTR_HANDLER(s2s_ArrayNewFixed, instr.pc);
    ...
    case kExprArrayNewDefault: {
      EMIT_INSTR_HANDLER(s2s_ArrayNewDefault, instr.pc);
      EmitI32Const(instr.optional.index);
      I32Pop();
    ...
    case kExprArrayNewData: {
      EMIT_INSTR_HANDLER(s2s_ArrayNewData, instr.pc);
    ...
    case kExprArrayNewElem: {
      EMIT_INSTR_HANDLER(s2s_ArrayNewElem, instr.pc);
      uint32_t array_index =
          instr.optional.gc_array_new_or_init_data.array_index;
      EmitI32Const(array_index);
      uint32_t data_index = instr.optional.gc_array_new_or_init_data.data_index;
    ...
    case kExprArrayInitData: {
      EMIT_INSTR_HANDLER(s2s_ArrayInitData, instr.pc);
      uint32_t array_index =
          instr.optional.gc_array_new_or_init_data.array_index;
      EmitI32Const(array_index);
      uint32_t data_index = instr.optional.gc_array_new_or_init_data.data_index;
    ...
    case kExprArrayInitElem: {
      EMIT_INSTR_HANDLER(s2s_ArrayInitElem, instr.pc);
      uint32_t array_index =
          instr.optional.gc_array_new_or_init_data.array_index;
      EmitI32Const(array_index);
      uint32_t data_index = instr.optional.gc_array_new_or_init_data.data_index;
    ...
    case kExprArrayLen: {
      EMIT_INSTR_HANDLER(s2s_ArrayLen, instr.pc);
      RefPop();
      I32Push();
      break;
    }
    ...
    case kExprArrayCopy: {
      EMIT_INSTR_HANDLER(s2s_ArrayCopy, instr.pc);
    ...
    case kExprArrayGetU: {
      ...
      switch (kind) {
        case kI8:
          if (is_signed) {
            EMIT_INSTR_HANDLER(s2s_I8SArrayGet, instr.pc);
          } else {
            EMIT_INSTR_HANDLER(s2s_I8UArrayGet, instr.pc);
          }
          I32Pop();
          RefPop();
          I32Push();
        ...
        case kI16:
          if (is_signed) {
            EMIT_INSTR_HANDLER(s2s_I16SArrayGet, instr.pc);
          } else {
            EMIT_INSTR_HANDLER(s2s_I16UArrayGet, instr.pc);
          }
          I32Pop();
          RefPop();
          I32Push();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32ArrayGet, instr.pc);
          I32Pop();
          RefPop();
          I32Push();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64ArrayGet, instr.pc);
          I32Pop();
          RefPop();
          I64Push();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32ArrayGet, instr.pc);
          I32Pop();
          RefPop();
          F32Push();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64ArrayGet, instr.pc);
          I32Pop();
          RefPop();
          F64Push();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128ArrayGet, instr.pc);
          I32Pop();
          RefPop();
          S128Push();
          break;
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefArrayGet, instr.pc);
          I32Pop();
          RefPop();
    ...
    case kExprArraySet: {
      ...
      switch (kind) {
        case kI8:
          EMIT_INSTR_HANDLER(s2s_I8ArraySet, instr.pc);
          I32Pop();
          I32Pop();
          RefPop();
          break;
        case kI16:
          EMIT_INSTR_HANDLER(s2s_I16ArraySet, instr.pc);
          I32Pop();
          I32Pop();
          RefPop();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32ArraySet, instr.pc);
          I32Pop();
          I32Pop();
          RefPop();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64ArraySet, instr.pc);
          I64Pop();
          I32Pop();
          RefPop();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32ArraySet, instr.pc);
          F32Pop();
          I32Pop();
          RefPop();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64ArraySet, instr.pc);
          F64Pop();
          I32Pop();
          RefPop();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128ArraySet, instr.pc);
          S128Pop();
          I32Pop();
          RefPop();
          break;
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefArraySet, instr.pc);
          RefPop();
          I32Pop();
          RefPop();
          break;
    ...
    case kExprArrayFill: {
      ...
      switch (kind) {
        case kI8:
          EMIT_INSTR_HANDLER(s2s_I8ArrayFill, instr.pc);
          ...
          break;
        case kI16:
          EMIT_INSTR_HANDLER(s2s_I16ArrayFill, instr.pc);
          I32Pop();
          I32Pop();
          I32Pop();
          RefPop();
          break;
        case kI32:
          EMIT_INSTR_HANDLER(s2s_I32ArrayFill, instr.pc);
          I32Pop();
          I32Pop();
          I32Pop();
          RefPop();
          break;
        case kI64:
          EMIT_INSTR_HANDLER(s2s_I64ArrayFill, instr.pc);
          I32Pop();
          I64Pop();
          I32Pop();
          RefPop();
          break;
        case kF32:
          EMIT_INSTR_HANDLER(s2s_F32ArrayFill, instr.pc);
          I32Pop();
          F32Pop();
          I32Pop();
          RefPop();
          break;
        case kF64:
          EMIT_INSTR_HANDLER(s2s_F64ArrayFill, instr.pc);
          I32Pop();
          F64Pop();
          I32Pop();
          RefPop();
          break;
        case kS128:
          EMIT_INSTR_HANDLER(s2s_S128ArrayFill, instr.pc);
          I32Pop();
          S128Pop();
          I32Pop();
          RefPop();
        ...
        case kRef:
        case kRefNull:
          EMIT_INSTR_HANDLER(s2s_RefArrayFill, instr.pc);
          I32Pop();
          RefPop();
          I32Pop();
          RefPop();
    ...
    case kExprI31GetS: {
      EMIT_INSTR_HANDLER(s2s_I31GetS, instr.pc);
      RefPop();
      I32Push();
      break;
    }
    ...
    case kExprI31GetU: {
      EMIT_INSTR_HANDLER(s2s_I31GetU, instr.pc);
      RefPop();
      I32Push();
      break;
    }
    ...
    case kExprRefCastNull: {
      ...
      if (V8_UNLIKELY(TypeCheckAlwaysSucceeds(obj_type, target_type))) {
        if (obj_type.is_nullable() && !null_succeeds) {
          EMIT_INSTR_HANDLER(s2s_AssertNotNullTypecheck, instr.pc);
          ValueType value_type = RefPop();
          EmitI32Const(value_type.raw_bit_field());
        ...
        // is null.
        if (obj_type.is_nullable() && null_succeeds) {
          EMIT_INSTR_HANDLER(s2s_AssertNullTypecheck, instr.pc);
          ValueType value_type = RefPop();
          EmitI32Const(value_type.raw_bit_field());
          RefPush(resulting_value_type);
        } else {
          // In this case we just trap.
          EMIT_INSTR_HANDLER(s2s_TrapIllegalCast, instr.pc);
        }
      } else {
        if (instr.opcode == kExprRefCast) {
          EMIT_INSTR_HANDLER(s2s_RefCast, instr.pc);
        } else {
          EMIT_INSTR_HANDLER(s2s_RefCastNull, instr.pc);
        }
    ...
    case kExprAnyConvertExtern: {
      EMIT_INSTR_HANDLER(s2s_AnyConvertExtern, instr.pc);
    ...
    case kExprMemoryInit:
      EMIT_INSTR_HANDLER(s2s_MemoryInit, instr.pc);
      EmitI32Const(instr.optional.index);
      I32Pop();
      I32Pop();
      I32Pop();
    ...
    case kExprMemoryCopy:
      EMIT_INSTR_HANDLER(s2s_MemoryCopy, instr.pc);
      I32Pop();
      I32Pop();
      I32Pop();
      break;
    ...
    case kExprMemoryFill:
      EMIT_INSTR_HANDLER(s2s_MemoryFill, instr.pc);
      I32Pop();
      I32Pop();
      I32Pop();
      break;
    ...
    case kExprTableInit:
      EMIT_INSTR_HANDLER(s2s_TableInit, instr.pc);
    ...
    case kExprTableCopy:
      EMIT_INSTR_HANDLER(s2s_TableCopy, instr.pc);
    ...
    case kExprTableFill:
      EMIT_INSTR_HANDLER(s2s_TableFill, instr.pc);
      EmitI32Const(instr.optional.index);
      I32Pop();
      RefPop();
      I32Pop();
    ...
    case kExprAtomicNotify:
      EMIT_INSTR_HANDLER(s2s_AtomicNotify, instr.pc);
      I32Pop();  // val
      EmitI64Const(instr.optional.offset);
      I32Pop();  // memory index
      I32Push();
    ...
    case kExprI32AtomicWait:
      EMIT_INSTR_HANDLER(s2s_I32AtomicWait, instr.pc);
      I64Pop();  // timeout
      I32Pop();  // val
      EmitI64Const(instr.optional.offset);
      I32Pop();  // memory index
    ...
    case kExprI64AtomicWait:
      EMIT_INSTR_HANDLER(s2s_I64AtomicWait, instr.pc);
      I64Pop();  // timeout
      I64Pop();  // val
      EmitI64Const(instr.optional.offset);
      I32Pop();  // memory index
...
#include "src/wasm/interpreter/wasm-interpreter-simd-encode.h"
...
#define LOAD_CASE(name, ctype, mtype, rep, type)                       \
  case kExpr##name: {                                                  \
    if (reg_mode == RegMode::kNoReg) {                                 \
      EMIT_INSTR_HANDLER(s2s_##name##_LocalSet, curr_instr.pc);        \
      ...
      EMIT_INSTR_HANDLER(r2s_##name, curr_instr.pc);                   \
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset)); \
      EmitI32Const(slots_[stack_[to_stack_index]].slot_offset);        \
      reg_mode = RegMode::kNoReg;                                      \
    }                                                                  \
...
#undef LOAD_CASE
    ...
    if (reg_mode == RegMode::kNoReg) {
      EMIT_INSTR_HANDLER(s2s_I32LoadStoreMem, curr_instr.pc);
      EmitI64Const(
          static_cast<uint64_t>(curr_instr.optional.offset));  // load_offset
      I32Pop();                                                // load_index
    } else {
      EMIT_INSTR_HANDLER(r2s_I32LoadStoreMem, curr_instr.pc);
      EmitI64Const(
          static_cast<uint64_t>(curr_instr.optional.offset));  // load_offset
    }
    EmitI64Const(
    ...
    if (reg_mode == RegMode::kNoReg) {
      EMIT_INSTR_HANDLER(s2s_I64LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
      I32Pop();
    } else {
      EMIT_INSTR_HANDLER(r2s_I64LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
    }
    ...
    if (reg_mode == RegMode::kNoReg) {
      EMIT_INSTR_HANDLER(s2s_F32LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
      I32Pop();
    } else {
      EMIT_INSTR_HANDLER(r2s_F32LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
    }
    EmitI64Const(static_cast<uint64_t>(next_instr.optional.offset));
    I32Pop();
    ...
    if (reg_mode == RegMode::kNoReg) {
      EMIT_INSTR_HANDLER(s2s_F64LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
      I32Pop();
    } else {
      EMIT_INSTR_HANDLER(r2s_F64LoadStoreMem, curr_instr.pc);
      EmitI64Const(static_cast<uint64_t>(curr_instr.optional.offset));
    }
    EmitI64Const(static_cast<uint64_t>(next_instr.optional.offset));
    I32Pop();
...
#define STORE_CASE(name, ctype, mtype, rep, type)                        \
  case kExpr##name: {                                                    \
    EMIT_INSTR_HANDLER(s2s_##name, curr_instr.pc);                       \
