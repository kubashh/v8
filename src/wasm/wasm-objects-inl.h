...
void WasmTrustedInstanceData::clear_padding() {
  ...
  static_assert(kPaddingBytes == 0 || kPaddingBytes == kInt32Size);
  if constexpr (kPaddingBytes != 0) {
    base::Memory<int32_t>(address() + kOptionalPaddingOffset) = 0;
  }
}
...
void WasmDispatchTable::clear_padding() {
  static_assert(kPaddingBytes == 0 || kPaddingBytes == kInt32Size);
  if constexpr (kPaddingBytes != 0) {
    base::Memory<int32_t>(address() + kPaddingOffset) = 0;
  }
}
