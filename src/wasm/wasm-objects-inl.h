...
struct WasmObject::ConversionHelper {
  ...
  static ElementType FromObject(Object value) {
    if (value.IsSmi()) {
      return FromScalar(Smi::ToInt(value));
    } else if (value.IsHeapNumber()) {
      return FromScalar(HeapNumber::cast(value).value());
    } else if (value.IsBigInt()) {
      return FromScalar(BigInt::cast(value).AsInt64());
    } else {
      // TODO(v8:11804): review decision to store zero for non-numeric values.
      DCHECK(!value.IsNumeric());
      return 0;
    }
  }
};
...
void WasmObject::WriteValueAt(Isolate* isolate, Handle<HeapObject> obj,
  ...
  switch (type.kind()) {
    case wasm::kI8: {
      auto scalar_value = ConversionHelper<int8_t>::FromObject(*value);
      base::Memory<int8_t>(field_address) = scalar_value;
      break;
    }
    case wasm::kI16: {
      auto scalar_value = ConversionHelper<int16_t>::FromObject(*value);
      base::Memory<int16_t>(field_address) = scalar_value;
      break;
    }
    case wasm::kI32: {
      auto scalar_value = ConversionHelper<int32_t>::FromObject(*value);
      base::Memory<int32_t>(field_address) = scalar_value;
      break;
    }
    case wasm::kI64: {
      auto scalar_value = ConversionHelper<int64_t>::FromObject(*value);
      base::WriteUnalignedValue<int64_t>(field_address, scalar_value);
      break;
    }
    case wasm::kF32: {
      auto scalar_value = ConversionHelper<float>::FromObject(*value);
      base::Memory<float>(field_address) = scalar_value;
      break;
    }
    case wasm::kF64: {
      auto scalar_value = ConversionHelper<double>::FromObject(*value);
