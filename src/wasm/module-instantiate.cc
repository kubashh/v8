...
bool InstanceBuilder::ProcessImportedFunction(
  ...
  switch (kind) {
    ...
    case ImportCallKind::kWasmToWasm: {
      ...
      auto wefd = Cast<WasmExportedFunctionData>(trusted_function_data);
      // The import reference is the trusted instance data itself.
      Tagged<WasmTrustedInstanceData> wtid = wefd->instance_data();
      Address imported_target = wtid->GetCallTarget(wefd->function_index());
      imported_entry.SetWasmToWasm(wtid, imported_target);
      break;
    }
