...
MaybeHandle<WasmInstanceObject> InstanceBuilder::Build() {
  ...
  // instance.
  //--------------------------------------------------------------------------
  Handle<FixedArray> memory_objects = isolate_->factory()->NewFixedArray(
      static_cast<int>(module_->memories.size()));
  if (is_asmjs_module(module_)) {
    ...
    memory_objects->set(kMemoryIndexZero, *memory_object);
  } else {
...
bool InstanceBuilder::ProcessImportedMemory(Handle<WasmInstanceObject> instance,
  ...
  CHECK_EQ(instance->memory_objects().get(memory_index), *memory_object);
