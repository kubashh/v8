...
bool InstanceBuilder::ProcessImportedFunction(
  ...
  switch (kind) {
    ...
    case compiler::WasmImportCallKind::kWasmToHost: {
      NativeModule* native_module = instance->module_object()->native_module();
      Address host_address =
          WasmCapiFunction::cast(*value)->GetHostCallTarget();
      WasmCodeRefScope code_ref_scope;
      // TODO(wasm): no need to hold the lock while compiling a host call.
      WasmCode* wasm_code = compiler::CompileWasmHostCallWrapper(
