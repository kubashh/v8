...
void FindAllInstances(Isolate* isolate, Handle<WasmModuleObject> module_object,
  ...
  WeakArrayList* weak_instance_list = module_object->weak_instance_list();
  for (int i = 0; i < weak_instance_list->length(); ++i) {
    MaybeObject* maybe_instance = weak_instance_list->Get(i);
    if (maybe_instance->IsWeakHeapObject()) {
      instances.push_back(
          handle(WasmInstanceObject::cast(maybe_instance->ToWeakHeapObject()),
                 isolate));
    }
  }
}
...
Handle<WasmInstanceObject> WasmInstanceObject::New(
  ...
  Handle<WeakArrayList> weak_instance_list(module_object->weak_instance_list(),
                                           isolate);
  weak_instance_list =
      WeakArrayList::AddToEnd(weak_instance_list, MaybeObjectHandle(instance));
