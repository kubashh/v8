...
bool WasmInstanceObject::CopyTableEntries(Isolate* isolate,
  ...
  if (dst > max) return false;          // out-of-bounds
  if (src > max) return false;          // out-of-bounds
  if (dst + count > max) return false;  // out-of-bounds
  if (src + count > max) return false;  // out-of-bounds
