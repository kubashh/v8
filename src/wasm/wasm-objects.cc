...
void WasmTableObject::UpdateDispatchTables(
  ...
  if (target_func_index <
    ...
    target_func_index = target_instance_data->imported_function_indexes()->get(
        target_func_index);
  }
#endif  // V8_WASM_INTERPRETER
  ...
  for (int i = 0, len = uses->length(); i < len; i += TableUses::kNumElements) {
    ...
    instance_data->dispatch_table(table_index)
        ->Set(entry_index, *call_ref, call_target, sig_id, target_func_index);
#endif  // !V8_WASM_INTERPRETER
  }
}
...
#if V8_WASM_INTERPRETER
    target_func_index_ =
        trusted_target_instance_data->imported_function_indexes()->get(
            target_func_index);
#endif  // V8_WASM_INTERPRETER
  } else {
...
#if V8_WASM_INTERPRETER
  trusted_instance_data->imported_function_indexes()->set(index_, -1);
#endif  // V8_WASM_INTERPRETER
}
...
void ImportedFunctionEntry::SetWasmToWasm(
  ...
  trusted_instance_data->imported_function_indexes()->set(
...
#if V8_WASM_INTERPRETER
  Handle<FixedInt32Array> imported_function_indexes =
...
#if V8_WASM_INTERPRETER
    trusted_data->set_imported_function_indexes(*imported_function_indexes);
#endif  // V8_WASM_INTERPRETER
...
void WasmDispatchTable::Set(int index, Tagged<Object> ref, Address call_target,
                            int sig_id, uint32_t function_index) {
#else   // V8_WASM_INTERPRETER
void WasmDispatchTable::Set(int index, Tagged<Object> ref, Address call_target,
                            int sig_id) {
#endif  // V8_WASM_INTERPRETER
...
#else   // !V8_WASM_INTERPRETER
    new_table->Set(i, old_table->ref(i), old_table->target(i),
                   old_table->sig(i), old_table->function_index(i));
#endif  // !V8_WASM_INTERPRETER
  }
  return new_table;
}
