...
Handle<WasmJSFunction> WasmJSFunction::New(Isolate* isolate,
  ...
  Handle<PodArray<wasm::ValueType>> serialized_sig =
      PodArray<wasm::ValueType>::New(isolate, sig_size);
...
wasm::FunctionSig* WasmJSFunction::GetSignature(Zone* zone) {
  ...
  int size = function_data.serialized_signature().length();
  wasm::ValueType* types = zone->NewArray<wasm::ValueType>(size);
  function_data.serialized_signature().copy_out(0, types, size);
