...
Handle<JSArrayBuffer> GrowMemoryBuffer(Isolate* isolate,
  ...
  // If guard pages are enabled, adjust memory protection, and use the same
  // backing store to create a new ArrayBuffer, as the old one might be
  // detached on Grow.
  if (enable_guard_regions && old_size != 0) {
    DCHECK(old_buffer->backing_store() != nullptr);
    isolate->array_buffer_allocator()->SetProtection(
        old_mem_start + old_size, pages * WasmModule::kPageSize,
