...
void WasmDispatchTable::Clear(int index) {
  DCHECK_LT(index, length());
  const int offset = OffsetOf(index);
  RawProtectedPointerField(offset + kRefBias).store(Smi::zero());
  WriteField<Address>(offset + kTargetBias, 0);
...
Handle<WasmDispatchTable> WasmDispatchTable::Grow(
  ...
  DCHECK_LE(old_length, new_length);
  if (old_length == new_length) return old_table;  // Nothing to do.
  ...
  int grow = old_capacity < 8 ? 8 : std::min(max_grow, old_capacity);
