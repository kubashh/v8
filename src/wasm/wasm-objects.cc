...
void IndirectFunctionTableEntry::set(int sig_id,
  ...
  PrintF("IFT entry %p[%d] = {sig_id=%d, instance=%p, target=%p}\n", *instance_,
         index_, sig_id, *instance, wasm_code->instructions().start());
...
void ImportedFunctionEntry::set(Handle<JSReceiver> callable,
  ...
  PrintF("Import callable %p[%d] = {callable=%p, target=%p}\n", *instance_,
...
void ImportedFunctionEntry::set(Handle<WasmInstanceObject> instance,
                                const wasm::WasmCode* wasm_code) {
  PrintF("Import WASM %p[%d] = {instance=%p, target=%p}\n", *instance_, index_,
         *instance, wasm_code->instructions().start());
...
bool ImportedFunctionEntry::is_wasm_to_wasm_entry() {
  return !instance_->imported_function_callables()->get(index_)->IsJSReceiver();
}
...
bool WasmInstanceObject::EnsureIndirectFunctionTableWithMinimumSize(
  ...
  if (has_indirect_function_table()) {
    // Reallocate the old storage.
    new_sig_ids =
        realloc(indirect_function_table_sig_ids(), new_size * sizeof(uint32_t));
    new_targets =
        realloc(indirect_function_table_targets(), new_size * sizeof(Address));

    Handle<FixedArray> old(indirect_function_table_instances(), isolate);
  ...
  set_indirect_function_table_sig_ids(reinterpret_cast<uint32_t*>(new_sig_ids));
  set_indirect_function_table_targets(reinterpret_cast<Address*>(new_targets));
  set_indirect_function_table_instances(*new_instances);
...
Handle<WasmInstanceObject> WasmInstanceObject::New(
  ...
  instance->set_imported_function_targets(reinterpret_cast<Address*>(
...
void InstanceFinalizer(const v8::WeakCallbackInfo<void>& data) {
  ...
  if (instance->has_indirect_function_table()) {
    free(instance->indirect_function_table_sig_ids());
    free(instance->indirect_function_table_targets());
  }
  free(instance->imported_function_targets());
