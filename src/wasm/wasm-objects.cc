...
void WasmTableObject::UpdateDispatchTables(
  ...
  PodArray<wasm::ValueType> serialized_sig = capi_function->shared()
  ...
  for (int i = 0; i <= total_count; i++) {
    ...
    reps[i] = serialized_sig->get(i);
  }
  int result_count = total_count - param_count;
  ...
  for (int i = 0; i < dispatch_tables->length();
    ...
    auto sig_id = instance->module()->signature_map.Find(sig);
    IndirectFunctionTableEntry(instance, entry_index)
        .Set(sig_id, capi_function->GetHostCallTarget(),
