...
void FindAllInstances(Isolate* isolate, Handle<WasmModuleObject> module_object,
                      std::vector<Handle<WasmInstanceObject>>& instances) {
  HeapIterator iterator(isolate->heap());
  while (HeapObject* obj = iterator.next()) {
    if (obj->IsWasmInstanceObject()) {
      WasmInstanceObject* instance = WasmInstanceObject::cast(obj);
      if (!instance->is_initialized(isolate)) continue;
      if (instance->module_object() != *module_object) continue;
      instances.push_back(handle(instance, isolate));
    }
  }
}
...
bool WasmModuleObject::SetBreakPoint(Handle<WasmModuleObject> module_object,
  ...
  // breakpoint. We do this using a full heap iteration first.
  std::vector<Handle<WasmInstanceObject>> instances;
  FindAllInstances(isolate, module_object, instances);
  for (Handle<WasmInstanceObject> instance : instances) {
