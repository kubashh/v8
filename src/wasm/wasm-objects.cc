...
void WasmTableObject::SetFunctionTableEntry(Isolate* isolate,
  ...
  Handle<Object> external =
      Handle<WasmInternalFunction>::cast(entry)->GetOrCreateExternal();
...
void WasmTableObject::GetFunctionTableEntry(
  ...
  *is_null = element->IsWasmNull(isolate);
  ...
  if (element->IsWasmInternalFunction()) {
    element =
        Handle<WasmInternalFunction>::cast(element)->GetOrCreateExternal();
  }
...
Handle<JSFunction> WasmInternalFunction::GetOrCreateExternal() {
  ...
  if (!external().IsUndefined()) {
    return handle(JSFunction::cast(external()), isolate);
  }
  ...
  // at creation.
  Handle<WasmInstanceObject> instance =
      handle(ref().IsWasmInstanceObject()
                 ? WasmInstanceObject::cast(ref())
                 : WasmInstanceObject::cast(
  ...
  auto result = WasmExportedFunction::New(
      isolate, instance, handle(*this, isolate), function_index(),
      static_cast<int>(function.sig->parameter_count()), wrapper);

  this->set_external(*result);
  return result;
}
  ...
  switch (type.representation()) {
    ...
    case i::wasm::HeapType::kFunc: {
      if (value->IsWasmNull()) {
        ...
        return i::Handle<i::WasmInternalFunction>::cast(value)
            ->GetOrCreateExternal();
      }
    }
