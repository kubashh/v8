...
class MultiLineStringBuilder : public StringBuilder {
 public:
  ...
  void PatchLabel(LabelInfo& label, const char* label_source) {
    ...
    size_t patched_length = l.len + label.length + 1;  // 1 extra space.
    if (length() == 0) {
      // No current unfinished line. Allocate the patched line as if it was
      // the next line.
      patched_line = write(patched_length);
      start_here();
    } else {
      ...
      start_here();
      char* new_location = write(unfinished_length);
