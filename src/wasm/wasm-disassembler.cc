...
void PrintSignatureOneLine(StringBuilder& out, const FunctionSig* sig,
                           ...
                           bool param_names, bool indices_as_comments) {
  if (param_names) {
...
void FunctionBodyDisassembler::DecodeWat(MultiLineStringBuilder& out,
  ...
  if (include_header) {
    ...
    names_->PrintFunctionName(out, func_index_, true);
...
class ImmediatesPrinter {
 public:
  explicit ImmediatesPrinter(StringBuilder& out,
  ...
  void FunctionIndex(IndexImmediate<validate>& imm) {
    out_ << " ";
    names()->PrintFunctionName(out_, imm.index, true);
  }
...
// TODO(jkummerow): If we need bytecode offsets for lines that aren't inside
// function bodies, then the overall approach needs to change: then we have
// to decode the original module again, instead of iterating over the already-
// decoded WasmModule.
...
void ModuleDisassembler::PrintModule(Zone* zone, Indentation indentation) {
  ...
  for (const WasmImport& import : module_->import_table) {
    ...
    switch (import.kind) {
      ...
      case kExternalFunction: {
        out_ << "(func ";
        names_->PrintFunctionName(out_, import.index, true, kIndicesAsComments);
  ...
  // V. Memories
  if (module_->has_memory && !memory_imported) {
  ...
  for (uint32_t i = 0; i < module_->globals.size(); i++) {
    ...
    PrintGlobal(global);
    PrintInitExpression(global.init, global.type, zone);
    out_ << ")";
    out_.NextLine(kNoByteOffsetOutsideFunctionBodies);
  }
  ...
  if (module_->start_function_index >= 0) {
    ...
    names_->PrintFunctionName(out_, module_->start_function_index, true);
    out_ << ")";
    out_.NextLine(kNoByteOffsetOutsideFunctionBodies);
  }
  ...
  for (uint32_t i = 0; i < module_->elem_segments.size(); i++) {
    ...
    if (elem.status == WasmElemSegment::kStatusDeclarative) {
      ...
      PrintInitExpression(elem.offset, kWasmI32, zone);
    }
    out_ << " ";
    names_->PrintValueType(out_, elem.type);
    for (const ConstantExpression& entry : elem.entries) {
      PrintInitExpression(entry, elem.type, zone);
    }
    out_ << ")";
    out_.NextLine(kNoByteOffsetOutsideFunctionBodies);
  }
  ...
  for (uint32_t i = module_->num_imported_functions;
    ...
    names_->PrintFunctionName(out_, i, true, kIndicesAsComments);
    ...
    FunctionBodyDisassembler d(zone, module_, i, &detected, func->sig,
                               code.begin(), code.end(), func->code.offset(),
                               names_);
    d.DecodeWat(out_, indentation, false /* skip header */);
  }
  ...
  for (uint32_t i = 0; i < module_->data_segments.size(); i++) {
    ...
    if (data.active) {
      ...
      PrintInitExpression(data.dest_addr, type, zone);
    }
    out_ << " \"";
...
void ModuleDisassembler::PrintInitExpression(const ConstantExpression& init,
  ...
  switch (init.kind()) {
    ...
    case ConstantExpression::kRefFunc:
      out_ << " (ref.func ";
      names_->PrintFunctionName(out_, init.index(), true);
      out_ << ")";
      break;
    case ConstantExpression::kWireBytesRef:
      ...
      WasmFeatures detected;
      FunctionBodyDisassembler d(zone, module_, 0, &detected, &sig, start, end,
                                 ref.offset(), names_);
