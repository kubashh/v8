...
class LinkageAllocator {
 public:
  ...
  int NextStackSlot(MachineRepresentation type) {
    int num_slots = AlignedSlotAllocator::NumSlots(ElementSizeInBytes(type));
  ...
  void SetStackOffset(int offset) {
    DCHECK_LE(0, offset);
    DCHECK_EQ(0, slot_allocator_.End());
    slot_allocator_.Reserve(offset);
  }

  int NumStackSlots() const { return slot_allocator_.End(); }

  void EndSlotArea() { slot_allocator_.Reserve(0); }
