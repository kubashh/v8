    ...
    switch (rep) {
      case MachineRepresentation::kFloat32: {
        if (extra_float_reg >= 0) {
          // Use the extra S-register from a previous float allocation.
      ...
      case MachineRepresentation::kSimd128: {
        int d_reg1_code = NextFpReg(MachineRepresentation::kFloat64);
        if (d_reg1_code % 2 != 0) {
          // Q-register must be an even-odd pair; skip odd D-register.
          extra_double_reg = d_reg1_code;
          d_reg1_code = NextFpReg(MachineRepresentation::kFloat64);
        }
        // Combine the current D-register with the next to form a Q-register.
        int d_reg2_code = NextFpReg(MachineRepresentation::kFloat64);
...
#if V8_TARGET_ARCH_ARM
  // ARM FP register aliasing may lead to us splitting or merging double
  // registers. Keep track of usable registers below fp_count_ here.
