...
class LinkageAllocator {
 public:
  ...
  int NextStackSlot(MachineRepresentation type, bool aligned = false) {
    int num_slots =
        AlignedSlotAllocator::NumSlotsForWidth(ElementSizeInBytes(type));
    int slot = aligned ? slot_allocator_.Allocate(num_slots)
                       : slot_allocator_.AllocateUnaligned(num_slots);
    return slot;
  }
