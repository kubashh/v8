...
WasmCode* WasmCompilationResult::Publish(NativeModule* native_module) {
  if (!succeeded()) {
    native_module->compilation_state()->SetError();
    return nullptr;
  }

  WasmCode::Tier code_tier = executed_tier == ExecutionTier::kBaseline
                                 ? WasmCode::kLiftoff
                                 : WasmCode::kTurbofan;
  DCHECK_EQ(code_desc.buffer, instr_buffer.get());
  WasmCode::Kind code_kind = executed_tier == ExecutionTier::kInterpreter
                                 ? WasmCode::Kind::kInterpreterEntry
                                 : WasmCode::Kind::kFunction;
  WasmCode* code = native_module->AddCode(
      func_index, code_desc, frame_slot_count, tagged_parameter_slots,
      std::move(protected_instructions), std::move(source_positions), code_kind,
      code_tier);
  return code;
}
