...
class V8_EXPORT_PRIVATE NativeModuleDeserializer {
 public:
  ...
  const std::vector<int>& missing_functions() { return missing_functions_; }
...
DeserializationUnit NativeModuleDeserializer::ReadCode(int fn_index,
  ...
  if (!has_code) {
    if (FLAG_wasm_lazy_compilation) {
      DCHECK(native_module_->enabled_features().has_compilation_hints());
      native_module_->UseLazyStub(fn_index);
    } else {
      missing_functions_.emplace_back(fn_index);
    }
    return {};
  }
...
MaybeHandle<WasmModuleObject> DeserializeNativeModule(
  ...
  if (shared_native_module == nullptr) {
    ...
    if (!deserializer.missing_functions().empty()) {
      shared_native_module->compilation_state()->WaitForBaselineFinished();
    }
