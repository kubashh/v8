...
struct Header {
  ...
  static constexpr size_t Measure() {
    return sizeof(total_function_count) + sizeof(imported_function_count);
  }
...
struct CodeHeader {
  ...
  static constexpr size_t Measure() {
    return sizeof(code_section_size) + sizeof(constant_pool_offset) +
           sizeof(safepoint_table_offset) + sizeof(handler_table_offset) +
           sizeof(code_size) + sizeof(reloc_size) +
           sizeof(source_positions_size) + sizeof(protected_instructions_size) +
           sizeof(stack_slots) + sizeof(tier);
  }
...
size_t NativeModuleSerializer::Measure() const {
  size_t size = Header::Measure() + MeasureCopiedStubs();
  uint32_t first_wasm_fn = native_module_->num_imported_functions();
  uint32_t total_fns = native_module_->function_count();
  for (uint32_t i = first_wasm_fn; i < total_fns; ++i) {
    size += CodeHeader::Measure();
  }
  for (uint32_t i = first_wasm_fn; i < total_fns; ++i) {
