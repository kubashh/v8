...
class DeserializeCodeTask : public JobTask {
 public:
  ...
  size_t GetMaxConcurrency(size_t /* worker_count */) const override {
    ...
    // publish.
    bool needs_publisher = publishing_.load(std::memory_order_relaxed) ||
                           publish_queue_.NumBatches() == 0;
    return reloc_queue_->NumBatches() + (needs_publisher ? 1 : 0);
  }

 private:
  void TryPublishing(JobDelegate* delegate) {
    // Publishing is sequential, so only start publishing if noone else is.
