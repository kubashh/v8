...
struct AbstractMerge {
  uint32_t arity;
  union {
    AbstractValue<Consumer>* array;
    AbstractValue<Consumer> first;
  } vals;  // Either multiple values or a single value.

  AbstractValue<Consumer>& operator[](size_t i) {
    DCHECK_GT(arity, i);
...
// This is the list of callback functions that a consumer of the WasmFullDecoder
// should implement.
// F(Name, args...)
...
template <bool validate, typename Consumer>
class WasmFullDecoder : public WasmDecoder<validate> {
  using Value = AbstractValue<Consumer>;
  using Control = AbstractControl<Consumer>;
  using MergeValues = AbstractMerge<Consumer>;
...
template <bool decoder_validate, typename Consumer>
class ConsumerTemplate {
 public:
  constexpr static bool validate = decoder_validate;
  using Decoder = WasmFullDecoder<validate, Consumer>;
  using Control = AbstractControl<Consumer>;
  using Value = AbstractValue<Consumer>;
  using MergeValues = AbstractMerge<Consumer>;
};

class EmptyConsumer : public ConsumerTemplate<true, EmptyConsumer> {
 public:
  Control* foo() { return nullptr; }
