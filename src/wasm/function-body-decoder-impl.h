...
#define TRACE(...) STREAM_TRACE_IF(v8_flags.trace_wasm_decoder, __VA_ARGS__)
...
class WasmDecoder : public Decoder {
 public:
  ...
  uint32_t DecodeLocals(const uint8_t* pc) {
    ...
    TRACE("local decls count: ", entries);
...
class WasmFullDecoder : public WasmDecoder<ValidationTag, decoding_mode> {
 ...
 public:
  ...
  void DecodeFunctionBody() {
    TRACE("wasm-decode ", this->start(), "...", this->end(), " (module+",
          this->pc_offset(), ", ", this->end() - this->start(), " bytes)");
    ...
    if (V8_LIKELY(this->current_inst_trace_->first == 0)) {
      ...
      while (this->pc_ < this->end_) {
        ...
        if (V8_UNLIKELY(this->current_inst_trace_->first ==
                        this->pc_offset())) {
          TRACE("Emit trace at 0x", this->pc_offset(), " with ID[0x",
