...
#undef BUILD_SIMPLE_OPCODE
  ...
  DECODE(LoadMem) {
    ...
    if (decoding_mode == kInitExpression) return NonConstError(this, opcode);
...
#define DECODE_IMPL2(opcode, name)                                            \
  if (idx == opcode) {                                                        \
    return decoding_mode == kInitExpression ? &WasmFullDecoder::NonConstError \
                                            : &WasmFullDecoder::Decode##name; \
  }
...
#define DECODE_LOAD_MEM(op, ...) DECODE_IMPL_CONST2(kExpr##op, LoadMem);
    FOREACH_LOAD_MEM_OPCODE(DECODE_LOAD_MEM)
#undef DECODE_LOAD_MEM
#define DECODE_STORE_MEM(op, ...) DECODE_IMPL_CONST2(kExpr##op, StoreMem);
