...
// Do only include this header for implementing new Consumers of the
// WasmFullDecoder.
...
// condition otherwise.
// TODO(clemensh): Rename all "checked" to "do_validation" and replace
...
#define CONSUMER_FUNCTIONS(F)                                                \
  ...
  F(Simd8x16ShuffleOp, const Simd8x16ShuffleOperand<do_validation>& operand, \
    ...
    Value<Consumer>* result)                                                 \
  F(Throw, const ExceptionIndexOperand<do_validation>&)                      \
  F(Catch, const ExceptionIndexOperand<do_validation>& operand,              \
    Control<Consumer>* block)                                                \
  F(AtomicOp, WasmOpcode opcode, Vector<Value<Consumer>> args,               \
    Value<Consumer>* result)
...
class WasmFullDecoder : public WasmDecoder<do_validation> {
 ...
 public:
  ...
  WasmFullDecoder(Zone* zone, const wasm::WasmModule* module,
                  const FunctionBody& body, ConsumerArgs&&... consumer_args)
      : WasmDecoder<do_validation>(module, body.sig, body.start, body.end,
                                   body.offset),
        zone_(zone),
        consumer_(std::forward<ConsumerArgs>(consumer_args)...),
  ...
  bool Decode() {
    ...
    DecodeFunctionBody();
    if (!this->failed()) consumer_.FinishFunction(this);
  ...
  inline Control<Consumer>* control_at(uint32_t depth) {
  ...
  inline Value<Consumer>* stack_value(uint32_t depth) {
 ...
 private:
        ...
        switch (opcode) {
          ...
          case kExprBr: {
            ...
            if (validate(this->Validate(this->pc_, operand, control_.size()) &&
              ...
              consumer_.BreakTo(this, control_at(operand.depth));
            }
            len = 1 + operand.length;
            EndControl();
            break;
          ...
          case kExprBrIf: {
            ...
            if (validate(this->ok() &&
                         ...
                         TypeCheckBreak(operand.depth))) {
              consumer_.BrIf(this, cond, control_at(operand.depth));
            }
            len = 1 + operand.length;
            break;
          }
          case kExprBrTable: {
            ...
            while (iterator.has_next()) {
              ...
              Control<Consumer>* c = control_at(target);
            ...
            if (operand.table_count > 0) {
              ...
              consumer_.BreakTo(this, control_at(target));
            }
          ...
          case kAtomicPrefix: {
            ...
            len++;
            byte atomic_index = this->template read_u8<do_validation>(
  ...
  bool TypeCheckBreak(unsigned depth) {
    ...
    Control<Consumer>* c = control_at(depth);
    if (c->is_loop()) {
...
class ConsumerTemplate {
 public:
#define DEFINE_UNIMPLEMENTED_CALLBACK(name, ...)                             \
  IMPL(name, ##__VA_ARGS__) {                                                \
    V8_Fatal(__FILE__, __LINE__, "Unimplemented WasmFullDecoder method: %s", \
             #name);                                                         \
  }
  CONSUMER_FUNCTIONS(DEFINE_UNIMPLEMENTED_CALLBACK)
...
#define DEFINE_EMPTY_CALLBACK(name, ...) \
  IMPL(name, ##__VA_ARGS__) {}
  CONSUMER_FUNCTIONS(DEFINE_EMPTY_CALLBACK)
#undef DEFINE_EMPTY_CALLBACK
};
