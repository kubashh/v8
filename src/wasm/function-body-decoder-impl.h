...
#define INTERFACE_FUNCTIONS(F)                                                 \
  ...
  F(Select, const Value& cond, const Value& fval, const Value& tval,           \
    Value* result)                                                             \
  F(BrOrRet, uint32_t depth, int drop_values)                                  \
...
#undef DECODE_IMPL2
  ...
  void EnsureStackArguments(int count) {
  ...
  Control* PushControl(ControlKind kind, uint32_t locals_count = 0,
    ...
    control_.emplace_back(kind, locals_count, stack_size() - drop_values,
  ...
  bool TypeCheckBrTable(const std::vector<ValueType>& result_types,
    ...
    if (V8_LIKELY(!control_.back().unreachable())) {
      int available = static_cast<int>(stack_size()) - drop_values -
      ...
      // Pop values from the stack, according to the expected signature.
      for (int i = 0; i < br_arity; ++i) Peek(i, i + 1, result_types[i]);
      Drop(br_arity);
    }
    return this->ok();
  }
...
#undef CASE_ATOMIC_OP
  ...
  bool TypeCheckUnreachableMerge(Merge<Value>& merge, bool conditional_branch,
    ...
    // Make sure we have enough (unreachable) values on the stack. This can
    // drop the "wrong" values if {drop_values != 0}; that's okay because
    // this is unreachable anyway.
    Drop(arity);
    EnsureStackSpace(arity);
    for (int i = 0; i < arity; i++) Push(UnreachableValue(this->pc_));
    return this->ok();
  }
  ...
  bool TypeCheckMergeValues(Control* c, int drop_values, Merge<Value>* merge) {
  ...
  TypeCheckBranchResult TypeCheckBranch(Control* c, bool conditional_branch,
    ...
    if (V8_LIKELY(control_.back().reachable())) {
      ...
      DCHECK_GE(stack_size() - drop_values, control_.back().stack_depth);
      uint32_t actual =
          stack_size() - drop_values - control_.back().stack_depth;
      if (!VALIDATE(actual >= expected)) {
        this->DecodeError(
