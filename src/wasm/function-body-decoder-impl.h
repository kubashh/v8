...
uint32_t read_value_type(Decoder* decoder, const byte* pc, ValueType* result,
                         const WasmFeatures& enabled, bool strict = true) {
  byte code = decoder->read_u8<validate>(pc, "value type opcode");
  ...
  switch (code) {
    ...
    default:
      *result = kWasmBottom;
      if (strict) decoder->error(pc, "invalid value type");
      return 0;
  }
}
}  // namespace value_type_reader
...
struct SelectTypeImmediate {
  ...
  inline SelectTypeImmediate(const WasmFeatures& enabled, Decoder* decoder,
    ...
    auto type_length = value_type_reader::read_value_type<validate>(
...
struct BlockTypeImmediate {
  ...
  inline BlockTypeImmediate(const WasmFeatures& enabled, Decoder* decoder,
    ...
    if (decoder->read_u8<validate>(pc + 1, "block type") != kLocalVoid) {
      ...
      length = value_type_reader::read_value_type<validate>(
          decoder, pc + 1, &type, enabled, /* strict = */ false);
      if (length == 0) {
        // 3rd case: multi-value block.
...
#define INTERFACE_FUNCTIONS(F)                                                \
  ...
  F(StructSet, const Value& struct_object, const Value& field_value,          \
  ...
  uint32_t DecodeGCOpcode(WasmOpcode opcode) {
    ...
    switch (opcode) {
      ...
      case kExprStructSet: {
        ...
        auto struct_obj =
            Pop(0, ValueType(ValueType::kOptRef, field.struct_index.index));
        CALL_INTERFACE_IF_REACHABLE(StructSet, struct_obj, field_value, field);
        break;
      }
      case kExprArrayNew:
        UNIMPLEMENTED();  // TODO(7748): Implement.
