...
class WasmFullDecoder : public WasmDecoder<validate> {
 ...
 private:
  ...
  // Generic base implementation, only handling asm.js opcodes. All other
  // operations are handled via template specializations below.
  template <WasmOpcode opcode>
  int DecodeOp() {
...
#undef BUILD_SIMPLE_OPCODE
  ...
  template <>
  int DecodeOp<kExprNop>() {
    TraceLine trace_msg(this);
    return 1;
  }

  template <>
  int DecodeOp<kExprBlock>() {
    TraceLine trace_msg(this);
  ...
  template <>
  int DecodeOp<kExprSelectWithType>() {
    TraceLine trace_msg(this);
  ...
  template <>
  int DecodeOp<kExprRefAsNonNull>() {
    TraceLine trace_msg(this);
    CHECK_PROTOTYPE_OPCODE(typed_funcref);
    Value value = Pop();
    switch (value.type.kind()) {
  ...
  template <>
  int DecodeOp<kExprCallFunction>() {
    TraceLine trace_msg(this);
  ...
  template <>
  int DecodeOp<kExprCallIndirect>() {
    TraceLine trace_msg(this);
  ...
  template <>
  int DecodeOp<kExprReturnCallIndirect>() {
    TraceLine trace_msg(this);
    CHECK_PROTOTYPE_OPCODE(return_call);
    CallIndirectImmediate<validate> imm(this->enabled_, this, this->pc_ + 1);
    if (!this->Validate(this->pc_ + 1, imm)) return 0;
...
#define DECODE_SIMPLE_PROTOTYPE_OP(name, opc, sig)    \
  template <>                                         \
  int DecodeOp<kExpr##name>() {                       \
    TraceLine trace_msg(this);                        \
    return BuildSimplePrototypeOperator(kExpr##name); \
  }
  FOREACH_SIMPLE_PROTOTYPE_OPCODE(DECODE_SIMPLE_PROTOTYPE_OP)
#undef DECODE_SIMPLE_PROTOTYPE_OP
