...
V8_INLINE WasmFeature feature_for_heap_repr(HeapType::Repr heap_repr) {
  switch (heap_repr) {
    case HeapType::kFunc:
...
HeapType::Repr read_heap_type(Decoder* decoder, const byte* pc,
                              uint32_t* const length,
                              const WasmFeatures& enabled) {
  int64_t heap_index = decoder->read_i33v<validate>(pc, length, "heap type");
  uint8_t code = static_cast<ValueTypeCode>(heap_index) & byte{0x7F};
  switch (code) {
    case kLocalFuncRef:
    case kLocalExnRef:
    case kLocalEqRef:
    case kLocalExternRef: {
      HeapType::Repr result = HeapType::heap_repr_from_code(code);
      if (!VALIDATE(enabled.contains(feature_for_heap_repr(result)))) {
        decoder->errorf(
            pc, "invalid heap type '%s', enable with --experimental-wasm-%s",
            HeapType(result).name().c_str(),
            WasmFeatures::name_for_feature(feature_for_heap_repr(result)));
    ...
    default:
      ...
      HeapType::Repr type_index = static_cast<HeapType::Repr>(heap_index);
...
ValueType read_value_type(Decoder* decoder, const byte* pc,
  ...
  *length = 1;
  ...
  switch (code) {
    ...
    case kLocalExternRef:
    case kLocalI31Ref: {
      HeapType::Repr heap = HeapType::heap_repr_from_code(code);
      ValueType result = ValueType::Ref(heap, kNullable);
      if (!VALIDATE(enabled.contains(feature_for_heap_repr(heap)))) {
        decoder->errorf(
            pc, "invalid value type '%s', enable with --experimental-wasm-%s",
            result.type_name().c_str(),
            WasmFeatures::name_for_feature(feature_for_heap_repr(heap)));
        return kWasmBottom;
      }
      return result;
    }
    ...
    case kLocalOptRef: {
      ...
      *length += 1;
      if (heap == HeapType::kBottom) return kWasmBottom;
      return ValueType::Ref(heap, nullability);
    }
    case kLocalRtt: {
      ...
      HeapType::Repr heap = read_heap_type<validate>(
          decoder, pc + depth_length + 1, length, enabled);
      *length += depth_length + 1;
      if (heap == HeapType::kBottom) return kWasmBottom;
      return ValueType::Rtt(heap, depth);
    }
    case kLocalS128:
...
struct HeapTypeImmediate {
  ...
  inline HeapTypeImmediate(const WasmFeatures& enabled, Decoder* decoder,
                           const byte* pc) {
    type = HeapType(value_type_reader::read_heap_type<validate>(
