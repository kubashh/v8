...
using ValidatedLocalIndexOperand = LocalIndexOperand<Decoder::kValidate>;
using NotValidatedLocalIndexOperand = LocalIndexOperand<Decoder::kNoValidate>;
...
#define INTERFACE_FUNCTIONS(F)                                                 \
  ...
  F(GetLocal, Value* result, const ValidatedLocalIndexOperand& operand)        \
  F(SetLocal, const Value& value, const ValidatedLocalIndexOperand& operand)   \
  F(TeeLocal, const Value& value, Value* result,                               \
    const ValidatedLocalIndexOperand& operand)                                 \
...
class WasmDecoder : public Decoder {
 public:
  ...
  static BitVector* AnalyzeLoopAssignment(Decoder* decoder, const byte* pc,
    ...
    while (pc < decoder->end() && decoder->ok()) {
      ...
      switch (opcode) {
        ...
        case kExprTeeLocal: {
          ValidatedLocalIndexOperand operand(decoder, pc);
  ...
  inline bool Validate(const byte* pc, ValidatedLocalIndexOperand& operand) {
    ...
    while (this->pc_ < this->end_) {  // decoding loop.
      ...
      if (sig) {
        ...
        switch (opcode) {
          ...
          case kExprGetLocal: {
            ValidatedLocalIndexOperand operand(this, this->pc_);
            if (!this->Validate(this->pc_, operand)) break;
      ...
      if (FLAG_trace_wasm_decoder) {
        ...
        for (size_t i = 0; i < stack_.size(); ++i) {
          ...
          switch (opcode) {
            ...
            case kExprSetLocal:
            case kExprTeeLocal: {
              ValidatedLocalIndexOperand operand(this, val.pc);
