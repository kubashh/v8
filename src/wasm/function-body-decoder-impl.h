...
#define INTERFACE_CONSTANT_FUNCTIONS(F) /*       force 80 columns           */ \
  ...
  F(ArrayNewElem, const ArrayIndexImmediate<validate>& array_imm,              \
  ...
  int DecodeGCOpcode(WasmOpcode opcode, uint32_t opcode_length) {
    switch (opcode) {
      ...
      case kExprArrayNewDataStatic: {
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(
            ArrayNewElem, array_imm, data_segment, offset, length, rtt, &array);
      ...
      case kExprArrayNewElemStatic: {
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(
            ArrayNewElem, array_imm, elem_segment, offset, length, rtt, &array);
        Drop(3);  // rtt, length, offset
        Push(array);
