...
#define REF_TYPE_CASE(heap_type, feature)                                  \
  case kLocal##heap_type##Ref: {                                           \
    ...
    if (!enabled.has_##feature()) {                                        \
      decoder->errorf(pc,                                                  \
                      "invalid value type '(ref [null] %s)', enable with " \
                      "--experimental-wasm-%s",                            \
                      heap_name(kHeap##heap_type).c_str(), #feature);      \
    }                                                                      \
    break;                                                                 \
  }
...
class WasmDecoder : public Decoder {
 public:
  ...
  inline bool Validate(const byte* pc, HeapTypeImmediate<validate>& imm) {
    // TODO(7748): Check for functions.
    if (!VALIDATE(imm.type >= kHeapFunc || static_cast<uint32_t>(imm.type) <
                                               module_->type_kinds.size())) {
      errorf(pc, "Index out of bounds");
      return false;
    }
    return true;
  }
...
#undef SIMPLE_PROTOTYPE_CASE
  ...
  uint32_t DecodeGCOpcode(WasmOpcode opcode) {
    ...
    switch (opcode) {
      ...
      case kExprRttCanon: {
        HeapTypeImmediate<validate> imm(this->enabled_, this, this->pc_ + len);
        len += imm.length;
        if (!this->Validate(this->pc_ + len, imm)) break;
