...
#undef DECLARE_OPCODE_CASE
      ...
      case kGCPrefix: {
        ...
        switch (opcode) {
          ...
          case kExprStructSet: {
            StructIndexImmediate<validate> simm(decoder, pc + 2);
            FieldIndexImmediate<validate> fimm(decoder, pc + 2 + simm.length);
            return 2 + simm.length + fimm.length;
          }
          case kExprArrayNew:
