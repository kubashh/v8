...
#undef BUILD_SIMPLE_OPCODE
  ...
  DECODE(TableGet) {
    ...
    ValueType table_type = TableIndexType(table);
    Value index = Pop(table_type);
  ...
  DECODE(TableSet) {
    ...
    const WasmTable* table = &this->module_->tables[imm.index];
    ValueType table_type = TableIndexType(table);
    auto [index, value] = Pop(table_type, table->type);
  ...
  DECODE(ReturnCallIndirect) {
    ...
    const WasmTable* table = &this->module_->tables[imm.table_imm.index];
    ValueType index_type = TableIndexType(table);
  ...
  unsigned DecodeNumericOpcode(WasmOpcode opcode, uint32_t opcode_length) {
    ...
    switch (opcode) {
      ...
      case kExprTableInit: {
        ...
        ValueType table_type = TableIndexType(table);
        auto [dst, src, size] = Pop(table_type, kWasmI32, kWasmI32);
      ...
      case kExprTableCopy: {
        ...
        if (table_type_dst != TableIndexType(table_src)) {
          // Disallow copying between a 32-bit and a 64-bit tables.
          this->DecodeError(this->pc_,
      ...
      case kExprTableGrow: {
        ...
        const WasmTable* table = &this->module_->tables[imm.index];
        ValueType table_type = TableIndexType(table);
        auto [value, delta] = Pop(table->type, table_type);
        Value* result = Push(table_type);
      ...
      case kExprTableFill: {
        ...
        auto [start, value, count] =
            Pop(table_type, this->module_->tables[imm.index].type, table_type);
