 ...
 public:
  ...
  void DecodeFunctionBody() {
    ...
    if (V8_LIKELY(this->current_inst_trace_->first == 0)) {
      ...
      // Decode the function body.
      while (this->pc_ < this->end_) {
        // Because the current module offset will never be zero, once we reach
        // the invalid state this will always evaluate to false #ifdef
        // _I_AM_DEF_NOT_DEFINED
        DCHECK(this->current_inst_trace_->first == 0 ||
               this->current_inst_trace_->first >= this->pc_offset());
        // auto off = static_cast<uint32_t>(this->pc_ - this->start_) +
        // this->buffer_offset_;
