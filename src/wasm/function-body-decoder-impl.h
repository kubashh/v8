...
V8_INLINE WasmFeature feature_for_heap(uint32_t heap) {
  switch (heap) {
    case HeapType::kFunc:
...
uint32_t read_heap_type(Decoder* decoder, const byte* pc,
  ...
  switch (code) {
    ...
    case kLocalExternRef: {
      uint32_t result = HeapType::heap_from_code(code);
      if (!VALIDATE(enabled.contains(feature_for_heap(result)))) {
        decoder->errorf(
            pc, "invalid heap type '%s', enable with --experimental-wasm-%s",
            HeapType(result).name().c_str(),
            WasmFeatures::name_for_feature(feature_for_heap(result)));
...
ValueType read_value_type(Decoder* decoder, const byte* pc,
  ...
  *length = 1;
  ...
  switch (code) {
    ...
    case kLocalEqRef:
    case kLocalExternRef: {
      uint32_t heap = HeapType::heap_from_code(code);
      ValueType result = ValueType::Ref(heap, kNullable);
      if (!VALIDATE(enabled.contains(feature_for_heap(heap)))) {
        decoder->errorf(
            pc, "invalid value type '%s', enable with --experimental-wasm-%s",
            result.type_name().c_str(),
            WasmFeatures::name_for_feature(feature_for_heap(heap)));
        return kWasmBottom;
      }
      return result;
    }
    ...
    case kLocalRtt: {
      ...
      uint32_t heap = read_heap_type<validate>(decoder, pc + depth_length + 1,
                                               length, enabled);
