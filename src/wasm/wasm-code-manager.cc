...
void NativeModule::PatchJumpTableLocked(const CodeSpaceData& code_space_data,
  ...
  GetProcessWideWasmCodePointerTable()->SetEntrypointWithWriteScope2(
      code_pointer_handles_[slot_index], target);
}
...
uint32_t NativeModule::GetFunctionIndexFromIndirectCallTarget(
  ...
  for (uint32_t i = 0; i < code_pointer_handles_size_; i++) {
    if (code_pointer_handles_[i] == target) {
      return module_->num_imported_functions + i;
    }
  }
  UNREACHABLE();
#else
...
WasmCodePointer NativeModule::GetIndirectCallTarget(int index) const {
  DCHECK_GE(index, module_->num_imported_functions);
  index -= module_->num_imported_functions;
  DCHECK_LT(index, module_->num_declared_functions);
#ifdef V8_ENABLE_WASM_CODE_POINTER_TABLE
  return code_pointer_handles_[index];
#else
  Address jump_table_slot =
      jump_table_start() + JumpTableAssembler::JumpSlotIndexToOffset(index);
