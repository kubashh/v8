...
void NativeModule::FreeCode(Vector<WasmCode*> codes) {
...
void DecrementRefCount(std::vector<WasmCode*> code_vec) {
  ...
  // For each native module, free all it's code objects at once.
  ...
  while (range_begin != code_vec.end()) {
    ...
    Vector<WasmCode*> code_vec{&*range_begin, range_end - range_begin};
...
WasmCodeRefScope::WasmCodeRefScope(
    WasmCodeRefScope::KeepWasmCodeAlive keep_code_alive)
    : keep_code_alive_(keep_code_alive),
...
void WasmCodeRefScope::AddRef(WasmCode* code) {
  ...
  if (!current_scope->keep_code_alive_) return;
