...
NativeModule::CloneCodeHelper::CloneCodeHelper(
  ...
  for (auto& pair1 : source_native_module_->stubs_) {
    Address old_dest1 = pair1.second->instructions().start();
    auto local1 = cloning_native_module_->stubs_.find(pair1.first);
    DCHECK(local1 != cloning_native_module_->stubs_.end());
    Address new_dest1 = local1->second->instructions().start();
    reverse_lookup_.emplace(old_dest1, new_dest1);
  }
}
