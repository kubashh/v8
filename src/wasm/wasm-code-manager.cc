...
NativeModule::CloneCodeHelper::CloneCodeHelper(
  ...
  CreateTrampolinesMapping();
  CreateStubsMapping();
}

void NativeModule::CloneCodeHelper::AddCode(
    std::pair<WasmCode*, uint32_t> code) {
  code_to_clone_.emplace_back(code);
}
...
void NativeModule::CloneCodeHelper::CloneAndPatchCode(
  ...
  for (auto& pair : code_to_clone_) {
    WasmCode* original_code = pair.first;
    ...
    switch (original_code->kind()) {
      case WasmCode::kLazyStub: {
        ...
        if (!original_code->IsAnonymous()) {
          WasmCode* new_code = cloning_native_module_->CloneLazyBuiltinInto(
              original_code, index, kNoFlushICache);
        ...
        if (anonymous_lazy_builtin == nullptr) {
          WasmCode* new_code =
              cloning_native_module_->CloneCode(original_code, kNoFlushICache);
          PatchTrampolineAndStubCalls(original_code, new_code, reverse_lookup_,
                                      kFlushICache);
...
void NativeModule::CloneCodeHelper::CreateTrampolinesMapping() {
...
void NativeModule::CloneCodeHelper::CreateStubsMapping() {
...
std::unique_ptr<NativeModule> NativeModule::Clone() {
  ...
  for (uint32_t i = num_imported_functions(), e = FunctionCount(); i < e; ++i) {
    helper.AddCode(std::make_pair(GetCode(i), i));
  }
