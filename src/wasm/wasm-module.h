...
class CallSiteFeedback {
 public:
  ...
  CallSiteFeedback(NestedCallSiteFeedback* polymorphic_cases, int num_cases)
  ...
  CallSiteFeedback& operator=(const CallSiteFeedback& other) V8_NOEXCEPT {
    ...
    if (other.is_polymorphic()) {
      ...
      NestedCallSiteFeedback* polymorphic =
          new NestedCallSiteFeedback[num_cases];
 ...
 private:
  ...
  const NestedCallSiteFeedback* polymorphic_storage() const {
    return reinterpret_cast<NestedCallSiteFeedback*>(frequency_or_ool_);
  }
