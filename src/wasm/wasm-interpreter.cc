...
struct nan_tester {
  ...
  V8_INLINE bool operator()(T val) { return false; }
};
template <typename T>
struct nan_tester<T, true> {
  V8_INLINE bool operator()(T val) { return std::isnan(val); }
};
template <>
struct nan_tester<Float32> {
  V8_INLINE bool operator()(Float32 val) { return val.is_nan(); }
};
template <>
struct nan_tester<Float64> {
  V8_INLINE bool operator()(Float64 val) { return val.is_nan(); }
};

template <typename T>
V8_INLINE bool is_nan(T val) {
  return nan_tester<T>{}(val);
}
...
#undef CASE_TYPE
  ...
  bool ExecuteStore(Decoder* decoder, InterpreterCode* code, pc_t pc, int& len,
    ...
    if (is_nan(val)) possible_nondeterminism_ = true;
