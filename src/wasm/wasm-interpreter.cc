...
Handle<HeapObject> UnwrapJSWrapper(Isolate* isolate, Handle<Code> js_wrapper) {
...
class CodeMap {
 ...
 public:
  ...
  Handle<Code> GetImportedFunction(uint32_t function_index) {
    DCHECK(!instance_.is_null());
    Isolate* isolate = instance_->GetIsolate();
    DCHECK_GT(module_->num_imported_functions, function_index);
    FixedArray* code_table = instance_->compiled_module()->ptr_to_code_table();
    return handle(Code::cast(code_table->get(static_cast<int>(function_index))),
                  isolate);
  }
  ...
  Handle<HeapObject> GetCallableObjectForJSImport(Isolate* isolate,
    ...
    Handle<HeapObject> called_obj = UnwrapJSWrapper(isolate, code);
    if (!called_obj.is_null()) {
      ...
      if (imported_functions_.is_null()) {
        ...
        Handle<FixedArray> new_imported_functions =
            isolate->factory()->NewFixedArray(8, TENURED);
      ...
      switch (orig) {
        ...
        case kExprCallFunction: {
          ...
          if (target->function->imported) {
            ...
            if (result.type == ExternalCallResult::EXTERNAL_UNWOUND) return;
            if (result.type == ExternalCallResult::EXTERNAL_RETURNED) {
              PAUSE_IF_BREAK_FLAG(AfterCall);
              len = 1 + operand.length;
              break;  // bump pc
            }
            DCHECK_EQ(ExternalCallResult::INTERNAL, result.type);
...
#undef EXECUTE_OTHER_UNOP
  ...
  ExternalCallResult CallIndirectFunction(uint32_t table_index,
    ...
    if (static_cast<uint32_t>(found_sig) != sig_index) {
      return {ExternalCallResult::SIGNATURE_MISMATCH};
    }
    ...
    Handle<Code> target(
        Code::cast(fun_table->get(static_cast<int>(entry_index))), isolate);

    // Call the code object.
    FunctionSig* signature =
        &codemap()->module()->signatures[table_index][sig_index];
    return CallCodeObject(isolate, target, signature);
  }
