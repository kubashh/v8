...
#undef CASE_TYPE
  ...
  pc_t ReturnPc(Decoder* decoder, InterpreterCode* code, pc_t pc) {
    switch (code->orig_start[pc]) {
      case kExprCallFunction: {
        CallFunctionOperand<Decoder::kUnchecked> operand(decoder, code->at(pc));
        return pc + 1 + operand.length;
      }
      case kExprCallIndirect: {
        CallIndirectOperand<Decoder::kUnchecked> operand(decoder, code->at(pc));
        return pc + 1 + operand.length;
      }
      default:
        UNREACHABLE();
  ...
  bool ExecuteLoad(Decoder* decoder, InterpreterCode* code, pc_t pc, int& len,
    ...
    MemoryAccessOperand<Decoder::kUnchecked> operand(decoder, code->at(pc),
                                                     sizeof(ctype));
    ...
    max = 0;                                                          \
      ...
      switch (orig) {
        case kExprNop:
          break;
        case kExprBlock: {
          BlockTypeOperand<Decoder::kUnchecked> operand(&decoder, code->at(pc));
          len = 1 + operand.length;
          break;
        }
        case kExprLoop: {
        ...
        case kExprBr: {
          BreakDepthOperand<Decoder::kUnchecked> operand(&decoder,
                                                         code->at(pc));
        ...
        case kExprBrTable: {
          BranchTableOperand<Decoder::kUnchecked> operand(&decoder,
                                                          code->at(pc));
          BranchTableIterator<Decoder::kUnchecked> iterator(&decoder, operand);
        ...
        case kExprI32Const: {
          ImmI32Operand<Decoder::kUnchecked> operand(&decoder, code->at(pc));
          Push(WasmValue(operand.value));
          len = 1 + operand.length;
          break;
        }
        case kExprI64Const: {
          ImmI64Operand<Decoder::kUnchecked> operand(&decoder, code->at(pc));
          Push(WasmValue(operand.value));
          len = 1 + operand.length;
          break;
        }
        case kExprF32Const: {
          ImmF32Operand<Decoder::kUnchecked> operand(&decoder, code->at(pc));
          Push(WasmValue(operand.value));
          len = 1 + operand.length;
          break;
        }
        case kExprF64Const: {
          ImmF64Operand<Decoder::kUnchecked> operand(&decoder, code->at(pc));
          Push(WasmValue(operand.value));
          len = 1 + operand.length;
          break;
        }
        case kExprGetLocal: {
          LocalIndexOperand<Decoder::kUnchecked> operand(&decoder,
                                                         code->at(pc));
        ...
        case kExprCallFunction: {
          CallFunctionOperand<Decoder::kUnchecked> operand(&decoder,
                                                           code->at(pc));
        ...
        case kExprCallIndirect: {
          CallIndirectOperand<Decoder::kUnchecked> operand(&decoder,
                                                           code->at(pc));
        ...
        case kExprGetGlobal: {
          GlobalIndexOperand<Decoder::kUnchecked> operand(&decoder,
                                                          code->at(pc));
...
#undef ASMJS_STORE_CASE
        case kExprGrowMemory: {
          MemoryIndexOperand<Decoder::kUnchecked> operand(&decoder,
                                                          code->at(pc));
