  ...
  case kExpr##format##ExtractLane##sign: {                               \
    ...
    *len += 1;                                                           \
    ...
    return true;                                                         \
      ...
      // will be signed extended. it will be signed extended, e.g. int8_t ->
      // int32_t -> uint32_t. So for unsigned extracts, we will cast it int8_t
      // -> uint8_t -> uint32_t. We add the DCHECK to ensure that if the array
      // type changes, we know to change this function.
