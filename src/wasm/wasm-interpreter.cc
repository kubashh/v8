...
#undef EXECUTE_OTHER_UNOP
  ...
  ExternalCallResult CallExternalWasmFunction(Isolate* isolate,
    ...
    std::vector<char> arg_buffer(num_args * 8);
    size_t offset = 0;
    WasmValue* wasm_args = sp_ - num_args;
    for (int i = 0; i < num_args; ++i) {
...
#define CASE_ARG_TYPE(type, ctype)                                             \
  case type:                                                                   \
    ...
    WriteUnalignedValue(arg_buffer.data() + offset, wasm_args[i].to<ctype>()); \
    break;
...
#define CASE_RET_TYPE(type, ctype)                                      \
  case type:                                                            \
    DCHECK_EQ(1 << ElementSizeLog2Of(sig->GetReturn()), sizeof(ctype)); \
    Push(WasmValue(ReadUnalignedValue<ctype>(arg_buffer.data())));      \
    break;
