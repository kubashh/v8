...
class WasmGraphBuildingInterface {
 public:
  ...
  void Loop(FullDecoder* decoder, Control* block) {
    ...
    for (uint32_t i = 0; i < block->start_merge.arity; ++i) {
      ...
      TFNode* vals[] = {val.node, block->end_env->control};
      val.node = builder_->Phi(val.type, 1, vals);
    }
  }
  ...
  void Select(FullDecoder* decoder, const Value& cond, const Value& fval,
    ...
    TFNode* vals[] = {tval.node, fval.node, merge};
    TFNode* phi = BUILD(Phi, tval.type, 2, vals);
    result->node = phi;
 ...
 private:
    ...
    switch (to->state) {
      ...
      case SsaEnv::kReached: {  // Create a new merge.
        ...
        if (from->effect != to->effect) {
          ...
          to->effect = builder_->EffectPhi(2, effects);
        }
        // Merge SSA values.
        for (int i = decoder->num_locals() - 1; i >= 0; i--) {
          ...
          if (a != b) {
            TFNode* vals[] = {a, b, merge};
            to->locals[i] = builder_->Phi(decoder->GetLocalType(i), 2, vals);
          }
        }
  ...
  SsaEnv* PrepareForLoop(FullDecoder* decoder, SsaEnv* env) {
    ...
    TFNode* effects[] = {env->effect, env->control};
    env->effect = builder_->EffectPhi(1, effects);
    ...
    if (assigned != nullptr) {
      ...
      for (int i = decoder->num_locals() - 1; i >= 0; i--) {
        ...
        TFNode* vals[] = {env->locals[i], env->control};
        env->locals[i] = builder_->Phi(decoder->GetLocalType(i), 1, vals);
      }
