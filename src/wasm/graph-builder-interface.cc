...
class WasmGraphBuildingInterface {
 ...
 private:
  ...
  uint32_t control_depth_for_current_catch(FullDecoder* decoder) {
  ...
  TryInfo* current_try_info(FullDecoder* decoder) {
    ...
    return decoder->control_at(control_depth_for_current_catch(decoder))
        ->try_info;
  }
  ...
  TFNode* CheckForException(FullDecoder* decoder, TFNode* node) {
    ...
    if (FLAG_wasm_loop_unrolling) {
      StackValueVector values;
      BuildNestedLoopExits(decoder, control_depth_for_current_catch(decoder),
                           true, values);
    }
