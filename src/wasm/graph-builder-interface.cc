...
class WasmGraphBuildingInterface {
 public:
  ...
  void ReturnCall(FullDecoder* decoder,
                  const CallFunctionImmediate<validate>& imm,
                  const Value args[]) {
    DoReturnCall(decoder, 0, nullptr, imm.index, args,
                 static_cast<int>(imm.sig->parameter_count()));
  }
  ...
  void ReturnCallIndirect(FullDecoder* decoder, const Value& index,
                          const CallIndirectImmediate<validate>& imm,
                          const Value args[]) {
    DoReturnCall(decoder, imm.table_index, index.node, imm.sig_index, args,
                 static_cast<int>(imm.sig->parameter_count()));
  }
 ...
 private:
  ...
  void DoReturnCall(FullDecoder* decoder, uint32_t table_index,
                    TFNode* index_node, uint32_t sig_index, const Value args[],
                    int arg_count) {
