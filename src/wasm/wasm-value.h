...
#define FOREACH_WASMVAL_TYPE(V)                               \
  V(i32, kWasmI32, int32_t, v, static_cast<int32_t>(p))       \
  V(u32, kWasmI32, uint32_t, v, static_cast<uint32_t>(p))     \
  V(i64, kWasmI64, int64_t, v, static_cast<int64_t>(p))       \
  V(u64, kWasmI64, uint64_t, v, p)                            \
  V(f32, kWasmF32, float, Float32(v).get_bits(),              \
    Float32::FromBits(static_cast<uint32_t>(p)).get_scalar()) \
  V(f32_boxed, kWasmF32, Float32, v.get_bits(),               \
    Float32::FromBits(static_cast<uint32_t>(p)))              \
  V(f64, kWasmF64, double, Float64(v).get_bits(),             \
    Float64::FromBits(p).get_scalar())                        \
