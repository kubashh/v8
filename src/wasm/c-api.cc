...
auto v8_to_val(i::Isolate* isolate, i::Handle<i::Object> value2,
               const ValType* t) -> own<Val> {
  switch (t->kind()) {
    case I32:
      do {
        if (value2->IsSmi()) return Val(i::Smi::ToInt(*value2));
        if (value2->IsHeapNumber()) {
          return Val(i::DoubleToInt32(i::HeapNumber::cast(*value2)->value()));
        }
        value2 = i::Object::ToInt32(isolate, value2).ToHandleChecked();
        // This will loop back at most once.
      } while (true);
      break;  // Make the compiler happy.
    case I64:
      if (value2->IsBigInt()) return Val(i::BigInt::cast(*value2)->AsInt64());
      return Val(
          i::BigInt::FromObject(isolate, value2).ToHandleChecked()->AsInt64());
    case F32:
      do {
        if (value2->IsSmi()) {
          return Val(static_cast<float32_t>(i::Smi::ToInt(*value2)));
        }
        if (value2->IsHeapNumber()) {
          double raw = i::HeapNumber::cast(*value2)->value();
          using limits = std::numeric_limits<float32_t>;
          if (raw > limits::max()) return Val(limits::max());
          if (raw < limits::lowest()) return Val(limits::lowest());
          return Val(static_cast<float32_t>(raw));
        }
        value2 = i::Object::ToNumber(isolate, value2).ToHandleChecked();
        // This will loop back at most once.
      } while (true);
      break;  // Make the compiler happy.
    case F64:
      do {
        if (value2->IsSmi()) {
          return Val(static_cast<float64_t>(i::Smi::ToInt(*value2)));
        }
        if (value2->IsHeapNumber()) {
          return Val(i::HeapNumber::cast(*value2)->value());
        }
        value2 = i::Object::ToNumber(isolate, value2).ToHandleChecked();
        // This will loop back at most once.
      } while (true);
...
class RefImpl {
 public:
  static own<Ref*> make(StoreImpl* store, i::Handle<i::JSReceiver> obj) {
  ...
  void Reset() { i::GlobalHandles::Destroy(location()); }
  ...
  i::Handle<i::JSReceiver> v8_object() const { return val_; }
...
auto Trap::make(Store* store_abs, const Message& message) -> own<Trap*> {
  ...
  return RefImpl<Trap>::make(store, exception);
}
...
auto Trap::message() const -> Message {
  ...
  return vec<byte_t>::adopt(length, utf8.get());
}
...
auto Module::make(Store* store_abs, const vec<byte_t>& binary) -> own<Module*> {
  ...
  return RefImpl<Module>::make(store, module);
}

auto Module::imports() const -> vec<ImportType*> {
  i::Handle<i::WasmModuleObject> module_obj =
      i::Handle<i::WasmModuleObject>::cast(impl(this)->v8_object());
...
auto Module::deserialize(Store* store_abs, const vec<byte_t>& serialized)
  ...
  return RefImpl<Module>::make(store, module_obj);
}
...
auto make_func(Store* store_abs, FuncData* data) -> own<Func*> {
  ...
  i::Handle<i::WasmInstanceObject> instance_obj =
      i::wasm::InstantiateToInstanceObject(
          isolate, &thrower,
          i::Handle<i::WasmModuleObject>::cast(impl(module.get())->v8_object()),
          imports_obj, i::MaybeHandle<i::JSArrayBuffer>())
          .ToHandleChecked();

  i::Handle<i::JSObject> exports_obj(instance_obj->exports_object(), isolate);
  i::Handle<i::JSFunction> wrapped_func_obj = i::Handle<i::JSFunction>::cast(
      i::JSObject::GetProperty(isolate, exports_obj, str).ToHandleChecked());
...
auto Func::type() const -> own<FuncType*> {
  i::Handle<i::WasmExportedFunction> function =
      i::Handle<i::WasmExportedFunction>::cast(impl(this)->v8_object());
...
auto Func::call(const Val args[], Val results[]) const -> own<Trap*> {
  ...
  auto v8_function = i::Handle<i::JSFunction>::cast(func->v8_object());
  i::MaybeHandle<i::Object> maybe_val = i::Execution::Call(
      i_isolate, v8_function, i_isolate->factory()->undefined_value(),
  ...
  if (handler.HasCaught()) {
    ...
    return RefImpl<Trap>::make(store,
                               i::Handle<i::JSReceiver>::cast(exception));
  }
...
void FuncData::v8_callback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (trap) {
    // TODO(jkummerow): Is just "Throw" enough?
...
auto Global::make(Store* store_abs, const GlobalType* type, const Val& val)
  ...
  // Create wrapper instance
  vec<char> binary = wasm::bin::wrapper(type);
  own<Module*> module = Module::make(store_abs, binary);

  i::Handle<i::WasmModuleObject> module_obj =
      i::Handle<i::WasmModuleObject>::cast(impl(module.get())->v8_object());
  NopErrorThrower thrower(isolate);
  i::Handle<i::WasmInstanceObject> instance_obj =
      i::wasm::InstantiateToInstanceObject(isolate, &thrower, module_obj,
                                           i::MaybeHandle<i::JSReceiver>(),
                                           i::MaybeHandle<i::JSArrayBuffer>())
          .ToHandleChecked();
  i::Handle<i::JSObject> exports_obj(instance_obj->exports_object(), isolate);
  i::Handle<i::JSReceiver> obj = i::Handle<i::JSReceiver>::cast(
      i::Object::GetProperty(isolate, exports_obj,
...
auto Global::type() const -> own<GlobalType*> {
  i::Handle<i::WasmGlobalObject> v8_global =
      i::Handle<i::WasmGlobalObject>::cast(impl(this)->v8_object());
...
auto Table::make(Store* store_abs, const TableType* type, const Ref* ref)
  ...
  return RefImpl<Table>::make(store, table_obj);
}

auto Table::type() const -> own<TableType*> {
  i::Handle<i::WasmTableObject> table =
      i::Handle<i::WasmTableObject>::cast(impl(this)->v8_object());
...
auto Table::get(size_t index) const -> own<Ref*> {
  ...
  i::Handle<i::Object> result =
      i::WasmTableObject::Get(isolate, table, static_cast<int>(index));
  if (!result->IsJSFunction()) return own<Ref*>();
  // TODO(wasm+): other references
  return RefImpl<Func>::make(impl(this)->store(),
...
auto Memory::make(Store* store_abs, const MemoryType* type) -> own<Memory*> {
  ...
  return RefImpl<Memory>::make(store, memory_obj);
}

auto Memory::type() const -> own<MemoryType*> {
  i::Handle<i::WasmMemoryObject> memory =
      i::Handle<i::WasmMemoryObject>::cast(impl(this)->v8_object());
...
auto Memory::data() const -> byte_t* {
  i::Handle<i::WasmMemoryObject> memory =
      i::Handle<i::WasmMemoryObject>::cast(impl(this)->v8_object());
  return reinterpret_cast<byte_t*>(memory->array_buffer()->backing_store());
}

auto Memory::data_size() const -> size_t {
  i::Handle<i::WasmMemoryObject> memory =
      i::Handle<i::WasmMemoryObject>::cast(impl(this)->v8_object());
  return memory->array_buffer()->byte_length();
}

auto Memory::size() const -> pages_t {
  i::Handle<i::WasmMemoryObject> memory =
      i::Handle<i::WasmMemoryObject>::cast(impl(this)->v8_object());
  return static_cast<pages_t>(memory->array_buffer()->byte_length() /
                              i::wasm::kWasmPageSize);
}
...
auto Instance::make(Store* store_abs, const Module* module_abs,
  ...
  StoreImpl* store = impl(store_abs);
  const RefImpl<Module>* module = impl(module_abs);
  i::Isolate* isolate = store->i_isolate();
  i::HandleScope handle_scope(isolate);
  ...
  i::Handle<i::WasmModuleObject> module_obj =
      i::Handle<i::WasmModuleObject>::cast(module->v8_object());
  NopErrorThrower thrower(isolate);
  i::Handle<i::WasmInstanceObject> instance_obj =
      i::wasm::InstantiateToInstanceObject(isolate, &thrower, module_obj,
                                           imports_obj,
                                           i::MaybeHandle<i::JSArrayBuffer>())
          .ToHandleChecked();
  return RefImpl<Instance>::make(store, instance_obj);
}

auto Instance::exports() const -> vec<Extern*> {
  const RefImpl<Instance>* instance = impl(this);
  StoreImpl* store = instance->store();
  i::Isolate* isolate = store->i_isolate();
  i::HandleScope handle_scope(isolate);
  i::Handle<i::WasmInstanceObject> instance_obj =
      i::Handle<i::WasmInstanceObject>::cast(instance->v8_object());
  ...
  for (size_t i = 0; i < export_types.size(); ++i) {
    ...
    switch (type->kind()) {
      case EXTERN_FUNC: {
        ...
        exports[i].reset(
            RefImpl<Func>::make(store, i::Handle<i::JSFunction>::cast(obj)));
      } break;
      case EXTERN_GLOBAL: {
        exports[i].reset(RefImpl<Global>::make(
            store, i::Handle<i::WasmGlobalObject>::cast(obj)));
      } break;
      case EXTERN_TABLE: {
        exports[i].reset(RefImpl<Table>::make(
            store, i::Handle<i::WasmTableObject>::cast(obj)));
      } break;
      case EXTERN_MEMORY: {
        exports[i].reset(RefImpl<Memory>::make(
