...
class SignatureHelper : public i::AllStatic {
 public:
  ...
  static own<FuncType*> Deserialize(i::PodArray<i::wasm::ValueType> sig) {
    int param_arity = ParamArity(sig);
    int result_arity = sig->length() - param_arity - 1;
  ...
  static int ParamArity(i::PodArray<i::wasm::ValueType> sig) {
    ...
    for (int i = 0; i < sig->length(); i++) {
      if (sig->get(i) == kMarker) break;
    }
    return count;
  }

  static int ResultArity(i::PodArray<i::wasm::ValueType> sig) {
    return sig->length() - ParamArity(sig) - 1;
  }

  static i::PodArray<i::wasm::ValueType> GetSig(
      i::Handle<i::JSFunction> function) {
    DCHECK(i::WasmCapiFunction::IsWasmCapiFunction(*function));
...
auto make_func(Store* store_abs, FuncData* data) -> own<Func*> {
  ...
  i::Handle<i::WasmCapiFunctionData> fun_data =
      i::Handle<i::WasmCapiFunctionData>::cast(isolate->factory()->NewStruct(
          i::WASM_CAPI_FUNCTION_DATA_TYPE, i::AllocationType::kOld));
  fun_data->set_call_target(
      reinterpret_cast<i::Address>(&FuncData::v8_callback));
  fun_data->set_embedder_data(data);
  fun_data->set_serialized_signature(
  ...
  // from Wasm code.
  fun_data->set_wrapper_code(
      isolate->builtins()->builtin(i::Builtins::kIllegal));
