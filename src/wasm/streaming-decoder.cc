...
#define BYTES(x) (x & 0xff), (x >> 8) & 0xff, (x >> 16) & 0xff, (x >> 24) & 0xff
  ...
  {
    base::LockGuard<base::Mutex> guard(&processor_mutex_);
    processor_->FinishStream(std::move(bytes), total_size_);
  }
}

void StreamingDecoder::Abort() {
  base::LockGuard<base::Mutex> guard(&processor_mutex_);
  if (ok()) processor_->Abort();
}
...
StreamingDecoder::DecodeVarInt32::Next(StreamingDecoder* streaming) {
  ...
  if (value() > max_value_) {
    ...
    oss << field_name_ << " is higher than the allowed maximum: " << value()
        << " > " << max_value_;
...
StreamingDecoder::DecodeModuleHeader::Next(StreamingDecoder* streaming) {
  if (!streaming->ProcessModuleHeader()) {
    return streaming->EnterErrorState();
  }
