...
#undef CASE_RET_TYPE
  ...
  Handle<WasmInstanceObject> GetInstanceObject() {
    ...
    // Check that this is indeeded the instance which is connected to this
    // interpreter.
...
wasm::InterpreterHandle* GetOrCreateInterpreterHandle(
  ...
  if (handle->IsUndefined(isolate)) {
    handle =
        Managed<wasm::InterpreterHandle>::New(isolate, isolate, *debug_info);
...
Handle<JSFunction> WasmDebugInfo::GetCWasmEntry(
  ...
  if (!debug_info->has_c_wasm_entries()) {
    ...
    auto managed_map = Managed<wasm::SignatureMap>::New(isolate);
