...
class DebugInfoImpl {
 public:
  ...
  Handle<JSObject> GetStackScopeObject(Isolate* isolate, Address pc,
                                       Address fp) {
    Handle<JSObject> stack_scope_obj =
        isolate->factory()->NewJSObjectWithNullProto();
    wasm::WasmCode* code =
    ...
    for (int i = num_locals; i < value_count; ++i) {
      WasmValue value =
          GetValue(debug_side_table_entry, value_count - i - 1, fp);
      Handle<Object> value_obj = WasmValueToValueObject(isolate, value);
...
Handle<JSObject> DebugInfo::GetStackScopeObject(Isolate* isolate, Address pc,
                                                Address fp) {
  return impl_->GetStackScopeObject(isolate, pc, fp);
}
