...
class InterpreterHandle {
  ...
  static uint32_t GetMemSize(WasmDebugInfo* debug_info) {
    ...
    if (debug_info->wasm_instance()->has_memory_object()) {
      return debug_info->wasm_instance()->wasm_context()->mem_size;
    } else {
      return 0;
    }
  }
  ...
  static byte* GetMemStart(WasmDebugInfo* debug_info) {
    DisallowHeapAllocation no_gc;
    if (debug_info->wasm_instance()->has_memory_object()) {
      return debug_info->wasm_instance()->wasm_context()->mem_start;
    } else {
      return nullptr;
    }
  }
