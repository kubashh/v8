...
// features get limited fuzzer coverage, but are otherwise not thorougly tested.
// Thus should be shipped with enough lead time to the next branch.
#define FOREACH_WASM_STAGING_FEATURE_FLAG(V) /*          (force 80 columns) */ \
  ...
  /* V8 side owner: jkummerow, ssauleau@igalia.com */                          \
  /* Staged in v7.9. */                                                        \
...
#define FOREACH_WASM_SHIPPED_FEATURE_FLAG(V) /*          (force 80 columns) */ \
  ...
  V(bulk_memory, "bulk memory opcodes", true)                                  \
                                                                               \
  /* Non-trappig (i.e. saturation) float-to-int conversions */                 \
