...
// addressable memory after the guard page can be made inaccessible.
  ...
  *allocation_length =
  ...
  if (!memory_tracker->ReserveAddressSpace(*allocation_length)) {
    ...
    //
    // Collect OLD_SPACE because Wasm Memories are allocated there and those are
    // the things we most want to collect.
    heap->CollectGarbage(OLD_SPACE, GarbageCollectionReason::kAllocationLimit);
    if (!memory_tracker->ReserveAddressSpace(*allocation_length)) {
