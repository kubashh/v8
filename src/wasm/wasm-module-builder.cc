...
void WasmModuleBuilder::WriteTo(ZoneBuffer* buffer) const {
  ...
  if (!memories_.empty()) {
    ...
    for (const WasmMemory& memory : memories_) {
      ...
      // Note that even though we use the proper emit function, the memory's min
      // and max value can be more than 32 bits only if we specify it in the
      // higher bits of the `limits_byte`. However, if we emit an up to 32-bit
      // value in the LEB encoding, we get the same results for both cases if we
      // emit it as a 32-bit value or a 64-bit value.
