...
uint32_t WasmModuleBuilder::AddMemory() {
  const uint32_t kMinMemorySize = 16;
  memories_.push_back({kMinMemorySize, 0, false, false});
...
void WasmModuleBuilder::WriteTo(ZoneBuffer* buffer) const {
  ...
  if (!memories_.empty()) {
    printf("Memory size = %zu", memories_.size());
    size_t start = EmitSection(kMemorySectionCode, buffer);
    buffer->write_u8(memories_.size());
