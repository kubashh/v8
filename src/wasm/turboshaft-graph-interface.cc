...
class TurboshaftGraphBuildingInterface {
 public:
  enum Mode { kRegular, kInlinedUnhandled, kInlinedWithHandler };
  ...
  TurboshaftGraphBuildingInterface(
      ...
      : mode_(catch_block == nullptr ? kInlinedUnhandled : kInlinedWithHandler),
        asm_(assembler),
        assumptions_(assumptions),
        inlining_positions_(inlining_positions),
        func_index_(func_index),
  ...
  void DoReturn(FullDecoder* decoder, uint32_t drop_values) {
    ...
    if (mode_ == kRegular) {
      ...
      for (size_t i = 0; i < return_values.size(); i++) {
        return_phis_.phi_inputs[i].emplace_back(return_values[i]);
      }
...
#undef CASE_STORE
 ...
 private:
  ...
  void BuildWasmCall(
      FullDecoder* decoder, const FunctionSig* sig, V<WordPtr> callee,
      V<HeapObject> ref, const Value args[], Value returns[],
      CheckForException check_for_exception = CheckForException::kYes) {
    ...
    OpIndex call =
        check_for_exception == CheckForException::kYes
            ? CallAndMaybeCatchException(
                  decoder, callee, base::VectorOf(arg_indices), descriptor)
            : __ Call(callee, OpIndex::Invalid(), base::VectorOf(arg_indices),
                      descriptor);
  ...
  void BuildWasmMaybeReturnCall(FullDecoder* decoder, const FunctionSig* sig,
    ...
    if (mode_ == kRegular) {
      ...
      // should not try to catch exceptions in the generated call either.
  ...
  OpIndex CallAndMaybeCatchException(FullDecoder* decoder, V<WordPtr> callee,
    ...
    if (!handled_in_this_frame && mode_ != kInlinedWithHandler) {
    ...
    if (handled_in_this_frame) {
      ...
      DCHECK_EQ(mode_, kInlinedWithHandler);
  ...
  void InlineWasmCall(FullDecoder* decoder, const CallFunctionImmediate& imm,
    ...
    Mode inlinee_mode =
        mode_ != kInlinedWithHandler && decoder->current_catch() == -1
            ? kInlinedUnhandled
            : kInlinedWithHandler;
    ...
    inlining_positions_->push_back(WasmInliningPosition{
        static_cast<int>(imm.index),
        SourcePosition(decoder->position(),
                       inlining_id_ == kNoInliningId ? -1 : inlining_id_)});
    inlinee_decoder.interface().set_inlining_id(inlining_positions_->size() -
                                                1);
    ...
    if (inlinee_mode == kInlinedWithHandler &&
...
#undef TRAPREASON_TO_TRAPID
  ...
  SourcePosition OpIndexToSourcePosition(OpIndex index) {
    if (!index.valid()) return SourcePosition(kNoCodePosition, -1);
  ...
  std::vector<OpIndex>& return_values() { return return_values_; }

  bool can_inline() { return inlining_budget_-- > 0; }
  ...
  std::vector<OpIndex>* real_parameters_;  // Used only in StartFunction();
  TSBlock* return_block_ = nullptr;
  BlockPhis return_phis_;
  std::vector<OpIndex> return_values_;
