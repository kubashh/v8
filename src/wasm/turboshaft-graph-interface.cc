...
class TurboshaftGraphBuildingInterface {
 public:
  ...
  V<Tagged> ExternRefToString(FullDecoder* decoder, const Value value,
  ...
  bool HandleWellKnownImport(FullDecoder* decoder, uint32_t index,
    ...
    switch (import) {
      ...
      case WKI::kStringCharCodeAt: {
        V<Tagged> string = ExternRefToString(decoder, args[0]);
      ...
      case WKI::kStringCodePointAt: {
        ...
        V<Tagged> view = __ StringAsWtf16(string);
        // TODO(14108): Annotate `view`'s type.
        result = GetStringCodePointAtImpl(decoder, view, args[1].op);
        decoder->detected_->Add(kFeature_imported_strings);
        break;
      }
      case WKI::kStringCompare: {
        V<Tagged> a_string = ExternRefToString(decoder, args[0]);
        V<Tagged> b_string = ExternRefToString(decoder, args[1]);
        result = __ UntagSmi(CallBuiltinFromRuntimeStub(
            decoder, WasmCode::kStringCompare, {a_string, b_string},
            Operator::kEliminatable));
        decoder->detected_->Add(kFeature_imported_strings);
        break;
      }
      case WKI::kStringConcat: {
        V<Tagged> head_string = ExternRefToString(decoder, args[0]);
        V<Tagged> tail_string = ExternRefToString(decoder, args[1]);
        V<HeapObject> native_context = LOAD_IMMUTABLE_INSTANCE_FIELD(
            NativeContext, MemoryRepresentation::TaggedPointer());
        result = CallBuiltinFromRuntimeStub(
      ...
      case WKI::kStringEquals: {
        ...
        V<Tagged> a_string = ExternRefToString(decoder, args[0], kNullSucceeds);
        V<Tagged> b_string = ExternRefToString(decoder, args[1], kNullSucceeds);
        result = GetStringEqImp(decoder, a_string, b_string, kWasmExternRef,
                                kWasmExternRef);
        decoder->detected_->Add(kFeature_imported_strings);
        break;
      }
      case WKI::kStringFromCharCode: {
        V<Word32> capped =
            __ Word32BitwiseAnd(args[0].op, __ Word32Constant(0xFFFF));
        result = CallBuiltinFromRuntimeStub(decoder,
                                            WasmCode::kWasmStringFromCodePoint,
      ...
      case WKI::kStringFromCodePoint:
        result = CallBuiltinFromRuntimeStub(
            decoder, WasmCode::kWasmStringFromCodePoint, {args[0].op},
            Operator::kEliminatable);
        // TODO(14108): Annotate `result`'s type.
        decoder->detected_->Add(kFeature_imported_strings);
        break;
      case WKI::kStringFromWtf16Array:
        result = CallBuiltinFromRuntimeStub(
            decoder, WasmCode::kWasmStringNewWtf16Array,
      ...
      case WKI::kStringFromWtf8Array:
        ...
        result = GetStringNewWtf8ArrayImpl(decoder, unibrow::Utf8Variant::kWtf8,
      ...
      case WKI::kStringLength: {
        V<Tagged> string = ExternRefToString(decoder, args[0]);
        result = __ Load(string, LoadOp::Kind::TaggedBase(),
...
#undef CASE_STORE
  ...
  V<Tagged> GetStringNewWtf8ArrayImpl(FullDecoder* decoder,
  ...
  void StringNewWtf8Array(FullDecoder* decoder,
    ...
    result->op = GetStringNewWtf8ArrayImpl(decoder, variant, array, start, end);
  }
  ...
  V<Word32> GetStringEqImp(FullDecoder* decoder, V<Tagged> a, V<Tagged> b,
    ...
    GOTO(done, CallBuiltinFromRuntimeStub(decoder, WasmCode::kWasmStringEqual,
  ...
  void StringEq(FullDecoder* decoder, const Value& a, const Value& b,
                Value* result) {
    result->op = GetStringEqImp(decoder, a.op, b.op, a.type, b.type);
  }
  ...
  void StringViewWtf16GetCodeUnit(FullDecoder* decoder, const Value& view,
    ...
    result->op = GetCodeUnitImpl(decoder, string, pos.op);
  }

  V<Word32> GetStringCodePointAtImpl(FullDecoder* decoder, V<Tagged> string,
                                     V<Word32> offset) {
    OpIndex prepare = __ StringPrepareForGetCodeUnit(string);
    V<Tagged> base =
        __ Projection(prepare, 0, RegisterRepresentation::Tagged());
    ...
    V<Word32> is_lead_surrogate =
        __ Word32Equal(__ Word32BitwiseAnd(lead, __ Word32Constant(0xFC00)),
                       __ Word32Constant(0xD800));
    GOTO_IF_NOT(is_lead_surrogate, done, lead);
    V<Word32> trail_offset = __ Word32Add(offset, __ Word32Constant(1));
    ...
    V<Word32> is_trail_surrogate =
        __ Word32Equal(__ Word32BitwiseAnd(trail, __ Word32Constant(0xFC00)),
                       __ Word32Constant(0xDC00));
    GOTO_IF_NOT(is_trail_surrogate, done, lead);
    V<Word32> surrogate_bias =
        __ Word32Constant(0x10000 - (0xD800 << 10) - 0xDC00);
    V<Word32> result =
        __ Word32Add(__ Word32ShiftLeft(lead, __ Word32Constant(10)),
    ...
    BIND(bailout);
    GOTO(done, CallBuiltinFromRuntimeStub(
                   decoder, wasm::WasmCode::kWasmStringCodePointAt,
                   {string, offset}, Operator::kPure));
