...
#undef CASE_STORE
 ...
 private:
  ...
  class InstanceCache {
   public:
    ...
    void Initialize(V<WasmInstanceObject> instance, const WasmModule* mod) {
      ...
      if (!mod->memories.empty()) {
        ...
        memory_index_type_ = mem.is_memory64 ? kWasmI64 : kWasmI32;
        ...
        if (memory_can_grow_) num_mutable_fields_++;
        if (memory_can_move_) num_mutable_fields_++;
        LoadMemStart();
        LoadMemSize();
      } else {
        memory_can_grow_ = false;
        memory_can_move_ = false;
      }
    }
    ...
    void ReloadCachedMemory() {
      if (memory_can_move_) LoadMemStart();
      if (memory_can_grow_) LoadMemSize();
    }

    uint32_t num_mutable_fields() { return num_mutable_fields_; }

    ValueType mutable_field_type(uint32_t index) {
      DCHECK_IMPLIES(!memory_can_grow_, !memory_can_move_);
      // There are three possibilities:
      // (1) Memory can neither grow nor move -> this function is never called.
      // (2) Memory can grow but not move -> size has index 0.
      // (3) Memory can grow and move -> size has index 0, start has index 1.
      if (index == 0) return memory_index_type_;
      DCHECK_EQ(1, index);
    ...
    OpIndex mutable_field_value(uint32_t index) {
      // See above for possible cases.
      if (index == 0) return mem_size_;
      DCHECK_EQ(1, index);
      return mem_start_;
    }

    void set_mutable_field_value(uint32_t index, OpIndex value) {
      if (index == 0) mem_size_ = V<WordPtr>::Cast(value);
      DCHECK_EQ(1, index);
   ...
   private:
    bool has_memory() { return memory_index_type_ != kWasmBottom; }

    void LoadMemStart() {
      DCHECK(has_memory());
      LoadOp::Kind kind = LoadOp::Kind::TaggedBase();
      if (!memory_can_move_) kind = kind.Immutable();
      mem_start_ = __ Load(instance_node_, kind, kMaybeSandboxedPointer,
    ...
    void LoadMemSize() {
      ...
      mem_size_ =
          __ Load(instance_node_, kind, MemoryRepresentation::PointerSized(),
    ...
    Assembler& asm_;
    ValueType memory_index_type_{kWasmBottom};
    bool memory_can_move_;
    bool memory_can_grow_;
