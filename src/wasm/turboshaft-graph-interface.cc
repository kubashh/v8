...
class TurboshaftGraphBuildingInterface : public WasmGraphBuilderBase {
 ...
 public:
  ...
  void GenerateDeoptIfNot(FullDecoder* decoder, OpIndex deopt_condition,
    ...
    // Add the closure. Wasm doesn't have one, use the WasmTrustedInstanceData
    // as a dummy value.
    ...
    for (uint32_t i = static_cast<uint32_t>(callee_sig->return_count());
      ...
      compiler::turboshaft::Print(__ output_graph().Get(val->op));
    }
    // Add the call_ref stack values.
    base::Vector<const Value> argsv(args, callee_sig->parameter_count());
    for (auto iter = argsv.begin(); iter != argsv.end(); ++iter) {
      builder.AddInput(iter->type.machine_type(), iter->op);
    }
    builder.AddInput(func_ref.type.machine_type(), func_ref.op);
    // Add the wasm trusted instance as a real input.
    builder.AddInput(MachineType::AnyTagged(), trusted_instance_data());
    const size_t kTheCallee = 1;   // The JS closure (not used).
    const size_t kTheContext = 1;  // The JS context (not used).
    const size_t kTheWasmInstance = 1;
    size_t local_count = callee_sig->parameter_count() + kTheCallee +
                         kTheContext + kTheWasmInstance +
  ...
  void CallRef(FullDecoder* decoder, const Value& func_ref,
    ...
    if (inlining_enabled(decoder) &&
      ...
      size_t case_count = feedback_cases.size() + !use_deopt;
      ...
      for (size_t i = 0; i < feedback_cases.size(); i++) {
        ...
        if (use_deopt && is_last_case) {
          GenerateDeoptIfNot(decoder,
                             __ TaggedEqual(func_ref.op, inlined_func_ref), sig,
                             func_ref, args);
        } else {
