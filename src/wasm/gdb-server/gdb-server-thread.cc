...
void GdbServerThread::InitializeThread() {
...
void GdbServerThread::Stop() {
  ...
  // initializing {transport_} and {target_}. If this happens and the
  // GdbServer thread is blocked waiting for an incoming connection or
  // for incoming packets, it will unblocked when {transport_} is closed.
  v8::base::MutexGuard guard(&mutex_);
