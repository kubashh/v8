...
void GdbServerThread::Run() {
  ...
  if (FLAG_wasm_pause_waiting_for_debugger) {
...
void GdbServerThread::OnSuspended(const std::vector<uint64_t>& callFrames) {
  target_->OnSuspended(kSigTrace, callFrames);
}

void GdbServerThread::OnException(const std::vector<uint64_t>& callFrames) {
  target_->OnSuspended(kSigSegv, callFrames);
}
