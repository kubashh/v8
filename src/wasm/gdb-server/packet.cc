...
void Packet::AddRawChar(char ch) {
  // Grow by a fixed amount whenever we are within the pad boundary.
  // The pad boundary allows for the addition of NUL termination.
...
void Packet::AddNumberSep(uint64_t val, char sep) {
  ...
  if (val == static_cast<uint64_t>(-1)) {
    ...
    // Assume we have the value 0x00001234
    for (size_t a = 0; a < sizeof(val); a++) {
      ...
      UInt8ToHex(byte, &out[nibbles], true);
      nibbles += 2;
