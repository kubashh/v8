...
bool Packet::EndOfPacket() const { return (read_index_ >= write_index_); }
...
void Packet::AddRawChar(char ch) {
  ...
  if (data_.size() <= (write_index_ + kMinPad)) {
    data_.resize(data_.size() + kGrowSize);
  }
...
void Packet::AddWord16(uint16_t val) { AddBlock(&val, sizeof(val)); }

void Packet::AddWord32(uint32_t val) { AddBlock(&val, sizeof(val)); }

void Packet::AddWord64(uint64_t val) { AddBlock(&val, sizeof(val)); }
...
bool Packet::GetRawChar(char* ch) {
  ...
  *ch = data_[read_index_++];
  ...
  if (*ch == '*') {
    if (read_index_ < 2) {
      GdbRemoteLog(LOG_ERROR, "Unexpected RLE at start of packet.\n");
      return false;
    }

    if (read_index_ >= write_index_) {
      GdbRemoteLog(LOG_ERROR, "Unexpected EoP during RLE.\n");
      return false;
    }
    ...
    if (cnt < 3) {
      GdbRemoteLog(LOG_ERROR, "Unexpected RLE length.\n");
      return false;
    }
...
bool Packet::GetHexString(std::string* str) {
  ...
  while (GetRawChar(&ch1)) {
    ...
    if (!GetRawChar(&ch2) || !NibbleToInt(ch2, &nib2)) {
      return false;
    }
...
const char* Packet::GetPayload() const { return &data_[0]; }

size_t Packet::GetPayloadSize() const { return write_index_; }
