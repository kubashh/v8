// Copyright 2019 the V8 project authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef V8_INSPECTOR_GDB_SERVER_GDB_SERVER_H_
#define V8_INSPECTOR_GDB_SERVER_GDB_SERVER_H_

#ifdef V8_ENABLE_WASM_GDB_REMOTE_DEBUGGING

#include <map>
#include <string>
#include "src/debug/debug.h"

namespace v8 {
namespace internal {

class Isolate;

namespace wasm {

class WasmEngine;

namespace gdb_server {

class GdbServerThread;
class TaskRunner;

class GdbServer {
 public:
  explicit GdbServer(Isolate* isolate, WasmEngine* wasm_engine);
  ~GdbServer();

  void AddIsolate(Isolate* isolate);
  void RemoveIsolate(Isolate* isolate);

  void Suspend();
  void PrepareStep();

  std::string GetWasmModuleString();

  bool GetWasmGlobal(uint32_t wasm_module_id, uint32_t index, uint64_t* value);
  bool GetWasmLocal(uint32_t wasm_module_id, uint32_t frame_index,
                    uint32_t index, uint64_t* value);
  bool GetWasmOperandStackValue(uint32_t wasm_module_id, uint32_t index,
                                uint64_t* value);
  uint32_t GetWasmMemory(uint32_t module_id, uint32_t offset, uint8_t* buffer,
                         uint32_t size);
  uint32_t GetWasmModuleBytes(uint64_t address, uint8_t* buffer, uint32_t size);
  bool AddBreakpoint(uint64_t address);
  bool RemoveBreakpoint(uint64_t address);
  void GetWasmCallStack(std::vector<uint64_t>* callStackPCs);

  void QuitMessageLoopOnPause();

  static uint64_t WasmAddressFromModuleAndOffset(uint32_t module_id,
                                                 uint32_t offset) {
    return static_cast<uint64_t>(module_id) << 32 | offset;
  }
  static uint32_t ModuleIdFromWasmAddress(uint64_t address) {
    return address >> 32;
  }
  static void ModuleIdAndOffsetFromWasmAddress(uint64_t address,
                                               uint32_t* module_id,
                                               uint32_t* offset) {
    *module_id = ModuleIdFromWasmAddress(address);
    *offset = address & 0xffffffff;
  }

 private:
  struct WasmDebugScript {
    WasmDebugScript(v8::Isolate* isolate, Local<debug::WasmScript> script);

    v8::Isolate* isolate_;
    Global<debug::WasmScript> wasm_script_;
  };

  class DebugDelegate : public debug::DebugDelegate {
   public:
    DebugDelegate(Isolate* isolate, GdbServer* gdb_server);

    // debug::DebugDelegate
    void ScriptCompiled(Local<debug::Script> script, bool is_live_edited,
                        bool has_compile_error) override;
    void BreakProgramRequested(Local<v8::Context> paused_context,
                               const std::vector<debug::BreakpointId>&
                                   inspector_break_points_hit) override;
    void ExceptionThrown(Local<v8::Context> paused_context,
                         Local<Value> exception, Local<Value> promise,
                         bool is_uncaught,
                         debug::ExceptionType exception_type) override;
    bool IsFunctionBlackboxed(Local<debug::Script> script,
                              const debug::Location& start,
                              const debug::Location& end) override;

   private:
    std::vector<uint64_t> CalculateCallStack(
        v8::Local<v8::StackTrace> stack_trace) const;

    Isolate* isolate_;
    GdbServer* gdb_server_;
  };

  void RunMessageLoopOnPause();

  v8::Local<debug::WasmScript> GetWasmScript(uint32_t module_id);
  Handle<WasmInstanceObject> GetWasmInstance(
      v8::Local<debug::WasmScript> wasm_script);

  std::unique_ptr<GdbServerThread> thread_;

  WasmEngine* wasm_engine_;
  std::unique_ptr<TaskRunner> task_runner_;

  typedef std::map<uint64_t, int> BreakpointsMap;
  BreakpointsMap breakpoints_;

  base::RecursiveMutex mutex_;
  //////////////////////////////////////////////////////////////////////////////
  // Protected by {mutex_}:

  typedef std::map<int, std::unique_ptr<WasmDebugScript>> ScriptsMap;
  ScriptsMap scripts_;

  typedef std::map<Isolate*, std::unique_ptr<DebugDelegate>>
      IsolateDebugDelegateMap;
  IsolateDebugDelegateMap isolate_delegates_;

  // End of fields protected by {mutex_}.
  //////////////////////////////////////////////////////////////////////////////
};

}  // namespace gdb_server
}  // namespace wasm
}  // namespace internal
}  // namespace v8

#endif  // V8_ENABLE_WASM_GDB_REMOTE_DEBUGGING
#endif  // V8_INSPECTOR_GDB_SERVER_GDB_SERVER_H_
