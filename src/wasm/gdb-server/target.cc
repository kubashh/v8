...
void Target::Run(Session* session) {
  ...
  do {
    ...
    ProcessDebugEvent();
    ProcessCommands();
  } while (!IsTerminated() && session_->Connected());
...
void Target::WaitForDebugEvent() {
  ...
  bool wait_for_debug_event = false;
  {
    v8::base::MutexGuard guard(&mutex_);
    if (status_ != Status::Terminated && status_ != Status::Suspended) {
      ...
      wait_for_debug_event = true;
    }
  }

  if (wait_for_debug_event) {
    // Wait for either:
...
void Target::ProcessDebugEvent() {
  ...
  bool suspend = false;
  {
    ...
    if (IsTerminated() || cur_signal_ != 0 || status_ == Status::Suspended ||
        status_ == WaitingForSuspension) {
      // Already terminated or suspended. Nothing to do.
    } else if (!waiting_for_initial_suspension_ &&
               !session_->IsDataAvailable()) {
...
void Target::OnSuspended(int signal, const std::vector<uint64_t>& call_frames) {
...
bool Target::ProcessPacket(Packet* pktIn, Packet* pktOut) {
  ...
  switch (cmd) {
    ...
    case 'm': {
      ...
      uint32_t module_id = address >= 1ull << 32;
      if (module_id > 0) {
        uint32_t read = gdb_server_->GetWasmModuleBytes(address, buff, length);
        if (read > 0) {
      ...
      if (toks[0] == "WasmCallStack") {  // qWasmCallStack
        ...
        if (toks.size() == 3) {
          ...
          uint32_t index =
              static_cast<uint32_t>(strtol(toks[2].data(), nullptr, 10));
          uint64_t value = 0;
          if (gdb_server_->GetWasmOperandStackValue(module_id, index, &value)) {
            pktOut->AddBlock(&value, sizeof(value));
          } else {
            err = FAILED;
          }
        ...
        if (toks.size() == 4) {
          uint32_t module_id =
              strtol(toks[1].data(), nullptr,
                     10);  // should identify a Memory instance.
          ...
          uint32_t read = gdb_server_->GetWasmMemory(
              module_id, address & 0xffffffff, buff, length);
          if (read > 0) {
            pktOut->AddBlock(buff, read);
          } else {
            err = FAILED;
