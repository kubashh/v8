...
void Target::WaitForDebugEvent() {
  ...
  if (status_ == Status::Suspended) {
    ...
    // If the debugger has just attached, wait for the wasm engine to be
    // suspended before consuming debugger messages.
    return;
  }
...
void Target::Suspend() {
  ...
  if (status_ == Status::Running) {
    auto suspendWasmInterpreter = [](v8::Isolate* isolate, void* data) {
      // Executed in the isolate thread

      Target* target = reinterpret_cast<Target*>(data);
      v8::base::MutexGuard guard(&target->mutex_);
      target->gdb_server_->suspend();
    };

    v8::Isolate* v8_isolate =
        reinterpret_cast<v8::Isolate*>(gdb_server_->isolate());
    v8_isolate->RequestInterrupt(suspendWasmInterpreter, this);
...
void Target::OnSuspended(const std::vector<uint64_t>& call_frames) {
  ...
  cur_signal_ = kSigInt;  // TODO(paolosev): handle traps

  char tmp[16];
  snprintf(tmp, sizeof(tmp), "QC%x", kThreadId);
  properties_["C"] = tmp;

  bool isWaitingForSuspendion = (status_ == Status::WaitingForSuspension);
  status_ = Status::Suspended;

  if (isWaitingForSuspendion) {
  ...
  switch (cmd) {
    ...
    case 'q': {
      ...
      if (toks[0] == "WasmCallStack") {  // qWasmCallStack
        ...
        err = BAD_FORMAT;
        break;
      } else if (toks[0] == "WasmLocal") {  // qWasmLocal:moduleId;index
        if (toks.size() == 3) {
          uint32_t module_id =
              static_cast<uint32_t>(strtol(toks[1].data(), nullptr, 10));
          uint32_t index =
