...
#undef BYTES
  ...
  void DecodeCompilationHintsSection() {
    ...
    // section are ignored.
    const bool after_function_section =
        next_ordered_section_ > kFunctionSectionCode;
    const bool before_code_section = next_ordered_section_ <= kCodeSectionCode;
    if (after_function_section && before_code_section &&
      ...
      for (uint32_t i = 0; decoder.ok() && i < hint_count; i++) {
        ...
        // Ensure that the second tier never dowgrades the compilation result.
    ...
    if (flags & 1) {
      ...
      *maximum = consume_u32v("maximum size");
      if (*maximum > max_maximum) {
        errorf(pos,
               "maximum %s size (%u %s) is larger than implementation limit "
               "(%u)",
