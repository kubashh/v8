    ...
    switch (section_code) {
      ...
      case kDebugInfoSectionCode:
        ...
        if (module_->debug_symbols.type ==
            debug::WasmDebugSymbols::Type::None) {
          module_->debug_symbols = {
              debug::WasmDebugSymbols::Type::EmbeddedDWARF, ""};
        }
  ...
  void DecodeExternalDebugInfoSection() {
    ...
    if (inner.ok() && module_->debug_symbols.type !=
      ...
      const byte* url_start =
          inner.start() + inner.GetBufferRelativeOffset(url.offset());
      module_->debug_symbols = {
          debug::WasmDebugSymbols::Type::ExternalDWARF,
          {reinterpret_cast<const char*>(url_start), url.length()}};
