...
#undef BYTES
 ...
 private:
        ...
        case kGCPrefix: {
          ...
          switch (opcode) {
            ...
            case kExprArrayInit: {
              ...
              stack.push_back(WasmInitExpr::ArrayNewWithRtt(
                  array_imm.index, std::move(arguments)));
              break;
            }
            case kExprRttCanon: {
  ...
  const ArrayType* consume_array(Zone* zone) {
    ...
    if (failed()) return nullptr;
    bool mutability = consume_mutability();
    if (!V8_LIKELY(mutability || enabled_features_.has_gc_experiments())) {
      error(this->pc() - 1,
            "immutable arrays are not supported. Enable with "
            "--experimental-wasm-gc-experiments");
    }
