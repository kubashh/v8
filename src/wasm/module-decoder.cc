    ...
    if (!enabled_features_.has_gc()) {
      for (uint32_t i = 0; i < types_count; ++i) {
        ...
        if (FLAG_wasm_type_canonicalization) {
          type_canon->add_recursive_group(module_.get(), 1);
        }
      }
      return;
    }
    ...
    if (types_count > 0) {
      ...
      if (first_type_opcode == kWasmFunctionNominalCode ||
        ...
        if (ok() && FLAG_wasm_type_canonicalization) {
          type_canon->add_recursive_group(module_.get(), types_count);
        }
      } else {
        ...
        for (uint32_t i = 0; ok() && i < types_count; ++i) {
          ...
          if (kind == kWasmRecursiveTypeGroupCode) {
            ...
            if (ok() && FLAG_wasm_type_canonicalization) {
              type_canon->add_recursive_group(module_.get(), group_size);
            }
          } else {
            TypeDefinition type = consume_subtype_definition();
            if (ok()) {
