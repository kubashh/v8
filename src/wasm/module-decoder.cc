 ...
 private:
  ...
  void next() {
    ...
    if (section_code == kUnknownSectionCode) {
      ...
      if (inner.ok()) {
        // We have to adjust the payload start to after the name of the custom
        // section. We also have to consume the string bytes in the main
        // decoder.
        payload_start_ = inner.pc();
        decoder_->consume_bytes(
            static_cast<uint32_t>(inner.pc() - inner.start()), "section name");
      }
