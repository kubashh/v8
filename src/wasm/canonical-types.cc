...
void TypeCanonicalizer::add_recursive_group(WasmModule* module, uint32_t size) {
  ...
  for (uint32_t i = 0; i < size; i++) {
    group.types[i] = canonicalize_type_def(
        module, module->types[start_index + i], start_index);
  }
  int canonical_index = find_canonical_group(group);
...
ValueType TypeCanonicalizer::canonicalize_value_type(
    const WasmModule* module, ValueType type,
    uint32_t recursive_group_start) const {
  return type.has_index()
             ? type.ref_index() >= recursive_group_start
                   ? ValueType::CanonicalWithRelativeIndex(
                         type.kind(), type.ref_index() - recursive_group_start)
                   : ValueType::FromIndex(
                         type.kind(), module->isorecursive_canonical_type_ids
                                          [type.ref_index()])
             : type;
}

bool TypeCanonicalizer::is_canonical_subtype(uint32_t sub_index,
...
TypeCanonicalizer::CanonicalType TypeCanonicalizer::canonicalize_type_def(
  ...
  switch (type.kind) {
    case TypeDefinition::kFunction: {
      ...
      for (ValueType ret : original_sig->returns()) {
        builder.AddReturn(
            canonicalize_value_type(module, ret, recursive_group_start));
      }
      for (ValueType param : original_sig->parameters()) {
        builder.AddParam(
            canonicalize_value_type(module, param, recursive_group_start));
      }
    ...
    case TypeDefinition::kStruct: {
      ...
      for (uint32_t i = 0; i < original_type->field_count(); i++) {
        builder.AddField(
            canonicalize_value_type(module, original_type->field(i),
    ...
    case TypeDefinition::kArray: {
      ValueType element_type = canonicalize_value_type(
...
int TypeCanonicalizer::find_canonical_group(CanonicalGroup& group) const {
