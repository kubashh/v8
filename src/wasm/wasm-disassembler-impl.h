...
constexpr bool kIndicesAsComments = true;
...
inline StringBuilder& operator<<(StringBuilder& sb, Indentation indentation) {
  char* ptr = sb.write(indentation.current());
...
void PrintSignatureOneLine(StringBuilder& out, const FunctionSig* sig,
                           ...
                           bool param_names, bool indices_as_comments = false);
...
class FunctionBodyDisassembler : public WasmDecoder<Decoder::kFullValidation> {
 public:
  ...
  void DecodeWat(MultiLineStringBuilder& out, Indentation indentation,
 ...
 protected:
  ...
  // name section, their order of occurrence matters; for generating names
...
class ModuleDisassembler {
 public:
  ...
  void PrintModule(Zone* zone, Indentation indentation);
 ...
 private:
  ...
  void PrintInitExpression(const ConstantExpression& init,
                           ValueType expected_type, Zone* zone);
