...
class LiftoffCompiler {
 public:
  ...
  struct Control : public ControlWithNamedConstructors<Control, Value> {
    ...
    std::unique_ptr<StateForElseBranch> else_state;
  ...
  void If(Decoder* decoder, const Value& cond, Control* if_block) {
    ...
    if_block->else_state = base::make_unique<StateForElseBranch>();
...
#define CASE_UNOP(opcode, type, fn) \
  case WasmOpcode::kExpr##opcode:   \
    return type##UnOp(&LiftoffAssembler::emit_##fn);
    switch (opcode) {
