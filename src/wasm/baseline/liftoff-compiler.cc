  ...
  void CallRefImpl(FullDecoder* decoder, ValueType func_ref_type,
    ...
    if (inlining_enabled(decoder)) {
      ...
      if (v8_flags.wasm_deopt) {
        ...
        frame_descriptions_->descriptions.emplace_back(decoder->pc_offset(),
                                                       __ pc_offset());
        LiftoffFrameDescription& frame =
            frame_descriptions_->descriptions.back();
        for (const VarState& state : __ cache_state()->stack_state) {
          frame.var_state.push_back(state);
        }
        frame.trusted_instance = __ cache_state() -> cached_instance_data;
      }
      LiftoffRegList pinned;
