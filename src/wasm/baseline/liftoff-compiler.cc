...
int GetNumberOfParamStackSlots(FunctionSig* sig) {
  ...
  for (wasm::ValueType type : sig->parameters()) {
    ...
    // For fp, We only allocate f64 registers, to avoid aliasing issues on arm.
    if (rc == kFpReg &&
        linkage.CanAllocateFP(MachineRepresentation::kFloat64)) {
      linkage.NextFpReg(MachineRepresentation::kFloat64);
      continue;
    }
    linkage.NextStackSlot(ValueTypes::MachineRepresentationFor(type));
  }
      ...
      if (rc == kGpReg && linkage->CanAllocateGP()) {
        ...
        if (kLiftoffAssemblerGpCacheRegs & in_reg.bit()) {
          ...
          __ Move(param_reg.gp(), in_reg, type);
                 ...
                 linkage->CanAllocateFP(MachineRepresentation::kFloat64)) {
        // Passed via fp reg.
        // We only allocate f64 registers, to avoid aliasing issues on arm.
...
#undef CASE_CCALL_BINOP
  ...
  void GenerateRuntimeCall(Runtime::FunctionId runtime_function, int num_args,
    ...
    // The argument to the runtime call goes on the stack.
