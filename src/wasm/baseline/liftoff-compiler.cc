...
bool LiftoffCompilationUnit::ExecuteCompilation(CompilationEnv* env,
  ...
  base::Optional<TimedHistogramScope> liftoff_compile_time_scope(
      base::in_place, env->counters->liftoff_compile_time());
  ...
  if (!compiler->ok()) {
    ...
    env->counters->liftoff_unsupported_functions()->Increment();
    return false;
  }

  env->counters->liftoff_compiled_functions()->Increment();
