...
#undef ATOMIC_COMPARE_EXCHANGE_LIST
  ...
  void StructNew(FullDecoder* decoder,
    ...
    // TODO(liftoff): We should be able to move pinning this register after the
    // call, but we run into an issue.
    LiftoffRegister obj(kReturnRegister0);
    LiftoffRegList pinned = {obj};

    LiftoffRegister instance_size = pinned.set(__ GetUnusedRegister(pinned));
