...
#undef CASE_SIMD_REPLACE_LANE_OP
  ...
  void AtomicWait(FullDecoder* decoder, ValueKind kind,
                  const MemoryAccessImmediate& imm) {
    {
      ...
      Register instance = LoadInstanceIntoRegister({timeout});
    ...
    if (offset) {
      ...
      LiftoffAssembler::VarState index = LiftoffAssembler::VarState{
          kI32, LiftoffRegister{index_plus_offset}, 0};

      auto target = kind == kI32 ? WasmCode::kWasmI32AtomicWait
                                 : WasmCode::kWasmI64AtomicWait;

      CallRuntimeStub(target, MakeSig::Params(kI32), {index},
                      decoder->position());
