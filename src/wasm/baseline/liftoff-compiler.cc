...
#undef CASE_CCALL_BINOP
  ...
  void AtomicWait(FullDecoder* decoder, ValueType type,
...
#else
    LiftoffRegList pinned;
    LiftoffRegister timeout = pinned.set(__ PopToRegister());
    LiftoffRegister expected_value = pinned.set(__ PopToRegister());
    Register index = pinned.set(__ PopToRegister(pinned)).gp();
    ...
    uint32_t offset = imm.offset;
    index = AddMemoryMasking(index, &offset, &pinned);
    __ emit_i32_add(index, index, offset);
    ...
    if (type == kWasmI32) {
      WasmI32AtomicWaitDescriptor descriptor;
      DCHECK_EQ(0, descriptor.GetStackParameterCount());
      ...
      WasmI64AtomicWaitDescriptor descriptor;
      DCHECK_EQ(0, descriptor.GetStackParameterCount());
      DCHECK_EQ(3, descriptor.GetRegisterParameterCount());
      params[0] = descriptor.GetRegisterParameter(0);
      params[1] = descriptor.GetRegisterParameter(1);
    ...
    if (type == kWasmI32) {
      __ CallRuntimeStub(WasmCode::kWasmI32AtomicWait);
    } else {
      __ CallRuntimeStub(WasmCode::kWasmI64AtomicWait);
    }
    RegisterDebugSideTableEntry(DebugSideTableBuilder::kDidSpill);
    safepoint_table_builder_.DefineSafepoint(&asm_, Safepoint::kNoLazyDeopt);
