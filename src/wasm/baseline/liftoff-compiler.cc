...
#undef ATOMIC_COMPARE_EXCHANGE_LIST
  ...
  VarState MemTypeToVarStateSaturating(int stack_index,
                                       LiftoffRegList* pinned) {
    VarState& slot = __ cache_state()->stack_state.end()[stack_index];
    const bool is_mem64 = slot.kind() == kI64;
    // For memory32 on a 32-bit system or memory64 on a 64-bit system, there is
    // nothing to do.
    if ((kSystemPointerSize == kInt64Size) == is_mem64) {
  ...
  bool MatchingMemType(const WasmMemory* memory, int stack_slot) {
    DCHECK_LE(-stack_slot, __ cache_state()->stack_state.size());
    ValueKind expected_kind = memory->is_memory64 ? kI64 : kI32;
    DCHECK_EQ(expected_kind,
              __ cache_state()->stack_state.end()[stack_slot].kind());
    return true;
  }
#endif
  ...
  void StringNewWtf8(FullDecoder* decoder, const MemoryIndexImmediate& imm,
    ...
    VarState address = MemTypeToVarStateSaturating(-2, &pinned);
  ...
  void StringEncodeWtf8(FullDecoder* decoder, const MemoryIndexImmediate& imm,
    ...
    VarState offset_var = MemTypeToVarStateSaturating(-1, &pinned);
  ...
  void StringViewWtf8Encode(FullDecoder* decoder,
    ...
    DCHECK(MatchingMemType(imm.memory, -3));
    VarState addr_var = MemTypeToVarStateSaturating(-3, &pinned);
  ...
  void StringViewWtf16Encode(FullDecoder* decoder,
    ...
    DCHECK(MatchingMemType(imm.memory, -3));
    VarState offset_var = MemTypeToVarStateSaturating(-3, &pinned);
