...
class LiftoffRegister {
 ...
 public:
  ...
  LiftoffRegister fst() const { return LiftoffRegister(fst_gp()); }

  LiftoffRegister snd() const { return LiftoffRegister(snd_gp()); }

  Register fst_gp() const {
    DCHECK(is_pair());
    static constexpr storage_t kCodeMask = (1 << kBitsPerGpRegCode) - 1;
    return Register::from_code(code_ & kCodeMask);
  }

  Register snd_gp() const {
    DCHECK(is_pair());
    static constexpr storage_t kCodeMask = (1 << kBitsPerGpRegCode) - 1;
  ...
  bool overlaps(const LiftoffRegister other) const {
    if (is_pair()) return fst().overlaps(other) || snd().overlaps(other);
    if (other.is_pair()) return *this == other.fst() || *this == other.snd();
    return *this == other;
  }
...
inline std::ostream& operator<<(std::ostream& os, LiftoffRegister reg) {
  if (reg.is_pair()) {
    return os << "<gp" << reg.fst_gp().code() << "+" << reg.snd_gp().code()
              << ">";
  } else if (reg.is_gp()) {
...
class LiftoffRegList {
 public:
  ...
  LiftoffRegister set(LiftoffRegister reg) {
    if (reg.is_pair()) {
      regs_ |= storage_t{1} << reg.fst().liftoff_code();
      regs_ |= storage_t{1} << reg.snd().liftoff_code();
    } else {
  ...
  bool has(LiftoffRegister reg) const {
    if (reg.is_pair()) {
      DCHECK_EQ(has(reg.fst()), has(reg.snd()));
      reg = reg.fst();
    }
