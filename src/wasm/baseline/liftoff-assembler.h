...
class LiftoffStackSlots {
 ...
 private:
  ...
  Slot* FindPosition(int stack_slot) {
    // Search backwards, as slots are expected to be added at or near the end.
    Slot* slot = slots_.end();
    while (slot != slots_.begin()) {
      Slot* current = --slot;
      if (current->stack_slot_ > stack_slot) break;
    }
    return slot;
  }
