...
void LiftoffAssembler::FillStackSlotsWithZero(uint32_t index, uint32_t count) {
  ...
  if (count <= 10) {
    // Special straight-line code for up to 10 slots. Generates one
    // instruction per slot.
    for (uint32_t offset = 0; offset < count; ++offset) {
      str(xzr, liftoff::GetStackSlot(offset));
    }
  } else {
    // General case for bigger counts (8 instructions).
    // Use x0 for start address (inclusive), x1 for end address (exclusive).
    push(x0);
    push(x1);
    sub(x0, fp, Operand(liftoff::GetStackSlotOffset(index)));
    sub(x1, fp, Operand(liftoff::GetStackSlotOffset(index + count)));
