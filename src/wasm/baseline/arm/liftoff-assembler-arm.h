...
void LiftoffAssembler::FillStackSlotsWithZero(uint32_t index, uint32_t count) {
  ...
  if (count <= 5) {
    ...
    for (uint32_t offset = 0; offset < count; ++offset) {
      str(r0, liftoff::GetHalfStackSlot(offset, kLowWord));
      str(r0, liftoff::GetHalfStackSlot(offset, kHighWord));
    }
  } else {
    ...
    push(r1);
    push(r2);
    sub(r1, fp, Operand(liftoff::GetStackSlotOffset(index)));
    sub(r2, fp, Operand(liftoff::GetStackSlotOffset(index + count)));
