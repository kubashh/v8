...
class StackTransferRecipe {
 ...
 public:
  ...
  void LoadI64HalfIntoRegister(LiftoffRegister dst,
                               const LiftoffAssembler::VarState& src,
                               uint32_t index, bool is_high_word) {
    ...
    switch (src.loc()) {
      case VarState::kStack:
        LoadI64HalfStackSlot(dst, 2 * index + is_high_word);
        break;
      case VarState::kRegister: {
        LiftoffRegister src_half =
            is_high_word ? src.reg().high() : src.reg().low();
        if (dst != src_half) MoveRegister(dst, src_half, kWasmI32);
        break;
      }
      case VarState::kI32Const:
        int32_t value = is_high_word ? 0 : src.i32_const();
...
void LiftoffAssembler::PrepareCall(wasm::FunctionSig* sig,
  ...
  for (uint32_t i = num_params; i > 0; --i) {
    ...
    for (int pair_idx = num_lowered_params - 1; pair_idx >= 0; --pair_idx) {
      --call_desc_input_idx;
