...
class StackTransferRecipe {
 ...
 public:
    ...
    if ((move_dst_regs_ & move_src_regs_).is_empty()) {
      ...
      while (!register_moves_.empty()) {
        ...
        if (executed_moves == 0) {
          ...
          if (next_spill_slot > max_used_stack_slot_idx_) {
            max_used_stack_slot_idx_ = next_spill_slot;
          }
          ++next_spill_slot;
          executed_moves = 1;
        }
  ...
  uint32_t max_used_stack_slot_idx() const { return max_used_stack_slot_idx_; }
 ...
 private:
  ...
  uint32_t max_used_stack_slot_idx_ = 0;
};
...
void LiftoffAssembler::PrepareCall(wasm::FunctionSig* sig,
  ...
  *max_used_stack_slot_idx = stack_transfers.max_used_stack_slot_idx();
