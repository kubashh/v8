...
class StackTransferRecipe {
  ...
  struct RegisterLoad {
    enum LoadKind : uint8_t { kConstant, kStack, kHalfStack };
    LiftoffRegister dst;
    LoadKind kind;
    ValueType type;
 ...
 public:
  ...
  void LoadI64HalfIntoRegister(LiftoffRegister dst,
                               const LiftoffAssembler::VarState& src,
                               uint32_t index, bool is_snd_half) {
    ...
    switch (src.loc()) {
      case VarState::kStack:
        LoadI64HalfStackSlot(dst, 2 * index + is_snd_half);
        break;
      case VarState::kRegister: {
        LiftoffRegister src_half =
            is_snd_half ? src.reg().snd() : src.reg().fst();
        if (dst != src_half) MoveRegister(dst, src_half, kWasmI32);
        break;
      }
      case VarState::kI32Const:
        int32_t value = is_snd_half ? 0 : src.i32_const();
  ...
  void MoveRegister(LiftoffRegister dst, LiftoffRegister src, ValueType type) {
    ...
    if (src.is_pair()) {
      ...
      if (dst.fst() != src.fst()) MoveRegister(dst.fst(), src.fst(), kWasmI32);
      if (dst.snd() != src.snd()) MoveRegister(dst.snd(), src.snd(), kWasmI32);
      return;
    }
    ...
    for (int pair_idx = 0; pair_idx < num_lowered_params; ++pair_idx) {
      ...
      if (loc.IsRegister()) {
        ...
        if (num_lowered_params == 1) {
          ...
          stack_transfers.LoadI64HalfIntoRegister(reg, slot, stack_idx,
                                                  pair_idx);
        }
      } else {
        DCHECK(loc.IsCallerFrameSlot());
        PushCallerFrameSlot(slot, stack_idx, pair_idx);
      }
    }
  }
...
void LiftoffAssembler::SpillRegister(LiftoffRegister reg) {
  ...
  for (uint32_t idx = cache_state_.stack_height() - 1;; --idx) {
    ...
    if (slot->reg().is_pair()) {
      cache_state_.dec_used(slot->reg().fst());
      cache_state_.dec_used(slot->reg().snd());
    }
    Spill(idx, slot->reg(), slot->type());
    slot->MakeStack();
