 ...
 private:
  ...
  void ExecuteMoves() {
    ...
    while (!move_dst_regs_.is_empty()) {
      ...
      // If we executed at least one move, just run the loop again and see if
      // can move more registers now (after decrementing {src_reg_use_count}).
      if (executed_move) continue;
    ...
    for (LiftoffRegister dst : load_dst_regs_) {
      ...
      switch (load->kind) {
        ...
        case RegisterLoad::kHighHalfStack:
          ...
          auto half =
              load->kind == RegisterLoad::kLowHalfStack ? kLowWord : kHighWord;
          asm_->FillI64Half(dst.gp(), load->value, half);
          break;
      }
    }
    load_dst_regs_ = {};
