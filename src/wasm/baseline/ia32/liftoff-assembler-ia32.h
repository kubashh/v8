...
void EmitSimdCommutativeBinOp(
  ...
  if (feature.has_value()) DCHECK(CpuFeatures::IsSupported(*feature));
  base::Optional<CpuFeatureScope> sse_scope =
      feature.has_value() ? base::make_optional<CpuFeatureScope>(assm, *feature)
                          : base::Optional<CpuFeatureScope>(base::nullopt);
