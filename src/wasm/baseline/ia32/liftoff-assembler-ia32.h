...
void LiftoffAssembler::FillStackSlotsWithZero(uint32_t index, uint32_t count) {
  ...
  if (count <= 2) {
    // Special straight-line code for up to two slots (6-9 bytes per word: C7
    // <1-4 bytes op> <4 bytes imm>, makes 12-18 bytes per slot).
    ...
    // From intel manual: repstosl fills ECX doublewords at [EDI] with EAX.
    push(eax);
    push(ecx);
    push(edi);
