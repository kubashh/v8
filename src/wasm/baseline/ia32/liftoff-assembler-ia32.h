...
void LiftoffStackSlots::Construct() {
  ...
  int last_stack_slot = TotalStackSlots();
  for (auto& slot : slots_) {
    const int stack_slot = slot.stack_slot_;
    int stack_decrement = (last_stack_slot - stack_slot) * kSystemPointerSize;
    if (reversed_) stack_decrement = -stack_decrement;
    DCHECK_LT(0, stack_decrement);
    last_stack_slot = stack_slot;
    const LiftoffAssembler::VarState& src = slot.src_;
    int push_size = NumStackSlots(slot) * kSystemPointerSize;
    switch (src.loc()) {
      case LiftoffAssembler::VarState::kStack:
        ...
        if (src.kind() == kF64) {
          if (stack_decrement > push_size) {
            asm_->AllocateStackSpace(stack_decrement - push_size);
          }
          DCHECK_EQ(kLowWord, slot.half_);
          asm_->push(liftoff::GetHalfStackSlot(slot.src_offset_, kHighWord));
          stack_decrement = push_size;
        }
        if (stack_decrement > push_size) {
          asm_->AllocateStackSpace(stack_decrement - push_size);
        }
      ...
      case LiftoffAssembler::VarState::kRegister:
        if (src.kind() == kI64) {
          liftoff::push(
              ...
              kI32, stack_decrement - push_size);
        } else {
