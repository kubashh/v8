...
void LiftoffAssembler::emit_i8x16_mul(LiftoffRegister dst, LiftoffRegister lhs,
  ...
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    vpsrlw(tmp.fp(), lhs.fp(), static_cast<byte>(8));
    vpsrlw(liftoff::kScratchDoubleReg, rhs.fp(), static_cast<byte>(8));
    // t = I16x8Mul(t0, t1)
    //    => __PP __PP ...  __PP  __PP
    vpmullw(tmp.fp(), tmp.fp(), liftoff::kScratchDoubleReg);
    // s = left * 256
    vpsllw(liftoff::kScratchDoubleReg, lhs.fp(), static_cast<byte>(8));
    ...
    psrlw(tmp.fp(), static_cast<byte>(8));
    psrlw(liftoff::kScratchDoubleReg, static_cast<byte>(8));
    // dst = left * 256
    psllw(dst.fp(), static_cast<byte>(8));
    // t = I16x8Mul(t, s)
    //    => __PP __PP ...  __PP  __PP
    ...
    // t = I16x8Shl(t, 8)
    //    => PP00 PP00 ...  PP00  PP00
    psllw(tmp.fp(), static_cast<byte>(8));
    // dst = I16x8Shr(dst, 8)
    //    => 00pp 00pp ...  00pp  00pp
    psrlw(dst.fp(), static_cast<byte>(8));
    // dst = I16x8Or(dst, t)
    //    => PPpp PPpp ...  PPpp  PPpp
    por(dst.fp(), tmp.fp());
  }
