...
void LiftoffAssembler::LoadTaggedPointer(Register dst, Register src_addr,
                                         Register offset_reg,
                                         int32_t offset_imm, bool as_index) {
  DCHECK_GE(offset_imm, 0);
  if (offset_reg != no_reg) AssertZeroExtended(offset_reg);
  ScaleFactor scale_factor =
      as_index ? COMPRESS_POINTERS_BOOL ? times_4 : times_8 : times_1;
  Operand src_op =
...
namespace {
ScaleFactor LoadTypeScaleFactor(LoadType::LoadTypeValue type) {
  switch (type) {
...
void LiftoffAssembler::Load(LiftoffRegister dst, Register src_addr,
  ...
  ScaleFactor scale_factor =
      as_index ? LoadTypeScaleFactor(type.value()) : times_1;
  Operand src_op =
