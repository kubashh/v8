...
void LiftoffAssembler::emit_i64x2_mul(LiftoffRegister dst, LiftoffRegister lhs,
  ...
  Psllq(tmp2.fp(), static_cast<byte>(32));
  if (dst.fp() != lhs.fp()) Movaps(dst.fp(), lhs.fp());
...
void LiftoffAssembler::emit_i8x16_mul(LiftoffRegister dst, LiftoffRegister lhs,
  ...
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    vpsrlw(kScratchDoubleReg, rhs.fp(), static_cast<byte>(8));
    // t = I16x8Mul(t0, t1)
    //    => __PP __PP ...  __PP  __PP
    vpmullw(tmp.fp(), tmp.fp(), kScratchDoubleReg);
    // s = left * 256
    vpsllw(kScratchDoubleReg, lhs.fp(), static_cast<byte>(8));
    ...
    psrlw(kScratchDoubleReg, static_cast<byte>(8));
    // dst = left * 256
