...
void LiftoffAssembler::emit_i64x2_neg(LiftoffRegister dst,
                                      LiftoffRegister src) {
  if (dst.fp() == src.fp()) {
    Movapd(kScratchDoubleReg, src.fp());
    src.fp() = kScratchDoubleReg;
  }
  Pxor(dst.fp(), dst.fp());
  Psubq(dst.fp(), src.fp());
}
...
void LiftoffAssembler::emit_i16x8_neg(LiftoffRegister dst,
  ...
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    if (dst.fp() == src.fp()) {
      ...
      vpsignw(dst.fp(), dst.fp(), kScratchDoubleReg);
    } else {
      vpxor(dst.fp(), dst.fp(), dst.fp());
      vpsubw(dst.fp(), dst.fp(), src.fp());
    }
  } else {
    if (dst.fp() == src.fp()) {
      pcmpeqd(kScratchDoubleReg, kScratchDoubleReg);
...
void LiftoffAssembler::emit_i8x16_neg(LiftoffRegister dst,
  ...
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    if (dst.fp() == src.fp()) {
      vpcmpeqd(kScratchDoubleReg, kScratchDoubleReg, kScratchDoubleReg);
      vpsignb(dst.fp(), dst.fp(), kScratchDoubleReg);
    } else {
      vpxor(dst.fp(), dst.fp(), dst.fp());
      vpsubb(dst.fp(), dst.fp(), src.fp());
    }
  } else {
    if (dst.fp() == src.fp()) {
      pcmpeqd(kScratchDoubleReg, kScratchDoubleReg);
