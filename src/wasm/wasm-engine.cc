  ...
  if (v8_flags.wasm_test_streaming) {
    ...
    for (int round = 0; round < 4; ++round) {
      for (auto it = ranges.begin(); it != ranges.end(); ++it) {
        auto bytes = *it;
        if (bytes.size() < 2 || !rng->NextBool()) continue;  // Do not split.
        // Choose split point within [1, bytes.size() - 1).
        size_t split_point =
            1 + static_cast<size_t>((bytes.size() - 1) * rng->NextDouble());
        // Insert first sub-range *before* {it} and update it to point after it.
        it = ranges.insert(it, bytes.SubVector(0, split_point)) + 1;
        *it = bytes.SubVectorFrom(split_point);
      }
    }
    for (auto bytes : ranges) {
      streaming_decoder->OnBytesReceived(bytes);
    }
