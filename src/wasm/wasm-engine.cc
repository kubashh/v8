...
struct WasmEngine::IsolateInfo {
  ...
  std::unordered_map<NativeModule*, std::unique_ptr<Address*>> weak_scripts;
...
void WasmEngine::FreeNativeModule(NativeModule* native_module) {
  ...
  for (Isolate* isolate : it->second->isolates) {
    ...
    info->weak_scripts.erase(native_module);
...
Handle<Script> WasmEngine::GetOrCreateScript(Isolate* isolate,
  ...
  base::MutexGuard guard(&mutex_);
  DCHECK_EQ(1, isolates_.count(isolate));
  auto& scripts = isolates_[isolate]->weak_scripts;
  auto it = scripts.find(native_module);
  if (it != scripts.end()) {
    Handle<Script> weak_script_handle{*it->second};
    if (weak_script_handle.is_null()) {
      scripts.erase(it);
    } else {
      return weak_script_handle;
    }
  }
  auto script = CreateWasmScript(isolate, native_module->wire_bytes(),
                                 source_map_url, name, source_url);
  std::unique_ptr<Address*> global_handle_location = std::make_unique<Address*>(
      isolate->global_handles()->Create(*script).location());
  GlobalHandles::MakeWeak(global_handle_location.get());
  scripts.emplace(native_module, std::move(global_handle_location));
  return script;
}
