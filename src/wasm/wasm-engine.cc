...
class WeakScriptHandle {
 public:
  ...
  Address* location() { return *location_; }
...
Handle<Script> WasmEngine::GetOrCreateScript(Isolate* isolate,
  ...
  if (it != scripts.end()) {
    if (it->second.location() == nullptr) {
      ...
      auto script = Script::unchecked_cast(Object(*it->second.location()));
      return Handle<Script>::New(script, isolate);
    }
  }
  auto script = CreateWasmScript(isolate, native_module->wire_bytes(),
                                 source_map_url, name, source_url);
  scripts.emplace(native_module, script);
  return script;
}
