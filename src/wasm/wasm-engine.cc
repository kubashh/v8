...
class WeakScriptHandle {
 public:
  explicit WeakScriptHandle(Handle<Script> handle)
      : handle_(std::make_unique<Handle<Script>>(handle)) {
    GlobalHandles::MakeWeak(handle_->location_address());
  }

  WeakScriptHandle(WeakScriptHandle&& other) V8_NOEXCEPT = default;

  ~WeakScriptHandle() { GlobalHandles::Destroy(handle_->location()); }

  Handle<Script>* handle() { return handle_.get(); }

 private:
  // Store the handle in a unique_ptr so that the location address stays the
  // same even when this object is moved/copied.
  std::unique_ptr<Handle<Script>> handle_;
};
...
Handle<Script> WasmEngine::GetOrCreateScript(Isolate* isolate,
  ...
  if (it != scripts.end()) {
    if (it->second.handle()->is_null()) {
      scripts.erase(it);
    } else {
      return Handle<Script>::New(**it->second.handle(), isolate);
    }
  }
  ...
  WeakScriptHandle weak_script_handle(global_handle);
  scripts.emplace(native_module, std::move(weak_script_handle));
  return script;
}
