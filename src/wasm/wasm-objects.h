...
class WasmDispatchTable : public TrustedObject {
 public:
  ...
  static constexpr int kPaddingBytes = kTaggedSize == 4 ? 4 : 0;
  static constexpr int kPaddingOffset = kHeaderSize;
  static constexpr size_t kLengthOffset = kHeaderSize + kPaddingBytes;
  ...
  // Check alignment.
  static_assert(kEntriesOffset == 16);
  static_assert(IsAligned(kEntriesOffset, kSystemPointerSize));
  static_assert(IsAligned(kTargetBias, kSystemPointerSize));
