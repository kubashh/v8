...
#define RETURN_NOTHING_IF_FALSE(call) \
 ...
 public:
  ...
  static Maybe<bool> CollectValuesOrEntriesImpl(
    ...
    KeyAccumulator accumulator(isolate, KeyCollectionMode::kOwnOnly,
                               ALL_PROPERTIES);
    RETURN_NOTHING_IF_FALSE(Subclass::CollectElementIndicesImpl(
  ...
  bool CollectElementIndices(Handle<JSObject> object,
                             Handle<FixedArrayBase> backing_store,
                             KeyAccumulator* keys) final {
    if (keys->filter() & ONLY_ALL_CAN_READ) return true;
    return Subclass::CollectElementIndicesImpl(object, backing_store, keys);
  }

  V8_WARN_UNUSED_RESULT static bool CollectElementIndicesImpl(
    ...
    for (uint32_t i = 0; i < length; i++) {
      if (Subclass::HasElementImpl(isolate, *object, i, *backing_store,
                                   filter)) {
        if (!keys->AddKey(factory->NewNumberFromUint(i))) {
          return false;
        }
      }
    }
  ...
  V8_WARN_UNUSED_RESULT bool AddElementsToKeyAccumulator(
...
class DictionaryElementsAccessor
 ...
 public:
  ...
  V8_WARN_UNUSED_RESULT static bool CollectElementIndicesImpl(
      Handle<JSObject> object, Handle<FixedArrayBase> backing_store,
      KeyAccumulator* keys) {
    if (keys->filter() & SKIP_STRINGS) return true;
    Isolate* isolate = keys->isolate();
    Handle<NumberDictionary> dictionary =
        Handle<NumberDictionary>::cast(backing_store);
    ...
    for (int i = 0; i < insertion_index; i++) {
      if (!keys->AddKey(elements->get(i))) return false;
    }
    return true;
  }
  ...
  V8_WARN_UNUSED_RESULT static bool AddElementsToKeyAccumulatorImpl(
      Handle<JSObject> receiver, KeyAccumulator* accumulator,
    ...
    for (int i = 0; i < capacity; i++) {
      ...
      if (!accumulator->AddKey(value, convert)) {
        return false;
      }
    }
    return true;
  ...
  V8_WARN_UNUSED_RESULT static bool AddElementsToKeyAccumulatorImpl(
    ...
    for (uint32_t i = 0; i < length; i++) {
      if (IsFastPackedElementsKind(KindTraits::Kind) ||
        ...
        if (!accumulator->AddKey(Subclass::GetImpl(isolate, *elements, i),
                                 convert)) {
          return false;
        }
      }
 ...
 public:
  ...
  V8_WARN_UNUSED_RESULT static bool AddElementsToKeyAccumulatorImpl(
    ...
    for (uint32_t i = 0; i < length; i++) {
      ...
      if (!accumulator->AddKey(value, convert)) return false;
    }
    return true;
  }
...
class SloppyArgumentsElementsAccessor
 ...
 public:
  ...
  V8_WARN_UNUSED_RESULT static bool CollectElementIndicesImpl(
    ...
    for (uint32_t i = 0; i < nof_indices; i++) {
      if (!keys->AddKey(indices->get(i))) return false;
    }
    return true;
  }
...
class StringWrapperElementsAccessor
 ...
 public:
  ...
  V8_WARN_UNUSED_RESULT static bool CollectElementIndicesImpl(
    ...
    for (uint32_t i = 0; i < length; i++) {
      if (!keys->AddKey(factory->NewNumberFromUint(i))) return false;
    }
