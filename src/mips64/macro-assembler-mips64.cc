...
void MacroAssembler::Uld(Register rd, const MemOperand& rs) {
  ...
  if (kArchVariant == kMips64r6) {
    ...
    MemOperand source = rs;
    // To simplify we set is_doubleword to true here to check if offset + 4 fits
    // into int16_t. We need to check if offset + 7 fits into int16_t so we do
    // that after calling AdjustBaseAndOffset and emit daddiu if needed.
    AdjustBaseAndOffset(source, /* is_doubleword */ true);
    if (!is_int16(source.offset() + kMipsLdrOffset) ||
        !is_int16(source.offset() + kMipsLdlOffset)) {
...
void MacroAssembler::Usd(Register rd, const MemOperand& rs) {
  ...
  if (kArchVariant == kMips64r6) {
    ...
    // that after calling AdjustBaseAndOffset and emit daddiu if needed.
    AdjustBaseAndOffset(source, /* is_doubleword */ true);
    if (!is_int16(source.offset() + kMipsSdrOffset) ||
        !is_int16(source.offset() + kMipsSdlOffset)) {
      daddiu(at, source.rm(), source.offset());
      source = MemOperand(at, 0);
    }
...
void MacroAssembler::Lwc1(FPURegister fd, const MemOperand& src) {
  MemOperand tmp = src;
  AdjustBaseAndOffset(tmp, /* is_doubleword */ false);
  lwc1(fd, tmp);
}
...
void MacroAssembler::Ldc1(FPURegister fd, const MemOperand& src) {
  MemOperand tmp = src;
  AdjustBaseAndOffset(tmp, /* is_doubleword */ true);
  ldc1(fd, tmp);
}
