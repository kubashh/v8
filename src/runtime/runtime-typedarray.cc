...
RUNTIME_FUNCTION(Runtime_TypedArrayCopyElements) {
  ...
  if (source->IsJSObject()) {
    Handle<JSObject> source_jsobject = Handle<JSObject>::cast(source);
    ElementsAccessor* source_accessor = source_jsobject->GetElementsAccessor();

    for (uint32_t i = 0; i < length; i++) {
      Handle<Object> element = source_accessor->Get(source_jsobject, i);
      holder_accessor->Set(holder, i, *element);
    }
  } else {
    DCHECK(source->IsJSProxy());

    for (uint32_t i = 0; i < length; i++) {
      Handle<Object> element;
      ASSIGN_RETURN_FAILURE_ON_EXCEPTION(
          isolate, element, JSReceiver::GetElement(isolate, source, i));
