...
RUNTIME_FUNCTION(Runtime_GetProperty) {
  ...
  CONVERT_ARG_HANDLE_CHECKED(Object, lookup_start_object, 0);
  CONVERT_ARG_HANDLE_CHECKED(Object, key_obj, 1);
  Handle<Object> receiver_obj = lookup_start_object;
  if (args.length() == 3) {
    CHECK(args[2].IsObject());
    receiver_obj = args.at<Object>(2);
  }

  // Fast cases for getting named properties of the lookup_start_object JSObject
  // itself.
  //
  ...
  if (lookup_start_object->IsJSObject()) {
    if (!lookup_start_object->IsJSGlobalProxy() &&
        !lookup_start_object->IsAccessCheckNeeded() && key_obj->IsName()) {
      Handle<JSObject> js_object = Handle<JSObject>::cast(lookup_start_object);
      Handle<Name> key = Handle<Name>::cast(key_obj);
      key_obj = key = isolate->factory()->InternalizeName(key);

      DisallowGarbageCollection no_gc;
      if (js_object->IsJSGlobalObject()) {
        // Attempt dictionary lookup.
        GlobalDictionary dictionary =
            JSGlobalObject::cast(*js_object).global_dictionary(kAcquireLoad);
      ...
      } else if (!js_object->HasFastProperties()) {
        // Attempt dictionary lookup.
        if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
          SwissNameDictionary dictionary =
              js_object->property_dictionary_swiss();
          InternalIndex entry = dictionary.FindEntry(isolate, *key);
          if (entry.is_found() &&
          ...
          NameDictionary dictionary = js_object->property_dictionary();
          InternalIndex entry = dictionary.FindEntry(isolate, key);
          if ((entry.is_found()) &&
              (dictionary.DetailsAt(entry).kind() == kData)) {
            return dictionary.ValueAt(entry);
  ...
  } else if (lookup_start_object->IsString() && key_obj->IsSmi()) {
    // Fast case for string indexing using [] with a smi index.
    Handle<String> str = Handle<String>::cast(lookup_start_object);
  ...
  RETURN_RESULT_OR_FAILURE(
      isolate, Runtime::GetObjectProperty(isolate, lookup_start_object, key_obj,
                                          receiver_obj));
}
