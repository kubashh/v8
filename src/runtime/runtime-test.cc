...
#define ABORT_INVALID_USE(isolate)                   \
  do {                                               \
    CHECK(false);                                    \
    return ReadOnlyRoots(isolate).undefined_value(); \
  } while (false);
...
RUNTIME_FUNCTION(Runtime_DeoptimizeFunction) {
  ...
  if (!function_object->IsJSFunction()) ABORT_INVALID_USE(isolate);
...
RUNTIME_FUNCTION(Runtime_DeoptimizeNow) {
  ...
  if (function.is_null()) ABORT_INVALID_USE(isolate);
...
RUNTIME_FUNCTION(Runtime_OptimizeFunctionOnNextCall) {
  ...
  if (args.length() != 1 && args.length() != 2) ABORT_INVALID_USE(isolate);
  ...
  if (function->shared().HasAsmWasmData()) {
    // XXX inspector/debugger/asm-js-breakpoint-*-exec
    return ReadOnlyRoots(isolate).undefined_value();
  }
  ...
  if (args.length() == 2) {
    ...
    if (!type->IsString()) ABORT_INVALID_USE(isolate);
...
RUNTIME_FUNCTION(Runtime_EnsureFeedbackVectorForFunction) {
  HandleScope scope(isolate);
  DCHECK_EQ(1, args.length());
  if (!args[0].IsJSFunction()) ABORT_INVALID_USE(isolate);
...
RUNTIME_FUNCTION(Runtime_PrepareFunctionForOptimization) {
  ...
  if (args.length() == 2) {
    ...
    if (!sync_object->IsString()) ABORT_INVALID_USE(isolate);
  ...
  if (function->shared().HasAsmWasmData()) ABORT_INVALID_USE(isolate);
...
RUNTIME_FUNCTION(Runtime_GetWasmExceptionId) {
  ...
  if (!tag->IsWasmExceptionTag()) ABORT_INVALID_USE(isolate);
