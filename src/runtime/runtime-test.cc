...
RUNTIME_FUNCTION(Runtime_OptimizeOsr) {
  ...
  if (FLAG_trace_osr) {
    ...
    PrintF(scope.file(), " for %s optimization]\n",
           concurrency_mode == ConcurrencyMode::kConcurrent ? "concurrent"
                                                            : "non-concurrent");
    if ((concurrency_mode == ConcurrencyMode::kConcurrent) !=
        isolate->concurrent_osr_enabled()) {
      PrintF(scope.file(),
             "Warning: concurrency mode mismatch. Concurrent osr is %s\n",
             isolate->concurrent_osr_enabled() ? "enabled" : "not enabled");
    }
  }
