...
V8_WARN_UNUSED_RESULT Object InvalidUse(Isolate* isolate) {
  CHECK(FLAG_fuzzing);
...
RUNTIME_FUNCTION(Runtime_DeoptimizeFunction) {
  ...
  if (!function_object->IsJSFunction()) return InvalidUse(isolate);
...
RUNTIME_FUNCTION(Runtime_DeoptimizeNow) {
  ...
  if (function.is_null()) return InvalidUse(isolate);
...
RUNTIME_FUNCTION(Runtime_OptimizeFunctionOnNextCall) {
  ...
  if (args.length() != 1 && args.length() != 2) return InvalidUse(isolate);
  ...
  if (function->shared().HasAsmWasmData()) return InvalidUse(isolate);
  ...
  if (args.length() == 2) {
    ...
    if (!type->IsString()) return InvalidUse(isolate);
...
RUNTIME_FUNCTION(Runtime_EnsureFeedbackVectorForFunction) {
  HandleScope scope(isolate);
  DCHECK_EQ(1, args.length());
  if (!args[0].IsJSFunction()) return InvalidUse(isolate);
...
RUNTIME_FUNCTION(Runtime_PrepareFunctionForOptimization) {
  ...
  if (args.length() == 2) {
    ...
    if (!sync_object->IsString()) return InvalidUse(isolate);
...
RUNTIME_FUNCTION(Runtime_GetWasmExceptionId) {
  ...
  if (!tag->IsWasmExceptionTag()) return InvalidUse(isolate);
