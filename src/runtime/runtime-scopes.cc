...
Object DeclareEvalHelper(Isolate* isolate, Handle<String> name,
  ...
  Handle<Object> holder =
      Context::Lookup(context, name, DONT_FOLLOW_CHAINS, &index, &attributes,
                      &init_flag, &mode);
  DCHECK(holder.is_null() || !holder->IsJSModule());
...
RUNTIME_FUNCTION(Runtime_DeleteLookupSlot) {
  ...
  if (holder->IsContext() || holder->IsJSModule()) {
...
MaybeHandle<Object> LoadLookupSlot(Isolate* isolate, Handle<String> name,
  ...
  if (!holder.is_null() && holder->IsJSModule()) {
    Handle<Object> receiver = isolate->factory()->undefined_value();
    if (receiver_return) *receiver_return = receiver;
    return JSModule::LoadVariable(isolate, Handle<JSModule>::cast(holder),
                                  index);
  }
  if (index != Context::kNotFound) {
...
MaybeHandle<Object> StoreLookupSlot(
  ...
  if (holder.is_null()) {
    // In case of JSProxy, an exception might have been thrown.
    if (isolate->has_pending_exception()) return MaybeHandle<Object>();
  } else if (holder->IsJSModule()) {
    if ((attributes & READ_ONLY) == 0) {
      JSModule::StoreVariable(Handle<JSModule>::cast(holder), index, value);
    } else {
      THROW_NEW_ERROR(
