...
Maybe<bool> ConditionalCopy(Handle<JSReceiver> source,
  ...
  if (target_has_prop.FromJust() || !source_has_prop.FromJust()) {
    return Just(false);
  }
...
// to object itself, if obj has holes. Return one more than the maximal
  ...
  for (PrototypeIterator iter(isolate, object, kStartAtPrototype);
    ...
    if (current->IsJSProxy()) {
      for (uint32_t i = 0; i < length; ++i) {
        Maybe<bool> did_copy = ConditionalCopy(current, object, i);
      ...
      for (uint32_t i = 0; i < num_indices; ++i) {
        ...
        Maybe<bool> did_copy = ConditionalCopy(current, object, idx);
        MAYBE_RETURN(did_copy, isolate->heap()->exception());
