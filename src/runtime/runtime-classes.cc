...
void AddDescriptorsByTemplate(
  ...
  while (i < computed_properties_length) {
    ...
    int key_index, enumeration_index;
    if (ComputedEntryFlags::IsShortEncodingBit::decode(flags)) {
      key_index = ComputedEntryFlags::ShortNameIndexBits::decode(flags);
      enumeration_index =
          ComputedEntryFlags::ShortEnumerationOrderValueBits::decode(flags);
    } else {
      key_index = ComputedEntryFlags::FullNameIndexBits::decode(flags);
      enumeration_index = Smi::ToInt(computed_properties->get(i++));
    }
