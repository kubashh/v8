...
Object* GetValue(Isolate* isolate, Object* key,
                 ClassBoilerplate::ValueKind value_kind, Arguments& args,
...
void AddDescriptorsByTemplate(
  ...
  for (int i = 0; i < nof_computed_properties_length;
    ...
    DCHECK(!ComputedEntryFlags::IsShortEncodingBit::decode(flags));
    ValueKind value_kind = ComputedEntryFlags::ValueKindBits::decode(flags);
    int key_index = ComputedEntryFlags::FullNameIndexBits::decode(flags);
    ...
    if (name->AsArrayIndex(&element)) {
      ...
      int enumeration_index = Smi::ToInt(computed_properties->get(
          i + ClassBoilerplate::kComputedEnumerationOrderValueIndex));
      name = isolate->factory()->InternalizeName(name);
      ClassBoilerplate::AddToPropertiesTemplate(isolate, properties_dictionary,
                                                name, key_index, value_kind,
                                                value, enumeration_index);
    }
  }
  ...
  if (install_name_accessor) {
    ...
    Handle<NameDictionary> dict = NameDictionary::Add(
        properties_dictionary, name, name_accessor, details);
