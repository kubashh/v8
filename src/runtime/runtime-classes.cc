...
// FIXME: This is a workaround to fix the Handle vs MaybeHandle difference in
// the return types of the different Add functions: OrderedNameDictionary::Add
// returns MaybeHandle, NameDictionary::Add returns Handle.  Finally change
// those types?
template <typename T>
...
template <typename PropertyDict>
bool AddDescriptorsByTemplate(
    Isolate* isolate, Handle<Map> map,
    Handle<PropertyDict> properties_dictionary_template,
    Handle<NumberDictionary> elements_dictionary_template,
  ...
  Handle<PropertyDict> properties_dictionary =
  ...
  // Replace all indices with proper methods.
  if (!SubstituteValues<PropertyDict>(isolate, properties_dictionary, receiver,
  ...
  if (install_name_accessor) {
    ...
    Handle<PropertyDict> dict = ToHandle(PropertyDict::Add(
