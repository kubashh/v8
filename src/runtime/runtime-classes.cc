...
Object* GetValueNoSetName(Isolate* isolate, Object* key, Arguments& args,
...
MaybeHandle<Object> GetValueSetName(Isolate* isolate, Handle<Object> key,
                                    Arguments& args, Smi* index,
  ...
  if (!method->shared()->has_shared_name()) {
    Handle<Name> name;
    if (key->IsSmi()) {
      name = isolate->factory()->NumberToString(key);
    } else {
...
MaybeHandle<Object> SubstituteValues(Isolate* isolate,
    ...
    if (value->IsAccessorPair()) {
      ...
      if (tmp->IsSmi()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(
            isolate, result,
            GetValueSetName(isolate, key, args, Smi::cast(tmp), receiver,
      ...
      ASSIGN_RETURN_ON_EXCEPTION(
          isolate, result,
          GetValueSetName(isolate, key, args, Smi::cast(*value), receiver,
  ...
  for (int i = 0; i < nof_descriptors; i++) {
    ...
    if (details.location() == kDescriptor) {
      if (details.kind() == kData) {
        if (value->IsSmi()) {
          value = GetValueNoSetName(isolate, name, args, value, *receiver);
        }
      } else {
        ...
        if (value->IsAccessorPair()) {
          ...
          if (tmp->IsSmi()) {
            pair->set_getter(
                GetValueNoSetName(isolate, name, args, tmp, *receiver));
          }
          tmp = pair->setter();
          if (tmp->IsSmi()) {
            pair->set_setter(
...
bool AddDescriptorsByTemplate(
  ...
  // Replace all indices with proper methods.
  if (SubstituteValues<NameDictionary>(isolate, properties_dictionary, receiver,
