...
void DeoptAllOsrLoopsContainingDeoptExit(Isolate* isolate, JSFunction function,
  ...
  bool is_deopt_exit_in_loop_range = false;
  // Visit from deopt exit to deoptimize type b and type c osr'd loops
  for (it.SetOffset(deopt_exit_offset.ToInt()); !it.done(); it.Advance()) {
    // We're only interested in loop ranges.
    if (it.current_bytecode() != interpreter::Bytecode::kJumpLoop) continue;
    // Is the deopt exit contained in the current loop?
    if (!is_deopt_exit_in_loop_range &&
        base::IsInRange(deopt_exit_offset.ToInt(), it.GetJumpTargetOffset(),
                        it.current_offset())) {
      is_deopt_exit_in_loop_range = true;
  ...
  if (it.done() || !is_deopt_exit_in_loop_range) return;
  // Revisit start of outermost loop to deoptimize type a osr'd loops
