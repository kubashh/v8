...
RUNTIME_FUNCTION(Runtime_FunctionFirstExecution) {
  ...
  DCHECK_EQ(function->feedback_vector()->optimization_marker(),
            OptimizationMarker::kNotYetExecuted);
  if (FLAG_log_function_events) {
    Handle<SharedFunctionInfo> sfi(function->shared());
    LOG(isolate, FunctionEvent("first-execution", Script::cast(sfi->script()),
                               -1, 0, sfi->start_position(),
                               sfi->end_position(), sfi->DebugName()));
  }
  function->feedback_vector()->SetOptimizationMarker(OptimizationMarker::kNone);
