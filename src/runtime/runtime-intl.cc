...
static std::vector<const char*> case_first_values = {"upper", "lower", "false"};
...
MaybeHandle<String> SetOptions(Isolate* isolate, Handle<JSReceiver> in_options,
                               Handle<JSReceiver> extension_map,
                               Handle<String> key, Handle<JSObject> out_options,
  ...
  for (const auto& option_to_bcp47 : kOptionToUnicodeTagMap) {
    if (strcmp(option_to_bcp47.key, key_str.get()) == 0) {
      ...
      if (is_boolean) {
        ...
        MAYBE_RETURN(
            maybe_get_option,
            Handle<String>(ReadOnlyRoots(isolate).empty_string(), isolate));
        if (maybe_get_option.FromJust()) {
          if (value_bool) {
            value =
                Handle<Object>(ReadOnlyRoots(isolate).true_string(), isolate);
          } else {
            value =
                Handle<Object>(ReadOnlyRoots(isolate).false_string(), isolate);
          }
        }
      } else {
        ...
        if (maybe_get_option.IsNothing()) {
          Handle<String> property_str =
              factory->NewStringFromAsciiChecked(option_to_bcp47.name);
          Handle<Object> out_of_range_value;
          ASSIGN_RETURN_ON_EXCEPTION(
              isolate, out_of_range_value,
              Object::GetPropertyOrElement(isolate, in_options, property_str),
              String);
          THROW_NEW_ERROR(
              isolate,
              NewRangeError(MessageTemplate::kValueOutOfRange,
                            out_of_range_value, caller, property_str),
              String);
        }
        if (maybe_get_option.FromJust()) {
...
RUNTIME_FUNCTION(Runtime_SetOptions) {
  ...
  RETURN_RESULT_OR_FAILURE(
      isolate,
      SetOptions(isolate, in_options, extension_map, key, out_options, caller));
}
