...
class V8_EXPORT_PRIVATE ControlEquivalence final
 ...
 private:
  NodeData* GetData(Node* node) {
    ...
    // Node data should be large enough for all nodes, and we never insert new
    // nodes during control equivalence.
    DCHECK(index < node_data_.size());
  ...
  size_t GetClass(Node* node) {
    if (!Participates(node)) return kInvalidClass;
