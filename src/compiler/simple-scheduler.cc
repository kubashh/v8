...
void SimpleScheduler::ComputeDominatorsLevel() {
  ...
  while (!queue.empty()) {
    ...
    int depth = GetDominatorLevel(node);
    queue.pop();
...
Node* SimpleScheduler::GetImmediateDominator(Node* node) {
  ...
  if (NodeProperties::IsPhi(node)) {
    ...
    for (int i = 0; i < node->InputCount(); ++i) {
      ...
      int depth = GetDominatorLevel(dominator);
...
bool SimpleScheduler::SameBasicBlock(Node* node_a, Node* node_b) {
