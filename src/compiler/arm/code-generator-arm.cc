...
void CheckOutputOfAtomicPairInstr(ArmOperandConverter* converter,
                                  const Instruction* instr, Register low,
                                  Register high) {
  if (instr->OutputCount() > 0) {
    DCHECK_EQ(converter->OutputRegister(0), low);
    if (instr->OutputCount() == 2) {
      DCHECK_EQ(converter->OutputRegister(1), high);
    }
  }
}
...
#undef ATOMIC_BINOP_CASE
    case kArmWord32AtomicPairLoad: {
      CheckOutputOfAtomicPairInstr(&i, instr, r0, r1);
...
#define ATOMIC_ARITH_BINOP_CASE(op, instr1, instr2)  \
  case kArmWord32AtomicPair##op: {                   \
    CheckOutputOfAtomicPairInstr(&i, instr, r2, r3); \
...
#undef ATOMIC_LOGIC_BINOP_CASE
    case kArmWord32AtomicPairExchange: {
      CheckOutputOfAtomicPairInstr(&i, instr, r6, r7);
      Label exchange;
      __ add(i.TempRegister(0), i.InputRegister(2), i.InputRegister(3));
      __ dmb(ISH);
      __ bind(&exchange);
