...
InstructionSelector::InstructionSelector(
    ...
    PoisoningMitigationLevel poisoning_level, bool record_origins)
    : zone_(zone),
      linkage_(linkage),
      sequence_(sequence),
      ...
      record_origins_(record_origins) {
  instructions_.reserve(node_count);
  continuation_inputs_.reserve(5);
  continuation_outputs_.reserve(2);

  if (record_origins_) {
    instr_origins_.assign(node_count, std::pair<int, int>(-1, 0));
  }
}
...
void InstructionSelector::VisitBlock(BasicBlock* block) {
  ...
  for (auto node : base::Reversed(*block)) {
    ...
    if (record_origins_) {
      int current_node_start = static_cast<int>(instructions_.size());
      instr_origins_[node->id()] =
          std::pair<int, int>(current_node_start, current_node_end);
    }
  }
...
void InstructionSelector::VisitControl(BasicBlock* block) {
  ...
  if (record_origins_ && input) {
    int instruction_start = static_cast<int>(instructions_.size());
    instr_origins_[input->id()] =
        std::pair<int, int>(instruction_start, instruction_end);
  }
}
