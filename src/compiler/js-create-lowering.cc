...
Reduction JSCreateLowering::ReduceNewArrayToStubCall(
  ...
  Node* type_info =
      site ? jsgraph()->Constant(site.value()) : jsgraph()->UndefinedConstant();
  ...
  if (new_target_type.IsHeapConstant() &&
    ...
    if (IsAllocationInlineable(constructor, original_constructor)) {
      ...
      if (site) {
        ...
        pretenure = dependencies()->DependOnPretenureMode(site.value());
        dependencies()->DependOnElementsKind(site.value());
      } else {
