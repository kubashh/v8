...
void DecompressionOptimizer::MaybeMarkAndQueueForRevisit(Node* const node,
  ...
  if (previous_state == State::kUnvisited ||
    ...
    // bits, we save it in compressed_nodes_ to be changed later if necessary.
    if (state == State::kOnly32BitsObserved && CanBeCompressed(node)) {
      compressed_nodes_.push_back(node);
    }
  }
}
...
void DecompressionOptimizer::ChangeNodes() {
  for (Node* const node : compressed_nodes_) {
    // compressed_nodes_ contains all the nodes that once had the
    ...
    // costly than removing them from the compressed_nodes_ NodeVector when we
