...
bool CanBeCompressedForChangeNode(Node* const node) {
  return IsCompressedHeapConstant(node) || IsCompressedMachineLoad(node) ||
         IsCompressedPhi(node) || CanBeCompressed(node);
}
...
void DecompressionOptimizer::RemoveChangeTaggedToCompressed(Node* const node) {
  ...
  if (IsOnly32BitsObserved(input) && CanBeCompressedForChangeNode(input)) {
