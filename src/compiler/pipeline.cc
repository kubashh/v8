...
struct WasmTypingPhase {
  ...
  void Run(PipelineData* data, Zone* temp_zone, uint32_t function_index) {
    GraphReducer graph_reducer(
        temp_zone, data->graph(), &data->info()->tick_counter(), data->broker(),
        data->jsgraph()->Dead(), data->observe_node_manager());
    // We need this because Int64Lowering produces additions of constants as
    // offsets for struct loads, which we do not want to handle.
    MachineOperatorReducer machine(&graph_reducer, data->mcgraph(), false);
    WasmTyper typer(&graph_reducer, data->mcgraph(), function_index);
    AddReducer(data, &graph_reducer, &machine);
