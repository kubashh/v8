...
class WasmHeapStubCompilationJob final : public TurbofanCompilationJob {
 ...
 protected:
  CompilationJob::Status PrepareJobImpl(Isolate* isolate) final;
  CompilationJob::Status ExecuteJobImpl(RuntimeCallStats* stats,
                                        LocalIsolate* local_isolate) final;
  CompilationJob::Status FinalizeJobImpl(Isolate* isolate) final;
...
class WasmTurboshaftHeapStubCompilationJob final
    : public turboshaft::TurboshaftCompilationJob {
 public:
  WasmTurboshaftHeapStubCompilationJob(
      Isolate* isolate, const wasm::FunctionSig* sig, bool is_import,
      const wasm::WasmModule* module, CallDescriptor* call_descriptor,
      std::unique_ptr<Zone> zone, MachineGraph* mcgraph, CodeKind kind,
      ...
      : TurboshaftCompilationJob(&info_,
        ...
        debug_name_(std::move(debug_name)),
        info_(base::CStrVector(debug_name_.get()), zone.get(), kind),
        sig_(sig),
        ...
        mcgraph_(mcgraph),
        data_(&zone_stats_, &info_, isolate, wasm::GetWasmEngine()->allocator(),
              mcgraph->graph(), nullptr, nullptr, nullptr, nullptr, nullptr,
              options, nullptr),
        pipeline_(&data_) {}

  WasmTurboshaftHeapStubCompilationJob(
      const WasmTurboshaftHeapStubCompilationJob&) = delete;
  WasmTurboshaftHeapStubCompilationJob& operator=(
      const WasmTurboshaftHeapStubCompilationJob&) = delete;
...
Pipeline::NewWasmTurboshaftHeapStubCompilationJob(
    Isolate* isolate, const wasm::FunctionSig* sig, bool is_import,
    const wasm::WasmModule* module, CallDescriptor* call_descriptor,
    std::unique_ptr<Zone> zone, MachineGraph* mcgraph, CodeKind kind,
    std::unique_ptr<char[]> debug_name, const AssemblerOptions& options) {
  return std::make_unique<WasmTurboshaftHeapStubCompilationJob>(
      isolate, sig, is_import, module, call_descriptor, std::move(zone),
      mcgraph, kind, std::move(debug_name), options);
}
...
void TraceWrapperCompilation(const char* compiler,
  ...
  if (info->trace_turbo_graph() &&
      data->graph() != nullptr) {  // Simple textual RPO.
...
CompilationJob::Status WasmTurboshaftHeapStubCompilationJob::PrepareJobImpl(
    Isolate* isolate) {
  UNREACHABLE();
}

CompilationJob::Status WasmTurboshaftHeapStubCompilationJob::ExecuteJobImpl(
    RuntimeCallStats* stats, LocalIsolate* local_isolate) {
  std::unique_ptr<TurbofanPipelineStatistics> pipeline_statistics;
  if (v8_flags.turbo_stats || v8_flags.turbo_stats_nvp) {
    pipeline_statistics.reset(new TurbofanPipelineStatistics(
  ...
  AccountingAllocator allocator;
  BuildWasmHeapStub(&allocator, turboshaft_pipeline.Value().graph(),
                    info_.code_kind(), sig_, is_import_, module_);
  CodeTracer* code_tracer = nullptr;
  if (turboshaft::PipelineData::Get().info()->trace_turbo_graph()) {
    // NOTE: We must not call `GetCodeTracer` if tracing is not enabled,
    // because it may not yet be initialized then and doing so from the
...
CompilationJob::Status WasmTurboshaftHeapStubCompilationJob::FinalizeJobImpl(
    Isolate* isolate) {
  return FinalizeWrapperCompilation(
      &pipeline_, &info_, call_descriptor_, isolate,
      "WasmTurboshaftHeapStubCompilationJob::FinalizeJobImpl");
}
