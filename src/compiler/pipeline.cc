...
struct OptimizeTurboshaftPhase {
  ...
  void Run(PipelineData* data, Zone* temp_zone) {
    ...
    turboshaft::OptimizationPhase<turboshaft::LivenessAnalyzer,
        ...
        phase(std::move(data->turboshaft_graph()));
    data->turboshaft_graph() = phase.Run();
    // }
  }
};
  ...
  void Run(PipelineData* data, Zone* temp_zone, Linkage* linkage) {
    if (FLAG_turboshaft) {
      ...
      bool success = turboshaft::SelectInstructions(
          temp_zone, data->turboshaft_graph().op_count(), linkage,
          data->sequence(), data->turboshaft_graph(), data->source_positions(),
