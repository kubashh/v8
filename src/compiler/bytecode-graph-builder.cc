...
class BytecodeGraphBuilder::OsrIteratorState {
 public:
  ...
  void ProcessOsrPrelude() {
    ...
    BytecodeAnalysis::OsrEntryPoint osr_entry =
    ...
    while (outermost_loop_offset != -1) {
      ...
      for (uint i = current_loop_level; i < loop_info_vector.size(); ++i) {
...
void BytecodeGraphBuilder::AdvanceToOsrEntryAndPeelLoops() {
  ...
  int parent_level = current_parent_offset == osr_entry.bytecode_offset
                         ...
                         : 0;
  // searching_level is used to search for the correct OSR loop at the
  // beginning.
  int searching_level = parent_level;
...
void BytecodeGraphBuilder::BuildLoopExitsForBranch(int target_offset) {
  ...
  if (target_offset > origin_offset) {
    BuildLoopExitsUntilLoop(
        bytecode_analysis().GetLoopOffsetFor(target_offset).first,
...
void BytecodeGraphBuilder::BuildLoopExitsUntilLoop(
  ...
  int current_loop_offset = loop_pair.first;
  int current_loop_level = loop_pair.second;
  ...
  while (loop_offset < current_loop_offset) {
    Node* loop_node =
        merge_environments_[current_loop_offset]->GetControlDependency();
    const ZoneVector<LoopInfo>& loop_info_vector =
        bytecode_analysis().GetLoopInfoFor(current_loop_offset);
    for (uint i = current_loop_level; i < loop_info_vector.size(); ++i) {
    ...
    loop_pair = {loop_info_vector.back().parent_offset(), 0};
  }
}
