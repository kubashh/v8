...
class BytecodeGraphBuilder {
 ...
 private:
  ...
  // Generate the optimized code for the implicit function entry StackCheck.
...
void BytecodeGraphBuilder::PrepareEagerCheckpoint(int current_offset) {
  if (needs_eager_checkpoint()) {
    ...
    // The implicit StackCheck in function entry has no liveness. Since
    // StackChecks doesn't modify the liveness at all, we can use the in
    // liveness for the first bytecode, which would be the empty one.
    const BytecodeLivenessState* liveness_before =
        bytecode_analysis().GetInLivenessFor(
            current_offset != kFunctionEntryBytecodeOffset ? current_offset
                                                           : 0);
...
void BytecodeGraphBuilder::PrepareFrameState(Node* node,
  ...
  if (OperatorProperties::HasFrameStateInput(node->op())) {
    ...
    // the liveness as if we were having the StackCheck (i.e, empty liveness).
    // We can get that by using the IN liveness of the first actual bytecode.
...
void BytecodeGraphBuilder::FunctionEntryStackCheck() {
  ...
  if (!skip_first_stack_check()) {
    PrepareEagerCheckpoint(kFunctionEntryBytecodeOffset);
    Node* node =
        NewNode(javascript()->StackCheck(StackCheckKind::kJSFunctionEntry));
    environment()->RecordAfterState(node,
