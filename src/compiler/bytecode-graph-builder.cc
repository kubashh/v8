...
class BytecodeGraphBuilder {
 ...
 private:
  ...
  // ignored.
  Node* GetOrCreateParameter(int index, const char* debug_name_hint = nullptr);
...
BytecodeGraphBuilder::Environment::Environment(
  ...
  for (int i = 0; i < parameter_count; i++) {
    ...
    Node* parameter = builder->GetOrCreateParameter(i, debug_name);
  ...
  context_ = builder->GetOrCreateParameter(context_index, "%context");
...
Node* BytecodeGraphBuilder::GetFunctionClosure() {
  if (!function_closure_.is_set()) {
    ...
    Node* node = GetOrCreateParameter(index, "%closure");
    function_closure_.set(node);
  }
  return function_closure_.get();
}

Node* BytecodeGraphBuilder::GetOrCreateParameter(int parameter_index,
...
void BytecodeGraphBuilder::MaybeBuildTierUpCheck() {
  ...
  Node* new_target = GetOrCreateParameter(
      Linkage::GetJSCallNewTargetParamIndex(parameter_count), "%new.target");
  Node* argc = GetOrCreateParameter(
...
Node* BytecodeGraphBuilder::MakeNode(const Operator* op, int value_input_count,
  ...
  // Parameter nodes must be created through GetOrCreateParameter.
  DCHECK_IMPLIES(
