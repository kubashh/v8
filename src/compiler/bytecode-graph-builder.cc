...
void BytecodeGraphBuilder::VisitThrowNotSuperConstructorIfNotConstructor() {
  Node* constructor =
      environment()->LookupRegister(bytecode_iterator().GetRegisterOperand(0));
  ...
  NewBranch(check_is_constructor, BranchHint::kTrue);
  {
    ...
    BuildLoopExitsForFunctionExit(bytecode_analysis().GetInLivenessFor(
        bytecode_iterator().current_offset()));
    const Operator* op =
        javascript()->CallRuntime(Runtime::kThrowNotSuperConstructor);
    Node* node = NewNode(op, constructor);
    environment()->RecordAfterState(node, Environment::kAttachFrameState);
    Node* control = NewNode(common()->Throw());
    MergeControlToLeaveFunction(control);
  }
