...
Reduction JSTypedLowering::ReduceJSCall(Node* node) {
  ...
  if (shared.has_value()) {
    ...
    } else if (shared->HasBuiltinId() && Builtins::IsCpp(shared->builtin())) {
      // Patch {node} to a direct CEntry call.
      ReduceBuiltin(jsgraph(), node, shared->builtin(), arity, flags);
    } else if (shared->HasBuiltinId()) {
      DCHECK(Builtins::HasJSLinkage(shared->builtin()));
      // Patch {node} to a direct code object call.
      Callable callable = Builtins::CallableFor(isolate(), shared->builtin());
      CallDescriptor::Flags flags = CallDescriptor::kNeedsFrameState;
