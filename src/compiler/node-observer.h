...
class NodeObserver : public v8::internal::ZoneObject {
 public:
  ...
  virtual Observation OnNodeReplaced(const Node* node) {
    // TODO(paolosev@microsoft.com): Support this.
    UNREACHABLE();
    return Observation::kContinue;
  }
  virtual void OnNodeDeleted(const char* reducer_name, const Node* node) {
    // TODO(paolosev@microsoft.com): Support this.
    UNREACHABLE();
  }
};
...
class NodeObservation : public ZoneObject {
 public:
  ...
  NodeObserver::Observation OnNodeDeleted(const char* reducer_name) {
    DCHECK_NE(base::nullopt, state_);
    DCHECK_NOT_NULL(observer_);
    observer_->OnNodeDeleted(reducer_name, node_);
...
class ObserveNodeManager {
 public:
  ...
  void OnNodeDeleted(const char* reducer_name, const Node* node) {
    const auto it = observations_.find(node->id());
    if (it == observations_.end()) return;

    NodeObservation* observation = it->second;
    observation->OnNodeDeleted(reducer_name);
  }

  // void OnOperatorChanged(Node* node, const Operator* new_op) {
  //  const auto it = observations_.find(old_node->id());
  //  if (it == observations_.end()) return;
