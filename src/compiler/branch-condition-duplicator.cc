...
void BranchConditionDuplicator::DuplicateConditionIfNeeded(Node* node) {
  ...
  if (condNode->UseCount() > 1) {
    Node* newCond = DuplicateNode(condNode);
    node->ReplaceInput(0, newCond);
  }
}
...
void BranchConditionDuplicator::WalkControlDepsAndDupNodesIfNeeded() {
  Enqueue(graph_->end());
...
void BranchConditionDuplicator::Reduce() {
  WalkControlDepsAndDupNodesIfNeeded();
}
