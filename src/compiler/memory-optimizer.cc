...
MemoryOptimizer::MemoryOptimizer(
    ...
    : graph_assembler_(broker, jsgraph, zone, BranchSemantics::kMachine),
      ...
      address_reassociation_(jsgraph, zone),
      jsgraph_(jsgraph),
      ...
      address_reassociate_(address_reassociate) {}
...
void MemoryOptimizer::VisitProtectedLoad(Node* node,
  ...
  if (address_reassociate_ ==
      MemoryLowering::AddressReassociate::kDoAddressReassociate) {
    address_reassociation()->VisitLoad(node, effect_chain);
    EnqueueUses(node, state, effect_chain);
