...
void MemoryOptimizer::VisitNode(Node* node, AllocationState const* state) {
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kCallWithCallerRegisters:
      return VisitCallWithCallerRegisters(node, state);
...
void MemoryOptimizer::VisitCallWithCallerRegisters(
    Node* node, AllocationState const* state) {
  DCHECK_EQ(IrOpcode::kCallWithCallerRegisters, node->opcode());
  // If the call can allocate, we start with a fresh state.
  if (!(CallDescriptorOf(node->op())->flags() & CallDescriptor::kNoAllocate)) {
    state = empty_state();
  }
