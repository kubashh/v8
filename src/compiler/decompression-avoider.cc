...
void DecompressionAvoider::MarkNodes() {
  ...
  while (!to_visit_.empty()) {
    // Get the new node, and unmark it as to be visited.
...
void DecompressionAvoider::MarkNodeInputs(Node* node) {
  switch (node->opcode()) {
    ...
    case IrOpcode::kUnalignedStore:
      ...
      break;
    default:
      // To be conservative, we assume that all inputs need to be 64 bits
      // unless noted otherwise.
      for (int i = 0; i < node->op()->ValueInputCount(); ++i) {
        MarkAs(node->InputAt(i), State::kNeeds64bits);
      }
      // We mark the non-value input nodes as 32 bits so that they will be
      // visited. If they need 64 bits, they will be marked as such in a
      // future pass.
...
void DecompressionAvoider::MarkAs(Node* const node, State state) {
  ...
  if (previous_state == State::kUnvisited ||
    ...
    // Set the new state, and mark to visit.
...
void DecompressionAvoider::ChangeLoads() {
  for (Node* const node : compressed_loads_) {
    ...
    // Change to a Compressed MachineRepresentation so as to not perform the
    // full decompression.
