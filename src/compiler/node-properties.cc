    ...
    if (!receiver.IsJSObject() ||
      ...
      if (receiver.map().is_stable()) {
        ...
        *maps_return = ZoneHandleSet<Map>(receiver.map().object());
        GetElementsKindFromMaps(broker, maps_return, elements_kind);
