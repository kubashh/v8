...
class OutOfLineRecordWrite final : public OutOfLineCode {
 public:
  ...
  void SaveRegisters(int num, const Register* registers) {
    DCHECK(num > 0);
    for (int i = 0; i < num; ++i) {
      __ pushq(registers[i]);
    }
  }

  void RestoreRegisters(int num, const Register* registers) {
    DCHECK(num > 0);
  ...
  void Generate() final {
    ...
    const Register* registers =
        callable.descriptor().allocatable_general_registers_array();

    SaveRegisters(num, registers);
    ...
    RestoreRegisters(num, registers);
