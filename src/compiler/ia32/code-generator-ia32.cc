...
void CheckOutputOfAtomicPairInstr(IA32OperandConverter* converter,
                                  const Instruction* instr) {
  if (instr->OutputCount() > 0) {
    DCHECK_EQ(converter->OutputRegister(0), eax);
    if (instr->OutputCount() == 2) {
      DCHECK_EQ(converter->OutputRegister(1), edx);
    }
  }
}
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleArchInstruction(
  ...
  switch (arch_opcode) {
    ...
    case kIA32Word32AtomicPairExchange: {
      CheckOutputOfAtomicPairInstr(&i, instr);
      __ mov(eax, i.MemoryOperand(2));
      __ mov(edx, i.NextMemoryOperand(2));
      __ lock();
      __ cmpxchg8b(i.MemoryOperand(2));
