...
class V8_EXPORT_PRIVATE WasmAddressReassociation final {
  ...
  class CandidateMemOps : ZoneObject {
   public:
    ...
    void AddCandidate(Node* mem_op, uint64_t imm_offset);
    size_t GetNumNodes() const;
    Node* mem_op(size_t i) const;
    uint64_t imm_offset(size_t i) const;

   private:
    ZoneVector<Node*> mem_ops_;
    ZoneVector<uint64_t> imm_offsets_;
  };
 ...
 private:
  ...
  void AddCandidate(Node* mem_op, Node* base_reg, Node* offset_reg,
                    uint64_t imm_offset, NodeId effect_chain);
