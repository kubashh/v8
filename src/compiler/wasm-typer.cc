...
wasm::ValueType StructFieldFromOffset(const wasm::StructType* type,
  ...
  for (uint32_t index = 0; index < type->field_count(); index++) {
    ...
    if (field_offset == offset) return type->field(index).Unpacked();
  }
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kPhi: {
      if (!AllInputsTyped(node)) {
        ...
        computed_type = NodeProperties::GetType(node).AsWasm();
    ...
    case IrOpcode::kLoadImmutableFromObject: {
      ...
      if (!object_type.type.is_object_reference())
        std::cout << object_type.type.name() << "\n";
      ...
      switch (type_def.kind) {
        ...
        case wasm::TypeDefinition::kArray: {
          ...
          wasm::ValueType element_type =
              type_def.array_type->element_type().Unpacked();
          computed_type = {element_type, object_type.module};
          break;
        }
      }
      break;
