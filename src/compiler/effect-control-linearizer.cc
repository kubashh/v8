...
class EffectControlLinearizer {
 ...
 private:
  ...
  void EndStringBuilderConcat(Node* node, BasicBlock* block);
...
// SeqString.
void EffectControlLinearizer::EndStringBuilderConcat(Node* node,
                                                     BasicBlock* block) {
  ...
  for (Edge edge : node->use_edges()) {
    ...
    if (BasicBlock::GetCommonDominator(block, schedule()->block(edge.from())) ==
        block) {
