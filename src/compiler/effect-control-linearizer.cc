...
void EffectControlLinearizer::LowerTierUpCheck(Node* node) {
  ...
  Node* optimization_marker = __ LoadField(
      AccessBuilder::ForFeedbackVectorOptimizationMarker(), vector);
  ...
  OptimizationMarker no_marker =
      broker()->is_turboprop() ? OptimizationMarker::kHasMidTierOptimizedCode
                               : OptimizationMarker::kNone;
  OptimizationMarker in_optimization_queue_marker =
      broker()->is_turboprop()
  ...
  __ BranchWithHint(
      __ Word32Equal(optimization_marker, __ Int32Constant(no_marker)),
      &fallthrough, &optimization_marker_is_not_none, BranchHint::kTrue);

  __ Bind(&optimization_marker_is_not_none);
  __ BranchWithHint(
      __ Word32Equal(optimization_marker,
                     __ Int32Constant(in_optimization_queue_marker)),
