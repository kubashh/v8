...
void EffectControlLinearizer::StoreLiteralStringToBuffer(Node* buffer,
  ...
  if (node->opcode() == IrOpcode::kHeapConstant &&
    ...
    const uint16_t* chars = GetLiteralString<uint16_t>(node, broker());
    for (int i = 0; i < len; i++) {
      __ StoreElement(AccessBuilder::ForSeqTwoByteStringCharacter(), buffer,
  ...
  // string. Thus, we generate both 1-byte and 2-byte cases, and we'll decide
  // dynamically which one to execute based on the value of {is_one_byte}.

  auto one_byte_lbl = __ MakeLabel(), two_byte_lbl = __ MakeLabel(),
       done = __ MakeLabel();
  __ Branch(is_one_byte, &one_byte_lbl, &two_byte_lbl);

  auto copy_constant = [&](auto access, bool can_be_two_bytes) {
    HeapObjectMatcher m(node);
    DCHECK(m.HasResolvedValue() && m.Ref(broker()).IsString());
    StringRef string = m.Ref(broker()).AsString();
    int len = string.length().value();
    DCHECK(string.object()->IsOneByteRepresentation());
    const uint8_t* chars = GetLiteralString<uint8_t>(node, broker());
    for (int i = 0; i < len; i++) {
      __ StoreElement(access, buffer, __ Int32Add(offset, __ Int32Constant(i)),
                      __ Int32Constant(chars[i]));
    }
  };
  auto copy_string_from_char_code = [&](auto access) {
    Node* c = __ Word32And(node->InputAt(0), __ Uint32Constant(0xFFFF));
    __ StoreElement(access, buffer, offset, c);
  };

  __ Bind(&one_byte_lbl);
  if (node->opcode() == IrOpcode::kHeapConstant) {
    copy_constant(AccessBuilder::ForSeqOneByteStringCharacter(), false);
  } else {
    copy_string_from_char_code(AccessBuilder::ForSeqOneByteStringCharacter());
  }
  __ Goto(&done);
  __ Bind(&two_byte_lbl);
  if (node->opcode() == IrOpcode::kHeapConstant) {
    copy_constant(AccessBuilder::ForSeqTwoByteStringCharacter(), true);
  } else {
    copy_string_from_char_code(AccessBuilder::ForSeqTwoByteStringCharacter());
  }
  __ Goto(&done);
  __ Bind(&done);
}
