...
class EffectControlLinearizer {
 ...
 private:
  ...
  void EndStringBuilderConcat(Node* node);
...
void EffectControlLinearizer::Run() {
  ...
  for (BasicBlock* block : *(schedule()->rpo_order())) {
    ...
    for (Node* node : concat_phis) {
      EndStringBuilderConcat(node);
    }
...
void EffectControlLinearizer::EndStringBuilderConcat(Node* node) {
  ...
  BasicBlock* block = schedule()->block(node);
  ...
  for (Edge edge : node->use_edges()) {
    ...
    if (block->LoopContains(schedule()->block(edge.from()))) {
    ...
    if (NodeProperties::IsControlEdge(edge)) {
      PrintF("Edge is control edge\n");
      edge.UpdateTo(gasm()->control());
    } else if (NodeProperties::IsEffectEdge(edge)) {
      PrintF("Edge is effect edge\n");
      edge.UpdateTo(gasm()->effect());
    } else {
