...
class EffectControlLinearizer {
 ...
 private:
  ...
  void BuildCallDynamicMapChecksBuiltin(
      Node* feedback_vector, Node* actual_value, Node* actual_value_map,
      Node* actual_handler, int feedback_slot_index,
...
void EffectControlLinearizer::BuildCallDynamicMapChecksBuiltin(
    Node* feedback_vector, Node* actual_value, Node* actual_value_map,
    Node* actual_handler, int feedback_slot_index, GraphAssemblerLabel<0>* done,
    Node* frame_state) {
  Node* slot_index = __ IntPtrConstant(feedback_slot_index);
  Operator::Properties properties = Operator::kNoDeopt | Operator::kNoThrow;
  auto builtin = Builtins::kDynamicMapChecks;
  Node* result = CallBuiltin(builtin, properties, feedback_vector, slot_index,
...
void EffectControlLinearizer::LowerDynamicCheckMaps(Node* node,
  ...
  if (p.state() == DynamicCheckMapsParameters::kMonomorphic) {
    DCHECK_EQ(p.maps().size(), 1);
    Node* expected_map = __ HeapConstant(p.maps().at(0));
    ...
    {
      BuildCallDynamicMapChecksBuiltin(feedback_vector, actual_value,
                                       actual_value_map, actual_handler,
