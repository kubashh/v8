...
void EffectControlLinearizer::Run() {
  ...
  for (BasicBlock* block : *(schedule()->rpo_order())) {
    ...
    for (Node* phi : concat_phis) {
      ...
      Node* trimmed = EndStringBuilderConcat(phi, base::nullopt);
...
// returns a true node if it's 1-byte, and a false node otherwise.
...
// Assuming that {orig} is a 1-byte backing store for a a string builder or
// total length of {total_length}, including {initialized_length} initialized
...
Node* EffectControlLinearizer::EndStringBuilderConcat(
  ...
  //     HasTurbofanStringBuilders method that tells the GC to ignore too-small
  //     SlicedString when checking verifying heap integrity.
    ...
    for (Edge edge : node->use_edges()) {
      ...
      if ((*user_block->nodes())[0]->opcode() == IrOpcode::kMerge) {
        if (std::find(user_block->predecessors().begin(),
                      ...
                      block.value()) != user_block->predecessors().end()) {
      ...
      if (BasicBlock::GetCommonDominator(block.value(), user_block) == block) {
...
Node* EffectControlLinearizer::LowerStringConcat(Node* node) {
  if (string_builder_optimizer_->IsFirstConcatInStringBuilder(node)) {
    ...
    {
      // Computing the new size: we double the size of the backing store until
      // its larger than {new_length}.
      ...
      // Actually reallocating the backing store: allocating a new SeqString,
      // and copying the content of the old backing store into the new one.
    ...
    if (string_builder_optimizer_->IsStringBuilderEnd(node)) {
      ...
      // right now.
      return EndStringBuilderConcat(sliced_string, base::nullopt);
    } else {
      return sliced_string;
    }
  } else {
...
void LinearizeEffectControl(JSGraph* graph, Schedule* schedule, Zone* temp_zone,
  ...
  StringBuilderOptimizer string_builder_optimizer(graph, schedule, temp_zone,
                                                  broker);
  if (v8_flags.turbo_string_builder) {
