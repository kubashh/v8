...
void SimplifiedLoweringVerifier::VisitNode(Node* node,
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kCheckedTaggedToTaggedSigned: {
      Type output_type = Type::SignedSmall();
      Truncation output_trunc = InputTruncation(node, 0);
      output_trunc = GeneralizeTruncation(output_trunc, output_type);
    ...
    case IrOpcode::kPhi: {
      // TODO(nicohartmann@): Supporting Phis is more complex.
#if 0
      ...
      for (int i = 1; i < arity; ++i) {
        ...
        output_trunc =
            LeastGeneralTruncation(output_trunc, InputTruncation(node, 0));
      }
      output_trunc = GeneralizeTruncation(output_trunc, output_type);
      CheckAndSet(node, output_type, output_trunc);
#endif
...
#define CASE(code, ...) case IrOpcode::k##code:
      ...
      MACHINE_SIMD_OP_LIST(CASE) {
        // SIMD operators should net be in the graph, yet.
        UNREACHABLE();
      }
#undef CASE
  }
