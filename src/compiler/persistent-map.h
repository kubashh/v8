...
// deleted by overwriting them with the default value. The hash values should
// have high variance in their high bits, so dense integers are a bad choice.
// Complexity:
...
class PersistentMap {
 ...
 private:
  ...
  struct hash_t : private std::bitset<kHashBits> {
    ...
    Bit operator[](int pos) const {
      return Base::operator[](kHashBits - pos - 1) ? kRight : kLeft;
    }
 ...
 public:
  // Depth of the last added path. This is a cheap estimate for the size of the
  // hash tree.
  ...
  PersistentMap Add(Key key, Value value) const {
    ...
    FocusedTree* tree =
        new (zone_->New(sizeof(FocusedTree) +
            ...
            FocusedTree{KeyValue(std::move(key), std::move(value)), length,
                        key_hash, more};
  ...
  // The {iterator} produces key-value pairs in the lexicographical ordering of
  // hash value and key.
  class iterator {
   public:
   ...
   private:
    ...
    array<const FocusedTree*, kHashBits> path_;
    Value def_value_;
 ...
 private:
  ...
  const FocusedTree* FindHash(hash_t hash) const {
  ...
  const FocusedTree* FindHash(hash_t hash,
                              array<const FocusedTree*, kHashBits>* path,
                              int* length) const {
    const FocusedTree* tree = tree_;
    int level = 0;
  ...
  // Load value from the leaf node on the path of {tree}.
  const Value& ValueFromMap(const FocusedTree* tree, const Key& key) const {
    if (!tree) {
      return def_value_;
    }
    if (tree->more) {
