...
Reduction JSNativeContextSpecialization::ReduceElementAccess(
  ...
  if ((access_mode == AccessMode::kLoad || access_mode == AccessMode::kHas) &&
    ...
    Reduction reduction = ReduceKeyedLoadFromHeapConstant(
        node, index, access_mode, processed.load_mode);
    ...
    for (ElementAccessInfo const& access_info : access_infos) {
      for (Handle<Map> map : access_info.receiver_maps()) {
        ...
        if ((IsHoleyOrDictionaryElementsKind(receiver_map.elements_kind()) ||
             IsGrowStoreMode(processed.store_mode)) &&
  ...
  if (access_infos.size() == 1) {
    ...
    ValueEffectControl continuation = BuildElementAccess(
        receiver, index, value, effect, control, access_info, access_mode,
        processed.load_mode, processed.store_mode);
    value = continuation.value();
    effect = continuation.effect();
    control = continuation.control();
  } else {
    ...
    for (size_t j = 0; j < access_infos.size(); ++j) {
      ...
      ValueEffectControl continuation = BuildElementAccess(
          ...
          access_info, access_mode, processed.load_mode, processed.store_mode);
      values.push_back(continuation.value());
      effects.push_back(continuation.effect());
      controls.push_back(continuation.control());
    }
...
namespace {
KeyedAccessLoadMode GetLoadMode(FeedbackNexus const& nexus) {
  return (IsKeyedLoadICKind(nexus.kind()) || IsKeyedHasICKind(nexus.kind()))
             ? nexus.GetKeyedAccessLoadMode()
             : STANDARD_LOAD;
}
KeyedAccessStoreMode GetStoreMode(FeedbackNexus const& nexus) {
  return (IsKeyedStoreICKind(nexus.kind()) ||
      ...
      if (!ExtractReceiverMaps(receiver, effect, nexus, &receiver_maps)) {
        ...
        if (name.has_value()) {
          ...
          processed = broker()->ProcessFeedbackMapsForElementAccess(
              receiver_maps, GetLoadMode(nexus), GetStoreMode(nexus));
        }
      }
    }
  }
