...
Reduction JSNativeContextSpecialization::ReduceJSInstanceOf(Node* node) {
  ...
  if (access_info.IsNotFound()) {
    ...
    return Changed(node).Combine(ReduceJSOrdinaryHasInstance(node));
  }
...
Reduction JSNativeContextSpecialization::ReduceJSOrdinaryHasInstance(
  ...
  if (m.Ref(broker()).IsJSBoundFunction()) {
    ...
    return Changed(node).Combine(ReduceJSInstanceOf(node));
  }
  ...
  if (m.Ref(broker()).IsJSFunction()) {
    ...
    return Changed(node).Combine(ReduceJSHasInPrototypeChain(node));
  }
