...
Reduction JSNativeContextSpecialization::ReduceJSInstanceOf(Node* node) {
  ...
  if (access_info.IsFastDataConstant()) {
    ...
    dependencies()->DependOnPropertyValueSame(
        holder_ref, access_info.field_representation(),
        access_info.field_index(), *constant);
...
Reduction JSNativeContextSpecialization::ReduceNamedAccess(
  ...
  if (lookup_start_object_maps.size() == 1) {
    ...
    if (lookup_start_object_map.equals(
      ...
      base::Optional<bool> is_detached =
          target_native_context.global_object().IsDetached();
      if (is_detached.has_value() && !is_detached.value()) {
