...
Reduction JSNativeContextSpecialization::ReduceNamedAccess(
  ...
  STATIC_ASSERT(JSLoadNamedFromSuperNode::HomeObjectProtoIndex() == 2);
  Node* lookup_start_object;
  Node* receiver;
  if (node->opcode() == IrOpcode::kJSLoadNamedFromSuper) {
    receiver = NodeProperties::GetValueInput(node, 0);
    lookup_start_object = NodeProperties::GetValueInput(node, 2);
  } else {
    lookup_start_object = receiver = NodeProperties::GetValueInput(node, 0);
  }
  ...
  if (!InferReceiverMaps(lookup_start_object, effect, &lookup_start_object_maps)) {
    lookup_start_object_maps = feedback.maps();
  }
  RemoveImpossibleReceiverMaps(lookup_start_object, &lookup_start_object_maps);
  ...
  if (lookup_start_object_maps.size() == 1) {
    ...
    if (lookup_start_object_map.equals(
      ...
      return ReduceGlobalAccess(node, lookup_start_object, value, feedback.name(),
                                access_mode, key);
    }
  }
  ...
  if (access_infos.size() == 1) {
    ...
    if (receiver == lookup_start_object && !access_builder.TryBuildStringCheck(broker(),
                                            access_info.receiver_maps(),
                                            &receiver, &effect, control) &&
    ...
    ValueEffectControl continuation = BuildPropertyAccess(lookup_start_object,
    ...
    for (size_t j = 0; j < access_infos.size(); ++j) {
          ...
          access_info.receiver_maps();
      {
        ...
        if (j == access_infos.size() - 1) {
          ...
          access_builder.BuildCheckMaps(lookup_start_object, &this_effect, this_control,
          ...
          Node* check = this_effect =
              graph()->NewNode(simplified()->CompareMaps(maps), lookup_start_object,
        ...
        if (insert_map_guard) {
          ...
          this_effect = graph()->NewNode(simplified()->MapGuard(maps), lookup_start_object,
                                         this_effect, this_control);
        }

        // If all {lookup_start_object_maps} are Strings we also need to rename the
        // {lookup_start_object} here to make sure that TurboFan knows that along this
        // path the {this_lookup_start_object} is a String. This is because we want
        ...
        if (HasOnlyStringMaps(broker(), lookup_start_object_maps)) {
          ...
          this_lookup_start_object = this_effect =
              graph()->NewNode(common()->TypeGuard(Type::String()), lookup_start_object,
                               this_effect, this_control);
        }
      }
      ...
      ValueEffectControl continuation =
          BuildPropertyAccess(this_lookup_start_object, receiver, this_value, context, frame_state,
...
Reduction JSNativeContextSpecialization::ReduceJSLoadNamedFromSuper(
  ...
  Node* home_object_map = effect = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForMap()),
  ...
  node->ReplaceInput(2, home_object_proto);
...
Reduction JSNativeContextSpecialization::ReducePropertyAccess(
  ...
  DCHECK_IMPLIES(node->opcode() == IrOpcode::kJSLoadNamedFromSuper,
...
JSNativeContextSpecialization::BuildPropertyLoad(Node* lookup_start_object,
    Node* receiver, Node* context, Node* frame_state, Node* effect,
  ...
  if (access_info.IsNotFound()) {
    ...
    PropertyAccessBuilder access_builder(jsgraph(), broker(), dependencies());
    value = access_builder.BuildLoadDataField(name, access_info, lookup_start_object,
                                              &effect, &control);
  }
...
JSNativeContextSpecialization::BuildPropertyAccess(
  ...
  switch (access_mode) {
    case AccessMode::kLoad:
      return BuildPropertyLoad(lookup_start_object, receiver, context, frame_state, effect, control,
