...
bool JSNativeContextSpecialization::CanTreatHoleAsUndefined(
  ...
  for (Handle<Map> map : receiver_maps) {
    ...
    if (!receiver_prototype.IsJSObject()) return false;
    if (!js_heap_broker()->IsArrayPrototype(receiver_prototype.AsJSObject()) &&
        !js_heap_broker()->IsObjectPrototype(receiver_prototype.AsJSObject())) {
      return false;
    }
  }
