...
bool HasOnlySequentialStringMaps(MapHandles const& maps) {
  for (auto map : maps) {
    if (!map->IsStringMap()) return false;
    if (map->instance_type() != STRING_TYPE &&
        map->instance_type() != ONE_BYTE_STRING_TYPE &&
        map->instance_type() != INTERNALIZED_STRING_TYPE &&
        map->instance_type() != ONE_BYTE_INTERNALIZED_STRING_TYPE) {
      return false;
    }
  }
  return true;
