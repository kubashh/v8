...
Reduction JSNativeContextSpecialization::ReduceNamedAccess(
  ...
  STATIC_ASSERT(JSLoadNamedFromSuperNode::HomeObjectIndex() == 1);
  ...
  // receiver = the object we pass to the accessor as the "this" value.
  Node* receiver;
  if (node->opcode() == IrOpcode::kJSLoadNamedFromSuper) {
    ...
    Node* home_object = NodeProperties::GetValueInput(node, 1);
    Node* home_object_map = effect =
        graph()->NewNode(simplified()->LoadField(AccessBuilder::ForMap()),
                         home_object, effect, control);
  ...
  if (!InferLookupStartObjectMaps(lookup_start_object, effect,
  ...
  if (lookup_start_object_maps.size() == 1) {
    ...
    if (lookup_start_object_map.equals(
      ...
      return ReduceGlobalAccess(node, lookup_start_object, value,
                                feedback.name(), access_mode, key);
    }
  }
  ...
  if (access_infos.size() == 1) {
    ...
    // If receiver != lookup_start_object (super property access),
    // lookup_start_object is an object or undefined. It can't be a number, a
    // string etc. So trying to build the following checks doesn't make sense.
    if (receiver == lookup_start_object) {
      // Try to build string check or number check if possible.
      // Otherwise build a map check.
      if (!access_builder.TryBuildStringCheck(
    ...
    } else {  // receiver != lookup_start_object
...
void JSNativeContextSpecialization::RemoveImpossibleReceiverMaps(
  ...
  base::Optional<MapRef> root_map = InferLookupStartObjectRootMap(receiver);
...
JSNativeContextSpecialization::TryRefineElementAccessFeedback(
  ...
  if (!InferLookupStartObjectMaps(receiver, effect, &inferred_maps))
    return feedback;
...
bool JSNativeContextSpecialization::InferLookupStartObjectMaps(
    Node* receiver, Node* effect,
    ZoneVector<Handle<Map>>* receiver_maps) const {
  ZoneHandleSet<Map> maps;
  NodeProperties::InferMapsResult result =
      NodeProperties::InferMapsUnsafe(broker(), receiver, effect, &maps);
...
JSNativeContextSpecialization::InferLookupStartObjectRootMap(
    Node* receiver) const {
  HeapObjectMatcher m(receiver);
  if (m.HasValue()) {
