...
Reduction JSNativeContextSpecialization::ReduceJSAdd(Node* node) {
  ...
  if (*lhs_len + *rhs_len <= String::kMaxLength) {
    HeapObjectMatcher lhs_matcher(lhs);
    HeapObjectMatcher rhs_matcher(rhs);

    if ((lhs_matcher.HasValue() && lhs_matcher.Value()->IsString()) ||
        (rhs_matcher.HasValue() && rhs_matcher.Value()->IsString())) {
