  ...
  while (true) {
    ...
    if (kind != FunctionKind::kDefaultDerivedConstructor) {
      ...
      if (kind == FunctionKind::kDefaultBaseConstructor) {
        ...
        // poked at indices slot(reg_2) (boolean output) and slot(reg_2 + 1)
        // (object output). Now we're reducing this bytecode to a builtin call
        // which only has one output (object_output). Change where in the
        // FrameState the output is poked at. The location for boolean_output is
        // already hard-wired to true (which is the correct value here) in
