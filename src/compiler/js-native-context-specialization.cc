  ...
  while (true) {
    ...
    if (kind != FunctionKind::kDefaultDerivedConstructor) {
      ...
      if (kind == FunctionKind::kDefaultBaseConstructor) {
        ...
        // poked at indices [reg_2 (boolean output), reg_2 + 1 (object output)].
        // Now we're reducing this bytecode to a builtin call which only has one
        // output (object_output). Change where in the FrameState the output is
