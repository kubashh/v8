...
class V8_EXPORT_PRIVATE NodeOriginTable final
 ...
 public:
  class Scope final {
   public:
    Scope(NodeOriginTable* node_origins, const char* reducer_name, Node* node)
        : node_origins_(node_origins), prev_node_(node_origins->current_node_) {
      node_origins->current_node_ = NodeOrigin(reducer_name, node->id());
    }

    ~Scope() { node_origins_->current_node_ = prev_node_; }

   private:
    NodeOriginTable* const node_origins_;
    NodeOrigin const prev_node_;
  ...
  void SetNodeOrigin(Node* node, const NodeOrigin& position);

  void SetCurrentPosition(const NodeOrigin& pos) { current_node_ = pos; }
