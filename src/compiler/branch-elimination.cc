...
Reduction BranchElimination::UpdateConditions(
  ...
  if (original == nullptr ||
      !original->EqualsAfterAddingCondition(prev_conditions, current_condition,
                                            is_true_branch)) {
...
bool BranchElimination::ControlPathConditions::EqualsAfterAddingCondition(
    ...
    bool branch_direction) const {
  // We would add an extra condition to the prev_conditions, so the count of
  ...
  // Check if the head is same as the curr_condition + branch_direction.
  if (head_->condition != curr_condition ||
      head_->is_true != branch_direction) {
    return false;
  }
