...
bool BranchElimination::PullTrapIntoMerge(Node* node) {
  DCHECK(node->opcode() == IrOpcode::kTrapIf ||
         node->opcode() == IrOpcode::kTrapUnless);
  Node* merge = NodeProperties::GetControlInput(node);
  if (merge->opcode() != IrOpcode::kMerge) return false;

  Node* condition = NodeProperties::GetValueInput(node, 0);
  Node* effect_input = NodeProperties::GetEffectInput(node);
  // We restrict the effect to something we know does not depend on the merge.
  // TODO(manoskouk): Can we relax this?
...
Reduction BranchElimination::ReduceTrapConditional(Node* node) {
  ...
  if (PullTrapIntoMerge(node)) return Replace(dead());
