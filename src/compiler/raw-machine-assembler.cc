...
Node* CallCFunctionImpl(
    ...
    bool caller_saved_regs = false, SaveFPRegsMode mode = kDontSaveFPRegs) {
  ...
  nodes[0] = function;
  std::transform(args.begin(), args.end(), std::next(nodes.begin()),
...
Node* RawMachineAssembler::CallCFunction(
    ...
    std::initializer_list<RawMachineAssembler::CFunctionArg> args) {
  return CallCFunctionImpl(this, function, return_type, args);
}
