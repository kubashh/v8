...
V8_EXPORT_PRIVATE MachineOperatorBuilder::Flags SupportedMachineOperatorFlags();

V8_EXPORT_PRIVATE MachineOperatorBuilder::AlignmentRequirements
AlignmentRequirements();

V8_EXPORT_PRIVATE const std::map<NodeId, int> GetVirtualRegistersForTesting(
    const InstructionSelectorT<TurbofanAdapter>* selector);
V8_EXPORT_PRIVATE const std::map<NodeId, int> GetVirtualRegistersForTesting(
    const InstructionSelectorT<TurboshaftAdapter>* selector);

// template<>
// extern const std::map<NodeId, int> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE)
