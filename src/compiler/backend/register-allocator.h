...
class DoubleEndedSplitVector {
 ...
 private:
  ...
  // Use template specializations, so we get a compile error in case we add
  // a `GrowthDirection` later on and forget to implement it.
  template <>
  void EnsureOneMoreCapacityAt<kFront>(Zone* zone) {
    if (V8_LIKELY(space_at_front() > 0)) return;
    GrowAt<kFront>(zone, capacity() * 2);
    DCHECK_GT(space_at_front(), 0);
  }
  template <>
  void EnsureOneMoreCapacityAt<kFrontOrBack>(Zone* zone) {
...
class LiveRangeBundle : public ZoneObject {
 public:
  explicit LiveRangeBundle(int id) : id_(id) {}
  ...
  bool TryAddRange(TopLevelLiveRange* range, Zone* zone);
  ...
  static LiveRangeBundle* TryMerge(LiveRangeBundle* lhs, LiveRangeBundle* rhs,
                                   Zone* zone, bool trace_alloc);

 private:
  void AddRange(TopLevelLiveRange* range, Zone* zone);

  // A flat set, sorted by `LiveRangeOrdering`.
  DoubleEndedSplitVector<TopLevelLiveRange*> ranges_;
