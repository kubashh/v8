...
class LiveRangeBundle : public ZoneObject {
 ...
 private:
  ...
  // A set, sorted by `LiveRangeOrdering`.
  ZoneVector<LiveRange*> ranges_;
  // A set, sorted by their `start()` position.
...
class V8_EXPORT_PRIVATE TopLevelLiveRange final : public LiveRange {
 ...
 private:
  ...
  // This is a cache for `GetChildCovers`.
  // TODO(dlehmann): Check if this can be removed, once the child `LiveRange`s
  // no longer form a linked list.
