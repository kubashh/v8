...
class DoubleEndedSplitVector {
 public:
  ...
  iterator insert(const_iterator position, const T& value) {
    ...
    EnsureOneMoreCapacityAt<kFrontOrBack>();
    ...
    if (space_at_front() >= space_at_back()) {
      // Copy to the left.
  ...
  iterator InsertSorted(const T& value, Comparator comp) {
    const_iterator position =
        std::upper_bound(begin(), end(), value, std::move(comp));
 ...
 private:
  ...
  enum Direction { kFront, kFrontOrBack };
