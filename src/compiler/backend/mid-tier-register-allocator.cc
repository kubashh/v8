    ...
    if (register_state_ == nullptr) {
      ...
      for (RegisterIndex reg : *successor_registers) {
        ...
        if (!kSimpleFPAliasing && (rep == MachineRepresentation::kFloat64 ||
          ...
          int shift = rep == MachineRepresentation::kFloat64 ? 1 : 2;
          int simd_reg_code = ToRegCode(reg, rep) >> shift;
          RegisterIndex simdReg =
              FromRegCode(simd_reg_code, MachineRepresentation::kSimd128);
          reg_in_use |=
              register_state_->IsAllocated(simdReg) &&
              VirtualRegisterDataFor(VirtualRegisterForRegister(simdReg))
...
void SinglePassRegisterAllocator::ReserveFixedRegister(
  ...
  if (!kSimpleFPAliasing && (rep == MachineRepresentation::kFloat64 ||
    ...
    RegisterIndex simdReg =
        FromRegCode(simd_reg_code, MachineRepresentation::kSimd128);
    DCHECK(simdReg == reg || simdSibling(simdReg) == reg);
    int allocated_vreg = VirtualRegisterForRegister(simdReg);
    if (simdReg != reg &&
        allocated_vreg != InstructionOperand::kInvalidVirtualRegister &&
        VirtualRegisterDataFor(allocated_vreg).rep() ==
            MachineRepresentation::kSimd128 &&
        !DefinedAfter(virtual_register, instr_index, pos)) {
      SpillRegister(simdReg);
    }
  }
