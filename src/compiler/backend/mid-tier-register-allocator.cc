...
class SinglePassRegisterAllocator final {
 ...
 private:
  ...
  // If {if FPAliasing kind is COMBINE}, two FP registers alias one SIMD
  ...
  RegisterIndex simdSibling(RegisterIndex reg) const {
    CHECK(kFPAliasing == AliasingKind::COMBINE);  // Statically evaluated.
...
void SinglePassRegisterAllocator::SpillRegisterAtMerge(
  ...
  if (kFPAliasing == COMBINE && rep == MachineRepresentation::kSimd128) {
  ...
  if (kFPAliasing == COMBINE && (rep == MachineRepresentation::kFloat64 ||
                                 rep == MachineRepresentation::kFloat32)) {
    int simd_reg_code;
...
RegisterIndex SinglePassRegisterAllocator::ChooseRegisterToSpill(
  ...
  for (RegisterIndex reg : *register_state_) {
    ...
    // register.
    DCHECK_IMPLIES(kFPAliasing != AliasingKind::COMBINE,
