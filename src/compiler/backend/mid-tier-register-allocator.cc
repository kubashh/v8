...
class SinglePassRegisterAllocator final {
 ...
 private:
  ...
  void MergeStateFrom(int instr_index,
...
void SinglePassRegisterAllocator::StartBlock(const InstructionBlock* block) {
  ...
  if (block->SuccessorCount() == 1) {
    ...
    // into our block.
    MergeStateFrom(block->last_instruction_index(), block->successors());
  }
}
...
void SinglePassRegisterAllocator::MergeStateFrom(
    int instr_index, const InstructionBlock::Successors& successors) {
