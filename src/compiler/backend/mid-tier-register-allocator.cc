...
void VirtualRegisterData::EnsureSpillRange(
  ...
  if (is_phi()) {
    ...
    if (is_exceptional_call_output()) {
      ...
      // catch handler, then the output will only be live in the IsSuccess
      // successor block, not the IsException side, so make the definition block
      // the IsSuccess successor block explicitly.
