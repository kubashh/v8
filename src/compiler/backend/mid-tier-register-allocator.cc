...
class BlockState final {
 public:
  ...
  // If this block is deffered, this represents virtual registers that have not
  // yet been spilled in non-deferred blocks, but were spilled within the
  // deferred blocks directly reachable from this block.
  DeferredBlocksRegion* deferred_spills() const { return deferred_spills_; }
  void set_deferred_spills(DeferredBlocksRegion* deferred_spills) {
    DCHECK_NULL(deferred_spills_);
    deferred_spills_ = deferred_spills;
  }
 ...
 private:
  ...
  DeferredBlocksRegion* deferred_spills_;
  ...
  class SpillRange : public ZoneObject {
   public:
    ...
    void AddDeferredSpillOutput(AllocatedOperand allocated_op, int instr_index,
      ...
      const BitVector* deferred_blocks = data->block_state(block->rpo_number())
                                             .deferred_spills()
                                             ->blocks_covered();
...
class MidTierOutputProcessor final {
 ...
 private:
  ...
  void InitializeDeferredBlocks(RpoNumber initial_block);
...
void MidTierOutputProcessor::InitializeBlockState(
  ...
  if (block->IsDeferred() && !block_state.deferred_spills()) {
  ...
  if (is_deferred_block_boundary && !block->IsDeferred()) {
    for (RpoNumber successor : block->successors()) {
      ...
      for (int virtual_register :
           *data()->block_state(successor).deferred_spills()) {
...
void MidTierReferenceMapPopulator::RecordReferences(
  ...
  for (int instr_index : data()->reference_map_instructions()) {
    ...
    if (spill_range->IsLiveAt(instr_index, instr->block(), data())) {
