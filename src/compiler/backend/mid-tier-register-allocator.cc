...
// A Range from (start, end] of instructions exclusive of start and inclusive
// of end. This representation is used since an operand is usually output by the
// instruction at |start| (and therefore isn't live until after that
// instruction), and is used as an input to instruction at |end| (and so is
// still live for that instruction).
class Range {
 public:
...
void VirtualRegisterData::EnsureSpillRange(
  ...
  if (is_phi()) {
    ...
    // instructions in the predecssor block to the live range.
    const InstructionBlock* definition_block =
        data->code()->InstructionAt(output_instr_index_)->block();
    spill_range->ExtendRangeTo(output_instr_index_ - 1);
    for (RpoNumber pred_rpo : definition_block->predecessors()) {
      const InstructionBlock* block = data->GetBlock(pred_rpo);
      spill_range->ExtendRangeTo(block->last_instruction_index() - 1);
    }
  }
