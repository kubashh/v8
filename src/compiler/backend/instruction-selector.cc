...
InstructionSelector::InstructionSelector(
    ...
    : zone_(zone),
      ...
      ,
      phi_states_1_(node_count, Upper32BitsState::kNotYetChecked, zone),
      phi_states_2_(node_count, Upper32BitsState::kNotYetChecked, zone)
#endif
{
...
bool InstructionSelector::ZeroExtendsWord32ToWord64(Node* node) {
  ...
  while (true) {
    ...
    if (node->opcode() == IrOpcode::kPhi) {
      if (type == kCall) {
        Upper32BitsState current = phi_states_2_[node->id()];
        ...
        // itself.
        phi_states_2_[node->id()] = Upper32BitsState::kUpperBitsGuaranteedZero;
      } else if (type == kReturnFalse) {
        phi_states_2_[node->id()] = Upper32BitsState::kNoGuarantee;
