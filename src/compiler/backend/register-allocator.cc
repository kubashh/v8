  ...
  for (const InstructionBlock* block : code()->instruction_blocks()) {
    ...
    for (int vreg : *live) {
      ...
      for (const RpoNumber& pred : block->predecessors()) {
        ...
        // Fastpath: The current range covers the predecessor, so we don't
        // need to search since we already know it will be the same range,
        // for which there is nothing to connect (see next bailout).
        ...
        // connect.
        if (cur_range == pred_range) continue;
