...
int LinearScanAllocator::PickRegisterThatIsAvailableLongest(
  ...
  for (int i = 0; i < num_codes; ++i) {
    ...
    TRACE("Register %s in free until %d and spill is %d\n", RegisterName(code),
          candidate_free, !spills || spills->at(code));
    if ((candidate_free > current_free) ||
        (candidate_free == current_free && reg != hint_reg &&
         ((data()->HasFixedUse(current->representation(), reg) &&
          ...
          ((spills != nullptr) && !spills->at(reg) && spills->at(code))))) {
      reg = code;
...
bool LinearScanAllocator::TryAllocateFreeReg(
  ...
  int reg = PickRegisterThatIsAvailableLongest(current, hint_reg,
                                               free_until_pos, nullptr);
...
void LinearScanAllocator::AllocateBlockedReg(LiveRange* current,
  ...
  EmbeddedVector<bool, RegisterConfiguration::kMaxRegisters> is_spilled(false);

  for (LiveRange* range : active_live_ranges()) {
    int cur_reg = range->assigned_register();
    //    is_spilled[cur_reg] = range->TopLevel()->HasSpillOperand();
