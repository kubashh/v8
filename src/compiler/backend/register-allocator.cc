...
void LiveRangeBuilder::BuildLiveRanges() {
  ...
  for (TopLevelLiveRange* range : data()->live_ranges()) {
    ...
    if (range->has_slot_use() && range->HasNoSpillType()) {
      data()->AssignSpillRangeToLiveRange(range, SpillMode::kSpillAtDefinition);
    }
...
void LinearScanAllocator::UndoPreviousSplit(LiveRange* range) {
...
void LinearScanAllocator::DropNotLiveRanges(RangeWithRegisterSet& to_be_live,
                                            LifetimePosition position,
  ...
  for (auto it = active_live_ranges().begin();
    ...
    if (found == to_be_live.end()) {
      // Was not life before, spill it.
      // Fixed registers are exempt from this. They might have been
      // added from incative at the block boundary but we know that
      ...
      if (toplevel->IsFixed()) {
        ...
        // When putting a range back, we have to add back the tail that
        // we might have split off when we reloaded it. Otherwise we
        // might keep generating small split-offs.
        UndoPreviousSplit(active_range);
      ...
      // This range is meant to be life, keep it.
      ...
      if (expected_register == active_range->assigned_register()) {
        ...
        // Was life before but wrong register. Split and schedule for
        // allocation (or assign directly).
...
void LinearScanAllocator::ReloadLiveRanges(RangeWithRegisterSet& to_be_live,
  ...
  for (RangeWithRegister range_with_register : to_be_live) {
    ...
    if (to_resurrect == nullptr) {
      ...
      UndoPreviousSplit(to_resurrect);
  ...
  while (
    ...
    if (FLAG_turbo_control_flow_aware_allocation) {
      ...
      if (position >= next_block_boundary) {
        ...
        // Change spilling mode in deferred blocks if the current range came to
        // life outside of a deferred block. If it came to life inside of
        // a deferred block, we can still spill at definition and avoid the
        // extra work. Also, the code that handles deferred spills assumes that
        // ranges come to life outside of deferred code.
        bool spill_deferred_only =
            current_block->IsDeferred() && (current != nullptr) &&
            !GetInstructionBlock(code(), current->TopLevel()->Start())
                 ->IsDeferred();
        SpillMode spill_mode = spill_deferred_only
        ...
        if (!fallthrough) {
          ...
          auto has_no_info = [this, current_block](RpoNumber pred) {
            return (pred > current_block->rpo_number()) ||
                   (!current_block->IsDeferred() &&
                    code()->InstructionBlockAt(pred)->IsDeferred());
          };
          ...
          if (current_block->PredecessorCount() == 1) {
            ...
            // here right away.
            if (has_no_info(current_block->predecessors()[0])) {
              no_change_required = pick_state_from(
                  current_block->predecessors()[1], &to_be_live);
            } else if (has_no_info(current_block->predecessors()[1])) {
              no_change_required = pick_state_from(
                  current_block->predecessors()[0], &to_be_live);
            } else {
              ...
              auto& left =
                  data()->GetSpillState(current_block->predecessors()[0]);
              auto& right =
                  data()->GetSpillState(current_block->predecessors()[1]);
              SmallRangeVector left_used;
              for (const auto item : left) {
                LiveRange* at_next_block =
                    item->TopLevel()->GetChildCovers(next_block_boundary);
                if (at_next_block != nullptr &&
                    at_next_block->NextUsePositionRegisterIsBeneficial(
                        next_block_boundary) != nullptr) {
              ...
              if (left_used.empty() && right_used.empty()) {
                ...
                for (const auto item : left) {
                  LiveRange* at_next_block =
                      item->TopLevel()->GetChildCovers(next_block_boundary);
                  if (at_next_block != nullptr &&
                      at_next_block->NextUsePosition(next_block_boundary) !=
                          nullptr) {
                    left_used.emplace_back(item->TopLevel());
                  }
                }
              ...
              TRACE("Vote went %zu vs %zu\n", left_used.size(),
                    right_used.size());
              // Pick the side with most ranges that matter.
              if (left_used.size() > right_used.size()) {
                no_change_required = pick_state_from(
                    current_block->predecessors()[0], &to_be_live);
              } else {
                no_change_required = pick_state_from(
            ...
            for (RpoNumber pred : current_block->predecessors()) {
              ...
              for (LiveRange* range : pred_state) {
                ...
                if (previous == counts.end()) {
                  auto result =
                      counts.emplace(std::make_pair(toplevel, Vote{1, {0}}));
                  CHECK(result.second);
                  result.first->second
                      .used_registers[range->assigned_register()]++;
                } else {
            ...
            auto assign_to_live =
                [this, counts, majority](
                    std::function<bool(TopLevelLiveRange*)> filter,
                    RangeWithRegisterSet* to_be_live, bool* taken_registers) {
                  for (const auto& val : counts) {
                    ...
                    if (val.second.count >= majority) {
                      ...
                      TRACE("Reset %d as live due vote %zu in %s\n",
                            ...
                            reg == kUnassignedRegister ? "unassigned"
                                                       : RegisterName(reg));
                    }
                  }
                };
            ...
            // conflicts.
            assign_to_live([](TopLevelLiveRange* r) { return r->IsFixed(); },
                           &to_be_live, taken_registers);
            // Second round, process the rest.
            assign_to_live([](TopLevelLiveRange* r) { return !r->IsFixed(); },
                           &to_be_live, taken_registers);
          }

          DCHECK_IMPLIES(no_change_required,
                         BlockIsDeferredOrImmediatePredecessorIsNotDeferred(
                             current_block));
          if (!no_change_required) {
            DropNotLiveRanges(to_be_live, next_block_boundary, spill_mode);
...
bool LinearScanAllocator::TryAllocateFreeReg(
  ...
  int reg = codes[0];
  if (current->RegisterFromControlFlow(&hint_reg) ||
...
void LinearScanAllocator::AllocateBlockedReg(LiveRange* current,
  ...
  int reg = codes[0];
  current->RegisterFromControlFlow(&reg) || register_use->HintRegister(&reg) ||
      current->RegisterFromBundle(&reg);
  for (int i = 0; i < num_codes; ++i) {
    int code = codes[i];
  ...
  if (spill_mode == SpillMode::kSpillDeferred) {
    ...
    while (deferred_block && deferred_block->IsDeferred()) {
      deferred_block = code()->GetInstructionBlock(
          deferred_block->last_instruction_index() + 1);
    }
    if (deferred_block) {
