  ...
  if (data()->is_turbo_control_flow_aware_allocation()) {
    while (loop_header != nullptr) {
      ...
      for (LiveRange* live_at_header : loop_header_state) {
        ...
        LifetimePosition start = loop_start;
        // Skip phi define use position which is register beneficial by default.
        if (check_use->TopLevel()->is_phi() &&
            check_use->TopLevel()->Start() == loop_start)
          start = loop_start.End();
        for (; check_use != nullptr && check_use->Start() < pos;
             check_use = check_use->next()) {
          const UsePosition* next_use =
              check_use->NextUsePositionRegisterIsBeneficial(start);
          if (next_use != nullptr) {
            // UsePosition at the end of a UseInterval may
            // have the same value as the start of next range.
            if ((start = next_use->pos()) <= pos) return pos;
          }
        }
