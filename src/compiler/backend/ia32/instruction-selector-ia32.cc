...
// information on the HeapConstant node already.
  ...
  enum MemoryOperandRegisterMode {
    kRegister,
    kUniqueRegister,
  };

  InstructionOperand UseRegisterWithMode(
      Node* node, MemoryOperandRegisterMode register_mode) {
  ...
  AddressingMode GenerateMemoryOperandInputs(
      ...
      size_t* input_count,
      MemoryOperandRegisterMode register_mode = kRegister) {
...
void InstructionSelector::VisitStackPointerGreaterThan(
  ...
  InstructionOperand temps[] = {g.TempRegister()};
  const int temp_count = (kind == StackCheckKind::kFunctionEntry) ? 1 : 0;
  const auto register_mode = (kind == StackCheckKind::kFunctionEntry)
                                 ? IA32OperandGenerator::kUniqueRegister
                                 : IA32OperandGenerator::kRegister;
