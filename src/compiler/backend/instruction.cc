...
bool InstructionOperand::InterferesWith(const InstructionOperand& other) const {
  const bool kComplexFPAliasing = kSimpleFPAliasing == AliasingKind::COMBINE &&
...
bool LocationOperand::IsCompatible(LocationOperand* op) {
  ...
  } else if (kSimpleFPAliasing != AliasingKind::COMBINE) {
...
void ParallelMove::PrepareInsertAfter(
  ...
  bool no_aliasing = kSimpleFPAliasing != AliasingKind::COMBINE ||
