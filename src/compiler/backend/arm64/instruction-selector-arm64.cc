...
void MaybeReplaceCmpZeroWithFlagSettingBinop(
  ...
  if constexpr (Adapter::IsTurboshaft) {
    ...
    } else if (op.Is<Opmask::kWord64BitwiseAnd>()) {
...
struct TestAndBranchMatcherTurboshaft {
  TestAndBranchMatcherTurboshaft(const turboshaft::Graph& graph,
                                 const turboshaft::WordBinopOp& binop)
      : graph_(graph), binop_(binop) {
    Initialize();
  }
 ...
 private:
  void Initialize() {
    ...
    // TODO(mliedtke): Use a constant view instead?
    const ConstantOp* constant =
        graph_.Get(binop_.right()).TryCast<ConstantOp>();
    if (constant == nullptr) return;
    uint64_t value;
    if (binop_.rep == WordRepresentation::Word32()) {
      // It is safe to zero-extend here as we only care about the ctz for non
      // zero values.
      value = constant->word32();
    } else if (binop_.rep == WordRepresentation::Word64()) {
      value = constant->word64();
    } else
      return;
    if (!base::bits::IsPowerOfTwo(value)) return;
...
void InstructionSelectorT<TurboshaftAdapter>::VisitWordCompareZero(
  ...
  if (cont->IsBranch()) {
        ...
        if (this->MatchIntegralZero(equal->right())) {
          ...
          if (left_binop) {
            ...
            if (matcher.Matches()) {
              ...
              cont->OverwriteAndNegateIfEqual(kEqual);
              this->EmitWithContinuation(
                  kArm64TestAndBranch, gen.UseRegister(left_binop->left()),
    ...
    if (const WordBinopOp* value_binop = value_op.TryCast<WordBinopOp>()) {
      TestAndBranchMatcherTurboshaft matcher(*turboshaft_graph(), *value_binop);
      if (matcher.Matches()) {
        // If the mask has only one bit set, we can use tbz/tbnz.
        DCHECK((cont->condition() == kEqual) ||
               (cont->condition() == kNotEqual));
        InstructionCode opcode =
            value_binop->rep == RegisterRepresentation::Word32()
                ...
                : kArm64TestAndBranch;
        Arm64OperandGeneratorT<TurboshaftAdapter> gen(this);
        this->EmitWithContinuation(opcode, gen.UseRegister(value_binop->left()),
                                   gen.TempImmediate(matcher.bit()), cont);
        return;
      }
    }
          ...
          if (const OverflowCheckedBinopOp* binop =
            ...
            switch (binop->kind) {
              case OverflowCheckedBinopOp::Kind::kSignedAdd:
                cont->OverwriteAndNegateIfEqual(kOverflow);
                UNIMPLEMENTED();
                // return VisitBinop(this, node, is64 ? kArm64Add : kArm64Add32,
                // kArithmeticImm, cont);
              case OverflowCheckedBinopOp::Kind::kSignedSub:
                cont->OverwriteAndNegateIfEqual(kOverflow);
                UNIMPLEMENTED();
                // return VisitBinop(this, node, is64 ? kArm64Sub : kArm64Sub32,
                // kArithmeticImm, cont);
