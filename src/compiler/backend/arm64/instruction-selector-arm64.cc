...
template <int N>
struct CbzOrTbzMatchInfo {};

template <>
struct CbzOrTbzMatchInfo<32> {
...
bool TryEmitCbzOrTbz(InstructionSelector* selector, Node* node,
                     typename CbzOrTbzMatchInfo<N>::IntegralType value,
  ...
  switch (cond) {
    ...
    case kSignedGreaterThanOrEqual: {
      ...
      selector->EmitWithContinuation(
          CbzOrTbzMatchInfo<N>::kTestAndBranchOpcode, g.UseRegister(node),
          g.TempImmediate(CbzOrTbzMatchInfo<N>::kSignBit), cont);
      return true;
    }
    case kEqual:
    case kNotEqual: {
      if (node->opcode() == CbzOrTbzMatchInfo<N>::kAndOpcode) {
        // Emit a tbz/tbnz if we are comparing with a single-bit mask:
        //   Branch(WordEqual(WordAnd(x, 1 << N), 1 << N), true, false)
        typename CbzOrTbzMatchInfo<N>::BinopMatcher m_and(node);
        if (cont->IsBranch() && base::bits::IsPowerOfTwo(value) &&
          ...
          cont->Negate();
          selector->EmitWithContinuation(
              CbzOrTbzMatchInfo<N>::kTestAndBranchOpcode,
    ...
    case kUnsignedGreaterThan: {
      ...
      EmitBranchOrDeoptimize(selector,
                             CbzOrTbzMatchInfo<N>::kCompareAndBranchOpcode,
