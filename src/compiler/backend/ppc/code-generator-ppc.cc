...
#define MAYBE_REVERE_IF_WASM(dst, src, op, reset) \
...
#define ASSEMBLE_ATOMIC_EXCHANGE(_type, reverse_op)                    \
  do {                                                                 \
    ...
    MAYBE_REVERE_IF_WASM(ip, val, reverse_op, true);                   \
    __ AtomicExchange<_type>(                                          \
        MemOperand(i.InputRegister(0), i.InputRegister(1)), val, dst); \
    MAYBE_REVERE_IF_WASM(dst, dst, reverse_op, false);                 \
  } while (false)
...
#define ASSEMBLE_ATOMIC_COMPARE_EXCHANGE(_type, reverse_op)               \
  do {                                                                    \
    ...
    Register dst = i.OutputRegister();                                    \
    MAYBE_REVERE_IF_WASM(ip, expected_val, reverse_op, true);             \
    MAYBE_REVERE_IF_WASM(r0, new_val, reverse_op, true);                  \
