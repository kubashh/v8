...
void VisitStoreCommon(InstructionSelector* selector, Node* node,
  ...
  // It is atomic if atomic_order is set (kAcqRel or kSeqCst).
  bool is_atomic = static_cast<bool>(atomic_order);
  ...
  if (write_barrier_kind != kNoWriteBarrier && !FLAG_disable_write_barriers) {
    ...
    InstructionCode code = is_atomic ? kArchAtomicStoreWithWriteBarrier
