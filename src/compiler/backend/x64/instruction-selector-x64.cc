...
class X64OperandGeneratorT final : public OperandGeneratorT<Adapter> {
 public:
  ...
  template <typename T>
  bool CanBeMemoryOperand(InstructionCode, T*, T*, int) {
    UNREACHABLE(/*REMOVE*/);
  }
  ...
  template <typename T>
  bool CanBeBetterLeftOperand(T*) const {
    UNREACHABLE(/*REMOVE*/);
  }
...
void VisitCompareZero(InstructionSelectorT<TurboshaftAdapter>* selector,
  ...
  if (cont->IsBranch() &&
    ...
    if (const auto binop = op.TryCast<turboshaft::WordBinopOp>()) {
      ...
      // #define TRY_VISIT_WORD32_SHIFT TryVisitWordShift<TurbofanAdapter,
      // Int32BinopMatcher, 32> #define TRY_VISIT_WORD64_SHIFT
      // TryVisitWordShift<TurbofanAdapter, Int64BinopMatcher, 64>
      //// Skip Word64Sar/Word32Sar since no instruction reduction in most
      /// cases.
      // #d efine FLAGS_SET_SHIFT_LIST(V)                    \
...
//  V(kWord64Shr, TRY_VISIT_WORD64_SHIFT, kX64Shr)
      // #d ef in e FLAGS_SET_SHIFT(opcode, TryVisit, archOpcode)         \
