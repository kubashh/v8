...
void InstructionSelectorT<Adapter>::VisitInt32Add(node_t node) {
  ...
  if constexpr (Adapter::IsTurbofan) {
    ...
    if (const turboshaft::ChangeOp* change =
            this->Get(left).template TryCast<turboshaft::ChangeOp>();
        change &&
        change->from == turboshaft::RegisterRepresentation::Word64() &&
        change->to == turboshaft::RegisterRepresentation::Word32()) {
      left = change->input();
    }
    if (const turboshaft::ChangeOp* change =
            this->Get(right).template TryCast<turboshaft::ChangeOp>();
        change &&
        change->from == turboshaft::RegisterRepresentation::Word64() &&
        change->to == turboshaft::RegisterRepresentation::Word32()) {
      right = change->input();
