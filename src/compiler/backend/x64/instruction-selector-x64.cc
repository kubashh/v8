...
void InstructionSelectorT<TurboshaftAdapter>::VisitWordCompareZero(
  ...
  using namespace turboshaft;  // NOLINT(build/namespaces)
  {
    const ComparisonOp* comparison = this->TryCast<ComparisonOp>(value);
    while (comparison != nullptr &&
           comparison->kind == ComparisonOp::Kind::kEqual &&
           CanCover(user, value)) {
      const ConstantOp* right_constant =
          this->TryCast<ConstantOp>(comparison->right());
      ...
      cont->Negate();
      comparison = this->TryCast<ComparisonOp>(value);
    }
  }
