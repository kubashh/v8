...
#ifdef V8_IS_TSAN
class OutOfLineTSANStore final : public OutOfLineCode {
 public:
  ...
  void Generate() final {
    ...
    if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
      ...
      // Just encode the stub index. This will be patched when the code
      // is added to the native module and copied into wasm code space.
      __ CallTSANStoreStub(scratch0_, value_, save_fp_mode, size_, order_,
                           StubCallMode::kCallWasmRuntimeStub);
      return;
    }
#endif  // V8_ENABLE_WEBASSEMBLY
...
class OutOfLineTSANLoad final : public OutOfLineCode {
 public:
  OutOfLineTSANLoad(CodeGenerator* gen, Operand operand, Register scratch0,
                    StubCallMode stub_mode, int size, std::memory_order order)
      : OutOfLineCode(gen),
        operand_(operand),
        scratch0_(scratch0),
#if V8_ENABLE_WEBASSEMBLY
  ...
  void Generate() final {
    ...
    if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
      ...
      // Just encode the stub index. This will be patched when the code
      // is added to the native module and copied into wasm code space.
      __ CallTSANLoadStub(scratch0_, save_fp_mode, size_, order_,
                          StubCallMode::kCallWasmRuntimeStub);
      return;
    }
#endif  // V8_ENABLE_WEBASSEMBLY
...
void EmitTSANLoadOOLIfNeeded(Zone* zone, CodeGenerator* codegen,
  ...
  Register scratch0 = i.TempRegister(0);
  auto tsan_ool = zone->New<OutOfLineTSANLoad>(codegen, operand, scratch0, mode,
                                               size, order);
  tasm->jmp(tsan_ool->entry());
  tasm->bind(tsan_ool->exit());
}
