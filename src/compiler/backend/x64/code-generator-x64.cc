...
#define ASSEMBLE_SIMD_PUNPCK_SHUFFLE(opcode)                  \
  do {                                                        \
    ...
    if (instr->InputAt(input_index)->IsSimd128Register()) {   \
      XMMRegister src1 = i.InputSimd128Register(input_index); \
      __ opcode(i.OutputSimd128Register(), src1);             \
    } else {                                                  \
      Operand src1 = i.InputOperand(input_index);             \
      __ opcode(i.OutputSimd128Register(), src1);             \
    }                                                         \
  } while (false)
  ...
  switch (arch_opcode) {
    ...
    case kX64S8x16Shuffle: {
      ...
      if (instr->InputCount() == 5) {  // only one input operand
        ...
        int64_t shuffle_mask =
            (mask[2]) | (static_cast<uint64_t>(mask[3]) << 32);
        __ movq(kScratchRegister, shuffle_mask);
        ...
        if (instr->InputAt(0)->IsSimd128Register()) {
          __ movups(kScratchDoubleReg, i.InputSimd128Register(0));
        } else {
          __ movups(kScratchDoubleReg, i.InputOperand(0));
        }
        uint32_t mask[4] = {};
        ...
        shuffle_mask = (mask1[2]) | (static_cast<uint64_t>(mask1[3]) << 32);
        __ movq(kScratchRegister, shuffle_mask);
        __ Push(kScratchRegister);
        shuffle_mask = (mask1[0]) | (static_cast<uint64_t>(mask1[1]) << 32);
        __ movq(kScratchRegister, shuffle_mask);
        __ Push(kScratchRegister);
        __ pshufb(dst, Operand(rsp, 0));
    ...
    case kX64S32x4Swizzle: {
      ...
      if (instr->InputAt(0)->IsSimd128Register()) {
        __ pshufd(i.OutputSimd128Register(), i.InputSimd128Register(0),
                  i.InputInt8(1));
      } else {
    ...
    case kX64S32x4Shuffle: {
      ...
      if (instr->InputAt(1)->IsSimd128Register()) {
        __ pshufd(kScratchDoubleReg, i.InputSimd128Register(1), shuffle);
      } else {
    ...
    case kX64S16x8HalfShuffle1: {
      XMMRegister dst = i.OutputSimd128Register();
      if (instr->InputAt(0)->IsSimd128Register()) {
        __ pshuflw(dst, i.InputSimd128Register(0), i.InputInt8(1));
      } else {
    ...
    case kX64S16x8HalfShuffle2: {
      ...
      XMMRegister dst = i.OutputSimd128Register();
      if (instr->InputAt(1)->IsSimd128Register()) {
        __ pshuflw(kScratchDoubleReg, i.InputSimd128Register(1),
                   i.InputInt8(2));
      } else {
      ...
      if (instr->InputAt(0)->IsSimd128Register()) {
        __ pshuflw(dst, i.InputSimd128Register(0), i.InputInt8(2));
      } else {
    ...
    case kX64S16x8Dup: {
      ...
      if (lane < 4) {
        if (instr->InputAt(0)->IsSimd128Register()) {
          __ pshuflw(dst, i.InputSimd128Register(0), half_dup);
        } else {
          __ pshuflw(dst, i.InputOperand(0), half_dup);
        }
        __ pshufd(dst, dst, 0);
      } else {
        if (instr->InputAt(0)->IsSimd128Register()) {
          __ pshufhw(dst, i.InputSimd128Register(0), half_dup);
        } else {
          __ pshufhw(dst, i.InputOperand(0), half_dup);
        }
    ...
    case kX64S16x8UnzipHigh: {
      ...
      if (instr->InputCount() == 2) {
        if (instr->InputAt(1)->IsSimd128Register()) {
          __ movups(kScratchDoubleReg, i.InputSimd128Register(1));
        } else {
    ...
    case kX64S16x8UnzipLow: {
      ...
      if (instr->InputCount() == 2) {
        if (instr->InputAt(1)->IsSimd128Register()) {
          __ pblendw(kScratchDoubleReg, i.InputSimd128Register(1), 0x55);
        } else {
