...
class OutOfLineTSANReleaseStore final : public OutOfLineCode {
 public:
  OutOfLineTSANReleaseStore(CodeGenerator* gen, Operand operand, Register value,
                            Register scratch0, StubCallMode stub_mode, int size)
      : OutOfLineCode(gen),
        operand_(operand),
        value_(value),
  ...
  void Generate() final {
    ...
    if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
      ...
      // Just encode the stub index. This will be patched when the code
      // is added to the native module and copied into wasm code space.
      __ CallTSANReleaseStoreStub(scratch0_, value_, save_fp_mode, size_,
                                  StubCallMode::kCallWasmRuntimeStub);
      return;
    }
#endif  // V8_ENABLE_WEBASSEMBLY
...
void EmitTSANReleaseStoreOOLIfNeeded(Zone* zone, CodeGenerator* codegen,
                                     TurboAssembler* tasm, Operand operand,
                                     Register value_reg, X64OperandConverter& i,
                                     StubCallMode mode, int size) {
  // The FOR_TESTING code doesn't initialize the root register. We can't call
  ...
  Register scratch0 = i.TempRegister(0);
  auto tsan_ool = zone->New<OutOfLineTSANReleaseStore>(
      codegen, operand, value_reg, scratch0, mode, size);
  tasm->jmp(tsan_ool->entry());
  tasm->bind(tsan_ool->exit());
}
...
void EmitTSANReleaseStoreOOLIfNeeded(Zone* zone, CodeGenerator* codegen,
  ...
  Register value_reg = i.TempRegister(1);
  tasm->movq(value_reg, value);
  EmitTSANReleaseStoreOOLIfNeeded(zone, codegen, tasm, operand, value_reg, i,
                                  mode, size);
}
  ...
  switch (arch_opcode) {
    ...
    case kX64Movb: {
      ...
      if (HasImmediateInput(instr, index)) {
        ...
        // TODO(solanes): This should be EmitTSANRelaxedStoreOOLIfNeeded. It is
        // EmitTSANReleaseStoreOOLIfNeeded so that the functions will not be
        // "unused" and therefore it will compile.
        EmitTSANReleaseStoreOOLIfNeeded(zone(), this, tasm(), operand, value, i,
