  ...
  for (const Instruction* instr : sequence->instructions()) {
    ...
    for (size_t i = 0; i < instr->OutputCount(); ++i, ++count) {
      ...
      if (op_constraints[count].type_ == kSameAsInput) {
        CHECK_LT(op_constraints[count].value_, instr->InputCount());
        op_constraints[count].type_ =
            op_constraints[op_constraints[count].value_].type_;
        op_constraints[count].value_ =
            op_constraints[op_constraints[count].value_].value_;
      }
