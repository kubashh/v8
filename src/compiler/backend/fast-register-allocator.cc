...
class SinglePassRegisterAllocator final {
 public:
  ...
  RegisterIndex FromRegCode(int reg_code) const;
  int ToRegCode(RegisterIndex index) const;
 ...
 private:
  ...
  // |instr_index|. The register |reg| will be reserved for this use for the
  // specified |pos| use position.
  void AllocatePendingUse(RegisterIndex reg, int virtual_register,
                          InstructionOperand* operand, int instr_index,
                          UsePosition pos);

  // Allocate a use of the existing register holding |virtual_register| for a
  // gap move to |reg| for for |operand| of the instruction at  |instr_index|.
  // Both |reg| and the existing register holding |virtual_register| will be
  ...
  V8_INLINE RegisterIndex GetRegisterFor(UsePosition pos,
                                         bool must_use_register);
  V8_INLINE RegisterIndex GetFreeRegister(UsePosition pos);
  V8_INLINE RegisterIndex GetUnblockedRegister(UsePosition pos);
...
RegisterIndex SinglePassRegisterAllocator::FromRegCode(int reg_code) const {
  return RegisterIndex(reg_code_to_index_[reg_code]);
}

int SinglePassRegisterAllocator::ToRegCode(RegisterIndex reg) const {
...
void SinglePassRegisterAllocator::AllocateRegister(RegisterIndex reg,
                                                   int virtual_register,
                                                   UsePosition pos) {
  assigned_registers()->Add(ToRegCode(reg));
...
void SinglePassRegisterAllocator::FreeRegister(RegisterIndex reg,
  ...
  allocated_registers_bits_ &= ~reg.ToBit();
  if (virtual_register != InstructionOperand::kInvalidVirtualRegister) {
    virtual_register_to_reg_[virtual_register] = {};
  }
}
...
RegisterIndex SinglePassRegisterAllocator::GetRegisterFor(
  ...
  if (!reg.is_valid() &&
    ...
    reg = GetRegisterFor(pos, must_use_register);
  }
  return reg;
}
...
RegisterIndex SinglePassRegisterAllocator::GetRegisterFor(
  ...
  RegisterIndex reg = GetFreeRegister(pos);
  if (!reg.is_valid() && must_use_register) {
    reg = GetUnblockedRegister(pos);
    SpillRegister(reg);
  }
  return reg;
}
...
RegisterIndex SinglePassRegisterAllocator::GetFreeRegister(UsePosition pos) {
  ...
  if (reg_index >= num_allocatable_registers()) return {};
...
AllocatedOperand SinglePassRegisterAllocator::AllocatedOperandForReg(
  ...
  return AllocatedOperand(AllocatedOperand::REGISTER, rep, ToRegCode(reg));
}
...
void SinglePassRegisterAllocator::AllocateUseWithMove(
  ...
  MoveOperands* move_ops =
      data()->AddGapMove(instr_index, Instruction::END, *operand, allocated);
  InstructionOperand::ReplaceWith(operand, &allocated);

  register_state()->AllocatePendingUse(existing_reg, virtual_register,
                                       &move_ops->source(), instr_index);
  AllocateRegister(existing_reg, virtual_register, pos);
  MarkRegisterUse(reg, pos);
  CheckConsistency();
}
...
void SinglePassRegisterAllocator::AllocateInput(UnallocatedOperand* operand,
  ...
  if (operand->HasFixedRegisterPolicy() ||
    ...
    RegisterIndex reg = FromRegCode(operand->fixed_register_index());
    ...
    if (reg.is_valid()) {
      if (must_use_register) {
        ...
        AllocatePendingUse(reg, virtual_register, operand, instr_index, pos);
      }
    } else {
      vreg_data.SpillOperand(operand, instr_index, data());
    }
...
void SinglePassRegisterAllocator::AllocateSameInputOutput(
  ...
  if (reg.is_valid()) {
    if (!VirtualRegisterIsUnallocatedOrInReg(input_vreg, reg)) {
      SpillRegisterForVirtualRegister(input_vreg);
    }
...
void SinglePassRegisterAllocator::AllocateTemp(UnallocatedOperand* operand,
  ...
  if (operand->HasSlotPolicy()) {
    ...
    reg = GetRegisterFor(UsePosition::kAll, operand->HasRegisterPolicy());
  }
...
void SinglePassRegisterAllocator::ReserveFixedRegister(
  ...
  EnsureRegisterState();
  RegisterIndex reg = FromRegCode(operand->fixed_register_index());
  if (!IsFreeOrSameVirtualRegister(reg, operand->virtual_register()) &&
      !DefinedAfter(operand->virtual_register(), instr_index, pos)) {
...
void SinglePassRegisterAllocator::AllocatePhiGapMove(int to_vreg, int from_vreg,
  ...
  if (to_register.is_valid() && !from_register.is_valid()) {
    ...
    if (!from_register.is_valid()) {
      from_register = GetFreeRegister(UsePosition::kAll);
    }
    if (from_register.is_valid()) {
