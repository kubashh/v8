...
class BlockState final {
 public:
  BlockState(int block_count, Zone* zone)
      : general_registers_out_state_(nullptr),
        double_registers_out_state_(nullptr),
        dominates_blocks_(block_count, zone),
        successors_phi_index_(-1) {}

  // Returns the RegisterState that applies to the output of this block. Can be
  // |nullptr| if no successor block has been allocated.
  RegisterState* register_out_state(RegisterKind kind);
  void set_register_out_state(RegisterState* register_state, RegisterKind kind);
  ...
  void set_successors_phi_index(int index) { successors_phi_index_ = index; }

  MOVE_ONLY_NO_DEFAULT_CONSTRUCTOR(BlockState);

 private:
  RegisterState* general_registers_out_state_;
  RegisterState* double_registers_out_state_;

  BitVector dominates_blocks_;
  int successors_phi_index_;
};

RegisterState* BlockState::register_out_state(RegisterKind kind) {
  switch (kind) {
    case RegisterKind::kGeneral:
      return general_registers_out_state_;
    case RegisterKind::kDouble:
      return double_registers_out_state_;
  }
}

void BlockState::set_register_out_state(RegisterState* register_state,
                                        RegisterKind kind) {
  switch (kind) {
    case RegisterKind::kGeneral:
      DCHECK_NULL(general_registers_out_state_);
      general_registers_out_state_ = register_state;
      break;
    case RegisterKind::kDouble:
      DCHECK_NULL(double_registers_out_state_);
      double_registers_out_state_ = register_state;
      break;
  }
}

FastRegisterAllocatorData::FastRegisterAllocatorData(
    const RegisterConfiguration* config, Zone* zone, Frame* frame,
    InstructionSequence* code, TickCounter* tick_counter,
    const char* debug_name)
    : RegisterAllocationData(Type::kFastRegisterAllocation),
...
MoveOperands* FastRegisterAllocatorData::AddGapMove(
...
MoveOperands* FastRegisterAllocatorData::AddPendingOperandGapMove(
    int instr_index, Instruction::GapPosition position) {
  Instruction* instr = code()->InstructionAt(instr_index);
  ParallelMove* moves = instr->GetOrCreateParallelMove(position, code_zone());
...
MachineRepresentation FastRegisterAllocatorData::RepresentationFor(
...
BlockState& FastRegisterAllocatorData::block_state(RpoNumber rpo_number) {
  return block_state_[rpo_number.ToInt()];
}

const InstructionBlock* FastRegisterAllocatorData::GetBlock(
    RpoNumber rpo_number) {
  return code()->InstructionBlockAt(rpo_number);
}

const InstructionBlock* FastRegisterAllocatorData::GetBlock(int instr_index) {
  return code()->InstructionAt(instr_index)->block();
}

const BitVector* FastRegisterAllocatorData::GetBlocksDominatedBy(
    int instr_index) {
...
class RegisterIndex final {
 public:
  ...
  int ToInt() const { return index_; }
  uintptr_t ToBit() const { return 1u << ToInt(); }
...
class VirtualRegisterData final {
 public:
  ...
  void SpillOperand(InstructionOperand* operand, int instr_index,
                    FastRegisterAllocatorData* data);
  ...
  class SpillRange : public ZoneObject {
   public:
    SpillRange(int definition_instr_index, FastRegisterAllocatorData* data)
 ...
 private:
  ...
  void AddSpillUse(int instr_index, FastRegisterAllocatorData* data);
  void EnsureSpillRange(FastRegisterAllocatorData* data);
...
VirtualRegisterData& FastRegisterAllocatorData::VirtualRegisterDataFor(
    int virtual_register) {
...
void VirtualRegisterData::EnsureSpillRange(FastRegisterAllocatorData* data) {
  DCHECK(!is_constant());
...
void VirtualRegisterData::EmitGapMoveFromOutputToSpillSlot(
    ...
    int instr_index, FastRegisterAllocatorData* data) {
...
// point in program execution. The RegisterState cam be cloned or merged with
...
class RegisterState final : public ZoneObject {
 public:
  ...
  void Commit(RegisterIndex reg, AllocatedOperand allocated,
              InstructionOperand* operand, FastRegisterAllocatorData* data);

  // Spill the contents of |reg| for an instruction in |current_block| using
  // the |allocated| to commit the spill gap move.
  ...
  // Returns true if |reg| only has pending use allocated to it.
 ...
 private:
  ...
  class Register final : public ZoneObject {
   public:
    ...
    int instr_index() const { return instr_index_; }
   ...
   private:
    void SpillPhiGapMove(AllocatedOperand allocated_op,
                         ...
                         FastRegisterAllocatorData* data);
    void SpillPendingUses(FastRegisterAllocatorData* data);

    bool is_shared_ : 1;
    bool is_phi_gap_move_ : 1;
    bool needs_gap_move_on_spill_ : 1;
...
void RegisterState::Register::PendingUse(InstructionOperand* operand,
  ...
  DCHECK_GE(instr_index_, instr_index);
...
void RegisterState::Register::AddSharedUse() {
  is_shared_ = true;
...
void RegisterState::Register::Spill(AllocatedOperand allocated_op,
  ...
  if (needs_gap_move_on_spill()) {
    ...
    vreg_data.EmitGapMoveToInputFromSpillSlot(allocated_op, instr_index(),
                                              data);
  }
...
void RegisterState::Register::SpillPhiGapMove(
  ...
  // assuming the phi wouldn't spill.
  VirtualRegisterData& vreg_data =
      data->VirtualRegisterDataFor(virtual_register());
...
void RegisterState::Register::SpillPendingUses(
  ...
  while (pending_use) {
    ...
    PendingOperand* next = pending_use->next();
    vreg_data.SpillOperand(pending_use, instr_index(), data);
    pending_use = next;
  }
  pending_uses_ = nullptr;
}
...
class SinglePassRegisterAllocator final {
 public:
  ...
  enum class UsePosition {
    ...
    kAll,
    // Operand is not used in the instruction (used t).
    kNone,
  };
  ...
  // Clone the current register state into the register_out_state of
  // |predecessors|.
  void CloneStateInto(const InstructionBlock::Predecessors& predecessors);
  // Merge our current register state into the register_out_state of
  // |predecessor|.
  void MergeStateInto(RpoNumber predecessor);
  ...
  // register is no subsiquently spilled) for |operand| of the instruction at
  // |instr_index|.
  void AllocatePendingUse(RegisterIndex reg, int virtual_register,
                          InstructionOperand* operand, int instr_index);
  ...
  V8_INLINE RegisterIndex GetRegisterFor(VirtualRegisterData& virtual_register,
                                         UsePosition pos,
                                         bool must_use_register);
  V8_INLINE RegisterIndex GetRegisterFor(MachineRepresentation rep,
                                         UsePosition pos,
                                         bool must_use_register);
  V8_INLINE RegisterIndex GetFreeRegister(MachineRepresentation rep,
                                          UsePosition pos);
  V8_INLINE RegisterIndex GetUnblockedRegister(MachineRepresentation rep,
                                               UsePosition pos);

  // Allocate, free and mark use's of |reg| for a |virtual_register| at use
  // position |pos|.
  V8_INLINE void AllocateRegister(RegisterIndex reg, int virtual_register,
                                  UsePosition pos);
  V8_INLINE void FreeRegister(RegisterIndex reg, int virtual_register);
  V8_INLINE void MarkRegisterUse(RegisterIndex reg, UsePosition pos);
  V8_INLINE uintptr_t InUseBitmap(UsePosition pos);
  V8_INLINE uintptr_t MaskForRepresentation(uintptr_t reg_bits,
  ...
  int float_to_simd128_transition_index_;
  uintptr_t float32_register_mask_;
  base::Optional<ZoneVector<RegisterIndex>> float32_reg_code_to_index_;
  const int* index_to_float32_reg_code_;
  uintptr_t simd128_register_mask_;
  base::Optional<ZoneVector<RegisterIndex>> simd128_reg_code_to_index_;
  const int* index_to_simd128_reg_code_;
};

SinglePassRegisterAllocator::SinglePassRegisterAllocator(
    RegisterKind kind, FastRegisterAllocatorData* data)
    : virtual_register_to_reg_(data->code()->VirtualRegisterCount(),
      ...
      float_to_simd128_transition_index_(0),
      float32_register_mask_(0),
      index_to_float32_reg_code_(nullptr),
      simd128_register_mask_(0),
      index_to_simd128_reg_code_(nullptr) {
  ...
  if (kind == RegisterKind::kDouble && !kSimpleFPAliasing) {
    const RegisterConfiguration* config = data->config();
    float_to_simd128_transition_index_ =
        config->GetFloatToSimd128TransitionIndex();

    //  Float registers.
    index_to_float32_reg_code_ = config->allocatable_float_codes();
    float32_reg_code_to_index_.emplace(config->num_float_registers(),
                                       data->allocation_zone());
    float32_register_mask_ = -1;
    for (int i = 0; i < config->num_allocatable_float_registers(); i++) {
      RegisterIndex reg(i);
      int reg_code = index_to_float32_reg_code_[i];
      float32_reg_code_to_index_->at(reg_code) = reg;
      float32_register_mask_ &= ~reg.ToBit();
    }

    //  Simd128 registers.
    index_to_simd128_reg_code_ = config->allocatable_simd128_codes();
    simd128_reg_code_to_index_.emplace(config->num_float_registers(),
                                       data->allocation_zone());
    simd128_register_mask_ = -1;
    for (int i = 0; i < config->num_allocatable_simd128_registers(); i++) {
      RegisterIndex reg(float_to_simd128_transition_index_ + i);
      int reg_code = index_to_simd128_reg_code_[i];
      simd128_reg_code_to_index_->at(reg_code) = reg;
      simd128_register_mask_ &= ~reg.ToBit();
    }
  }
}
...
void SinglePassRegisterAllocator::StartBlock(const InstructionBlock* block) {
  ...
  // Get our starting register state from the block's register_out_state.
  BlockState& block_state = data()->block_state(block->rpo_number());
  register_state_ = block_state.register_out_state(kind());
  ...
  for (RegisterIndex reg : *register_state()) {
    ...
    if (virtual_register != InstructionOperand::kInvalidVirtualRegister) {
      AllocateRegister(reg, virtual_register, UsePosition::kNone);
    }
  }
  CheckConsistency();
}
...
void SinglePassRegisterAllocator::EndBlock(const InstructionBlock* block) {
  ...
  if (block->PredecessorCount() == 1) {
    // If we have only a single predecesor, merge our state into that
    // predecessor.
    MergeStateInto(block->predecessors()[0]);
  } else {
    // Since we know the graph is in edge-split form, if we have multiple
    // predecessors, then we must be the only successor to those blocks, and so
    // can clone our state directly into each of the predecessors.
    CloneStateInto(block->predecessors());
  }
...
void SinglePassRegisterAllocator::CloneStateInto(
    const InstructionBlock::Predecessors& predecessors) {
  bool first_predecessor = true;
  for (RpoNumber predecessor : predecessors) {
    BlockState& block_state = data()->block_state(predecessor);
    if (first_predecessor) {
      // Set the state of the first predecessor to be the same as ours. This
      // avoids cloning for single predecessor blocks.
      block_state.set_register_out_state(register_state(), kind());
      first_predecessor = false;
    } else {
      block_state.set_register_out_state(register_state()->Clone(), kind());
    }
  }
}

void SinglePassRegisterAllocator::MergeStateInto(RpoNumber predecessor) {
  BlockState& block_state = data()->block_state(predecessor);
  RegisterState* predecessor_registers = block_state.register_out_state(kind());

  if (predecessor_registers == nullptr) {
    // If predecessor doesn't have any register state merged yet, set it to this
    // block's state.
    block_state.set_register_out_state(register_state(), kind());
  } else {
    // Otherwise try to merge our state with the existing state.
    for (RegisterIndex reg : *register_state()) {
      if (predecessor_registers->IsAllocated(reg)) {
        if (predecessor_registers->Equals(reg, register_state())) {
          // Both match, keep the target's register data.
          predecessor_registers->CommitAtMerge(reg);
        } else {
          // TODO(rmcilroy) consider adding a gap move to shuffle register into
          // the same as the target. For now just spill.
          SpillRegister(reg);
          // We also need to spill any register in our state that matches the
          // target state's virtual register.
          SpillRegisterForVirtualRegister(
              predecessor_registers->VirtualRegisterForRegister(reg));
        }
      } else if (register_state()->IsAllocated(reg)) {
        // Register is free in target state, so merge our register details into
        // it.
        predecessor_registers->CopyFrom(reg, register_state());
      }
    }
  }
}
...
#ifdef DEBUG
  for (int vreg = 0; vreg < data()->code()->VirtualRegisterCount(); vreg++) {
    RegisterIndex reg = RegisterForVirtualRegister(vreg);
    if (reg.is_valid()) {
      CHECK_EQ(vreg, VirtualRegisterForRegister(reg));
      CHECK(allocated_registers_bits_ & reg.ToBit());
    }
  }
...
int SinglePassRegisterAllocator::ToRegCode(RegisterIndex reg,
  ...
  if (!kSimpleFPAliasing && kind() == RegisterKind::kDouble) {
    if (rep == MachineRepresentation::kFloat32) {
      DCHECK_GE(reg.ToInt(), 0);
      DCHECK_LT(reg.ToInt(),
                data()->config()->num_allocatable_float_registers());
      return index_to_float32_reg_code_[reg.ToInt()];
    } else if (rep == MachineRepresentation::kSimd128) {
      int simd128_index = reg.ToInt() - float_to_simd128_transition_index_;
      DCHECK_GE(simd128_index, 0);
      DCHECK_LT(simd128_index,
                data()->config()->num_allocatable_simd128_registers());
      return index_to_simd128_reg_code_[simd128_index];
    }
  }
...
void SinglePassRegisterAllocator::AllocateRegister(RegisterIndex reg,
                                                   int virtual_register,
                                                   UsePosition pos) {
  assigned_registers()->Add(
      ToRegCode(reg, MachineRepresentation::kNone));  // FIXME!!
  MarkRegisterUse(reg, pos);
  allocated_registers_bits_ |= reg.ToBit();
  if (virtual_register != InstructionOperand::kInvalidVirtualRegister) {
...
void SinglePassRegisterAllocator::MarkRegisterUse(RegisterIndex reg,
                                                  UsePosition pos) {
  // if (pos != UsePosition::kNone) {
  if (pos == UsePosition::kStart || pos == UsePosition::kAll) {
    in_use_at_instr_start_bits_ |= reg.ToBit();
  }
  if (pos == UsePosition::kEnd || pos == UsePosition::kAll) {
    in_use_at_instr_end_bits_ |= reg.ToBit();
  }
}
...
RegisterIndex SinglePassRegisterAllocator::GetRegisterFor(
  ...
  if (!reg.is_valid() &&
    ...
    reg = GetRegisterFor(RepresentationFor(virtual_register.vreg()), pos,
                         must_use_register);
  }
  return reg;
}
...
RegisterIndex SinglePassRegisterAllocator::GetRegisterFor(
  ...
  RegisterIndex reg = GetFreeRegister(rep, pos);
  if (!reg.is_valid() && must_use_register) {
    reg = GetUnblockedRegister(rep, pos);
    SpillRegister(reg);
  }
  return reg;
}
...
uintptr_t SinglePassRegisterAllocator::MaskForRepresentation(
    uintptr_t reg_bits, MachineRepresentation rep) {
  if (!kSimpleFPAliasing && kind() != RegisterKind::kDouble) return reg_bits;
  switch (rep) {
    case MachineRepresentation::kFloat32:
      return reg_bits | float32_register_mask_;
    case MachineRepresentation::kFloat64:
      return reg_bits;
    case MachineRepresentation::kSimd128:
      return reg_bits | simd128_register_mask_;
    default:
      UNREACHABLE();
  }
}

RegisterIndex SinglePassRegisterAllocator::GetFreeRegister(
  ...
  allocated_or_in_use = MaskForRepresentation(allocated_or_in_use, rep);
  int reg_index = base::bits::CountTrailingZeros(~allocated_or_in_use);
  if (reg_index >= num_allocatable_registers()) return RegisterIndex::Invalid();
  return RegisterIndex(reg_index);
}

RegisterIndex SinglePassRegisterAllocator::GetUnblockedRegister(
    MachineRepresentation rep, UsePosition pos) {
  uintptr_t in_use = InUseBitmap(pos);
  in_use = MaskForRepresentation(in_use, rep);

  // Choose a register that will need to be spilled. Preferentially choose:
  //  - A regiser with only pending uses, to avoid having to add a gap move for
  //    a non-pending use.
  //  - A register holding a virtual register that has already been spilled, to
  //    avoid the gap move for spilling the virtual register.
...
void SinglePassRegisterAllocator::SpillRegisterForVirtualRegister(
    int virtual_register) {
  DCHECK_NE(virtual_register, InstructionOperand::kInvalidVirtualRegister);
  // We only want a single register to hold each virtual register, so free
  // the current register allocated to the virtual register.
...
void SinglePassRegisterAllocator::AllocateUse(RegisterIndex reg,
  ...
  register_state()->AllocateUse(reg, virtual_register, operand, instr_index,
                                data());
  AllocateRegister(reg, virtual_register, pos);
  CheckConsistency();
}
...
void SinglePassRegisterAllocator::AllocateInput(UnallocatedOperand* operand,
  ...
  if (operand->HasFixedRegisterPolicy() ||
    ...
    if (!VirtualRegisterIsUnallocatedOrInReg(virtual_register, reg)) {
      ...
      // add an gap move from that register to the fixed register, and then
      // add a pending use of that register.
    ...
    RegisterIndex reg;
    bool must_use_register =
        operand->HasRegisterPolicy() || vreg_data.is_constant();
    reg = GetRegisterFor(vreg_data, pos, must_use_register);
...
void SinglePassRegisterAllocator::AllocateGapMoveInput(
  ...
  RegisterIndex reg = GetRegisterFor(vreg_data, UsePosition::kStart, false);
  if (reg.is_valid()) {
    AllocatePendingUse(reg, virtual_register, operand, instr_index);
  } else {
    vreg_data.SpillOperand(operand, instr_index, data());
...
RegisterIndex SinglePassRegisterAllocator::AllocateOutput(
  ...
  if (operand->HasSlotPolicy() || operand->HasFixedSlotPolicy()) {
    ...
    reg = GetRegisterFor(vreg_data, pos, operand->HasRegisterPolicy());
  }
...
void SinglePassRegisterAllocator::AllocateSameInputOutput(
  ...
  if (reg.is_valid()) {
    if (!VirtualRegisterIsUnallocatedOrInReg(input_vreg, reg)) {
      // If |input_vreg| is already allocated to a different register, add a
      // gap move to shift it into the register allocated for this input/output
      // operand.
      AllocateUseWithMove(reg, input_vreg, input, instr_index,
                          UsePosition::kAll);
    } else {
      // Otherwise, just allocate it to the same register which is now free.
      AllocateUse(reg, input_vreg, input, instr_index, UsePosition::kAll);
    }
  } else {
    // Output was spilled. We need to make the input operand the same as the
    // output, so add it to the output's pending spilled operands.
    ...
    VirtualRegisterData& input_vreg_data = VirtualRegisterDataFor(input_vreg);
    MoveOperands* move_ops =
        data()->AddPendingOperandGapMove(instr_index, Instruction::END);
    InstructionOperand* gap_from = &move_ops->source();
    InstructionOperand* gap_dest = &move_ops->destination();

    RegisterIndex input_reg;
    if (!input_vreg_data.HasSpillOperand()) {
      input_reg = GetRegisterFor(input_vreg_data, UsePosition::kStart, false);
    }
    if (input_reg.is_valid()) {
      AllocatePendingUse(input_reg, input_vreg, gap_from, instr_index);
    } else {
      input_vreg_data.SpillOperand(gap_from, instr_index, data());
    }
    output_vreg_data.SpillOperand(gap_dest, instr_index, data());
  }
}
...
void SinglePassRegisterAllocator::AllocateTemp(UnallocatedOperand* operand,
  ...
  if (operand->HasSlotPolicy()) {
    ...
    reg = GetRegisterFor(RepresentationFor(virtual_register), UsePosition::kAll,
...
void SinglePassRegisterAllocator::AllocatePhiGapMove(int to_vreg, int from_vreg,
  ...
  if (to_register.is_valid() && !from_register.is_valid()) {
    ...
    // Commit the |from| side to either to a register or spill it.
    if (!from_register.is_valid()) {
      from_register =
          GetFreeRegister(RepresentationFor(from_vreg), UsePosition::kAll);
    }
    if (from_register.is_valid()) {
      AllocatePendingUse(from_register, from_vreg, from_operand, instr_index);
    } else {
      VirtualRegisterDataFor(from_vreg).SpillOperand(from_operand, instr_index,
                                                     data());
    }
  }
}
...
FastRegisterAllocator::FastRegisterAllocator(FastRegisterAllocatorData* data)
    : data_(data),
...
FastRegisterAllocator::~FastRegisterAllocator() = default;

void FastRegisterAllocator::DefineOutputs() {
...
void FastRegisterAllocator::InitializeBlockState(
    const InstructionBlock* block) {
...
void FastRegisterAllocator::DefineOutputs(const InstructionBlock* block) {
    ...
    for (size_t i = 0; i < instr->OutputCount(); i++) {
      ...
      if (output->IsConstant()) {
        ...
        if (unallocated_operand->HasFixedSlotPolicy()) {
          // If output is a fixed slot policy, allocate it's spill operand now
...
void FastRegisterAllocator::AllocateRegisters() {
...
void FastRegisterAllocator::AllocateRegisters(const InstructionBlock* block) {
  ...
  for (int instr_index = block->last_instruction_index();
    ...
    for (size_t i = 0; i < instr->OutputCount(); i++) {
      ...
      if (output->IsConstant()) {
        ...
        if (unallocated_output->HasSameAsInputPolicy()) {
          DCHECK(!instr->ClobbersRegisters() &&
    ...
    // Allocate inputs who's use spans the whole instruction first.
    ...
    // If we are allocating for the last instruction in the block, allocate the
    // phi gap move operations.
    if (instr_index == block->last_instruction_index()) {
...
void FastRegisterAllocator::UpdateSpillRangesForLoops() {
...
SinglePassRegisterAllocator& FastRegisterAllocator::AllocatorFor(
    MachineRepresentation rep) {
...
bool FastRegisterAllocator::IsFixedRegisterPolicy(
    const UnallocatedOperand* operand) {
  return operand->HasFixedRegisterPolicy() ||
         operand->HasFixedFPRegisterPolicy();
}

void FastRegisterAllocator::AllocatePhiGapMoves(const InstructionBlock* block) {
...
void FastRegisterAllocator::AllocatePhis(const InstructionBlock* block) {
  for (PhiInstruction* phi : block->phis()) {
    int virtual_register = phi->virtual_register();
    MachineRepresentation rep = RepresentationFor(virtual_register);
...
void FastRegisterAllocator::ReserveFixedRegisters(int instr_index) {
  Instruction* instr = code()->InstructionAt(instr_index);
  for (size_t i = 0; i < instr->OutputCount(); i++) {
...
class FastSpillSlotAllocator::SpillSlot : public ZoneObject {
 public:
...
bool FastSpillSlotAllocator::OrderByLastUse::operator()(
    const SpillSlot* a, const SpillSlot* b) const {
  return a->last_use() > b->last_use();
}

FastSpillSlotAllocator::FastSpillSlotAllocator(FastRegisterAllocatorData* data)
    : data_(data),
      allocated_slots_(data->allocation_zone()),
      free_slots_(data->allocation_zone()),
      position_(0) {}

void FastSpillSlotAllocator::AdvanceTo(int instr_index) {
...
FastSpillSlotAllocator::SpillSlot* FastSpillSlotAllocator::GetFreeSpillSlot(
    int byte_width) {
...
void FastSpillSlotAllocator::Allocate(VirtualRegisterData* virtual_register) {
...
void FastSpillSlotAllocator::AllocateSpillSlots() {
...
FastReferenceMapPopulator::FastReferenceMapPopulator(
    FastRegisterAllocatorData* data)
    : data_(data) {}

void FastReferenceMapPopulator::RecordReferences(
...
void FastReferenceMapPopulator::PopulateReferenceMaps() {
  BitVector::Iterator iterator(&data()->spilled_virtual_registers());
  for (; !iterator.Done(); iterator.Advance()) {
