...
#define DECLARE_UNREACHABLE_TURBOSHAFT_FALLBACK(ret, name)           \
  ...
  std::enable_if_t<Adapter::IsTurboshaft &&                          \
                       (... || (std::is_same_v<Args, Node*> ||       \
                                std::is_same_v<Args, BasicBlock*>)), \
                   ret>                                              \
  name(Args...) {                                                    \
    UNREACHABLE();                                                   \
  }
...
struct TurboshaftAdapter {
  ...
  class LoadView {
   public:
    explicit LoadView(turboshaft::Graph* graph, node_t node)
      ...
      op_ = &graph_->Get(node_).Cast<turboshaft::LoadOp>();
    }
  ...
  bool is_exclusive_user_of(node_t user, node_t value) const {
    ...
    return graph_->Get(value).saturated_use_count.Get() == use_count;
  }
