...
PropertyAccessInfo const& JSHeapBroker::CreatePropertyAccessInfoForLoad(
    MapRef map, NameRef name, CompilationDependencies* dependencies) {
  MapNameRefPair pair({map, name});
  auto it = ais_for_named_properties_.find(pair);
  if (it != ais_for_named_properties_.end()) {
    return it->second;
  }
  ...
  PropertyAccessInfo access_info =
      access_info_factory.ComputePropertyAccessInfo(map.object(), name.object(),
                                                    AccessMode::kLoad);

  auto inserted_ai =
      ais_for_named_properties_.insert(std::make_pair(pair, access_info));
