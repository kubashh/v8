...
void TFTask::ProcessOnMainThread(Isolate* isolate) {
  switch (kind_) {
    ...
    case kGetPropertyAccessInfo: {
      ...
      result1_ = factory.ComputePropertyAccessInfo2Wrapper(
          map.object(), name.object(), access_mode);
    }
  }
}
...
void TFTask::PostprocessOnBackgroundThread(JSHeapBroker* broker) {
  switch (kind_) {
    ...
    case kGetPropertyAccessInfo: {
      ...
      PropertyAccessInfo result = result1_.value().ToPAI(broker->zone());
      result.MoveToBackgroundThread(broker);

      PropertyAccessTarget target({map, name, access_mode});
      broker->property_access_infos_.insert({target, result});
      break;
    }
  }
}
