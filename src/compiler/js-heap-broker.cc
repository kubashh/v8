...
// TODO(solanes): Remove once we remove the FLAG_local_heap and the special
// casing for concurrent_inlining off.
...
// TODO(solanes): Remove once we remove the FLAG_local_heap and the special
// casing for concurrent_inlining off.
// This macro define the Asxxx methods which should only be used with concurrent
// inlining off.
#define DEFINE_AS_CONCURRENT_INLINING_OFF(Name)     \
  Name##Data* ObjectData::As##Name() {              \
    CHECK(Is##Name() && !FLAG_concurrent_inlining); \
    return static_cast<Name##Data*>(this);          \
  }
HEAP_BROKER_NEVER_SERIALIZED_OBJECT_LIST(DEFINE_AS_CONCURRENT_INLINING_OFF)
#undef DEFINE_AS_CONCURRENT_INLINING_OFF
  ...
  if (object_data == nullptr) {
    ...
    if (object->IsSmi()) {
      ...
      object_data = zone()->New<ObjectData>(this, data_storage, object,
                                            kUnserializedReadOnlyHeapObject);
// TODO(solanes): Remove the if/else in this macro once we remove the
// FLAG_local_heap and the special casing for concurrent_inlining off.
