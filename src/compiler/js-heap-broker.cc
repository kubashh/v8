...
class JSObjectField {
 public:
  bool IsDouble() const { return object_or_smi_ == nullptr; }
  double AsDouble() const {
    CHECK(IsDouble());
    return number_;
  }

  bool IsObject() const { return object_or_smi_ != nullptr; }
  ObjectData* AsObject() const {
    CHECK(IsObject());
    return object_or_smi_;
  }

  explicit JSObjectField(double value) : number_(value) {}
  explicit JSObjectField(ObjectData* value) : object_or_smi_(value) {}

 private:
  ObjectData* object_or_smi_ = nullptr;
  double number_ = 0;
};
...
bool JSObjectData::elements_tenured() const { return elements_tenured_; }
...
ObjectRef FixedArrayRef::get(int i) const {
  if (broker()->mode() == JSHeapBroker::kDisabled) {
    ...
    return ObjectRef(fixed_array->Get(i));
  }
}
...
bool FixedDoubleArrayRef::is_the_hole(int i) const {
  if (broker()->mode() == JSHeapBroker::kDisabled) {
    ...
    return fixed_array->Get(i).is_hole_nan();
  }
}
...
double FixedDoubleArrayRef::get_scalar(int i) const {
  if (broker()->mode() == JSHeapBroker::kDisabled) {
    ...
    CHECK(!fixed_array->Get(i).is_hole_nan());
    return fixed_array->Get(i).get_scalar();
  }
}
