...
//
// kPossiblyBackgroundSerializedHeapObject: The underlying V8 object behaves
//   like a kSerializedHeapObject, but allows serialization from the background
//   thread where this is safe (indicated by corresponding ObjectRef constructor
//   argument).
//
    ...
    CHECK_IMPLIES(broker->mode() == JSHeapBroker::kSerialized,
                  (kind == kUnserializedReadOnlyHeapObject &&
                   IsReadOnlyHeapObject(*object)) ||
                      kind == kSmi || kind == kNeverSerializedHeapObject ||
