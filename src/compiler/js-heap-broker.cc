...
JSHeapBroker::JSHeapBroker(Isolate* isolate, Zone* broker_zone)
    : isolate_(isolate),
      ...
      array_prototypes_(zone()),
      object_prototypes_(zone()) {
...
void JSHeapBroker::CollectArrayAndObjectPrototypes() {
  ...
  CHECK(array_prototypes_.empty());
  CHECK(object_prototypes_.empty());
  ...
  while (!maybe_context->IsUndefined(isolate())) {
    ...
    array_prototypes_.emplace(JSObject::cast(array_prot), isolate());
    object_prototypes_.emplace(JSObject::cast(object_prot), isolate());
...
bool JSHeapBroker::IsArrayPrototype(const JSObjectRef& object) const {
  if (mode() == kDisabled) {
    return isolate()->IsInAnyContext(*object.object(),
                                     Context::INITIAL_ARRAY_PROTOTYPE_INDEX);
  }
  CHECK(!array_prototypes_.empty());
  return array_prototypes_.find(object.object<JSObject>()) !=
         array_prototypes_.end();
}

bool JSHeapBroker::IsObjectPrototype(const JSObjectRef& object) const {
  if (mode() == kDisabled) {
    return isolate()->IsInAnyContext(*object.object(),
                                     Context::INITIAL_OBJECT_PROTOTYPE_INDEX);
  }
  CHECK(!object_prototypes_.empty());
  return object_prototypes_.find(object.object<JSObject>()) !=
         object_prototypes_.end();
}
