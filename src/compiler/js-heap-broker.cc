...
JSHeapBroker::JSHeapBroker(Isolate* isolate, Zone* broker_zone)
  ...
  if (!isolate->compiler_cache()) {
    ...
    isolate->set_compiler_cache(
        new (compiler_zone) PerIsolateCompilerCache(isolate, compiler_zone));
  }
...
void JSHeapBroker::SerializeShareableObjects() {
  if (mode() == kDisabled) {
    if (!refs_) {
      refs_ = new (zone())
          RefsMap(kInitialRefsBucketCount, AddressMatcher(), zone());
    }
    return;
  }
    ...
    case JSHeapBroker::kDisabled:
      ...
      if (!*(storage)) {
        ...
        entry->value = new (broker->zone())
            ObjectData(broker, storage, object, object->IsSmi());
      }
      data_ = *storage;
      break;
  }
