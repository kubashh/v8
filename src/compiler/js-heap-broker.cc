...
class MapData : public HeapObjectData {
 public:
  ...
  // Extra information.
  void SerializeElementsKindVariations();
  const ZoneVector<MapData*>& elements_kind_variations() {
    return elements_kind_variations_;
  }

 private:
  ZoneVector<MapData*> elements_kind_variations_;
};
...
MapData::MapData(JSHeapBroker* broker_, Handle<Map> object_,
    ...
    : HeapObjectData(broker_, object_, type_),
      ...
      elements_kind_variations_(broker->zone()) {}
...
JSFunctionData::JSFunctionData(JSHeapBroker* broker_,
  ...
  if (prototype_or_initial_map != nullptr &&
    ...
    if (initial_map->instance_type == JS_ARRAY_TYPE) {
      initial_map->SerializeElementsKindVariations();
    }
  }
}

void MapData::SerializeElementsKindVariations() {
  broker->Trace("Computing ElementsKind variations of %p.\n", *object);
  ...
  for (int i = FIRST_FAST_ELEMENTS_KIND; i <= LAST_FAST_ELEMENTS_KIND; i++) {
    ...
    if (IsMoreGeneralElementsKindTransition(from_kind, to_kind)) {
      ...
      elements_kind_variations_.push_back(
...
void JSHeapBroker::AddData(Handle<Object> object, ObjectData* data) {
  Trace("Creating data %p for handle 0x%" V8PRIxPTR " (", data,
        object.address());
  if (FLAG_trace_heap_broker) {
    object->ShortPrint();
    PrintF(")\n");
...
base::Optional<MapRef> MapRef::AsElementsKind(ElementsKind kind) const {
  if (broker()->mode() == JSHeapBroker::kDisabled) {
    ...
    const ZoneVector<MapData*>& elements_kind_variations =
        data()->AsMap()->elements_kind_variations();
    for (auto data : elements_kind_variations) {
      MapRef map(data);
