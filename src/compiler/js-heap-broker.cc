...
class JSFunctionData : public JSObjectData {
 public:
  ...
  bool serialized_for_compilation() const { return serialized_for_inlining_; }
...
void JSFunctionRef::SerializeForCompilation() {
  if (broker()->mode() == JSHeapBroker::kDisabled) return;
  CHECK_EQ(broker()->mode(), JSHeapBroker::kSerializing);
  data()->AsJSFunction()->SerializeForCompilation(broker());
}
