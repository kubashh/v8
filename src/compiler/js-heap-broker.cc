...
#define BIMODAL_ACCESSOR_(OwnerType, OwnerClass, ResultType, MethodName)    \
  ResultType##Ref OwnerType##Ref::MethodName() const {                      \
    if (broker()->mode() == JSHeapBroker::kDisabled) {                      \
      AllowHandleAllocation handle_allocation;                              \
      AllowHandleDereference allow_handle_dereference;                      \
      return ResultType##Ref(                                               \
          broker(),                                                         \
          handle(object<OwnerClass>()->MethodName(), broker()->isolate())); \
    } else {                                                                \
      return ResultType##Ref(data()->As##OwnerType()->MethodName);          \
    }                                                                       \
  }

#define BIMODAL_ACCESSOR(OwnerType, ResultType, MethodName) \
  BIMODAL_ACCESSOR_(OwnerType, OwnerType, ResultType, MethodName)

#define HANDLE_ACCESSOR_(OwnerType, OwnerClass, ResultType, MethodName)   \
  ResultType##Ref OwnerType##Ref::MethodName() const {                    \
    AllowHandleAllocation handle_allocation;                              \
    AllowHandleDereference allow_handle_dereference;                      \
    return ResultType##Ref(                                               \
...
#define HANDLE_ACCESSOR(OwnerType, ResultType, MethodName) \
  HANDLE_ACCESSOR_(OwnerType, OwnerType, ResultType, MethodName)

#define HANDLE_ACCESSOR_C(OwnerType, ResultType, MethodName) \
  ResultType OwnerType##Ref::MethodName() const {            \
    AllowHandleAllocation handle_allocation;                 \
    AllowHandleDereference allow_handle_dereference;         \
    return object<OwnerType>()->MethodName();                \
  }
