...
FrameState JSInliner::CreateInlinedExtraArgumentsFrameState(
  ...
  return FrameState{graph()->NewNode(op, extra_args_node, node0, node0,
...
Reduction JSInliner::ReduceJSCall(Node* node) {
  ...
  // TODO(victorgomes): Figure out a way to propagate the real argument count to
  // argument object reducers. When creating the arguments object, we get the
  // real argument count by using the ExtraArguments frame state in case of
  // underapplication. This allows FrameStateInfo with negative parameter count.
