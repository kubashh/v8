...
base::Optional<SharedFunctionInfoRef> JSInliner::DetermineCallTarget(
  ...
  if (match.HasResolvedValue() && match.Ref(broker()).IsJSFunction()) {
    ...
    if (!function.has_feedback_vector(nullptr)) {
      return base::nullopt;
    }
    ...
    if (!function.native_context(nullptr).equals(
            broker()->target_native_context())) {
      return base::nullopt;
    }

    return function.shared(nullptr);
  }
...
FeedbackCellRef JSInliner::DetermineCallContext(Node* node,
  ...
  if (match.HasResolvedValue() && match.Ref(broker()).IsJSFunction()) {
    ...
    CHECK(function.has_feedback_vector(nullptr));

    // The inlinee specializes to the context from the JSFunction object.
    *context_out = jsgraph()->Constant(function.context(nullptr));
    return function.raw_feedback_cell(nullptr);
  }
