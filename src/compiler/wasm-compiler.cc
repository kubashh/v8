...
Node* LoadWithAlignment(WasmGraphAssembler* gasm, MachineType type, Node* base,
                        Node* offset) {
  if (type.representation() == MachineRepresentation::kWord64 ||
      type.representation() == MachineRepresentation::kFloat64) {
...
Node* StoreWithValueType(WasmGraphAssembler* gasm, Node* base, Node* offset,
                         Node* value, wasm::ValueType type) {
  MachineRepresentation rep = type.machine_representation();
...
Node* StoreStructFieldUnchecked(MachineGraph* graph, WasmGraphAssembler* gasm,
  ...
  return StoreWithValueType(gasm, struct_object,
...
Node* WasmGraphBuilder::ArrayNew(uint32_t array_index,
  ...
  gasm_->Bind(&loop);
  {
    ...
    StoreWithValueType(gasm_.get(), a, offset, initial_value,
                       type->element_type());
...
Node* WasmGraphBuilder::StructGet(Node* struct_object,
  ...
  return LoadWithAlignment(gasm_.get(), machine_type, struct_object, offset);
}
...
Node* WasmGraphBuilder::ArrayGet(Node* array_object,
  ...
  return LoadWithAlignment(gasm_.get(), machine_type, array_object, offset);
}
...
Node* WasmGraphBuilder::ArraySet(Node* array_object,
  ...
  BoundsCheck(array_object, index, position);
  Node* offset = ArrayElementOffset(gasm_.get(), index, type->element_type());
  return StoreWithValueType(gasm_.get(), array_object, offset, value,
                            type->element_type());
}
