...
Node* WasmGraphBuilder::CallDirect(uint32_t index, Node** args, Node*** rets,
  ...
  if (env_ && index < env_->module->num_imported_functions) {
    ...
    Node* target_node = graph()->NewNode(
        ...
        jsgraph()->Int32Constant(index * sizeof(Address)), *effect_, *control_);
    args[0] = target_node;
...
Node* WasmGraphBuilder::CallIndirect(uint32_t sig_index, Node** args,
  ...
  Node* scaled_key = graph()->NewNode(machine->Int32Mul(), key,
                                      Int32Constant(sizeof(uint32_t)));
  ...
  Node* ift_targets =                                      // --
      LOAD_INSTANCE_FIELD(IndirectFunctionTableTargets,    // --
                          MachineType::Pointer());         // --
  Node* ift_instances =                                    // --
      LOAD_INSTANCE_FIELD(IndirectFunctionTableInstances,  // --
                          MachineType::TaggedPointer());   // --

  scaled_key =
      graph()->NewNode(machine->Int32Mul(), key, Int32Constant(kPointerSize));

  Node* target =
      graph()->NewNode(machine->Load(MachineType::Pointer()),
                       graph()->NewNode(add, ift_targets, scaled_key),
  ...
  if (target->IsJSFunction()) {
    ...
    if (function->shared()->internal_formal_parameter_count() == wasm_count) {
      ...
      if (is_sloppy(function->shared()->language_mode()) &&
        ...
        Node* global_proxy =
            LOAD_FIXED_ARRAY_SLOT(native_context, Context::GLOBAL_PROXY_INDEX);
