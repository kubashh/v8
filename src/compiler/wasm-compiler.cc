...
#define LOAD_INSTANCE_FIELD(name, type)                                     \
  gasm_->LoadFromObject(assert_size(WASM_INSTANCE_OBJECT_SIZE(name), type), \
                        instance_node_.get(),                               \
                        WASM_INSTANCE_OBJECT_OFFSET(name))
...
ObjectAccess ObjectAccessForGCStores(wasm::ValueType type) {
  return ObjectAccess(
      MachineType::TypeForRepresentation(type.machine_representation(),
                                         !type.is_packed()),
      type.is_reference_type() ? kFullWriteBarrier : kNoWriteBarrier);
}
}  // namespace
...
void WasmGraphBuilder::GetGlobalBaseAndOffset(MachineType mem_type,
  ...
  if (global.mutability && global.imported) {
    ...
    *offset_node = Int32Constant(0);
    ...
    Node* globals_start = gasm_->LoadFromObject(
        MachineType::UintPtr(), instance_node_.get(),
 ...
 public:
  ...
  Node* BuildLoadUndefinedValueFromInstance() {
    if (undefined_value_node_ == nullptr) {
      ...
      undefined_value_node_ = gasm_->LoadFromObject(
