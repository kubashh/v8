...
Node* WasmGraphBuilder::BuildWasmCall(wasm::FunctionSig* sig, Node** args,
  ...
  if (position != -1) SetSourcePosition(call, position);
  ...
  void BuildJSToWasmWrapper(uint32_t wasm_func_index, Address call_target) {
    ...
    *effect_ = start;
    ...
    if (env_ && wasm_func_index < env_->module->num_imported_functions) {
      ...
      BuildImportWasmCall(sig_, args, &rets, wasm::WasmCodePosition{-1},
                          wasm_func_index);
    } else {
      ...
      BuildWasmCall(sig_, args, &rets, wasm::WasmCodePosition{-1}, nullptr,
                    kNoRetpoline);
    }
