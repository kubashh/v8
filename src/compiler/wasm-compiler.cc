...
Node* WasmGraphBuilder::MemBuffer(uint32_t offset) {
  Node* mem_buffer =
      graph()->NewNode(jsgraph()->machine()->Load(MachineType::UintPtr()),
                       jsgraph()->ExternalConstant(
                           ExternalReference::wasm_context_mem_start_address()),
                       jsgraph()->Int32Constant(0), *effect_, *control_);

  if (offset == 0) return mem_buffer;

  Node* mem_buffer_offset = graph()->NewNode(jsgraph()->machine()->IntAdd(),
                                             mem_buffer, Int32Constant(offset));
...
Node* WasmGraphBuilder::MemSize() {
  Node* mem_size =
      graph()->NewNode(jsgraph()->machine()->Load(MachineType::Uint32()),
                       jsgraph()->ExternalConstant(
                           ExternalReference::wasm_context_mem_size_address()),
                       jsgraph()->Int32Constant(0), *effect_, *control_);
  *effect_ = mem_size;
  return mem_size;
}
...
void WasmGraphBuilder::BoundsCheckMem(MachineType memtype, Node* index,
  ...
  Node* effective_size_node =
      graph()->NewNode(jsgraph()->machine()->Int32Sub(), MemSize(),
                       Int32Constant(offset + memsize - 1));
