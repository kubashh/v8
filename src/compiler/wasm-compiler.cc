...
Node* WasmGraphBuilder::Simd8x16ShuffleOp(const uint8_t shuffle[16],
  ...
  has_simd_ = true;
  uint8_t s[16];
  base::Memcpy(s, shuffle, kSimd128Size);
  bool needs_swap;
  bool inputs_equal = inputs[0]->id() == inputs[1]->id();
  bool is_swizzle;
  wasm::SimdShuffle::CanonicalizeShuffle(inputs_equal, s, &needs_swap,
                                         &is_swizzle);
  ...
  return graph()->NewNode(mcgraph()->machine()->I8x16Shuffle(s, is_swizzle),
                          lhs, rhs);
}
