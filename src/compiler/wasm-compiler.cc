...
Node* WasmGraphBuilder::BuildImportCall(const wasm::FunctionSig* sig,
  ...
  Node* target_node =
      gasm_->Load(MachineType::Pointer(), imported_targets,
                  mcgraph()->Int32Constant(func_index * kSystemPointerSize));
  args[0] = target_node;
...
Node* WasmGraphBuilder::ElemDrop(uint32_t elem_segment_index,
  ...
  return gasm_->Store(store_rep, dropped_elem_segments,
...
class WasmWrapperGraphBuilder : public WasmGraphBuilder {
 public:
  ...
  void BuildCapiCallWrapper(Address address) {
    ...
    Node* throw_call = gasm_->Call(call_descriptor, call_target, return_value);
    TerminateThrow(throw_call, control());
