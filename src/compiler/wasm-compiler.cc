...
Node* WasmGraphBuilder::CallIndirect(uint32_t sig_index, Node** args,
  ...
  if (WASM_INSTANCE_TABLES) {
    ...
    Node* target_instance = graph()->NewNode(
        machine->Load(MachineType::Pointer()), entry_address,
        Int32Constant(offsetof(IndirectFunctionTableEntry, instance)), *effect_,
        *control_);
  ...
  Node* entry = graph()->NewNode(
      ...
      *effect_, *control_);
  args[0] = entry;
  constexpr Node* instance_node = nullptr;
  const bool use_retpoline = FLAG_untrusted_code_mitigations;
  return BuildWasmCall(sig, args, rets, position, instance_node, use_retpoline);
}
...
void WasmGraphBuilder::BuildJSToWasmWrapper(WasmCodeWrapper wasm_code) {
  ...
  *effect_ = start;
  ...
  // Create the instance_node node to pass as parameter. This must be a
  ...
  instance_node_ = jsgraph()->HeapConstant(handle(wasm_code.wasm_instance()));
...
void WasmGraphBuilder::BuildWasmToWasmWrapper(WasmCodeWrapper wasm_code) {
  int wasm_count = static_cast<int>(sig_->parameter_count());
  int count = wasm_count + 4;  // wasm_code, instance_node, effect, and control.
  Node** args = Buffer(count);
  ...
  *effect_ = start;
  ...
  args[pos++] = jsgraph()->HeapConstant(handle(wasm_code.wasm_instance()));
  // Add the parameters starting from index 1 since the parameter with index 0
  // is the old instance_node.
  for (int i = 0; i < wasm_count; ++i) {
    args[pos++] = Param(i + 1);
  }
  args[pos++] = *effect_;
...
void WasmGraphBuilder::InitInstanceCache(
  ...
  Node* mem_start = graph()->NewNode(
      ...
      jsgraph()->Int32Constant(WasmInstanceObject::kMemoryStartOffset),
      *effect_, *control_);
  *effect_ = mem_start;
  ...
  Node* mem_size = graph()->NewNode(
      ...
      jsgraph()->Int32Constant(WasmInstanceObject::kMemorySizeOffset), *effect_,
      *control_);
  *effect_ = mem_size;
  ...
  if (untrusted_code_mitigations_) {
    ...
    Node* mem_mask = graph()->NewNode(
        jsgraph()->machine()->Load(MachineType::Uint32()), instance_node_.get(),
        jsgraph()->Int32Constant(WasmInstanceObject::kMemoryMaskOffset),
        *effect_, *control_);
    *effect_ = mem_mask;
...
void WasmGraphBuilder::GetGlobalBaseAndOffset(MachineType mem_type,
  ...
  if (globals_start_ == nullptr) {
    ...
    globals_start_ = graph()->NewNode(
        ...
        instance_node_.get(),
        jsgraph()->Int32Constant(WasmInstanceObject::kGlobalsStartOffset),
...
void WasmGraphBuilder::GetFunctionTableNodes(uint32_t table_index, Node** table,
                                             Node** table_size) {
  if (WASM_INSTANCE_TABLES) {
    // The table address and size are stored in the instance object.
    ...
    *table = graph()->NewNode(
        jsgraph()->machine()->Load(MachineType::UintPtr()),
        instance_node_.get(),
        jsgraph()->Int32Constant(WasmInstanceObject::kIftOffset), *effect_,
        *control_);
    *table_size = graph()->NewNode(
        jsgraph()->machine()->Load(MachineType::Uint32()), instance_node_.get(),
        jsgraph()->Int32Constant(WasmInstanceObject::kIftSizeOffset), *effect_,
        *control_);
  } else {
...
Handle<Code> CompileWasmToWasmWrapper(Isolate* isolate, WasmCodeWrapper target,
  ...
  builder.set_control_ptr(&control);
  builder.set_effect_ptr(&effect);
  builder.BuildWasmToWasmWrapper(target);
