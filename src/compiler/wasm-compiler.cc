...
Node* WasmGraphBuilder::BuildWasmCall(wasm::FunctionSig* sig, Node** args,
                                      Node*** rets,
                                      wasm::WasmCodePosition position) {
  DCHECK_NOT_NULL(context_address_);
  SetNeedsStackCheck();
  const size_t params = sig->parameter_count();
  const size_t extra = 3;  // context_address, effect and control.
  const size_t count = 1 + params + extra;

  // Reallocate the buffer to make space for extra inputs.
  args = Realloc(args, 1 + params, count);

  // Start filling the args from the back so that we can shift the params to
  // make room for the context_address parameter at index 1, just after code.
  ...
  // Context address can now be placed at index 1. Code will remain at index 0.
  DCHECK_EQ(index, 1);
  args[index--] = context_address_;
...
void WasmGraphBuilder::BuildJSToWasmWrapper(Handle<Code> wasm_code,
  ...
  int count = wasm_count + 4;  // wasm_code, context, effect, and control.
  ...
  *effect_ = start;
  ...
  // Create the context address node to pass as parameter.
  DCHECK_NULL(context_address_);
  context_address_ = jsgraph()->RelocatableIntPtrConstant(
      reinterpret_cast<uintptr_t>(context_address),
...
int WasmGraphBuilder::AddParameterNodes(Node** args, int pos, int param_count,
  ...
  for (int i = 0; i < param_count; ++i) {
    Node* param = Param(i + 1);  // Start from index 1 to drop the context.
...
void WasmGraphBuilder::BuildWasmToWasmWrapper(Handle<Code> target,
  ...
  *effect_ = start;
  ...
  args[pos++] = HeapConstant(target);
  // Add the context address of the other instance.
  ...
  // Call the wasm code. We can tail call since we just need to change the
  // context_address and reuse the frame.
...
void WasmGraphBuilder::BuildWasmInterpreterEntry(
  ...
  *control_ = start;
  ...
  for (int i = 0; i < param_count; ++i) {
    ...
    // Start from the parameter with index 1 to drop the context address.
...
void WasmGraphBuilder::BuildCWasmEntry(Address context_address) {
  // Build the start and the JS parameter nodes.
  ...
  *effect_ = start;
  ...
  DCHECK_NULL(context_address_);
  context_address_ =
      jsgraph()->IntPtrConstant(reinterpret_cast<uintptr_t>(context_address));
  ...
  int arg_count = wasm_arg_count + 4;  // code, context, control, effect
...
Node* WasmGraphBuilder::LoadMemStart() {
  ...
  Node* mem_buffer = graph()->NewNode(
      jsgraph()->machine()->Load(MachineType::UintPtr()), context_address_,
      jsgraph()->Int32Constant(offsetof(WasmContext, mem_start)), *effect_,
      *control_);
  *effect_ = mem_buffer;
  return mem_buffer;
}
...
Node* WasmGraphBuilder::LoadMemSize() {
  ...
  Node* mem_size = graph()->NewNode(
      jsgraph()->machine()->Load(MachineType::Uint32()), context_address_,
      jsgraph()->Int32Constant(offsetof(WasmContext, mem_size)), *effect_,
      *control_);
  *effect_ = mem_size;
  return mem_size;
}
...
Handle<Code> CompileJSToWasmWrapper(Isolate* isolate, wasm::WasmModule* module,
  ...
  builder.BuildJSToWasmWrapper(wasm_code, context_address);
...
Handle<Code> CompileWasmToWasmWrapper(Isolate* isolate, Handle<Code> target,
  ...
  builder.set_control_ptr(&control);
  builder.set_effect_ptr(&effect);
  builder.BuildWasmToWasmWrapper(target, new_context_address);
...
Handle<Code> CompileCWasmEntry(Isolate* isolate, wasm::FunctionSig* sig,
  ...
  builder.set_control_ptr(&control);
  builder.set_effect_ptr(&effect);
  builder.BuildCWasmEntry(context_address);
