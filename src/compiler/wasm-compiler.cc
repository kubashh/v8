...
JSWasmCallData::JSWasmCallData(const wasm::FunctionSig* wasm_signature) {
  for (size_t i = 0; i < wasm_signature->parameter_count(); i++) {
    ...
    set_arg_needs_conversion(i, type.kind() != wasm::ValueType::kI32 &&
                                    type.kind() != wasm::ValueType::kF32 &&
                                    type.kind() != wasm::ValueType::kF64);
  }

  if (wasm_signature->return_count() == 1 &&
      wasm_signature->GetReturn().kind() == wasm::ValueType::kI64) {
    set_result_needs_conversion(true);
  }
}
...
class WasmWrapperGraphBuilder : public WasmGraphBuilder {
 public:
  ...
  Node* BuildCallAndReturn(bool is_import, Node* js_context,
    ...
    DCHECK(!js_wasm_call_data || (native_module && wasm_function_index >= 0));
    ...
    // Set the ThreadInWasm flag before we do the actual call.
    {
      ...
      if (is_import) {
        ...
        Node* call = BuildWasmCall(sig_, VectorOf(args), VectorOf(rets),
                                   wasm::kNoCodePosition, nullptr, kNoRetpoline,
                                   frame_state);
        if (FLAG_turbo_inline_small_wasm_functions && js_wasm_call_data) {
          auto params = OpParameter<CallDescriptor*>(call->op());
          const Operator* op = mcgraph()->common()->WasmCall(
              params, native_module, wasm_function_index);
          NodeProperties::ChangeOp(call, op);
        }
      }
    }
  ...
  void BuildJSToWasmWrapper(bool is_import,
                            ...
                            Node* frame_state = nullptr,
                            const wasm::NativeModule* native_module = nullptr,
                            int wasm_function_index = -1) {
...
void BuildInlinedJSToWasmWrapper(
    ...
    const wasm::NativeModule* native_module, int function_index,
  ...
  WasmWrapperGraphBuilder builder(zone, mcgraph, signature,
                                  native_module->module(), spt, stub_mode,
                                  features);
...
Vector<const char> GetWasmFunctionDebugName(Zone* zone, int index) {
...
wasm::WasmCompilationResult ExecuteTurbofanWasmCompilation(
  ...
  OptimizedCompilationInfo info(GetWasmFunctionDebugName(&zone, func_index),
