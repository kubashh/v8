...
class PropertyCellData : public HeapObjectData {
 public:
  ...
  bool Cache(JSHeapBroker* broker, ThreadSafeTag tag);
...
bool PropertyCellData::Cache(JSHeapBroker* broker, ThreadSafeTag tag) {
...
class JSFunctionData : public JSObjectData {
 ...
 private:
  void Cache(JSHeapBroker* broker, ThreadSafeTag);
  bool serialized_ = false;
...
void JSFunctionData::Cache(JSHeapBroker* broker, ThreadSafeTag) {
  CHECK(!serialized_);
...
ObjectData* JSGlobalObjectData::GetPropertyCell(JSHeapBroker* broker,
  ...
  if (cell.has_value()) {
    ...
    if (!result->should_access_heap()) {
      result->AsPropertyCell()->Cache(broker, ThreadSafeTag{});
    }
  }
...
void JSHeapBroker::InitializeAndStartSerializing() {
  ...
  if (!is_concurrent_inlining()) {
    ...
    if (!data->should_access_heap()) {
      data->AsPropertyCell()->Cache(this, tag);
    }
...
bool PropertyCellRef::Cache(ThreadSafeTag tag) const {
  if (data_->should_access_heap()) return true;
  CHECK(broker()->mode() == JSHeapBroker::kSerializing ||
        broker()->mode() == JSHeapBroker::kSerialized);
  return data()->AsPropertyCell()->Cache(broker(), tag);
}
