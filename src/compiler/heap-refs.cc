...
BIMODAL_ACCESSOR_C(JSFunction, bool, has_instance_prototype)
...
ObjectRef JSFunctionRef::instance_prototype() const {
  if (data_->should_access_heap()) {
    return MakeRefAssumeMemoryFence(broker(), object()->instance_prototype());
  }
  return TryMakeRef<Object>(broker(),
                            data_->AsJSFunction()->instance_prototype())
      .value();
}
