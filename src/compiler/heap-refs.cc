...
class JSFunctionData : public JSObjectData {
 ...
 private:
  ...
  ObjectData* prototype_ = nullptr;  // Derives from prototype_or_initial_map_.
...
bool JSFunctionData::Serialize(JSHeapBroker* broker) {
  ...
  context_ = broker->TryGetOrCreateData(context, kAssumeMemoryFence);
  if (context_ == nullptr) {
  ...
  native_context_ = broker->TryGetOrCreateData(context.map().native_context(),
                                               kAssumeMemoryFence);
  if (native_context_ == nullptr) {
    TRACE_BROKER_MISSING(broker, "JSFunction::native_context");
    return false;
  }
  CHECK(native_context_->IsNativeContext());

  SharedFunctionInfo shared = function->shared(kRelaxedLoad);
  shared_ = broker->TryGetOrCreateData(shared, kAssumeMemoryFence);
  if (shared_ == nullptr) {
    TRACE_BROKER_MISSING(broker, "JSFunction::shared");
    return false;
  }

  function_data_ = broker->TryGetOrCreateData(
      shared.function_data(kAcquireLoad), kAssumeMemoryFence);
  if (function_data_ == nullptr) {
  ...
  if (function->has_prototype_slot()) {
    ...
    if (prototype_or_initial_map_ == nullptr) {
    ...
    // TODO: Tricky. Concurrent modifications and looking at multiple fields.
    has_prototype_ = function->has_prototype();
    if (has_prototype_) {
      prototype_ =
          broker->TryGetOrCreateData(function->prototype(), kAssumeMemoryFence);
      if (prototype_ == nullptr) {
        TRACE_BROKER_MISSING(broker, "JSFunction::prototype");
        return false;
      }
    }
  }
  ...
  feedback_cell_ =
      broker->TryGetOrCreateData(feedback_cell, kAssumeMemoryFence);
  if (feedback_cell_ == nullptr) {
    TRACE_BROKER_MISSING(broker, "JSFunction::feedback_cell");
    return false;
  }

  ObjectData* maybe_feedback_vector = broker->TryGetOrCreateData(
      feedback_cell.value(kAcquireLoad), kAssumeMemoryFence);
  if (shared.is_compiled() && maybe_feedback_vector != nullptr &&
      maybe_feedback_vector->IsFeedbackVector()) {
...
bool JSFunctionData::IsConsistentWithHeapState(JSHeapBroker* broker) const {
  ...
  /* Unclear if this is problematic but it happens
  ...
  */
  if (f->has_prototype_slot()) {
    ...
    */
    if (has_prototype_ != f->has_prototype()) {
      TRACE_BROKER_MISSING(broker, "JSFunction::has_prototype");
      return false;
    }
  } else {
    DCHECK(!has_initial_map_);
    DCHECK(!has_prototype_);
  }
  ...
  if (*native_context_->object() != f->native_context()) {
    TRACE_BROKER_MISSING(broker, "JSFunction::native_context");
    return false;
  }
  if (has_initial_map()) {
  ...
  if (has_prototype()) {
    if (*prototype_->object() != f->prototype()) {
      TRACE_BROKER_MISSING(broker, "JSFunction::prototype");
      return false;
    }
  } else {
...
template <class T>
constexpr bool NeverEverSerialize() {
  return false;
}
...
#define NEVER_EVER_SERIALIZE(Type)            \
  template <>                                 \
  constexpr bool NeverEverSerialize<Type>() { \
    return true;                              \
  }
...
STATIC_ASSERT(!NeverEverSerialize<Code>());  // See JSFunctionData::code_.
...
bool JSFunctionRef::Serialize() const {
  if (data_->should_access_heap()) return true;
  if (data_->AsJSFunction()->Serialize(broker())) return true;
