...
class PropertyAccessInfo final {
 public:
  enum Kind {
    ...
    kDataField,
    kDataFieldConstant,
    kDataDictionaryProtoConstant,
    kAccessorFieldConstant,
    kAccessorDictionaryProtoConstant,
    kModuleExport,
    kStringLength
  };
  ...
  static PropertyAccessInfo DataFieldConstant(
      Zone* zone, Handle<Map> receiver_map,
      ZoneVector<CompilationDependency const*>&& unrecorded_dependencies,
      FieldIndex field_index, Representation field_representation,
      ...
      MaybeHandle<JSObject> holder,
      MaybeHandle<Map> transition_map = MaybeHandle<Map>());
  static PropertyAccessInfo AccessorFieldConstant(Zone* zone,
  ...
  static PropertyAccessInfo DataDictionaryProtoConstant(
      Zone* zone, Handle<Map> receiver_map, Handle<JSObject> holder,
      InternalIndex dict_index);
  static PropertyAccessInfo AccessorDictionaryProtoConstant(
  ...
  bool IsDataFieldConstant() const { return kind() == kDataFieldConstant; }
  bool IsAccessorFieldConstant() const {
    return kind() == kAccessorFieldConstant;
  }
  bool IsModuleExport() const { return kind() == kModuleExport; }
  bool IsStringLength() const { return kind() == kStringLength; }
  bool IsDataDictionaryProtoConstant() const {
    return kind() == kDataDictionaryProtoConstant;
  }
  bool IsAccessorDictionaryProtoConstant() const {
    return kind() == kAccessorDictionaryProtoConstant;
  }

  bool HasTransitionMap() const { return !transition_map().is_null(); }
  bool HasDictionaryHolder() const {
    return kind_ == kDataDictionaryProtoConstant ||
           kind_ == kAccessorDictionaryProtoConstant;
  }
