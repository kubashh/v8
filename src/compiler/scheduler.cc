...
#undef DEFINE_CONTROL_CASE
  ...
  int coupled_control_edge = GetCoupledControlEdge(node);
...
int Scheduler::GetCoupledControlEdge(Node* node) {
...
class PrepareUsesVisitor {
 ...
 private:
  ...
  void Post(Node* node) {
    ...
    int coupled_control_edge = scheduler_->GetCoupledControlEdge(node);
