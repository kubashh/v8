...
class V8_EXPORT_PRIVATE Frame : public ZoneObject {
 public:
  ...
  inline int GetTotalFrameSlotCount() const { return slot_allocator_.End(); }
  ...
  void AlignSavedCalleeRegisterSlots(int alignment = kDoubleSize) {
    int end_before = slot_allocator_.End();
    slot_allocator_.Align(AlignedSlotAllocator::NumSlots(alignment));
    spill_slot_count_ += slot_allocator_.End() - end_before;
  }

  void AllocateSavedCalleeRegisterSlots(int count) {
    slot_allocator_.Reserve(count);
  }
  ...
  int AllocateSpillSlot(int width, int alignment = 0) {
    ...
    int slots = AlignedSlotAllocator::NumSlots(width);
    int old_end = slot_allocator_.End();
    int slot;
    if (alignment) {
      slot = slot_allocator_.Allocate(slots);
    } else {
      slot = slot_allocator_.Reserve(slots);
    }
    int end = slot_allocator_.End();
  ...
  int ReserveSpillSlots(size_t slot_count) {
    ...
    slot_allocator_.Reserve(static_cast<int>(slot_count));
    return slot_allocator_.End() - 1;
  }
