...
void CsaLoadElimination::AbstractState::KillOffset(ConstantOffsetInfos& infos,
                                                   ...
                                                   Zone* zone) {
  // All elements in the range [{offset}, {ElementSizeInBytes(repr)}) are in the
  // killed range. We do not need to traverse the inner maps, we can just clear
  // them.
  for (int i = 0; i < ElementSizeInBytes(repr); i++) {
    infos.Set(offset + i, InnerMap(zone));
  }
  // Now we have to remove all elements that overlap with an element in
  // {offset}.
  ...
  for (uint32_t i = initial_offset; i < offset; i++) {
    ...
    for (std::pair<Node*, FieldInfo> info : infos.Get(i)) {
