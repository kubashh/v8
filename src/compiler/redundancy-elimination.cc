...
bool CheckSubsumes(Node const* a, Node const* b) {
  if (a->op() != b->op()) {
    ...
    } else if ((a->opcode() == IrOpcode::kStringCharCodeAt &&
                b->opcode() == IrOpcode::kStringCharCodeAtWithFeedback) ||
               (a->opcode() == IrOpcode::kStringCharCodeAtWithFeedback &&
                b->opcode() == IrOpcode::kStringCharCodeAt)) {
      // StringCharCodeAt and kStringCharCodeAtWithFeedback return the same
      // thing; the latter is just more efficient in some cases.
