...
base::Optional<std::pair<int64_t, int64_t>> OneOrTwoByteAnalysis::TryGetRange(
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kCall: {
      ...
      if (m.HasResolvedValue() && m.Ref(broker()).IsCodeT()) {
        CodeTRef code = m.Ref(broker()).AsCodeT();
...
OneOrTwoByteAnalysis::State OneOrTwoByteAnalysis::OneOrTwoByte(Node* node) {
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kNewConsString: {
      ...
      State lhs_state =
          IsLiteralString(lhs, broker()) ? OneOrTwoByte(lhs) : State::kCantKnow;

      State node_state = MergeStates(rhs_state, lhs_state);
...
bool StringBuilderOptimizer::CheckNodeUses(Node* node,
                                           ...
                                           Status status) {
  std::cout << "CheckNodeUses:\n +";
  node->Print();
  std::cout << " + ";
  string_builder_child->Print();
...
bool StringBuilderOptimizer::CheckPreviousNodeUses(Node* child, Status status,
  ...
  std::cout << "CheckPreviousNodeUses : ";
  child->Print();
  if (IsConcat(child)) {
...
void StringBuilderOptimizer::VisitNode(Node* node, BasicBlock* block) {
  ...
  node->Print();
  if (IsConcat(node)) {
    std::cout << "Looks like regular concat\n";
    Node* lhs = node->InputAt(1);
    Node* rhs = node->InputAt(2);
    ...
    if (IsLiteralString(lhs, broker())) {
      ...
      // regular concatenations of 2 constant strings and that can't be begining
      // of string builders.
...
// For each potential string builder, checks that their begining has status
...
// with the string builder), which implicetely made node [3] a valid part of the
...
// builder should be done (either right after the last node, or at the begining
...
void StringBuilderOptimizer::FinalizeStringBuilders() {
  ...
  std::cout << "FinalizeStringBuilders. Count = " << string_builder_count_ << "\n";
  ...
  // bit early, and we .clear() them at the begining of each iteration (which
  ...
  for (unsigned int string_builder_id = 0;
    ...
    std::cout << "Looking at string builder " << string_builder_id << "\n";
  ...
  if (one_string_builder_or_more_valid) {
    broker()->isolate()->SetHasTurbofanStringBuilders();
  }
#else
...
void StringBuilderOptimizer::VisitGraph() {
  std::cout << "Using string builder, apparently...\n";
