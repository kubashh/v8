...
bool StringBuilderOptimizer::IsStringBuilderEnd(Node* node) {
  ...
  DCHECK_IMPLIES(
      ...
      status.id != -1 && StringBuilderIsValid(string_builders_[status.id]));
...
// following node in the string builder. In a nutshell, for uses of a node (that
// is part of the string builder) to be valid, they need to all appear before
// the next node of the string builder (because after, the node is node valid
// anymore because we mutate SlicedString and the backing store in place). For
// instance:
//
//     s1 = "123" + "abc";
//     s2 = s1 + "def";
...
bool StringBuilderOptimizer::CheckNodeUses(Node* node, Node* concat_child,
                                           Status status) {
  DCHECK(GetStatus(concat_child).state == State::kInStringBuilder ||
...
// node).
void StringBuilderOptimizer::FinishStringBuilders() {
  ...
  for (unsigned int string_builder_id = 0;
    ...
    StringBuilder* concat = &string_builders_[string_builder_id];
    ...
    // loop Phi where the Phi is the last in a string builder), setting
