  ...
  for (unsigned int concat_id = 0; concat_id < concat_count_; concat_id++) {
    ...
    while (!queue.empty()) {
      ...
      if (IsConcat(curr)) {
        ...
        // Note that the 1st input (input number 0, the length) of the concat is
        // not used anymore, but we don't explicitely get rid of it, as it
        // should be removed later in the pipeline, thanks to
        // DeadCodeElimination. Often, this length is computed by adding the
        // length of the 2 strings of the concatenation, which introduces in
        // EffectControlLinearizer some LoadField(ForStringLength) nodes, which
        // cannot be removed by DeadCodeElimination because they are on the
        // effect chain. Nevertheless, the instruction selector will not emit
        // code for such nodes, since they'll have no uses (checked by
        // InstructionSelector::IsUsed).
      }
