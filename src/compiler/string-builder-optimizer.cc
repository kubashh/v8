...
bool StringBuilderOptimizer::CheckNodeUses(Node* node, Node* concat_child,
                                           Status status) {
  DCHECK_EQ(GetStatus(concat_child).state, State::kInConcat);
  ...
  bool is_loop_phi =
      node->opcode() == IrOpcode::kPhi && node_block->IsLoopHeader();
  bool child_is_in_loop = is_loop_phi && node_block->LoopContains(child_block);
...
void StringBuilderOptimizer::VisitNode(Node* node, BasicBlock* block) {
  if (IsConcat(node)) {
    ...
    if (node->UseCount() > 2) {
      ...
      // Ideally, we'd have a more precise analysis of uses, in order to allow
    ...
    if (IsLiteralString(lhs, broker())) {
      ...
      concat_starts_.push_back(node);
      // A kBeginConcat has no predecessor in the concatenation chain, and there
      // is thus no more checks/bookkeeping required ==> early return.
      return;
    } else {
...
void StringBuilderOptimizer::FinishConcatenations() {
  for (unsigned int concat_id = 0; concat_id < concat_count_; concat_id++) {
    ...
    if (start_status.state != State::kBeginConcat) {
      // {start} has already been invalidated. We set the corresponding entry
      // in {concat_starts_} to nullptr, in order to be able to use this later
      // to figure out if a node is really in a concat or not.
    ...
    // Collecting concatenation ends, and making sure that the concatenation
    // contains a loop phi.
    bool contains_loop_phi = false;
    ...
    while (!queue.empty()) {
      ...
      if (curr_status.state != State::kInConcat) {
        fail = true;
        break;
      }
      DCHECK_EQ(curr_status.id, start_status.id);

      if (curr->opcode() == IrOpcode::kPhi) {
        contains_loop_phi =
            contains_loop_phi || schedule()->block(curr)->IsLoopHeader();
      }
    ...
    // TODO(dm): finish this function!!! :D
  }
}
