      ...
      switch (input->opcode()) {
        ...
        default: {
          ...
          if (!range.has_value()) {
            ...
            states_[node->id()] = State::kOneByte;
            return State::kOneByte;
          } else if (range->second <= 0xFFFF) {
            states_[node->id()] = State::kTwoByte;
            return State::kTwoByte;
          } else {
            // For values greater than 0xFFFF, a truncation happens, which means
            // that the result could be either on 1 or 2 byte. We adopt a
            // convervative approach in that case and return kCantKnow here.
            states_[node->id()] = State::kCantKnow;
            return State::kCantKnow;
          }
        }
    ...
    while (!to_visit.empty()) {
      ...
      for (Node* next : curr->uses()) {
        ...
        if ((next_status.state == State::kInStringBuilder ||
          ...
          if (next_status.state == State::kInStringBuilder) {
            // We only add to {to_visit} when the state in kInStringBuilder to
