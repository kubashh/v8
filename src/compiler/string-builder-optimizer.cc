...
OneOrTwoByteAnalysis::State OneOrTwoByteAnalysis::OneOrTwoByte(Node* node) {
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kNewConsString: {
      ...
      //    string builder.
      //  - between the begining of the string builder and a literal string.
...
// Returns true if {node} keeps its input(s) alive. For instance, a Phi keeps
// its inputs alive, a Store keeps its input alive, but a kStringLength consumes
// its inputs.
...
// builder), but {other_child} is not). The string builder should not be used in
...
bool StringBuilderOptimizer::CheckNodeUses(Node* node,
  ...
  for (Node* other_child : node->uses()) {
    ...
    if (!OpcodeIsAllowed(other_child->opcode())) {
      // {other_child} could write {node} (the begining of the string builder)
