...
CallDescriptor* Linkage::GetStubCallDescriptor(
  ...
  // Add returns.
  Register return_registers[] = {kReturnRegister0, kReturnRegister1,
                                 kReturnRegister2};
  const size_t kMaxReturns = arraysize(return_registers);
  const size_t kMaxFPReturns = 1;
  ...
  for (size_t i = 0; i < locations.return_count_; i++) {
    ...
    if (IsFloatingPoint(type.representation())) {
      DCHECK_LT(num_fp_returns, kMaxFPReturns);
      if (num_fp_returns == kMaxFPReturns) continue;
      locations.AddReturn(regloc(kFPReturnRegister0, type));
      num_fp_returns++;
    } else {
      DCHECK_LT(num_returns, kMaxReturns);
      if (num_returns == kMaxReturns) continue;
