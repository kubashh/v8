...
int CallDescriptor::GetStackParameterDelta(
  ...
  if (ShouldPadArguments(stack_param_delta)) {
    // If the frame is misaligned due to the callee, we need to add padding.
    int callee_padding = ArgumentPaddingSlots(callee_slots_above_sp);
    stack_param_delta += callee_padding;
    // If the frame is misaligned due to the caller, we already have padding
    // that we can reuse for arguments, so we will need fewer slots.
    int caller_padding = ArgumentPaddingSlots(tail_caller_slots_above_sp);
    stack_param_delta -= caller_padding;
