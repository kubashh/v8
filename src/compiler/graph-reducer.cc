...
Reduction Reducer::DoReduce(Node* node,
                            ObserveNodeManager* observe_node_manager) {
  Reduction reduction = Reduce(node);
  if (reduction.Changed() && observe_node_manager) {
    observe_node_manager->OnNodeChanged(node, reduction.replacement());
  }
  return reduction;
}
...
GraphReducer::GraphReducer(Zone* zone, Graph* graph, TickCounter* tick_counter,
                           JSHeapBroker* broker,
                           ObserveNodeManager* observe_node_manager, Node* dead)
    : graph_(graph),
      dead_(dead),
      state_(graph, 4),
      reducers_(zone),
...
Reduction GraphReducer::Reduce(Node* const node) {
  ...
  for (auto i = reducers_.begin(); i != reducers_.end();) {
    if (i != skip) {
      ...
      Reduction reduction = (*i)->DoReduce(node, observe_node_manager_);
