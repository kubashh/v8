...
void GraphReducer::ReduceNode(Node* node) {
  ...
  for (;;) {
    ...
    } else if (got_broker && (!broker_->broker_task_queue_backlog_.empty() ||
  ...
  if (FLAG_trace_turbo_stw) {
    if (blocked_count != 0) {
      printf("ReduceNode blocked on MT %d times for %d busy-waits %d usec\n",
             blocked_count, busy_waits, busy_waits_in_usec);
    }
  }
