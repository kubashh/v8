...
void SerializerForBackgroundCompilation::ProcessMapForNamedPropertyAccess(
  ...
  if ((access_info.IsFastAccessorConstant() ||
    ...
    if (constant->IsJSFunction()) {
      ...
      JSBoundFunctionRef function = constant->AsJSBoundFunction();
      function.Serialize(NotConcurrentInliningTag{broker()});
    } else {
      FunctionTemplateInfoRef fti = constant->AsFunctionTemplateInfo();
      if (!broker()->is_concurrent_inlining()) {
...
void SerializerForBackgroundCompilation::ProcessNamedAccess(
  ...
  for (Handle<Object> hint : receiver->constants()) {
    ...
    if (access_mode == AccessMode::kLoad && object.IsJSFunction() &&
      ...
      if (result_hints != nullptr && function.has_instance_prototype()) {
        result_hints->AddConstant(function.instance_prototype().object(),
                                  zone(), broker());
      }
    }
    ...
    *walk_prototypes =
        *walk_prototypes ||
        (constructor.map().has_prototype_slot() &&
         constructor.AsJSFunction().has_instance_prototype() &&
