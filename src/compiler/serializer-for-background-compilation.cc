...
void SerializerForBackgroundCompilation::ProcessBuiltinCall(
  ...
  // TODO(mvstanton): Please check whether we should add the condition for
  // Process...ForPromises also for SpeculationMode::kDisallowSpeculation.
  switch (builtin_id) {
...
void SerializerForBackgroundCompilation::ProcessHintsForFunctionBind(
  ...
  for (auto constant : receiver_hints.constants()) {
    ...
    if (func.map().object()->NumberOfOwnDescriptors() < 2) {
      func.map().SerializeOwnDescriptors();
      for (int index = 0; index < func.map().object()->NumberOfOwnDescriptors();
           ++index)
        func.GetOwnAccessorProperty(index, true);
    }
  }
  ...
  for (auto map : receiver_hints.maps()) {
    ...
    if (map->NumberOfOwnDescriptors() < 2) map_ref.SerializeOwnDescriptors();
  }
}
