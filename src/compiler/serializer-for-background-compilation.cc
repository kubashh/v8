...
Hints& SerializerForBackgroundCompilation::Environment::LookupReturnValue() {
...
void SerializerForBackgroundCompilation::VisitLdar(
    interpreter::BytecodeArrayIterator* iterator) {
  Hints hints = environment()->LookupRegister(iterator->GetRegisterOperand(0));
  environment()->SetAccumulatorHints(hints);
}

void SerializerForBackgroundCompilation::VisitStar(
    interpreter::BytecodeArrayIterator* iterator) {
  Hints hints = environment()->LookupAccumulator();
