...
void ProcessMapForFunctionBind(MapRef map) {
  map.SerializePrototype();
  int number_of_own_descriptors = map.object()->NumberOfOwnDescriptors();
  if (number_of_own_descriptors >= 2) {
    for (int i = 0; i < std::min(2, number_of_own_descriptors); ++i) {
      map.SerializeOwnDescriptor(i);
    }
  }
}
...
void SerializerForBackgroundCompilation::ProcessHintsForFunctionBind(
  ...
  for (auto constant : receiver_hints.constants()) {
    ...
    JSFunctionRef func(broker(), constant);
    func.Serialize();
    ProcessMapForFunctionBind(func.map());
  }
