...
class SerializerForBackgroundCompilation::Environment : public ZoneObject {
 public:
  ...
  Environment(SerializerForBackgroundCompilation* serializer, Isolate* isolate,
              int register_count, bool has_receiver, Hints receiver_state,
...
SerializerForBackgroundCompilation::Environment::Environment(
    ...
    const HintsVector& arguments_state)
    : Environment(
          serializer->zone(), isolate, register_count,
          static_cast<int>(arguments_state.size()) + (has_receiver ? 1 : 0)) {
...
SerializerForBackgroundCompilation::SerializerForBackgroundCompilation(
    JSHeapBroker* broker, Zone* zone, Handle<JSFunction> closure,
    bool has_receiver, const Hints& receiver_state,
    const HintsVector& parameters_state)
    : broker_(broker),
      zone_(zone),
      environment_(new (zone) Environment(
          this, broker->isolate(),
          closure->shared()->GetBytecodeArray()->register_count(), has_receiver,
...
void SerializerForBackgroundCompilation::VisitCallProperty0(
  ...
  ProcessCall(callee_state, receiver_state, parameters_state, true);
}
...
void SerializerForBackgroundCompilation::ProcessCall(
    ...
    const HintsVector& parameters_state, bool has_receiver) {
  for (auto value : callee_state) {
    if (!value->IsJSFunction()) continue;

    RecurseOnCallee(receiver_state, parameters_state, has_receiver,
...
void SerializerForBackgroundCompilation::RecurseOnCallee(
    ...
    bool has_receiver, Handle<JSFunction> callee) {
  ...
  SerializerForBackgroundCompilation child_serializer(
      broker(), zone(), callee, has_receiver, receiver_state, parameters_state);
...
void SerializerForBackgroundCompilation::ProcessCallVarArgs(
  ...
  ProcessCall(callee_state, receiver_state, arguments_state,
