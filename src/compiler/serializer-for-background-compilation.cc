...
PropertyAccessInfo SerializerForBackgroundCompilation::ProcessMapForRegExpTest(
  ...
  if (ai_exec.IsDataFieldConstant() && ai_exec.holder().ToHandle(&holder)) {
  ...
  switch (access_mode) {
    case AccessMode::kLoad:
      ...
      if (access_info.IsDataFieldConstant() ||
          access_info.IsDataDictionaryProtoConstant()) {
        ...
        if (holder.has_value()) {
          ...
          base::Optional<ObjectRef> constant =
              access_info.IsDataFieldConstant()
    ...
    case AccessMode::kStoreInLiteral:
      ...
      if (access_info.IsDataField() || access_info.IsDataFieldConstant()) {
...
void SerializerForBackgroundCompilation::ProcessConstantForInstanceOf(
  ...
  if (access_info.IsNotFound()) {
    ProcessConstantForOrdinaryHasInstance(constructor_heap_object,
                                          walk_prototypes);
  } else if (access_info.IsDataFieldConstant()) {
    Handle<JSObject> holder;
