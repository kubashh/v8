...
class SerializerForBackgroundCompilation::Environment : public ZoneObject {
 public:
  ...
  void AddAccumulatorHints(const Hints& values);
...
SerializerForBackgroundCompilation::Environment::Environment(
    ...
    : Environment(serializer->zone(), isolate, register_count,
                  parameter_count) {
  environment_hints[kReceiverIndex] = receiver;

  size_t param_count = static_cast<size_t>(parameter_count);

  // Copy the state of the actually passed arguments, up until the expected
  // count taken from the bytecode.
  for (size_t i = 0; i < std::min(arguments.size(), param_count); ++i) {
    environment_hints[kParameterBase + i] = arguments[i];
  }
  ...
  for (size_t i = arguments.size(); i < param_count; ++i) {
    environment_hints[kParameterBase + i] = undefined_hint;
  }
}
...
Hints& SerializerForBackgroundCompilation::Environment::LookupAccumulator() {
  return environment_hints[accumulator_base()];
}
...
void SerializerForBackgroundCompilation::Environment::AddAccumulatorHints(
    const Hints& values) {
  for (auto v : values) {
    environment_hints[accumulator_base()].push_back(v);
  }
}
...
void SerializerForBackgroundCompilation::Environment::ReplaceAccumulatorHint(
  ...
  environment_hints[accumulator_base()].push_back(value);
}
...
void SerializerForBackgroundCompilation::Environment::SetAccumulatorHints(
    const Hints& hints) {
  environment_hints[accumulator_base()] = hints;
}
...
void SerializerForBackgroundCompilation::Environment::SetRegisterHints(
  ...
  int local_index = RegisterToLocalIndex(the_register);
  DCHECK(static_cast<size_t>(local_index) < environment_hints.size());
  environment_hints[local_index] = hints;
}
...
Hints SerializerForBackgroundCompilation::Run() {
  ...
  closure_ref.SerializeForCompilation();

  TraverseBytecode();

  return environment()->LookupAccumulator();
}
...
void SerializerForBackgroundCompilation::VisitCallUndefinedReceiver0(
  ...
  Hints callee = environment()->LookupRegister(iterator->GetRegisterOperand(0));
...
void SerializerForBackgroundCompilation::ProcessCall(
  ...
  for (auto value : callee) {
    ...
    if (!callee->shared()->HasBytecodeArray()) continue;
