...
void SerializerForBackgroundCompilation::ProcessHintsForFunctionBind(
  ...
  for (auto constant : receiver_hints.constants()) {
    ...
    int number_of_descriptors = func.map().object()->NumberOfOwnDescriptors();
    if (number_of_descriptors >= 2) {
      for (int i = 0; i < number_of_descriptors; ++i)
        func.map().SerializeOwnDescriptor(i);
    }
  }
  ...
  for (auto map : receiver_hints.maps()) {
    ...
    int number_of_descriptors = map_ref.object()->NumberOfOwnDescriptors();
    if (number_of_descriptors >= 2) {
      for (int i = 0; i < number_of_descriptors; ++i)
        map_ref.SerializeOwnDescriptor(i);
    }
  }
}
