...
MaybeHandle<Object> SerializerForBackgroundCompilation::SetValueOnObject(
    Handle<Object> object, Handle<Name> name, Handle<Object> value) {
  if (!object->IsJSReceiver()) return MaybeHandle<Object>();

  MaybeHandle<Object> prototype = JSReceiver::GetPrototype(
      broker()->isolate(),
      handle(JSReceiver::cast(*object), broker()->isolate()));
  if (prototype.is_null()) return MaybeHandle<Object>();

  MaybeHandle<JSReceiver> maybe_target =
      JSObject::ObjectCreate(broker()->isolate(), prototype.ToHandleChecked());
  if (maybe_target.is_null()) {
    return MaybeHandle<Object>();
  }

  Handle<JSReceiver> target = maybe_target.ToHandleChecked();
  Maybe<bool> copied =
      JSReceiver::SetOrCopyDataProperties(broker()->isolate(), target, object);
  if (!copied.IsJust()) return MaybeHandle<Object>();

  LookupIterator iterator = LookupIterator(broker()->isolate(), target, name);
  iterator.TryLookupCachedProperty();
  if (iterator.state() != LookupIterator::DATA &&
      iterator.state() != LookupIterator::NOT_FOUND)
    return MaybeHandle<Object>();

  if (iterator.state() == LookupIterator::DATA) {
    Maybe<bool> success =
        static_cast<Object*>(*target)->SetDataProperty(&iterator, value);
    if (!success.IsJust()) return MaybeHandle<Object>();
  } else if (iterator.state() == LookupIterator::NOT_FOUND) {
    Maybe<bool> success = static_cast<Object*>(*target)->AddDataProperty(
        &iterator, value, PropertyAttributes::NONE, ShouldThrow::kDontThrow,
...
void SerializerForBackgroundCompilation::VisitStaNamedProperty() {
  const InferredState& value_state = environment()->LookupAccumulator();
  InferredState& object_state =
      environment()->LookupRegister(bytecode_iterator().GetRegisterOperand(0));
  Handle<Name> name(
      Name::cast(bytecode_iterator().GetConstantForIndexOperand(1)),
  ...
  if (value_state.kind != InferredState::Kind::kValuesAndMaps) return;

  for (auto value : value_state.values) {
    if (object_state.kind != InferredState::Kind::kValuesAndMaps) continue;

    unsigned success = 0;
    for (size_t i = 0; i < object_state.values.size(); ++i) {
      Handle<Object> object = object_state.values[i];
      MaybeHandle<Object> new_object = SetValueOnObject(object, name, value);
      if (!new_object.is_null()) {
        object_state.values[i] = new_object.ToHandleChecked();
        ++success;
      }
    }
  }
...
void SerializerForBackgroundCompilation::VisitCreateEmptyObjectLiteral() {
  Handle<Object> empty_object =
      broker()->isolate()->factory()->NewJSObjectWithNullProto();
  environment()->AddSeenAccumulatorValue(empty_object);
}
