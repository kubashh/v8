...
Node* DeadCodeElimination::DeadValue(Node* none_node) {
  if (none_node->opcode() == IrOpcode::kDeadValue) return none_node;
  return graph()->NewNode(common()->DeadValue(), none_node);
}
