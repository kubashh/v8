...
class OneOrTwoByteAnalysis final {
 public:
  ...
  // TODO(dm): can we get rid of kEventuallyTwoByte?
  enum class State {
    kUnknown,
    kOneByte,            // Only 1-byte strings in the concatenation
    kTwoByte,            // Only 2-byte strings in the concatenation
    kEventuallyTwoByte,  // Mix of 1-byte and 2-byte strings
    kCantKnow
  };
  ...
  static void PrintOneOrTwoByte(State state) {
    switch (state) {
      ...
      case State::kTwoByte:
        PrintF("kTwoByte\n");
        break;
      case State::kEventuallyTwoByte:
        PrintF("kEventuallyTwoByte\n");
        break;
...
class V8_EXPORT_PRIVATE StringBuilderOptimizer final {
 ...
 private:
  ...
  void ComputeTrimmingsPerBlocks();
  ...
  ZoneVector<ZoneVector<BasicBlock*>>
      trimming_blocks;  // From concats to succesor Blocks
  ZoneVector<base::Optional<ZoneVector<Node*>>>
      reverse_trimmings;  // From Blocks to concat's Phis to trim
  ZoneVector<Node*> concat_lasts;
