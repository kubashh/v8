...
class V8_EXPORT_PRIVATE StringBuilderOptimizer final {
 ...
 private:
  ...
  struct Status {
    int id;  // The id of the StringBuilder that the node belongs to (or -1).
  ...
  const StringBuilder kInvalidStringBuilder = {
      nullptr, -1, false, OneOrTwoByteAnalysis::State::kUnknown};

#ifdef DEBUG
  bool StringBuilderIsValid(StringBuilder string_builder) {
    return string_builder.start != nullptr && string_builder.id != -1 &&
