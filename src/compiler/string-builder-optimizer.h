...
class V8_EXPORT_PRIVATE StringBuilderOptimizer final {
 ...
 private:
  ...
  // State DiscoverNode(Node* node);

  // void TryCandidate(Node* start) ;
  // NodeVector FindStringLiterals();
  NodeVector FindPhis();
  // void FindCandidatesFromStringLiterals(NodeVector string_literals);
  void FindCandidatesFromPhis(NodeVector phis);

  // void ReplaceStringBuild(Node* node);
