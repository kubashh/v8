...
base::Optional<ElementAccessInfo> AccessInfoFactory::ComputeElementAccessInfo(
  ...
  ObjectData* d = broker()->TryGetOrCreateData(map);
  if (d == nullptr) return {};

  // Check if it is safe to inline element access for the {map}.
  MapRef map_ref(broker(), d);
...
PropertyAccessInfo AccessInfoFactory::ComputeDataFieldAccessInfo(
  ...
  if (details_representation.IsSmi()) {
    ...
    if (descriptors_field_type->IsClass()) {
      ...
      ObjectData* d = broker()->TryGetOrCreateData(map);
      if (d == nullptr) return Invalid();
      field_type = Type::For(MapRef(broker(), d));
...
PropertyAccessInfo AccessorAccessInfoHelper(
  ...
  ObjectData* d = broker->TryGetOrCreateData(accessor);
  if (d == nullptr) return PropertyAccessInfo::Invalid(zone);

  if (!accessor->IsJSFunction()) {
    CallOptimization optimization(isolate, accessor, [&](Object o) {
      return broker->CanonicalPersistentHandle(o);
    });
...
PropertyAccessInfo AccessInfoFactory::ComputePropertyAccessInfo(
  ...
  while (true) {
    ...
    ObjectData* d = broker()->TryGetOrCreateData(
        map, false, ObjectRef::BackgroundSerialization::kAllowed);
    if (d == nullptr) return Invalid();

    if (!MapRef(broker(), d).TrySerializePrototype()) {
      return Invalid();
    }
...
base::Optional<ElementAccessInfo> AccessInfoFactory::ConsolidateElementLoad(
  ...
  for (auto const& group : feedback.transition_groups()) {
    for (Handle<Map> map_handle : group) {
      ObjectData* d = broker()->TryGetOrCreateData(map_handle);
      if (d == nullptr) return {};
      MapRef map(broker(), d);
...
PropertyAccessInfo AccessInfoFactory::LookupTransition(
  ...
  ObjectData* d = broker()->TryGetOrCreateData(transition_map);
  if (d == nullptr) return Invalid();
  MapRef transition_map_ref(broker(), d);
  ...
  }
  {
    ...
    if (d == nullptr) return Invalid();
    unrecorded_dependencies.push_back(
        dependencies()->TransitionDependencyOffTheRecord(MapRef(broker(), d)));
  }
