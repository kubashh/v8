...
bool CanInlinePropertyAccess(Handle<Map> map, AccessMode access_mode) {
  ...
  // We can only inline accesses to dictionary mode holders if the access is not
  // a store and the holder is a prototype. The latter ensures a 1:1
  ...
  if (map->IsJSObjectMap()) {
    if (map->is_dictionary_map()) {
      ...
      return !IsAnyStore(access_mode) && map->is_prototype_map();
    }
...
// static
PropertyAccessInfo PropertyAccessInfo::DataFieldConstant(
    Zone* zone, Handle<Map> receiver_map,
    ZoneVector<CompilationDependency const*>&& dependencies,
    FieldIndex field_index, Representation field_representation,
    Type field_type, Handle<Map> field_owner_map, MaybeHandle<Map> field_map,
    MaybeHandle<JSObject> holder, MaybeHandle<Map> transition_map) {
  return PropertyAccessInfo(kDataFieldConstant, holder, transition_map,
...
// static
PropertyAccessInfo PropertyAccessInfo::AccessorFieldConstant(
    Zone* zone, Handle<Map> receiver_map, Handle<Object> constant,
    MaybeHandle<JSObject> holder) {
  return PropertyAccessInfo(zone, kAccessorFieldConstant, holder, constant,
                            {{receiver_map}, zone});
}
...
// static
PropertyAccessInfo PropertyAccessInfo::DataDictionaryProtoConstant(
    Zone* zone, Handle<Map> receiver_map, Handle<JSObject> holder,
    InternalIndex dictionary_index) {
  return PropertyAccessInfo(zone, kDataDictionaryProtoConstant, holder,
                            {{receiver_map}, zone}, dictionary_index);
}

// static
PropertyAccessInfo PropertyAccessInfo::AccessorDictionaryProtoConstant(
    Zone* zone, Handle<Map> receiver_map, MaybeHandle<JSObject> holder,
    Handle<Object> constant) {
  return PropertyAccessInfo(zone, kAccessorDictionaryProtoConstant, holder,
...
PropertyAccessInfo::PropertyAccessInfo(
    ...
    : kind_(kind),
      lookup_start_object_maps_(lookup_start_object_maps),
      unrecorded_dependencies_(zone),  // FIXME: right?
      holder_(holder),
      field_representation_(Representation::None()),
      field_type_(Type::Any()),
  ...
  switch (this->kind_) {
    ...
    case kDataDictionaryProtoConstant: {
      ...
      if (this->dictionary_index_ == that->dictionary_index_) {
        // FIXME: need to merge this->unrecorded_dependencies_??
        this->lookup_start_object_maps_.insert(
            this->lookup_start_object_maps_.end(),
            that->lookup_start_object_maps_.begin(),
            that->lookup_start_object_maps_.end());
...
PropertyAccessInfo AccessInfoFactory::ComputeDataFieldAccessInfo(
  ...
  switch (constness) {
    ...
    case PropertyConstness::kConst:
      return PropertyAccessInfo::DataFieldConstant(
          zone(), receiver_map, std::move(unrecorded_dependencies), field_index,
          details_representation, field_type, field_owner_map, field_map,
          holder);
  }
...
PropertyAccessInfo AccessInfoFactory::ComputeAccessorDescriptorAccessInfo(
  ...
  auto mk_accessor_constant = [&](Handle<Object> accessor,
                                  MaybeHandle<JSObject> holder) {
    return PropertyAccessInfo::AccessorFieldConstant(zone(), receiver_map,
                                                     accessor, holder);
  };
...
PropertyAccessInfo AccessInfoFactory::ComputeDictionaryProtoAccessInfo(
  ...
  auto mk_accessor_constant = [&](Handle<Object> accessor,
                                  MaybeHandle<JSObject> holder) {
    return PropertyAccessInfo::AccessorDictionaryProtoConstant(
        zone(), receiver_map, holder, accessor);
  };
