...
PropertyAccessInfo AccessorAccessInfoHelper(
  ...
  if (access_mode == AccessMode::kHas) {
    // kHas is not supported for dictionary mode helpers, yet.
  ...
  if (map->is_dictionary_map()) {
    if (holder.is_null()) {
      // FIXME Question for review: This can only happen due to {holder
      // = optimization.LookupHolderOfExpectedType(...)}. Is it safe to fall
      // back to using the original holder (which is guaranteed to be non-null
      // in this situation)?
      return PropertyAccessInfo::Invalid(zone);
    }
...
PropertyAccessInfo AccessInfoFactory::ComputeAccessorDescriptorAccessInfo(
  ...
  // FIXME Question for review: This is making the lookup of the accessor object
  // lazy, because AccessorAccessInfoHelper may or may not need it. Should we
  // just always do the lookup here already and pass the result diretly to
  // AccessorAccessInfoHelper? It seems like the GetStrongValue should be fairly
  // cheap, but I'm not sure if it would always be legal to do the call here,
  // given that the original ComputeAccessorDescriptorAccessInfo may not have
  // done it in certain situations. Similar for ComputePropertyAccessInfo below,
  // but there the lookup is always guaranteed to succeed.
  auto get_accessors = [&]() {
    return handle(descriptors->GetStrongValue(descriptor), isolate());
  };
  return AccessorAccessInfoHelper(isolate(), zone(), broker(), this,
                                  receiver_map, name, map, holder, access_mode,
                                  get_accessors);
}
