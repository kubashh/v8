...
PropertyAccessInfo::PropertyAccessInfo(
    ...
    : kind_(kind),
      receiver_maps_(receiver_maps),
      unrecorded_dependencies_(unrecorded_dependencies),
...
PropertyAccessInfo AccessInfoFactory::ComputeDataFieldAccessInfo(
  ...
  MapRef map_ref(broker(), map);
  std::vector<CompilationDependencies::Dependency*> unrecorded_dependencies;
  ...
  return PropertyAccessInfo::DataField(
      details.constness(), MapHandles{receiver_map}, unrecorded_dependencies,
...
void PropertyAccessInfo::RecordDependencies(
    CompilationDependencies* dependencies) {
  for (CompilationDependencies::Dependency* d : unrecorded_dependencies_) {
...
PropertyAccessInfo AccessInfoFactory::LookupTransition(
  ...
  return PropertyAccessInfo::DataField(
      PropertyConstness::kMutable, MapHandles{map}, unrecorded_dependencies,
      field_index, field_representation, field_type, field_map, holder,
      transition_map);
}
