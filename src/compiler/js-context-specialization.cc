...
base::Optional<ContextHeapReference> GetSpecializationContext(
  ...
  switch (node->opcode()) {
    case IrOpcode::kHeapConstant: {
      ...
      if (object.IsContext())
        return base::Optional<ContextHeapReference>(object.AsContext());
      break;
    }
    case IrOpcode::kParameter: {
      OuterContext outer;
      if (maybe_outer.To(&outer) && IsContextParameter(node) &&
        ...
        *distance -= outer.distance;
        return base::Optional<ContextHeapReference>(
            broker->HeapReferenceForObject(outer.context).AsContext());
      }
      break;
    }
    default:
