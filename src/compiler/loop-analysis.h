...
class NodeCopier {
 public:
  ...
  NodeCopier(Graph* graph, size_t max, NodeVector* p, uint32_t copy_count)
      : node_map_(graph, static_cast<uint32_t>(max)),
        copies_(p),
        copy_count_(copy_count) {
    DCHECK_GT(copy_count, 0);
  }

  // Returns the mapping of {node} in the {copy_index}'th copy.
  ...
  void CopyNodes(Graph* graph, Zone* tmp_zone_, Node* dead, NodeRange nodes,
    ...
    for (Node* node : nodes) {
      ...
      if (inputs.size() < static_cast<size_t>(node->InputCount())) {
        inputs.resize(node->InputCount(), dead);
      }
      node_map_.Set(node, copies_->size() + 1);
      copies_->push_back(node);
