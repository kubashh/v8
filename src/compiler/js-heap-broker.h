...
class V8_EXPORT_PRIVATE JSHeapBroker {
 public:
  ...
  Handle<T> CanonicalPersistentHandle(T object) {
    if (canonical_handles_) {
      return Handle<T>(GetOrCreateCanonicalPersistentHandle(object.ptr()));
    } else {
 ...
 private:
  ...
  // {address} in the CanonicalHandlesMap. Create one if it does not exist.
  Address* GetOrCreateCanonicalPersistentHandle(Address address);
