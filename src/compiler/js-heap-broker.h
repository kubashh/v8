...
class V8_EXPORT_PRIVATE JSHeapBroker {
 public:
  ...
  template <typename T>
  Handle<T> CanonicalPersistentHandle(Object object) {
    return Handle<T>(GetOrCreateCanonicalPersistentHandle(object));
  }

  template <typename T>
  Handle<T> CanonicalPersistentHandle(Handle<Object> object) {
 ...
 private:
  ...
  // Return the location of the corresponding canonical persistent handle for
  // {object} in the CanonicalHandlesMap. Create one if it does not exist.
