...
class FieldConstantDepenedency final
    : public CompilationDependencies::Dependency {
 public:
  FieldConstantDepenedency(const MapRef& owner, int descriptor)
...
PropertyConstness CompilationDependencies::DependOnFieldConstant(
  ...
  bool can_have_fast_transitionable_elements_kind =
      Map::CanHaveFastTransitionableElementsKind(map.instance_type());
  if (can_have_fast_transitionable_elements_kind) {
    // If the map can already transition away, let us report the field as
    // mutable.
    if (!map.is_stable()) {
      return PropertyConstness::kMutable;
    }
    // If the map is stable, take a depenency on not transitioning in the
    // future.
  ...
  dependencies_.push_front(new (zone_)
                               FieldConstantDepenedency(owner, descriptor));
  return PropertyConstness::kConst;
}
