...
void DependOnStablePrototypeChain(CompilationDependencies* deps, MapRef map,
  ...
  while (true) {
    ...
    HeapObjectRef proto = map.prototype().AsHeapObject();
    if (!proto.IsJSObject()) {
...
void CompilationDependencies::DependOnStablePrototypeChains(
    MapContainer const& receiver_maps,
    base::Optional<JSObjectRef> last_prototype, WhereToStart start) {
...
template void CompilationDependencies::DependOnStablePrototypeChains(
    MapHandles const& receiver_maps, base::Optional<JSObjectRef> last_prototype,
    WhereToStart start);
template void CompilationDependencies::DependOnStablePrototypeChains(
    ZoneHandleSet<Map> const& receiver_maps,
    base::Optional<JSObjectRef> last_prototype, WhereToStart start);
