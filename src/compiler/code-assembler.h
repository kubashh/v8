  ...
  class CheckedNode {
   public:
    ...
    operator TNode<A>() {
      ...
      static_assert((std::is_same<PreviousType, MaybeObject>::value &&
                     std::is_same<A, HeapObjectReference>::value) ||
      ...
      if (FLAG_debug_code) {
        if (std::is_same<PreviousType, MaybeObject>::value) {
          if (std::is_same<A, HeapObjectReference>::value) {
            code_assembler_->GenerateCheckMaybeObjectIsHeapObjectReference(
...
#define TORQUE_CAST(x) ca_.Cast(x)
  ...
  void GenerateCheckMaybeObjectIsObject(TNode<MaybeObject> node,
                                        const char* location);
  void GenerateCheckMaybeObjectIsHeapObjectReference(TNode<MaybeObject> node,
                                                     const char* location);
#endif
