...
Frame::Frame(int fixed_frame_size_in_slots)
  ...
  slot_allocator_.Reserve(fixed_frame_size_in_slots);
}

int Frame::AlignFrame(int alignment) {
  int alignment_slots = AlignedSlotAllocator::NumSlots(alignment);
  ...
  int delta = alignment_slots - (slot_allocator_.End() & (alignment_slots - 1));
