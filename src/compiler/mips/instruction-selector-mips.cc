...
static const ShuffleEntry<4> arch_s32x4_shuffles[] = {
    {{0, 4, 1, 5}, kMipsS32x4InterleaveRight},
    {{2, 6, 3, 7}, kMipsS32x4InterleaveLeft},
    {{0, 2, 4, 6}, kMipsS32x4PackEven},
    {{1, 3, 5, 7}, kMipsS32x4PackOdd},
    {{0, 4, 2, 6}, kMipsS32x4InterleaveEven},
    {{1, 5, 3, 7}, kMipsS32x4InterleaveOdd}};

static const ShuffleEntry<8> arch_s16x8_shuffles[] = {
    {{0, 8, 1, 9, 2, 10, 3, 11}, kMipsS16x8InterleaveRight},
    {{4, 12, 5, 13, 6, 14, 7, 15}, kMipsS16x8InterleaveLeft},
    {{0, 2, 4, 6, 8, 10, 12, 14}, kMipsS16x8PackEven},
    {{1, 3, 5, 7, 9, 11, 13, 15}, kMipsS16x8PackOdd},
    {{0, 8, 2, 10, 4, 12, 6, 14}, kMipsS16x8InterleaveEven},
    {{1, 9, 3, 11, 5, 13, 7, 15}, kMipsS16x8InterleaveOdd},
    {{3, 2, 1, 0, 7, 6, 5, 4}, kMipsS16x4Reverse},
    {{1, 0, 3, 2, 5, 4, 7, 6}, kMipsS16x2Reverse}};
...
static const ArchOpcode kNoShuffle = kMipsS128Not;
...
void InstructionSelector::VisitS16x8Shuffle(Node* node) {
  ...
  Emit(kMipsS16x8Shuffle, g.DefineAsRegister(node),
       g.UseRegister(node->InputAt(0)), g.UseRegister(node->InputAt(1)),
       g.UseImmediate(Pack4Lanes(shuffle, mask)),
