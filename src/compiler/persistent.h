...
// PersMap is a persistent map datastructure based on hash trees (a binary tree
// using the bits of a hash value as addresses).
// Complexity:
...
class PersMap {
 ...
 private:
  ...
  struct Accessor {
    PersMap& map;
    const Key& key;
    PersMap& operator=(Value value) const { return map = map.Add(key, value); }
 ...
 public:
  ...
  PersMap Add(Key key, Value value) const {
    hash_t key_hash = hash_t(Hash()(key));
    ...
    return PersMap(map, zone_, def_value_);
  }
  ...
  bool operator==(const PersMap& other) const {
  ...
  bool operator!=(const PersMap& other) const { return !(*this == other); }
  ...
  Zipper Zip(const PersMap& other) const { return {*this, other}; }

  PersMap Join(const PersMap& other) const {
    PersMap result = *this;
  ...
  explicit PersMap(Zone* zone, Value def_value = Value())
      : PersMap(nullptr, zone, def_value) {}
 ...
 private:
  ...
  PersMap(const Map* map, Zone* zone, Value def_value)
...
std::ostream& operator<<(std::ostream& os,
                         const PersMap<Key, Value, Hash>& map) {
  os << "{";
  bool first = true;
  for (auto pair : map) {
    if (!first) os << ", ";
