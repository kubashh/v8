...
class DynamicCheckMapsParameters final {
 public:
  ...
  // TODO(gsathya): Don't emit extra deopt data to handle this like we
  // do for CallIC as we will not be updating the feedback on deopt.
  FeedbackSource const& feedback() const { return feedback_; }
...
class V8_EXPORT_PRIVATE SimplifiedOperatorBuilder final
 ...
 public:
  ...
  const Operator* DynamicCheckMaps(
      ...
      const DynamicCheckMapsParameters::ICState& ic_state);
