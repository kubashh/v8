    ...
    if (result == NodeProperties::kUnreliableReceiverMaps) {
      for (size_t i = 0; i < object_maps.size(); ++i) {
        dependencies()->DependOnStableMap(
            js_heap_broker()->MapRef(object_maps[i]));
      }
    }
...
Reduction JSCallReducer::ReduceArrayMap(Node* node,
  ...
  dependencies()->DependOnProtector(
      js_heap_broker()->PropertyCellRef(factory()->array_species_protector()));
  ...
  Node* a;  // Construct the output array.
  {
    ...
    for (int i = 0; i < initial_map->GetInObjectProperties(); ++i) {
      ab.Store(AccessBuilder::ForJSObjectInObjectProperty(
                   js_heap_broker()->MapRef(initial_map), i),
               jsgraph()->UndefinedConstant());
    }
    a = effect = ab.Finish();
  }
...
Reduction JSCallReducer::ReduceCallApiFunction(
  ...
  if (result == NodeProperties::kUnreliableReceiverMaps) {
    for (size_t i = 0; i < receiver_maps.size(); ++i) {
      dependencies()->DependOnStableMap(
          js_heap_broker()->MapRef(receiver_maps[i]));
    }
  }
...
Reduction JSCallReducer::ReduceAsyncFunctionPromiseCreate(Node* node) {
  ...
  dependencies()->DependOnProtector(
      js_heap_broker()->PropertyCellRef(factory()->promise_hook_protector()));
...
Reduction JSCallReducer::ReducePromisePrototypeCatch(Node* node) {
  ...
  dependencies()->DependOnProtector(
      js_heap_broker()->PropertyCellRef(factory()->promise_then_protector()));
...
Reduction JSCallReducer::ReducePromisePrototypeFinally(Node* node) {
  ...
  dependencies()->DependOnProtector(
      js_heap_broker()->PropertyCellRef(factory()->promise_then_protector()));
  dependencies()->DependOnProtector(js_heap_broker()->PropertyCellRef(
      factory()->promise_species_protector()));
