...
class FastApiCallReducerAssembler : public JSCallReducerAssembler {
 public:
  ...
  TNode<Object> ReduceFastApiCall(GraphAssemblerLabel<0> if_success,
    ...
    return FastApiCall(inputs.begin(), inputs.size(), &if_success, &if_error);
  }
 ...
 private:
  ...
  TNode<Object> FastApiCall(Node** inputs, size_t inputs_size,
                            ...
                            GraphAssemblerLabel<0>* if_error) {
    return AddNode<Object>(graph()->NewNode(
        simplified()->FastApiCall(c_candidate_functions_, feedback(),
                                  if_success, if_error),
...
Reduction JSCallReducer::ReduceCallApiFunction(Node* node,
  ...
  JSCallReducerAssembler gasm(this, node);
  auto if_success = gasm.MakeLabel();
  auto if_error = gasm.MakeDeferredLabel();
  if (!c_candidate_functions.empty() &&
    ...
    Node* fast_call_subgraph = a.ReduceFastApiCall(if_success, if_error);
  ...
  if (fast_node) {
    ...
    auto merge = gasm.MakeLabel(MachineRepresentation::kTagged);
    gasm.Goto(&merge, node);

    gasm.Bind(&if_success);
    gasm.Goto(&merge, fast_node);

    gasm.Bind(&merge);
    return Replace(merge.PhiAt(0));
  }
