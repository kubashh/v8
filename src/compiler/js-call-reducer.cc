...
// Called VNode, because TNode is taken and UNode sounds awkward.
// Maybe instead: iNode?
template <class T>
class VNode {
 public:
...
class JSCallReducerAssembler : public GraphAssembler {
 ...
 private:
  ...
  VNode<Any> ValueInput(int index) {
    return NodeProperties::GetValueInput(node_, index);
  }

  VNode<Any> ValueInputOrNaN(int index) {
    return node_->op()->ValueInputCount() > index
               ? NodeProperties::GetValueInput(node_, index)
               : NaNConstant();
  }

  VNode<Any> ValueInputOrUndefined(int index) {
    return node_->op()->ValueInputCount() > index
               ? NodeProperties::GetValueInput(node_, index)
               : UndefinedConstant();
  }
...
VNode<Any> JSCallReducerAssembler::ReduceMathBinary(VNode<Any> node,
                                                    const Operator* op) {
  VNode<Any> left = ValueInput(2);
  VNode<Any> right = ValueInputOrNaN(3);
  VNode<Number> left_number = SpeculativeToNumber(left, feedback());
  VNode<Number> right_number = SpeculativeToNumber(right, feedback());
...
VNode<String> JSCallReducerAssembler::ReduceStringPrototypeSubstring(
    VNode<Any> node) {
  VNode<Any> receiver = ValueInput(1);
  VNode<Any> start = ValueInput(2);
  VNode<Any> end = ValueInputOrUndefined(3);

  VNode<String> receiver_string = CheckString(receiver, feedback());
  VNode<Smi> start_smi = CheckSmi(start, feedback());

  VNode<Smi> length = StringLength(receiver_string);

  VNode<Smi> end_smi = VNode<Smi>::UncheckedCast(
      SelectIf(IsUndefined(end))
          .Then(λ { return length; })
          .Else(λ { return CheckSmi(end, feedback()); })
          .Value());

  VNode<Number> finalStart = NumberMin(NumberMax(start_smi, Zero()), length);
  VNode<Number> finalEnd = NumberMin(NumberMax(end_smi, Zero()), length);
