...
class MapInspection {
 public:
  ...
  // The following methods make it safe to rely on the information provided by
  // {GetMaps} and {AllOfInstanceTypes}.
  //
  ...
  //   if (foo) return reduction.NoChange();
 ...
 private:
  ...
  MapHandles maps_;
  enum { kReliable, kUnreliableUnused, kUnreliableUsed } maps_state_;

  V8_WARN_UNUSED_RESULT bool RelyOnMapsInternal(
...
MapInspection::MapInspection(JSHeapBroker* broker, Node* object, Node* effect)
    : broker_(broker), object_(object), maps_state_(kReliable) {
  ...
  if (result == NodeProperties::kUnreliableReceiverMaps) {
    DCHECK(!maps_.empty());
    maps_state_ = kUnreliableUnused;
  }
  DCHECK_EQ(maps_.empty(), result == NodeProperties::kNoReceiverMaps);
}

MapInspection::~MapInspection() { CHECK_NE(maps_state_, kUnreliableUsed); }

bool MapInspection::HaveMaps() const { return !maps_.empty(); }

bool MapInspection::AllOfInstanceTypes(bool (*f)(InstanceType)) {
  DCHECK(!maps_.empty());
  if (maps_state_ == kUnreliableUnused) maps_state_ = kUnreliableUsed;
  return std::all_of(maps_.begin(), maps_.end(), [f](Handle<Map> map) {
...
void MapInspection::InsertMapChecks(JSGraph* jsgraph, Node** effect,
  ...
  *effect = jsgraph->graph()->NewNode(
  ...
  maps_state_ = kReliable;
}

bool MapInspection::RelyOnMapsViaStability(
    CompilationDependencies* dependencies) {
  return RelyOnMapsInternal(dependencies, nullptr, nullptr, nullptr, {});
}

bool MapInspection::RelyOnMapsPreferStability(
    CompilationDependencies* dependencies, JSGraph* jsgraph, Node** effect,
    Node* control, const VectorSlotPair& feedback) {
  if (maps_state_ != kUnreliableUsed) return false;
  if (RelyOnMapsViaStability(dependencies)) return true;
  CHECK(RelyOnMapsInternal(nullptr, jsgraph, effect, control, feedback));
  return false;
}

bool MapInspection::RelyOnMapsInternal(CompilationDependencies* dependencies,
                                       JSGraph* jsgraph, Node** effect,
                                       Node* control,
                                       const VectorSlotPair& feedback) {
  CHECK(!maps_.empty());

  if (maps_state_ != kUnreliableUsed) return true;
...
Reduction JSCallReducer::ReduceArrayIterator(Node* node, IterationKind kind) {
  ...
  if (!inspection.HaveMaps() ||
      !inspection.AllOfInstanceTypes(InstanceTypeChecker::IsJSReceiver) ||
      !inspection.RelyOnMapsViaStability(dependencies())) {
    return inspection.NoChange();
  }
