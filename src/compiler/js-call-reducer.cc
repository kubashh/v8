...
bool CanInlineJSToWasmCall(const wasm::CanonicalSig* wasm_signature) {
  ...
  for (auto type : wasm_signature->all()) {
    ...
    if (type == wasm::CanonicalValueType::Primitive(wasm::kI64)) return false;
#endif
    if (type != wasm::CanonicalValueType::Primitive(wasm::kI32) &&
        type != wasm::CanonicalValueType::Primitive(wasm::kI64) &&
        type != wasm::CanonicalValueType::Primitive(wasm::kF32) &&
        type != wasm::CanonicalValueType::Primitive(wasm::kF64) &&
        type != wasm::CanonicalValueType::Ref(wasm::HeapType::kExtern)) {
      return false;
    }
  }
