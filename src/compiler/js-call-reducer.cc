...
bool CanInlineArrayResizingBuiltin(
    JSHeapBroker* broker, std::vector<ArrayInlineInfo> array_inline_infos,
    std::vector<ElementsKind>* kinds, bool builtin_is_push = false) {
  DCHECK_NE(0, array_inline_infos.size());
  for (ArrayInlineInfo info : array_inline_infos) {
    if (!info.supports_fast_array_resize) return false;
    // TODO(turbofan): We should also handle fast holey double elements once
    // we got the hole NaN mess sorted out in TurboFan/V8.
    if (info.elements_kind == HOLEY_DOUBLE_ELEMENTS && !builtin_is_push) {
      return false;
    }
    ElementsKind current_kind = info.elements_kind;
...
Reduction JSCallReducer::ReduceArrayPrototypePush(Node* node) {
  ...
  std::vector<ArrayInlineInfo> array_inline_infos =
      inference.GetArrayInlineInfos();
  if (array_inline_infos.empty()) return NoChange();

  std::vector<ElementsKind> kinds;
  if (!CanInlineArrayResizingBuiltin(broker(), array_inline_infos, &kinds,
                                     true)) {
    return inference.NoChange();
  }
  if (!dependencies()->DependOnNoElementsProtector()) UNREACHABLE();
...
Reduction JSCallReducer::ReduceArrayPrototypeSlice(Node* node) {
  ...
  for (ArrayInlineInfo info : array_inline_infos) {
    if (!info.supports_fast_array_resize) return inference.NoChange();
    if (IsHoleyElementsKind(info.elements_kind)) {
      can_be_holey = true;
    }
  }
