...
Reduction JSCallReducer::ReduceDataViewPrototypeGet(
  ...
  // Only do stuff is the {receiver} is really a DataView.
  if (NodeProperties::HasInstanceTypeWitness(isolate(), receiver, effect,
                                             JS_DATA_VIEW_TYPE)) {
    ...
    // The end offset is the offset plus the byte length
    // of the type that we want to load.
    // Since we only load int8 and uint8 for now, that length is 1.
    Node* end_offset = graph()->NewNode(simplified()->NumberAdd(), offset,
                                        jsgraph()->OneConstant());

    // We need to check that {end_offset} <= {byte_length}.
    Node* ok_range = graph()->NewNode(simplified()->NumberLessThanOrEqual(),
                                      end_offset, byte_length);
    Node* check_range = graph()->NewNode(simplified()->BooleanNot(), ok_range);
    ...
    Node* efalse_range = efalse_neutered;
    {
      ...
      // Perform the load.
      vfalse_neutered = efalse_range = graph()->NewNode(
    ...
    // Continue on the regular path.
    ReplaceWithValue(node, vfalse_neutered, efalse_range, if_false_range);
    return Changed(vfalse_neutered);
  }
