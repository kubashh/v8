...
bool JSCallReducer::property_accesses_were_serialized() const {
...
Reduction JSCallReducer::ReduceFunctionPrototypeBind(Node* node) {
  ...
  if (!first_receiver_map.is_never_serialized() &&
  ...
  for (Handle<Map> const map : receiver_maps) {
    ...
    if (!receiver_map.is_never_serialized() &&
...
Reduction JSCallReducer::ReduceObjectGetPrototype(Node* node, Node* object) {
  ...
  if (!candidate_map.is_never_serialized() &&
  ...
  for (size_t i = 0; i < object_maps.size(); ++i) {
    ...
    if (!object_map.is_never_serialized() &&
