 ...
 private:
  ...
  TNode<JSFunction> CreateClosureFromBuiltinSharedFunctionInfo(
    ...
    Callable const callable =
        Builtins::CallableFor(isolate(), shared.builtin());
...
Reduction JSCallReducer::ReduceJSCall(Node* node,
  ...
  Builtin builtin =
      shared.HasBuiltinId() ? shared.builtin() : Builtin::kNoBuiltinId;
  switch (builtin) {
...
Node* JSCallReducer::CreateClosureFromBuiltinSharedFunctionInfo(
  ...
  Handle<FeedbackCell> feedback_cell =
      isolate()->factory()->many_closures_cell();
  Callable const callable = Builtins::CallableFor(isolate(), shared.builtin());
  return graph()->NewNode(
      javascript()->CreateClosure(shared.object(), callable.code()),
