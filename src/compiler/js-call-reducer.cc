...
Reduction JSCallReducer::ReduceFunctionPrototypeBind(Node* node) {
  ...
  for (Handle<Map> const map : receiver_maps) {
    ...
    // runtime otherwise.
    if (!receiver_map.instance_descriptors().has_value()) {
      TRACE_BROKER_MISSING(broker(),
    ...
    if (!receiver_map.GetStrongValue(JSFunction::kLengthDescriptorIndex)
             .IsAccessorInfo()) {
      return inference.NoChange();
    }
    if (!receiver_map.GetStrongValue(JSFunction::kNameDescriptorIndex)
             .IsAccessorInfo()) {
      return inference.NoChange();
    }
  }
