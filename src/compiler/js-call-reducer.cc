...
enum class FastCallArgumentCategory : uint8_t {
  kInvalid,
  kSequence,    // CTypeInfo::sequence_type_ == kIsSequence
  kTypedArray,  // CTypeInfo::sequence_type_ == kIsTypedArray
};

struct FastCallOverloadsResolutionResult {
  explicit FastCallOverloadsResolutionResult(Zone* zone)
      : distinguishable_arg_index(-1),
        categories(zone),
        typed_array_type(CTypeInfo::Type::kVoid) {}
  FastCallOverloadsResolutionResult(
      int distinguishable_arg_index_,
      const ZoneVector<FastCallArgumentCategory>& categories_,
      CTypeInfo::Type typed_array_type_)
      : distinguishable_arg_index(distinguishable_arg_index_),
        categories(categories_),
        typed_array_type(typed_array_type_) {}

  bool is_valid() { return distinguishable_arg_index >= 0; }

  int distinguishable_arg_index;
  ZoneVector<FastCallArgumentCategory> categories;
  CTypeInfo::Type typed_array_type;
};
 ...
 public:
  ...
  Node* GenerateGraph(const ZoneVector<Node*>& inputs, int arg_index,
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
      Node* check_undefined =
          graph()->NewNode(simplified()->ReferenceEqual(), inputs[i],
  ...
  Node* GenerateOverloadResolutionGraph(
    ...
    Bind(&if_heap_object_label);
    {
      ...
      DCHECK_EQ(overloads_resolution.categories.size(), 2);
      for (size_t i = 0; i < overloads_resolution.categories.size(); i++) {
        auto next = MakeLabel();

        if (overloads_resolution.categories[i] ==
            FastCallArgumentCategory::kSequence) {
        ...
        } else if (overloads_resolution.categories[i] ==
                   FastCallArgumentCategory::kTypedArray) {
          ...
          ElementsKind typed_array_elements_kind =
              GetTypedArrayElementsKind(overloads_resolution.typed_array_type);
...
namespace {
FastCallArgumentCategory GetFastCallArgumentCategory(
    const CTypeInfo* type_info) {
  if (type_info->GetSequenceType() == CTypeInfo::SequenceType::kIsSequence) {
    return FastCallArgumentCategory::kSequence;
  } else if (type_info->GetSequenceType() ==
             CTypeInfo::SequenceType::kIsTypedArray) {
    return FastCallArgumentCategory::kTypedArray;
  } else {
    return FastCallArgumentCategory::kInvalid;
  }
}
...
FastCallOverloadsResolutionResult ResolveFastCallOverloads(
  ...
  for (unsigned int arg_index = 0; arg_index < arg_count; arg_index++) {
    using CategoryFunctionMap = ZoneMultimap<FastCallArgumentCategory, int>;
    CategoryFunctionMap map(zone);
    for (size_t i = 0; i < candidates.size(); i++) {
      const CTypeInfo& type_info =
          candidates[i].second->ArgumentInfo(arg_index + kReceiver);
      FastCallArgumentCategory category =
          GetFastCallArgumentCategory(&type_info);
      map.insert(std::pair<FastCallArgumentCategory, int>(category,
                                                          static_cast<int>(i)));
    }

    // Overload resolution only supported with sequences and typed arrays.
    // The presence of a JSArray or of a typed array argument identifies the
    // distinguishable_arg_index.
    if (map.find(FastCallArgumentCategory::kSequence) != map.end() &&
        map.find(FastCallArgumentCategory::kTypedArray) != map.end()) {
      // Only the case of two candidate functions, one with a JSArray param and
      // the other with a typed array param is currently supported;
      DCHECK_EQ(map.size(), 2);

      // At the distinguishable argument index all types must be
      // distinguishable and valid.
      if (map.find(FastCallArgumentCategory::kInvalid) != map.end()) {
        return FastCallOverloadsResolutionResult(zone);
      }

      ZoneVector<FastCallArgumentCategory> categories(zone);
      categories.resize(candidates.size());
      CTypeInfo::Type typed_array_type = CTypeInfo::Type::kVoid;
      for (auto it = map.begin(); it != map.end(); ++it) {
        FastCallArgumentCategory category = it->first;
        // If there are multiple functions with the same category for the i-th
        // argument the overload resolution fails.
        if (map.count(category) != 1) {
          return FastCallOverloadsResolutionResult(zone);
        }
        int candidate_index = it->second;
        categories[candidate_index] = category;
        if (category == FastCallArgumentCategory::kTypedArray) {
          typed_array_type = candidates[candidate_index]
                                 .second->ArgumentInfo(arg_index + kReceiver)
                                 .GetType();
        }
      }
      DCHECK_EQ(categories.size(), 2);
      return {static_cast<int>(arg_index), categories, typed_array_type};
    } else {
      // Before the distinguishable argument index the types must match. So
      // there must be only one category in the map.
      DCHECK_GT(map.size(), 0);
      FastCallArgumentCategory category = map.begin()->first;
      if (map.upper_bound(category) != map.end()) {
        return FastCallOverloadsResolutionResult(zone);
      }
    }
  }
  // No overload found with a JSArray or of a typed array argument.
  return FastCallOverloadsResolutionResult(zone);
}
