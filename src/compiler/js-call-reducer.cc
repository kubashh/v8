...
Reduction JSCallReducer::ReduceArrayFilter(Handle<JSFunction> function,
  ...
  // TODO(mvstanton) : should I go ahead and transition it?
  Node* a = control = effect = graph()->NewNode(
      javascript()->CreateArray(1, Handle<AllocationSite>::null()),
  ...
  std::vector<Node*> checkpoint_params1({receiver, fncallback, this_arg, a, k,
                                         original_length, to, callback_value});
  const int stack_parameters1 = static_cast<int>(checkpoint_params1.size());

  frame_state = CreateJavaScriptBuiltinContinuationFrameState(
      jsgraph(), function, Builtins::kArrayFilterLoopLazyDeoptContinuation,
      node->InputAt(0), context, &checkpoint_params1[0], stack_parameters1,
      outer_frame_state, ContinuationFrameStateMode::EAGER);

  // We have to coerce callback_value to boolean, and only store the element in
  // a if it's true.
  to = DoFilterPostCallbackWork(kind, context, frame_state, &control, &effect,
                                a, to, element, callback_value);
  k = next_k;
...
Node* JSCallReducer::DoFilterPostCallbackWork(ElementsKind kind, Node* context,
  ...
  Node* vtrue;
  {
    ...
    // MaybeGrowFastElements may force deoptimization. We've been given a
    // frame_state that re-runs all operations from the start of this helper
    // function. The ToBoolean will re-run, but it doesn't have side effects
    // so that's okay.
    etrue =
        graph()->NewNode(common()->Checkpoint(), frame_state, etrue, if_true);
    ...
    elements = etrue =
        graph()->NewNode(simplified()->MaybeGrowFastElements(flags), a,
                         elements, to, to, etrue, if_true);
    ...
    vtrue = graph()->NewNode(simplified()->NumberAdd(), to,
                             jsgraph()->OneConstant());
  }
