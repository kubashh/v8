...
Reduction JSCallReducer::ReduceJSCall(Node* node) {
  ...
  if (feedback->IsWeakCell()) {
    ...
    if (cell->value()->IsCallable()) {
      ...
      Reduction const reduction = ReduceJSCall(node);
      return reduction.Changed() ? reduction : Changed(node);
    } else if (cell->value()->IsFeedbackCell()) {
      Handle<FeedbackCell> feedback_cell(FeedbackCell::cast(cell->value()),
                                         isolate());
      if (feedback_cell->value()->IsFeedbackVector()) {
        ...
        Node* target_closure = effect = graph()->NewNode(
            simplified()->CheckClosure(feedback_cell), target, effect, control);
