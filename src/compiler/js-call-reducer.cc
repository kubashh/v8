...
Reduction JSCallReducer::ReduceArrayIndexOfIncludes(
  ...
  Callable const callable =
      search_variant == SearchVariant::kIndexOf
          ...
          : GetCallableForArrayIncludes(receiver_map->elements_kind(),
                                        isolate());
  // The operator flags should be Operator::kEliminatable, containing
  ...
  // output, reporting a GraphError that the call to ArrayIndexOf does not
  // produce control output used by node Return.
  ...
  Node* replacement_node = graph()->NewNode(
      ...
      search_element, length, new_from_index, context, effect, control);
...
Reduction JSCallReducer::ReduceTypedArrayIterator(Node* node,
                                                  IterationKind kind) {
  Handle<Map> receiver_map;
  if (NodeProperties::GetMapWitness(node).ToHandle(&receiver_map) &&
...
Reduction JSCallReducer::ReduceArrayIterator(Handle<Map> receiver_map,
...
Reduction JSCallReducer::ReduceFastArrayIteratorNext(InstanceType type,
                                                     Node* node,
                                                     IterationKind kind) {
...
Reduction JSCallReducer::ReduceTypedArrayIteratorNext(InstanceType type,
                                                      Node* node,
                                                      IterationKind kind) {
  Node* iterator = NodeProperties::GetValueInput(node, 1);
  Node* effect = NodeProperties::GetEffectInput(node);
...
Reduction JSCallReducer::ReduceArrayIteratorNext(Node* node) {
