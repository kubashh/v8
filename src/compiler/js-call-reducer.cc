...
Reduction JSCallReducer::ReduceStringPrototypeSlice(Node* node) {
  ...
  Node* value = graph()->NewNode(
      common()->Select(MachineRepresentation::kTagged, BranchHint::kFalse),
      graph()->NewNode(simplified()->NumberLessThan(), from, to), substring,
...
Reduction JSCallReducer::ReduceStringPrototypeSubstr(Node* node) {
  ...
  Node* resultLength = graph()->NewNode(
      ...
      graph()->NewNode(simplified()->NumberMax(), end,
                       jsgraph()->ZeroConstant()),
      graph()->NewNode(simplified()->NumberSubtract(), length, start));
  ...
  Node* to = effect = graph()->NewNode(
      ...
      effect, control);
  Node* substring = graph()->NewNode(simplified()->StringSubstring(), receiver,
                                     initStart, to);
  Node* value = graph()->NewNode(
      common()->Select(MachineRepresentation::kTagged, BranchHint::kFalse),
      graph()->NewNode(simplified()->NumberLessThan(),
                       jsgraph()->ZeroConstant(), resultLength),
      substring, jsgraph()->EmptyStringConstant());
  ReplaceWithValue(node, value, effect, control);
  return Replace(node);
}
