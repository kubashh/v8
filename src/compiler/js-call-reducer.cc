...
class JSBuiltinReducerHelper {
 public:
  explicit JSBuiltinReducerHelper(JSCallReducer* jscallreducer)
      : jscallreducer_(jscallreducer) {}

 protected:
  static Reduction Replace(Node* node) { return JSCallReducer::Replace(node); }

  void Replace(Node* node, Node* replacement) {
    jscallreducer_->Replace(node, replacement);
  }
  void Revisit(Node* node) { jscallreducer_->Revisit(node); }

  void ReplaceWithValue(Node* node, Node* value, Node* effect = nullptr,
                        Node* control = nullptr) {
    jscallreducer_->ReplaceWithValue(node, value, effect, control);
  }
  ...
  Graph* graph() const { return jscallreducer_->graph(); }

  JSGraph* jsgraph() const { return jscallreducer_->jsgraph(); }
  Isolate* isolate() const { return jscallreducer_->isolate(); }

  Factory* factory() const { return jscallreducer_->factory(); }

  CommonOperatorBuilder* common() const { return jscallreducer_->common(); }

  JSOperatorBuilder* javascript() const { return jscallreducer_->javascript(); }

  SimplifiedOperatorBuilder* simplified() const {
    return jscallreducer_->simplified();
  }
  Handle<Context> native_context() const {
    return jscallreducer_->native_context();
  }
  CompilationDependencies* dependencies() const {
    return jscallreducer_->dependencies();
  }
  Handle<JSGlobalProxy> global_proxy() const {
    return jscallreducer_->global_proxy();
  }

 private:
  JSCallReducer* const jscallreducer_;
};

class JSCallReducer::JSMathBuiltinReducer : public JSBuiltinReducerHelper {
 public:
  explicit JSMathBuiltinReducer(JSCallReducer* jscallreducer)
      : JSBuiltinReducerHelper(jscallreducer) {}
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceMathUnary(
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceMathBinary(
    Node* node, const Operator* op) {
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceMathImul(Node* node) {
  if (node->op()->ValueInputCount() < 4) {
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceMathClz32(Node* node) {
  if (node->op()->ValueInputCount() < 3) {
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceMathMinMax(
    Node* node, const Operator* op, Node* empty_value) {
...
Reduction JSCallReducer::JSMathBuiltinReducer::ReduceJSCall(
...
Reduction JSCallReducer::ReduceJSCall(Node* node) {
  ...
  if (m.HasValue()) {
    if (m.Value()->IsJSFunction()) {
      ...
      JSMathBuiltinReducer math_reducer(this);
