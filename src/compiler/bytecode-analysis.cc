...
void BytecodeAnalysis::Analyze() {
  ...
  for (iterator.GoToEnd(); iterator.IsValid(); --iterator) {
    ...
    // 2) If we have a JumpLoop that jumps to istelf (i.e empty loop) we have to
    // pop if from the loop_stack_ now.
    bool pop_from_stack = bytecode != Bytecode::kJumpLoop ||
                          (bytecode == Bytecode::kJumpLoop &&
                           iterator.GetJumpTargetOffset() == current_offset);

    if (loop_stack_.size() > 1 && pop_from_stack) {
