...
class GraphAssembler {
 public:
  ...
  void InitializeEffectControl(Node* node);
...
#undef BINOP_DECL
  ...
  Node* StoreElement(ElementAccess const&, Node* object, Node* index,
                     Node* value);
  Node* SpeculativeToNumber(
      Node* value, FeedbackSource const& feedback,
      NumberOperationHint hint = NumberOperationHint::kNumberOrOddball);
  Node* CheckSmi(Node* value, FeedbackSource const& feedback);
  Node* CheckString(Node* value, FeedbackSource const& feedback);
  ...
  Node* Select(Node* condition, NodeGenerator true_body,
  ...
  // TODO(jgruber): Consider removing these in favor of shorter variants below.
  Node* current_control() { return current_control_; }
  Node* current_effect() { return current_effect_; }

  Node* control() { return current_control_; }
  Node* effect() { return current_effect_; }

 private:
  Node* NewNode(const Operator* op, Node* input0);
