...
Node* GetNodeAddr(const Node* node) {
...
bool IsContinuousAccess(const std::vector<Node*>& node_group) {
...
bool AllConstant(const std::vector<Node*>& node_group) {
  for (Node* node : node_group)
    if (!NodeProperties::IsConstant(node)) return false;
  return true;
}

bool AllSameAddr(const std::vector<Node*>& nodes) {
...
bool IsSplat(const T& node_group) {
  for (typename T::size_type i = 1, e = node_group.size(); i < e; ++i) {
...
// otherwise.
bool AllSameOperator(const std::vector<Node*>& node_group) {
  auto op = node_group[0]->op();
  for (std::vector<Node*>::size_type i = 1, e = node_group.size(); i < e; i++) {
    if (node_group[i]->op() != op) return false;
  }
  return true;
}
}  // anonymous namespace
...
void SLPTree::ClearStack() {
  stack_ = ZoneStack<std::vector<Node*>>(zone_);
  on_stack_.clear();
}
...
PackNode* SLPTree::BuildTreeRec(const std::vector<Node*>& node_group,
  ...
  switch (op) {
    ...
    case IrOpcode::kProtectedStore: {
      ...
      PackNode* pnode = BuildPackNode(node_group, 2, 1, recursion_depth);
      PopStack();
      return pnode;
    }
    default:
...
bool Revectorizer::TryRevectorize(const char* function) {
  bool success = false;
  if (graph_->HasSimd() && (has_avx_ || has_avx2_) &&
      graph_->GetStoreNodes().size()) {
