...
bool SLPTree::IsSupported(const std::vector<Node*>& node_group) {
  ...
  IrOpcode::Value op = node_group[0]->opcode();
  if (!NodeProperties::IsSimd(node_group[0]) && (op != IrOpcode::kStore) &&
...
bool Revectorizer::TryRevectorize(const char* function) {
  ...
  snprintf(target, sizeof(target), "wasm-function#%d",
           FLAG_wasm_revectorize_function_index.value());

  if (FLAG_wasm_revectorize_function_index != -1 && function != nullptr &&
