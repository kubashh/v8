...
struct GraphBuilder {
  ...
  // Projections should always be in the same block as their input Operation. We
  // enforce this when building the turboshaft graph by always generating
  // Projections after an operation as well as a Tuple of those Projections
  // after: this way, subsequent Projections to the same Operation will be
  // Projections to Tuples, which are automatically removed. However, for
  // CallAndCatchExceptionOps, this isn't possible, because it is a block
  // terminator. When we reach a Turbofan IfSuccess, we thus generate the
  // projections corresponding to the CallAndCatchException that was ending the
  // previous block, and we store them in {call_projections}, which acts as a
  // Projection cache: subsequent identical Projections will not be emitted and
  // will instead be fetched from this cache.
  ZoneUnorderedMap<OpIndex, base::Vector<OpIndex>> call_projections{phase_zone};

  // When emitting Operations that have more than one outputs, we actually emit
  // all projections and return a tuple of projections rather than returning the
  // Operation itself. {tuple_inputs} is (re)used to store the tuple's input,
  // so that we don't have to reallocate memory every time we want to emit such
  // operation.
  ZoneVector<OpIndex> tuple_inputs{phase_zone};

  base::Optional<BailoutReason> Run();

 private:
  OpIndex Map(Node* old_node, bool keep_call = false) {
    OpIndex result = op_mapping.Get(old_node);
    DCHECK(assembler.output_graph().IsValid(result));
    if (keep_call) return result;
    if (assembler.output_graph().Get(result).Is<CallAndCatchExceptionOp>()) {
      // CallAndCatchExceptionOps should never be used directly, except by
      // Projections. If it's used directly, then it means that it has a single
      // output, which in Turbofan can be used without projections. In
      // turboshaft however we always use projections, because a
      // CallAndCatchExceptionOp always at at least 2 outputs: output and
      // exception.
      return call_projections[result][0];
    }
    return result;
  }
  ...
  OpIndex EmitProjectionsAndTuple(OpIndex op_idx) {
    ...
    tuple_inputs.clear();
    for (size_t i = 0; i < op.outputs_rep().size(); i++) {
      tuple_inputs.push_back(
          assembler.Projection(op_idx, i, op.outputs_rep()[i]));
    }
    return assembler.ReduceTuple(base::VectorOf(tuple_inputs));
  }
};
...
OpIndex GraphBuilder::Process(
  ...
  switch (opcode) {
    ...
    case IrOpcode::kIfException: {
      ...
      OpIndex call_idx = Map(call, true);
      CallAndCatchExceptionOp& op = assembler.output_graph()
                                        .Get(call_idx)
                                        .Cast<CallAndCatchExceptionOp>();
      return assembler.Projection(call_idx, op.ExceptionOffset(),
                                  WordRepresentation::Tagged());
    }

    case IrOpcode::kIfSuccess: {
      // We emit all of the value projections of the call now.
      Node* call = node->InputAt(0);
      DCHECK_EQ(call->opcode(), IrOpcode::kCall);
      OpIndex call_idx = Map(call, true);
      CallAndCatchExceptionOp& op = assembler.output_graph()
      ...
      size_t return_count = op.descriptor->descriptor->ReturnCount();
      DCHECK_EQ(return_count, op.outputs_rep().size() - 1);
      ...
      call_projections[call_idx] = projections;
...
#undef TUPLE_UNARY_CASE
    ...
    case IrOpcode::kCall: {
      ...
      base::Vector<RegisterRepresentation> output_reps =
          graph_zone->NewVector<RegisterRepresentation>(
              call_descriptor->ReturnCount() + is_final_control);
      ...
      if (call_descriptor->NeedsFrameState()) {
        ...
        arguments.push_back(Map(frame_state));
      }

      if (!is_final_control) {
        return EmitProjectionsAndTuple(
            assembler.Call(callee, base::VectorOf(arguments), ts_descriptor));
      } else {
        DCHECK_EQ(block->SuccessorCount(), 2);
        ...
        // successor.
        return assembler.CallAndCatchException(callee, if_success, if_exception,
    ...
    case IrOpcode::kProjection: {
      ...
      OpIndex input_idx = Map(input, true);
      if (call_projections.find(input_idx) != call_projections.end()) {
        // This is a projection to a call that was mapped to a
        // CallAndCatchException; all of its projections should already have
        // been emitted, we thus just reuse them.
        return call_projections[input_idx][index];
      }
      return assembler.Projection(input_idx, index, rep);
    }
