...
Graph GraphBuilder::Run() {
  ...
  for (BasicBlock* block : *schedule.rpo_order()) {
    assembler.Bind(Map(block));
    for (Node* node : *block->nodes()) {
      instr_mapping.Set(node, Process(node, block));
    }
...
InstrIndex GraphBuilder::Process(Node* node, BasicBlock* block) {
  ...
  switch (op->opcode()) {
    ...
    case IrOpcode::kCall: {
      ...
      for (int i = 0; i < static_cast<int>(call_descriptor->InputCount());
           ++i) {
        inputs.emplace_back(Map(node->InputAt(1 + i)));
      }
