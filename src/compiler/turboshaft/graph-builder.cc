...
#undef UNARY_CASE
    ...
    case IrOpcode::kCall: {
      ...
      if (call_descriptor->NeedsFrameState()) {
        FrameState tf_frame_state{
            node->InputAt(static_cast<int>(call_descriptor->InputCount()))};
        arguments.push_back(Map(tf_frame_state));
      }
    ...
    case IrOpcode::kProjection: {
      ...
      OpIndex ts_input = Map(input, true);
      if (call_projections.find(ts_input) != call_projections.end()) {
        ...
        return call_projections[ts_input][index];
      }
      return assembler.Projection(ts_input, index, rep);
    }
