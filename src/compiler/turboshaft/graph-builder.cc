 ...
 private:
  ...
  void FixLoopPhis(Block* loop, Block* backedge) {
    ...
    for (Operation& op : assembler.graph().BlockIterator(*loop)) {
      ...
      LoopPhiOp new_phi(pending_phi.first(), Map(pending_phi.old_backedge_node),
                        backedge->index);
  ...
  for (BasicBlock* block : *schedule.rpo_order()) {
    ...
    switch (block->control()) {
      case BasicBlock::kGoto:
        ...
        Emit(GotoOp(Map(block->SuccessorAt(0))));
        break;
...
OpIndex GraphBuilder::Process(Node* node, BasicBlock* block) {
  ...
  switch (op->opcode()) {
    ...
    case IrOpcode::kParameter: {
      ...
      return Emit(ParameterOp(info.index(), info.debug_name()));
    }

    case IrOpcode::kInt64Constant:
      return Emit(ConstantOp::Word64(OpParameter<int64_t>(op)));
    case IrOpcode::kInt32Constant:
      return Emit(ConstantOp::Word64(OpParameter<int32_t>(op)));
    case IrOpcode::kHeapConstant:
      return Emit(ConstantOp::HeapObject(HeapConstantOf(op)));
    case IrOpcode::kCompressedHeapConstant:
      return Emit(ConstantOp::CompressedHeapObject(HeapConstantOf(op)));
    case IrOpcode::kExternalConstant:
      return Emit(ConstantOp::External(OpParameter<ExternalReference>(op)));

    case IrOpcode::kWord32And:
      return Emit(BitwiseAndOp(Map(node->InputAt(0)), Map(node->InputAt(1)),
    ...
    case IrOpcode::kWord32Equal:
      return Emit(EqualOp(Map(node->InputAt(0)), Map(node->InputAt(1)),
                          MachineRepresentation::kWord32));
    ...
    case IrOpcode::kInt32Add:
      return Emit(AddOp(Map(node->InputAt(0)), Map(node->InputAt(1)),
                        MachineRepresentation::kWord32));
    ...
    case IrOpcode::kInt32Sub:
      return Emit(SubOp(Map(node->InputAt(0)), Map(node->InputAt(1)),
                        MachineRepresentation::kWord32));
    ...
    case IrOpcode::kLoad:
      return Emit(LoadOp::Raw(LoadRepresentationOf(op), Map(node->InputAt(0))));

    case IrOpcode::kStackPointerGreaterThan:
      return Emit(StackPointerGreaterThanOp(StackCheckKindOf(op),
                                            Map(node->InputAt(0))));
    ...
    case IrOpcode::kBranch:
      ...
      return Emit(BranchOp(Map(node->InputAt(0)), Map(block->SuccessorAt(0)),
    ...
    case IrOpcode::kCall: {
      ...
      OpIndex call = Emit(CallOp(call_descriptor, Map(node->InputAt(0)),
      ...
      OpIndex checkpoint =
          Emit(CheckpointOp::Full(base::VectorOf<OpIndex>({}),
                                  frame_state.frame_state_info(), graph_zone));
      Emit(CheckLazyDeoptOp(checkpoint));
      return call;
    }
    ...
    case IrOpcode::kReturn: {
      ...
      return Emit(ReturnOp(return_values.vector(),
                           OpParameter<int32_t>(pop_count->op()), graph_zone));
    }
