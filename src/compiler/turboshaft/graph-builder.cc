...
OpIndex GraphBuilder::Process(
  ...
  switch (opcode) {
    ...
    case IrOpcode::kInt64SubWithOverflow: {
      MachineRepresentation rep = (opcode == IrOpcode::kInt32SubWithOverflow ||
                                   opcode == IrOpcode::kInt32Sub)
                                      ? MachineRepresentation::kWord32
                                      : MachineRepresentation::kWord64;
      SubOp::Kind kind = (opcode == IrOpcode::kInt32SubWithOverflow ||
                          opcode == IrOpcode::kInt64SubWithOverflow)
                             ? SubOp::Kind::kWithOverflowBit
                             : SubOp::Kind::kWithoutOverflowBit;
      return assembler.Sub(Map(node->InputAt(0)), Map(node->InputAt(1)), kind,
                           rep);
    }
    ...
    case IrOpcode::kLoad: {
      ...
      return assembler.IndexedLoad(Map(base), Map(index),
                                   IndexedLoadOp::Kind::kRaw, loaded_rep, 0, 0);
    }
    ...
    case IrOpcode::kUnreachable:
      for (Node* use : node->uses()) {
        CHECK(use->opcode() == IrOpcode::kThrow);
      }
      return OpIndex::Invalid();
