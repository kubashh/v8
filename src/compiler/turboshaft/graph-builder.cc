...
struct GraphBuilder {
 ...
 private:
  ...
  void ProcessStateValues(CheckpointData::Builder* builder,
                          Node* state_values) {
  ...
  void BuildCheckpointData(CheckpointData::Builder* builder,
...
OpIndex GraphBuilder::Process(Node* node, BasicBlock* block) {
  ...
  switch (op->opcode()) {
    ...
    case IrOpcode::kLoad:
      return assembler.Load(Map(node->InputAt(0)), LoadOp::Kind::kRaw,
                            LoadRepresentationOf(op));
    ...
    case IrOpcode::kCall: {
      ...
      CheckpointData::Builder builder;
      BuildCheckpointData(&builder, frame_state);
      OpIndex checkpoint = assembler.FullCheckpoint(
          builder.Inputs(), builder.AllocateCheckpointData(
                                frame_state.frame_state_info(), graph_zone));
      assembler.CheckLazyDeopt(call, checkpoint);
      return call;
    }
