...
struct GraphBuilder {
 ...
 private:
  OpIndex Map(Node* old_node, bool keep_tuples = false) {
    OpIndex result = op_mapping.Get(old_node);
    DCHECK(assembler.output_graph().IsValid(result));
    if (keep_tuples) return result;
    if (TupleOp* tuple =
            assembler.output_graph().Get(result).TryCast<TupleOp>()) {
      if (tuple->input_count == 1) return result;
      // The only tuples present in the graph that we are building come from
      // EmitProjectionsAndTuple. They should only be used as input when they
      // come from a Call that has a single output and was thus used without
      // going through projections. However, if we generated a
      // CallAndCatchException, then its output should be accessed through
      // Projections (0 for the output and 1 for the exception). The exception
      // was accessed through a IfException; so if we are here we are trying to
      // access the output; we thus return the 1st projection instead of the
      // tuple.
...
OpIndex GraphBuilder::Process(
  ...
  switch (opcode) {
    ...
    case IrOpcode::kIfException: {
      ...
      TupleOp& op = assembler.output_graph().Get(call_idx).Cast<TupleOp>();
      return assembler.Projection(call_idx, op.input_count - 1,
...
#undef UNARY_CASE
    ...
    case IrOpcode::kCall: {
      ...
      if (!is_final_control) {
        ...
        return EmitProjectionsAndTuple(assembler.CallAndCatchException(
            callee, if_success, if_exception, base::VectorOf(arguments),
            ts_descriptor));
      }
    }
    ...
    case IrOpcode::kProjection: {
      ...
      return assembler.Projection(Map(input, true), index, rep);
    }
