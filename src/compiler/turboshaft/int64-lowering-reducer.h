...
class Int64LoweringReducer : public Next {
 public:
  ...
  OpIndex REDUCE(Store)(OpIndex base, OpIndex index, OpIndex value,
    ...
    if (kind.is_atomic) {
      const Operation& value_op = __ output_graph().Get(value);
      if (value_op.template Is<TupleOp>() ||
          value_op.outputs_rep().length() == 2) {
        ...
        // The atomic store has a 64-bit value input but store less than 32 bit
        // of that input anyways. just transform it to a smaller atomic store.
        return Next::ReduceStore(base, index, low, kind, stored_rep,
