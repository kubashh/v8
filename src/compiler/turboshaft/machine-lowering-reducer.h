...
class MachineLoweringReducer : public Next {
 public:
      ...
      LOOP(loop) {
        ...
        if (BIND(runtime)) {
          V<Word32> value = __ UntagSmi(
              __ CallRuntime_StringCharCodeAt(isolate_, __ NoContextConstant(),
                                              *receiver, __ TagSmi(*position)));
          GOTO(done, value);
        }
      }
  ...
  OpIndex REDUCE(LoadDataViewElement)(V<Object> object, V<Object> storage,
    ...
    OpIndex value =
        __ Load(storage, index, LoadOp::Kind::RawUnaligned().IsArrayBuffer(),
  ...
  OpIndex REDUCE(StoreDataViewElement)(V<Object> object, V<Object> storage,
    ...
    __ Store(storage, index, Asm().GetVariable(value_to_store),
             StoreOp::Kind::RawUnaligned().IsArrayBuffer(),
             MemoryRepresentation::FromMachineType(machine_type),
             WriteBarrierKind::kNoWriteBarrier);
