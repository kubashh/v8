...
class MachineLoweringReducer : public Next {
 public:
  ...
  OpIndex REDUCE(TransitionAndStoreArrayElement)(
    ...
    switch (kind) {
      case TransitionAndStoreArrayElementOp::Kind::kElement: {
        ...
        IF_NOT (LIKELY(IsElementsKindGreaterThan(elements_kind,
                                                 HOLEY_SMI_ELEMENTS))) {
        ...
        GOTO_IF_NOT(
            LIKELY(IsElementsKindGreaterThan(elements_kind, HOLEY_ELEMENTS)),
        ...
        V<Object> elements = __ template LoadField<Object>(
            array, AccessBuilder::ForJSObjectElements());
        IF (IsElementsKindGreaterThan(store_kind, HOLEY_ELEMENTS)) {
      ...
      case TransitionAndStoreArrayElementOp::Kind::kNonNumberElement: {
        ...
        ELSE_IF(UNLIKELY(
            IsElementsKindGreaterThan(elements_kind, HOLEY_ELEMENTS))) {
          TransitionElementsTo(array, HOLEY_DOUBLE_ELEMENTS, HOLEY_ELEMENTS,
                               fast_map);
        }
        END_IF
    ...
    switch (kind) {
      ...
      case FindOrderedHashEntryOp::Kind::kFindOrderedHashMapEntryForInt32Key: {
        ...
        LOOP(loop, entry) {
          ...
          ELSE_IF(
              __ TaggedEqual(__ LoadMapField(candidate_key),
                             __ HeapConstant(factory_->heap_number_map()))) {
  ...
  V<Word32> IsElementsKindGreaterThan(V<Word32> input, ElementsKind kind) {
    return __ Int32LessThan(kind, input);
  }
