...
class MachineLoweringReducer : public Next {
 public:
  ...
  OpIndex REDUCE(ConvertUntaggedToJSPrimitiveOrDeopt)(
      ...
      ConvertUntaggedToJSPrimitiveOrDeoptOp::InputInterpretation
          input_interpretation,
      const FeedbackSource& feedback) {
    DCHECK_EQ(kind, ConvertUntaggedToJSPrimitiveOrDeoptOp::Kind::kSmi);
    if (input_rep == RegisterRepresentation::Word32()) {
      if (input_interpretation ==
  ...
  OpIndex REDUCE(ConvertJSPrimitiveToUntaggedOrDeopt)(
    ...
    switch (to_kind) {
      case ConvertJSPrimitiveToUntaggedOrDeoptOp::PrimitiveKind::kInt32: {
        if (from_kind ==
            ConvertJSPrimitiveToUntaggedOrDeoptOp::ObjectKind::kSmi) {
          __ DeoptimizeIfNot(__ ObjectIsSmi(object), frame_state,
                             DeoptimizeReason::kNotASmi, feedback);
          return __ UntagSmi(object);
        } else {
          DCHECK_EQ(from_kind,
                    ConvertJSPrimitiveToUntaggedOrDeoptOp::ObjectKind::kNumber);
          Label<Word32> done(this);
      ...
      case ConvertJSPrimitiveToUntaggedOrDeoptOp::PrimitiveKind::kArrayIndex: {
        DCHECK_EQ(
            from_kind,
            ConvertJSPrimitiveToUntaggedOrDeoptOp::ObjectKind::kNumberOrString);
  ...
  OpIndex REDUCE(TruncateJSPrimitiveToUntaggedOrDeopt)(
      ...
      TruncateJSPrimitiveToUntaggedOrDeoptOp::InputRequirement
          input_requirement,
      const FeedbackSource& feedback) {
    DCHECK_EQ(kind, TruncateJSPrimitiveToUntaggedOrDeoptOp::Kind::kInt32);
    Label<Word32> done(this);
 ...
 private:
  ...
  V<Float64> ConvertHeapObjectToFloat64OrDeopt(
      ...
      ConvertJSPrimitiveToUntaggedOrDeoptOp::ObjectKind input_kind,
      const FeedbackSource& feedback) {
    ...
    switch (input_kind) {
      ...
      case ConvertJSPrimitiveToUntaggedOrDeoptOp::ObjectKind::
          kNumberOrBoolean: {
        IF_NOT(check_number) {
          STATIC_ASSERT_FIELD_OFFSETS_EQUAL(HeapNumber::kValueOffset,
                                            Oddball::kToNumberRawOffset);
