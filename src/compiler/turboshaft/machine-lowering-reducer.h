  ...
  OpIndex ReduceCheck(OpIndex input, OpIndex frame_state, CheckOp::Kind kind,
    ...
    switch (kind) {
      case CheckOp::Kind::kCheckBigInt: {
        ...
        OpIndex bi_check = Asm().TaggedEqual(
            value_map, Asm().HeapConstant(factory_->bigint_map()));
        Asm().DeoptimizeIfNot(bi_check, frame_state,
    ...
    switch (kind) {
      case ConvertToObjectOp::Kind::kInt64ToBigInt64: {
        ...
        if (Asm().Bind(zero_block)) {
          zero_result =
              BuildAllocateBigInt(OpIndex::Invalid(), OpIndex::Invalid());
          Asm().Goto(merge_block);
        }

        OpIndex non_zero_result;
        if (Asm().Bind(non_zero_block)) {
          ...
          non_zero_result = BuildAllocateBigInt(bitfield, absolute_value);
          Asm().Goto(merge_block);
        }

        Asm().BindReachable(merge_block);
        if (zero_result.valid() && non_zero_result.valid()) {
          return Asm().Phi(base::VectorOf({zero_result, non_zero_result}),
                           RegisterRepresentation::Tagged());
        } else if (zero_result.valid()) {
          return zero_result;
        } else {
          DCHECK(non_zero_result.valid());
