...
class MemoryOptimizationReducer : public Next {
 public:
  ...
  void Analyze() {
    analyzer_.emplace(Asm().phase_zone(), Asm().input_graph(), this);
    analyzer_->Run();
    Next::Analyze();
  }
 ...
 private:
  base::Optional<MemoryAnalyzer<Next>> analyzer_;
  Isolate* isolate_;
...
inline void MemoryAnalyzer<Assembler>::Process(const Operation& op) {
  if (assembler->SkipOperation(op, input_graph.Index(op))) {
    return;
  }
    ...
    if (input_graph.IsLoopBackedge(*goto_op)) {
      ...
      for (const Operation& op :
        ...
        if (op.Properties().can_allocate &&
            !assembler->SkipOperation(op, input_graph.Index(op))) {
          state = BlockState();
          break;
        }
      }
