...
class BranchConditionDuplicationReducer : public Next {
 public:
  ...
  OpIndex REDUCE_INPUT_GRAPH(Branch)(OpIndex ig_index, const BranchOp& branch) {
    ...
    if (!ShouldOptimizeCond(cond, branch.condition(), new_cond)) goto no_change;
  ...
  OpIndex REDUCE_INPUT_GRAPH(Select)(OpIndex ig_index, const SelectOp& select) {
    ...
    OpIndex new_cond = OpIndex::Invalid();
    if (!ShouldOptimizeCond(cond, select.cond(), new_cond)) goto no_change;

    __ Select(new_cond, __ MapToNewGraph(select.vtrue()),
 ...
 private:
  bool ShouldOptimizeCond(const Operation& cond, OpIndex input_idx,
                          OpIndex& new_cond) {
