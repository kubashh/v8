...
base::Optional<BailoutReason> BuildGraphPhase::Run(DataComponentProvider* data_provider,
      Zone* temp_zone, Schedule* schedule, Linkage* linkage) {
  DCHECK_NOT_NULL(schedule);
  if (auto bailout = turboshaft::BuildGraph(data_provider, schedule, temp_zone, linkage)) {
    return bailout;
  }
  return {};
}
...
base::Optional<BailoutReason> BuildGraphPhase::Run(
      Zone* temp_zone, Linkage* linkage) {
