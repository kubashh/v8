...
// `ContextualData` is persistent throughout the entire pipeline, but it's owned and provided by the surrounding context.
struct ContextualData {
  DEFINE_DATA_COMPONENT(ContextualData)

  Isolate* isolate;

  explicit ContextualData(Isolate* isolate)
    : isolate(isolate) {}
};

// `CompilationData` persists throughout the entire pipeline and is owned by the pipeline.
struct CompilationData {
  ...
  CompilationData(CodeKind code_kind, const char* debug_name, Zone* graph_zone, 
    TurboshaftPipelineKind pipeline_kind, AccountingAllocator* allocator)
    : info(base::CStrVector(debug_name), graph_zone, code_kind), pipeline_kind(pipeline_kind), zone_stats(allocator)
  {}
};
...
struct GraphData {
  ...
  explicit GraphData(ZoneStats* zone_stats)
    : graph_zone_scope(zone_stats, kGraphZoneName, kCompressGraphZone), graph_zone(graph_zone_scope.zone()),
...
struct StatisticsData {
  ...
  StatisticsData(OptimizedCompilationInfo* info, std::shared_ptr<CompilationStatistics> compilation_statistics, ZoneStats* zone_stats,
    NodeOriginTable* node_origins)
    : pipeline_statistics(info, compilation_statistics, zone_stats), node_origins(node_origins) {}
};

class DataComponentProvider {
 public:
  explicit DataComponentProvider(Zone* zone)
    : zone_(zone) {}
...
class PipelineBase : public DataComponentProvider {
 public:
  ...
  StatisticsData* CreateStatistics(NodeOriginTable* node_origins = nullptr) {
    ...
    return &InitializeDataComponent<StatisticsData>(&compilation_data.info, contextual_data.isolate->GetTurboStatistics(), &compilation_data.zone_stats,
      node_origins);
  }

  const ContextualData* contextual_data() const { return &GetDataComponent<ContextualData>(); }
  CompilationData* compilation_data() { return &GetDataComponent<CompilationData>(); }
  RuntimeCallStats* runtime_call_stats() const { return runtime_call_stats_; }
  void set_runtime_call_stats(RuntimeCallStats* stats) {
    runtime_call_stats_ = stats;
  }

  base::Optional<BailoutReason> InitializeFromTurbofanGraph(Schedule* turbofan_graph, CallDescriptor* call_descriptor);

 protected:
  PipelineBase(ContextualData contextual_data, CodeKind code_kind, const char* debug_name, Zone* graph_zone, TurboshaftPipelineKind pipeline_kind)
  // TODO(nicohartmann@): This needs a permanent zone.
    : DataComponentProvider(graph_zone) {
      ...
      auto& compilation_data = InitializeDataComponent<CompilationData>(code_kind,
          debug_name, graph_zone, pipeline_kind, isolate->allocator());
  ...
  auto Run(Args&&... args) {
    ...
    PipelineRunScope scope(pipeline_statistics, &compilation_data.zone_stats, node_origins, runtime_call_stats_, Phase::phase_name(),
    ...
    Phase phase;
    using result_t = decltype(phase.Run(this, scope.zone(), std::forward<Args>(args)...));
...
class BuiltinPipeline : public PipelineBase {
 public:
  BuiltinPipeline(ContextualData contextual_data, CodeKind code_kind, const char* debug_name, Zone* graph_zone, Builtin builtin)
    : PipelineBase(std::move(contextual_data), code_kind, debug_name, graph_zone, TurboshaftPipelineKind::kCSA) {
