...
class GraphVisitor {
 public:
  ...
  void VisitGraph() {
    ...
    for (const Block& input_block : input_graph_.blocks()) {
    ...
    if (!input_graph_.source_positions().empty()) {
      for (OpIndex index : output_graph_.AllOperationIndices()) {
        ...
        output_graph_.source_positions()[index] =
            input_graph_.source_positions()[origin];
      }
    }
 ...
 private:
  ...
  void VisitAllBlocks() {
    ...
    visit_stack.push_back(input_graph_.GetPtr(0));
  ...
  void VisitBlock(const Block* input_block) {
    ...
    if (!assembler().Bind(MapToNewGraph(input_block->index()))) {
      ...
      const Operation& last_op =
          *base::Reversed(input_graph_.operations(*input_block)).begin();
    ...
    for (OpIndex index : input_graph_.OperationIndices(*input_block)) {
  ...
  bool VisitOp(OpIndex index, const Block* input_block) {
    ...
    const Operation& op = input_graph_.Get(index);
    OpIndex new_index;
...
#undef EMIT_INSTR_CASE
  ...
  void TraceReductionStart(OpIndex index) {
    std::cout << "╭── o" << index.id() << ": "
              ...
              << OperationPrintStyle{input_graph_.Get(index), "#o"} << "\n";
  }
