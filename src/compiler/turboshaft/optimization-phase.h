...
class GraphVisitor {
 ...
 public:
  ...
  void CloneAndInlineBlock(const Block* input_block) {
    ...
    //     p2 = phi(b, p1)
    //
    // In this case, when inlining the block from the backedge, if we visit Phis
    // and emit new mapping (with CreateOldToNewMapping) as we go along, we
    // would visit p1 and emit a mapping saying "p1 = p2", and use this mapping
    // when visiting p2, then we'd map p2 to p2 instead of p1. To overcome this
    // issue, we first visit the Phis of the loop, emit the new phis, and record
    // the new mapping in a side-table ({new_phi_values}). Then, we visit all of
    // the operations of the loop and commit the new mappings: phis were emitted
    // before using the old mapping, and all of the other operations will use
    // the new mapping (as they should).
    ...
    for (OpIndex index : input_graph().OperationIndices(*input_block)) {
      if (const PhiOp* phi =
        ...
        // predecessor, and shouldn't be a Phi anymore.
         new_phi_values.push_back(MapToNewGraph(
             phi->input(added_block_phi_input), added_block_phi_input));
      } else if (!input_graph().Get(index).template Is<FrameStateOp>()) {
        // No more Phis can appear past this point.
        break;
      }
    }
  ...
  Block* MapToNewGraph(const Block* block) const {
    ...
    // TODO: have some kind of variables for Blocks, I think.
    UNREACHABLE();
  }
  ...
  const Block* OriginForBlockStart(Block* block) const {
    ...
    Block* thingy = MapToNewGraph(origin);
    if (origin && thingy == block) return origin;
    return nullptr;
  }

  // Clone all of the blocks in {sub_graph}.
  void CloneSubGraph(ZoneVector<Block*> sub_graph) {
    std::cout << "CloneSubGraph\n";
    DCHECK(std::is_sorted(
        sub_graph.begin(), sub_graph.end(),
        [](Block* a, Block* b) { return a->index().id() <= b->index().id(); }));

    // 1. Create new blocks, and update old->new mapping.
    ZoneVector<Block*> old_mappings(sub_graph.size(), phase_zone_);
    for (size_t i = 0; i < sub_graph.size(); i++) {
      Block* input_block = sub_graph[i];
      old_mappings[i] = block_mapping_[input_block->index()];
      // Note that we set the kind to kMerge, even for the loop header, since it
      // shouldn't be a loop header anymore, as we are unrolling the loop.
      block_mapping_[input_block->index()] =
          output_graph().NewBlock(Block::Kind::kMerge, input_block);
    }

    // 2. Visit block in correct order (begin to end)

    // Emit a goto to 1st block.
    std::cout << "Inserting jump-in Goto\n";
    assembler().Goto(block_mapping_[sub_graph[0]->index()]);

    for (Block* block : sub_graph) {
      std::cout << "Visiting loop block " << block->index() << "\n";
      blocks_needing_variables_.insert(block->index());
      VisitBlock<false>(block);
    }

    // ZoneVector<Block*> visit_stack(phase_zone());
    // visit_stack.push_back(*sub_graph.begin());
    ...
    //     if (std::find(sub_graph.begin(), sub_graph.end(), child)) {
    //       visit_stack.push_back(child);
    //     }
    //   }
    // }

    // 3. Restore old mapping
    for (size_t i = 0; i < sub_graph.size(); i++) {
      Block* input_block = sub_graph[i];
      block_mapping_[input_block->index()] = old_mappings[i];
    }
  }
...
#define EMIT_INSTR_CASE(Name)                                           \
  case Opcode::k##Name:                                                 \
    if (MayThrow(Opcode::k##Name)) return true;                         \
    new_index =                                                         \
