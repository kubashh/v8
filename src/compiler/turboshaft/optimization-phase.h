...
struct GraphVisitorState {
 public:
  GraphVisitorState(Graph& input_graph, Graph& output_graph, Zone* phase_zone,
                    compiler::NodeOriginTable* origins)
      : input_graph(input_graph),
        output_graph(output_graph),
        phase_zone(phase_zone),
        origins(origins),
        current_input_block(nullptr),
        block_mapping(input_graph.block_count(), nullptr, phase_zone),
        op_mapping(input_graph.op_id_count(), OpIndex::Invalid(), phase_zone) {}

  OpIndex TryMapToNewGraph(OpIndex old_index) const {
    return op_mapping[old_index.id()];
  }
...
class GraphVisitor {
 public:
  GraphVisitor(Graph& input_graph, Graph& output_graph, Zone* phase_zone,
      ...
      : state_(input_graph, output_graph, phase_zone, origins),
        analyzer(input_graph, phase_zone) {}
  ...
  void VisitGraph() {
    ...
    for (const Block& input_block : state_.input_graph.blocks()) {
      state_.block_mapping[input_block.index().id()] =
    ...
    if (!state_.input_graph.source_positions().empty()) {
      for (OpIndex index : state_.output_graph.AllOperationIndices()) {
        OpIndex origin = state_.output_graph.operation_origins()[index];
        state_.output_graph.source_positions()[index] =
            state_.input_graph.source_positions()[origin];
      }
    }
    // Updating the operation indices.
    if (state_.origins) {
      for (OpIndex index : assembler().output_graph().AllOperationIndices()) {
        ...
        state_.origins->SetNodeOrigin(index.id(), origin.id());
      }
    }

    state_.input_graph.SwapWithCompanion();
  }

 private:
  template <bool trace_reduction>
  void VisitAllBlocks() {
    base::SmallVector<Block*, 128> visit_stack;

    visit_stack.push_back(state_.input_graph.GetPtr(0));
    while (!visit_stack.empty()) {
      Block* block = visit_stack.back();
  ...
  void VisitBlock(const Block* input_block) {
    state_.current_input_block = input_block;
    if constexpr (trace_reduction) {
      std::cout << PrintAsBlockHeader{*input_block} << "\n";
    }
    if (!assembler().Bind(state_.MapToNewGraph(input_block->index()))) {
      ...
      const Operation& last_op =
          *base::Reversed(state_.input_graph.operations(*input_block)).begin();
      if (auto* final_goto = last_op.TryCast<GotoOp>()) {
        if (final_goto->destination->IsLoop()) {
          Block* new_loop =
              state_.MapToNewGraph(final_goto->destination->index());
          DCHECK(new_loop->IsLoop());
          if (new_loop->IsLoop() && new_loop->PredecessorCount() == 1) {
            state_.output_graph.TurnLoopIntoMerge(new_loop);
          }
        }
      }
      return;
    ...
    for (OpIndex index : state_.input_graph.OperationIndices(*input_block)) {
  ...
  bool VisitOp(OpIndex index, const Block* input_block) {
    ...
    const Operation& op = state_.input_graph.Get(index);
    OpIndex new_index;
...
#undef EMIT_INSTR_CASE
    ...
    state_.op_mapping[index.id()] = new_index;
    return true;
  }
  ...
  void TraceReductionStart(OpIndex index) {
    std::cout << "╭── o" << index.id() << ": "
              ...
              << OperationPrintStyle{state_.input_graph.Get(index), "#o"}
              << "\n";
  }
  ...
  void TraceReductionResult(OpIndex first_output_index, OpIndex new_index) {
    ...
    for (const Operation& op : state_.output_graph.operations(
             first_output_index, state_.output_graph.next_operation_index())) {
      OpIndex index = state_.output_graph.Index(op);
      const char* prefix;
      if (index == new_index) {
        prefix = "╰─>";
        before_arrow = false;
      ...
      std::cout << prefix << " n" << index.id() << ": "
                << PaddingSpace{5 - CountDecimalDigits(index.id())}
                << OperationPrintStyle{state_.output_graph.Get(index), "#n"}
                << "\n";
    }
    std::cout << "\n";
  }
  ...
  V8_INLINE OpIndex VisitGoto(const GotoOp& op) {
    Block* destination = state_.MapToNewGraph(op.destination->index());
    assembler().current_block()->SetOrigin(state_.current_input_block);
  ...
  V8_INLINE OpIndex VisitBranch(const BranchOp& op) {
    Block* if_true = state_.MapToNewGraph(op.if_true->index());
    Block* if_false = state_.MapToNewGraph(op.if_false->index());
    return assembler().ReduceBranch(MapToNewGraph(op.condition()), if_true,
                                    if_false);
  }
  OpIndex VisitCatchException(const CatchExceptionOp& op) {
    Block* if_success = state_.MapToNewGraph(op.if_success->index());
    Block* if_exception = state_.MapToNewGraph(op.if_exception->index());
  ...
  OpIndex VisitPhi(const PhiOp& op) {
    ...
    if (new_pred != nullptr) {
      ...
      for (old_pred = state_.current_input_block->LastPredecessor();
  ...
  OpIndex MapToNewGraph(OpIndex old_index) {
    OpIndex result = state_.TryMapToNewGraph(old_index);
    DCHECK(result.valid());
    return result;
  }
