...
class GraphVisitor {
 ...
 public:
  ...
  void VisitGraph() {
    ...
    for (Block& input_block : modifiable_input_graph().blocks()) {
      block_mapping_[input_block.index()] = output_graph().NewBlock(
          input_block.IsLoop() ? Block::Kind::kLoopHeader
                               : Block::Kind::kMerge);
    }
  ...
  Block* MapToNewGraph(const Block* block) {
  ...
  const Block* OriginForBlockStart(Block* block) const {
    ...
    if (origin && origin == block) return origin;
    return nullptr;
  }
  ...
  void CloneSubGraph(ZoneVector<Block*> sub_graph) {
    ...
    for (size_t i = 0; i < sub_graph.size(); i++) {
      ...
      block_mapping_[input_block->index()] =
          output_graph().NewBlock(Block::Kind::kMerge);
    }
    ...
    assembler().ReduceGoto(block_mapping_[sub_graph[0]->index()]);
...
#undef EMIT_INSTR_CASE
  ...
  OpIndex AssembleOutputGraphDidntThrow(const DidntThrowOp& op) {
    ...
    std::cout << "Going to re-emit call: " << throwing_operation << "\n";
    OpIndex result;
