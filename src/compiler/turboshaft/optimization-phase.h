...
class GraphVisitor {
 ...
 public:
  ...
  Block* CloneSubGraph(Set sub_graph, bool keep_loop_kinds) {
    ...
    int block_num = 0;
    for (Block* input_block : sub_graph) {
      old_mappings[block_num++] = block_mapping_[input_block->index()];
    ...
    block_num = 0;
    for (Block* input_block : sub_graph) {
      block_mapping_[input_block->index()] = old_mappings[block_num++];
    }
    // TODO: consider using this instead:
    // for (auto& [input_block, old_mapping] : zip(sub_graph, old_mappings)) {
