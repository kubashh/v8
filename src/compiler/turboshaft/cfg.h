  ...
  class ReplaceScope {
   public:
    ...
    ~ReplaceScope() {
      ...
      buffer_->operation_sizes_
          [OpIndex(replaced_.offset() + old_slot_count_).id() - 1] =
          old_slot_count_;
    }
...
class Graph {
 public:
  ...
  bool Add(Block* block) {
    ...
    for (auto it = bound_blocks_.rbegin(); it != bound_blocks_.rend(); ++it) {
  ...
  base::iterator_range<Block* const*> blocks() {
    return {bound_blocks_.data(), bound_blocks_.data() + bound_blocks_.size()};
  }
...
class Assembler {
 public:
  ...
  bool Bind(Block* block) {
    ...
    DCHECK(block->kind <= Block::Kind::kBranch);
...
class BasicAssembler : public Assembler {
 public:
  ...
  template <class Op, class... Args>
  OpIndex Emit(Args... args) {
    return Base::Emit<Op>(args...);
  }

  template <>
  OpIndex Emit<PhiOp>(base::Vector<const OpIndex> inputs) {
    DCHECK(current_block()->IsMerge() &&
           inputs.size() == current_block()->predecessors.size());
    return Base::Emit<PhiOp>(inputs);
  }

  template <>
  OpIndex Emit<PendingLoopPhiOp>(OpIndex first, Node* old_backedge_node) {
    DCHECK(current_block()->IsLoop());
    return Base::Emit<PendingLoopPhiOp>(first, old_backedge_node);
  }
  template <>
  OpIndex Emit<PendingLoopPhiOp>(OpIndex first, OpIndex old_backedge_index) {
    DCHECK(current_block()->IsLoop());
    return Base::Emit<PendingLoopPhiOp>(first, old_backedge_index);
  }

  template <>
  OpIndex Emit<GotoOp>(Block* destination) {
    ...
    return Base::Emit<GotoOp>(destination);
  }

  template <>
  OpIndex Emit<BranchOp>(OpIndex condition, Block* if_true, Block* if_false) {
    ...
    return Base::Emit<BranchOp>(condition, if_true, if_false);
  }
};
    ...
    if (block->IsLoop()) {
      ...
      for (Variable* var : variables_.MinRange(dominating_pos.offset())) {
        ...
        if (needs_phi && is_valid) {
          Write(var, Emit<PhiOp>(base::VectorOf(inputs)));
        }
      }
    }
    return true;
