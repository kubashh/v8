...
class WasmLoweringReducer : public Next {
 ...
 private:
  ...
  V<Word32> IsSmi(V<Tagged> object) {
    // TODO(14108): This is a copy of the same code in the
    // turboshaft-graph-interface.
    if constexpr (COMPRESS_POINTERS_BOOL) {
      return __ Word32Equal(
          __ Word32BitwiseAnd(V<Word32>::Cast(object), kSmiTagMask), kSmiTag);
    } else {
      return __ WordPtrEqual(
          __ WordPtrBitwiseAnd(V<WordPtr>::Cast(object), kSmiTagMask), kSmiTag);
    }
  }
  ...
  OpIndex ReduceWasmTypeCheckAbstract(V<Tagged> object,
    ...
    do {
      ...
      if (to_rep == wasm::HeapType::kI31) {
        ...
        result = object_can_be_i31 ? IsSmi(object) : __ Word32Constant(0);
        break;
      }
      if (to_rep == wasm::HeapType::kEq) {
        if (object_can_be_i31) {
          GOTO_IF(UNLIKELY(IsSmi(object)), end_label, __ Word32Constant(1));
        }
      ...
      if (object_can_be_i31) {
        GOTO_IF(UNLIKELY(IsSmi(object)), end_label, __ Word32Constant(0));
      }
  ...
  V<WordPtr> ChangeSmiToWordPtr(V<Tagged> smi) {
    return __ ChangeInt32ToIntPtr(__ ConvertJSPrimitiveToUntagged(
        smi, ConvertJSPrimitiveToUntaggedOp::UntaggedKind::kInt32,
