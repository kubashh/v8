...
void CsaEarlyMachineOptimizationPhase::Run(DataComponentProvider* data_provider, Zone* temp_zone) {
  CopyingPhase<MachineOptimizationReducer, ValueNumberingReducer>::Run(
      temp_zone);
}

void CsaLoadEliminationPhase::Run(DataComponentProvider* data_provider, Zone* temp_zone) {
  CopyingPhase<LateLoadEliminationReducer, MachineOptimizationReducer,
               ValueNumberingReducer>::Run(temp_zone);
}

void CsaLateEscapeAnalysisPhase::Run(DataComponentProvider* data_provider, Zone* temp_zone) {
  CopyingPhase<LateEscapeAnalysisReducer, MachineOptimizationReducer,
               ValueNumberingReducer>::Run(temp_zone);
}

void CsaBranchEliminationPhase::Run(DataComponentProvider* data_provider, Zone* temp_zone) {
  CopyingPhase<MachineOptimizationReducer, BranchEliminationReducer,
               ValueNumberingReducer>::Run(temp_zone);
}

void CsaOptimizePhase::Run(DataComponentProvider* data_provider, Zone* temp_zone) {
