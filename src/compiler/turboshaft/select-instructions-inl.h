...
class Sidetable {
 public:
  Value& operator[](Key key) {
    size_t index = ToUnderlyingType(key);
...
struct InstructionSelector {
  ...
  Sidetable<OpIndex, uint32_t> graph_use_count{temp_zone, graph.op_count()};
  Sidetable<OpIndex, uint32_t> used_by_instruction{temp_zone, graph.op_count()};
  Sidetable<OpIndex, uint32_t> operation_has_been_processed{temp_zone,
                                                            graph.op_count()};
  Sidetable<OpIndex, int> effect_level{temp_zone, graph.op_count()};
...
bool InstructionSelector::Run() {
  instruction_buffer.reserve(graph.op_count());
  ...
  for (auto block : graph.blocks()) {
    ...
    for (const Operation& instr : graph.BlockIterator(*block)) {
      if (auto* phi = instr.TryCast<LoopPhiOp>()) {
...
void InstructionSelector::Visit(const Op& op) {
  ...
  FATAL("Unexpected operation #%d: %s", Index(op), op.ToString().c_str());
}
...
void InstructionSelector::Visit(const CallOp& op) {
  ...
  Instruction* call_instr = Emit(opcode, outputs.vector(), inputs.vector());
  if (instruction_selection_failed) return;
  call_instr->MarkAsCall();

  EmitPrepareResults(results.vector(), call_descriptor);
...
Instruction* InstructionSelector::EmitWithContinuation(
  ...
  return Emit(opcode, new_outputs.vector(), new_inputs.vector(), temps);
}
