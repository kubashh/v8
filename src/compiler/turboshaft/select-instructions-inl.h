...
struct InstructionSelector {
  ...
  ZoneVector<Instruction*> instruction_buffer{phase_zone};

  Sidetable<OpIndex, uint32_t> graph_use_count{phase_zone, graph.op_id_count()};
  Sidetable<OpIndex, uint32_t> used_by_instruction{phase_zone,
                                                   graph.op_id_count()};
  Sidetable<OpIndex, uint32_t> operation_has_been_processed{
      phase_zone, graph.op_id_count()};
  Sidetable<OpIndex, int> effect_level{phase_zone, graph.op_id_count()};
  ...
  InstructionSelector(Zone* phase_zone, const Graph& graph,
...
bool InstructionSelector::Run() {
  ...
  if (enable_instruction_scheduling) {
    scheduler = phase_zone->New<InstructionScheduler>(phase_zone, sequence);
  }
...
void InstructionSelector::Visit(const CallOp& op) {
  ...
  // CallBuffer buffer(phase_zone, call_descriptor, frame_state_descriptor);
