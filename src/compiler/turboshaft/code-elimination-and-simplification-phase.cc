...
void CodeEliminationAndSimplificationPhase::Run(
    DataComponentProvider* data_provider, Zone* temp_zone) {
      JSHeapBroker* broker = data_provider ? data_provider->GetDataComponent<CompilationData>().broker.get() 
        : PipelineData::Get().broker();
  UnparkedScopeIfNeeded scope(broker, DEBUG_BOOL);
