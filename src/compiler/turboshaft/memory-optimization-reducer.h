...
class MemoryOptimizationReducer : public Next {
 public:
  ...
  OpIndex REDUCE(Allocate)(OpIndex size, AllocationType type) {
    ...
    // than kMaxRegularHeapObjectSize, use different implementation which does
    // not update top in case of a runtime call.
    // TODO(mliedtke): Why do we do that in the implementation below? Why can't
    // we merge these two together?
    ...
    if (!__ MatchWordConstant(size, WordRepresentation::PointerSized(),
      ...
      if (!constant_size) {
        ...
        Asm().GotoIfNot(Asm().UintPtrLessThan(new_top, limit), call_runtime);
        Asm().GotoIfNot(Asm().UintPtrLessThan(
