...
class OptimizationPhase {
 public:
  static void Run(Graph* input, Zone* phase_zone) {
    OptimizationPhase phase(input, phase_zone);
 ...
 private:
  ...
  void Run() {
    ...
    for (const Block& block : input.blocks()) {
      if (trace_reduction) {
        std::cout << AsBlockHeader{block} << "\n";
      }
      if (!analyzer.BlockReachable(block.index) ||
      ...
      for (const Operation& op : input.operations(block)) {
        ...
        OpIndex next_output_index = assembler.graph().next_operation_index();
        if (trace_reduction) {
          std::cout << "╭── o" << index.id() << ": "
                    << Padding{5 - CountDecimalDigits(index.id())}
                    << OperationPrintStyle{op, "#o"} << "\n";
        }
        ...
        if (replacement != index) {
          OpIndex new_index = MapToNewGraph(replacement);
          if (trace_reduction) {
            std::cout << "╰─> #o" << replacement.id() << " / #n"
                      << new_index.id() << "\n";
          }
        ...
        if (block.IsLoop() && op.Is<PhiOp>()) {
          ...
          if (trace_reduction) {
            std::cout << "╰─> n" << new_index.id() << ": "
                      << Padding{5 - CountDecimalDigits(new_index.id())}
                      << OperationPrintStyle{assembler.graph().Get(new_index),
                                             "#n"}
                      << "\n";
          }
        } else {
...
#undef EMIT_INSTR_CASE
          ...
          if (trace_reduction) {
            if (new_index < next_output_index) {
              std::cout << "╰─> #n" << new_index.id() << "\n";
            }
            bool before_arrow = new_index >= next_output_index;
            for (const Operation& op : assembler.graph().operations(
                     next_output_index,
                     assembler.graph().next_operation_index())) {
              ...
              std::cout << prefix << " n" << index.id() << ": "
                        << Padding{5 - CountDecimalDigits(index.id())}
                        << OperationPrintStyle{assembler.graph().Get(index),
                                               "#n"}
                        << "\n";
            }
            std::cout << "\n";
  ...
  OpIndex Reduce(const OverflowCheckedBinaryOp& op) {
    return assembler.OverflowCheckedBinary(
