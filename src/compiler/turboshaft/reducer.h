...
#undef EMIT_INSTR_CASE
 ...
 private:
  ...
  OpIndex Reduce(const MulOp& op) {
    return assembler.Mul(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                         op.kind, op.rep);
  }
  ...
  OpIndex Reduce(const BitwiseOrOp& op) {
    return assembler.BitwiseOr(MapToNewGraph(op.left()),
                               MapToNewGraph(op.right()), op.rep);
  }
  OpIndex Reduce(const BitwiseXorOp& op) {
    return assembler.BitwiseOr(MapToNewGraph(op.left()),
                               MapToNewGraph(op.right()), op.rep);
  }
