...
struct LivenessAnalyzer : AnalyzerBase {
  ...
  ZoneVector<uint8_t> block_reachable;
  ZoneVector<uint8_t> op_used;
...
struct OptimizationPhase {
  ...
  explicit OptimizationPhase(Graph input)
      : input(std::move(input)),
        ...
        assembler(zone),
        block_mapping(this->input.block_count(), nullptr, zone),
        op_mapping(this->input.op_count(), OpIndex::kInvalid, zone) {}
  ...
  Graph Run() {
    ...
    assembler.graph().ReserveMemory(input);
    ...
    for (const Block* block : input.blocks()) {
      ...
      for (const Operation& op : input.BlockIterator(*block)) {
        ...
        OpIndex new_index;
        if (auto* phi = op.TryCast<LoopPhiOp>()) {
