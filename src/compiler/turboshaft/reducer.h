...
#undef EMIT_INSTR_CASE
 ...
 private:
  ...
  OpIndex Reduce(const AddOp& op) {
    return assembler.Add(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                         op.rep);
  }
  OpIndex Reduce(const SubOp& op) {
    return assembler.Sub(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                         op.rep);
  }
  OpIndex Reduce(const BitwiseAndOp& op) {
    return assembler.BitwiseAnd(MapToNewGraph(op.left()),
  ...
  OpIndex Reduce(const LoadOp& op) {
    return assembler.Load(op.kind, op.loaded_rep, MapToNewGraph(op.base()),
                          op.offset);
  }
  ...
  OpIndex Reduce(const StackPointerGreaterThanOp& op) {
    return assembler.StackPointerGreaterThan(op.kind,
  ...
  OpIndex Reduce(const CheckLazyDeoptOp& op) {
    return assembler.CheckLazyDeopt(MapToNewGraph(op.checkpoint()));
  }
  ...
  OpIndex Reduce(const CheckpointOp& op) {
    ...
    for (OpIndex& input : inputs) input = MapToNewGraph(input);
    return assembler.Checkpoint(base::VectorOf(inputs));
  }
  OpIndex Reduce(const CallOp& op) {
    ...
    return assembler.Call(op.descriptor, callee, base::VectorOf(arguments));
  }
