...
struct OptimizationPhase {
  ...
  explicit OptimizationPhase(Graph* input)
      : input(*input),
        ...
        assembler(&input->GetOrCreateCompanion()),
    ...
    for (const Block* block : input.blocks()) {
      ...
      for (const Operation& op : input.BlockIterator(*block)) {
        ...
        if (block->IsLoop() && op.Is<PhiOp>()) {
          const PhiOp& phi = op.Cast<PhiOp>();
          new_index = assembler.template Emit<PendingLoopPhiOp>(
              MapToNewGraph(phi.inputs()[0]), phi.inputs()[1]);
        } else {
          switch (op.opcode) {
...
#undef EMIT_INSTR_CASE
 ...
 private:
  ...
  OpIndex Emit(Args... args) {
    return assembler.template Emit<Op>(args...);
  }
  ...
  OpIndex Reduce(const GotoOp& op) {
    ...
    return Emit<GotoOp>(destination);
  }
  OpIndex Reduce(const BranchOp& op) {
    ...
    return Emit<BranchOp>(MapToNewGraph(op.condition()), if_true, if_false);
  }

  OpIndex Reduce(const AddOp& op) {
    return Emit<AddOp>(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                       op.rep);
  }
  OpIndex Reduce(const SubOp& op) {
    return Emit<SubOp>(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                       op.rep);
  }
  OpIndex Reduce(const BitwiseAndOp& op) {
    return Emit<BitwiseAndOp>(MapToNewGraph(op.left()),
                              MapToNewGraph(op.right()), op.rep);
  }
  OpIndex Reduce(const EqualOp& op) {
    return Emit<EqualOp>(MapToNewGraph(op.left()), MapToNewGraph(op.right()),
                         op.rep);
  }
  ...
  OpIndex Reduce(const ConstantOp& op) {
    return Emit<ConstantOp>(op.kind, op.storage);
  }
  OpIndex Reduce(const LoadOp& op) {
    return Emit<LoadOp>(op.kind, op.loaded_rep, MapToNewGraph(op.base()),
                        op.offset);
  }
  OpIndex Reduce(const ParameterOp& op) {
    return Emit<ParameterOp>(op.parameter_index, op.debug_name);
  }
  OpIndex Reduce(const StackPointerGreaterThanOp& op) {
    return Emit<StackPointerGreaterThanOp>(op.kind,
                                           MapToNewGraph(op.stack_limit()));
  }
  OpIndex Reduce(const LoadStackCheckOffsetOp& op) {
    return Emit<LoadStackCheckOffsetOp>();
  }
  OpIndex Reduce(const CheckLazyDeoptOp& op) {
    return Emit<CheckLazyDeoptOp>(MapToNewGraph(op.checkpoint()));
  }
  OpIndex Reduce(const PhiOp& op) {
    ...
    return Emit<PhiOp>(base::VectorOf(inputs));
  }
  OpIndex Reduce(const CheckpointOp& op) {
    base::SmallVector<OpIndex, 16> inputs = op.inputs();
    for (OpIndex& input : inputs) input = MapToNewGraph(input);
    return Emit<CheckpointOp>(base::VectorOf(inputs));
  }
  OpIndex Reduce(const CallOp& op) {
    ...
    return Emit<CallOp>(op.descriptor, callee, base::VectorOf(arguments));
  }
  OpIndex Reduce(const ReturnOp& op) {
    base::SmallVector<OpIndex, 16> inputs = op.inputs();
    for (OpIndex& input : inputs) input = MapToNewGraph(input);
    return Emit<ReturnOp>(base::VectorOf(inputs), op.pop_count);
  }
