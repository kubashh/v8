...
class ExplicitTruncationReducer
 ...
 public:
  ...
  OpIndex ReduceOperation(Ts... args) {
    ...
    using Op = opcode_to_operation_map<opcode>::Op;
    ...
    for (size_t i = 0; i < reps.size(); ++i) {
      if (reps[i] == MaybeRegisterRepresentation::Word32()) {
        base::Vector<const RegisterRepresentation> actual_input_rep =
            Asm().input_graph().Get(inputs[i]).outputs_rep();
        if (actual_input_rep.size() == 1 &&
            actual_input_rep[0] == RegisterRepresentation::Word64()) {
          has_truncation = true;
    ...
    return CreateCallbackWithExpandedArguments([this](auto... args) {
      return Continuation{this}.Reduce(args...);
    })(*operation);
  }
