    ...
    if (Is<ConstantOp>(right)) {
      ...
      }
      {
        ...
        if (IsIntegral(rep) &&
            ...
            shift_amount <=
                static_cast<int>(base::bits::CountLeadingZeros(mask)) &&
            shift_amount <=
                static_cast<int>(base::bits::CountLeadingZeros(k)) &&
            mask << shift_amount <= std::numeric_limits<uint32_t>::max()) {
 ...
 private:
  ...
  bool MatchConstantShift(
    ...
    if (MatchIntegralConstant(shift_op.right(), AllowTruncation(shift_op.rep),
                              &rhs_constant) &&
        rhs_constant < ElementSizeInBits(shift_op.rep)) {
