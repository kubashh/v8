...
#undef PROCESS_INT32_SHIFT
 ...
 private:
  ...
  class ThrowingScope {
    ...
    // that can throw). Then, when calling Process(Phi), for exception phis,
    // their Turboshaft inputs come from the Variable corresponding to their
    // owning intepreter register.
   ...
   public:
    ...
    ~ThrowingScope() {
      ...
      // before emitting a throwing node, so there is no need to
