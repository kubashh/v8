...
#undef PROCESS_INT32_SHIFT
 ...
 private:
  ...
  // inputs come the Variable corresponding to their owning intepreter register.
  void MaybeRegisterExceptionHandler(maglev::NodeBase* node) {
    ...
    maglev::LazyDeoptInfo* deopt_info = node->lazy_deopt_info();
    ...
    lazy_frame->as_interpreted().frame_state()->ForEachValue(
        ...
        [&](maglev::ValueNode* node, interpreter::Register reg) {
