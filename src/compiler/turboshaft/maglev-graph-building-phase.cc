...
#undef PROCESS_GENERIC_UNOP
 ...
 private:
  ...
  V<FrameState> BuildFrameState(maglev::LazyDeoptInfo* lazy_deopt_info) {
    ...
    if (lazy_deopt_info->top_frame().type() ==
        maglev::DeoptFrame::FrameType::kConstructInvokeStubFrame) {
    }
  }

  V<FrameState> BuildFrameState(maglev::DeoptFrame& frame,
    ...
    switch (frame.type()) {
      ...
      case maglev::DeoptFrame::FrameType::kConstructInvokeStubFrame:
        DCHECK_EQ(combine, OutputFrameStateCombine::Ignore());
  ...
  V<FrameState> BuildFrameState(maglev::ConstructInvokeStubDeoptFrame& frame) {
    ...
    if (frame.parent() != nullptr) {
      OpIndex parent_frame = BuildFrameState(
          *frame.parent(), OutputFrameStateCombine::Ignore(), false);
  ...
  const FrameStateInfo* MakeFrameStateInfo(
    ...
    FrameStateFunctionInfo* info =
        graph_zone()->New<FrameStateFunctionInfo>(type, 1, 0, shared_info);
