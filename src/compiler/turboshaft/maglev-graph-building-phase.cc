...
class GeneratorAnalyzer {
 ...
 public:
  ...
  bool has_header_bypasses() const { return has_header_bypasses_; }
 ...
 private:
  ...
  void FindLoopBody(const maglev::BasicBlock* header) {
    ...
    const maglev::BasicBlock* backedge_block =
        header->predecessor_at(header->predecessor_count() - 1);
    ...
    while (!visit_queue_.empty()) {
      ...
      if (block_to_header_.contains(curr)) continue;
  ...
  void FindLoopHeaderBypasses(maglev::Graph* graph) {
    ...
    for (maglev::BasicBlock* target : generator_switch_block->successors()) {
      ...
      // Depending oin the order of the yields, {target} could already be inside
      // a loop.
      ...
      if (!innermost_header) {
        ...
        if (!innermost_header) {
          ...
          for (maglev::BasicBlock* succ : real_target->successors()) {
            if (GetLoopHeader(succ)) {
              innermost_header = GetLoopHeader(succ);
              break;
            }
          }
        }
  ...
  bool has_header_bypasses_ = false;
 ...
 public:
  ...
  void EmitLoopSinglePredecessorBlock(maglev::BasicBlock* maglev_loop_header) {
    ...
    if (has_special_generator_handling) {
      ...
      for (int i = 0; i < loop_pred->PredecessorCount() - 1; i++) {
        inputs.push_back(loop_default_generator_value_);
      }
      DCHECK(switch_var_first_input.valid());
...
#undef PROCESS_GENERIC_UNOP
  ...
  maglev::ProcessResult Process(maglev::GeneratorStore* node,
    ...
    __ GeneratorStore(Map(node->context_input()), Map(node->generator_input()),
                      parameters_and_registers, num_parameters_and_registers,
                      node->suspend_id(), node->bytecode_offset());
    return maglev::ProcessResult::kContinue;
  }
