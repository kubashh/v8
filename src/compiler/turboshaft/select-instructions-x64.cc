...
void InstructionSelector::VisitBinop(OpIndex op, Binop op_kind,
  ...
  if (left == right) {
    ...
    inputs = {UseRegister(left), UseImmediate(Get(right).Cast<ConstantOp>())};
  } else {
    ...
    if (specific().CanBeMemoryOperand(opcode, right_op)) {
      ...
      inputs = {UseRegister(left), Use(right)};
    }
  }
  auto outputs = base::VectorOf({DefineSameAsInput(op, 0)});
  Emit(opcode, outputs, inputs.vector());
}
...
void InstructionSelector::VisitStackPointerGreaterThan(
  ...
  if (specific().CanBeMemoryOperand(kX64Cmp, stack_limit)) {
    ...
    EmitWithContinuation(opcode, cont, {}, inputs.vector());
  } else {
...
bool SelectInstructions(
    Zone* temp_zone, size_t node_count, Linkage* linkage,
    InstructionSequence* sequence, const turboshaft::Graph& graph,
  ...
  PlatformSpecificInstructionSelector selector{temp_zone,
                                               graph,
                                               sequence,
                                               frame,
                                               linkage,
