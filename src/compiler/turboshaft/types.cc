...
Handle<TurboshaftType> FloatType<Bits>::AllocateOnHeap(Factory* factory) const {
  ...
  if (has_nan()) k = TurboshaftTypeKind::NanBit::update(k, true);
  if (!is_only_nan()) {
    k = TurboshaftTypeKind::RangeBit::update(k, true);
    std::tie(min, max) = minmax();
  }
  if constexpr (Bits == 32) {
    return factory->NewTurboshaftFloat32Type(k, 0 /*padding*/, min, max,
                                             AllocationType::kYoung);
  } else {
    return factory->NewTurboshaftFloat64Type(k, 0 /*padding*/, min, max,
                                             AllocationType::kYoung);
  }
}
