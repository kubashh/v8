...
class MemoryContentTable
 ...
 public:
  // With potentially-overlapping loads/stores, loads and stores of variable
  // sizes could overlap (because of aliasing). For instance, consider this
  // example:
  //
  //     function f(u32s, u8s) {
  //        u32s[0] = 0xffffffff;
  ...
  enum class CanOverlap { kYes, kNo };
  ...
  explicit MemoryContentTable(
      ...
      FixedSidetable<OpIndex>& replacements, CanOverlap can_overlap)
  ...
  void Invalidate(OpIndex base, OpIndex index, int32_t offset) {
    ...
    if (non_aliasing_objects_.Get(base)) {
      ...
      if (can_overlap_ == CanOverlap::kNo) {
        InvalidateAtOffset(offset, base);
      } else {
        DCHECK_EQ(can_overlap_, CanOverlap::kYes);
...
#undef CHECK_ELEMENT_KIND
 ...
 private:
  ...
  // See comment at the begining of this class about CanOverlap.
  CanOverlap can_overlap_;
};
...
class LateLoadEliminationAnalyzer {
 public:
  ...
  LateLoadEliminationAnalyzer(Graph& graph, Zone* phase_zone,
      ...
      : graph_(graph),
        ...
        non_overlapping_memory_(phase_zone, non_aliasing_objects_, object_maps_,
                                replacements_,
                                MemoryContentTable::CanOverlap::kNo),
        overlapping_memory_(phase_zone, non_aliasing_objects_, object_maps_,
                            replacements_,
                            MemoryContentTable::CanOverlap::kYes),
        ...
        predecessor_non_overlapping_memory_snapshots_(phase_zone),
        predecessor_overlapping_memory_snapshots_(phase_zone) {}
 ...
 private:
  ...
  MemoryContentTable non_overlapping_memory_;
  MemoryContentTable overlapping_memory_;
  static constexpr uint32_t kAliasSnapshotOffset = 0;
  static constexpr uint32_t kMapSnapshotOffset = 1;
  static constexpr uint32_t kNonOverlappingMemorySnapshotOffset = 2;
  static constexpr uint32_t kOverlappingMemorySnapshotOffset = 3;
  ...
  ZoneVector<MemorySnapshot> predecessor_non_overlapping_memory_snapshots_;
  ZoneVector<MemorySnapshot> predecessor_overlapping_memory_snapshots_;
};
