...
void LoopUnrollingAnalyzer::DetectUnrollableLoops() {
  for (const auto& [start, info] : loop_finder_.LoopHeaders()) {
    if (!info.has_inner_loops) {
      loop_iteration_count_.insert({start, CanFullyUnrollLoop(info)});
    }
  }
}

int LoopUnrollingAnalyzer::CanFullyUnrollLoop(LoopFinder::LoopInfo info) {
  ...
  return CanFullyUnrollLoopWithCondition(cond);
}

int LoopUnrollingAnalyzer::CanFullyUnrollLoopWithCondition(
    const Operation& cond) {
    ...
    if (match_word_binop(phi.input(1), &left, &right, &binop_op, &binop_rep) ||
      ...
      if (left == input_graph_->Index(phi)) {
        ...
        if (matcher_.MatchUnsignedIntegralConstant(right, &binop_cst)) {
          ...
          int res = CanFullyUnrollCompareBinop(cmp_cst, cmp_op, phi_cst,
...
int LoopUnrollingAnalyzer::CanFullyUnrollCompareBinop(
