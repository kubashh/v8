...
std::ostream& operator<<(std::ostream& os, const IterationCount& count) {
  if (count.IsExact()) {
    return os << "Exact[" << count.count() << "]";
  } else if (count.IsApprox()) {
    return os << "Approx[" << count.count() << "]";
  } else {
...
IterationCount StaticCanonicalForLoopMatcher::CountIterationsImpl(
  ...
  // TODO: more precise for specific things (eg, Add + SignedLessThan).

  // if (binop_op == StaticCanonicalForLoopMatcher::BinOp::kAdd) {
  //   //     init + binop_cst*k < max
  //   // ==> binop_cst*k < max - init
  //   // ==> k <  (max - init) / binop_cst
  // } else if (binop_op == StaticCanonicalForLoopMatcher::BinOp::kSub) {
  ...
  // std::cout << "for(i = " << init << "; i " << cmp_op << " " << max
  //           << "; i = i " << binop_op << " " << binop_cst << ")\n";

  Int curr = init;
  size_t iter_count = 0;
  for (; iter_count < kMaxExactIter; iter_count++) {
    if (Cmp(curr, max, cmp_op) != loop_if_cond_is) {
      // std::cout << "  ==> Exact: " << iter_count << "\n";
    ...
    if (auto next = Next(curr, binop_cst, binop_op, binop_rep)) {
      ...
      // std::cout << "  ==> Overflow after: " << iter_count << "\n";
      break;
    }
  }
  // std::cout << "  ==> Bailing out after: " << iter_count << "\n";
...
IterationCount StaticCanonicalForLoopMatcher::CountIterations(
  ...
  switch (cmp_op) {
    ...
    case CmpOp::kUnsignedGreaterThanOrEqual:
      if (binop_rep == WordRepresentation::Word32()) {
        res = CountIterationsImpl<uint32_t>(
        ...
        DCHECK_EQ(binop_rep, WordRepresentation::Word64());
        res = CountIterationsImpl<uint64_t>(initial_input, cmp_cst, cmp_op,
