...
void LateLoadEliminationAnalyzer::ProcessLoad(OpIndex op_idx,
                                              const LoadOp& load) {
  auto& memory =
      load.kind.tagged_base ? tagged_base_memory_ : untagged_base_memory_;
...
void LateLoadEliminationAnalyzer::ProcessStore(OpIndex op_idx,
                                               const StoreOp& store) {
  auto& memory =
      store.kind.tagged_base ? tagged_base_memory_ : untagged_base_memory_;
...
void LateLoadEliminationAnalyzer::ProcessCall(OpIndex op_idx,
    ...
    if (auto builtin_id = TryGetBuiltinId(
      ...
      switch (*builtin_id) {
        ...
        case Builtin::kCopyFastSmiOrObjectElements:
          ...
          tagged_base_memory_.Invalidate(call->arguments()[0],
                                         OpIndex::Invalid(),
  ...
  tagged_base_memory_.InvalidateMaybeAliasing();
  untagged_base_memory_.InvalidateMaybeAliasing();
}
...
void LateLoadEliminationAnalyzer::FinishBlock(const Block* block) {
  block_to_snapshot_mapping_[block->index()] =
      std::tuple{non_aliasing_objects_.Seal(), object_maps_.Seal(),
                 tagged_base_memory_.Seal(), untagged_base_memory_.Seal()};
}

void LateLoadEliminationAnalyzer::SealAndDiscard() {
  non_aliasing_objects_.Seal();
  object_maps_.Seal();
  tagged_base_memory_.Seal();
  untagged_base_memory_.Seal();
}
...
bool LateLoadEliminationAnalyzer::BeginBlock(const Block* block) {
  ...
  // Collect the snapshots of all predecessors.
  {
    ...
    predecessor_tagged_base_memory_snapshots_.clear();
    predecessor_untagged_base_memory_snapshots_.clear();
    for (const Block* p = block->LastPredecessor(); p != nullptr;
      ...
      predecessor_tagged_base_memory_snapshots_.push_back(
          std::get<kTaggedBaseSnapshotOffset>(*pred_snapshots));
      predecessor_untagged_base_memory_snapshots_.push_back(
  ...
  tagged_base_memory_.StartNewSnapshot(
      base::VectorOf(predecessor_tagged_base_memory_snapshots_), merge_memory);
  untagged_base_memory_.StartNewSnapshot(
      base::VectorOf(predecessor_untagged_base_memory_snapshots_),
      merge_memory);
