...
bool SLPTree::IsSideEffectFree(OpIndex first, OpIndex second) {
  ...
  while (prev_node != first) {
    ...
    if ((IsLoad(graph().Get(second).opcode) &&
         IsLoad(graph().Get(prev_node).opcode))
            ? CannotSwapLoads(prev_effects, effects)
