...
#define UNOP_CASE(op_low, not_used, op_high)                                   \
  case Simd128UnaryOp::Kind::k##op_low: {                                      \
    if (op1.Cast<Simd128UnaryOp>().kind == Simd128UnaryOp::Kind::k##op_high && \
        op0.input(0) == op1.input(0)) {                                        \
      return {true, true};                                                     \
    }                                                                          \
    V8_FALLTHROUGH;                                                            \
  }                                                                            \
...
#define BINOP_CASE(op_low, not_used, op_high)                                  \
  case Simd128BinopOp::Kind::k##op_low: {                                      \
    if (op1.Cast<Simd128BinopOp>().kind == Simd128BinopOp::Kind::k##op_high && \
        op0.input(0) == op1.input(0) && op0.input(1) == op1.input(1)) {        \
      return {true, true};                                                     \
    }                                                                          \
    V8_FALLTHROUGH;                                                            \
  }                                                                            \
...
bool SLPTree::CanBePacked(const NodeGroup& node_group) {
  ...
  bool is_sign_ext, is_packable_sign_ext;
  std::tie(is_sign_ext, is_packable_sign_ext) =
  ...
  case Simd128UnaryOp::Kind::k##op_low:
      switch (op0.Cast<Simd128UnaryOp>().kind) {
        SIMD256_UNARY_SIGN_EXTENSION_OP(UNARY_SING_EXTENSION_CASE) {
          TRACE("Added a vector of sign extension unop and stop bulid tree\n");
  ...
  case Simd128BinopOp::Kind::k##op_low:
      switch (op0.Cast<Simd128BinopOp>().kind) {
        SIMD256_BINOP_SIGN_EXTENSION_OP(BINOP_SING_EXTENSION_CASE) {
          TRACE("Added a vector of sign extension binop and stop bulid tree\n");
          return NewPackNode(node_group);
        }
