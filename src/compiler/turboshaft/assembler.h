...
class LoopLabel : public LabelBase<true, Reps...> {
 ...
 public:
  ...
  template <typename A>
  void Goto(A& assembler, const super::values_t& values) {
  ...
  template <typename A>
  base::prepend_tuple_type<bool, typename super::values_t> Bind(A& assembler) {
    // LoopLabels must not be bound  using `Bind`, but with `Loop`.
    UNREACHABLE();
  }

  template <typename A>
  base::prepend_tuple_type<bool, typename super::values_t> Loop(A& assembler) {
    if (!assembler.Bind(loop_header_data_.block)) {
      return std::tuple_cat(std::tuple{false}, typename super::values_t{});
    }
 ...
 private:
  template <typename A>
  static super::values_t MaterializeLoopPhis(A& assembler, BlockData& data) {
    return MaterializeLoopPhisImpl(assembler, data,
                                   std::make_index_sequence<super::size>());
  }

  template <typename A, size_t... indices>
  static super::values_t MaterializeLoopPhisImpl(
      A& assembler, BlockData& data, std::index_sequence<indices...>) {
    size_t predecessor_count = data.block->PredecessorCount();
...
#undef DECL_TRY_CHANGE
  ...
  Ret CallBuiltinImpl(Isolate* isolate, Builtin function,
    ...
    if constexpr (std::is_same_v<Ret, void>) {
      Call(HeapConstant(callable.code()), {}, base::VectorOf(inputs), desc);
    } else {
      return Call(HeapConstant(callable.code()), {}, base::VectorOf(inputs),
                  desc);
    }
  }
  ...
  Ret CallRuntimeImpl(Isolate* isolate, Runtime::FunctionId function,
    ...
    constexpr size_t argc = std::tuple_size_v<Args>;
    USE(kMaxNumArgs);
    DCHECK_GE(kMaxNumArgs, argc);
    auto inputs = std::apply(
        [](auto&&... as) {
          return base::SmallVector<OpIndex, argc + 4>{
              std::forward<decltype(as)>(as)...};
        },
        args);
...
#undef BIGINT_BINOP
  ...
  template <typename L>
  auto ControlFlowHelper_Loop(L& label)
      -> base::prepend_tuple_type<bool, typename L::values_t> {
    // Only LoopLabels can be bound with `LOOP`. Otherwise use `BIND`.
    static_assert(L::is_loop);
    return label.Loop(stack());
  }
  ...
  bool ControlFlowHelper_ElseIf(std::function<V<Word32>()> condition_builder) {
