...
void OrderBy(base::SmallVector<T, N>& values,
             const base::SmallVector<size_t, N>& order) {
...
#endif
    (void)std::initializer_list<int>{(std::get<indices>(recorded_values_)
  ...
  values_t MaterializePhisImpl(A& assembler, std::index_sequence<indices...>) {
    ...
    if constexpr (is_loop) {
      ...
      if (predecessor_count == 1) {
        values_t result;
        (void)std::initializer_list<int>{
            (std::get<indices>(result) = std::get<indices>(recorded_values_)[0],
             0)...};
        return result;
      }
