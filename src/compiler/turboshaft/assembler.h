...
class ReducerBase : public ReducerBaseForwarder<Next> {
 public:
  ...
  OpIndex ReduceProjection(OpIndex tuple, uint16_t index,
    ...
    return Base::ReduceProjection(tuple, index, rep);
  }
  ...
  OpIndex ReduceCallAndCatchException(OpIndex callee, Block* if_success,
                                      Block* if_exception,
    ...
    OpIndex new_opindex = Base::ReduceCallAndCatchException(
        callee, if_success, if_exception, arguments, descriptor);
...
#undef DECL_TRY_CHANGE
  ...
  OpIndex CallAndCatchException(OpIndex callee, Block* if_success,
                                Block* if_exception,
                                base::Vector<const OpIndex> arguments,
                                const TSCallDescriptor* descriptor) {
    return stack().ReduceCallAndCatchException(callee, if_success, if_exception,
