...
#undef DECL_MULTI_REP_UNARY
  ...
  OpIndex Word32Select(OpIndex condition, OpIndex left, OpIndex right) {
    return subclass().ReduceSelect(condition, left, right,
                                   WordRepresentation::Word32());
  }
...
#undef DECL_TRY_CHANGE
  ...
  void Store(OpIndex base, OpIndex value, StoreOp::Kind kind,
             ...
             int32_t offset = 0) {
    Store(base, value, OpIndex::Invalid(), kind, stored_rep, write_barrier,
          offset);
  }
  ...
  OpIndex TailCall(OpIndex callee, base::Vector<const OpIndex> arguments,
                   const CallDescriptor* descriptor) {
    return subclass().ReduceTailCall(callee, arguments, descriptor);
  }
  ...
  OpIndex DeoptimizeIf(OpIndex condition, OpIndex frame_state,
                       const DeoptimizeParameters* parameters) {
    return subclass().ReduceDeoptimizeIf(condition, frame_state, false,
                                         parameters);
  }
  OpIndex DeoptimizeIfNot(OpIndex condition, OpIndex frame_state,
                          const DeoptimizeParameters* parameters) {
    return subclass().ReduceDeoptimizeIf(condition, frame_state, true,
                                         parameters);
  }
  ...
  OpIndex TrapIf(OpIndex condition, TrapId trap_id) {
    return subclass().ReduceTrapIf(condition, false, trap_id);
  }
  OpIndex TrapIfNot(OpIndex condition, TrapId trap_id) {
    return subclass().ReduceTrapIf(condition, true, trap_id);
  }
