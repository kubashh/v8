...
// TODO(mliedtke): Move to separate file.
// TODO(mliedtke): Can we unify this with optimization-phase.h?
template <class Callback>
class CallWithArgsFromOp {
 public:
  explicit CallWithArgsFromOp(Callback callback)
...
// provided callback with the parameter signature matching the constructor of
// the operation. This can be used to "unpack" an Operation and pass its values
// inta a Reduce function.
template <class Callback>
auto CreateCallbackWithExpandedArguments(Callback callback) {
  return CallWithArgsFromOp<Callback>(callback);
}
