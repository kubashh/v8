...
struct ThrowingOpHasLazyDeoptOption {
  static constexpr bool value =
      TupleHasType<LazyDeoptOnThrow,
                   decltype(std::declval<Op>().options())>::value;
};
...
struct OperationT : Operation {
  ...
  static Derived& New(Graph* graph, size_t input_count, Args... args) {
    ...
    if constexpr (MayThrow(opcode)) {
      static_assert(details::ThrowingOpHasProperMembers<Derived>());
      if constexpr (opcode != Opcode::kCall) {
        static_assert(details::ThrowingOpHasLazyDeoptOption<Derived>::value);
      }
    }
