...
struct CallAndCatchExceptionOp : OperationT<CallAndCatchExceptionOp> {
  ...
  base::Vector<const RegisterRepresentation> outputs_rep() const {
    // TODO: on the exception side, we should be able to access this as a value,
    // which means that {outputs_rep} in the case of CallAndCatchExceptionOp is
    // quite meaningless. Maybe we should add an UNREACHABLE() here. And
    // CallAndCatchExceptionOp should probably be mergeable then, but only on
    // the {if_exception} side (and the rep of the Phi would be Tagged).
    return descriptor->out_reps;
  }
  size_t output_count() const { return outputs_rep().size() - 1; }
