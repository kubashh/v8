...
struct ThrowingOpHasProperOutputsMember : std::false_type {};
template <typename Op>
struct ThrowingOpHasProperOutputsMember<
    Op, std::void_t<decltype(Op::kOutputRepsStorage)>> : std::true_type {};
// The OutputReps for Calls are dynamic, so we special-case Calls.
template <>
struct ThrowingOpHasProperOutputsMember<CallOp, void> : std::true_type {};
...
struct OperationT : Operation {
  ...
  static Derived& New(Graph* graph, size_t input_count, Args... args) {
    ...
    if constexpr (MayThrow(opcode)) {
      static_assert(ThrowingOpHasProperOutputsMember<Derived>());
    }
