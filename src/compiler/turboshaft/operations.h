...
struct AtomicWord32PairOp : OperationT<AtomicWord32PairOp> {
  ...
  V<Word32> value_low() const {
    return (input_count > 3) ? input(1 + has_index) : OpIndex::Invalid();
  }
  V<Word32> value_high() const {
    return (input_count > 3) ? input(2 + has_index) : OpIndex::Invalid();
  }
  V<Word32> expected_low() const {
    return (input_count > 5) ? input(3 + has_index) : OpIndex::Invalid();
  }
  V<Word32> expected_high() const {
    return (input_count > 5) ? input(4 + has_index) : OpIndex::Invalid();
  }
