...
struct OutOfLineOpT : OperationT<Derived, OutOfLineOp> {
  ...
  OutOfLineOpT(size_t input_count, const Data& data, Zone* zone)
    ...
    size_t inputs_size = input_count * sizeof(OpIndex);
...
struct CallOp : OutOfLineOpT<CallOp> {
  ...
  const CallDescriptor* descriptor() const { return data().descriptor; }

  CallOp(const CallDescriptor* descriptor, OpIndex callee,
...
struct ReturnOp : OutOfLineOpT<ReturnOp> {
  struct Data {
    int32_t pop_count;
  };
  ...
  int32_t pop_count() const { return data().pop_count; }
  base::Vector<const OpIndex> return_values() const { return Inputs(); }

  explicit ReturnOp(base::Vector<const OpIndex> return_values,
                    int32_t pop_count, Zone* zone)
      : OutOfLineOpT<ReturnOp>(return_values, Data{pop_count}, zone) {}
};
...
template <class Op>
struct static_operation_storage_size<Op, base::void_t<typename Op::Storage>>
    : std::integral_constant<size_t, sizeof(typename Op::Storage)> {};
constexpr size_t kOperationStorageSizeTable[kNumberOfOpcodes] = {
#define INPUTS_OFFSET(Name) static_operation_storage_size<Name##Op>::value,
    TURBOSHAFT_OPERATION_LIST(INPUTS_OFFSET)
#undef INPUTS_OFFSET
};
