...
struct CheckpointData {
  ...
  class Builder {
   public:
    ...
    const CheckpointData* AllocateCheckpointData(
        const FrameStateInfo& frame_state_info, Zone* zone) {
      return zone->New<CheckpointData>(CheckpointData{
   ...
   private:
    ...
    base::SmallVector<int32_t, 16> int_operands_;
  ...
  struct Iterator {
    ...
    base::Vector<const int32_t> int_operands;
    ...
    void ConsumeDematerializedObject(int32_t* id, int32_t* field_count) {
    ...
    void ConsumeDematerializedObjectReference(int32_t* id) {
  ...
  Iterator iterator(base::Vector<const OpIndex> inputs) const {
    return Iterator{instructions, machine_types, int_operands, inputs};
  }

  const FrameStateInfo& frame_state_info;
  base::Vector<Instr> instructions;
  base::Vector<MachineType> machine_types;
  base::Vector<int32_t> int_operands;
};
...
struct FullCheckpointOp : OperationT<FullCheckpointOp> {
  const CheckpointData* data;

  static constexpr OpProperties properties = OpProperties::Reading();

  explicit FullCheckpointOp(base::Vector<const OpIndex> inputs,
                            const CheckpointData* data)
      : Base(inputs), data(data) {}
};
...
struct CheckLazyDeoptOp : FixedOperationT<2, CheckLazyDeoptOp> {
  ...
  OpIndex checkpoint() const { return inputs()[1]; }

  explicit CheckLazyDeoptOp(OpIndex call, OpIndex checkpoint)
      : Base({call, checkpoint}) {}
};
