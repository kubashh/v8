...
struct PhiOp : OperationT<PhiOp> {
  ...
  explicit PhiOp(base::Vector<const OpIndex> inputs) : Base(inputs) {}
...
struct PendingLoopPhiOp : FixedOperationT<1, PendingLoopPhiOp> {
  ...
  PendingLoopPhiOp(OpIndex first, OpIndex old_backedge_index)
      : Base({first}), old_backedge_index(old_backedge_index) {
    DCHECK(old_backedge_index.valid());
  }
  PendingLoopPhiOp(OpIndex first, Node* old_backedge_node)
      : Base({first}), old_backedge_node(old_backedge_node) {}
};
...
#define OPERATION_OPCODE_MAP_CASE(Name) \
  template <>                           \
  const Opcode OperationT<Name##Op>::opcode = Opcode::k##Name;
