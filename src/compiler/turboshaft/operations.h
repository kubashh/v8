...
struct ChangeOp : FixedOperationT<1, ChangeOp> {
  enum class Kind : uint8_t {
    kNarrow,      // undefined behavior if value cannot be represented precisely
    kTruncate,    // reduce bit-width, resulting in a modulo operation
    kZeroExtend,  // increase bit-width for unsigned integer values
    kSignExtend,  // increase bid-width for signed integer values
    kBitcast      // preserve bits, change meaning
  };
  Kind kind;
