...
struct AtomicWord32PairOp : OperationT<AtomicWord32PairOp> {
  ...
  V<WordPtr> index() const {
    return has_index_ ? input(1) : OpIndex::Invalid();
  }
  V<Word32> value_low() const {
    return (input_count > 2) ? input(1 + has_index_) : OpIndex::Invalid();
  }
  V<Word32> value_high() const {
    return (input_count > 2) ? input(2 + has_index_) : OpIndex::Invalid();
  }
  V<Word32> expected_low() const {
    return (input_count > 4) ? input(3 + has_index_) : OpIndex::Invalid();
  }
  V<Word32> expected_high() const {
    return (input_count > 4) ? input(4 + has_index_) : OpIndex::Invalid();
  }

  void Validate(const Graph& graph) const {
    if (op_kind == OpKind::kLoad) {
      DCHECK_EQ(input_count, 1 + has_index_);
    }
    DCHECK_EQ(op_kind == OpKind::kLoad, input_count == 1 + has_index_);
    DCHECK_EQ(op_kind == OpKind::kCompareExchange,
              input_count == 5 + has_index_);
    DCHECK_EQ(op_kind != OpKind::kCompareExchange && op_kind != OpKind::kLoad,
              input_count == 3 + has_index_);
  }
  ...
  AtomicWord32PairOp(V<WordPtr> base, V<WordPtr> index, V<Word32> value_low,
      ...
      : Base(1 + index.valid() + value_low.valid() + value_high.valid() +
        ...
        op_kind(op_kind),
        offset(offset) {
    has_index_ = index.valid();
    input(0) = base;
    if (index.valid()) {
      input(1) = index;
    }
    if (value_low.valid()) {
      input(1 + has_index_) = value_low;
    }
    if (value_high.valid()) {
      input(2 + has_index_) = value_high;
    }
    if (expected_low.valid()) {
      input(3 + has_index_) = expected_low;
    }
    if (expected_high.valid()) {
      input(4 + has_index_) = expected_high;
    }
  }
    ...
    DCHECK(
        kind.tagged_base
            ...
            : ValidOpInputRep(graph, base(),
                              {RegisterRepresentation::PointerSized(),
                               RegisterRepresentation::Tagged()}));
    if (!ValidOpInputRep(graph, value(),
                         stored_rep.ToRegisterRepresentationForStore())) {
      PrintInputs(std::cerr, "#");
      UNREACHABLE();
    }
    if (index().valid()) {
      DCHECK(ValidOpInputRep(graph, index(),
