...
struct BinaryOp : FixedOperationT<2, BinaryOp> {
  ...
  BinaryOp(OpIndex left, OpIndex right, Kind kind, bool with_overflow_bit,
           MachineRepresentation rep)
      : Base({left, right}),
        kind(kind),
        with_overflow_bit(with_overflow_bit),
        rep(rep) {}
  auto options() const { return std::make_tuple(kind, with_overflow_bit, rep); }
  void PrintOptions(std::ostream& os) const;
};
