...
// ReinterpretWord32Sign is a type-hint used during Maglev->Turboshaft
...
// also have them in Turboshaft. Eventually, ReinterpretWord32Sign is just a
// nope in Turboshaft, since as far as Machine level graph is concerned, both
// Int32 and Uint32 are just Word32 registers.
struct ReinterpretWord32SignOp
    : FixedArityOperationT<1, ReinterpretWord32SignOp> {
  ...
  ReinterpretWord32SignOp(V<Word32> input, Sign from, Sign to)
      : Base(input), from(from), to(to) {}

  void Validate(const Graph& graph) const { DCHECK_NE(from, to); }
  auto options() const { return std::tuple{from, to}; }
};
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream& os,
                                           ReinterpretWord32SignOp::Sign sign);
