...
struct TSCallDescriptor : public NON_EXPORTED_BASE(ZoneObject) {
  ...
  // one particular call site, this assumes (only works correctly) if
  // `TSCallDescriptor`s are not shared across different calls (which they are
  // not at the moment).
  // Then, one would need to make this an argument of the Turboshaft CallOp
  // instead, which causes a lot of code churn (needs touching all reducers).
