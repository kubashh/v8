...
void JSONTurboshaftGraphWriter::PrintEdges() {
  ...
  for (const Block& block : turboshaft_graph_.blocks()) {
    for (const Operation& op : turboshaft_graph_.operations(block)) {
      ...
      // Reorder the inputs of loads and stores to correspond to the order used
      // in constructor and assembler functions.
      if (auto* load = op.TryCast<LoadOp>()) {
        if (load->index().valid()) {
          DCHECK_EQ(load->input_count, 2);
          inputs = {load->base(), load->index()};
        }
      } else if (auto* store = op.TryCast<StoreOp>()) {
