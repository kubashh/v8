...
void DecompressionAnalyzer::ProcessOperation(const Operation& op) {
  switch (op.opcode) {
    ...
    case Opcode::kBitwiseAnd: {
      auto& and_op = op.Cast<BitwiseAndOp>();
      if (and_op.rep == MachineRepresentation::kWord64) {
        MarkAsNeedsDecompression(and_op.left());
        MarkAsNeedsDecompression(and_op.right());
      }
      break;
    }
    case Opcode::kBitwiseOr: {
      auto& or_op = op.Cast<BitwiseOrOp>();
      if (or_op.rep == MachineRepresentation::kWord64) {
        MarkAsNeedsDecompression(or_op.left());
        MarkAsNeedsDecompression(or_op.right());
      }
      break;
    }
    case Opcode::kBitwiseXor: {
      auto& or_op = op.Cast<BitwiseXorOp>();
      if (or_op.rep == MachineRepresentation::kWord64) {
        MarkAsNeedsDecompression(or_op.left());
        MarkAsNeedsDecompression(or_op.right());
      }
...
void RunDecompressionOptimization(Graph& graph, Zone* temp_zone) {
  DecompressionAnalyzer analyzer(graph, temp_zone);
  analyzer.Run();
