...
struct BitfieldCheck {
  ...
  static base::Optional<BitfieldCheck> Detect(const OperationMatcher& matcher,
    ...
    if (const ComparisonOp* equal = op.TryCast<Opmask::kWord32Equal>()) {
      ...
      return TryDetectShiftAndMaskOneBit<WordRepresentation::Word64().value()>(
          matcher, truncate->input());
    } else {
      return TryDetectShiftAndMaskOneBit<WordRepresentation::Word32().value()>(
          matcher, index);
    }
    return base::nullopt;
  }
 ...
 private:
  ...
  static base::Optional<BitfieldCheck> TryDetectShiftAndMaskOneBit(
    ...
    if (OpIndex left, right;
        matcher.MatchBitwiseAnd(index, &left, &right, Rep)) {
      if (int64_t right_value;
          matcher.MatchIntegralWordConstant(right, Rep, &right_value) &&
          right_value == 1) {
        OpIndex input;
        if (int shift_amount;
            matcher.MatchConstantRightShift(left, &input, Rep, &shift_amount) &&
        ...
        return BitfieldCheck{left, 1, 1, Rep == WordRepresentation::Word64()};
      }
    }
    return base::nullopt;
  }
