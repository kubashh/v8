...
struct BitfieldCheck {
  ...
  static std::optional<BitfieldCheck> Detect(const OperationMatcher& matcher_,
                                             const Graph& graph,
                                             OpIndex index) {
    ...
    if (const ComparisonOp* equal = op.TryCast<Opmask::kWord32Equal>()) {
      if (const WordBinopOp* left_and =
        ...
        if (matcher_.MatchIntegralWord32Constant(left_and->right(), &mask) &&
            matcher_.MatchIntegralWord32Constant(equal->right(),
                                                &masked_value)) {
      ...
      return TryDetectShiftAndMaskOneBit<Word64>(matcher_, truncate->input());
    } else {
      return TryDetectShiftAndMaskOneBit<Word32>(matcher_, index);
    }
    return std::nullopt;
  }
 ...
 private:
  ...
  static std::optional<BitfieldCheck> TryDetectShiftAndMaskOneBit(
      const OperationMatcher& matcher_, OpIndex index) {
    constexpr WordRepresentation Rep = V<WordType>::rep;
    // Look for the pattern `(val >> shift) & 1`. The shift may be omitted.
    V<WordType> value;
    uint64_t constant;
    if (matcher_.MatchBitwiseAndWithConstant(index, &value, &constant, Rep) &&
        constant == 1) {
      OpIndex input;
      if (int shift_amount;
          matcher_.MatchConstantRightShift(value, &input, Rep, &shift_amount) &&
  ...
  OpIndex REDUCE(Shift)(OpIndex left, OpIndex right, ShiftOp::Kind kind,
    ...
    if (int32_t amount; matcher_.MatchIntegralWord32Constant(right, &amount) &&
      ...
      if (kind == any_of(Kind::kShiftRightArithmetic,
        ...
        // (x << k) >> k
        if (matcher_.MatchConstantShift(left, &x, ShiftOp::Kind::kShiftLeft, rep,
                                       &left_shift_amount) &&
