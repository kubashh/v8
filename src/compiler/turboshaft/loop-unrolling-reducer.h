 ...
 public:
  ...
  bool ShouldFullyUnrollLoop(const Block* loop_header) const {
    ...
    return iter_count.IsExact() &&
           iter_count.count() < kMaxLoopIterationsForFullUnrolling;
  }
...
class LoopStackCheckElisionReducer : public Next {
 ...
 private:
  ...
  // should be removed.
  const absl::flat_hash_set<uint32_t>& stack_checks_to_remove_ =
...
void LoopUnrollingReducer<Next>::FullyUnrollLoop(const Block* header) {
  ...
  size_t iter_count = analyzer_.GetIterationCount(header).count();
