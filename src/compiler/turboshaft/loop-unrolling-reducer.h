...
class LoopFinder {
 ...
 public:
  struct LoopInfo {
    ...
    int size; // Number of blocks in this loop (excluding inner loops)
  };
...
class LoopUnrollingAnalyzer {
 public:
  ...
  //void GetLoopBody(ZoneUnorderedSet<Block*>& loop_body, BlockIndex loop_header);
 ...
 private:
  ...
  int CanUnrollCompareBinop(uint64_t equal_cst, uint64_t initial_input,
                             uint64_t binop_cst, WordBinopOp::Kind kind);
