...
class LoopUnrollingAnalyzer {
 ...
 private:
  ...
  int CanFullyUnrollLoop(LoopFinder::LoopInfo info);
  int CanFullyUnrollLoopWithCondition(const Operation& cond);
  int CanFullyUnrollCompareBinop(uint64_t equal_cst, CmpOp cmp_op,
 ...
 public:
  ...
  OpIndex REDUCE_INPUT_GRAPH(Goto)(OpIndex ig_idx, const GotoOp& gto) {
    ...
    if (unrolling_ == UnrollingStatus::kNotUnrolling && dst->IsLoop() &&
        Asm().current_input_block() != dst->LastPredecessor() &&
...
void LoopUnrollingReducer<Next>::FixLoopPhis(Block* input_graph_loop,
  ...
  for (const Operation& op : Asm().input_graph().operations(
    ...
    if (auto* input_phi = op.TryCast<PhiOp>()) {
      OpIndex phi_index = Asm().template MapToNewGraph<true>(
  ...
  Asm().RestoreTemporaryVariableSnapshotAfter(backedge_block);
