 ...
 private:
  void RecordAllocateUse(OpIndex source, OpIndex dst) {
    ...
    if (alloc_uses_.find(source) == alloc_uses_.end()) {
      uses = phase_zone_->New<ZoneVector<OpIndex>>(phase_zone_);
...
class LateEscapeAnalysisReducer : public Next {
 public:
  ...
  bool SkipOperation(const StoreOp& op, OpIndex old_idx) {
    return analyzer_.ShouldSkipOperation(old_idx) ||
           Next::SkipOperation(op, old_idx);
  }

  bool SkipOperation(const AllocateOp& op, OpIndex old_idx) {
    return analyzer_.ShouldSkipOperation(old_idx) ||
           Next::SkipOperation(op, old_idx);
  }
