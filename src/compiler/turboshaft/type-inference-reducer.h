...
struct WordOperationTyper {
  ...
  static word_t distance(const std::pair<word_t, word_t>& range) {
    return range.first <= range.second ? range.first - range.second
...
class TypeInferenceReducer : public Next {
 ...
 public:
  ...
  void RefineTypesAfterBranch(const BranchOp* branch, bool then_branch) {
    OpIndex refined_op = OpIndex::Invalid();
    ...
    if (const ComparisonOp* comparison = condition.TryCast<ComparisonOp>()) {
      ...
      switch (comparison->rep.value()) {
        case RegisterRepresentation::Word32(): {
          ...
          if (then_branch) {
            l_min = 0;
            l_max = is_less_than ? next_smaller(r.unsigned_max())
                                 : r.unsigned_max();
            r_min =
                is_less_than ? next_larger(l.unsigned_min()) : l.unsigned_min();
            r_max = std::numeric_limits<uint32_t>::max();
          } else {
            l_min =
                is_less_than ? r.unsigned_min() : next_larger(r.unsigned_min());
            l_max = std::numeric_limits<uint32_t>::max();
            r_min = 0;
            r_max = is_less_than ? l.unsigned_max()
                                 : next_smaller(l.unsigned_max());
          }
        ...
        case RegisterRepresentation::Float64(): {
          ...
          if (then_branch) {
            l_min = -infty;
            l_max = is_less_than ? next_smaller(r.max()) : r.max();
            r_min = is_less_than ? next_larger(l.min()) : l.min();
            r_max = infty;
          } else {
            l_min = is_less_than ? r.min() : next_larger(r.min());
            l_max = infty;
            r_min = -infty;
            r_max = is_less_than ? l.max() : next_smaller(l.max());
  ...
  void RefineOperationType(OpIndex op, const Type& type,
    ...
    const auto op_str = Asm().output_graph().Get(op).ToString().substr(0, 40);
    USE(op_str);
    TRACE_TYPING("\033[32m  %c: %3d:%-40s ~~> %s\033[0m\n", case_for_tracing,
                 op.id(), op_str.c_str(), type.ToString().c_str());
 ...
 private:
  enum class BranchCase { kTrue, kFalse };
