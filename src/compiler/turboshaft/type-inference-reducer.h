    ...
    if (const ComparisonOp* comparison = condition.TryCast<ComparisonOp>()) {
      ...
      switch (comparison->rep.value()) {
        case RegisterRepresentation::Word32(): {
          ...
          const bool lhs_is_word32_truncated = lhs.IsWord64();
          const bool rhs_is_word32_truncated = lhs.IsWord64();
          lhs = Typer::TruncateWord32Input(lhs, true, zone);
          rhs = Typer::TruncateWord32Input(rhs, true, zone);
          ...
          if (l_refined.IsWord32() && lhs_is_word32_truncated) {
            l_refined = Typer::ExtendWord32ToWord64(l_refined.AsWord32(), zone);
          }
          r_refined = Word32Type::Intersect(
              r, r_restrict, Type::ResolutionMode::kOverApproximate, zone);
          if (r_refined.IsWord32() && rhs_is_word32_truncated) {
            r_refined = Typer::ExtendWord32ToWord64(r_refined.AsWord32(), zone);
          }
          break;
        }
