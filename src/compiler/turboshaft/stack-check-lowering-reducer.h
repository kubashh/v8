...
class StackCheckLoweringReducer : public Next {
 public:
  ...
  OpIndex REDUCE(StackCheck)(StackCheckOp::CheckKind kind) {
#ifdef V8_ENABLE_WEBASSEMBLY
    if (kind == StackCheckOp::CheckKind::kWasmFunctionHeader &&
        __ IsLeafFunction()) {
    ...
    compiler::StackCheckKind check_kind =
        kind == StackCheckOp::CheckKind::kJSFunctionHeader
    ...
    IF_NOT (LIKELY(check)) {
      if (kind == StackCheckOp::CheckKind::kJSFunctionHeader) {
        __ CallRuntime_StackGuardWithGap(isolate(), __ NoContextConstant(),
                                         __ StackCheckOffset());
      }
#ifdef V8_ENABLE_WEBASSEMBLY
      else {
        DCHECK(kind == StackCheckOp::CheckKind::kWasmFunctionHeader ||
               kind == StackCheckOp::CheckKind::kWasmLoop);
