...
namespace {
bool IsFeedbackMegamorphic(VectorSlotPair const& vector_slot_pair,
                           JSHeapBroker* broker) {
  if (!FLAG_concurrent_inlining) {
    return vector_slot_pair.ic_state() == MEGAMORPHIC;
  }
  ProcessedFeedback const& feedback = broker->GetFeedbackForPropertyAccess(
      FeedbackSource(vector_slot_pair), AccessMode::kLoad, base::nullopt);
  ...
  if (feedback.kind() == ProcessedFeedback::kElementAccess) {
    ...
    TRACE_BROKER_MISSING(broker, "insufficient feedback");
    // Assume megamorphic.
    return true;
  }
  CHECK(false && "Feedback should be either element or named access");
}
}  // namespace
...
void JSGenericLowering::LowerJSLoadProperty(Node* node) {
  ...
  if (outer_state->opcode() != IrOpcode::kFrameState) {
    Callable callable = Builtins::CallableFor(
        isolate(), IsFeedbackMegamorphic(p.feedback(), broker())
