...
void JSGenericLowering::LowerJSCreateArray(Node* node) {
  ...
  Node* stub_arity = jsgraph()->Int32Constant(arity + kArgcReceiverSlots);
...
void JSGenericLowering::LowerJSConstructForwardVarargs(Node* node) {
  ...
  Node* stub_code = jsgraph()->HeapConstant(callable.code());
  Node* stub_arity = jsgraph()->Int32Constant(arg_count + kArgcReceiverSlots);
...
void JSGenericLowering::LowerJSConstructWithSpread(Node* node) {
  ...
  Node* stub_arity =
      jsgraph()->Int32Constant(arg_count + kArgcReceiverSlots - kTheSpread);
  Node* receiver = jsgraph()->UndefinedConstant();
