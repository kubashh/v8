...
std::unique_ptr<TurbofanPipelineStatistics> CreatePipelineStatistics(
...
//        pipeline_statistics_(pipeline_statistics),
  ...
  static PipelineData ForCodeStubAssembler(Isolate* isolate, OptimizedCompilationInfo* info, Graph* graph, JSGraph* jsgraph, Schedule* schedule,
  ...
  JumpOptimizationInfo* jump_opt, const ProfileDataFromFile* profile_data) {
    ...
    PipelineData data(isolate, wasm_engine, isolate->allocator(), info, {}, turboshaft::TurboshaftPipelineKind::kCSA,
      assembler_options);
...
//  PipelineData& operator=(PipelineData&&) = default;
 ...
 // return jsgraph_;
  ...
  void InitializeFrameData(OptimizedCompilationInfo* info, const CallDescriptor* call_descriptor) {
    DCHECK(!codegen_data_.has_value());
