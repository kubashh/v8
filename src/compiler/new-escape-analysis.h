...
class EffectGraphReducer {
 public:
  ...
  void AddRoot(Node* node) {
    if (state_.Get(node) == State::kUnvisited) {
 ...
 protected:
  virtual void Reduce(Node* node, Reduction* reduction) = 0;
  virtual ~EffectGraphReducer() = default;
...
// and phi nodes by {VariableStates}.
class Variable {
 public:
  ...
  class Scope : public VariableStates::Scope {
   public:
    ...
    const VirtualObject* GetVirtualObject(Node* node) {
      VirtualObject* vobject = reducer_->virtual_objects_[node];
    ...
    void SetReplacement(Node* replacement) {
      ...
      vobject_ =
          replacement ? reducer_->virtual_objects_[replacement] : nullptr;
    }

    void MarkForDeletion() { SetReplacement(reducer_->jsgraph()->Dead()); }
...
class V8_EXPORT_PRIVATE NewEscapeAnalysis final
 ...
 public:
  ...
  void Reduce(Node* node, Reduction* reduction) override;
};
