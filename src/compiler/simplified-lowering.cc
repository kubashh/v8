...
// static methods
bool SimplifiedLowering::NodeProperties::IsControlEdge(Edge edge) {
  return compiler::NodeProperties::IsControlEdge(edge);
}
bool SimplifiedLowering::NodeProperties::IsEffectEdge(Edge edge) {
  return compiler::NodeProperties::IsEffectEdge(edge);
}
bool SimplifiedLowering::NodeProperties::IsExceptionalCall(
    Node* node, Node** out_exception) {
  return compiler::NodeProperties::IsExceptionalCall(node, out_exception);
}
void SimplifiedLowering::NodeProperties::ReplaceControlInput(Node* node,
                                                             Node* control,
                                                             int index) {
  return compiler::NodeProperties::ReplaceControlInput(node, control, index);
}
void SimplifiedLowering::NodeProperties::ReplaceEffectInput(Node* node,
                                                            Node* control,
                                                            int index) {
  return compiler::NodeProperties::ReplaceEffectInput(node, control, index);
}
...
class RepresentationSelector {
  ...
  // by code in RepresentationSelector and SimplifiedLowering. In particular,
  // RepresentationSelector code should call RepresentationSelector::ChangeOp in
  // place of NodeProperies::ChangeOp, in order to notify the changes to
  // registered ObserveNodeManager and support the %ObserveNode intrinsic.
  // Therefore, a static method ChangeOp() should not be declared in this class.
  class NodeProperties {
   public:
    static Node* FindSuccessfulControlProjection(Node* node) {
      return compiler::NodeProperties::FindSuccessfulControlProjection(node);
    }
    static int FirstContextIndex(Node* node) {
      return compiler::NodeProperties::FirstContextIndex(node);
    }
    static int FirstControlIndex(Node* node) {
      return compiler::NodeProperties::FirstControlIndex(node);
    }
    static int FirstEffectIndex(Node* node) {
      return compiler::NodeProperties::FirstEffectIndex(node);
    }
    static Node* GetControlInput(Node* node, int index = 0) {
      return compiler::NodeProperties::GetControlInput(node, index);
    }
    static Node* GetEffectInput(Node* node, int index = 0) {
      return compiler::NodeProperties::GetEffectInput(node, index);
    }
    static Type GetType(Node* node) {
      return compiler::NodeProperties::GetType(node);
    }
    static Node* GetValueInput(Node* node, int index) {
      return compiler::NodeProperties::GetValueInput(node, index);
    }
    static bool IsEffectEdge(Edge edge) {
      return compiler::NodeProperties::IsEffectEdge(edge);
    }
    static bool IsTyped(Node* node) {
      return compiler::NodeProperties::IsTyped(node);
    }
    static int PastContextIndex(Node* node) {
      return compiler::NodeProperties::PastContextIndex(node);
    }
    static int PastValueIndex(Node* node) {
      return compiler::NodeProperties::PastValueIndex(node);
    }
  };
...
#undef OPCODE_CASE
 ...
 private:
  ...
  void NotifyNodeChanged(Node* node) {
