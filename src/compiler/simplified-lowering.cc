...
// Classes used to instantiate a particular phase, for template purposes.
struct PROPAGATE_PHASE;
struct RETYPE_PHASE;
struct LOWER_PHASE;
 ...
 public:
  ...
  void RunTypePropagationPhase() {
    ...
    while (!typing_stack_.empty()) {
      ...
      VisitNode<RETYPE_PHASE>(node, info->truncation(), nullptr);
      TRACE("  ==> output ");
...
#undef DECLARE_CASE
  ...
  void RunTruncationPropagationPhase() {
    ...
    while (!queue_.empty()) {
      ...
      VisitNode<PROPAGATE_PHASE>(node, info->truncation(), nullptr);
    }
  }
  ...
  void Run(SimplifiedLowering* lowering) {
    ...
    for (NodeVector::iterator i = nodes_.begin(); i != nodes_.end(); ++i) {
      ...
      NodeOriginTable::Scope origin_scope(node_origins_, "simplified lowering",
                                          node);
      VisitNode<LOWER_PHASE>(node, info->truncation(), lowering);
    }
  ...
  template <>
  void SetOutput<PROPAGATE_PHASE>(Node* node,
  ...
  template <>
  void SetOutput<RETYPE_PHASE>(Node* node, MachineRepresentation representation,
                               Type restriction_type) {
    NodeInfo* const info = GetInfo(node);
  ...
  template <>
  void SetOutput<LOWER_PHASE>(Node* node, MachineRepresentation representation,
                              Type restriction_type) {
    NodeInfo* const info = GetInfo(node);
  ...
  template <>
  void ProcessInput<PROPAGATE_PHASE>(Node* node, int index, UseInfo use) {
  ...
  template <>
  void ProcessInput<RETYPE_PHASE>(Node* node, int index, UseInfo use) {
    DCHECK_IMPLIES(use.type_check() != TypeCheckKind::kNone,
                   !node->op()->HasProperty(Operator::kNoDeopt) &&
                       node->op()->EffectInputCount() > 0);
  }
  ...
  template <>
  void ProcessInput<LOWER_PHASE>(Node* node, int index, UseInfo use) {
    DCHECK_IMPLIES(use.type_check() != TypeCheckKind::kNone,
                   !node->op()->HasProperty(Operator::kNoDeopt) &&
                       node->op()->EffectInputCount() > 0);
