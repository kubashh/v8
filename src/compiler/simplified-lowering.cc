...
#undef DECLARE_CASE
  ...
  template <>
  void EnqueueInput<PROPAGATE>(Node* use_node, int index, UseInfo use_info) {
  ...
  template <>
  void SetOutput<PROPAGATE>(Node* node, MachineRepresentation representation,
  ...
  template <>
  void SetOutput<RETYPE>(Node* node, MachineRepresentation representation,
                         Type restriction_type) {
    NodeInfo* const info = GetInfo(node);
  ...
  template <>
  void SetOutput<LOWER>(Node* node, MachineRepresentation representation,
                        Type restriction_type) {
    NodeInfo* const info = GetInfo(node);
  ...
  template <>
  void ProcessInput<PROPAGATE>(Node* node, int index, UseInfo use) {
  ...
  template <>
  void ProcessInput<RETYPE>(Node* node, int index, UseInfo use) {
    DCHECK_IMPLIES(use.type_check() != TypeCheckKind::kNone,
                   !node->op()->HasProperty(Operator::kNoDeopt) &&
                       node->op()->EffectInputCount() > 0);
  }
  ...
  template <>
  void ProcessInput<LOWER>(Node* node, int index, UseInfo use) {
    DCHECK_IMPLIES(use.type_check() != TypeCheckKind::kNone,
                   !node->op()->HasProperty(Operator::kNoDeopt) &&
                       node->op()->EffectInputCount() > 0);
  ...
  template <>
  void ProcessRemainingInputs<PROPAGATE>(Node* node, int index) {
    DCHECK_GE(index, NodeProperties::PastValueIndex(node));
    DCHECK_GE(index, NodeProperties::PastContextIndex(node));
  ...
  void MarkAsPossibleRevisit(Node* node, Node* input) {
    DCHECK(retype<T>());
    auto it = might_need_revisit.find(input);
    if (it == might_need_revisit.end()) {
      it = might_need_revisit.insert({input, ZoneVector<Node*>(zone())}).first;
    }
  ...
  template <>
  void VisitInputs<PROPAGATE>(Node* node) {
  ...
  template <>
  void VisitInputs<RETYPE>(Node* node) {}

  template <>
  void VisitInputs<LOWER>(Node* node) {
    int tagged_count = node->op()->ValueInputCount() +
                       OperatorProperties::GetContextInputCount(node->op()) +
                       OperatorProperties::GetFrameStateInputCount(node->op());
    // Visit value, context and frame state inputs as tagged.
  ...
  template <>
  void InsertUnreachableIfNecessary<LOWER>(Node* node) {
  ...
  void VisitCheckBounds(Node* node, SimplifiedLowering* lowering) {
    ...
    if (length_type.Is(Type::Unsigned31())) {
      if (index_type.Is(Type::Integral32OrMinusZero())) {
        ...
        VisitBinop<T>(node, UseInfo::CheckedTaggedAsArrayIndex(p.feedback()),
      ...
      VisitBinop<T>(
          node, UseInfo::CheckedSigned64AsWord64(kIdentifyZeros, p.feedback()),
...
#undef OPCODE_CASE
 ...
 private:
  ...
  ZoneMap<Node*, ZoneVector<Node*>> might_need_revisit;
