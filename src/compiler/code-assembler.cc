...
#define ATOMIC_FUNCTION(name)                                                 \
  TNode<Word32T> CodeAssembler::Atomic##name(                                 \
    ...
    return UncheckedCast<Word32T>(                                            \
        raw_assembler()->Atomic##name(type, base, offset, value, nullptr));   \
  }                                                                           \
  template <class Type>                                                       \
  TNode<Type> CodeAssembler::Atomic##name##64(                                \
    ...
    return UncheckedCast<Type>(raw_assembler()->Atomic##name(                 \
        MachineType::Uint64(), base, offset, value, value_high));             \
  }                                                                           \
...
TNode<Word32T> CodeAssembler::AtomicCompareExchange(MachineType type,
  ...
  return UncheckedCast<Word32T>(raw_assembler()->AtomicCompareExchange(
      type, base, offset, old_value, nullptr, new_value, nullptr));
}
...
TNode<Type> CodeAssembler::AtomicCompareExchange64(
  ...
  return UncheckedCast<Type>(raw_assembler()->AtomicCompareExchange(
      MachineType::Uint64(), base, offset, old_value, old_value_high, new_value,
      new_value_high));
}
