...
Node** SimdScalarLowering::GetReplacementsWithType(Node* node, SimdType type) {
  ...
  if (type == SimdType::kInt32x4) {
    ...
    if (ReplacementType(node) == SimdType::kFloat64x2) {
      Node** float_to_int64 = zone()->NewArray<Node*>(kNumLanes64);
      Node** float_to_int32 = zone()->NewArray<Node*>(kNumLanes32);
      Float64ToInt64(replacements, float_to_int64);
      Int64ToInt32(float_to_int64, float_to_int32);
      Int32ToFloat32(float_to_int32, result);
