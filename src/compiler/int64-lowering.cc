  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kCall: {
      ...
      if (returns_require_lowering) {
        size_t arity = descriptor->ReturnCount();
        if (arity == 1) {
          ...
          ZoneVector<Node*> projections(arity, zone());
          NodeProperties::CollectValueProjections(node, projections.data(),
                                                  arity);
          for (size_t old_index = 0, new_index = 0; old_index < arity;
               ++old_index, ++new_index) {
            Node* use_node = projections[old_index];
            DCHECK_EQ(ProjectionIndexOf(use_node->op()), old_index);
            DCHECK_EQ(GetReturnIndexAfterLowering(descriptor, old_index),
                      new_index);
