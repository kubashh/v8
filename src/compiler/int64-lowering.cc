...
void Int64Lowering::LowerNode(Node* node) {
  switch (node->opcode()) {
    ...
    case IrOpcode::kLoopExitValue: {
      ...
      if (rep == MachineRepresentation::kWord64) {
        Node* low_node = graph()->NewNode(common()->LoopExitValue(rep),
                                          GetReplacementLow(node->InputAt(0)),
                                          node->InputAt(1));
        Node* high_node = graph()->NewNode(common()->LoopExitValue(rep),
                                           GetReplacementHigh(node->InputAt(0)),
                                           node->InputAt(1));
        ReplaceNode(node, low_node, high_node);
      } else {
        DefaultLowering(node);
