...
Int64Lowering::Int64Lowering(
    ...
    : zone_(zone),
      ...
      int32_type_(Type::None()),
      float64_type_(Type::None()) {
...
void Int64Lowering::SetInt32Type(Node* node) {
  if (int32_type_.IsNone()) {
    int32_type_ = Type::Wasm({wasm::kWasmI32, nullptr}, graph_->zone());
  }
  NodeProperties::SetType(node, int32_type_);
}

void Int64Lowering::SetFloat64Type(Node* node) {
  if (float64_type_.IsNone()) {
    float64_type_ = Type::Wasm({wasm::kWasmF64, nullptr}, graph_->zone());
  }
