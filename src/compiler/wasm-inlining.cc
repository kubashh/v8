  ...
  for (Node* const input : callee_end->inputs()) {
    ...
    switch (input->opcode()) {
      ...
      case IrOpcode::kTailCall: {
        ...
        NodeVector projections(return_arity + 3, zone());
        // The first input of a return node is always the 0 constant.
        projections[0] = graph()->NewNode(common()->Int32Constant(0));
        if (return_arity == 1) {
          projections[1] = input;
        ...
        projections[return_arity + 1] = input;  // effect
        projections[return_arity + 2] = input;  // control
        Node* ret = graph()->NewNode(common()->Return(return_arity),
                                     return_arity + 3, projections.data());
