...
class JsonParser final {
 ...
 private:
  // A literal buffer is passed if the chars could move on GC.
  template <typename LiteralChar>
  Handle<String> InternalizeString(bool requires_internalization,
                                   const Vector<const LiteralChar>& chars,
                                   LiteralBuffer* literal_buffer = nullptr);
 ...
 private:
  ...
  bool is_at_end() const { return cursor_ >= end_; }
