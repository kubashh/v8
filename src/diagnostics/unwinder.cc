...
bool PCIsInCodePages(size_t code_pages_length, MemoryRange* code_pages,
                     void* pc) {
  for (size_t i = 0; i < code_pages_length; i++) {
    const MemoryRange& range = code_pages[i];
    if (PCIsInCodeRange(range, pc)) return true;
  }
  return false;
}
...
bool Unwinder::PCIsInV8(size_t code_pages_length, MemoryRange* code_pages,
                        void* pc) {
