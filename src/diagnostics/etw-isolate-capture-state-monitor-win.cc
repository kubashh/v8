...
bool EtwIsolateCaptureStateMonitor::WaitFor(const base::TimeDelta& delta) {
  ...
  DBGOUT << "Waiting for " << isolate_count_ << " isolates for up to "
         << remaining.InMilliseconds() << std::endl;
  while (cv_.WaitFor(mutex_, remaining)) {
    DBGOUT << "WaitFor woke up: " << isolate_count_ << " isolates remaining "
           << std::endl;
    ...
    if (elapsed >= remaining) {
      DBGOUT << "Elapsed is " << elapsed.InMilliseconds()
    ...
    remaining -= elapsed;
    DBGOUT << "New remaining " << remaining.InMilliseconds()
...
void EtwIsolateCaptureStateMonitor::Notify() {
  {
    DBGOUT << "Notify taking mutex" << std::endl;
    base::MutexGuard lock(mutex_);
    isolate_count_--;
    DBGOUT << "Got mutex and isolate count reduced to " << isolate_count_
           << std::endl;
  }
  DBGOUT << "Released mutex preparing to notifyOne " << std::endl;
  cv_.NotifyOne();
  DBGOUT << "Finished notifyOne " << std::endl;
}
