...
bool WebSnapshotSerializerDeserializer::IsConstructor(uint32_t flags) {
  uint32_t mask = (1 << (DerivedConstructorBitField::kShift + 1)) -
                  (1 << ClassConstructorBitField::kShift);
...
PropertyAttributes WebSnapshotSerializerDeserializer::FlagsToAttributes(
  ...
  uint32_t attributes =
      ...
      (!EnumerableBitField::decode(flags) ? DONT_ENUM : NONE);
...
void WebSnapshotSerializer::SerializeMap(Handle<Map> map, uint32_t& id) {
  ...
  bool has_custom_property_attributes = false;
  ...
  map_serializer_.WriteUint32(!has_custom_property_attributes
...
void WebSnapshotDeserializer::DeserializeClasses() {
  ...
  shared_function_infos_ = shared_function_infos_->EnsureSpace(
      isolate_, class_count_, AllocationType::kYoung);
  script_->set_shared_function_infos(*shared_function_infos_);
