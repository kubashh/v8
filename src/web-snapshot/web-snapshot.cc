...
uint32_t WebSnapshotSerializerDeserializer::AttributesToFlags(
  ...
  auto thing = ReadOnlyBitField::encode(details.IsReadOnly()) +
               ConfigurableBitField::encode(details.IsConfigurable()) +
               EnumberableBitField::encode(details.IsEnumerable());
  return thing;
}
...
PropertyAttributes WebSnapshotSerializerDeserializer::FlagsToAttributes(
    uint32_t flags) {
  uint32_t attributes =
      (ReadOnlyBitField::decode(flags) ? READ_ONLY : NONE) |
      (!ConfigurableBitField::decode(flags) ? DONT_DELETE : NONE) |
      (!EnumberableBitField::decode(flags) ? DONT_ENUM : NONE);
...
void WebSnapshotSerializer::SerializeMap(Handle<Map> map, uint32_t& id) {
  ...
  map_serializer_.WriteUint32(!has_attributes ? MapType::SIMPLE
                                              : MapType::WITH_ATTRIBUTES);
  map_serializer_.WriteUint32(static_cast<uint32_t>(string_ids.size()));
  for (size_t i = 0; i < string_ids.size(); ++i) {
    if (has_attributes) {
      map_serializer_.WriteUint32(attributes[i]);
    }
    ...
    for (uint32_t p = 0; p < property_count; ++p) {
      ...
      if (has_attributes) {
        ...
        if (!deserializer_->ReadUint32(&flags)) {
          Throw("Web snahspot: Malformed shape");
          return;
        }
