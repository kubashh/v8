...
// - 0 if the symbol is not-global and there's no description, 1 if the symbol
...
void WebSnapshotSerializer::SerializeSymbol(Handle<Symbol> symbol) {
  if (symbol->description().IsUndefined()) {
    ...
    symbol_serializer_.WriteUint32(0);
  } else {
    symbol_serializer_.WriteUint32(symbol->is_in_public_symbol_table() ? 2 : 1);
...
void WebSnapshotDeserializer::DeserializeSymbols() {
  ...
  for (uint32_t i = 0; i < symbol_count_; ++i) {
    ...
    if (symbol_type == 0) {  // Symbol without description
