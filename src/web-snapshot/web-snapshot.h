...
class WebSnapshotSerializerDeserializer {
 public:
  ...
  enum MapType : uint8_t { SIMPLE, WITH_ATTRIBUTES };

  bool IsFunction(uint32_t flags);
 ...
 private:
  ...
  using ConstructorBitField = StaticBitField::Next<bool, 1>;
  ...
  using EnumberableBitField = ConfigurableBitField::Next<bool, 1>;
};
...
class V8_EXPORT WebSnapshotDeserializer
 ...
 private:
  ...
  MaybeHandle<ScopeInfo> CreateScopeInfo(uint32_t variable_count,
                                         bool has_parent,
                                         ContextType context_type);
  Handle<JSFunction> StoreFunction(int index, uint32_t start, uint32_t length,
