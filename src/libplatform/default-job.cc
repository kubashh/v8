  ...
  for (bool first_iteration = true;; first_iteration = false) {
    size_t num_tasks_to_post = 0;
    {
      ...
      if (first_iteration) {
        ...
        // Reserve a worker for the joining thread. GetMaxConcurrency() is
        // ignored here, but if necessary we wait below for workers to return so
        // we don't exceed GetMaxConcurrency().
      ...
      // The job is done once max_concurrency drops to zero.
      ...
      size_t missing_tasks = max_concurrency - active_workers_;
      num_tasks_to_post =
          pending_tasks_ >= missing_tasks ? 0 : missing_tasks - pending_tasks_;
      pending_tasks_ += num_tasks_to_post;
    }
