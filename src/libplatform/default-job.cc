...
void DefaultJobState::Join() {
  ...
  auto WaitForParticipationOpportunity = [this, &max_concurrency]() -> bool {
  ...
  size_t num_tasks_to_post;
  {
    ...
    ++num_worker_threads_;
    ++active_workers_;
    if (!WaitForParticipationOpportunity()) return;
    // Compute the number of additional worker tasks to spawn.
    DCHECK_LE(active_workers_, max_concurrency);
