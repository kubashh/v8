...
constexpr auto Header(size_t size) {
  const char header_bytes[3] = {static_cast<char>(size & 0xFF),
                                static_cast<char>(size >> 8 & 0xFF), '\0'};
...
constexpr auto EventDescriptor(const tracing::EventInfo& info) {
  return EVENT_DESCRIPTOR{info.id,
                          0,  // Version
...
void SetMetaDescriptors(EVENT_DATA_DESCRIPTOR* data_descriptor,
                        const char* traits, const void* metadata, size_t size) {
...
void LogEventData(const tracing::ProviderState& state,
  ...
  SetMetaDescriptors(descriptors, state.provider_trait, meta->bytes,
                     meta->size);

  EVENT_DATA_DESCRIPTOR* data_descriptors = descriptors + 2;
  SetFieldDescriptors(data_descriptors, fields...);

  LogEvent(state.traceProvider->RegHandle, event_descriptor, descriptors,
           descriptor_count);
}
