...
std::unique_ptr<Task> TaskQueue::GetNext() {
  for (;;) {
    ...
    if (terminated_) {
      printf("terminate in loop notify all\n");
      process_queue_condition_var_.NotifyAll();
      printf("terminate in loop notify all done\n");
      return nullptr;
    }
    ...
    if (task_queue_.empty() && !delayed_task_queue_.empty()) {
      ...
      printf("WaitFor ms = %ld\n", wait_time.InMilliseconds());
      bool notified = process_queue_condition_var_.WaitFor(&lock_, wait_time);
      USE(notified);
      printf("WaitFor notified: %d\n", notified);
    } else {
      printf("Wait in loop\n");
