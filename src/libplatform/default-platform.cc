...
void DeinitIsolate(v8::Platform* platform, Isolate* isolate) {
  static_cast<DefaultPlatform*>(platform)->DeinitIsolate(isolate);
}
...
void DefaultPlatform::DeinitIsolate(Isolate* isolate) {
  base::MutexGuard guard(&lock_);
  auto it = foreground_task_runner_map_.find(isolate);
