...
void AsyncHooksWrap::Enable() { enabled = true; }

void AsyncHooksWrap::Disable() { enabled = false; }
...
void AsyncHooks::ShellPromiseHook(PromiseHookType type, Local<Promise> promise,
  ...
  if (type == PromiseHookType::kInit) {
    ...
    AsyncContext ctx;
    ctx.execution_async_id = static_cast<int>(
        promise
            ->GetPrivate(hooks->isolate_->GetCurrentContext(),
                         hooks->async_id_smb.Get(hooks->isolate_))
            .ToLocalChecked()
            .As<Integer>()
            ->Value());
    ctx.trigger_async_id = static_cast<int>(
        promise
            ->GetPrivate(hooks->isolate_->GetCurrentContext(),
                         hooks->trigger_id_smb.Get(hooks->isolate_))
            .ToLocalChecked()
