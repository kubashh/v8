...
class JsonString : public JsonValue {
 public:
  ...
  explicit JsonString(Isolate* isolate)
      : JsonValue(JsonValue::Kind::STRING), is_one_byte_(true), map_(nullptr) {
  ...
  uint32_t AsArrayIndex() const;

  uint32_t hash() const { return hash_; }

  template <typename char_type>
  bool Matches(const Vector<const char_type>& chars) {
    if ((sizeof(char_type) == 1) != is_one_byte_) return false;
    Vector<const uint8_t> bytes = Vector<const uint8_t>::cast(chars);
    if (bytes_.length() != bytes.length()) return false;
    bool result = memcmp(bytes.start(), bytes_.start(), bytes.length()) == 0;
    return result;
  }
  ...
  void set_map(JsonMap* map) {
    DCHECK_NULL(map_);
    map_ = map;
  }
  void clear_map() {
    DCHECK_NOT_NULL(map_);
    map_ = nullptr;
  }

  JsonValue* value() const { return value_; }
 ...
 private:
  ...
  // Used during object internalization to quickly find duplicate properties.
  union {
    JsonMap* map_;
    JsonValue* value_;
  };
...
class JsonNumber : public JsonValue {
 public:
  explicit JsonNumber(double d)
      : JsonValue(JsonValue::Kind::NUMBER), is_smi_(false), d_(d) {}
  explicit JsonNumber(int32_t i)
      : JsonValue(JsonValue::Kind::NUMBER), is_smi_(true), i_(i) {
    DCHECK(Smi::IsValid(i_));
  }
...
class JsonMap : public ZoneObject {
 public:
  JsonMap(JsonMap* parent, JsonString* property)
      : parent_(parent),
        property_(property),
        is_dictionary_map_(false),
        has_slow_elements_(false),
  ...
  // |parent| is the last non-dictionary-mode map, describing the properties up
  // to the point where we go dictionary-mode.
  JsonMap(Zone* zone, JsonMap* parent, JsonString* property)
      : parent_(parent),
        properties_(new (zone) ZonePtrList<JsonString>(4, zone)),
        is_dictionary_map_(true),
        has_slow_elements_(false),
        number_of_properties_(0),
        hash_(0) {
    AddProperty(property, zone);
  }

  uint32_t number_of_properties() const { return number_of_properties_; }
  uint32_t hash() const { return hash_; }
  bool is_dictionary_map() const { return is_dictionary_map_; }
  bool has_slow_elements() const { return has_slow_elements_; }

  JsonString* property() const {
    DCHECK(!is_dictionary_map_);
    return property_;
  }

  const ZonePtrList<JsonString>* properties() const {
    DCHECK(is_dictionary_map_);
    return properties_;
  }

  void AddProperty(JsonString* property, Zone* zone) {
    DCHECK(is_dictionary_map_);
    properties_->Add(property, zone);
    ++number_of_properties_;
  }

  JsonMap* field_type() const { return field_type_; }
  JsonMap* parent() const { return parent_; }
  ...
  // Caches a unique outgoing transition, |nullptr| if no transition or multiple
  // transitions were seen.
  void set_transition(JsonMap* transition) {
    DCHECK_NE(transition, transition_);
    transition_ = seen_transition_ ? nullptr : transition;
    seen_transition_ = true;
  }

  // Destroys the transition link. Only to be used during internalization.
  void reset_transition(JsonMap* transition) { transition_ = transition; }

  JsonMap* TransitionToSlowElements(Zone* zone) {
    if (slow_elements_map_ == nullptr) {
      slow_elements_map_ = new (zone) JsonMap(this);
    }
    return slow_elements_map_;
  }

 private:
  explicit JsonMap(JsonMap* parent)
      : parent_(parent),
        property_(nullptr),
        is_dictionary_map_(parent->is_dictionary_map_),
        has_slow_elements_(true),
        number_of_properties_(parent->number_of_properties_),
        hash_(0) {}
  ...
  union {
    ...
    ZonePtrList<JsonString>* properties_;
  };
  // TODO(verwaest): Pack?
  JsonMap* field_type_ = nullptr;
  JsonMap* transition_ = nullptr;
  JsonMap* slow_elements_map_ = nullptr;
  Representation representation_ = Representation::None();
  bool seen_transition_ = false;
  const bool is_dictionary_map_;
  const bool has_slow_elements_;
...
class JsonArray : public JsonValue {
 public:
  JsonArray(Zone* zone, ElementsKind kind,
            const ScopedPtrList<JsonValue>& elements)
      : JsonValue(JsonValue::Kind::ARRAY), kind_(kind), elements_(0, zone) {
    elements.CopyTo(&elements_, zone);
  }
...
class JsonParser2 {
 ...
 private:
  ...
  JsonNumber* ScanNumber(int sign, char_type* start);
  JsonString* ScanString(JsonString* hint = nullptr);
