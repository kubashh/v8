...
JsonObject* JsonParser2<char_type>::ParseObject() {
  ...
  if (ShouldConvertToSlowElements(elements.length() / 2, max_index + 1)) {
  ...
  while (map != last) {
    ...
    if (property->map() != nullptr) {
      ...
      // JsonMap* field_type_ = map->field_type();
      {
        ...
        TransitionsAccessor transitions(isolate, *object_, &no_gc);
        Handle<String> key = transitions.ExpectedTransitionKey();
        if (key.is_identical_to(property->object())) {
          object_ = transitions.ExpectedTransitionTarget();
          continue;
        }
      }
...
class JsonStringInternalizationKey : public StringTableKey {
 public:
  explicit JsonStringInternalizationKey(const Vector<const uint8_t>& bytes,
                                        int hash)
      : StringTableKey(hash), bytes_(bytes) {}

  bool IsMatch(Object other) override {
    if (sizeof(Char) == 1) {
      return String::cast(other).IsOneByteEqualTo(bytes_);
    }
    return String::cast(other).IsTwoByteEqualTo(
        Vector<const uint16_t>::cast(bytes_));
  }

  Handle<String> AsHandle(Isolate* isolate) override {
    if (sizeof(Char) == 1) {
      return isolate->factory()->NewOneByteInternalizedString(bytes_,
                                                              HashField());
    }
    return isolate->factory()->NewTwoByteInternalizedString(
        Vector<const uint16_t>::cast(bytes_), HashField());
  }
...
void JsonString::Internalize(Isolate* isolate, AllocationType allocation) {
  if (is_one_byte_) {
    JsonStringInternalizationKey<uint8_t> key(bytes_, hash_);
    SetObject(StringTable::LookupKey(isolate, &key));
  } else {
    JsonStringInternalizationKey<uint16_t> key(bytes_, hash_);
    SetObject(StringTable::LookupKey(isolate, &key));
  }
}
