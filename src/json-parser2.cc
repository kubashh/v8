...
Handle<Map> JsonMap::Internalize(Isolate* isolate, AllocationType allocation,
                                 bool slow_elements) {
  Handle<Map>& cache = slow_elements ? object_ : slow_elements_object_;
  if (!cache.is_null()) return cache;
  cache = DoInternalize(isolate, allocation, slow_elements);
  return cache;
}

Handle<Map> JsonMap::DoInternalize(Isolate* isolate, AllocationType allocation,
                                   bool slow_elements) {
  if (V8_UNLIKELY(is_dictionary_map())) {
    ...
    Handle<Map> result = isolate->slow_object_with_object_prototype_map();
    if (slow_elements) {
      return Map::AsElementsKind(isolate, result, DICTIONARY_ELEMENTS);
    }
    return result;
  }
  ...
  Handle<Map> result = isolate->factory()->ObjectLiteralMapFromCache(
      isolate->native_context(), number_of_properties_ - duplicates);
  if (slow_elements) {
    result = Map::AsElementsKind(isolate, result, DICTIONARY_ELEMENTS);
  }
  ...
  while (map != this) {
    ...
    if (property->map() != nullptr) {
      ...
      result = Map::TransitionToDataProperty(
          isolate, result, property->object(),
...
void JsonObject::Internalize(Isolate* isolate, AllocationType allocation) {
  Handle<Map> heap_map =
      map()->Internalize(isolate, allocation, slow_elements_);
  ...
  if (V8_UNLIKELY(map()->is_dictionary_map())) {
    ...
    JsonMap* map;
    for (map = this->map(); map->parent() != nullptr; map = map->parent()) {
    ...
    uint32_t i = 0;
    for (map = map->expected_transition(); map != this->map();
    ...
    const ZonePtrList<JsonString>* properties = this->map()->properties();
