...
Handle<JSFunction> ApiNatives::CreateApiFunction(
  ...
  if (!shared->has_shared_name()) {
    Handle<Name> name;
    if (maybe_name.ToHandle(&name)) {
      JSFunction::SetName(result, name, isolate->factory()->empty_string());
    }
  }
  ...
  if (obj->remove_prototype()) {
    if (shared->has_shared_name()) {
      ...
      result->set_map(
          *isolate->sloppy_function_with_name_and_without_prototype_map());
    }
  ...
  if (obj->read_only_prototype()) {
    if (shared->has_shared_name()) {
      ...
      result->set_map(
          *isolate->sloppy_function_with_name_and_readonly_prototype_map());
    }
  }
