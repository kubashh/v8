...
class V8_EXPORT_PRIVATE UnoptimizedCompilePerThreadState {
 public:
  UnoptimizedCompilePerThreadState() = default;
  explicit UnoptimizedCompilePerThreadState(Isolate* isolate);
  UnoptimizedCompilePerThreadState(uintptr_t stack_limit,
...
class V8_EXPORT_PRIVATE ParseInfo {
 public:
  ParseInfo(const UnoptimizedCompileFlags flags, UnoptimizedCompileState* state,
            UnoptimizedCompilePerThreadState per_thread_state);
  ...
  // Getters for per-thread state.
  uintptr_t stack_limit() const { return per_thread_state_.stack_limit(); }
  RuntimeCallStats* runtime_call_stats() const {
    return per_thread_state_.runtime_call_stats();
  }

  UnoptimizedCompilePerThreadState per_thread_state() const {
    return per_thread_state_;
  }
  void set_per_thread_state(UnoptimizedCompilePerThreadState per_thread_state) {
    per_thread_state_ = per_thread_state;
  }
 ...
 private:
  ...
  UnoptimizedCompilePerThreadState per_thread_state_;
