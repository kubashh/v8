...
v8::ScriptCompiler::CachedData* CompileHints::Serialize(
  ...
  // FIXME: Better encoding. Varint? also this is UB, no?
...
std::vector<int> CompileHints::Deserialize(const byte* data, int length) {
  ...
  result.reserve(length / sizeof(int));
  const int* raw_data = reinterpret_cast<const int*>(data);
  for (size_t i = 0; i < length / sizeof(int); ++i) {
    printf("deserialized compile hint: %d\n", raw_data[i]);
    result.push_back(raw_data[i]);
  }
  return result;
}
