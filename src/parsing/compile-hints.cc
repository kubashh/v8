...
std::vector<int> CompileHints::Deserialize(AlignedCachedData* cached_data) {
  // Discard the int64_t prefix.
  size_t length = cached_data->length();
  const byte* data = cached_data->data();
  constexpr size_t prefix_size = sizeof(int64_t);
  std::vector<int> result;

  if (length < prefix_size || (length - prefix_size) % sizeof(int) != 0) {
    cached_data->Reject();
    return result;
  }
