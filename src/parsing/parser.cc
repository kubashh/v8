...
Expression* Parser::RewriteClassLiteral(Scope* block_scope,
  ...
  ExpressionT result = class_literal;
  ...
  if (class_info->static_fields->length() > 0) {
    ...
    {
      // temp = class { .. };
      class_var = NewTemporary(ast_value_factory()->empty_string());
    ...
    FunctionLiteral* initializer;
    {
      ...
      ClassFields* class_fields = factory()->NewClassFields(
          class_info->static_fields, class_info->field_scope->NeedsHomeObject(),
          kNoSourcePosition);
      statements->Add(
          factory()->NewExpressionStatement(class_fields, kNoSourcePosition),
          zone());
      initializer = factory()->NewFunctionLiteral(
          impl()->EmptyIdentifierString(), class_info->field_scope, statements,
          class_info->static_fields->length(), 0, 0,
          FunctionLiteral::kNoDuplicateParameters,
