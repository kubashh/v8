...
Statement* Parser::BuildInitializationBlock(
  ...
  for (const auto& declaration : parsing_result->declarations) {
    DeclareVariablesAndInitialize(&statements, &(parsing_result->descriptor),
                                  &declaration, names);
  }
...
void Parser::RewriteCatchPattern(CatchInfo* catch_info) {
  ...
  if (catch_info->pattern != nullptr) {
    ...
    DeclareVariablesAndInitialize(&init_statements, &descriptor, &decl,
                                  &catch_info->bound_names);
    catch_info->init = factory()->NewBlock(true, init_statements);
  } else {
...
void Parser::DesugarBindingInForEachStatement(ForInfo* for_info,
  ...
  ScopedPtrList<Statement> each_initialization_statements(pointer_buffer());
  {
    ...
    DeclareVariablesAndInitialize(
        &each_initialization_statements, &descriptor, &decl,
...
Block* Parser::BuildParameterInitializationBlock(
  ...
  for (auto parameter : parameters.params) {
    ...
    DeclareVariablesAndInitialize(param_init_statements, &descriptor, &decl,
                                  nullptr);
