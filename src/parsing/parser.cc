...
FunctionLiteral* Parser::ParseProgram(Isolate* isolate, ParseInfo* info) {
  ...
  if (FLAG_trace_parse && result != nullptr) {
    ...
    const char* event_name =
        info->is_eval() ? "parse-eval" : "parse-script";
    Script* script = *info->script();
    LOG(script->GetIsolate(), FunctionEvent(event_name, script, ms));
  }
...
FunctionLiteral* Parser::ParseFunction(Isolate* isolate, ParseInfo* info,
  ...
  if (FLAG_trace_parse && result != NULL) {
    ...
    LOG(script->GetIsolate(),
        FunctionEvent("parse-function", script, ms,
...

  {
    ...
    if (V8_UNLIKELY(FLAG_trace_preparse)) {
      ...
      const char* reason = should_preparse
                               ...
                               : "parse-full";
      LOG(Isolate::Current(),
          FunctionEvent(reason, nullptr, ms, scope->start_position(),
                        scope->end_position(), function_name->raw_data(),
