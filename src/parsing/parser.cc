...
void Parser::ParseAndRewriteGeneratorFunctionBody(int pos, FunctionKind kind,
  ...
  // - %AsyncGeneratorResolved is produced by BytecodeGenerator via
  //   execution_control()->AsyncGeneratorReturnAccumulator(), and is not
...
Expression* Parser::RewriteYieldStar(Expression* generator,
  ...
  Statement* maybe_return_value;
  {
    ...
    Expression* property = factory()->NewProperty(output_proxy, literal, nopos);
    Expression* result =
        is_async_generator() ? property : BuildIteratorResult(property, true);
    Statement* return_value = BuildReturnStatement(result, nopos);
...
void Parser::BuildIteratorClose(ZoneList<Statement*>* statements,
  ...
  Statement* check_return;
  {
    ...
    Expression* result =
        is_async_generator() ? value : BuildIteratorResult(value, true);
    Statement* return_input = BuildReturnStatement(result, nopos);
