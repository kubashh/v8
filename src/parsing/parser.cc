...
Expression* Parser::RewriteClassLiteral(Scope* block_scope,
  ...
  if (class_info->static_fields->length() > 0) {
    // We need a class variable to use as the receiver for the
    // initializer function.
    if (class_info->variable == nullptr) {
      DeclareClassVariable(ast_value_factory()->empty_string(), class_info,
                           kNoSourcePosition, ok);
      class_info->variable->set_initializer_position(end_pos);
    }
