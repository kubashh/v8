...
class ExpressionScope {
 ...
 private:
  ...
  bool IsArrowHeadParsingScope() const {
    return IsInRange(type_, kArrowParameterDeclaration,
                     kAsyncArrowParameterDeclaration);
  }
...
class ExpressionParsingScope : public ExpressionScope<Types> {
 public:
  typedef typename Types::Impl ParserT;
  typedef typename Types::Expression ExpressionT;
  typedef class ExpressionScope<Types> ExpressionScope;
  typedef typename ExpressionScope::ScopeType ScopeType;

  ExpressionParsingScope(ParserT* parser,
                         ScopeType type = ExpressionScope::kExpression)
      : ExpressionScope(parser, type),
        has_async_arrow_in_scope_chain_(
            type == ExpressionScope::kAsyncArrowParameterDeclaration ||
  ...
  void RecordAsyncArrowParametersError(const Scanner::Location& loc,
    ...
    for (ExpressionScope* scope = this; scope != nullptr;
      ...
      if (scope->type_ == ExpressionScope::kAsyncArrowParameterDeclaration) {
        scope->AsArrowHeadParsingScope()->RecordDeclarationError(loc, message);
      }
    }
  }
  ...
  void ValidatePattern(ExpressionT expression, int begin, int end) {
    ...
    if (expression->is_parenthesized()) {
      ExpressionScope::Report(Scanner::Location(begin, end),
 ...
 private:
  ...
  void Report(int index) const {
    ExpressionScope::Report(locations_[index], messages_[index]);
  }
...
// in the same scope. It provides a clean error state in the underlying scope
// to parse the individual expressions, while keeping track of the expression
// and pattern errors since the start. The AccumulationScope is only used to
// keep track of the errors so far, and the underlying ExpressionScope keeps
// being used as the expression_scope(). If the expression_scope() isn't
...
class AccumulationScope {
 public:
  ...
  explicit AccumulationScope(ExpressionScope* scope) : scope_(nullptr) {
 ...
 public:
  ...
  void ValidateDeclaration() {
    ...
    if (declaration_error_location.IsValid()) {
      ExpressionScope::Report(declaration_error_location,
  ...
  bool is_simple_parameter_list() const { return is_simple_parameter_list_; }

  void RecordNonSimpleParameter() { is_simple_parameter_list_ = false; }
