...
//
// One of the specific subclasses needs to be used.
template <typename Types>
...
class AccumulationScope {
 public:
  typedef typename Types::Impl ParserT;
  explicit AccumulationScope(ParserT* parser) : scope_(nullptr) {
    if (parser->expression_scope()->CanBeExpression()) {
      scope_ = parser->expression_scope()->AsExpressionParsingScope();
      clear(0);
      clear(1);
      Accumulate();
    }
  ...
  void Accumulate() {
    if (scope_ == nullptr) return;
    DCHECK(!scope_->is_verified());
    accumulate(0, ExpressionParsingScope<Types>::kPatternIndex);
    accumulate(1, ExpressionParsingScope<Types>::kExpressionIndex);
  }

  ~AccumulationScope() {
    if (scope_ == nullptr) return;
    copy_back(ExpressionParsingScope<Types>::kPatternIndex, 0);
    copy_back(ExpressionParsingScope<Types>::kExpressionIndex, 1);
  }
...
class ArrowHeadParsingScope : public ExpressionParsingScope<Types> {
 public:
  ...
  ArrowHeadParsingScope(ParserT* parser, ScopeType type)
    ...
    DCHECK(!this->IsCertainlyDeclaration());
    this->clear(this->kDeclarationIndex);
  }

  void ValidateDeclaration() {
    DCHECK(!this->is_verified());
    if (!this->is_valid(this->kDeclarationIndex)) {
      this->Report(this->kDeclarationIndex);
    }
    if (!this->is_valid(this->kPatternIndex)) {
      this->Report(this->kPatternIndex);
    }
    this->mark_verified();
  }
  ...
  void RecordDeclarationError(const Scanner::Location& loc,
                              MessageTemplate message) {
    this->Record(this->kDeclarationIndex, loc, message);
  }
};
