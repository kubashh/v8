...
Maybe<JSRegExp::Flags> Scanner::ScanRegExpFlags() {
  DCHECK_EQ(Token::REGEXP_LITERAL, next().token);

  // Scan regular expression flags.
  JSRegExp::Flags flags = 0;
  while (IsIdentifierPart(c0_)) {
    JSRegExp::Flags flag = JSRegExp::FlagFromChar(c0_);
    if (flag == JSRegExp::kInvalid) return Nothing<JSRegExp::Flags>();
    if (flags & flag) return Nothing<JSRegExp::Flags>();
    Advance();
    flags |= flag;
  }
