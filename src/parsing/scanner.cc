...
Token::Value Scanner::Next() {
  if (next().token == Token::EOS) {
    next().location.beg_pos = current().location.beg_pos;
    next().location.end_pos = current().location.end_pos;
  }
  ...
  if (V8_LIKELY(next_next().token == Token::UNINITIALIZED)) {
    ...
    next_ = next_next_;
    next_next_ = previous;
    next_next().token = Token::UNINITIALIZED;
    next_next().contextual_token = Token::UNINITIALIZED;
