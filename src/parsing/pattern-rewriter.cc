...
class PatternRewriter final : public AstVisitor<PatternRewriter> {
 public:
  ...
  static void VisitDestructuringAssignment(Parser* parser,
...
void Parser::VisitDestructuringAssignment(RewritableExpression* to_rewrite) {
...
void PatternRewriter::VisitDestructuringAssignment(Parser* parser,
                                                   Assignment* assignment) {
  DCHECK(!parser->scope()->HasBeenRemoved());

  PatternRewriter rewriter(parser, ASSIGNMENT);
  rewriter.Visit(assignment);
}
...
void PatternRewriter::VisitVariableProxy(VariableProxy* proxy) {
  ...
  // TODO(leszeks): Check if we can remove this declaration scope logic and
  // compare var_init_scope against
  // target_scope/target_scope->GetDeclarationScope().
  Scope* declaration_scope = declares_parameter_containing_sloppy_eval_
                                 ? scope()->outer_scope()
  ...
  DCHECK_EQ(declaration_scope, var->scope());
