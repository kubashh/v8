...
class Utf16CharacterStream {
 public:
  ...
  V8_INLINE uc32 AdvanceUtil(FuncTy check, Args... args) {
    auto next_cursor_pos = std::find_if((buffer_cursor_++), buffer_end_,
                                        [&check, args...](uint16_t raw_c0_) {
                                          uc32 c0_ = static_cast<uc32>(raw_c0_);
                                          return check(c0_, args...);
                                        });
    if (next_cursor_pos == buffer_end_) {
      buffer_cursor_ = buffer_end_;
      if (ReadBlockChecked()) {
        return AdvanceUtil(check, args...);
      } else {
        buffer_cursor_++;
        return kEndOfInput;
      }
...
class Scanner {
 ...
 private:
  ...
  template <typename FuncTy, typename... Args>
  V8_INLINE void AdvanceUtil(FuncTy check, Args... args) {
    c0_ = source_->AdvanceUtil(check, std::forward<Args>(args)...);
  }
