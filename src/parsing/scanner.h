...
class Scanner {
 public:
  ...
  inline bool CurrentMatchesContextualEscaped(Token::Value token) const {
    ...
    // Conveniently, !current_->literal_chars.is_used() for all proper
    // keywords, so this second condition should exit early in common cases.
    return (current_->contextual_token == token) ||
           (current_->literal_chars.is_used() &&
  ...
  bool HasAnyLineTerminatorBeforeNext() const {
    return next_->after_line_terminator || next_->after_multiline_comment;
  }

  bool HasAnyLineTerminatorAfterNext() {
    Token::Value ensure_next_next = PeekAhead();
    USE(ensure_next_next);
    return next_next_->after_line_terminator;
  }
 ...
 private:
  ...
  inline void StartLiteral() { next_->literal_chars.Start(); }

  inline void StartRawLiteral() { next_->raw_literal_chars.Start(); }

  V8_INLINE void AddLiteralChar(uc32 c) { next_->literal_chars.AddChar(c); }

  V8_INLINE void AddLiteralChar(char c) { next_->literal_chars.AddChar(c); }
  ...
  inline void DropLiteral() {
    next_->literal_chars.Drop();
    next_->raw_literal_chars.Drop();
  }
  ...
  Vector<const uint8_t> literal_one_byte_string() const {
    if (current_->literal_chars.is_used())
  ...
  Vector<const uint8_t> raw_literal_one_byte_string() const {
    DCHECK(current_->raw_literal_chars.is_used());
