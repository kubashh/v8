...
#include "src/parsing/preparsed-scope-data-inl.h"
...
// Instantiate templated classes in cc file to avoid link errors in cctests.
template class ZoneConsumedPreParsedScopeData::ByteData;
template class OnHeapConsumedPreParsedScopeData::ByteData;
...
ZoneConsumedPreParsedScopeData::ZoneConsumedPreParsedScopeData(
    Zone* zone, ProducedPreParsedScopeData* data)
    : data_(data->Serialize(zone)), scope_data_wrapper_(data_->byte_data()) {
#ifdef DEBUG
  VerifyDataStart();
#endif
}
