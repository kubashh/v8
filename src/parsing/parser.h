...
struct ParserFormalParameters : FormalParametersBase {
  struct Parameter : public ZoneObject {
    Parameter(const AstRawString* name, Expression* pattern,
      ...
      name_and_is_rest.SetPointer(initializer);
      name_and_is_rest.SetStorage(is_rest);
    }

    const AstRawString* name;

    PointerWithStorageBits<Expression*, bool, 1> name_and_is_rest;

    Expression* pattern;
    Expression* initializer() const { return name_and_is_rest.GetPointer(); }
    int position;
    int initializer_end_position;
    inline bool is_rest() const { return name_and_is_rest.GetStorage(); }
