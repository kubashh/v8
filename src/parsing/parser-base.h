...
typename ParserBase<Impl>::ExpressionT ParserBase<Impl>::ParseBinaryExpression(
  ...
  DCHECK_GE(prec, 4);
  ExpressionT x;
  // "#foo in ShiftExpression" needs to be parsed separately, since private identifiers are not valid PrimaryExpressins.
  if (FLAG_harmony_private_brand_checks && peek() == Token::PRIVATE_NAME) {
