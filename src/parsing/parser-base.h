...
#undef ALLOW_ACCESSORS
 ...
 protected:
  ...
  inline static bool IsGetterOrSetter(PropertyKind kind) {
  ...
  *is_generator = Check(Token::MUL);
  ...
  if (token == Token::IDENTIFIER && !*is_generator && !*is_async) {
    ...
    if (!previous_token_was_name && Token::IsGetOrSet(contextual_token)) {
      // TODO(cbruni): use math to convert
  ...
  switch (token) {
    ...
    case Token::ELLIPSIS:
      if (!*is_generator && !*is_async && !IsGetterOrSetter(*kind)) {
        *name = impl()->NullIdentifier();
        Consume(Token::ELLIPSIS);
        expression = ParseAssignmentExpression(true, CHECK_OK);
...
void ParserBase<Impl>::ClassLiteralChecker::CheckClassMethodName(
    ...
    bool is_static, bool* ok) {
  DCHECK(type == PropertyKind::kMethodProperty || IsGetterOrSetter(type));
  ...
  if (is_static) {
    ...
    if (is_generator || is_async || IsGetterOrSetter(type)) {
