...
ParserBase<Impl>::ParsePrimaryExpression() {
  ...
  if (Token::IsAnyIdentifier(token)) {
    ...
    typename ExpressionScope::ScopeType arrow_type =
        ExpressionScope::kArrowParameterDeclaration;
    ...
    if (V8_UNLIKELY(token == Token::ASYNC &&
      ...
      if (peek_any_identifier() && PeekAhead() == Token::ARROW) {
        arrow_type = ExpressionScope::kAsyncArrowParameterDeclaration;
    ...
    if (V8_UNLIKELY(peek() == Token::ARROW)) {
      ArrowHeadParsingScope parsing_scope(impl(), arrow_type);
