...
class ParserBase {
 public:
  ...
  typedef typename v8::internal::DeclarationParsingScope<Types>
      DeclarationParsingScope;
...
#undef ALLOW_ACCESSORS
 ...
 protected:
  ...
  friend class v8::internal::AccumulationScope<ParserTypes<Impl>>;
  ...
  switch (token) {
    ...
    case Token::LPAREN: {
      ...
      ArrowHeadParsingScope maybe_arrow(
          impl(), ExpressionScope::kArrowParameterDeclaration);
      ...
      ExpressionT expr = ParseExpressionCoverGrammar(kMaybeArrowHead);
...
ParserBase<Impl>::ParseExpressionCoverGrammar(
  ...
  while (true) {
    ...
    if (maybe_arrow == kMaybeArrowHead && peek() == Token::RPAREN &&
...
ParserBase<Impl>::ParseAssignmentExpression() {
  ...
  if (V8_UNLIKELY(expression->IsPattern() && op == Token::ASSIGN)) {
    expression_scope()->ClassifyPattern(expression, lhs_beg_pos,
                                        end_position());
...
ParserBase<Impl>::ParseLeftHandSideContinuation(ExpressionT result) {
  ...
  if (V8_UNLIKELY(peek() == Token::LPAREN && impl()->IsIdentifier(result) &&
    ...
    ArrowHeadParsingScope maybe_arrow(
        impl(), ExpressionScope::kAsyncArrowParameterDeclaration);
    Scope::Snapshot scope_snapshot(scope());
    int rewritable_length = static_cast<int>(
        function_state_->destructuring_assignments_to_rewrite().size());
