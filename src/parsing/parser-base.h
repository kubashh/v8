...
#undef ALLOW_ACCESSORS
 ...
 protected:
  ...
  V8_INLINE IdentifierT ParseIdentifierOrStrictReservedWord(
      bool* is_strict_reserved, bool* is_await) {
...
typename ParserBase<Impl>::IdentifierT ParserBase<Impl>::ParseIdentifier(
  ...
  if (V8_UNLIKELY(!Token::IsAnyIdentifier(peek()))) {
...
ParserBase<Impl>::ParseAndClassifyIdentifier() {
  Token::Value next = Next();
  DCHECK(Token::IsAnyIdentifier(next));
...
ParserBase<Impl>::ParseIdentifierOrStrictReservedWord(
  ...
  *is_strict_reserved = Token::IsStrictReservedWord(next);
  ...
  if (V8_LIKELY(!Token::IsAwaitOrYield(next))) return name;
...
ParserBase<Impl>::ParseHoistableDeclaration(
  ...
  IdentifierT name = impl()->NullIdentifier();
  FunctionNameValidity name_validity = kFunctionNameValidityUnknown;
  ...
  if (default_export && peek() == Token::LPAREN) {
    ...
    if (is_strict_reserved) name_validity = kFunctionNameIsStrictReserved;
    variable_name = name;
  } else {
    ReportUnexpectedToken(peek());
    return impl()->NullStatement();
