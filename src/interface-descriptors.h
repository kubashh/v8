...
class V8_EXPORT_PRIVATE CallInterfaceDescriptorData {
 ...
 private:
  ...
  // parameters.
  // Both arrays are are allocated dynamically by the InterfaceDescriptor and
...
class V8_EXPORT_PRIVATE CallInterfaceDescriptor {
 ...
 protected:
  ...
  virtual void InitializePlatformIndependent(
    ...
    // in registers and all parameters has MachineType::AnyTagged() type.
...
#define DECLARE_DEFAULT_DESCRIPTOR(name, base, parameter_count)               \
 ...
 protected:                                                                   \
  ...
  void InitializePlatformIndependent(CallInterfaceDescriptorData* data)       \
      override {                                                              \
    data->InitializePlatformIndependent(kResultCount, parameter_count,        \
                                        nullptr, 0);                          \
  }                                                                           \
...
#define DEFINE_RESULT_AND_PARAMETERS(result_count, ...)   \
  static constexpr int kResultCount = result_count;       \
...
#define DEFINE_RESULT_AND_PARAMETERS_NO_CONTEXT(result_count, ...) \
  static constexpr int kResultCount = result_count;                \
  enum ParameterIndices {                                          \
    __dummy = -1, /* to be able to pass zero arguments */          \
    ##__VA_ARGS__,                                                 \
...
#define DEFINE_RESULT_AND_PARAMETER_TYPES(...)                                \
  void InitializePlatformIndependent(CallInterfaceDescriptorData* data)       \
    ...
    static_assert(                                                            \
        kResultCount + kParameterCount == arraysize(machine_types),           \
        "Parameter names definition is not consistent with parameter types"); \
    data->InitializePlatformIndependent(kResultCount, kParameterCount,        \
                                        machine_types,                        \
...
#define DEFINE_JS_PARAMETERS(...) \
  DEFINE_PARAMETERS(kTarget, kNewTarget, kActualArgumentsCount, ##__VA_ARGS__)
