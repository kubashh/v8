...
bool FastInitializeDerivedMap(Isolate* isolate, Handle<JSFunction> new_target,
  ...
  CHECK(CanSubclassHaveInobjectProperties(instance_type));
...
void JSFunction::CalculateInstanceSizeHelper(InstanceType instance_type,
  ...
  requested_in_object_properties =
      Min(requested_in_object_properties, kMaxNumberOfDescriptors);
  CHECK_LE(requested_embedder_fields,
           max_nof_fields - requested_in_object_properties);
...
bool JSFunction::CalculateInstanceSizeForDerivedClass(
  ...
  for (PrototypeIterator iter(isolate, function, kStartAtReceiver);
    ...
    if (shared->is_compiled() ||
      ...
      if (expected_nof_properties > kMaxNumberOfDescriptors - count) {
        return false;
      }
