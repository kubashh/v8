...
WeakArrayList* WeakArrayListWithEmptySlots::Compact(
  ...
  Handle<WeakArrayList> new_array = WeakArrayList::EnsureSpace(
      handle(heap->empty_weak_array_list(), heap->isolate()), new_length);
  new_array->set_length(new_length);
  for (int i = kFirstIndex, copy_to = kFirstIndex; i < array->length(); i++) {
    MaybeObject* element = array->Get(i);
    if (element->IsSmi()) continue;
    if (element->IsClearedWeakHeapObject()) continue;
  ...
  set_empty_slot_index(*new_array, 0);
  return *new_array;
}
