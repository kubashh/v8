...
Maybe<bool> Object::SetPropertyWithAccessor(LookupIterator* it,
  ...
  if (structure->IsAccessorInfo()) {
    ...
    PropertyCallbackArguments args(isolate, info->data(), *receiver, *holder,
                                   should_throw);
    // TODO(cbruni): Make this call specifi.
    Handle<Object> result = args.CallSetterCallback(info, name, value);
...
MaybeHandle<Object> GetPropertyWithInterceptorInternal(
  ...
  *done = false;
  ...
  if (it->IsElement()) {
    ...
    result = args.CallIndexedGetter(interceptor, index);
  } else {
    Handle<Name> name = it->name();
    DCHECK_IMPLIES(name->IsSymbol(), interceptor->can_intercept_symbols());
    result = args.CallNamedGetter(interceptor, name);
  }
...
Maybe<bool> SetPropertyWithInterceptorInternal(
  ...
  if (it->IsElement()) {
    ...
    DCHECK(!name->IsPrivate());
    DCHECK_IMPLIES(name->IsSymbol(), interceptor->can_intercept_symbols());
    result = !args.CallNamedSetter(interceptor, name, value).is_null();
  }
