...
Handle<WeakArrayList> WeakArrayList::EnsureSpace(Handle<WeakArrayList> array,
  ...
  if (capacity < length) {
    ...
    printf("EnsureSpace did something; new capacity %d\n", new_capacity);
  }
  return array;
}
...
Handle<WeakArrayList> WeakArrayListWithEmptySlots::Add(Handle<WeakArrayList> array,
                                                       MaybeObjectHandle value, int* assigned_index) {
  printf("Add len %d cap %d\n", array->length(), array->capacity());
  array->Print();
  // Can't add SMIs since they're used as placeholders.
  DCHECK(!value->IsSmi());

  int length = array->length();
  if (length == 0) {
    printf("Zero length thingy\n");
    // Uninitialized WeakArrayList; need to initialize empty_slot_index.
    array = WeakArrayList::EnsureSpace(array, kFirstIndex + 1);
    set_empty_slot_index(array, kEmptySlotIndex);
    array->Set(kFirstIndex, *value);
  ...
  // Try to store the new entry if there's room.
  if (!array->IsFull()) {
    printf("Length less than capacity\n");
    array->Set(length, *value);
    array->set_length(length + 1);
    if (assigned_index != nullptr) *assigned_index = length;
    printf("after adding\n");
  ...
  if (empty_slot >= kFirstIndex) {
    printf("Found empty slot %d\n", empty_slot);
    CHECK_LT(empty_slot, array->capacity());
    printf("empty slot is %d, stuff is %p\n", empty_slot, array->Get(empty_slot));
    int next_empty_slot = Smi::ToInt(array->Get(empty_slot)->ToSmi());

    array->Set(empty_slot, *value);
    if (assigned_index != nullptr) *assigned_index = empty_slot;

    set_empty_slot_index(array, next_empty_slot);
    printf("next empty slot is %d\n", next_empty_slot);
    return array;
  }

  array = WeakArrayList::EnsureSpace(array, length + 1);
  array->Set(length, *value);
  if (assigned_index != nullptr) *assigned_index = length;
  printf("after adding\n");
  array->Print();
...
void JSObject::LazyRegisterPrototypeUser(Handle<Map> user, Isolate* isolate) {
  ...
  for (PrototypeIterator iter(user); !iter.IsAtEnd(); iter.Advance()) {
    ...
    Handle<WeakArrayList> registry = maybe_registry->IsSmi() ? handle(isolate->heap()->empty_weak_array_list()) : Handle<WeakArrayList>::cast(maybe_registry);
    int slot = 0;
    Handle<WeakArrayList> new_array =
        WeakArrayListWithEmptySlots::Add(registry, MaybeObjectHandle::Weak(current_user), &slot);
...
bool JSObject::UnregisterPrototypeUser(Handle<Map> user, Isolate* isolate) {
  ...
  Handle<PrototypeInfo> proto_info(PrototypeInfo::cast(maybe_proto_info),
                                   isolate);
  Handle<WeakArrayList> prototype_users(WeakArrayList::cast(proto_info->prototype_users()));
  DCHECK(prototype_users->Get(slot) == HeapObjectReference::Weak(*user));
  WeakArrayListWithEmptySlots::MarkSlotEmpty(prototype_users, slot);
  if (FLAG_trace_prototype_users) {
...
void InvalidatePrototypeChainsInternal(Map* map) {
  ...
  for (int i = WeakArrayListWithEmptySlots::kFirstIndex; i < prototype_users->length(); ++i) {
    HeapObject* heap_object;
    if (prototype_users->Get(i)->ToWeakHeapObject(&heap_object) && heap_object->IsMap()) {
