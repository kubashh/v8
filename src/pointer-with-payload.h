...
class PointerWithPayload {
  ...
  static constexpr int kAvailBits =
      AvailBits<typename std::remove_const<PointerType>::type>::value;
  static_assert(
      kAvailBits >= NumPayloadBits,
      "Ptr does not have sufficient alignment for the selected amount of "
      "storage bits.");

  static constexpr uintptr_t kPointerMask = ~((uintptr_t{1} << kAvailBits) - 1);
 ...
 public:
  ...
  V8_INLINE void update(PointerType* new_pointer, PayloadType new_payload) {
    pointer_ = reinterpret_cast<uintptr_t>(new_pointer) | new_payload;
    DCHECK_EQ(GetPayload(), new_payload);
    DCHECK_EQ(GetPointer(), new_pointer);
  }

  V8_INLINE void SetPointer(PointerType* newptr) {
    DCHECK_EQ(reinterpret_cast<PointerType*>(
                  reinterpret_cast<uintptr_t>(newptr) & kPointerMask),
              newptr);
