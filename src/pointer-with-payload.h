...
class PointerWithPayload {
 ...
 public:
  ...
  template <typename PayLoadArgType = PayloadType>
  V8_INLINE void SetPayload(
      // Prevent const payloads from changing.
      typename std::enable_if<!std::is_const<PayLoadArgType>::value,
                              PayLoadArgType>::type new_payload) {
