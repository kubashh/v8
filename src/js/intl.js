...
// GetDefaultICULocaleJS is available in C++
%InstallToContext([
  "get_default_icu_locale_js", GetDefaultICULocaleJS
]);
...
function toDateTimeOptions(options, required, defaults) {
  print('required ' + required +  ' defaults '  + defaults);
  if (IS_UNDEFINED(options)) {
    options = {__proto__: null};
  } else {
    options = TO_OBJECT(options);
...
function cachedOrNewService(service, locales, options, defaults) {
  print ('service ' + service + ' locales ' + locales + ' defaults ' + defaults);
