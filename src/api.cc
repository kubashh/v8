...
MaybeLocal<Object> Function::NewInstance(
  ...
  i::Handle<i::Object>* args = reinterpret_cast<i::Handle<i::Object>*>(argv);
  Local<Object> result;
  bool allow_next_side_effect =
      side_effect_type == v8::SideEffectType::kHasNoSideEffect &&
      isolate->debug_execution_mode() == i::DebugInfo::kSideEffects;
  if (allow_next_side_effect) {
    isolate->debug()->SkipNextSideEffectFromApi();
  }
  has_pending_exception = !ToLocal<Object>(
      i::Execution::New(isolate, self, self, argc, args), &result);
  if (allow_next_side_effect) {
    isolate->debug()->CancelSkipNextSideEffectFromApi();
  }
  RETURN_ON_FAILED_EXECUTION(Object);
  RETURN_ESCAPED(result);
}
...
MaybeLocal<v8::Object> ObjectTemplate::NewInstance(
    Local<Context> context, SideEffectType side_effect_type) {
