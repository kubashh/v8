...
// microtask queue. The tasks form a singly-linked list terminated by
// Smi 0. For every microtask we remember the native context in which
// it was created, and run the operation in that same native context
// later (uniformly).
class Microtask : public Struct {
 public:
  DECL_ACCESSORS(context, Context)

  static const int kContextOffset = HeapObject::kHeaderSize;
  static const int kHeaderSize = kContextOffset + kPointerSize;
...
class CallableTask : public Microtask {
 public:
  ...
  static const int kSize = kCallableOffset + kPointerSize;
...
class PromiseReactionJobTask : public Microtask {
 public:
  ...
  // Either a JSPromise or a PromiseCapability.
  DECL_ACCESSORS(promise_or_capability, HeapObject)

  static const int kArgumentOffset = Microtask::kHeaderSize;
  static const int kHandlerOffset = kArgumentOffset + kPointerSize;
...
class PromiseResolveThenableJobTask : public Microtask {
 public:
  ...
  static const int kPromiseToResolveOffset = Microtask::kHeaderSize;
...
class PromiseCapability : public Struct {
 public:
  ...
  DECL_ACCESSORS(resolve, HeapObject)
  DECL_ACCESSORS(reject, HeapObject)
