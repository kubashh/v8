...
#define IS_TYPE_FUNCTION_DECL(Type, Value)               \
  ...
  V8_INLINE bool Is##Type(ReadOnlyRoots ro_roots) const; \
  V8_INLINE bool Is##Type() const;
  ODDBALL_LIST(IS_TYPE_FUNCTION_DECL)
#undef IS_TYPE_FUNCTION_DECL

  V8_INLINE bool IsNullOrUndefined(Isolate* isolate) const;
  V8_INLINE bool IsNullOrUndefined(ReadOnlyRoots ro_roots) const;
...
// Oddball checks are faster when they are raw pointer comparisons, so the
// isolate/read-only roots overloads on Object should be preferred where
// possible.
