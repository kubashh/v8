...
void RelocIteratorBase<RelocInfoType>::AdvanceReadInt() {
  int x = 0;
  for (int i = 0; i < kIntSize; i++) {
...
template <typename RelocInfoType>
void RelocIteratorBase<RelocInfoType>::AdvanceReadLongPCJump() {
...
template <typename RelocInfoType>
inline void RelocIteratorBase<RelocInfoType>::ReadShortData() {
  uint8_t unsigned_b = *pos_;
  rinfo_.data_ = unsigned_b;
}

template <typename RelocInfoType>
void RelocIteratorBase<RelocInfoType>::next() {
  DCHECK(!done());
...
WritableRelocIterator::WritableRelocIterator(
    ...
    Tagged<InstructionStream> istream, Address constant_pool, int mode_mask)
...
template <typename RelocInfoType>
RelocIteratorBase<RelocInfoType>::RelocIteratorBase(Address pc,
