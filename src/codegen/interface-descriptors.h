 ...
 private:
  bool IsInitializedRegisters() const {
    const bool initialized =
        return_count_ >= 0 && param_count_ >= 0 &&
  ...
  // parameters.
  // RegisterParams is owner by the caller, and it's expected that it is a
  // static local stored in a  Both arrays are allocated dynamically
  // by the InterfaceDescriptor and freed on destruction. This is because static
  // arrays cause creation of runtime static initializers which we don't want.
...
// Polymorphic base class for CallInterfaceDescriptors, which defines getters
...
// CRTP base class for CallInterfaceDescriptors, which defines static getters
...
class StaticCallInterfaceDescriptor : public CallInterfaceDescriptor {
 public:
  ...
  // larger then any reamining registers are ignored; if smaller, any parameters
 ...
 private:
  // {CallDescriptors} is allowed to call the private {Initialize} metod.
