...
#undef DEF_SETTER
 ...
 private:
  ...
  // LocalHeap, then again by JSHeapBroker for a brief moment (when tearing down
  // the LocalHeap as part of exiting LocalHeapScope), and then
  // OptimizedCompilationInfo when exiting the LocalHeapScope. In normal
  // execution it gets destroyed when PipelineData gets destroyed when e.g.
  // PipelineCompilationJob gets destroyed. Since it is a member of
