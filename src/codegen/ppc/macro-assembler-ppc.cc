...
void TurboAssembler::MultiPushV128(Simd128RegList simdd_regs,
                                   Register location) {
  int16_t num_to_push = simdd_regs.Count();
  int16_t stack_offset = num_to_push * kSimd128Size;

  subi(location, location, Operand(stack_offset));
  for (int16_t i = Simd128Register::kNumRegisters - 1; i >= 0; i--) {
    if ((simdd_regs.Count() & (1 << i)) != 0) {
      Simd128Register simdd_reg = Simd128Register::from_code(i);
      stack_offset -= kSimd128Size;
      li(ip, Operand(stack_offset));
      StoreSimd128(simdd_reg, MemOperand(location, ip));
    }
  }
}
...
void TurboAssembler::MultiPopV128(Simd128RegList simdd_regs,
                                  Register location) {
  int16_t stack_offset = 0;

  for (int16_t i = 0; i < Simd128Register::kNumRegisters; i++) {
    if ((simdd_regs.bits() & (1 << i)) != 0) {
      Simd128Register simdd_reg = Simd128Register::from_code(i);
      li(ip, Operand(stack_offset));
      LoadSimd128(simdd_reg, MemOperand(location, ip));
      stack_offset += kSimd128Size;
    }
  }
  addi(location, location, Operand(stack_offset));
