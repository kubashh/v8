...
void MacroAssembler::StoreTrustedPointerField(
...
void MacroAssembler::ResolveIndirectPointerHandle(Register destination,
                                                  ...
                                                  IndirectPointerTag tag,
                                                  Register scratch) {
  // TODO(tpearson): We don't have enough registers to properly implement
  // this.  For now, we abuse destination as a scratch register in
  // ResolveIndirectPointerHandle(), but this also means that there is no
  // scratch register available in Resolve[Trusted|Code]PointerHandle()
  // should the logic increase beyond what is currently present...
  Register scratch2 = destination;
  ...
  if (tag == kUnknownIndirectPointerTag) {
    ...
    mov(scratch2, Operand(kCodePointerHandleMarker));
    AndU64(scratch2, handle, scratch2, SetRC);
...
void MacroAssembler::ResolveTrustedPointerHandle(Register destination,
  ...
  LoadU64(destination, MemOperand(table, handle), handle);
...
void MacroAssembler::LoadCodeEntrypointViaCodePointer(
  ...
  ShiftRightU64(destination, destination,
                Operand(kCodePointerHandleShift));
...
#ifdef V8_ENABLE_SANDBOX
  LoadCodeEntrypointViaCodePointer(scratch2,
