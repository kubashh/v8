...
int MacroAssembler::CallCFunction(Register function, int num_reg_arguments,
  ...
  Label start_call;
  Register scratch = r11;
  DCHECK(!AreAliased(scratch, function));
  LoadPC(scratch);
  bind(&start_call);
  ...
  addi(scratch, scratch, Operand::Zero());
  ...
  if (set_isolate_data_slots == SetIsolateDataSlots::kYes) {
    ...
    if (root_array_available()) {
      ...
      StoreU64(scratch, MemOperand(addr_scratch));
      Move(addr_scratch,
  ...
  patch_pc_address(scratch, start_pc_offset, offset_since_start_call);
