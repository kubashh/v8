...
void MacroAssembler::ResolveCodePointerHandle(Register destination,
  ...
  LoadU64(destination,
          MemOperand(handle, kCodePointerTableEntryCodeObjectOffset),
          destination);
...
void MacroAssembler::IsObjectType(Register object, Register scratch1,
  ...
  if (InstanceTypeChecker::UniqueMapOfInstanceType(type)) {
    LoadCompressedMap(scratch1, object,
                      scratch1 != scratch2 ? scratch2 : Register::no_reg());
...
void MacroAssembler::BailoutIfDeoptimized() {
  if (v8_flags.debug_code) {
    ...
    ComputeCodeStartAddress(ip);
    CmpS64(ip, kJavaScriptCallCodeStartRegister);
    Assert(eq, AbortReason::kWrongFunctionCodeStart);
  }

  int offset = InstructionStream::kCodeOffset - InstructionStream::kHeaderSize;
  LoadCodePointerField(
      r11, MemOperand(kJavaScriptCallCodeStartRegister, offset), ip);
