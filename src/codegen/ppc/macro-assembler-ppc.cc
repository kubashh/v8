...
void TurboAssembler::MultiPushV128(Simd128RegList dregs, Register location) {
  int16_t num_to_push = dregs.Count();
  int16_t stack_offset = num_to_push * kSimd128Size;

  subi(location, location, Operand(stack_offset));
  for (int16_t i = Simd128Register::kNumRegisters - 1; i >= 0; i--) {
    if ((dregs.Count() & (1 << i)) != 0) {
...
void TurboAssembler::MultiPopV128(Simd128RegList dregs, Register location) {
  int16_t stack_offset = 0;
