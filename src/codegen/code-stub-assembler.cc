...
TNode<Numeric> CodeStubAssembler::NonNumberToNumberOrNumeric(
  ...
  Label end(this), final_check(this),
      if_inputisreceiver(this, Label::kDeferred), if_inputisnotreceiver(this);
  ...
  {
    // The {var_input.value()} is a JSReceiver, we need to convert it to a Primitive
  ...
  // String and Oddball cases.
  {
    ...
    // We are guarenteed to have a Number and can skip the final check.
    Goto(&end);
  }
  ...
  BIND(&if_inputisbigint);
  {
    if (mode == Object::Conversion::kToNumeric) {
      ...
      if (bigint_handling == BigIntHandling::kThrow) {
        ...
        var_result = CAST(
            CallRuntime(Runtime::kBigIntToNumber, context, var_input.value()));
        Goto(&final_check);
      }
    }
  }
  ...
  BIND(&if_inputisother);
  {
    ...
    // runtime figure out the correct exception. Note: We cannot tail call to
    // the runtime here, as js-to-wasm trampolines also use this code
    // currently, and they declare all outgoing parameters as untagged, while
    // we would push a tagged object here.
