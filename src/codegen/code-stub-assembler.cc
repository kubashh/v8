...
TNode<Object> CodeStubAssembler::CallGetterIfAccessor(
    TNode<Object> value, TNode<JSObject> holder, TNode<Uint32T> details,
    ...
    if (mode == kCallJSGetter) {
      ...
      BIND(&if_function_template_info);
      {
        ...
        TNode<NativeContext> creation_context =
            GetCreationContext(holder, if_bailout);
...
void CodeStubAssembler::TryGetOwnProperty(
  ...
  BIND(&if_found);
  {
    ...
    TNode<Object> value = CallGetterIfAccessor(var_value->value(), CAST(object),
                                               var_details->value(), context,
