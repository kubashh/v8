...
void CodeStubAssembler::FillFixedArrayWithSmiZero(
    ElementsKind kind, TNode<FixedArray> array,
    base::Optional<TNode<IntPtrT>> start, TNode<IntPtrT> length) {
  DCHECK(IsSmiOrObjectElementsKind(kind));
  CSA_DCHECK(this,
             IntPtrLessThanOrEqual(start ? IntPtrAdd(*start, length) : length,
                                   LoadAndUntagFixedArrayBaseLength(array)));
  ...
  TNode<IntPtrT> offset;
  if (start) {
    offset = ElementOffsetFromIndex(*start, kind, fa_base_data_offset);
  } else {
    offset = IntPtrConstant(fa_base_data_offset);
  }
...
void CodeStubAssembler::FillFixedDoubleArrayWithZero(
    TNode<FixedDoubleArray> array, base::Optional<TNode<IntPtrT>> start,
  ...
  TNode<IntPtrT> offset;
  if (start) {
    offset = ElementOffsetFromIndex(*start, PACKED_DOUBLE_ELEMENTS,
                                    fa_base_data_offset);
  } else {
    offset = IntPtrConstant(fa_base_data_offset);
  }
