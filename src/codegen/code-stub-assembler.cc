...
TNode<Int32T> CodeStubAssembler::Word32CountTrailingZeros(
  ...
  if (Is32()) {
    ...
    TNode<WordT> lhs = WordNot(ReinterpretCast<WordT>(value));
    TNode<WordT> rhs =
        ReinterpretCast<WordT>(Int32Sub(Signed(value), Int32Constant(1)));
    return Word32PopulationCount(ReinterpretCast<Word32T>(WordAnd(lhs, rhs)));
  } else {
