...
TNode<HeapObject> CodeStubAssembler::ResolveIndirectPointerHandle(
  ...
  if (tag == kUnknownIndirectPointerTag) {
    ...
    Label is_trusted_pointer_handle(this), is_code_pointer_handle(this),
        done(this, &result);
    Branch(IsTrustedPointerHandle(handle), &is_trusted_pointer_handle,
           &is_code_pointer_handle);
    BIND(&is_trusted_pointer_handle);
    result = ResolveTrustedPointerHandle(handle, tag);
    Goto(&done);
    BIND(&is_code_pointer_handle);
    result = ResolveCodePointerHandle(handle);
    Goto(&done);
    BIND(&done);
    return result.value();
...
TNode<Object> CodeStubAssembler::LoadSharedFunctionInfoData(
  ...
  TVARIABLE(Object, var_result);
  Label done(this, &var_result);
  Label use_tagged_field(this);
  GotoIf(Word32Equal(trusted_data_handle,
                     Int32Constant(kNullIndirectPointerHandle)),
         &use_tagged_field);

  var_result = ResolveIndirectPointerHandle(trusted_data_handle,
                                            kUnknownIndirectPointerTag);
  Goto(&done);

  BIND(&use_tagged_field);
  var_result =
