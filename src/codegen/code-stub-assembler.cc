...
TNode<ByteArray> CodeStubAssembler::AllocateNonEmptyByteArray(
  ...
  StoreObjectFieldNoWriteBarrier(result, ByteArray::kCapacityOffset,
...
void CodeStubAssembler::CopyRange(TNode<HeapObject> dst_object, int dst_offset,
                                  ...
                                  WriteBarrierMode mode) {
  // TODO: Something probably still broken here.
  ...
  BuildFastLoop<IntPtrT>(
      ...
      [=](TNode<IntPtrT> index) {
        ...
        TNode<AnyTaggedT> value =
            Load<AnyTaggedT>(src_object, current_src_offset);
        TNode<IntPtrT> current_dst_offset =
            IntPtrAdd(TimesTaggedSize(index), IntPtrConstant(dst_offset));
        if (mode == SKIP_WRITE_BARRIER) {
          StoreNoWriteBarrier(MachineRepresentation::kTagged, dst_object,
                              current_dst_offset, value);
        } else {
          Store(dst_object, current_dst_offset, value);
        }
      },
      1, LoopUnrollingMode::kYes, IndexAdvanceMode::kPre);
}
