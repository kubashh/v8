...
void CodeStubAssembler::TryGetOwnPropertyDescriptor(
    TNode<Context> context, TNode<Object> receiver, TNode<JSReceiver> object,
    TNode<Map> map, TNode<Int32T> instance_type, TNode<Name> unique_name,
    Reference out_descriptor, Label* if_found, Label* if_notfound,
    Label* if_bailout) {
  TVARIABLE(Object, value, UndefinedConstant());
  TVARIABLE(Object, raw_value, UndefinedConstant());
  TVARIABLE(Uint32T, details, Uint32Constant(0));

  Label done(this);
  StoreReference(out_descriptor, UndefinedConstant());
  TryGetOwnProperty(context, receiver, object, map, instance_type, unique_name,
                    &done, &value, &details, &raw_value, if_notfound,
                    if_bailout, kReturnAccessorPair);

  BIND(&done);
  {
    TNode<PropertyDescriptorObject> descriptor =
        CAST(Allocate(PropertyDescriptorObject::kSize));
    TNode<Map> map = GetInstanceTypeMap(PROPERTY_DESCRIPTOR_OBJECT_TYPE);
    StoreMapNoWriteBarrier(descriptor, map);
    InitializePropertyDescriptorObject(descriptor, value.value(),
                                       details.value(), if_bailout);
    StoreReference(out_descriptor, descriptor);
    Goto(if_found);
  }
}
