...
void MacroAssembler::LoadEntrypointAndParameterCountFromJSDispatchTable(
  ...
  movq(entrypoint, Operand(kScratchRegister, offset, times_1,
                           JSDispatchEntry::kEntrypointOffset));
  movq(parameter_count, Operand(kScratchRegister, offset, times_1,
                                JSDispatchEntry::kCodeObjectOffset));
  andl(parameter_count, Immediate(JSDispatchEntry::kParameterCountMask));
}
#endif
...
void MacroAssembler::CallJSFunction(Register function_object,
  ...
  Label ok;
  cmpl(rbx, Immediate(actual_argument_count));
...
void MacroAssembler::InvokeFunctionCode(Register function, Register new_target,
  ...
  // call sites.
  static_assert(kJavaScriptCallCodeStartRegister == rcx, "ABI mismatch");
  constexpr int unused_expected_parameter_count = 0;
  switch (type) {
    case InvokeType::kCall:
      CallJSFunction(function, unused_expected_parameter_count);
      break;
    case InvokeType::kJump:
