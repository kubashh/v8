...
void ArgumentsHelper::AddArgumentsFrom(Register src, Register count,
  ...
  ArgcIncludingReceiverAndReturnAddress(shift_count);
...
void ArgumentsHelper::PushArgumentsFrom(Register src,
  ...
  // Check for a stack overflow.
  {
    ...
    UseScratchRegisterScope scratch(masm_);
    // kScratchRegister is used in StackOverflowCheck. Exclude it.
    scratch.Exclude(kScratchRegister);
...
void ArgumentsHelper::ArgcIncludingReceiverAndReturnAddress(Register dest) {
  int argc_addition = kReturnAddressStackSlotCount;
...
void MacroAssembler::StackOverflowCheck(
  ...
  ASM_CODE_COMMENT(this);
  DCHECK(!AreAliased(num_args, scratch, kScratchRegister));
