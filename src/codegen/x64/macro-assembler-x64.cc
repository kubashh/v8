...
void TurboAssembler::Movdqa(XMMRegister dst, XMMRegister src) {
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    // Many AVX process have separate integer/floating-point domains. Use the
    // appropriate instructions.
    vmovdqa(dst, src);
  } else {
    // On SSE, movaps is 1 byte shorter movdqa, and has the same behavior.
    // Most SSE processors also don't the same delay moving between integer and
...
void TurboAssembler::Movapd(XMMRegister dst, XMMRegister src) {
  if (CpuFeatures::IsSupported(AVX)) {
    ...
    // On SSE, movaps is 1 byte shorter movapd, and has the same behavior.
    movaps(dst, src);
  }
}
