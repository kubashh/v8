...
void ArgumentsHelper::AddArgumentsFrom(Register src, Register count,
  ...
  Register shift_count = scratch.Acquire();
  ArgcIncludingReceiverAndReturnAddress(shift_count, flags);
  ShiftStackArguments(count, shift_count);

  // Set destination to the top of shifted existing arguments.
  Register dest = shift_count;
  __ leaq(dest, Operand(rsp, shift_count, times_system_pointer_size, 0));

  // Copy new arguments.
  CopyArguments(src, dest, count, elements_type, flags);
...
void ArgumentsHelper::AddArgumentsFromFrame(Register count,
                                            Register start_index, Flags flags) {
  UseScratchRegisterScope scratch(masm_);
  // Shift existing arguments.
  Register shift_count = scratch.Acquire();
  ArgcIncludingReceiverAndReturnAddress(shift_count, flags);
  ...
  // Copy new arguments from the frame.
...
void ArgumentsHelper::PrepareInvoke(InvokeType type, Label* done) {
  // If the expected parameter count is equal to the adaptor sentinel, no need
  // to push undefined value as arguments.
  __ cmpl(formal_parameter_count_, Immediate(kDontAdaptArgumentsSentinel));
  __ j(equal, done);

  // If overapplication or if the actual argument count is equal to the
  // formal parameter count, no need to push extra undefined values.
  __ subq(formal_parameter_count_, argc_);
  ...
  UseScratchRegisterScope scratch(masm_);
  Register shift_count = scratch.Acquire();
  auto flags = type == InvokeType::kCall ? kNone : kPreserveReturnAddress;
  ArgcIncludingReceiverAndReturnAddress(shift_count, flags);
  ShiftStackArguments(formal_parameter_count_, shift_count);

  // Fill the slots without actual arguments with undefined.
  Register dest = shift_count;
  __ leaq(dest, Operand(rsp, shift_count, times_system_pointer_size, 0));

  Register undefined = scratch.Acquire();
  __ LoadRoot(undefined, RootIndex::kUndefinedValue);
  auto loop_body = [&](TurboAssembler* masm, Register current) {
    masm->movq(Operand(dest, current, times_system_pointer_size, 0), undefined);
  };
  Register current = scratch.Acquire();
  __ LoopBackward(formal_parameter_count_, current, loop_body);
  __ jmp(done);
}
...
void ArgumentsHelper::ShiftStackArguments(Register offset, Register count) {
  UseScratchRegisterScope scratch(masm_);
  // Check for a stack overflow.
  {
    DCHECK_NE(stack_overflow_label_, nullptr);
...
void ArgumentsHelper::CopyArguments(Register src, Register dest, Register count,
                                    ElementsType elements_type, Flags flags) {
  UseScratchRegisterScope scratch(masm_);
...
void ArgumentsHelper::ArgcIncludingReceiverAndReturnAddress(Register dest,
                                                            Flags flags) {
  int argc_addition = 0;
  if ((flags & kPreserveReturnAddress) != 0) {
    argc_addition += kReturnAddressStackSlotCount;
  }
  if (!argc_includes_receiver_) ++argc_addition;
  if (argc_addition != 0) {
    __ leaq(dest, Operand(argc_, argc_addition));
  } else if (dest != argc_) {
    __ movq(dest, argc_);
  }
}
...
void MacroAssembler::StackOverflowCheck(
  ...
  ASM_CODE_COMMENT(this);
  UseScratchRegisterScope scratch_scope(this);
  Register limit = scratch_scope.Acquire();
  DCHECK(!AreAliased(num_args, scratch, limit));
  // Check the stack for overflow. We are not trying to catch
  // interruptions (e.g. debug break and preemption) here, so the "real stack
  // limit" is checked.
  movq(limit, StackLimitAsOperand(StackLimitKind::kRealStackLimit));
  movq(scratch, rsp);
...
void MacroAssembler::InvokePrologue(Register expected_parameter_count,
    ...
    Label stack_overflow, regular_invoke;
    {
      UseScratchRegisterScope scratch(this);
      scratch.Include(Register::ListOf(r8, r9, r11));
      ArgumentsHelper args(this, actual_parameter_count);
      args.set_formal_parameter_count(expected_parameter_count);
      args.set_stack_overflow_label(&stack_overflow);
      args.PrepareInvoke(type, &regular_invoke);
    }
