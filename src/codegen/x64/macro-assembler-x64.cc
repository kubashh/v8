...
void MacroAssembler::CallIndirectPointerBarrier(Register object,
  ...
  // In this case, we also have to pass the indirect pointer tag to the
  // write barrier stub. We do this by encoding it into the slot address,
  // which is possible because the slot addresse's upper 16 bits, and the
  // tag's lower 48 bits, will always be zero.
  // In addition to requiring fewer registers (compared to an
  // implementation where we pass the tag in a separate register), this
  // also has a small added benefit in that it becomes impossible to
  // dereference the slot pointer without "untagging" it first. This can
  // catch issues where code loads the slot's value and expects it to be a
  // tagged pointer (which, in this case, it is not).
