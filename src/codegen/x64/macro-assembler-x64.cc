...
void TurboAssembler::CallTSANSeqCstStoreStub(Register address, Register value,
                                             SaveFPRegsMode fp_mode, int size,
                                             StubCallMode mode) {
  ASM_CODE_COMMENT(this);
  DCHECK(!AreAliased(address, value));
  TSANSeqCstStoreDescriptor descriptor;
  RegList registers = descriptor.allocatable_registers();

  MaybeSaveRegisters(registers);

  Register address_parameter(
      descriptor.GetRegisterParameter(TSANSeqCstStoreDescriptor::kAddress));
  Register value_parameter(
      descriptor.GetRegisterParameter(TSANSeqCstStoreDescriptor::kValue));

  // Prepare argument registers for calling GetTSANSeqCstStoreStub.
  MovePair(address_parameter, address, value_parameter, value);

  if (isolate()) {
    Builtin builtin = CodeFactory::GetTSANSeqCstStoreStub(fp_mode, size);
    Handle<Code> code_target = isolate()->builtins()->code_handle(builtin);
    Call(code_target, RelocInfo::CODE_TARGET);
  }
#if V8_ENABLE_WEBASSEMBLY
  ...
  // NOLINTNEXTLINE(readability/braces)
    ...
    DCHECK_EQ(mode, StubCallMode::kCallWasmRuntimeStub);
    // Use {near_call} for direct Wasm call within a module.
    auto wasm_target = wasm::WasmCode::GetTSANSeqCstStoreStub(fp_mode, size);
    near_call(wasm_target, RelocInfo::WASM_STUB_CALL);
  }
#endif  // V8_ENABLE_WEBASSEMBLY
