...
int64_t Instruction::ImmPCOffset() {
  ...
  if (IsPCRelAddressing()) {
    ...
    offset = static_cast<int32_t>(static_cast<uint32_t>(ImmBranch())
                                  << kInstrSizeLog2);
  } else if (IsUnresolvedInternalReference()) {
    // Internal references are always word-aligned.
    offset = static_cast<int32_t>(
        static_cast<uint32_t>(ImmUnresolvedInternalReference())
        << kInstrSizeLog2);
  } else {
    ...
    // registers.
    offset = static_cast<int32_t>(static_cast<uint32_t>(ImmLLiteral())
                                  << kInstrSizeLog2);
  }
  return offset;
}
