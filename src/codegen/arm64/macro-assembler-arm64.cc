...
void MacroAssembler::CallIndirectPointerBarrier(Register object, Operand offset,
  ...
  // The indirect pointer tag is encoded into the slot address. See also
  // CallIndirectPointerBarrier in the x64 MacroAssembler.
  Register slot_register = WriteBarrierDescriptor::SlotAddressRegister();
  UseScratchRegisterScope temps(this);
  Register tag_reg = temps.AcquireX();
  Mov(tag_reg, Immediate(~tag));
  Orr(slot_register, slot_register, tag_reg);
