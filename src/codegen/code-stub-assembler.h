  ...
  TNode<Smi> SmiOpName(TNode<Smi> a, TNode<Smi> b) {                      \
    if (SmiValuesAre32Bits()) {                                           \
      return BitcastWordToTaggedSigned(                                   \
          IntPtrOpName(BitcastTaggedNonPointerBitsToWord(a),              \
                       BitcastTaggedNonPointerBitsToWord(b)));            \
    } else {                                                              \
      ...
      return BitcastWordToTaggedSigned(ChangeInt32ToIntPtr(Int32OpName(   \
          TruncateIntPtrToInt32(BitcastTaggedNonPointerBitsToWord(a)),    \
          TruncateIntPtrToInt32(BitcastTaggedNonPointerBitsToWord(b))))); \
    }                                                                     \
  }
...
#undef SMI_ARITHMETIC_BINOP
  ...
  TNode<Smi> SmiShl(TNode<Smi> a, int shift) {
    return BitcastWordToTaggedSigned(
        WordShl(BitcastTaggedNonPointerBitsToWord(a), shift));
  }

  TNode<Smi> SmiShr(TNode<Smi> a, int shift) {
    if (kTaggedSize == kInt64Size) {
      return BitcastWordToTaggedSigned(
          WordAnd(WordShr(BitcastTaggedNonPointerBitsToWord(a), shift),
                  BitcastTaggedNonPointerBitsToWord(SmiConstant(-1))));
    } else {
      ...
      return BitcastWordToTaggedSigned(
          WordAnd(ChangeInt32ToIntPtr(Word32Shr(
                      TruncateWordToInt32(BitcastTaggedNonPointerBitsToWord(a)),
                      shift)),
                  BitcastTaggedNonPointerBitsToWord(SmiConstant(-1))));
    }
  }
  ...
  TNode<Smi> SmiSar(TNode<Smi> a, int shift) {
    if (kTaggedSize == kInt64Size) {
      return BitcastWordToTaggedSigned(
          WordAnd(WordSar(BitcastTaggedNonPointerBitsToWord(a), shift),
                  BitcastTaggedNonPointerBitsToWord(SmiConstant(-1))));
    } else {
      // For pointer compressed Smis, we want to make sure that we truncate to
      // int32 before shifting, to avoid the values of the top 32-bits from
      ...
      return BitcastWordToTaggedSigned(
          WordAnd(ChangeInt32ToIntPtr(Word32Sar(
                      TruncateWordToInt32(BitcastTaggedNonPointerBitsToWord(a)),
                      shift)),
                  BitcastTaggedNonPointerBitsToWord(SmiConstant(-1))));
    }
...
#define SMI_COMPARISON_OP(SmiOpName, IntPtrOpName, Int32OpName)         \
  TNode<BoolT> SmiOpName(TNode<Smi> a, TNode<Smi> b) {                  \
    if (kTaggedSize == kInt64Size) {                                    \
      return IntPtrOpName(BitcastTaggedNonPointerBitsToWord(a),         \
                          BitcastTaggedNonPointerBitsToWord(b));        \
    } else {                                                            \
...
#undef BINT_COMPARISON_OP
  ...
  TNode<BoolT> IsSetSmi(SloppyTNode<Smi> smi, int untagged_mask) {
    ...
    return WordNotEqual(WordAnd(BitcastTaggedNonPointerBitsToWord(smi),
