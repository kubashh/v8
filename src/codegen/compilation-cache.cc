...
void CompilationCacheScript::Age() {
  ...
  for (InternalIndex entry : table.IterateEntries()) {
    ...
    Object value = table.FirstValueAt(entry);
...
void CompilationCacheEval::Age() {
  ...
  for (InternalIndex entry : table.IterateEntries()) {
    ...
    if (key.IsNumber(isolate())) {
      ...
      const int new_count = Smi::ToInt(table.FirstValueAt(entry)) - 1;
      if (new_count == 0) {
        table.RemoveEntry(entry);
      } else {
        DCHECK_GT(new_count, 0);
        table.SetFirstValueAt(entry, Smi::FromInt(new_count),
                              SKIP_WRITE_BARRIER);
      }
    } else {
      ...
      SharedFunctionInfo info =
          SharedFunctionInfo::cast(table.FirstValueAt(entry));
...
CompilationCacheScript::LookupResult CompilationCacheScript::Lookup(
  ...
  // into the caller's handle scope.
  {
    ...
    if (probe.ToHandle(&script)) {
      ...
      if (HasOrigin(isolate(), script, script_details)) {
        result.script = scope.CloseAndEscape(script);
      }
    }
  }
  ...
  if (result.script.ToHandle(&script)) {
    ...
    if (script->TryGetRootSharedFunctionInfo(&sfi, isolate())) {
      result.shared = sfi;
      result.is_compiled_scope = sfi->is_compiled_scope(isolate());
      if (result.is_compiled_scope.is_compiled()) {
