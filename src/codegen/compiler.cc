  ...
  if (mode == ConcurrencyMode::kConcurrent) {
    if (GetOptimizedCodeLater(std::move(job), isolate, compilation_info,
      ...
      if (osr_offset != BytecodeOffset::None()) {
        ...
        compilation_info->set_osr_level(osr_level);
        if (FLAG_trace_osr) {
          CodeTracer::Scope scope(isolate->GetCodeTracer());
...
MaybeHandle<CodeT> Compiler::GetOptimizedCodeForOSR(Isolate* isolate,
  ...
  DCHECK(0 <= osr_level && osr_level <= AbstractCode::kMaxLoopNestingMarker);
  ...
  // plan to remove
  if (isolate->concurrent_osr_enabled() && function->IsInOptimizationQueue()) {
    if (FLAG_trace_osr) {
      CodeTracer::Scope scope(isolate->GetCodeTracer());
      PrintF(scope.file(),
             "[OSR - One compilation in queue, won't start another job "
  ...
  return GetOptimizedCode(
      ...
      GetOptimizedCodeResultHandling::kDefault, osr_level);
}
    ...
    if (shared->optimization_disabled()) {
      ...
      if (V8_LIKELY(use_result)) {
        ...
        if (!compilation_info->is_osr()) {
          ...
          int osr_level = compilation_info->osr_level();
          shared->GetBytecodeArray(isolate).set_osr_loop_nesting_level(
              std::min({osr_level + 1, AbstractCode::kMaxLoopNestingMarker}));
        }
      }
