...
MaybeHandle<SharedFunctionInfo> GetSharedFunctionInfoForScriptImpl(
  ...
  if (use_compilation_cache) {
    ...
    MaybeHandle<HeapObject> maybe_shared_or_script =
        compilation_cache->LookupScript(source, script_details, language_mode);
    Handle<HeapObject> shared_or_script;
    if (maybe_shared_or_script.ToHandle(&shared_or_script)) {
      if (shared_or_script->IsSharedFunctionInfo(isolate)) {
        maybe_result = Handle<SharedFunctionInfo>::cast(shared_or_script);
      } else {
        maybe_script = Handle<Script>::cast(shared_or_script);
      }
    }
    if (!maybe_result.is_null()) {
...
Compiler::GetSharedFunctionInfoForStreamedScript(
  ...
  CompilationCache* compilation_cache = isolate->compilation_cache();
  {
    ...
    // SharedFunctionInfo. For now, we just ignore it and create a new Script.
    Handle<HeapObject> shared_or_script;
    if (maybe_shared_or_script.ToHandle(&shared_or_script)) {
      if (shared_or_script->IsSharedFunctionInfo(isolate)) {
        maybe_result = Handle<SharedFunctionInfo>::cast(shared_or_script);
