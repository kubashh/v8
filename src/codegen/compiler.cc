...
// static
void Compiler::LogFunctionCompilation(Isolate* isolate, CodeEventListener::LogEventsAndTags tag,
...
void RecordUnoptimizedFunctionCompilation(
  ...
  Compiler::LogFunctionCompilation(isolate, tag, shared, script, abstract_code, false,
                         time_taken_ms);
}
...
MaybeHandle<JSFunction> Compiler::GetFunctionFromEval(
  ...
  if (eval_result.has_shared()) {
    ...
    result = Factory::JSFunctionBuilder{isolate, shared_info, context}
                 .set_allocation_type(AllocationType::kYoung)
                 .Build();
    // TODO(mythria): we don't have a compiled SFI here. I don't think we should
    // be initializing a feedback cell here. This was a bug, not sure why it
    // didn't fail earlier. It will be initialized when SFI get's compiled.
    // JSFunction::InitializeFeedbackCell(result, &is_compiled_scope, true);
    if (allow_eval_cache) {
