...
MaybeHandle<SharedFunctionInfo> CompileToplevel(
  ...
  event.script_id = parse_info->flags().script_id();
  event.is_toplevel = parse_info->flags().is_toplevel();
  event.is_module = parse_info->flags().is_module();
  auto context_id = v8::metrics::Recorder::ContextId::Empty();
