  ...
  static V8_WARN_UNUSED_RESULT MaybeHandle<Code> Get(
    ...
    if (IsOSR(osr_offset)) {
      ...
      if (maybe_code.has_value()) {
        ...
        // In case of OSR Tf code can be used interchangeably instead of Maglev.
        // This mainly happens as an artifact of our OSR from Maglev to TF
        // implementation.
...
MaybeHandle<Code> GetOrCompileOptimized(
  ...
  if (OptimizedCodeCache::Get(isolate, function, osr_offset, code_kind)
    ...
    if (IsOSR(osr_offset) && !IsInProgress(function->osr_tiering_state())) {
