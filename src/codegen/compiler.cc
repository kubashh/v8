...
bool BackgroundCompileTask::FinalizeFunction(
  ...
  // Move the compiled data from the placeholder SFI back to the real SFI. Make
  // sure however to preserve the disable optimization reason, if any, from the
  // current SFI, in case it was changed after the task was posted..
  BailoutReason disabled_optimization_reason =
      input_shared_info->disabled_optimization_reason();
  input_shared_info->CopyFrom(*result);
  if (disabled_optimization_reason != BailoutReason::kNoReason) {
    input_shared_info->DisableOptimization(disabled_optimization_reason);
  }
...
bool Compiler::CollectSourcePositions(Isolate* isolate,
  ...
  // This is a reparse, so don't spawn parallel tasks.
