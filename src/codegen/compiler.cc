...
MaybeHandle<SharedFunctionInfo> CompileScriptOnMainThread(
    ...
    NativesFlag natives, Isolate* isolate, MaybeHandle<Script> maybe_script,
    IsCompiledScope* is_compiled_scope, ParseInfo* parse_info) {
  Handle<Script> script;
...
MaybeHandle<SharedFunctionInfo> CompileScriptOnMainThread(
  ...
  return CompileScriptOnMainThread(source, script_details, natives, isolate,
    ...
    if (v8_flags.stress_background_compile &&
      ...
      if (compile_options == ScriptCompiler::kConsumeCompileHints) {
        maybe_result = CompileScriptOnMainThread(
            ...
            maybe_script, &is_compiled_scope,
            CompileHints::Deserialize(cached_data));
      } else {
        maybe_result = CompileScriptOnMainThread(
            flags, source, script_details, natives, extension, isolate,
            maybe_script, &is_compiled_scope);
