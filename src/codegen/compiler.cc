...
void BackgroundCompileTask::Run() {
  ...
  if (!result.is_null()) outer_function_sfi_ = *result;
  script_ = *script;
...
class DummySourceStream : public v8::ScriptCompiler::ExternalSourceStream {
 public:
  DummySourceStream(Handle<String> source, Isolate* isolate) : done_(false) {
    source_buffer_ =
        source->ToCString(ALLOW_NULLS, FAST_STRING_TRAVERSAL, &source_length_);
  }
...
class BackgroundCompileThread : public base::Thread {
 public:
  BackgroundCompileThread(Isolate* isolate, Handle<String> source,
                          ScriptCompiler::CompileOptions compile_options)
      : base::Thread(
            base::Thread::Options("BackgroundCompileThread", 2 * i::MB)),
        source_(source),
...
MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScript(
  ...
  if (maybe_result.is_null()) {
    // No cache entry found compile the script. If the
    // --stress-background-compile flag is set, do the actual compilation on a
    // background thread, and wait for its result.
    if (FLAG_stress_background_compile && !extension &&
      ...
      // TODO(leszeks): Compare the results of the two compiles.
      {
        ...
        USE(CompileScriptOnMainThread(flags, source, script_details,
                                      origin_options, NOT_NATIVES_CODE, nullptr,
                                      isolate, &inner_is_compiled_scope));
      }
      // Join with background thread and finalize compilation.
      background_compile_thread.Join();
      maybe_result = Compiler::GetSharedFunctionInfoForStreamedScript(
...
Compiler::GetSharedFunctionInfoForStreamedScript(
  ...
  if (maybe_result.is_null()) {
    if (task->finalize_on_background_thread()) {
      ...
      if (!task->outer_function_sfi().is_null()) {
        // Off-thread parse, compile and finalization has succeeded - grab the
        // finalized toplevel function's handle.
        maybe_result = handle(task->outer_function_sfi(), isolate);
      }
      Handle<Script> script(task->script(), isolate);
      task->pending_error_handler()->FixUpHandlesAfterOffThreadFinalization(
          isolate);
