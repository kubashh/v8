...
MaybeHandle<SharedFunctionInfo> CompileScriptOnBothBackgroundAndMainThread(
    ...
    Isolate* isolate, MaybeHandle<Script> maybe_script,
    IsCompiledScope* is_compiled_scope) {
  ...
  // In parallel, compile on the main thread to flush out any data races.
  {
    ...
    main_thread_maybe_result = CompileScriptOnMainThread(
        ...
        maybe_script, &inner_is_compiled_scope);
...
MaybeHandle<SharedFunctionInfo> GetSharedFunctionInfoForScriptImpl(
  ...
  if (use_compilation_cache) {
    ...
    MaybeHandle<SharedFunctionInfo> maybe_uncompiled_result;
    if (Handle<Script> script; maybe_script.ToHandle(&script)) {
      if (Handle<SharedFunctionInfo> sfi;
          script->TryGetRootSharedFunctionInfo(&sfi, isolate)) {
        is_compiled_scope = sfi->is_compiled_scope(isolate);
        if (is_compiled_scope.is_compiled()) {
          maybe_result = sfi;
        } else {
          maybe_uncompiled_result = sfi;
        }
      }
    }
    if (!maybe_result.is_null()) {
    ...
    } else if (Handle<SharedFunctionInfo> uncompiled_result;
               maybe_uncompiled_result.ToHandle(&uncompiled_result)) {
      // There is an uncompiled root SharedFunctionInfo. This case should be
      // rare; it requires that something other than the compilation cache was
      // retaining the root SharedFunctionInfo after it aged out and had its
      // bytecode flushed. The simplest option is to just return the uncompiled
      // SharedFunctionInfo, which will automatically compile when called.
      maybe_result = maybe_uncompiled_result;

      // Put the result back into the cache, assuming it will get compiled soon.
      // If it is not compiled before the next marking phase starts, it will be
      // evicted again.
      compilation_cache->PutScript(source, language_mode, uncompiled_result);
    }
  }
  ...
  if (maybe_result.is_null()) {
    ...
    if (FLAG_stress_background_compile &&
      ...
      maybe_result = CompileScriptOnBothBackgroundAndMainThread(
          source, script_details, isolate, maybe_script, &is_compiled_scope);
    } else {
      UnoptimizedCompileFlags flags =
...
Compiler::GetSharedFunctionInfoForStreamedScript(
  ...
  CompilationCache* compilation_cache = isolate->compilation_cache();
  {
    ...
    MaybeHandle<Script> maybe_script = compilation_cache->LookupScript(
