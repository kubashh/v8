...
void ConstantPool::EmitAndClear(Jump require_jump) {
  ...
  if (require_alignment == Alignment::kRequired) assm_->Align(8);
  EmitEntries();
  assm_->RecordComment("]");
...
bool ConstantPool::ShouldEmitNow(Jump require_jump) const {
  ...
  size_t pool_end_32 = assm_->pc_offset() + worst_case_size;
...
int ConstantPool::ComputeSize(Jump require_jump,
  ...
  size_t size_after_marker =
      Entry32Count() * 4 + alignment + Entry64Count() * 8;
...
Alignment ConstantPool::IsAlignmentRequiredIfEmittedAt(Jump require_jump,
                                                       int pc_offset) const {
  int size_up_to_marker = PrologueSize(require_jump);
  if (Entry64Count() != 0 && !IsAligned(pc_offset + size_up_to_marker, 8)) {
...
bool ConstantPool::IsInImmRangeIfEmittedAt(int pc_offset) {
  ...
  size_t last_entry_32 =
      pc_offset + ComputeSize(Jump::kRequired, require_alignment) - 4;
