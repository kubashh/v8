...
RegisterConfiguration::RegisterConfiguration(
    ...
    const int* allocatable_simd128_codes_if_independent)
  ...
  if (fp_aliasing_kind_ == AliasingKind::COMBINE) {
  ...
  } else if (fp_aliasing_kind_ == AliasingKind::OVERLAP) {
    ...
    DCHECK_EQ(fp_aliasing_kind_, AliasingKind::INDEPENDENT);
    DCHECK_NE(allocatable_simd128_codes_if_independent, nullptr);
    ...
    for (int i = 0; i < num_allocatable_simd128_registers; i++) {
      allocatable_simd128_codes_[i] =
          allocatable_simd128_codes_if_independent[i];
    }
...
int RegisterConfiguration::GetAliases(MachineRepresentation rep, int index,
  ...
  DCHECK(fp_aliasing_kind_ == AliasingKind::COMBINE);
