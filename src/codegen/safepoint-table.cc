...
void SafepointTableBuilder::Emit(Assembler* assembler, int tagged_slots_size) {
  ...
  auto value_to_bytes = [](int value) {
    ...
    for (int bytes = 1;; ++bytes) {
      DCHECK_GE(4, bytes);
      // Look at the MSB that would be included plus all bits before that.
      int sign_bits = value >> (8 * bytes - 1);
      if (sign_bits == 0 || sign_bits == -1) return bytes;
    }
  };
