...
// Mimics the store in generated code by having a store to the same address,
// with the same value and a specificed memory order. This is done in order for
// TSAN to see these stores from generated code.
...
void tsan_store_common(Address addr, int64_t value,
                       std::memory_order memory_order) {
#if V8_TARGET_ARCH_X64
  std::atomic_store_explicit(
      base::helper::to_std_atomic(reinterpret_cast<AtomicT*>(addr)),
      static_cast<AtomicT>(value), memory_order);
#else
  UNREACHABLE();
#endif  // V8_TARGET_ARCH_X64
}

void tsan_store_8_bits(Address addr, int64_t value, std::memory_order order) {
  tsan_store_common<base::Atomic8>(addr, value, order);
}

void tsan_store_16_bits(Address addr, int64_t value, std::memory_order order) {
  tsan_store_common<base::Atomic16>(addr, value, order);
}

void tsan_store_32_bits(Address addr, int64_t value, std::memory_order order) {
  tsan_store_common<base::Atomic32>(addr, value, order);
}

void tsan_store_64_bits(Address addr, int64_t value, std::memory_order order) {
  tsan_store_common<base::Atomic64>(addr, value, order);
}

template <typename AtomicT>
AtomicT tsan_load_common(Address addr, std::memory_order order) {
#if V8_TARGET_ARCH_X64
  return std::atomic_load_explicit(
      base::helper::to_std_atomic_const(reinterpret_cast<AtomicT*>(addr)),
      order);
#else
  UNREACHABLE();
#endif  // V8_TARGET_ARCH_X64
...
base::Atomic32 tsan_load_32_bits(Address addr, int64_t value,
                                 std::memory_order order) {
  return tsan_load_common<base::Atomic32>(addr, order);
}

base::Atomic64 tsan_load_64_bits(Address addr, int64_t value,
                                 std::memory_order order) {
  return tsan_load_common<base::Atomic64>(addr, order);
}

}  // namespace
#endif  // V8_IS_TSAN

IF_TSAN(FUNCTION_REFERENCE, tsan_store_function_8_bits, tsan_store_8_bits)
IF_TSAN(FUNCTION_REFERENCE, tsan_store_function_16_bits, tsan_store_16_bits)
IF_TSAN(FUNCTION_REFERENCE, tsan_store_function_32_bits, tsan_store_32_bits)
IF_TSAN(FUNCTION_REFERENCE, tsan_store_function_64_bits, tsan_store_64_bits)
IF_TSAN(FUNCTION_REFERENCE, tsan_load_function_32_bits, tsan_load_32_bits)
IF_TSAN(FUNCTION_REFERENCE, tsan_load_function_64_bits, tsan_load_64_bits)
