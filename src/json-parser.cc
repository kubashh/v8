...
Handle<Object> JsonParser<Char>::BuildJsonObject(
  ...
  if (cont.elements > 0) {
    ...
    if (ShouldConvertToSlowElements(cont.elements, cont.max_index + 1)) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        uint32_t index = property.string.index;
        Handle<Object> value = property.value;
        elms->set(static_cast<int>(index), *value);
      }
      elements = elms;
    }
  }
  ...
  for (i = 0; i < length; i++) {
    ...
    if (!value->FitsRepresentation(expected_representation)) break;
  ...
  for (int j = 0; j < i; j++) {
    ...
    JSObject::cast(*object)->WriteToField(descriptor, details, *property.value);
    descriptor++;
  }
...
Handle<Object> JsonParser<Char>::BuildJsonArray(
  ...
  for (size_t i = start; i < element_stack.size(); i++) {
    if (element_stack[i]->IsHeapNumber()) {
      kind = PACKED_DOUBLE_ELEMENTS;
    } else if (element_stack[i]->IsHeapObject()) {
  ...
  if (kind == PACKED_DOUBLE_ELEMENTS) {
    ...
    for (int i = 0; i < length; i++) {
      elements->set(i, *element_stack[start + i]);
    }
  }
  return array;
}
    ...
    while (true) {
      ...
      switch (peek()) {
        ...
        case JsonToken::EOS:
          ...
          cont.type = JsonContinuation::kFail;
...
Handle<String> JsonParser<Char>::DecodeString(
    ...
    Handle<String> hint) {
  {
    ...
    if (!hint.is_null() && Matches(data, hint)) {
      DCHECK(string.needs_internalization);
      return hint;
    }
  }

  if (string.needs_internalization) {
...
Handle<String> JsonParser<Char>::MakeString(const JsonString& string,
  ...
  if (!hint.is_null() && !string.has_escape) {
    Vector<const Char> data(chars_ + string.start, string.length);
    if (Matches(data, hint)) return hint;
  }
...
void JsonParser<Char>::DecodeString(SinkChar* sink, int start, int length) {
  ...
  while (true) {
    ...
    switch (GetEscapeKind(character_scan_flags[*cursor])) {
      ...
      switch (GetEscapeKind(character_scan_flags[c])) {
        ...
        case EscapeKind::kUnicode: {
          ...
          if (value == -1) {
            AllowHeapAllocation allow_before_exception;
            ReportUnexpectedCharacter(CurrentCharacter());
            return JsonString(0, 0, false, false, false);
          }
          bits |= value;
