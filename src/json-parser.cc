...
class JsonStringInternalizationKey final : public StringTableKey {
 public:
  ...
  explicit JsonStringInternalizationKey(const Vector<const Char>& chars,
                                        int hash, LiteralBuffer* literal_buffer)
      : StringTableKey(hash), chars_(chars), literal_buffer_(literal_buffer) {}
  ...
  Handle<String> AsHandle(Isolate* isolate) override {
    if (sizeof(Char) == 1) {
      if (literal_buffer_) {
        ...
        for (Char c : chars_) literal_buffer_->AddChar(c);
        chars_ = Vector<const Char>::cast(literal_buffer_->one_byte_literal());
      }
      return isolate->factory()->NewOneByteInternalizedString(
          Vector<const uint8_t>::cast(chars_), HashField());
    }
    DCHECK_NULL(literal_buffer_);
...
Handle<String> JsonParser<Char>::InternalizeString(
  ...
  JsonStringInternalizationKey<LiteralChar> key(chars, hash, literal_buffer);
