...
static const int kMaxLatin1 = 255;
...
class JsonStringInternalizationKey final : public StringTableKey {
 public:
  // A literal buffer is passed if the underlying chars could move on allocate.
  explicit JsonStringInternalizationKey(const Vector<const Char>& chars,
                                        int hash, bool chars_may_relocate)
      : StringTableKey(hash),
        chars_(chars),
        chars_may_relocate_(chars_may_relocate) {}
  ...
  bool IsMatch(Object other) override {
    ...
    if (s->IsOneByteRepresentation()) {
      if (sizeof(Char) == 2) return false;
      const uint8_t* chars =
          StringShape(s).IsExternal()
              ? CharTraits<uint8_t>::ExternalString::cast(s)->GetChars()
              : CharTraits<uint8_t>::String::cast(s)->GetChars(no_gc);
      return CompareChars(chars, chars_.start(), chars_.length()) == 0;
    }
    const uint16_t* chars =
        StringShape(s).IsExternal()
            ? CharTraits<uint16_t>::ExternalString::cast(s)->GetChars()
            : CharTraits<uint16_t>::String::cast(s)->GetChars(no_gc);
    return CompareChars(chars, chars_.start(), chars_.length()) == 0;
  }
  ...
  Handle<String> AsHandle(Isolate* isolate) override {
    if (sizeof(Char) == 1) {
      if (chars_may_relocate_) {
        // TODO(verwaest): If chars may relocate we probably shouldn't use
        // Vector<>, or a GC-supported vector that can update itself.
        std::unique_ptr<uint8_t> buffer(new uint8_t[chars_.length()]);
        memcpy(buffer.get(), chars_.start(), chars_.length());
        return isolate->factory()->NewOneByteInternalizedString(
            Vector<const uint8_t>(buffer.get(), chars_.length()), HashField());
      }
...
void JsonParser<Char>::SkipWhitespace() {
  ...
  cursor_ = std::find_if(cursor_, end_, [this](Char c) {
    JsonToken current =
        V8_LIKELY(c <= kMaxLatin1) ? one_char_tokens[c] : JsonToken::ILLEGAL;
...
ParseElementResult JsonParser<Char>::ParseElement(
  ...
  uint32_t index = 0;
  {
    ...
    if (*cursor++ != '"') return kElementNotFound;
    cursor_ = cursor;
  }
...
Handle<Object> JsonParser<Char>::ParseJsonNumber(int sign, const Char* start) {
...

  {
    ...
    if (*cursor_ == '0') {
      ...
      Char c = *cursor_;
      if (c <= kMaxLatin1 && IsNumberPart(character_scan_flags[c])) {
        if (V8_UNLIKELY(IsDecimalDigit(*cursor_))) ReportUnexpectedCharacter();
      } else if (sign > 0) {
      ...
      cursor_ = std::find_if(cursor_, end_, [&i, &digits](Char c) {
        if (!IsDecimalDigit(c)) return true;
        i = i * 10 + (c - '0');
        digits++;
        return false;
      });
      if (V8_UNLIKELY(cursor_ == start)) ReportUnexpectedCharacter();
      if ((is_at_end() || *cursor_ > kMaxLatin1 ||
    ...
    cursor_ = std::find_if(cursor_, end_, [](Char c) {
      return !(c <= kMaxLatin1 && IsNumberPart(character_scan_flags[c])) ||
             c == '.';
    });
...
bool Matches(const Vector<const Char>& chars, Handle<String> string) {
  ...
  const Char* string_data =
      StringShape(*string).IsExternal()
          ? CharTraits<Char>::ExternalString::cast(*string)->GetChars()
          : CharTraits<Char>::String::cast(*string)->GetChars(no_gc);
  return CompareChars(chars.start(), string_data, chars.length()) == 0;
}
...
Handle<String> JsonParser<Char>::InternalizeString(
    bool requires_internalization, const Vector<const LiteralChar>& chars,
    bool chars_may_relocate) {
  if (!requires_internalization &&
      chars.length() > kMaxInternalizedStringValueLength) {
    if (sizeof(LiteralChar) == 1) {
      if (chars_may_relocate) {
        // TODO(verwaest): If chars may relocate we probably shouldn't use
        // Vector<>, or a GC-supported vector that can update itself.
        std::unique_ptr<uint8_t> buffer(new uint8_t[chars.length()]);
        memcpy(buffer.get(), chars.start(), chars.length());
        return factory()
            ->NewStringFromOneByte(
                Vector<uint8_t>(buffer.get(), chars.length()), allocation_)
            .ToHandleChecked();
      }
      return factory()
  ...
  uint32_t hash = StringHasher::HashSequentialString(
      chars.start(), chars.length(), hash_seed_);
  JsonStringInternalizationKey<LiteralChar> key(chars, hash,
                                                chars_may_relocate);
    ...
    while (true) {
      ...
      if (*cursor_ == '"') {
        ...
        Handle<String> result =
            Matches(chars, hint)
                ? hint
                : InternalizeString(requires_internalization,
      ...
      if (*cursor_ == '"') {
        ...
        if (literal_buffer_.is_one_byte()) {
          ...
          result = Matches(chars, hint)
                       ? hint
                       : InternalizeString(requires_internalization, chars);
        } else {
