...
Handle<String> JsonParser<Char>::MakeString(
  ...
  AllowHeapAllocation allow_gc;
  DCHECK_IMPLIES(chars_may_relocate_,
                 sizeof(LiteralChar) == 1
                     ? Vector<const uint8_t>::cast(chars).start() ==
                           literal_buffer_.one_byte_literal().start()
                     : Vector<const uint16_t>::cast(chars).start() ==
                           literal_buffer_.two_byte_literal().start());
