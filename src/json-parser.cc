...
class MovingVector final : public Vector<const uint8_t> {
 public:
  MovingVector(Isolate* isolate, Handle<SeqOneByteString> source,
               const uint8_t* cursor, size_t length)
      : Vector<const uint8_t>(cursor, length),
        isolate_(isolate),
        source_(source) {
    DisallowHeapAllocation no_gc;
    start_ = source_->GetChars(no_gc);
  ...
  static void UpdatePointersCallback(v8::Isolate* v8_isolate, v8::GCType type,
                                     v8::GCCallbackFlags flags, void* vector) {
    reinterpret_cast<MovingVector*>(vector)->UpdatePointers();
  }

  void UpdatePointers() {
    DisallowHeapAllocation no_gc;
    const uint8_t* new_start = source_->GetChars(no_gc);
    if (start_ != new_start) {
      size_t position = start() - start_;
      UpdateStart(new_start + position);
      start_ = new_start;
    }
  }
 ...
 private:
  Isolate* isolate_;
  Handle<SeqOneByteString> source_;
    ...
    while (true) {
      ...
      if (*cursor_ == '"') {
        ...
        if (Matches(chars, hint)) {
          ...
          result = InternalizeString(
              requires_internalization,
              MovingVector(isolate_, Handle<SeqOneByteString>::cast(source_),
                           Vector<const uint8_t>::cast(chars).start(),
                           cursor_ - start));
        } else {
          result = InternalizeString(requires_internalization, chars);
        }
        advance();
        return result;
      }
