...
def _should_emit_noicu_and_icu(noicu_srcs, noicu_deps, noicu_includes, icu_srcs, icu_deps, icu_includes):
    return noicu_srcs != [] or noicu_deps != [] or noicu_includes != [] or icu_srcs != [] or icu_deps != [] or icu_includes != []
...
def v8_binary(
    ...
    if _should_emit_noicu_and_icu(noicu_srcs, noicu_deps, noicu_includes, icu_srcs, icu_deps, icu_includes):
        native.cc_binary(
            name = "noicu/" + name,
            srcs = srcs + noicu_srcs,
            deps = deps + noicu_deps + default.deps,
            includes = includes + noicu_includes + default.includes,
        ...
        native.cc_binary(
            ...
            includes = includes + icu_includes + default.includes,
