...
// The conversion routines, UTF8ToUTF16, and
// UTF16ToUTF8, are copied from src/inspector/string-16.cc to avoid depending on
// the entire inspector library. The only adaptation for this file are the
// signature, e.g., usage of std::vector<uint16> directly as the test requires
// it.
...
#incldue "../../../src/base/base-export.h"
...
std::string UTF16ToUTF8(span<uint16_t> in) {
  ...
  const UChar* characters = in.data();
...
std::vector<uint16_t> UTF8ToUTF16(span<uint8_t> in) {
  ...
  std::vector<uint16_t> buffer(in.size());
  UChar* bufferStart = buffer.data();

  UChar* bufferCurrent = bufferStart;
  const char* stringCurrent = reinterpret_cast<const char*>(in.data());
  ...
  return std::vector<uint16_t>(bufferStart, bufferStart + utf16Length);
}
