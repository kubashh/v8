...
class String16 {
 public:
  ...
  static std::vector<uint16_t> fromUTF8(const char* stringStart,
                                        size_t length) {
...
std::string UTF16ToUTF8(span<uint16_t> in) {
  return String16(in.data(), in.size()).utf8();
}

std::vector<uint16_t> UTF8ToUTF16(span<uint8_t> in) {
  return String16::fromUTF8(reinterpret_cast<const char*>(in.data()),
                            in.size());
}
