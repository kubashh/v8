...
TEST(DispatchableTest, SessionIdMustBe7BitUSASCII) {
  ...
  cbor::EncodeString8(SpanFrom("Foo.excuteBar"), &cbor);
...
TEST(DispatchableTest, FaultyCBORTrailingJunk) {
  ...
  Dispatchable dispatchable(SpanFrom(cbor));
  EXPECT_FALSE(dispatchable.ok());
  EXPECT_EQ(DispatchResponse::kParseError, dispatchable.ErrorCode());
  EXPECT_EQ(56u, trailing_junk_pos);
  EXPECT_EQ("CBOR: trailing junk at position 56", dispatchable.ErrorMessage());
}
...
class TestDomain : public DomainDispatcher {
 public:
  TestDomain(FrontendChannel* channel) : DomainDispatcher(channel) {}
