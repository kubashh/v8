...
class Dispatchable {
 public:
  ...
  // (See SendErrorVia).
  DispatchResponse::ErrorCode Code() const;
...
// Given a |dispatchable| which didn't parse correctly (|!dispatchable.ok()|),
// generates an error response or notification (if no call id is available) and
// send it via |channel|.
void SendErrorVia(const Dispatchable& dispatchable, FrontendChannel* channel);

// Helpers for creating error responses and notifications that can be
