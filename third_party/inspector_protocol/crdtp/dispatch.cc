...
DispatchResponse::ErrorCode Dispatchable::Code() const {
...
void SendErrorVia(const Dispatchable& dispatchable, FrontendChannel* channel) {
  if (dispatchable.HasCallId()) {
    channel->SendProtocolResponse(
        dispatchable.CallId(),
        createErrorResponse(dispatchable.CallId(), dispatchable.Code(),
                            dispatchable.ErrorMessage()));
    return;
  }
  channel->SendProtocolNotification(createErrorNotification(
      dispatchable.Code(), dispatchable.ErrorMessage()));
}
