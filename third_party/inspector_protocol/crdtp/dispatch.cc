...
const char kMessageUnknownPropertyError[] =
    "Message may only have 'id', 'method', 'sessionId', and 'params' "
    "properties";
  ...
  while (tokenizer.TokenTag() != cbor::CBORTokenTag::STOP) {
    switch (tokenizer.TokenTag()) {
      ...
      case cbor::CBORTokenTag::STRING8:
        if (SpanEquals(SpanFrom("sessionId"), tokenizer.GetString8())) {
          MaybeParseString8Field(&tokenizer, &session_id_,
                                 kMessageSessionIdError);
          if (!ok())
            return;
          continue;
        }
        if (SpanEquals(SpanFrom("id"), tokenizer.GetString8())) {
        ...
        if (SpanEquals(SpanFrom("method"), tokenizer.GetString8())) {
          MaybeParseString8Field(&tokenizer, &method_, kMessageMethodError);
          if (!ok())
            return;
          continue;
        }
        if (SpanEquals(SpanFrom("params"), tokenizer.GetString8())) {
          // This is only a shallow parse - we extract the raw contents of the
          // field but do not descend farther into it.
          MaybeParseEnvelopeField(&tokenizer, &params_);
          if (!ok())
            return;
          continue;
        }
        error_ = kMessageUnknownPropertyError;
        return;
      default:
...
DispatchResponse Dispatchable::DispatchError() const {
  ...
  if (status_.ok() && error_ == nullptr)
    return DispatchResponse::Success();

  if (error_)
    return DispatchResponse::InvalidRequest(error_);
