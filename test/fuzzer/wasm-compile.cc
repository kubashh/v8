...
class WasmGenerator {
  ...
  void struct_get(DataRange* data) {
    ...
    ZoneVector<uint32_t> field_index = ZoneVector<uint32_t>(builder->zone());
    ZoneVector<uint32_t> struct_index = ZoneVector<uint32_t>(builder->zone());
    ...
    if (static_cast<int>(field_index.size()) == 0) {
      Generate<wanted_kind>(data);
      return;
    }
