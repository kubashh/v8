...
std::vector<ExecutionResult> PerformReferenceRun(
    ...
    CompileTimeImports compile_imports, WasmFeatures enabled_features,
  ...
  FlagScope<bool> no_tierup(&v8_flags.wasm_tier_up, false);
  ...
  MaybeHandle<WasmModuleObject> compiled_module = GetWasmEngine()->SyncCompile(
      isolate, enabled_features, compile_imports, &thrower, wire_bytes);
...
void FuzzIt(base::Vector<const uint8_t> data) {
  ...
  // TODO(mliedtke): Also collect results and compare them (differential
  // fuzzing).
  ...
  for (size_t i = 0; i < num_callees; ++i) {
    ...
    // TODO(mliedtke): The main function has a fixed signature, meaning that it
    // never has any parameter stack slots and only one return value.
    // Instead of tiering up main, there could be an advantage in tiering up
    // the first callee instead (if the deopt isn't at the main function
    // directly).
