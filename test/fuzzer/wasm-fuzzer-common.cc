...
#undef EMPTY_INTERFACE_FUNCTION
  ...
  void I32Const(FullDecoder* decoder, Value* result, int32_t value) {
    result->init_expr = zone_->New<WasmInitExpr>(value);
  }
  ...
  void S128Const(FullDecoder* decoder, Simd128Immediate<validate>& imm,
                 Value* result) {
    result->init_expr = zone_->New<WasmInitExpr>(imm.value);
  }

  void RefNull(FullDecoder* decoder, ValueType type, Value* result) {
    result->init_expr = zone_->New<WasmInitExpr>();
  ...
  void StructNewWithRtt(FullDecoder* decoder,
    ...
    for (size_t i = 0; i < imm.struct_type->field_count(); i++) {
      elements->push_back(std::move(*args[i].init_expr));
    }
    bool nominal = decoder->module_->has_supertype(imm.index);

    if (!nominal) elements->push_back(std::move(*rtt.init_expr));
  ...
  void StructNewDefault(FullDecoder* decoder,
    ...
    *result->init_expr = nominal
                             ? WasmInitExpr::StructNewDefault(imm.index)
                             : WasmInitExpr::StructNewDefaultWithRtt(
                                   zone_, imm.index, std::move(*rtt.init_expr));
  }
  ...
  void ArrayInit(FullDecoder* decoder, const ArrayIndexImmediate<validate>& imm,
    ...
    ZoneVector<WasmInitExpr>* args =
        zone_->New<ZoneVector<WasmInitExpr>>(zone_);
    for (Value expr : elements) args->push_back(std::move(*expr.init_expr));
    bool nominal = decoder->module_->has_supertype(imm.index);

    if (!nominal) args->push_back(std::move(*rtt.init_expr));
    result->init_expr = zone_->New<WasmInitExpr>();
    *result->init_expr = nominal
                             ? WasmInitExpr::ArrayInitStatic(imm.index, args)
  ...
  void RttSub(FullDecoder* decoder, uint32_t type_index, const Value& parent,
    ...
    result->init_expr = zone_->New<WasmInitExpr>();
    *result->init_expr =
        WasmInitExpr::RttSub(zone_, type_index, std::move(*parent.init_expr));
  }
