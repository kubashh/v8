...
CompilationResult CompileStreaming(v8_fuzzer::FuzzerSupport* support,
  ...
  std::weak_ptr<NativeModule> weak_native_module;
  {
    ...
    if (resolver->failed()) {
      result = CompilationResult::ForFailure(
          base::VectorOf(resolver->error_message()));
    } else {
      result =
          CompilationResult::ForSuccess(resolver->native_module()->module());
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // TODO(clemensb): Enable this test later, after other bugs are flushed out.
  ...
  // We shouldn't leave pending exceptions behind.
