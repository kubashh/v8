...
var settings = {
  output: true,
  harness_timeout: {'normal': 10000, 'long': 60000},
  test_timeout: null,
...
function WindowTestEnvironment() {
  ...
  this.message_functions = {
    start: [
      add_start_callback, remove_start_callback,
      function(properties) {
        this_obj._dispatch(
            'start_callback', [properties],
            {type: 'start', properties: properties});
      }
    ],

    test_state: [
      add_test_state_callback, remove_test_state_callback,
      function(test) {
        this_obj._dispatch(
            'test_state_callback', [test],
            {type: 'test_state', test: test.structured_clone()});
      }
    ],
    result: [
      add_result_callback, remove_result_callback,
      function(test) {
    ...
    completion: [
      add_completion_callback, remove_completion_callback,
...
WindowTestEnvironment.prototype._dispatch = function(
    selector, callback_args, message_arg) {
...
WorkerTestEnvironment.prototype.on_tests_ready = function() {
  ...
  add_test_state_callback(function(test) {
    this_obj._dispatch({type: 'test_state', test: test.structured_clone()});
  });
  add_result_callback(function(test) {
    this_obj._dispatch({type: 'result', test: test.structured_clone()});
  });
  add_completion_callback(function(tests, harness_status) {
    this_obj._dispatch({
      type: 'complete',
...
DedicatedWorkerTestEnvironment.prototype =
...
SharedWorkerTestEnvironment.prototype =
    Object.create(WorkerTestEnvironment.prototype);
...
function test(func, name, properties) {
  ...
  if (value !== undefined) {
    var msg = 'Test named "' + test_name + '" inappropriately returned a value';
...
function promise_test(func, name, properties) {
  ...
  tests.promise_tests = tests.promise_tests.then(function() {
    return new Promise(function(resolve) {
      ...
      test.step(function() {
        assert(
            !!promise, 'promise_test', null,
            'test body must return a \'thenable\' object (received ${value})',
            {value: promise});
        assert(
            typeof promise.then === 'function', 'promise_test', null,
...
function promise_rejects_dom(
    test, type, promiseOrConstructor, descriptionOrPromise, maybeDescription) {
...
function EventWatcher(test, watchedNode, eventTypes, timeoutPromise) {
  ...
  var eventHandler = test.step_func(function(evt) {
    assert_true(
        !!waitingFor, 'Not expecting event, but got ' + evt.type + ' event');
    assert_equals(
        evt.type, waitingFor.types[0],
        'Expected ' + waitingFor.types[0] + ' event, but got ' + evt.type +
            ' event instead');
...
function promise_setup(func, maybe_properties) {
  if (typeof func !== 'function') {
    tests.set_status(
        tests.status.ERROR, 'promise_test invoked without a function');
    tests.complete();
    return;
  }
...
function is_node(object) {
  ...
  try {
    var has_node_properties =
        ('nodeType' in object && 'nodeName' in object &&
         'nodeValue' in object && 'childNodes' in object);
  } catch (e) {
  ...
  switch (typeof val) {
      ...
      if (is_node(val)) {
        switch (val.nodeType) {
          ...
          case Node.DOCUMENT_NODE:
            return 'Document node with ' + val.childNodes.length +
                (val.childNodes.length == 1 ? ' child' : ' children');
          case Node.DOCUMENT_TYPE_NODE:
            return 'DocumentType node';
          case Node.DOCUMENT_FRAGMENT_NODE:
            return 'DocumentFragment node with ' + val.childNodes.length +
                (val.childNodes.length == 1 ? ' child' : ' children');
          default:
    ...
    default:
      try {
        ...
        return (
            '[stringifying object threw ' + String(e) + ' with type ' +
            String(typeof e) + ']');
      }
  }
}
...
function assert_true(actual, description) {
  assert(
      actual === true, 'assert_true', description,
...
function assert_false(actual, description) {
  assert(
      actual === false, 'assert_false', description,
...
function assert_equals(actual, expected, description) {
  ...
  assert(
      same_value(actual, expected), 'assert_equals', description,
...
function assert_not_equals(actual, expected, description) {
   ...
   */
  assert(
      !same_value(actual, expected), 'assert_not_equals', description,
...
function assert_in_array(actual, expected, description) {
  assert(
      expected.indexOf(actual) != -1, 'assert_in_array', description,
...
function assert_object_equals(actual, expected, description) {
  assert(
      typeof actual === 'object' && actual !== null, 'assert_object_equals',
      description, 'value is ${actual}, expected object', {actual: actual});
  ...
  function check_equal(actual, expected, stack) {
    ...
    for (p in actual) {
      assert(
          expected.hasOwnProperty(p), 'assert_object_equals', description,
      ...
      if (typeof actual[p] === 'object' && actual[p] !== null) {
        ...
        assert(
            same_value(actual[p], expected[p]), 'assert_object_equals',
            description, 'property ${p} expected ${expected} got ${actual}',
            {p: p, expected: expected[p], actual: actual[p]});
      }
    }
    for (p in expected) {
      assert(
          actual.hasOwnProperty(p), 'assert_object_equals', description,
...
function assert_array_equals(actual, expected, description) {
  ...
  function shorten_array(arr, offset = 0) {
    // Make ", …" only show up when it would likely reduce the length, not
    // accounting for fonts.
...
function assert_array_approx_equals(actual, expected, epsilon, description) {
   ...
   */
  assert(
      actual.length === expected.length, 'assert_array_approx_equals',
      description, 'lengths differ, expected ${expected} got ${actual}',
  ...
  for (var i = 0; i < actual.length; i++) {
    ...
    assert(
        typeof actual[i] === 'number', 'assert_array_approx_equals',
        description,
...
function assert_approx_equals(actual, expected, epsilon, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_approx_equals', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      Math.abs(actual - expected) <= epsilon, 'assert_approx_equals',
...
function assert_less_than(actual, expected, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_less_than', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual < expected, 'assert_less_than', description,
...
function assert_greater_than(actual, expected, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_greater_than', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual > expected, 'assert_greater_than', description,
...
function assert_between_exclusive(actual, lower, upper, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_between_exclusive', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual > lower && actual < upper, 'assert_between_exclusive', description,
...
function assert_less_than_equal(actual, expected, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_less_than_equal', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual <= expected, 'assert_less_than_equal', description,
...
function assert_greater_than_equal(actual, expected, description) {
   ...
   */
  assert(
      typeof actual === 'number', 'assert_greater_than_equal', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual >= expected, 'assert_greater_than_equal', description,
...
function assert_between_inclusive(actual, lower, upper, description) {
  /*
   * Test if a primitive number is between to two others or equal to either of
   * them
   */
  assert(
      typeof actual === 'number', 'assert_between_inclusive', description,
      'expected a number but got a ${type_actual}',
      {type_actual: typeof actual});

  assert(
      actual >= lower && actual <= upper, 'assert_between_inclusive',
      description,
...
function assert_regexp_match(actual, expected, description) {
   ...
   */
  assert(
      expected.test(actual), 'assert_regexp_match', description,
      'expected ${expected} but got ${actual}',
      {expected: expected, actual: actual});
}
...
function assert_class_string(object, class_string, description) {
  ...
  assert(
      same_value(actual, expected), 'assert_class_string', description,
      'expected ${expected} but got ${actual}',
      {expected: expected, actual: actual});
}
...
function assert_own_property(object, property_name, description) {
  assert(
      object.hasOwnProperty(property_name), 'assert_own_property', description,
...
function assert_not_own_property(object, property_name, description) {
  assert(
      !object.hasOwnProperty(property_name), 'assert_not_own_property',
      description, 'unexpected property ${p} is found on object',
      {p: property_name});
}
...
function _assert_inherits(name) {
  return function(object, property_name, description) {
    ...
    assert(
        'hasOwnProperty' in object, name, description,
        'provided value is an object but has no hasOwnProperty method');

    assert(
        !object.hasOwnProperty(property_name), name, description,
        'property ${p} found on object expected in prototype chain',
        {p: property_name});

    assert(
        property_name in object, name, description,
        'property ${p} not found in prototype chain', {p: property_name});
  };
}
...
function assert_readonly(object, property_name, description) {
  ...
  try {
    ...
    object[property_name] =
        initial_value + 'a';  // XXX use some other value here?
    assert(
        same_value(object[property_name], initial_value), 'assert_readonly',
        description, 'changing property ${p} succeeded', {p: property_name});
  } finally {
 ...
 * @param {string} description Error description for the case that the error is
 *     not thrown.
 */
...
function assert_throws_js_impl(constructor, func, description, assertion_type) {
  try {
    func.call(this);
    assert(
        false, assertion_type, description, '${func} did not throw',
        {func: func});
  } catch (e) {
    ...
    assert(
        typeof e === 'object', assertion_type, description,
        '${func} threw ${e} with type ${type}, not an object',
        {func: func, e: e, type: typeof e});

    assert(
        e !== null, assertion_type, description,
        '${func} threw null, not an object', {func: func});

    // Basic sanity-check on the passed-in constructor
    assert(
        typeof constructor == 'function', assertion_type, description,
        '${constructor} is not a constructor', {constructor: constructor});
    var obj = constructor;
    while (obj) {
    ...
    assert(
        obj != null, assertion_type, description,
        '${constructor} is not an Error subtype', {constructor: constructor});
...
function assert_throws_dom(
    type, funcOrConstructor, descriptionOrFunc, maybeDescription) {
  ...
  assert_throws_dom_impl(
      type, func, description, 'assert_throws_dom', constructor)
}
...
function assert_throws_dom_impl(
    type, func, description, assertion_type, constructor) {
  try {
    func.call(this);
    assert(
        false, assertion_type, description, '${func} did not throw',
    ...
    assert(
        typeof type == 'number' || typeof type == 'string', assertion_type,
        description, '${type} is not a number or string', {type: type});
    ...
    if (typeof type === 'number') {
      if (type === 0) {
        ...
        throw new AssertionError(
            'Test bug: unrecognized DOMException code "' + type +
      ...
      if (!(name in name_code_map)) {
        throw new AssertionError(
            'Test bug: unrecognized DOMException code name or name "' + type +
            '" passed to assert_throws_dom()');
      }
    ...
    for (var prop in required_props) {
      assert(
          prop in e && e[prop] == required_props[prop], assertion_type,
          description,
          '${func} threw ${e} that is not a DOMException ' + type +
    ...
    assert(
        e.constructor === constructor, assertion_type, description,
        '${func} threw an exception from the wrong global', {func});
  }
}
...
function assert_throws_exactly(exception, func, description) {
  assert_throws_exactly_impl(
      exception, func, description, 'assert_throws_exactly');
}
...
function assert_throws_exactly_impl(
    exception, func, description, assertion_type) {
  try {
    func.call(this);
    assert(
        false, assertion_type, description, '${func} did not throw',
    ...
    assert(
        same_value(e, exception), assertion_type, description,
 ...
 * @param {string} description Error description for the case that the condition
 *     is not truthy.
 */
...
/**
 * Assert that an optional feature is implemented, based on a 'truthy'
 * condition.
 *
 * This function should be used to early-exit from tests in which there is
...
Test.prototype.step = function(func, this_obj) {
  ...
  // If we don't get a result before the harness times out that will be a test
  // timeout
  ...
  try {
    ...
    var status = e instanceof OptionalFeatureUnsupportedError ?
...
function cleanup_done(test, error_count, bad_value_count) {
  if (error_count || bad_value_count) {
    ...
    tests.status.status = tests.status.ERROR;
    tests.status.message = 'Test named \'' + test.name + '\' specified ' +
...
RemoteTest.prototype.structured_clone = function() {
  ...
  Object.keys(this).forEach((function(key) {
                              var value = this[key];
                              // `RemoteTest` instances are responsible for
                              // managing their own "done" callback functions,
                              // so those functions are not relevant in other
                              // execution contexts. Because of this (and
                              // because Function values cannot be serialized
                              // for cross-realm transmittance), the property
                              // should not be considered when cloning
                              // instances.
...
RemoteTest.prototype.done =
  ...
  forEach(this._done_callbacks, function(callback) {
    ...
    function RemoteContext(remote, message_target, message_filter) {
      ...
      // remote_done() is seen prevents the remote object and its message
      // channel from going away before all the messages are dispatched.
      this.remote = remote;
...
RemoteContext.prototype.report_uncaught = function(error) {
  ...
  // error state.
  tests.set_status(
      tests.status.ERROR, 'Error in remote' + filename + ': ' + message,
      error.stack);
};
...
TestsStatus.prototype.structured_clone = function() {
  if (!this._structured_clone) {
    ...
    msg = msg ? String(msg) : msg;
    this._structured_clone = merge(
        {status: this.status, message: msg, stack: this.stack},
        TestsStatus.statuses);
  }
  return this._structured_clone;
};
...
function Tests() {
  ...
  this.phases =
      {INITIAL: 0, SETUP: 1, HAVE_TESTS: 2, HAVE_RESULTS: 3, COMPLETE: 4};
  this.phase = this.phases.INITIAL;
...
Tests.prototype.setup = function(func, properties) {
  ...
  if (func) {
    try {
      ...
      this.status.status = e instanceof OptionalFeatureUnsupportedError ?
...
Tests.prototype.all_done = function() {
  return this.tests.length > 0 && test_environment.all_loaded &&
      ...
      !this.processing_callbacks && !this.pending_remotes.some(function(w) {
        return w.running;
      });
};
...
Tests.prototype.notify_complete = function() {
  ...
  if (this.status.status === null) {
    ...
    if (duplicates.length) {
      this.status.status = this.status.ERROR;
      this.status.message = duplicates.length + ' duplicate test name' +
          (duplicates.length > 1 ? 's' : '') + ': "' + duplicates.join('", "') +
          '"';
    } else {
...
Output.prototype.setup = function(properties) {
  ...
  this.enabled = this.enabled &&
      (properties.hasOwnProperty('output') ? properties.output :
                                             settings.output);
};
...
Output.prototype.resolve_log = function() {
  ...
  if (!node) {
    ...
    node =
        output_document.createElementNS('http://www.w3.org/1999/xhtml', 'div');
    node.id = 'log';
    if (output_document.body) {
      ...
      var is_html =
          (root && root.namespaceURI == 'http://www.w3.org/1999/xhtml' &&
           root.localName == 'html');
      var is_svg =
      ...
      if (is_svg) {
        ...
        root.appendChild(output_document.createElementNS(
...
Output.prototype.show_status = function() {
  ...
  if (this.output_node) {
    if (done_count < 100 || (done_count < 1000 && done_count % 100 === 0) ||
        done_count % 1000 === 0) {
      this.output_node.textContent = 'Running, ' + done_count + ' complete, ' +
...
Output.prototype.show_results = function(tests, harness_status) {
  ...
  status_text_harness[harness_status.PRECONDITION_FAILED] =
  ...
  status_text[Test.prototype.PRECONDITION_FAILED] =
      'Optional Feature Unsupported';
  ...
  forEach(
      ...
      function(element) {
        on_event(element, 'click', function(e) {
          ...
          var style_element =
              output_document.querySelector('style#hide-' + result_class);
  ...
  for (var i = 0; i < tests.length; i++) {
    html += '<tr class="' +
        escape_html(status_class(status_text[tests[i].status])) + '"><td>' +
        escape_html(status_text[tests[i].status]) + '</td><td>' +
        escape_html(tests[i].name) + '</td><td>' +
        (assertions ? escape_html(get_assertion(tests[i])) + '</td><td>' : '') +
        escape_html(tests[i].message ? tests[i].message : ' ') +
        (tests[i].stack ? '<pre>' + escape_html(tests[i].stack) + '</pre>' :
                          '') +
        '</td></tr>';
  }
  html += '</tbody></table>';
 ...
 *
 * the children can either be strings (which act like text nodes), other
 * templates or functions (see below)
 *
 ...
 * foo.
 *
 * It is possible to embed logic in templates by using a function in a place
 * where a node would usually go. The function must either return part of a
 * template or null.
 *
 * In cases where a set of nodes are required as output rather than a single
 * node with children it is possible to just use a list [node1, node2, node3]
 *
 * Usage:
 *
...
AssertionError.prototype.get_stack =
    function() {
  ...
  // Create a pattern to match stack frames originating within testharness.js.
  // These include the script URL, followed by the line/col (e.g.,
  // '/resources/testharness.js:120:21'). Escape the URL per
  ...
  // Some browsers include a preamble that specifies the type of the error
  // object.  Skip this by advancing until we find the first stack frame
  // originating from testharness.js.
  var i = 0;
  while (!re.test(lines[i]) && i < lines.length) {
    i++;
  }

  // Then skip the top frames originating from testharness.js to begin the stack
  // at the test code.
  while (re.test(lines[i]) && i < lines.length) {
    i++;
  }

  // Paranoid check that we didn't skip all frames.  If so, return the original
  // stack unmodified.
  if (i >= lines.length) {
    return stack;
  }
  ...
  return lines.slice(i).join('\n');
    ...
    } OptionalFeatureUnsupportedError.prototype =
        Object.create(AssertionError.prototype);
...
/**
 * Returns the 'src' URL of the first <script> tag in the page to include the
 * file 'testharness.js'.
 */
...
function get_title() {
  ...
  if ('location' in global_scope) {
    return location.pathname.substring(
        location.pathname.lastIndexOf('/') + 1, location.pathname.indexOf('.'));
  }
  return 'Untitled';
}
...
function supports_post_message(w) {
  ...
  // windows or tabs, you can't infer cross-origin communication from the
  // presence of postMessage on the current window object only.
  //
...
if (global_scope.addEventListener) {
  var error_handler = function(error, message, stack) {
    ...
    if (tests.file_is_test) {
      ...
      var status = optional_unsupported ? tests.status.PRECONDITION_FAILED :
                                          tests.status.ERROR;
