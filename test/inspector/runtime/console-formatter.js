...
InspectorTest.runAsyncTestSuite([
  async function testFloatFormatter() {
    ...
    await test(`console.trace('%f', {[Symbol.toPrimitive]() { return 2.78; }})`);
  ...
  async function testStringFormatter() {
    ...
    await test(`console.trace('%s', {[Symbol.toPrimitive]() { return true; }})`);
  ...
  async function testOtherFormatters() {
    ...
    await test(`console.log('We have reached 100% of our users', 'with this!')`);
    await Promise.all([
      Protocol.Runtime.discardConsoleEntries(),
      Protocol.Runtime.disable(),
    ]);
  ...
  async function testMultipleFormatters() {
    ...
    await test(`console.error('%c%i%c%s', 'color:red', 42, 'color:green', 'Message!')`);
    await test(`console.info('%s', {toString() { return '%i% %s %s'; }}, {toString() { return '100'; }}, 'more', 'arguments')`);
    await test(`console.log('%s %s', {toString() { return 'Too %s %s'; }}, 'many', 'specifiers')`);
    await test(`console.trace('%s %f', {toString() { return '%s'; }}, {[Symbol.toPrimitive]() { return 'foo'; }}, 1, 'Test')`);
    await Promise.all([
      Protocol.Runtime.discardConsoleEntries(),
      Protocol.Runtime.disable(),
    ]);
  ...
  async function testAssert() {
    await Protocol.Runtime.enable();
    await test(`console.assert(true, '%s', {toString() { throw new Error(); }})`);
    await test(`console.assert(false, '%s %i', {toString() { return '%s'; }}, {[Symbol.toPrimitive]() { return 1; }}, 1, 'Test')`);
    await test(`console.assert(false, '%s', {toString() { throw new Error(); }})`);
    await Promise.all([
      Protocol.Runtime.discardConsoleEntries(),
      Protocol.Runtime.disable(),
    ]);
