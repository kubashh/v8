...
// Flags: --experimental-wasm-gc --experimental-wasm-typed-funcref --experimental-wasm-type-reflection
...
async function instantiateWasm() {
  ...
  let imported_ref_table = builder.addImportedTable('import', 'any_table', 3, 3, kWasmAnyRef);
  let imported_func_table = builder.addImportedTable('import', 'func_table', 3, 3, kWasmFuncRef);
  ...
  let imports = `{'import' : {
      'any_table': (() => {
        let js_table = new WebAssembly.Table({element: 'anyref', initial: 3, maximum: 3});
        js_table.set(0, ['JavaScript', 'value']);
        return js_table;
      })(),
      'func_table': (() => {
        let func_table = new WebAssembly.Table({element: 'anyfunc', initial: 3, maximum: 3});
