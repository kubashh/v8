...
(async function test(){
  ...
  InspectorTest.runAsyncTestSuite([
    ...
    async function testInvalidFrame() {
      ...
      expectedError('setVariableValue with invalid callFrameId', result);
      ...
      expectedError('setVariableValue with invalid scopeNumber', result);
      ...
      expectedError('setVariableValue with invalid variableName', result);
    },
    async function testNewValueErrors() {
      ...
      expectedError('setVariableValue with invalid unserializableValue', result);
      result = await Protocol.Debugger.setVariableValue({ scopeNumber: 0, variableName: 'num', newValue: { objectId: 2000 }, callFrameId });
      expectedError('setVariableValue with invalid objectId', result);
    }
  ]);

  function expectedError(message, resultObj) {
    if (!!resultObj.error) {
      InspectorTest.log(`PASS: ${message}`);
    } else {
      InspectorTest.log(`FAIL: ${message} : ${JSON.stringify(resultObj)}`);
    }
  }
