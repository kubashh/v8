...
Protocol.Profiler.onConsoleProfileFinished(e => {
  ...
  for (let i = 0; i <= function_names.length - expected.length; ++i) {
    if (expected.every((val, idx) => val.includes(function_names[i + idx]))) {
      ...
      const checkUnique = (value, index, self) => self.indexOf(value) === index;
      found_unique_script_ids =
          script_ids.slice(i, i + expected.length).filter(checkUnique).length;
    }
  }
});
...
async function runFibUntilProfileFound() {
  ...
  // Print the number of (unique) script IDs to check that wasm code has a
  // script ID attached.
  InspectorTest.log(
      'Number of unique script IDs in found portion: ' +
      found_unique_script_ids);
}
