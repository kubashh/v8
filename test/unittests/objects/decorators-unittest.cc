...
TEST_F(DecoratorsTest, DecoratorAccessObjectTest) {
  ...
  RunJS("var y = {x: 1};");
  Handle<Object> result = RunJS<Object>("access.get(y)");
...
TEST_F(DecoratorsTest, ClassDecoratorContextObjectTest) {
  ...
  Handle<String> class_string_result = RunJS<String>("(context.kind)");
  Handle<String> name_result = RunJS<String>("(context.name)");
  Handle<JSFunction> add_initializer_result =
...
TEST_F(DecoratorsTest, ClassElementDecoratorContextObjectTest) {
  ...
  Maybe<bool> result = context()->Global()->Set(
      context(), NewString("context"), ToApiHandle<Value>(context_object));
  CHECK(result.FromJust());
  Handle<String> kind_string_result = RunJS<String>("(context.kind)");
  Handle<JSObject> access_result = RunJS<JSObject>("(context.access)");
  Handle<Object> is_static_result = RunJS<Object>("(context.static)");
  Handle<Object> is_private_result = RunJS<Object>("(context.private)");
  Handle<String> name_result = RunJS<String>("(context.name)");
  Handle<JSFunction> add_initializer_result =
      RunJS<JSFunction>("(context.addInitializer)");
