...
TEST(Ieee754, Cos) {
  ...
  EXPECT_THAT(COS_IMPL(kQNaN), IsNaN());
  EXPECT_THAT(COS_IMPL(kSNaN), IsNaN());
  EXPECT_THAT(COS_IMPL(kInfinity), IsNaN());
  EXPECT_THAT(COS_IMPL(-kInfinity), IsNaN());

  // Tests for cos for |x| < pi/4
  EXPECT_EQ(1.0, 1 / COS_IMPL(-0.0));
  EXPECT_EQ(1.0, 1 / COS_IMPL(0.0));
  // cos(x) = 1 for |x| < 2^-27
  EXPECT_EQ(1, COS_IMPL(2.3283064365386963e-10));
  EXPECT_EQ(1, COS_IMPL(-2.3283064365386963e-10));
  // Test KERNELCOS for |x| < 0.3.
  // cos(pi/20) = sqrt(sqrt(2)*sqrt(sqrt(5)+5)+4)/2^(3/2)
  EXPECT_EQ(0.9876883405951378, COS_IMPL(0.15707963267948966));
  // Test KERNELCOS for x ~= 0.78125
  EXPECT_EQ(0.7100335477927638, COS_IMPL(0.7812504768371582));
  EXPECT_EQ(0.7100338835660797, COS_IMPL(0.78125));
  // Test KERNELCOS for |x| > 0.3.
  // cos(pi/8) = sqrt(sqrt(2)+1)/2^(3/4)
  EXPECT_EQ(0.9238795325112867, COS_IMPL(0.39269908169872414));
  // Test KERNELTAN for |x| < 0.67434.
  EXPECT_EQ(0.9238795325112867, COS_IMPL(-0.39269908169872414));

  // Tests for cos.
  EXPECT_EQ(1, COS_IMPL(3.725290298461914e-9));
  // Cover different code paths in KERNELCOS.
  EXPECT_EQ(0.9689124217106447, COS_IMPL(0.25));
  EXPECT_EQ(0.8775825618903728, COS_IMPL(0.5));
  EXPECT_EQ(0.7073882691671998, COS_IMPL(0.785));
  // Test that cos(Math.PI/2) != 0 since Math.PI is not exact.
  EXPECT_EQ(6.123233995736766e-17, COS_IMPL(1.5707963267948966));
  // Test cos for various phases.
  EXPECT_EQ(0.7071067811865474, COS_IMPL(7.0 / 4 * kPI));
  EXPECT_EQ(0.7071067811865477, COS_IMPL(9.0 / 4 * kPI));
  EXPECT_EQ(-0.7071067811865467, COS_IMPL(11.0 / 4 * kPI));
  EXPECT_EQ(-0.7071067811865471, COS_IMPL(13.0 / 4 * kPI));
  EXPECT_EQ(0.9367521275331447, COS_IMPL(1000000.0));
  EXPECT_EQ(-3.435757038074824e-12, COS_IMPL(1048575.0 / 2 * kPI));

  // Test Hayne-Panek reduction.
  EXPECT_EQ(-0.9258790228548379e0, COS_IMPL(kTwo120));
  EXPECT_EQ(-0.9258790228548379e0, COS_IMPL(-kTwo120));
}
...
TEST(Ieee754, Sin) {
  // Test values mentioned in the EcmaScript spec.
  EXPECT_THAT(SIN_IMPL(kQNaN), IsNaN());
  EXPECT_THAT(SIN_IMPL(kSNaN), IsNaN());
  EXPECT_THAT(SIN_IMPL(kInfinity), IsNaN());
  EXPECT_THAT(SIN_IMPL(-kInfinity), IsNaN());

  // Tests for sin for |x| < pi/4
  EXPECT_EQ(-kInfinity, Divide(1.0, SIN_IMPL(-0.0)));
  EXPECT_EQ(kInfinity, Divide(1.0, SIN_IMPL(0.0)));
  // sin(x) = x for x < 2^-27
  EXPECT_EQ(2.3283064365386963e-10, SIN_IMPL(2.3283064365386963e-10));
  EXPECT_EQ(-2.3283064365386963e-10, SIN_IMPL(-2.3283064365386963e-10));
  // sin(pi/8) = sqrt(sqrt(2)-1)/2^(3/4)
  EXPECT_EQ(0.3826834323650898, SIN_IMPL(0.39269908169872414));
  EXPECT_EQ(-0.3826834323650898, SIN_IMPL(-0.39269908169872414));

  // Tests for sin.
  EXPECT_EQ(0.479425538604203, SIN_IMPL(0.5));
  EXPECT_EQ(-0.479425538604203, SIN_IMPL(-0.5));
  EXPECT_EQ(1, SIN_IMPL(kPI / 2.0));
  EXPECT_EQ(-1, SIN_IMPL(-kPI / 2.0));
  // Test that sin(Math.PI) != 0 since Math.PI is not exact.
  EXPECT_EQ(1.2246467991473532e-16, SIN_IMPL(kPI));
  EXPECT_EQ(-7.047032979958965e-14, SIN_IMPL(2200.0 * kPI));
  // Test sin for various phases.
  EXPECT_EQ(-0.7071067811865477, SIN_IMPL(7.0 / 4.0 * kPI));
  EXPECT_EQ(0.7071067811865474, SIN_IMPL(9.0 / 4.0 * kPI));
  EXPECT_EQ(0.7071067811865483, SIN_IMPL(11.0 / 4.0 * kPI));
  EXPECT_EQ(-0.7071067811865479, SIN_IMPL(13.0 / 4.0 * kPI));
  EXPECT_EQ(-3.2103381051568376e-11, SIN_IMPL(1048576.0 / 4 * kPI));

  // Test Hayne-Panek reduction.
  EXPECT_EQ(0.377820109360752e0, SIN_IMPL(kTwo120));
  EXPECT_EQ(-0.377820109360752e0, SIN_IMPL(-kTwo120));
}
