...
std::vector<size_t> Sample(size_t n, size_t max) {
  RandomNumberGenerator* gen = new base::RandomNumberGenerator(123);
  std::vector<size_t> result = base::RandomSample(gen, max, n);
  delete gen;
  return result;
}
...
TEST(RandomSample, LessThanHalfLimit) {
  size_t n = 10;
  std::vector<size_t> sample = Sample(n, 100);
...
TEST(RandomSample, CheckOutput) {
  size_t n = 4;
  std::vector<size_t> sample = Sample(n, 10);
