...
TEST(VirtualAddressSpaceTest, TestPagePermissions) {
  EXPECT_LE(PagePermissions::kNoAccess, PagePermissions::kReadWriteExecute);
  EXPECT_LE(PagePermissions::kReadExecute, PagePermissions::kReadWriteExecute);
  EXPECT_LE(PagePermissions::kReadWrite, PagePermissions::kReadWriteExecute);
  EXPECT_LE(PagePermissions::kRead, PagePermissions::kReadWriteExecute);

  EXPECT_LE(PagePermissions::kNoAccess, PagePermissions::kReadWrite);
  EXPECT_LE(PagePermissions::kRead, PagePermissions::kReadWrite);

  EXPECT_LE(PagePermissions::kNoAccess, PagePermissions::kReadExecute);
  EXPECT_LE(PagePermissions::kRead, PagePermissions::kReadExecute);

  EXPECT_FALSE(PagePermissions::kReadExecute <= PagePermissions::kRead);
  EXPECT_FALSE(PagePermissions::kReadWrite <= PagePermissions::kRead);
  EXPECT_FALSE(PagePermissions::kReadExecute <= PagePermissions::kReadWrite);
  EXPECT_FALSE(PagePermissions::kReadWrite <= PagePermissions::kReadExecute);

  EXPECT_GE(PagePermissions::kReadWriteExecute, PagePermissions::kReadWrite);
  EXPECT_GE(PagePermissions::kReadWrite, PagePermissions::kRead);
  EXPECT_GE(PagePermissions::kRead, PagePermissions::kNoAccess);

  EXPECT_FALSE(PagePermissions::kRead >= PagePermissions::kReadWrite);
  EXPECT_FALSE(PagePermissions::kReadWrite >= PagePermissions::kReadExecute);
  EXPECT_FALSE(PagePermissions::kReadExecute >= PagePermissions::kReadWrite);
}
...
TEST(VirtualAddressSpaceTest, TestEmulatedSubspace) {
  ...
  EXPECT_EQ(kSubspaceSize, subspace.size());
  EXPECT_EQ(rootspace.max_page_permissions(),
            subsubspace.max_page_permissions());
