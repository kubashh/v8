...
TEST_F(FunctionBodyDecoderTest, NonDefaultableLocal) {
  WASM_FEATURE_SCOPE(typed_funcref);
  AddLocals(ValueType::Ref(HeapType::kExtern), 1);
...
TEST_F(FunctionBodyDecoderTest, RefAsNonNull) {
  ...
  uint32_t heap_types[] = {struct_type_index, array_type_index,
                           HeapType::kFunc,   HeapType::kEq,
                           HeapType::kExtern, HeapType::kI31};
...
TEST_F(FunctionBodyDecoderTest, RefNull) {
  ...
  uint32_t type_reprs[] = {
      ...
      HeapType::kExtern, HeapType::kI31,   HeapType::kNone};
