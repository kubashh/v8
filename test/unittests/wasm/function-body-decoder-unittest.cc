...
TEST_F(FunctionBodyDecoderTest, RefTestCast) {
  ...
  // Mismached object heap immediate.
  {
    ...
    ExpectFailure(&sig,
                  {WASM_REF_TEST(kLocalEqRef, array_heap, WASM_GET_LOCAL(0),
                                 WASM_RTT_CANON(kLocalI31Ref)),
                   kExprDrop},
                  kAppendEnd,
                  "ref.test: expected rtt for type 0 but got (rtt 1 i31)");
    ExpectFailure(&sig,
                  {WASM_REF_CAST(kLocalEqRef, array_heap, WASM_GET_LOCAL(0),
                                 WASM_RTT_CANON(kLocalI31Ref)),
                   kExprDrop},
                  kAppendEnd,
                  "ref.cast: expected rtt for type 0 but got (rtt 1 i31)");
  }
}
