...
class MockStreamingProcessor : public StreamingProcessor {
 public:
  ...
  bool StartCodeSection(size_t num_functions, uint32_t offset) override {
    return true;
  }
  ...
  void Error(DecodeResult result) override { ok_ = false; }

  // Finish the processing of the stream.
  void FinishStream(std::unique_ptr<uint8_t[]> bytes, size_t length) override {
