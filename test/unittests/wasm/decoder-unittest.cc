...
#define CHECK_UINT32V_INLINE(expected, expected_length, ...)                   \
  do {                                                                         \
    ...
    EXPECT_EQ(static_cast<uint32_t>(expected),                                 \
              decoder.read_u32v<Decoder::kChecked>(decoder.start(), &length)); \
...
#define CHECK_INT32V_INLINE(expected, expected_length, ...)                    \
  do {                                                                         \
    ...
    unsigned length;                                                           \
    EXPECT_EQ(expected,                                                        \
              decoder.read_i32v<Decoder::kChecked>(decoder.start(), &length)); \
    EXPECT_EQ(static_cast<unsigned>(expected_length), length);                 \
    EXPECT_EQ(data, decoder.pc());                                             \
    EXPECT_TRUE(decoder.ok());                                                 \
...
#define CHECK_UINT64V_INLINE(expected, expected_length, ...)                   \
  do {                                                                         \
    ...
    EXPECT_EQ(static_cast<uint64_t>(expected),                                 \
              decoder.read_u64v<Decoder::kChecked>(decoder.start(), &length)); \
    EXPECT_EQ(static_cast<unsigned>(expected_length), length);                 \
  } while (false)
...
#define CHECK_INT64V_INLINE(expected, expected_length, ...)                    \
  do {                                                                         \
    ...
    unsigned length;                                                           \
    EXPECT_EQ(expected,                                                        \
              decoder.read_i64v<Decoder::kChecked>(decoder.start(), &length)); \
    EXPECT_EQ(static_cast<unsigned>(expected_length), length);                 \
  } while (false)
  ...
  for (size_t v = 0; v < arraysize(kVals); v++) {
    ...
    for (int i = 1; i <= 32; i++) {
      ...
      for (unsigned limit = 0; limit <= kMaxSize; limit++) {
        ...
        unsigned rlen;
        uint32_t result = decoder.read_u32v<Decoder::kChecked>(data, &rlen);
        if (limit < length) {
          EXPECT_FALSE(decoder.ok());
        } else {
          EXPECT_TRUE(decoder.ok());
...
TEST_F(DecoderTest, ReadU64v_PowerOf2) {
  ...
  for (unsigned i = 0; i < 64; i++) {
    ...
    for (unsigned limit = 0; limit <= kMaxSize; limit++) {
      decoder.Reset(data, data + limit);
      unsigned length;
      uint64_t result = decoder.read_u64v<Decoder::kChecked>(data, &length);
      if (limit <= index) {
        EXPECT_FALSE(decoder.ok());
      } else {
        EXPECT_TRUE(decoder.ok());
  ...
  for (size_t v = 0; v < arraysize(kVals); v++) {
    ...
    for (int i = 1; i <= 64; i++) {
      ...
      for (unsigned limit = 0; limit <= kMaxSize; limit++) {
        decoder.Reset(data, data + limit);
        unsigned rlen;
        uint64_t result = decoder.read_u64v<Decoder::kChecked>(data, &rlen);
        if (limit < length) {
          EXPECT_FALSE(decoder.ok());
        } else {
          EXPECT_TRUE(decoder.ok());
  ...
  for (size_t v = 0; v < arraysize(kVals); v++) {
    ...
    for (int i = 1; i <= 64; i++) {
      ...
      for (unsigned limit = 0; limit <= kMaxSize; limit++) {
        decoder.Reset(data, data + limit);
        unsigned rlen;
        int64_t result = decoder.read_i64v<Decoder::kChecked>(data, &rlen);
        if (limit < length) {
          EXPECT_FALSE(decoder.ok());
        } else {
          EXPECT_TRUE(decoder.ok());
