...
class ExceptionHandlerFallbackTest : public ::testing::Test {
 protected:
  ...
  void WriteToTestMemory(int value) {
    *reinterpret_cast<int*>(g_start_address) = value;
  }

  int ReadFromTestMemory() { return *reinterpret_cast<int*>(g_start_address); }
 ...
 private:
  static LONG WINAPI TestHandler(EXCEPTION_POINTERS* exception) {
    ...
    v8::PageAllocator* page_allocator = i::GetPlatformPageAllocator();
    // Make the allocated memory accessible so that from now on memory access do
    // not cause an exception anymore.
