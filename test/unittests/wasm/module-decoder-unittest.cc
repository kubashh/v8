...
TEST_F(WasmModuleVerifyTest, OneI32Exception) {
  ...
  EXPECT_EQ(wasm::kWasmI32, e0.sig->GetParam(0));
}
...
TEST_F(WasmModuleVerifyTest, TwoExceptions) {
  ...
  EXPECT_EQ(wasm::kWasmF32, e0.sig->GetParam(0));
  EXPECT_EQ(wasm::kWasmI64, e0.sig->GetParam(1));
  const WasmException& e1 = result.val->exceptions.back();
  EXPECT_EQ(wasm::kWasmI32, e1.sig->GetParam(0));
}
...
TEST_F(WasmSignatureDecodeTest, Fail_anyref_without_flag) {
  ...
  byte ref_types[] = {kLocalAnyFunc, kLocalAnyRef, kLocalEqRef};
  for (byte kInvalidType : ref_types) {
