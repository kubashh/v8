...
TEST_F(Memory64DecodingTest, MemoryLimitLEB64) {
  ...
  ModuleResult result = DecodeModule(
      {SECTION(Memory, ENTRY_COUNT(1), kNoMaximumFlag, U32V_2(5))});
  EXPECT_OK(result);
  ...
  result = DecodeModule({SECTION(Memory, ENTRY_COUNT(1), kHasMaximumFlag,
                                 U32V_2(7), U32V_2(47))});
  EXPECT_OK(result);
  ...
  result = DecodeModule(
      {SECTION(Memory, ENTRY_COUNT(1), kNoMaximumFlag, U64V_10(2))});
  EXPECT_OK(result);
