...
TEST_F(MicrotaskQueueTest, PromiseHandlerContext) {
  ...
  // Create a JSFunction on |context2|
  {
    ...
    EXPECT_EQ(*context2, *JSReceiver::GetContextForTaskCancellation(handler));
  }
  ...
  // Create a JSProxy on |context3|.
  {
    ...
    EXPECT_EQ(*context3, *JSReceiver::GetContextForTaskCancellation(proxy));
  }
  ...
  // Note that its CreationContext and ContextForTaskCancellation is |context2|.
  {
    ...
    EXPECT_EQ(*context2, *JSReceiver::GetContextForTaskCancellation(bound));
  }
