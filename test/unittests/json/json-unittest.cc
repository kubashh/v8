...
static fuzztest::Domain<Json::Value> LeafJson() {
  return fuzztest::OneOf(JustJsonNullPtr(), ArbitraryJson<bool>(),
                         ArbitraryJson<double>(), ArbitraryJson<std::string>());
}

static auto ArbitraryJSON() {
  ...
  builder.Set<Json::Value>("root_json",
                           fuzztest::OneOf(array_domain, object_domain));
  return std::move(builder).Finalize<Json::Value>("root_json");
}
...
void JSONTest::ParseValidJsonP(const std::string& input) {
  ...
  ASSERT_TRUE(v8::JSON::Parse(context_, source).IsEmpty());
