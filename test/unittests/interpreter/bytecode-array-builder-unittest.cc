...
TEST_F(BytecodeArrayBuilderTest, ForwardJumps) {
  ...
  Handle<BytecodeArray> array = builder.ToBytecodeArray(isolate());
  DCHECK_EQ(array->length(), 52 + kFarJumpDistance - 22 + 1);
  ...
  CHECK_EQ(iterator.GetUnsignedImmediateOperand(0), 19);
  iterator.Advance();
  ...
  CHECK_EQ(iterator.GetUnsignedImmediateOperand(0), 13);
  iterator.Advance();
  ...
  CHECK_EQ(*(iterator.GetConstantForIndexOperand(0, isolate())),
           Smi::FromInt(kFarJumpDistance - 3));
  iterator.Advance();
  ...
  CHECK_EQ(*(iterator.GetConstantForIndexOperand(0, isolate())),
           Smi::FromInt(kFarJumpDistance - 9));
  iterator.Advance();
