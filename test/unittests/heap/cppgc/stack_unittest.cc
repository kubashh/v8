...
// Only 64-bit non-Windows support for now.
#if defined(V8_TARGET_ARCH_64_BIT) && !defined(V8_OS_WIN)
...
TEST_F(StackTest, IsOnStackForStackValue) {
  void* dummy;
  EXPECT_TRUE(GetStack()->IsOnStack(&dummy));
}

TEST_F(StackTest, IsOnStackForHeapValue) {
...
TEST_F(StackTest, IterateFindsCaleeSavedRegisters) {
...
// Use a macro loop as asm() can only be passed string literals.
...
#endif  // defined(V8_TARGET_ARCH_64_BIT) && !defined(V8_OS_WIN)
