...
class SuperClass : public GarbageCollected<SuperClass> {
 public:
  ...
  void DoStuff(WorkloadsTest* test, SuperClass* target, PointsBack* points_back,
...
TEST_F(WorkloadsTest, Transition) {
  ...
  super_class->DoStuff(this, super_class.Release(), points_back1.Get(), 2);
  PreciseGC();
  EXPECT_EQ(2, PointsBack::alive_count_);
  ...
  sub_class->DoStuff(this, sub_class.Release(), points_back2.Get(), 1);
  PreciseGC();
  EXPECT_EQ(1, PointsBack::alive_count_);
