...
class ConcurrentMarkingTest : public testing::TestWithHeap {
 public:
  ...
  static constexpr GCConfig ConcurrentPreciseConfig = {
      GCConfig::CollectionType::kMajor, GCConfig::StackState::kNoHeapPointers,
  ...
  bool SingleStep(GCConfig::StackState stack_state) {
...
// static
constexpr GCConfig ConcurrentMarkingTest::ConcurrentPreciseConfig;
...
TEST_F(ConcurrentMarkingTest, MarkingObjects) {
  ...
  for (int i = 0; i < kNumStep; ++i) {
    ...
    SingleStep(GCConfig::StackState::kNoHeapPointers);
  }
  FinishGC();
}
