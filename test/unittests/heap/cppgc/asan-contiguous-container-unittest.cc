...
class AsanContiguousContainerTest : public testing::TestWithPlatform {
 protected:
  ...
  Persistent<ObjectWithAsanAnnotation>& holder() { return holder_; }

  void TearDown() override {
    // Avoid leaking Dummy object in holder_.
    holder_.Clear();
    PreciseGC();
 ...
 private:
  ...
  Persistent<ObjectWithAsanAnnotation> holder_;
};
...
TEST_F(AsanContiguousContainerTest, RegularMarking) {
  holder() = MakeGarbageCollected<ObjectWithAsanAnnotation>(
      GetHeap()->GetAllocationHandle());
  PreciseGC();
}
