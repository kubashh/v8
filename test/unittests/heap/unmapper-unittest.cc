...
class TrackingPageAllocator : public ::v8::PageAllocator {
 public:
  ...
  void* AllocatePages(void* address, size_t size, size_t alignment,
    ...
    if (result) {
      ...
      }
      {
        ...
        Address start = reinterpret_cast<Address>(result);
        os << "=========================================\n"
           << "AllocatePages(), address: " << start << ", size: " << size
           << "\n";
      }
      Print("AllocatePages() #1");
    }
    return result;
  }
  ...
  bool FreePages(void* address, size_t size) override {
    ...
    if (result) {
      ...
      CHECK(IsAligned(size, allocate_page_size_));
      {
        ...
        os << std::hex << std::showbase;
        os << "=========================================\n"
           << "FreePages(), address: " << start << ", size: " << size << "\n";
      }
      Print("FreePages() #1");
      CHECK_EQ(region_allocator_.FreeRegion(start), size);
  ...
  bool ReleasePages(void* address, size_t size, size_t new_size) override {
    ...
    if (result) {
      ...
      CHECK(IsAligned(new_size, commit_page_size_));
      {
        ...
        os << std::hex << std::showbase;
        os << "=========================================\n"
           << "ReleasePages(), address: " << start << ", size: " << size
           << ", new_size: " << new_size << "\n";
      }
      Print("ReleasePages() #1");
  ...
  void Print(const char* comment) const {
    ...
    Address contiguous_region_start = Address{-1};
    Address contiguous_region_end = Address{-1};
...
class SequentialUnmapperTest : public TestWithIsolate {
 public:
  ...
  static void SetUpTestCase() {
    ...
    tracking_page_allocator_->Print("SetUpTestCase");
    TestWithIsolate::SetUpTestCase();
    tracking_page_allocator_->Print("#0");
  }

  static void TearDownTestCase() {
    TestWithIsolate::TearDownTestCase();
    i::FLAG_concurrent_sweeping = old_flag_;
    tracking_page_allocator_->Print("TearDownTestCase");
    CHECK(tracking_page_allocator_->IsEmpty());
...
TEST_F(SequentialUnmapperTest, UnmapOnTeardownAfterAlreadyFreeingPooled) {
  tracking_page_allocator()->Print("#1");
  Page* page = allocator()->AllocatePage(
      MemoryChunkLayout::AllocatableMemoryInDataPage(),
      static_cast<PagedSpace*>(heap()->old_space()),
      Executability::NOT_EXECUTABLE);
  EXPECT_NE(nullptr, page);
  tracking_page_allocator()->Print("#2");
  ...
  tracking_page_allocator()->Print("#3");
  tracking_page_allocator()->CheckPagePermissions(page->address(), page_size,
                                                  PageAllocator::kReadWrite);
  unmapper()->FreeQueuedChunks();
  tracking_page_allocator()->Print("#4");
  tracking_page_allocator()->CheckPagePermissions(page->address(), page_size,
                                                  PageAllocator::kNoAccess);
  unmapper()->TearDown();
  tracking_page_allocator()->Print("#5");
