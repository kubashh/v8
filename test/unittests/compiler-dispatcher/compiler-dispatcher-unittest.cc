...
TEST_F(CompilerDispatcherTest, EnqueueWithoutSFI) {
  ...
  dispatcher.Enqueue(CreateSource(i_isolate(), resource.get()), 0,
                     static_cast<int>(resource->length()), SLOPPY, 1, false,
                     false, false, false, 0, callback.get());
  ASSERT_TRUE(!dispatcher.jobs_.empty());
  ASSERT_TRUE(dispatcher.jobs_.begin()->second->status() ==
              CompileJobStatus::kReadyToParse);
  ASSERT_TRUE(dispatcher.shared_to_job_id_.empty());
...
TEST_F(CompilerDispatcherTest, EnqueueAndStepWithoutSFI) {
  MockPlatform platform;
  CompilerDispatcher dispatcher(i_isolate(), &platform, FLAG_stack_size);
  ASSERT_TRUE(dispatcher.jobs_.empty());
  ASSERT_TRUE(dispatcher.shared_to_job_id_.empty());
  ...
  std::unique_ptr<test::ScriptResource> resource(
      new test::ScriptResource(test_script, strlen(test_script)));
  ASSERT_TRUE(callback->result() == nullptr);
  dispatcher.EnqueueAndStep(CreateSource(i_isolate(), resource.get()), 0,
                            static_cast<int>(resource->length()), SLOPPY, 1,
                            false, false, false, false, 0, callback.get());
  // job should be already finished
  ASSERT_TRUE(dispatcher.jobs_.empty());
  ASSERT_TRUE(dispatcher.shared_to_job_id_.empty());
  ASSERT_TRUE(callback->result() != nullptr);
  ParseInfo* result = callback->result();
  ASSERT_TRUE(result->literal() != nullptr);
}
