...
class SimpleSchedulerTest : public TestWithIsolateAndZone {
 public:
  SimpleSchedulerTest()
...
TEST_F(SimpleSchedulerTest, BuildSimpleScheduleEmpty) {
  ...
  SimpleScheduler* simple_scheduler = new SimpleScheduler(zone(), graph());
  EXPECT_FALSE(simple_scheduler->SameBasicBlock(start, end));
}

TEST_F(SimpleSchedulerTest, BuildSimpleScheduleOneParameter) {
...
TARGET_TEST_F(SimpleSchedulerTest, FloatingDiamond) {
...
TARGET_TEST_F(SimpleSchedulerTest, NestedFloatingDiamonds) {
...
TARGET_TEST_F(SimpleSchedulerTest, LoopedFloatingDiamond) {
  Node* start = graph()->NewNode(common()->Start(2));
  graph()->SetStart(start);
