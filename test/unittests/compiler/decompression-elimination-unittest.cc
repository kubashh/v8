...
// For example, if we are coming from a CheckedCompressedToTaggedPointer, in the
// effect linearization phase we will change that to
// ChangeCompressedPointerToTaggedPointer and we might end up with a chain of
// Parent <- ChangeCompressedPointerToTaggedPointer <- ChangeTaggedToCompressed
// <- Child Similarly, we have cases with Signed instead of pointer. The
// following border case tests will test that the functionality is robust enough
// to handle that.
