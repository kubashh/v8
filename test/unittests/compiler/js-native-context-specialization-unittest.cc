...
TEST_F(JSNativeContextSpecializationTest, GetMaxStringLengthOfString) {
  ...
  Node* const str_node = graph()->NewNode(
      common()->HeapConstant(factory()->InternalizeUtf8String("str")), control);
  EXPECT_EQ(JSNativeContextSpecialization::GetMaxStringLength(str_node),
            str_len);

  Node* const num_node =
      graph()->NewNode(common()->NumberConstant(10.0 / 3), control);
  ...
  Node* const add_node =
      graph()->NewNode(javascript()->Add(hint), str_node, num_node);
  EXPECT_EQ(JSNativeContextSpecialization::GetMaxStringLength(add_node),
            add_len);

  Node* const recursive_node =
      graph()->NewNode(javascript()->Add(hint), str_node, add_node);
