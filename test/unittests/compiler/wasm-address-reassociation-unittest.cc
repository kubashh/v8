...
void TestAllEqual(std::vector<NodeId>& ids, uint64_t expected_length) {
...
#define CHECK_MEM_OP(mem_op)                             \
  Node* add = mem_op->InputAt(0);                        \
  EXPECT_TRUE(IsNativeAdd(add));                         \
  ...
  EXPECT_TRUE(NodeProperties::IsConstant(mem_op->InputAt(1)));
...
TEST_F(WasmAddressReassociationTest, ProtectedBase) {
  ...
  const Operator* protected_store_op =
  ...
  for (unsigned i = 0; i < 3; ++i) {
    ...
    load = graph()->NewNode(protected_load_op, object, reg_offset, effect,
                            control);
    store = graph()->NewNode(protected_store_op, object, reg_offset, load, load,
                             control);
    ar()->VisitProtectedMemOp(load, effect_id);
    ar()->VisitProtectedMemOp(store, effect_id);
    effect = store;
  }
  graph()->end()->InsertInput(zone(), 0, store);
  ar()->Optimize();
...
TEST_F(WasmAddressReassociationTest, ProtectedIndex) {
  ...
  for (unsigned i = 0; i < 3; ++i) {
    Node* index = IntPtrConstant((i + 1) * 8);
    Node* add = Int64Add(reg_offset, index);
    load = graph()->NewNode(protected_load_op, base, add, effect, control);
    store =
        graph()->NewNode(protected_store_op, base, add, load, load, control);
    ar()->VisitProtectedMemOp(load, effect_id);
    ar()->VisitProtectedMemOp(store, effect_id);
    effect = store;
  }
