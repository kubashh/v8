...
class JSMathBuiltinReducerTest : public TypedGraphTest {
 ...
 protected:
  Reduction Reduce(Node* node, Handle<JSFunction> function,
    ...
    JSMathBuiltinReducer reducer(&graph_reducer, &jsgraph, native_context(),
                                 nullptr);
    return reducer.ReduceJSCall(node, function, shared);
  }
...
TEST_F(JSMathBuiltinReducerTest, MathUnaryWithNumber) {
  TRACED_FOREACH(const char*, fnc, kMathUnaries) {
    ...
    Node* call = graph()->NewNode(javascript()->Call(3), jsfunction.node_,
                                  ...
                                  effect, control);
    Reduction r = Reduce(call, jsfunction.function_);
