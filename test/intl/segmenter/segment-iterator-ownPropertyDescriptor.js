...
checkGetterProperty(segmentsPrototype, 'string');
...
for (let func of ["next"]) {
  let descriptor = Object.getOwnPropertyDescriptor(segmentIteratorPrototype, func);
  assertTrue(descriptor.writable);
  assertFalse(descriptor.enumerable);
  assertTrue(descriptor.configurable);
}

assertEquals("Segmenter String Iterator", segmentIteratorPrototype[Symbol.toStringTag]);
