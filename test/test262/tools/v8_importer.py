...
class V8TestImporter(TestImporter):
  ...
  def update_status_file(self, v8_test262_revision, test262_revision,
    ...
    updated_status = DeletedTestsRemover(self,
          self.status_file_content() ,
          v8_test262_revision,
          test262_revision
        ).update()
    updated_status = RenamedTestsUpdater(self,
          updated_status,
          v8_test262_revision,
          test262_revision
        ).update()
...
class StatusFileUpdater:
  def __init__(self, importer, status_file_content, v8_test262_revision, test262_revision):
...
class DeletedTestsRemover(StatusFileUpdater):
  def collect_updateable_tests(self):
    lines = self.importer.test262_git.run([
        ...
        self.v8_test262_revision, self.test262_revision, '--', 'test'
    ]).splitlines()
    self.to_update = [
