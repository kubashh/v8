...
class TestTransitionsAccessor : public TransitionsAccessor {
 public:
  TestTransitionsAccessor(Map* map) : TransitionsAccessor(map) {}
  TestTransitionsAccessor(Handle<Map> map) : TransitionsAccessor(map) {}

  // Expose internals for tests.
  void Reload() { TransitionsAccessor::Reload(); }
...
TEST(TransitionArray_SameFieldNamesDifferentAttributes) {
  ...
  for (int i = 0; i < ATTRS_COUNT; i++) {
    ...
    CHECK_EQ(*attr_maps[i],
             transitions.SearchTransition(*name, kData, attr));
  }
