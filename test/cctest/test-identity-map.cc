...
TEST(Delete_smi_resizes) {
  ...
  t.map.Set(t.smi(kKeyCount), reinterpret_cast<void*>(kKeyCount));
  ...
  for (size_t i = 1; i < 1024; i++) {
    t.map.Set(t.smi(i), reinterpret_cast<void*>(i));
  }
  ...
  // Delete keys.
  for (size_t i = kKeyCount; i > 0; i--) {
    t.CheckDelete(t.smi(i), reinterpret_cast<void*>(i));
  }
