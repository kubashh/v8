...
static void* fake_stack_base = nullptr;
...
TEST(Unwind_BuiltinPCAtStart_Success_CodePagesAPI) {
  ...
  size_t pages_length =
      isolate->CopyCodePages(arraysize(code_pages), code_pages);
  CHECK_LE(pages_length + 1, arraysize(code_pages));
  RegisterState register_state;

  const size_t code_length = 40;
  uintptr_t code[code_length] = {0};
  code_pages[pages_length].start = code;
  code_pages[pages_length].length_in_bytes = code_length * sizeof(uintptr_t);
  pages_length++;
...
void TestRangeBoundaries(size_t pages_length, MemoryRange* code_pages,
  ...
  for (size_t i = 0; i < pages_length; i++) {
    printf("code page: %p, len = %zu\n", code_pages[i].start,
           code_pages[i].length_in_bytes);
  }
  printf("Range start: %p, len = %zu\n", range_start, range_length);
