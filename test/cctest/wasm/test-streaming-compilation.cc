...
class StreamTester {
 public:
  StreamTester() : zone_(&allocator_, "StreamTester") {
    ...
    i::Handle<i::JSPromise> i_promise = Utils::OpenHandle(*promise_);

    stream_ = i_isolate->wasm_compilation_manager()->StartStreamingCompilation(
        i_isolate, Utils::OpenHandle(*context), i_promise);
  }

  std::shared_ptr<StreamingDecoder> stream() { return stream_; }

  // Run all compiler tasks until both the foreground and background task queue
  // are empty.
