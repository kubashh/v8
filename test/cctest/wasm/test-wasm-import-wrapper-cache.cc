...
TEST(CacheHit) {
      ...
      GetTypeCanonicalizer()->LookupFunctionSignature(canonical_type_index);
  {
    ...
    WasmCode* c1 = CompileImportWrapperForTest(
        module.get(), isolate->counters(), kind, canonical_sig,
        canonical_type_index, expected_arity, kNoSuspend);
...
TEST(CacheMissSig) {
  ...
  WasmCode* c1 = CompileImportWrapperForTest(
      module.get(), isolate->counters(), kind, canonical_sig1,
      canonical_type_index1, expected_arity1, kNoSuspend);
...
TEST(CacheMissKind) {
  ...
  WasmCode* c1 = CompileImportWrapperForTest(
      module.get(), isolate->counters(), kind1, canonical_sig,
      canonical_type_index, expected_arity, kNoSuspend);
...
TEST(CacheHitMissSig) {
  ...
  c2 = CompileImportWrapperForTest(module.get(), isolate->counters(), kind,
                                   canonical_sig2, canonical_type_index2,
