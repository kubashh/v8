...
class WasmRunner : public WasmRunnerBase {
 public:
  ...
  ReturnType Call(ParamTypes... p) {
    ...
    Handle<Object> result = retval.ToHandleChecked();
    if (IsSmi(*result)) return Smi::ToInt(*result);
    if (IsHeapNumber(*result)) return HeapNumber::cast(*result)->value();
    if constexpr (std::is_integral_v<ReturnType> &&
                  sizeof(ReturnType) == sizeof(int64_t)) {
      if (IsBigInt(*result)) return BigInt::cast(*result).AsInt64();
    }
    Print(*result);
    FATAL("Unexpected result type (see output above)");
  }
