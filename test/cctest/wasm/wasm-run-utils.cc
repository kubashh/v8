...
byte* TestingModuleBuilder::AddMemory(uint32_t size) {
  ...
  WasmCompiledModule::recreate_embedded_context(
      compiled_module, factory, reinterpret_cast<size_t>(context_address_));
  ...
  context_address_ = reinterpret_cast<byte*>(instance_object_->wasm_context());
...
Handle<JSFunction> TestingModuleBuilder::WrapCode(uint32_t index) {
  ...
  Handle<Code> ret_code = compiler::CompileJSToWasmWrapper(
      isolate_, &test_module_, code, index, context_address_);
