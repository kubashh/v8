...
WASM_SIMD_TEST_NO_LOWERING(ZHIN) {
  ...
  printf("x: %" PRIu64 " actual: %a expected: %a \n", x, actual,
         static_cast<double>(0x8000000000000401));
  CHECK_EQ(actual, static_cast<double>(0x8000000000000401));
}
...
WASM_SIMD_TEST_NO_LOWERING(F64x2ConvertI64x2) {
  ...
  FOR_INT64_INPUTS(x) {
    ...
    for (int i = 0; i < 2; i++) {
      printf("i: %d x: %" PRIu64 " expected_unsigned: %a read: %a\n", i, x,
             expected_unsigned, ReadLittleEndianValue<double>(&g1[i]));
