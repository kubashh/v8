...
TEST(ZHIN) {
  double x = 0x8000000000000401;
  double y = static_cast<double>(static_cast<uint64_t>(x));
  CHECK_EQ(x, y);
}
...
WASM_SIMD_TEST_NO_LOWERING(F64x2ConvertI64x2) {
  ...
  FOR_INT64_INPUTS(x) {
    ...
    for (int i = 0; i < 2; i++) {
      CheckDoubleResult(x, x, expected_signed,
                        ReadLittleEndianValue<double>(&g0[i]), true);
      CheckDoubleResult(x, x, expected_unsigned,
                        ReadLittleEndianValue<double>(&g1[i]), true);
    }
  }
}
