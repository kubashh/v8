...
void RunF32x4BinOpTest(ExecutionTier execution_tier, LowerSimd lower_simd,
  ...
  if (!stress_nans) return;

  // Test some values not included in the float inputs from value_helper. These
  // tests are useful for binary opcodes that are synthesized for code gen, like
  // Min and Max on Intel hardware.
  static constexpr uint32_t test_array[] = {
  ...
  for (size_t i = 0; i < arraysize(test_array); ++i) {
    float x = bit_cast<float>(test_array[i]);
    if (!PlatformCanRepresent(x)) continue;
    for (size_t j = 0; j < arraysize(test_array); ++j) {
      float y = bit_cast<float>(test_array[j]);
      if (!PlatformCanRepresent(y)) continue;
      float expected = expected_op(x, y);
      if (!PlatformCanRepresent(expected)) continue;
      r.Call(x, y);
