...
bool IsSameNan(float x, float y) {
  uint32_t x_bits = bit_cast<uint32_t>(x) & ~0x80000000;
  uint32_t y_bits = bit_cast<uint32_t>(y) & ~0x80000000;
  return x_bits == y_bits;
}

bool IsCanonicalNaN(float x) {
  uint32_t x_bits = bit_cast<uint32_t>(x);
  return (x_bits & ~0xffc00000) == 0;
}
...
void CheckFloatResult(float x, float y, float expected, float actual,
  ...
  if (std::isnan(expected)) {
    CHECK(std::isnan(actual));
    CHECK(IsSameNan(x, actual) || IsSameNan(y, actual) ||
          IsCanonicalNaN(actual));
  } else {
    if (exact) {
...
void RunF32x4UnOpTest(ExecutionTier execution_tier, LowerSimd lower_simd,
  ...
  // Don't stress the approximation instructions.
  if (!exact) return;

  FOR_INT32_INPUTS(x_bits) {
    float x = bit_cast<float>(x_bits);
    if (!PlatformCanRepresent(x)) continue;
    float expected = expected_op(x);
    if (!PlatformCanRepresent(expected)) continue;
    r.Call(x);
...
void RunF32x4BinOpTest(ExecutionTier execution_tier, LowerSimd lower_simd,
  ...
  FOR_FLOAT32_INPUTS(x) {
    ...
    FOR_FLOAT32_INPUTS(y) {
      ...
      for (int i = 0; i < 4; i++) {
        float actual = ReadLittleEndianValue<float>(&g[i]);
        printf("x=%e,y=%e,expected=%x,actual=%x\n", x, y,
               bit_cast<uint32_t>(expected), bit_cast<uint32_t>(actual));
        CheckFloatResult(x, y, expected, actual, true /* exact */);
      }
    }
  }
  // Try integer values, which contain some unusual NaN values.
  FOR_INT32_INPUTS(x_bits) {
    float x = bit_cast<float>(x_bits);
    if (!PlatformCanRepresent(x)) continue;
    FOR_INT32_INPUTS(y_bits) {
      float y = bit_cast<float>(y_bits);
      if (!PlatformCanRepresent(y)) continue;
      float expected = expected_op(x, y);
      if (!PlatformCanRepresent(expected)) continue;
      r.Call(x, y);
