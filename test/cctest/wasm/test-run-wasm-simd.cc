...
void BuildSimdOp(std::vector<byte>* buffer, SimdOp simd_op, int depth) {
  v8::base::RandomNumberGenerator* rng = CcTest::random_number_generator();
  size_t num_operands;
  const size_t kMaxOperands = 3;
  SimdType operand_type[kMaxOperands];
  switch (simd_op) {
    case kLoadGlobal0:
    case kLoadGlobal1:
      num_operands = 0;
  ...
  DCHECK_GE(kMaxOperands, num_operands);
  const int kMaxTreeDepth = 16;
  ...
  for (size_t i = 0; i < num_operands; ++i) {
    ...
    int random_index = rng->NextInt(static_cast<int>(num_opcodes));
    BuildSimdOp(buffer, opcodes[random_index], depth + 1);
  }
  ...
  switch (simd_op) {
    case kLoadGlobal0:
    case kLoadGlobal1: {
      byte code[] = {WASM_GET_GLOBAL(simd_op == kLoadGlobal0 ? 0 : 1)};
...
WASM_SIMD_COMPILED_TEST(SimdFuzz) {
  // TODO(wasm) Some programs hit asserts in simd-scalar-lowering.cc.
  if (lower_simd) return;
