...
TEST(WrapperBudget) {
  {
    ...
    Handle<WasmExportedFunction> main_export =
        testing::GetExportedFunction(isolate, instance, "main")
            .ToHandleChecked();
    WasmExportedFunctionData main_function_data =
        main_export->shared().wasm_exported_function_data();
...
TEST(WrapperReplacement) {
  {
    ...
    const int remaining_budget =
        std::min(static_cast<int>(kGenericWrapperBudget), 10);
    ...
    for (int i = remaining_budget; i > 1; --i) {
      // Call the function.
      ...
      CHECK(IsGeneric(main_function_data.wrapper_code()));
    ...
    // wrapper replacement.
    Code wrapper_before_call = main_function_data.wrapper_code();

    // Call the exported Wasm function one more time to kick off the wrapper
    // replacement.
    Handle<Object> params[1] = {SmiHandle(isolate, 42)};
    SmiCall(isolate, main_export, 1, params, 42);
