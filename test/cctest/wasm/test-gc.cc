...
#define TYPE_CHECK_NULL(type) \
  ...
  byte kDataCheckNull = TYPE_CHECK_NULL(DATA) byte kFuncCheckNull =
      TYPE_CHECK_NULL(FUNC) byte kI31CheckNull = TYPE_CHECK_NULL(I31)
...
#define TYPE_CAST_NULL(type)                                             \
  tester.DefineFunction(tester.sigs.i_v(), {},                           \
          ...
          byte kDataCastNull = TYPE_CAST_NULL(DATA) byte kFuncCastNull =
              TYPE_CAST_NULL(FUNC) byte kI31CastNull = TYPE_CAST_NULL(I31)
#undef TYPE_CHECK_NULL
...
#define TYPE_CHECK(type, value)                              \
  tester.DefineFunction(tester.sigs.i_v(), {kWasmAnyRef},    \
                  ...
                  byte kDataCheckSuccess =
                      TYPE_CHECK(DATA, WASM_ARRAY_NEW_DEFAULT(
                                           array_index, WASM_I32V(10),
                                           WASM_RTT_CANON(array_index)));
