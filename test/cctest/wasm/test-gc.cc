...
void CheckHasThrown(ExecEnv* env, const char* function,
                    std::initializer_list<Object> args) {
  TryCatch try_catch(reinterpret_cast<v8::Isolate*>(isolate(env)));
  MaybeHandle<Object> result = GetJSResult(env, function, args);
  CHECK(result.is_null());
  CHECK(try_catch.HasCaught());
  isolate(env)->clear_pending_exception();
}
...
WASM_EXEC_TEST(BasicStruct) {
  ...
  // need to be changed.
  CHECK(GetJSResult(&env, "h", {}).ToHandleChecked()->IsWasmStruct());
...
WASM_EXEC_TEST(BasicArray) {
  ...
  CheckHasThrown(&env, "f", {Smi::FromInt(3)});
  CheckHasThrown(&env, "f", {Smi::FromInt(-1)});
