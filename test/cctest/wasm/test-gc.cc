...
WASM_COMPILED_EXEC_TEST(BrOnCast) {
  ...
  const byte kTestStruct = tester.DefineFunction(
      ...
      {WASM_BLOCK_R(ValueType::Ref(type_index, kNullable),
                    ...
                    WASM_BR_ON_CAST(0, WASM_RTT_CANON(type_index)), WASM_DROP,
                    ...
                    WASM_BR_ON_CAST(0, WASM_RTT_CANON(type_index)),
...
#define TYPE_CHECK_NULL(type) \
  tester.DefineFunction(      \
      tester.sigs.i_v(), {},  \
      {WASM_REF_IS_##type(WASM_REF_NULL(kAnyRefCode)), kExprEnd})

  byte kDataCheckNull = TYPE_CHECK_NULL(DATA);
  byte kFuncCheckNull = TYPE_CHECK_NULL(FUNC);
  byte kI31CheckNull = TYPE_CHECK_NULL(I31);
#undef TYPE_CHECK_NULL

#define TYPE_CAST_NULL(type)                                             \
  tester.DefineFunction(tester.sigs.i_v(), {},                           \
                        {WASM_REF_AS_##type(WASM_REF_NULL(kAnyRefCode)), \
                         WASM_DROP, WASM_I32V(1), kExprEnd})

  byte kDataCastNull = TYPE_CAST_NULL(DATA);
  byte kFuncCastNull = TYPE_CAST_NULL(FUNC);
  byte kI31CastNull = TYPE_CAST_NULL(I31);
#undef TYPE_CHECK_NULL
...
#define BR_ON(TYPE, type, value)                                               \
  tester.DefineFunction(                                                       \
      ...
      {WASM_LOCAL_SET(0, WASM_SEQ(value)),                                     \
       WASM_BLOCK_R(kWasm##type##Ref, WASM_BR_ON_##TYPE(0, WASM_LOCAL_GET(0)), \
                    WASM_RETURN(WASM_I32V(0))),                                \
       WASM_GC_OP(kExprRefIs##type), kExprEnd})
