...
COMPILE_TEST(TestEventMetrics) {
  ...
  CHECK_LE(0, recorder->module_decoded_.back().duration_in_us);
  ...
  CHECK_LE(0, recorder->module_compiled_.back().duration_in_us);
  ...
  CHECK_LE(0, recorder->module_instantiated_.back().duration_in_us);
  ...
  CHECK_GE(native_module->committed_code_space(),
           recorder->module_tiered_up_.back().code_size_in_bytes);
  CHECK_LE(0, recorder->module_tiered_up_.back().duration_in_us);
}
