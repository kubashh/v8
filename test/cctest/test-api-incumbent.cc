...
//    Ignition, Sparplug, Maglev, TurboFan.
...
// 5) Each context gets an Api function "f" which is checks current context
...
std::vector<v8::Local<Context>> SetupCrossContextTest(
    v8::Isolate* isolate, IncumbentTestExpectations* expected) {
  const int n = 4;  // Ignition, Sparplug, Maglev, TurboFan.
  ...
  for (int i = 0; i < n; i++) {
    ...
    // i::StdoutStream os;
    // os << "context" << i << ": " << *v8::Utils::OpenHandle(*context) << "\n";
  ...
  Local<External> expected_incumbent_context_ptr =
      v8::External::New(isolate, expected);
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      CHECK_EQ(j, value.As<v8::Integer>()->Value());
    }
    ...
    CompileRun(R"JS(
        ...
        // will be hoisted above and it wouldn't be possible to compute the
        // diff of properties set before and after.
