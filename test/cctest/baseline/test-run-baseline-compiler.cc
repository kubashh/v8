...
template <class... A>
class BaselineCallable {
 public:
  BaselineCallable(Isolate* isolate, Handle<JSFunction> function)
      : isolate_(isolate), function_(function) {}
  virtual ~BaselineCallable() {}
...
class BaselineTester {
 public:
  BaselineTester(Isolate* isolate, const char* script,
                 const char* filter = kFunctionName)
      : isolate_(isolate), script_(script) {
    i::FLAG_always_opt = false;
    i::FLAG_allow_natives_syntax = true;
  }
  virtual ~BaselineTester() {}

  template <class... A>
  BaselineCallable<A...> GetCallable(const char* functionName = kFunctionName) {
    return BaselineCallable<A...>(isolate_, GetFunction(functionName));
  }
 ...
 private:
  ...
  Handle<JSFunction> GetFunction(const char* functionName) {
    ...
    BaselineCompilationInfo compilation_info(&zone, shared, feedback);
    ...
    CHECK(job->ExecuteJob() == CompilationJob::SUCCEEDED);
  ...
  DISALLOW_COPY_AND_ASSIGN(BaselineTester);
};
...
TEST(BaselineBuilderReturnStatements) {
  ...
  for (size_t i = 0; i < arraysize(snippets); i++) {
    ...
    BaselineTester tester(isolate, script.start());
...
TEST(BaselineBuilderPrimitiveExpressions) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderTwoParameterTests) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<1> snippets[] = {
      {"return p1.val;",
       {factory->NewNumberFromInt(10),
        BaselineTester::NewObject("({val : 10})")}},
      {"return p1[\"name\"];",
       {factory->NewStringFromStaticChars("abc"),
        BaselineTester::NewObject("({name : 'abc'})")}},
      {"'use strict'; return p1.val;",
       {factory->NewNumberFromInt(10),
        BaselineTester::NewObject("({val : 10 })")}},
      {"'use strict'; return p1[\"val\"];",
       {factory->NewNumberFromInt(10),
        BaselineTester::NewObject("({val : 10, name : 'abc'})")}},
...
TEST(BaselineBuilderKeyedLoad) {
  ...
  ExpectedSnippet<2> snippets[] = {
      ...
      {"return p1[100];",
       {factory->NewStringFromStaticChars("abc"),
        BaselineTester::NewObject("({100 : 'abc'})"),
      ...
      {"'use strict'; return p1[100];",
       {factory->NewNumberFromInt(10),
        BaselineTester::NewObject("({100 : 10})"),
        factory->NewNumberFromInt(0)}},
...
void TestBaselineBuilderNamedStore(size_t shard) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
SHARD_TEST_BY_2(BaselineBuilderNamedStore)

void TestBaselineBuilderKeyedStore(size_t shard) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
SHARD_TEST_BY_2(BaselineBuilderKeyedStore)

TEST(BaselineBuilderPropertyCall) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<1> snippets[] = {
      {"return p1.func();",
       {factory->NewNumberFromInt(25),
        BaselineTester::NewObject("({func() { return 25; }})")}},
      {"return p1.func('abc');",
       {factory->NewStringFromStaticChars("abc"),
        BaselineTester::NewObject("({func(a) { return a; }})")}},
      {"return p1.func(1, 2, 3, 4, 5, 6, 7, 8);",
       {factory->NewNumberFromInt(36),
        BaselineTester::NewObject(
...
TEST(BaselineBuilderCallNew) {
  ...
  for (size_t i = 0; i < arraysize(snippets); i++) {
    BaselineTester tester(isolate, snippets[i].code_snippet);
    auto callable = tester.GetCallable<>();
    Handle<Object> return_value = callable().ToHandleChecked();
    CHECK(return_value->SameValue(*snippets[i].return_value()));
  }
...
TEST(BaselineBuilderCreateClosure) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderCallRuntime) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<1> snippets[] = {
      ...
      {"function f(arg0) { return %IsArray(arg0) }\nf(undefined)",
       {factory->true_value(), BaselineTester::NewObject("[1, 2, 3]")}},
...
TEST(BaselineBuilderInvokeIntrinsic) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
void TestBaselineBuilderGlobals(size_t shard) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
SHARD_TEST_BY_2(BaselineBuilderGlobals)

TEST(BaselineBuilderToObject) {
  // TODO(mythria): tests for ToObject. Needs ForIn.
}

TEST(BaselineBuilderToName) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderLogicalNot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<1> snippets[] = {
      {"return !p1;",
       {factory->false_value(), BaselineTester::NewObject("({val : 10})")}},
...
TEST(BaselineBuilderCompareTypeOf) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<1> snippets[] = {
      ...
      {"return typeof p1 === 'object';",
       {factory->true_value(), BaselineTester::NewObject("({val : 10})")}},
...
TEST(BaselineBuilderCountOperation) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderDelete) {
  ...
  ExpectedSnippet<1> snippets[] = {
      ...
      {"delete p1.val; return p1.val;",
       {factory->undefined_value(), BaselineTester::NewObject("({val : 10})")}},
...
TEST(BaselineBuilderDeleteGlobal) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderDeleteLookupSlot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  for (size_t i = 0; i < arraysize(snippets); i++) {
    ...
    BaselineTester tester(isolate, script.start(), "t");
    auto callable = tester.GetCallable<>();
    Handle<Object> return_value = callable().ToHandleChecked();
    CHECK(return_value->SameValue(*snippets[i].return_value()));
  }
...
TEST(BaselineBuilderLookupSlot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderLookupContextSlot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderLookupGlobalSlot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderLookupSlotWide) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderCallLookupSlot) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderEvalParams) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderEvalGlobal) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
  ...
  ExpectedSnippet<2> snippets[] = {
      {"return p2 in p1;",
       {factory->true_value(), BaselineTester::NewObject("({val : 10})"),
        factory->NewStringFromStaticChars("val")}},
      {"return p2 in p1;",
       {factory->true_value(), BaselineTester::NewObject("[]"),
      ...
      {"return p2 in p1;",
       {factory->false_value(), BaselineTester::NewObject("({val : 10})"),
        factory->NewNumberFromInt(10)}},
      {"return p2 in p1;",
       {factory->true_value(), BaselineTester::NewObject("({10 : 'val'})"),
        factory->NewNumberFromInt(10)}},
      {"return p2 in p1;",
       {factory->false_value(), BaselineTester::NewObject("({10 : 'val'})"),
...
TEST(BaselineBuilderTestInstanceOf) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderTryFinally1) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderTryFinally2) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderContext) {
  ...
  for (size_t i = 0; i < arraysize(snippets); i++) {
    ...
    BaselineTester tester(isolate, script.start(), "f");
...
TEST(BaselineBuilderLoadContext) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderCreateArgumentsNoParameters) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderCreateArguments) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderCreateRestArguments) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderRegExpLiterals) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderArrayLiterals) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderObjectLiterals) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderConditionalOperator) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderSwitchMerge) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderNestedSwitch) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderBreakableBlocks) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
void TestJumpWithConstantsAndWideConstants(size_t shard) {
  ...
  for (int constants = start; constants < 300; constants += kStep) {
    ...
    BaselineTester tester(isolate, script.c_str());
    auto callable = tester.GetCallable<Handle<Object>>();
    for (int a = 0; a < 3; a++) {
...
TEST(BaselineBuilderDoExpressions) {
...
TEST(BaselineBuilderWithStatement) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderConstDeclaration) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderConstDeclarationLookupSlots) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
  Factory* factory = isolate->factory();
...
TEST(BaselineBuilderConstInLookupContextChain) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
...
TEST(BaselineBuilderIllegalConstDeclaration) {
  HandleAndZoneScope scope;
  Isolate* isolate = scope.main_isolate();
