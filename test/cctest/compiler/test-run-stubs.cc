...
class StubTester {
 public:
  StubTester(Isolate* isolate, Zone* zone, CodeStub* stub)
      : zone_(zone),
        ...
        graph_(zone_),
        common_(zone_),
        tester_(Initialize(stub), interface_descriptor_.GetParameterCount()) {}

  Handle<Context> GetContext() {
    return Handle<Context>(tester_.function->context());
  }
 ...
 private:
  Graph* Initialize(CodeStub* stub) {
    int parameter_count =
    ...
    std::unique_ptr<Node* []> node_array(new Node*[node_count]);
