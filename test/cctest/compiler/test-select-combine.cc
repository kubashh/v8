...
// (return (select (compare bin_res, zero, cond),
//                 tval, fval))
//
// kTwoUsesOneBlock:
// (bin_res = binop lhs, rhs)
// (return (add (select (compare bin_res, zero, cond), tval, fval)))
//
// kTwoUsesTwoBlocks:
...
template <typename BinopTy, typename ResTy>
class ConditionalSelectGen {
 public:
  ConditionalSelectGen(BufferedRawMachineAssemblerTester<ResTy>& m,
  ...
  ResTy expected(BinopTy lhs, BinopTy rhs, ResTy tval, ResTy fval) {
    BinopTy bin_node_res = binw().eval(lhs, rhs);
    ResTy res =
    ...
    if (config() != GraphConfig::kOneUse) {
      res += static_cast<ResTy>(bin_node_res);
    }
    return res;
  }

  BufferedRawMachineAssemblerTester<ResTy>& m() { return m_; }
  GraphConfig config() const { return config_; }
  const IntBinopWrapper<BinopTy>& binw() const { return binw_; }
 ...
 private:
  BufferedRawMachineAssemblerTester<ResTy>& m_;
  GraphConfig config_;
  CompareWrapper cmpw_;
  IntBinopWrapper<BinopTy> binw_;
  Node* bin_node_;
  Node* select_;
...
class Int32ConditionalSelectGen : public ConditionalSelectGen<int32_t, ResTy> {
 public:
  using ConditionalSelectGen<int32_t, ResTy>::ConditionalSelectGen;

  Node* AddBinopUse() override {
    BufferedRawMachineAssemblerTester<ResTy>& m = this->m();
...
template <typename ResTy>
class Int64ConditionalSelectGen : public ConditionalSelectGen<int64_t, ResTy> {
 public:
  using ConditionalSelectGen<int64_t, ResTy>::ConditionalSelectGen;
