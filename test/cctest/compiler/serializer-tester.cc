...
SerializerTester::SerializerTester(const char* source)
  ...
  function =
      Handle<JSFunction>(broker_->FindCanonicalPersistentHandle(*function));
...
void CheckForSerializedInlinee(const char* source, int argc = 0,
  ...
  Handle<SharedFunctionInfo> sfi(
      tester.broker()->FindCanonicalPersistentHandle(g_func->shared()));
  SharedFunctionInfoRef g_sfi(tester.broker(), sfi);
  Handle<FeedbackVector> fv(tester.broker()->FindCanonicalPersistentHandle(
