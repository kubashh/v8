...
void GetCanonicalOperands(const InstructionOperand& op,
  ...
  CHECK(kSimpleFPAliasing == COMBINE);
 ...
 public:
  void ExecuteInParallel(const ParallelMove* moves) {
    ...
    for (const auto m : *moves) {
      ...
      if (kSimpleFPAliasing == COMBINE && src.IsFPLocationOperand() &&
  ...
  static Key KeyFor(const InstructionOperand& op) {
    ...
    if (!is_constant) {
      ...
      if (IsFloatingPoint(loc_op.representation())) {
        rep = kSimpleFPAliasing != COMBINE ? MachineRepresentation::kFloat64
...
TEST(Aliasing) {
  ...
  if (kSimpleFPAliasing != COMBINE) return;
