...
void GetCanonicalOperands(const InstructionOperand& op,
  ...
  CHECK_EQ(kFPAliasing, AliasingKind::COMBINE);
 ...
 public:
  void ExecuteInParallel(const ParallelMove* moves) {
    ...
    for (const auto m : *moves) {
      ...
      if (kFPAliasing == AliasingKind::COMBINE && src.IsFPLocationOperand() &&
  ...
  static Key KeyFor(const InstructionOperand& op) {
    ...
    if (!is_constant) {
      ...
      if (IsFloatingPoint(loc_op.representation())) {
        ...
        } else if (kFPAliasing == AliasingKind::OVERLAP) {
...
TEST(Aliasing) {
  ...
  if (kFPAliasing != AliasingKind::COMBINE) return;
