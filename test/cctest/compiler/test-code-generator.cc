...
class CodeGeneratorTester {
 public:
  explicit CodeGeneratorTester(TestEnvironment* environment,
    ...
    // slots, seperated by `extra_stack_space` bytes. This allows us to test
    // negative, positive, far and near ranges for moves.
    for (int i = 0; i < (environment->stack_slot_count() / 2);) {
      ...
      if (GetSpilledSlot(&old_slot) != spilled_slots_.end()) {
        continue;
      }
      ...
      spilled_slots_.emplace_back(old_slot, new_slot);
      i++;
    }
    ...
    generator_ = new CodeGenerator(
        ...
        kNoSourcePosition, nullptr, nullptr, LoadPoisoning::kDontPoison);
  ...
  GetSpilledSlot(InstructionOperand* op) {
    if (op->IsAnyStackSlot()) {
      LocationOperand slot = LocationOperand::cast(*op);
      return std::find_if(
          spilled_slots_.begin(), spilled_slots_.end(),
          [slot](
              const std::pair<LocationOperand, LocationOperand>& moved_pair) {
            return moved_pair.first.index() == slot.index();
          });
    } else {
      return spilled_slots_.end();
    }
  }

  // If the operand corresponds to a spilled slot, return the spilled slot. Else
  // just pass it through.
  InstructionOperand* MaybeTranslateSlot(InstructionOperand* op) {
    auto it = GetSpilledSlot(op);
    if (it != spilled_slots_.end()) {
      // The second element is the spilled slot associated with op.
      return &it->second;
    } else {
      return op;
    }
  ...
  Handle<Code> FinalizeForExecuting() {
    // Move spill slots back into parameters.
    for (auto move : spilled_slots_) {
 ...
 private:
  ...
  // List of slots to be moved from stack parameters to spill slots.
  std::vector<std::pair<LocationOperand, LocationOperand>> spilled_slots_;
};
...
TEST(FuzzAssembleMove) {
  ...
  // Test small and potentially large ranges separately. Note that the second
  // argument affects how much stack is allocated when running the generated
  // code. This means we have to be careful not to exceed the stack limit, which
  // is lower on Windows.
  for (int extra_space : {0, 1 * KB, 2 * KB}) {
