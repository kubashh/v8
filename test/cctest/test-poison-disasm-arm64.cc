...
TEST(DisasmPoisonPolymorphicLoad) {
  ...
  std::vector<std::string> patterns_array = {
      ...
      "ldr x<<ExpMap:[0-9]+>>, pc",                         // load map const #1
      "cmp <<Map0>>, w<<ExpMap>>",                          // compare maps
      "b.eq",                                               // ? go to the load
      "csel " + kPReg + ", xzr, " + kPReg + ", eq",         // update the poison
      "csdb",                                               // spec. barrier
      "ldur <<Map1:w[0-9]+>>, \\[<<Obj>>, #-1\\]",          // load map
      "ldr x<<ExpMap1:[0-9]+>>, pc",                        // load map const #2
      "cmp <<Map1>>, w<<ExpMap1>>",                         // compare maps
      "b.ne",                                            // deopt if different
      "csel " + kPReg + ", xzr, " + kPReg + ", ne",      // update the poison
      "csdb",                                            // spec. barrier
      "ldur w<<Field:[0-9]+>>, \\[<<Obj>>, #[0-9]+\\]",  // load the field
...
TEST(DisasmPoisonMonomorphicLoadFloat64) {
  ...
  std::vector<std::string> patterns_array = {
      ...
      "csel " + kPReg + ", xzr, " + kPReg + ", ne",        // update the poison
      "csdb",                                              // spec. barrier
      "ldur w<<F1:[0-9]+>>, \\[<<Obj>>, #11\\]",           // load field
      "sbfx <<Temp:x[0-9]+>>, x<<F1>>, #0, #1",            // Decompress ref
      "and <<Temp>>, <<Temp>>, x26",                       // Decompress ref
      "add x<<F1>>, <<Temp>>, w<<F1>>, sxtw",              // Decompress ref
