...
namespace {
// Poison register.
constexpr int kPRegCode = kSpeculationPoisonRegister.code();
...
TEST(DisasmPoisonMonomorphicLoad) {
  ...
  std::vector<std::string> patterns_array = {
      "ldur w<<Map:[0-9]+>>, \\[<<Obj:x[0-9]+>>, #-1\\]",  // load map
      "add x<<Map>>, x[0-9]+, w<<Map>>, sxtw",             // decompress ref
      "ldr <<ExpMap:x[0-9]+>>, pc",                        // load expected map
      "cmp x<<Map>>, <<ExpMap>>",                          // compare maps
      "b.ne",                                              // deopt if different
      "csel " + p_reg + ", xzr, " + p_reg + ", ne",        // update the poison
      "csdb",                                              // spec. barrier
      "ldur w<<Field:[0-9]+>>, \\[<<Obj>>, #[0-9]+\\]",    // load the field
      "and x<<Field>>, x<<Field>>, " + p_reg,              // apply the poison
  };
#else
  std::vector<std::string> patterns_array = {
...
#if defined(V8_COMPRESS_POINTERS)
  std::vector<std::string> patterns_array = {
      "ldur w<<Map0:[0-9]+>>, \\[<<Obj:x[0-9]+>>, #-1\\]",  // load map
      "add x<<Map0>>, x[0-9]+, w<<Map0>>, sxtw",            // decompress ref
      "ldr <<ExpMap:x[0-9]+>>, pc",                         // load map const #1
      "cmp x<<Map0>>, <<ExpMap>>",                          // compare maps
      "b.eq",                                               // ? go to the load
      "csel " + p_reg + ", xzr, " + p_reg + ", eq",         // update the poison
      "csdb",                                               // spec. barrier
      "ldur w<<Map1:[0-9]+>>, \\[<<Obj>>, #-1\\]",          // load map
      "add x<<Map1>>, x[0-9]+, w<<Map1>>, sxtw",            // decompress ref
      "ldr <<ExpMap1:x[0-9]+>>, pc",                        // load map const #2
      "cmp x<<Map1>>, <<ExpMap1>>",                         // compare maps
      "b.ne",                                            // deopt if different
      "csel " + p_reg + ", xzr, " + p_reg + ", ne",      // update the poison
      "csdb",                                            // spec. barrier
      "ldur w<<Field:[0-9]+>>, \\[<<Obj>>, #[0-9]+\\]",  // load the field
      ...
      "and x<<BSt>>, x<<BSt>>, " + p_reg,                // apply the poison
      ...
      "and x<<Prop>>, x<<Prop>>, " + p_reg,              // apply the poison
                                                         // Ldone:
  };
#else
  std::vector<std::string> patterns_array = {
