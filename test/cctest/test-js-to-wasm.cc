...
class FastJSWasmCallTester {
 public:
  ...
  void SetExportedMemory() {
    builder_->AddExport(CStrVector("memory"), kExternalMemory, 0);
  }
  ...
  void CallAndCheckWasmFunctionWithInstanceArg() {
    const std::string exported_function_name = "load_i32";
    const int kOffset = 3;
    const int kValue = 42;
    const v8::Local<v8::Value> arg{v8_int(kOffset)};
    ...
    std::string js_code =
        ...
        "mem[" +
        std::to_string(kOffset) + "] = " + std::to_string(kValue) +
        ";"
        "let " +
        exported_function_name + " = instance.exports." +
        exported_function_name +
...
TEST(TestLoadStore) {
  ...
  tester.CallAndCheckWasmFunctionWithInstanceArg();
}
...
TEST(TestFastJSWasmCall_LazyDeopt_I64Result) {
  ...
  // This test would fail if the result was converted into a HeapNumber
  // through a double, losing precision.
  tester.CallAndCheckWasmFunctionBigInt(
...
TEST(TestFastJSWasmCall_Trap_f32) {
  ...
  tester.CallAndCheckWithTryCatch("load_f32", {v8_bigint(0x7fffffff)});
}
...
TEST(TestFastJSWasmCall_Trap_f64) {
  ...
  tester.CallAndCheckWithTryCatch("load_f64", {v8_bigint(0x7fffffff)});
}
