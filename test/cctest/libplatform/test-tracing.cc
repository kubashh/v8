...
TEST(TracingPerfetto) {
...

  {
    ...
    // base::Semaphore done_semaphore(0);

    // tracing_session_->ReadTrace(
    //     [&json_listener_, &done_semaphore](
    //         perfetto::TracingSession::ReadTraceCallbackArgs args) {
    //       perfetto::protos::TracePacket proto_packet;
    //       bool success = proto_packet.ParseFromArray(
    //           args.data, static_cast<int>(args.size));
    //       CHECK(success);
    //       json_listener_.ProcessPacket(proto_packet);
    //       if (!args.has_more) {
    //         done_semaphore.Signal();
    //       }
    //     });
