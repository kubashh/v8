...
class VeryTracingMuchThread : public base::Thread {
 public:
  VeryTracingMuchThread(
      v8::platform::tracing::TracingController* tracing_controller)
      : base::Thread(base::Thread::Options("TraceThread")),
...
TEST(AddTraceEventMultiThreaded) {
  ...
  VeryTracingMuchThread thread(tracing_controller);
