...
TEST(VectorCallICStateApplyFixedDoubleArray) {
  if (!i::FLAG_use_ic) return;
  if (i::FLAG_always_turbofan) return;
  FLAG_allow_natives_syntax = true;
  ...
  CompileRun(
      ...
      "function foo(maybe_double_array) { return F.apply(null, "
      "maybe_double_array); }"
      "F = Math.min;"
      "lst = [0.5, 1.5, 2.5, 3.5];"
      "foo(lst);");
  ...
  CHECK_EQ(ArgumentsListType::kDoubleArray, nexus.GetArgumentsListType());
  ...
  CHECK_EQ(ArgumentsListType::kOthers, nexus.GetArgumentsListType());
