...
TEST(TransitionLookup) {
  ...
  class TempAssembler : public CodeStubAssembler {
   public:
    ...
    void Generate() {
      ...
      BIND(&if_found);
      GotoIfNot(WordEqual(expected_result, SmiConstant(Smi::FromInt(kFound))),
                &failed);
      Branch(WordEqual(expected_arg, SmiTag(var_transition_index.value())),
             &passed, &failed);

      BIND(&if_not_found);
      Branch(WordEqual(expected_result, SmiConstant(Smi::FromInt(kNotFound))),
             &passed, &failed);
  ...
  for (int i = 0; i < kKeysCount; i++) {
    Handle<Name> name;
    if (i % 10 == 0) {
      name = (i % 3 == 0) ? factory->NewPrivateSymbol() : factory->NewSymbol();
    } else {
      int random_key = rand_gen.NextInt(Smi::kMaxValue);
  ...
  CHECK(transitions->IsSortedNoDuplicates());

  // Ensure we didn't overflow transition array.
