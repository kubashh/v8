...
template <typename T>
void TestEmptyMapLike(Isolate* isolate, Factory* factory, Handle<T> map) {
  Verify(isolate, map);
  CHECK_EQ(0, map->NumberOfElements());
  ...
  Handle<Smi> value1(Smi::FromInt(1), isolate);
  CHECK(!map->HasKey(isolate, key1));
  map = T::Add(isolate, map, key1, value1).ToHandleChecked();
  Verify(isolate, map);
  CHECK_EQ(1, map->NumberOfElements());
  CHECK(map->HasKey(isolate, key1));
  ...
  Handle<String> value = factory->NewStringFromAsciiChecked("foo");
  CHECK(!map->HasKey(isolate, key2));
  map = T::Add(isolate, map, key2, value).ToHandleChecked();
  Verify(isolate, map);
  CHECK_EQ(2, map->NumberOfElements());
  CHECK(map->HasKey(isolate, key1));
  CHECK(map->HasKey(isolate, key2));
...
TEST(ZeroSizeOrderedNameDictionary) {
  ...
  // name dictionaries don't have EnsureGrowable
  // {
  //   Handle<OrderedHashMap> dict =
  //       OrderedHashMap::AllocateEmpty(isolate).ToHandleChecked();

  //   dict = OrderedHashMap::EnsureGrowable(isolate, dict).ToHandleChecked();

  //   CHECK_LT(0, dict->NumberOfBuckets());
  //   CHECK_LT(0, dict->NumberOfElements());
  // }
  {
    Handle<OrderedNameDictionary> dict = empty;
  ...
  // name dictionaries don't have clear
  // {
  //   Handle<OrderedNameDictionary> dict =
  //       empty;

  //   dict = OrderedNameDictionary::Clear(isolate, dict);
  ...
  }
  {
    ...
    OrderedNameDictionary::Delete(isolate, *dict, *key1);
