...
TEST(InterpreterGetBytecodeHandler) {
  ...
  CHECK_EQ(wide_handler.builtin(), Builtin::kWideHandler);

  Code add_handler =
      interpreter->GetBytecodeHandler(Bytecode::kAdd, OperandScale::kSingle);

  CHECK_EQ(add_handler.builtin(), Builtin::kAddHandler);
  ...
  CHECK_EQ(wide_wide_handler.builtin(), Builtin::kIllegalHandler);

  Code add_wide_handler =
      interpreter->GetBytecodeHandler(Bytecode::kAdd, OperandScale::kDouble);

  CHECK_EQ(add_wide_handler.builtin(), Builtin::kAddWideHandler);
}
