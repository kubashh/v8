...
TEST(ModuleInstantiationFailures) {
  ...
  std::pair<int, int> position = module->GetModuleRequestPosition(0);
  CHECK_EQ(0, position.first);
  CHECK_EQ(17, position.second);

  CHECK(v8_str("./bar.js")->StrictEquals(module->GetModuleRequest(1)));
  position = module->GetModuleRequestPosition(1);
  CHECK_EQ(1, position.first);
  CHECK_EQ(7, position.second);
