...
class UnifiedHeapEnvironment final {
 public:
  UnifiedHeapEnvironment()
    ...
    cpp_heap_ = std::make_unique<CppHeap>(CcTest::isolate(), 0);
    CcTest::i_isolate()->heap()->SetEmbedderHeapTracer(&cpp_heap());
  }

  ~UnifiedHeapEnvironment() {
    CcTest::i_isolate()->heap()->SetEmbedderHeapTracer(nullptr);
...
TEST(UnifiedHeapFindsV8ToBlinkReference) {
  UnifiedHeapEnvironment env;
  v8::HandleScope scope(CcTest::isolate());
  v8::Local<v8::Context> context = v8::Context::New(CcTest::isolate());
  v8::Context::Scope context_scope(context);
  auto* wrappable =
      cppgc::MakeGarbageCollected<Wrappable>(env.allocation_handle());
  v8::Local<v8::Object> api_object =
      ConstructTraceableJSApiObject(context, wrappable, wrappable);
  CHECK(!api_object.IsEmpty());
  wrappable = reinterpret_cast<Wrappable*>(
      api_object->GetAlignedPointerFromInternalField(0));
  CHECK_EQ(0u, Wrappable::destructor_callcount);
  CcTest::CollectGarbage(OLD_SPACE);
  CHECK_EQ(0u, Wrappable::destructor_callcount);
  ResetWrappableConnection(api_object);
  CcTest::CollectGarbage(OLD_SPACE);
  CHECK_EQ(1u, Wrappable::destructor_callcount);
}
