...
TEST(PrototypeUsersBasic) {
  ...
  int index = -1;
  {
    ...
    array = PrototypeUsers::Add(array, map, &index);
...
TEST(PrototypeUsersCompacted) {
  ...
  array = PrototypeUsers::Add(array, map_cleared_by_user, &index);
  CHECK_EQ(index, 1);
  Handle<Map> live_map = factory->NewMap(JS_OBJECT_TYPE, JSObject::kHeaderSize);
  array = PrototypeUsers::Add(array, live_map, &index);
  CHECK_EQ(index, 2);
  {
    HandleScope inner_scope(isolate);
    Handle<Map> soon_dead_map =
        factory->NewMap(JS_OBJECT_TYPE, JSObject::kHeaderSize);
    array = PrototypeUsers::Add(array, soon_dead_map, &index);
    CHECK_EQ(index, 3);
  }
