...
class InterruptTest {
 public:
  ...
  static void IterateStack(Isolate* isolate, void* data) {
    ...
    if (!simulator_helper.Init(i_isolate)) return;
    ...
    i::StringStream::ClearMentionedObjectCache(i_isolate);
    i::StringStream accumulator(&allocator);
    int frame_index = 0;
    for (; !it.done(); it.Advance()) {
      // Just access the frame a bit (doesn't matter how).
      it.frame()->Print(&accumulator, i::StackFrame::OVERVIEW, frame_index++);
    }
