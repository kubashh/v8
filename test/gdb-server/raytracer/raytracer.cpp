...
extern "C" {
extern void consoleLog(const char* string);
extern void out_result(uint8_t* offset, int length);
}
...
Vec3f trace(const Vec3f& rayorig, const Vec3f& raydir,
  ...
  // want positive.
  float bias =
      (float)1e-4;  // add some bias to the point from which we will be tracing
  bool inside = false;
...
//[/comment]
void render(const std::vector<Sphere>& spheres, unsigned width,
            unsigned height) {
  ...
  uint8_t* buff = new uint8_t[16 + width * height * 3];
  int pos = 0;
  buff[pos++] = 'P';
  buff[pos++] = '6';
  buff[pos++] = '\n';
  buff[pos++] = '0' + width / 100;
  buff[pos++] = '0' + ((width % 100) / 10);
  buff[pos++] = '0' + (width % 10);
  buff[pos++] = ' ';
  buff[pos++] = '0' + height / 100;
  buff[pos++] = '0' + ((height % 100) / 10);
  buff[pos++] = '0' + (height % 10);
  buff[pos++] = '\n';
  buff[pos++] = '2';
  buff[pos++] = '5';
  buff[pos++] = '5';
  buff[pos++] = '\n';
  for (unsigned i = 0; i < width * height; ++i) {
    buff[pos++] = (uint8_t)(std::min(float(1), image[i].x) * 255);
    buff[pos++] = (uint8_t)(std::min(float(1), image[i].y) * 255);
    buff[pos++] = (uint8_t)(std::min(float(1), image[i].z) * 255);
  }
  buff[pos++] = '\0';
  // consoleLog(buff);
  out_result(buff, pos);
...
//[/comment]
void _raytrace(unsigned width, unsigned height) {
  srand48(13);
  std::vector<Sphere> spheres;
  // position, radius, surface color, reflectivity, transparency, emission color
  spheres.push_back(Sphere(Vec3f(0.0f, -10004, -20), 10000,
                           Vec3f(0.20f, 0.20f, 0.20f), 0, 0.0));
  spheres.push_back(
      Sphere(Vec3f(0.0f, 0, -20), 4, Vec3f(1.00f, 0.32f, 0.36f), 1, 0.5));
  spheres.push_back(
      Sphere(Vec3f(5.0f, -1, -15), 2, Vec3f(0.90f, 0.76f, 0.46f), 1, 0.0));
  spheres.push_back(
      Sphere(Vec3f(5.0f, 0, -25), 3, Vec3f(0.65f, 0.77f, 0.97f), 1, 0.0));
  spheres.push_back(
      Sphere(Vec3f(-5.5f, 0, -15), 3, Vec3f(0.90f, 0.90f, 0.90f), 1, 0.0));
  // light
  spheres.push_back(Sphere(Vec3f(0.0f, 20, -30), 3, Vec3f(0.00f, 0.00f, 0.00f),
                           0, 0.0, Vec3f(3.0f)));
  render(spheres, width, height);
}
...
extern "C" {
void raytrace(unsigned width, unsigned height) { _raytrace(width, height); }
}
