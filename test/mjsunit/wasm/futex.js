...
if (this.Worker) {
  ...
  // This test creates 4 workers that wait on consecutive (8 byte separated to satisfy alignments for all kinds
  // of wait) memory locations to test various wait/wake combinations. For each combination, each thread waits
  // 3 times expecting all 4 threads to be woken with wake(4) in first iteration, all 4 to be woken with wake(5)
  // in second iteration and, 3 and 1 to be woken in third iteration.
  ...
  let workerScript = `onmessage = function(msg) {
    ...
    let i32a = new Int32Array(memory.buffer);
    // indices are right shifted by 2 for Atomics.wait to convert them to index for Int32Array
    let result = Atomics.wait(i32a, 0>>>2, 0);                    // for wasm-wake numWorkers threads
    postMessage(result);
    result = Atomics.wait(i32a, 8>>>2, 0);                        // for wasm-wake numWorkers + 1 threads
    postMessage(result);
    result = Atomics.wait(i32a, 16>>>2, 0);                       // for wasm-wake numWorkers - 1 threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 24, 0, -1);     // for js-wake numWorkers threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 32, 0, -1);     // for js-wake numWorkers + 1 threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 40, 0, -1);     // for js-wake numWorkers - 1 threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 48, 0, -1);     // for wasm-wake numWorkers threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 56, 0, -1);     // for wasm-wake numWorkers + 1 threads
    postMessage(result);
    result = WasmI32AtomicWaitFunction(memory, 0, 64, 0, -1);     // for wasm-wake numWorkers - 1 threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 72, 0, 0, -1);  // for js-wake numWorkers threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 80, 0, 0, -1);  // for js-wake numWorkers + 1 threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 88, 0, 0, -1);  // for js-wake numWorkers - 1 threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 96, 0, 0, -1);  // for wasm-wake numWorkers threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 104, 0, 0, -1); // for wasm-wake numWorkers + 1 threads
    postMessage(result);
    result = WasmI64AtomicWaitFunction(memory, 0, 112, 0, 0, -1); // for wasm-wake numWorkers - 1 threads
    postMessage(result);
  };`;
  ...
  let jsWakeCheck = function(index, num, workers, msg) {
    ...
    if (num >= numWorkers) {
      // if numWorkers or more is passed to wake, numWorkers workers should be woken.
      assertEquals(numWorkers, Atomics.wake(i32a, indexJs, num));
    } else {
      // if num < numWorkers is passed to wake, num workers should be woken. Then the remaining
      // workers are woken for the next part
  ...
  let wasmWakeCheck = function(index, num, workers, msg) {
    ...
    if (num >= numWorkers) {
      // if numWorkers or more is passed to wake, numWorkers workers should be woken.
      assertEquals(numWorkers, WasmAtomicWakeFunction(memory, 0, index, num));
    } else {
      // if num < numWorkers is passed to wake, num workers should be woken. Then the remaining
      // workers are woken for the next part
      assertEquals(num, WasmAtomicWakeFunction(memory, 0, index, num));
      assertEquals(numWorkers-num, WasmAtomicWakeFunction(memory, 0, index, numWorkers));
    }
    for (let id = 0; id < numWorkers; id++) {
      assertEquals(msg, workers[id].getMessage());
    }
