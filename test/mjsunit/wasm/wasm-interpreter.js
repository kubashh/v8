...
(function testRegressADO39209724() {
  print(arguments.callee.name);
  var builder = new WasmModuleBuilder();
...
(function testRegressADO39180511() {
  print(arguments.callee.name);
  var builder = new WasmModuleBuilder();
  ...
  //
  // But the ADO bug 39180511 shows that the logic here was broken in the way
...
(function testRegressADO39395594() {
  print(arguments.callee.name);
  var builder = new WasmModuleBuilder();

  // A Wasm module can have invalid instructions (like the first local.set here)
  // if they are unreachable.As in ADO bug 39395594, we need to make sure that
  // these instructions are handled correctly when they can be merged in a
  // super-instruction, as in the pair (f64.load + local.set) here.
...
// ADO bug 39759467: in case of stack overflow was not handled correctly, the
// call stack was not fully unwound and the execution continued from the next
// instruction of the caller function.
(function testRegressADO39759467() {
  print(arguments.callee.name);
  var builder = new WasmModuleBuilder();
...
// ADO bug 39892888: error decoding i64 atomic instructions.
(function testRegressADO39892888() {
  print(arguments.callee.name);
  var builder = new WasmModuleBuilder();
