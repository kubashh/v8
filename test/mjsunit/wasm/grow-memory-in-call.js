...
(function TestGrowMemoryInFunction() {
  ...
  let kGrowFunction = builder.addFunction("grow", kSig_i_i)
  ...
  var deltaPages = 1;
  assertEquals(initialMemoryPages + deltaPages,
               instance.exports.main(deltaPages));
})();
...
(function TestGrowMemoryAndStoreInFunction() {
  ...
  let builder = new WasmModuleBuilder();
  builder.addMemory(initialMemoryPages, maximumMemoryPages, true);
  let kGrowAndStoreFunction = builder.addFunction("grow_and_store", kSig_v_v)
    .addBody([kExprI32Const, 1,              // always grow memory by 1 page
              kExprGrowMemory, kMemoryZero,  // grow memory
              kExprDrop,                     // drop the result of grow
              kExprI32Const, 0,              // put index on stack
              kExprI32Const, 42,             // put new value on stack
              kExprI32StoreMem, 0, 0])       // store
    .exportFunc()
    .index;
  builder.addFunction("main", kSig_i_i)
    .addBody([kExprI32Const, 0,              // put index on stack
              kExprI32Const, 21,             // put old value on stack
              kExprI32StoreMem, 0, 0,        // store
              kExprCallFunction, kGrowAndStoreFunction,  // call grow_and_store
              kExprI32Const, 0,              // put index on stack
...
(function TestGrowMemoryAndAccessInIndirectCall() {
  ...
  assertTraps(kTrapMemOutOfBounds, () => instance.exports.load(index));
  var deltaPages = 1;
  instance.exports.main(kGrowFunction, deltaPages, index, 1234);
  // The caller should be able to access memory that was grown by the callee.
  assertEquals(1234, instance.exports.load(index));
})();
...
(function TestGrowMemoryAndStoreInIndirectCall() {
  ...
  builder.addFunction("main", kSig_i_i)
    .addBody([kExprI32Const, 0,              // put index on stack
              ...
              kExprI32LoadMem, 0, 0])        // load from grown memory
    .exportFunc();
  builder.appendToTable([kGrowAndStoreFunction]);
  var instance = builder.instantiate();
  // The caller should always read from grown memory.
  assertEquals(42, instance.exports.main(kGrowAndStoreFunction));
})();
