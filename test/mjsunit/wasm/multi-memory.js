...
// Add a {copyN_M} function for copying from memory N to memory M.
function addMemoryCopyFunction(builder, mem_src_index, mem_dst_index) {
  builder.addFunction(`copy${mem_src_index}_${mem_dst_index}`, kSig_v_iii)
      .addBody([
        kExprLocalGet, 0,  // dst
        kExprLocalGet, 1,  // src
        kExprLocalGet, 2,  // size
        kNumericPrefix, kExprMemoryCopy, mem_src_index, mem_dst_index
      ])
      .exportFunc();
}
...
(function testMultiMemoryCopy() {
  ...
  instance.exports.copy0_1(2, 3, 5);  // dst, src, size
  expected_memory1.set([0, 0, 4, 5, 0, 0, 4]);
  assertMemoryEquals(expected_memory0, instance.exports.mem0);
  assertMemoryEquals(expected_memory1, instance.exports.mem1);

  // Copy [3..4] from mem1 to [1..3] in mem0.
  instance.exports.copy1_0(1, 3, 2);  // dst, src, size
  ...
  // First test just at the boundary.
  const mem0_size = kPageSize;
  const mem1_size = 2 * kPageSize;
  for ([method, src_size, dst_size] of [
           [instance.exports.copy0_0, mem0_size, mem0_size],
           [instance.exports.copy0_1, mem1_size, mem0_size],
           [instance.exports.copy1_0, mem0_size, mem1_size],
           [instance.exports.copy1_1, mem1_size, mem1_size]]) {
    print(`- ${method}`);
    // Test at the boundary.
    method(src_size - 1, dst_size - 1, 1);
    // Then test one byte further.
    assertThrows(
        () => method(src_size, dst_size - 1, 1), WebAssembly.RuntimeError,
        'memory access out of bounds');
    assertThrows(
        () => method(src_size - 1, dst_size, 1), WebAssembly.RuntimeError,
        'memory access out of bounds');
    assertThrows(
        () => method(src_size - 1, dst_size - 2, 2), WebAssembly.RuntimeError,
        'memory access out of bounds');
    assertThrows(
        () => method(src_size - 2, dst_size - 1, 2), WebAssembly.RuntimeError,
        'memory access out of bounds');
  }
})();
