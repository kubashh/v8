...
// Flags: --wasm-speculative-inlining

d8.file.execute("test/mjsunit/wasm/wasm-module-builder.js");

let builder = new WasmModuleBuilder();
builder.addMemory(65001, undefined, false, false);

let kSig_w_v = makeSig([], [kWasmStringRef]);
let kSig_w_i = makeSig([kWasmI32], [kWasmStringRef]);
let kSig_v_w = makeSig([kWasmStringRef], []);

let huge = builder.addFunction("huge", kSig_w_v).exportFunc().addBody([
  kExprI32Const, 0,                  // address
  ...wasmI32Const(65000 * 65536),    // bytes
  ...GCInstr(kExprStringNewUtf8), 0  // memory index
]);

let callee = builder.addFunction("callee", kSig_w_i).addBody([
  kExprI32Const, 0,                  // address
  kExprLocalGet, 0,                  // bytes
  ...GCInstr(kExprStringNewUtf8), 0  // memory index
]);

let caller = builder.addFunction("caller", kSig_i_i).exportFunc().addBody([
  kExprTry, kWasmI32,
    kExprLocalGet, 0,
