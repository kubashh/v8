...
(function TestExceptionConstructorWithPayload() {
  print(arguments.callee.name);
  let tag = new WebAssembly.Tag({parameters: ['i32', 'f32', 'i64', 'f64', 'externref']});
  assertThrows(() => new WebAssembly.Exception(tag, [1n, 2, 3n, 4, {}]), TypeError);
...
(function TestGetArg() {
  ...
  TestGetArgHelper(['i64'], [BigInt(2)]);
  TestGetArgHelper(['f32'], [3]);
  TestGetArgHelper(['f64'], [4]);
  TestGetArgHelper(['externref'], [{val: 5}]);
  TestGetArgHelper(['i32', 'i64', 'f32', 'f64', 'externref'], [5, BigInt(6), 7, 8, {val: 9}]);
})();
