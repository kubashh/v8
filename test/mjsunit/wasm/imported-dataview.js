...
function MakeBuilder() {
  ...
  kDataViewGetInt32 = builder.addImport(
    'DataView', 'getInt32Import', kSig_i_rii);
...
function CheckStackTrace(thrower, reference, topmost_wasm_func) {
  ...
  if (!actual_stack[2].startsWith(`    at ${topmost_wasm_func} (wasm://wasm/`)) {
    // Artifically print out the mismatch.
    console.log(`expected starts with:\n` +
    `"    at ${topmost_wasm_func} (wasm://wasm/"` + `\n` +
    `found:\n` +
    actual_stack[2]);
    assertTrue(false);
  }
...
(function TestGetInt32() {
  ...
  // Interpret the littleEndian parameter as false in the following cases:
  let constant = 0x78563412;
  assertEquals(constant, instance.exports.getInt32(dataview, 4, 0));
  assertEquals(constant, instance.exports.getInt32(dataview, 4));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, undefined));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, null));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, 0.0));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, -0.0));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, NaN));
  assertEquals(constant, instance.exports.getInt32(dataview, 4, ""));

  // Incompatible receiver.
  assertThrows(()=>{instance.exports.getInt32("test_string", 0, 1)}, TypeError);
  CheckStackTrace(
  ...
  assertThrows(()=>{instance.exports.getInt32(dataview, 0x7FFF_FFFF, 1)}, RangeError);
  CheckStackTrace(
