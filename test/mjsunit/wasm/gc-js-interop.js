...
for (const wasm_obj of [struct, array]) {
  ...
  repeated(() => assertEquals(null, Object.getPrototypeOf(wasm_obj)));
  ...
  testThrowsRepeated(() => Reflect.get(wasm_obj, "prop"),
                TypeError);
  // FIXME(mliedtke): Same as Object.getPrototypeOf, we might expect a
  // TypeError here.
  repeated(() => assertEquals(null, Reflect.getPrototypeOf(wasm_obj)));
  // FIXME(mliedtke): This should probably throw!
  assertFalse(Reflect.has(wasm_obj, "prop"));
  // FIXME(mliedtke): This shouldn't throw!
  assertThrows(() => assertTrue(Reflect.has({wasm_obj}, wasm_obj)), TypeError);
  ...
  // FIXME(mliedtke): This should throw!
  repeated(() => Reflect.setPrototypeOf(wasm_obj, Object.prototype));
