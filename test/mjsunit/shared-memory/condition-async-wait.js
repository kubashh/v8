...
(function TestAsyncWait() {
  ...
  let locked = Atomics.Mutex.lockAsync(mutex, async function() {
    ...
    // There is only one waiter in this isolate's async locked waiter queue
    // even though the lock has been taked 2 times (once in the lockAsync and
    // once after the wait was notified).
    assertEquals(1, %AtomicsSychronizationNumAsyncLockedWaitersInIsolate());
    notified = true;
  });
  ...
  setTimeout(() => {
    ...
    // The condition variable waiters was added to this isolate's async unlocked
    // waiter queue.
    assertEquals(1, %AtomicsSychronizationNumAsyncUnlockedWaitersInIsolate());
    // The mutex waiter is still in the async locked waiter queue.
    assertEquals(1, %AtomicsSychronizationNumAsyncLockedWaitersInIsolate());
    ...
    setTimeout(() => {
      assertTrue(notified);
      // The waiter was removed from this isolate's async unlocked waiter queue.
      assertEquals(
          0, %AtomicsSychronizationNumAsyncUnlockedWaitersInIsolate());
    }, 0);
  }, 0);
})();
