...
(async function TestAsyncPromiseResolveValueAndChain() {
  ...
  await asyncPromise;
  let asyncThen = asyncPromise.then((result) => {
...
(function TestAsyncLockUnlockedMutex() {
  let mutex = new Atomics.Mutex;
  assertThrows(() => {Atomics.Mutex.lock(mutex, () => {
                 Atomics.Mutex.lockAsync(mutex, function() {});
               })});
})();
