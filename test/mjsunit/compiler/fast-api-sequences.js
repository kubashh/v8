...
const add_all_result_smi = -42 + 45;
const add_all_result_full = -42 + 45 + Number.MIN_SAFE_INTEGER + Number.MAX_SAFE_INTEGER +
  max_safe_float * 0.5 + Math.PI;

function add_all_sequence_smi(should_fallback = false) {
  return fast_c_api.add_all_sequence(should_fallback,
    [-42, 45]);
}

%PrepareFunctionForOptimization(add_all_sequence_smi);
assertEquals(add_all_result_smi, add_all_sequence_smi());
%OptimizeFunctionOnNextCall(add_all_sequence_smi);

function add_all_sequence_full(should_fallback = false) {
  return fast_c_api.add_all_sequence(should_fallback,
...
if (fast_c_api.supports_fp_params) {
  assertEquals(add_all_result_full, add_all_sequence_full());
} else {
  assertEquals(add_all_result_smi, add_all_sequence_full());
}
