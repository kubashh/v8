...
function mapped_args() {
  // TODO(pthier): Analyze why this is not working.
...
assertEquals(run(mapped_args, 1, 2, 3), 3);
//assertEquals(run(mapped_args_dup, 1, 2, 3), [3,1,2,3]);
// TODO(pthier): Path to runtime not working yet.
//assertEquals(run(mapped_args_dup, 1, 2, 3), 3);
...
assertEquals(run(unmapped_args, 1, 2, 3), 3);
...
assertEquals(run(rest_args, 1, 2, 3), 3);
