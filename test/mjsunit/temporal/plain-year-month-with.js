...
let badYearMonth = { with: d1.with }
assertThrows(() => badYearMonth.with(), TypeError,
    "Method Temporal.PlainYearMonth.prototype.with called on incompatible receiver #<Object>");
...
assertThrows(() => d1.with({day: 12}), TypeError, invalidArgument);
assertThrows(() => d1.with({hour: 23}), TypeError, invalidArgument);

function assertPlainYearMonth(yearMonth, year, month) {
  assertEquals(year, yearMonth.year, "year");
  assertEquals(month, yearMonth.month, "month");
}

assertPlainYearMonth(d1.with({year: 1956}), 1956, 10);
assertPlainYearMonth(d1.with({month: 3, monthCode: "M03"}), 2021, 3);
assertPlainYearMonth(d1.with({monthCode: "M05"}), 2021, 5);
assertPlainYearMonth(d1.with({month: 3, year: 1956}), 1956, 3);
