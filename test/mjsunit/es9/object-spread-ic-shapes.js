...
// Test that cloning an object with a single data field and `null` prototype
// (in fast mode) yields the same shape in monomorphic state.
(function() {
  function clone(o) {
    return {...o, __proto__: null};
  }
...
// Test that cloning fast mode objects with `null` prototype yields the same
// shape in polymorphic state.
(function() {
  function clone(o) {
    return {...o, __proto__: null};
  }
  ...
  const o0 = {};
  Object.setPrototypeOf(o0, null)
  assertTrue(%HasFastProperties(o0));
  const o1 = {x: 1};
...
(function() {
  ...
  assertFalse(%HaveSameMap(clone1(new A()), new A()));
  assertFalse(%HaveSameMap(clone2(new A()), new A()));
...
// Test that different CloneObjectICs produce the same shape for compatible
// constructor instances in polymorphic state.
(function() {
  function clone(o) {
    return {...o};
  }
  ...
  for (let i = 0; i < 10; ++i) new B();  // Finish slack tracking

  assertFalse(%HaveSameMap(new A(), new B()));
  assertTrue(%HaveSameMap(clone(new A()), clone(new B())));
  %PrepareFunctionForOptimization(clone);
  assertTrue(%HaveSameMap(clone(new A()), clone(new B())));
  assertTrue(%HaveSameMap(clone(new A()), clone(new B())));
  %OptimizeFunctionOnNextCall(clone);
