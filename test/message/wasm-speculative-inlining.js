...
// Flags: --trace-wasm-inlining --turboshaft-wasm
...
function CallRefTest(callee_count) {
  ...
  for (let i = 0; i < 10; i++) {
    // Call each of the `callee`s except the last once to collect feedback.
  ...
  // This should still return the correct result, but never be inlined.
