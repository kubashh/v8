...
const global = builder.addGlobal(kWasmI32, true, false);
...
// TODO(dlehmann,353475584): The following two testcases we want to support soon
// to reach feature parity with the TurboFan Wasm-in-JS inlining, but it's not
// there yet.
...
// We need an `eq ref` type, but those can't be passed from JS.
// TODO(dlehmann,353475584): Add this test once we have support for creating `eq ref` values.
// addBinaryTestcase('RefEq', makeSig([kWasmExternRef, kWasmExternRef], [kWasmExternRef]), {}, null);
...
for (const [ name, { wasmArguments, jsFunctionSrc } ] of Object.entries(tests)) {
  ...
  assertEquals(%GetOptimizationStatus(jsFunction), V8OptimizationStatus.kIsFunction | V8OptimizationStatus.kIsLazy);

  %PrepareFunctionForOptimization(jsFunction);
  let resultUnopt = jsFunction(...wasmArguments);
  assertEquals(%GetOptimizationStatus(jsFunction), V8OptimizationStatus.kIsFunction | V8OptimizationStatus.kInterpreted);

  %OptimizeFunctionOnNextCall(jsFunction);
  let resultOpt = jsFunction(...wasmArguments);
  assertEquals(%GetOptimizationStatus(jsFunction), V8OptimizationStatus.kIsFunction | V8OptimizationStatus.kOptimized | V8OptimizationStatus.kTurboFanned);
